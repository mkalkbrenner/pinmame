...
/* layer types */
enum { LAYER_UNKNOWN, LAYER_BACKDROP, LAYER_OVERLAY, LAYER_BEZEL, LAYER_MARQUEE, LAYER_PANEL, LAYER_SIDE, LAYER_FLYER };
...
static void render_game_bitmap_underlay_overlay(struct mame_bitmap* bitmap, const rgb_t* palette,
                                                struct mame_display* display);
static void render_ui_overlay(struct mame_bitmap* bitmap, UINT32* dirty, const rgb_t* palette,
                              struct mame_display* display);
static void erase_rect(struct mame_bitmap* bitmap, const struct rectangle* bounds, UINT32 color);
static void alpha_blend_intersecting_rect(struct mame_bitmap* dstbitmap, const struct rectangle* dstbounds,
                                          struct mame_bitmap* srcbitmap, const struct rectangle* srcbounds,
                                          const UINT32* hintlist);
static void add_intersecting_rect(struct mame_bitmap* dstbitmap, const struct rectangle* dstbounds,
                                  struct mame_bitmap* srcbitmap, const struct rectangle* srcbounds);
static void cmy_blend_intersecting_rect(struct mame_bitmap* dstbitmap, struct mame_bitmap* dstyrgbbitmap,
                                        const struct rectangle* dstbounds, struct mame_bitmap* srcbitmap,
                                        struct mame_bitmap* srcyrgbbitmap, const struct rectangle* srcbounds,
                                        UINT8 blendflags);
...
INLINE void
union_rect(struct rectangle* dst, const struct rectangle* src) {
    if (dst->max_x == 0)
        *dst = *src;
    else if (src->max_x != 0) {
...
INLINE UINT32
compute_pre_pixel(UINT8 a, UINT8 r, UINT8 g, UINT8 b) {
...
INLINE UINT32
compute_yrgb_pixel(UINT8 a, UINT8 r, UINT8 g, UINT8 b) {
...
INLINE UINT32
add_and_clamp(UINT32 game, UINT32 underpix) {
...
INLINE UINT32
blend_over(UINT32 game, UINT32 pre, UINT32 yrgb) {
...
int
artwork_create_display(struct osd_create_params* params, UINT32* rgb_components,
                       const struct artwork_callbacks* callbacks) {
    ...
    if (!options.artwork_crop)
        for (piece = artwork_list; piece; piece = piece->next) {
            ...
            if (piece->left < min_x)
                min_x = piece->left;
            if (piece->right > max_x)
                max_x = piece->right;
            if (piece->top < min_y)
                min_y = piece->top;
            if (piece->bottom > max_y)
                max_y = piece->bottom;
        }
...
void
artwork_update_video_and_audio(struct mame_display* display) {
            ...
            if (num_bezels) {
                ...
                for (piece = artwork_list; piece; piece = piece->next)
                    if (piece->layer >= LAYER_BEZEL && piece->intersects_game)
                        alpha_blend_intersecting_rect(final, &gamerect, piece->prebitmap, &piece->bounds,
                                                      piece->scanlinehint);
            }
        }
...
void
artwork_override_screenshot_params(struct mame_bitmap** bitmap, struct rectangle* rect, UINT32* rgb_components) {
...
void
artwork_mark_ui_dirty(int minx, int miny, int maxx, int maxy) {
...
void
artwork_get_screensize(int* width, int* height) {
    if (artwork_list) {
...
void
artwork_set_overlay(const struct overlay_piece* overlist) {
...
void
artwork_show(const char* tag, int show) {
    struct artwork_piece* piece;
    ...
    if (underlay_invalid.max_x != 0) {
        ...
        for (piece = artwork_list; piece; piece = piece->next)
            if (piece->layer == LAYER_BACKDROP && piece->visible && piece->prebitmap)
                alpha_blend_intersecting_rect(underlay, &underlay_invalid, piece->prebitmap, &piece->bounds,
                                              piece->scanlinehint);
    }
    ...
    if (overlay_invalid.max_x != 0) {
        ...
        for (piece = artwork_list; piece; piece = piece->next)
            if (piece->layer == LAYER_OVERLAY && piece->visible && piece->prebitmap)
                cmy_blend_intersecting_rect(overlay, overlay_yrgb, &overlay_invalid, piece->prebitmap,
                                            piece->yrgbbitmap, &piece->bounds, piece->blendflags);
    }
    ...
    if (bezel_invalid.max_x != 0) {
        ...
        for (piece = artwork_list; piece; piece = piece->next)
            if (piece->layer >= LAYER_BEZEL && piece->visible && piece->prebitmap)
                alpha_blend_intersecting_rect(bezel, &bezel_invalid, piece->prebitmap, &piece->bounds,
                                              piece->scanlinehint);
    }
...
static void
erase_rect(struct mame_bitmap* bitmap, const struct rectangle* bounds, UINT32 color) {
    int x, y;
...
static void
alpha_blend_intersecting_rect(struct mame_bitmap* dstbitmap, const struct rectangle* dstbounds,
                              struct mame_bitmap* srcbitmap, const struct rectangle* srcbounds,
                              const UINT32* hintlist) {
...
static void
add_intersecting_rect(struct mame_bitmap* dstbitmap, const struct rectangle* dstbounds, struct mame_bitmap* srcbitmap,
                      const struct rectangle* srcbounds) {
    struct rectangle sect = *srcbounds;
    int x, y, width;
    ...
    for (y = sect.min_y; y <= sect.max_y; y++) {
        UINT32* src =
            (UINT32*)srcbitmap->base + (y - srcbounds->min_y) * srcbitmap->rowpixels + (sect.min_x - srcbounds->min_x);
...
static void
cmy_blend_intersecting_rect(struct mame_bitmap* dstprebitmap, struct mame_bitmap* dstyrgbbitmap,
                            const struct rectangle* dstbounds, struct mame_bitmap* srcprebitmap,
                            struct mame_bitmap* srcyrgbbitmap, const struct rectangle* srcbounds, UINT8 blendflags) {
    struct rectangle sect = *srcbounds;
    int x, y, width;
    ...
    /* loop over rows */
    for (y = sect.min_y; y <= sect.max_y; y++) {
        UINT32* srcpre = (UINT32*)srcprebitmap->base + (y - srcbounds->min_y) * srcprebitmap->rowpixels
                         + (sect.min_x - srcbounds->min_x);
        UINT32* srcyrgb = (UINT32*)srcyrgbbitmap->base + (y - srcbounds->min_y) * srcyrgbbitmap->rowpixels
                          + (sect.min_x - srcbounds->min_x);
...
static void
update_palette_lookup(struct mame_display* display) {
    int i, j;
...
static void
render_game_bitmap(struct mame_bitmap* bitmap, const rgb_t* palette, struct mame_display* display) {
...
static void
render_game_bitmap_underlay(struct mame_bitmap* bitmap, const rgb_t* palette, struct mame_display* display) {
    int srcrowpixels = bitmap->rowpixels;
    int dstrowpixels = final->rowpixels;
    ...
    if (display->changed_flags & VECTOR_PIXELS_CHANGED) {
        ...
        if (bitmap->depth != 32) {
            while (*list != VECTOR_PIXEL_END) {
                ...
                y = VECTOR_PIXEL_Y(coords);
                *list++ = coords + offset;
                PIXEL(x, y, dst, dst, 32) =
                    add_and_clamp(palette[PIXEL(x, y, src, src, 16)], PIXEL(x, y, und, dst, 32));
            }
        }
...
static void
render_game_bitmap_overlay(struct mame_bitmap* bitmap, const rgb_t* palette, struct mame_display* display) {
    int srcrowpixels = bitmap->rowpixels;
    int dstrowpixels = final->rowpixels;
    ...
    overyrgbbase =
        (UINT8*)overlay_yrgb->base + gamerect.min_y * overlay_yrgb->rowbytes + gamerect.min_x * sizeof(UINT32);
    ...
    if (display->changed_flags & VECTOR_PIXELS_CHANGED) {
        ...
        if (bitmap->depth != 32) {
            while (*list != VECTOR_PIXEL_END) {
                ...
                y = VECTOR_PIXEL_Y(coords);
                *list++ = coords + offset;
                PIXEL(x, y, dst, dst, 32) = blend_over(palette[PIXEL(x, y, src, src, 16)], PIXEL(x, y, over, dst, 32),
                                                       PIXEL(x, y, overyrgb, dst, 32));
            }
        }
        ...
        /* 32bpp case */
            ...
            while (*list != VECTOR_PIXEL_END) {
                ...
                *list++ = coords + offset;
                PIXEL(x, y, dst, dst, 32) =
                    blend_over(PIXEL(x, y, src, src, 32), PIXEL(x, y, over, dst, 32), PIXEL(x, y, overyrgb, dst, 32));
            }
        }
    }
...
static void
render_game_bitmap_underlay_overlay(struct mame_bitmap* bitmap, const rgb_t* palette, struct mame_display* display) {
    int srcrowpixels = bitmap->rowpixels;
    int dstrowpixels = final->rowpixels;
    ...
    if (display->changed_flags & VECTOR_PIXELS_CHANGED) {
        ...
        if (bitmap->depth != 32) {
            while (*list != VECTOR_PIXEL_END) {
                ...
                *list++ = coords + offset;
                PIXEL(x, y, dst, dst, 32) =
                    add_and_clamp(blend_over(palette[PIXEL(x, y, src, src, 16)], PIXEL(x, y, over, dst, 32),
                                             PIXEL(x, y, overyrgb, dst, 32)),
                                  PIXEL(x, y, und, dst, 32));
            }
        }
        ...
        /* 32bpp case */
            ...
            while (*list != VECTOR_PIXEL_END) {
                ...
                y = VECTOR_PIXEL_Y(coords);
                *list++ = coords + offset;
                PIXEL(x, y, dst, dst, 32) = add_and_clamp(
                    blend_over(PIXEL(x, y, src, src, 32), PIXEL(x, y, over, dst, 32), PIXEL(x, y, overyrgb, dst, 32)),
                    PIXEL(x, y, und, dst, 32));
            }
        }
        ...
        if (bitmap->depth != 32) {
            for (y = 0; y < height; y++) {
                ...
                for (x = 0; x < width; x++) {
                    ...
                    dst[dstrowpixels] =
                        add_and_clamp(blend_over(val, over[dstrowpixels], overyrgb[dstrowpixels]), und[dstrowpixels]);
                    dst[dstrowpixels + 1] = add_and_clamp(
                        blend_over(val, over[dstrowpixels + 1], overyrgb[dstrowpixels + 1]), und[dstrowpixels + 1]);
                    dst += 2;
                    und += 2;
                    over += 2;
                    overyrgb += 2;
...
static void
render_ui_overlay(struct mame_bitmap* bitmap, UINT32* dirty, const rgb_t* palette, struct mame_display* display) {
    int srcrowpixels = bitmap->rowpixels;
    int dstrowpixels = final->rowpixels;
    void *srcbase, *dstbase;
    int height;
...
mame_file*
artwork_load_artwork_file(const struct GameDriver** driver) {
    char filename[100];
...
static int
artwork_load(const struct GameDriver* driver, int width, int height, const struct artwork_callbacks* callbacks) {
    ...
    for (piece = artwork_list; piece; piece = piece->next) {
        ...
        if (fabsf(piece->left) > 4.0f || fabsf(piece->right) > 4.0f || fabsf(piece->top) > 4.0f
            || fabsf(piece->bottom) > 4.0f) {
...
static int
open_and_read_png(const char* gamename, const char* filename, struct png_info* png) {
    int result;
    mame_file* file;
...
static int
load_bitmap(const char* gamename, struct artwork_piece* piece) {
    struct png_info png;
    UINT8* src;
    int x, y;
    ...
    if (png.color_type == 3) {
        ...
        for (y = 0; y < png.height; y++)
            for (x = 0; x < png.width; x++, src++) {
                ...
                plot_pixel(
                    piece->rawbitmap, x, y,
                    MAKE_ARGB(alpha, png.palette[*src * 3], png.palette[*src * 3 + 1], png.palette[*src * 3 + 2]));
            }
...
static int
load_alpha_bitmap(const char* gamename, struct artwork_piece* piece, const struct png_info* original) {
    struct png_info png;
    UINT8* src;
    int x, y;
    ...
    if (png.color_type == 3) {
        ...
        for (y = 0; y < png.height; y++)
            for (x = 0; x < png.width; x++, src++) {
                ...
                UINT8 alpha = compute_brightness(
                    MAKE_RGB(png.palette[*src * 3], png.palette[*src * 3 + 1], png.palette[*src * 3 + 2]));
...
artwork_prep(void) {
    ...
    for (piece = artwork_list; piece; piece = piece->next) {
        ...
        if (!scale_bitmap(piece, piece->bounds.max_x - piece->bounds.min_x + 1,
                          piece->bounds.max_y - piece->bounds.min_y + 1))
            return 1;
...
static int
scale_bitmap(struct artwork_piece* piece, int newwidth, int newheight) {
...
static void
trim_bitmap(struct artwork_piece* piece) {
    ...
    logerror("Trimming bitmap from (%d,%d)-(%d,%d) to (%d,%d)-(%d,%d)\n", piece->bounds.min_x, piece->bounds.min_y,
             piece->bounds.max_x, piece->bounds.max_y, piece->bounds.min_x + left, piece->bounds.min_y + top,
             piece->bounds.min_x + right, piece->bounds.min_y + bottom);
...
static struct artwork_piece*
create_new_piece(const char* tag) {
...
static int CLIB_DECL
artwork_sort_compare(const void* item1, const void* item2) {
...
static int
generate_rect_piece(struct artwork_piece* piece, const struct overlay_piece* data, int width, int height) {
...
static void
render_disk(struct mame_bitmap* bitmap, int r, UINT32 color) {
...
static int
generate_disk_piece(struct artwork_piece* piece, const struct overlay_piece* data, int width, int height) {
...
static int
generate_overlay(const struct overlay_piece* list, int width, int height) {
    int priority = 0;
...
static int
parse_tag_value(struct artwork_piece* piece, const char* tag, const char* value) {
...
static int
compute_rgb_components(int depth, UINT32 rgb_components[3], UINT32 rgb32_components[3]) {
    UINT32 temp;
    int r, g, b;
...
static void
add_range_to_hint(UINT32* hintbase, int scanline, int startx, int endx) {
