...
enum { TOKEN_COMMA, TOKEN_EQUALS, TOKEN_SYMBOL, TOKEN_LINEBREAK, TOKEN_INVALID = -1 };
...
GetGameNameIndex(const char* name) {
    ...
    driver_index_info = bsearch(&key, sorted_drivers, num_games, sizeof(driver_data_type), DriverDataCompareFunc);
 ...
 ****************************************************************************/
static UINT32
GetNextToken(char** ppszTokenText, UINT64* pdwPosition) {
    ...
    while (1) {
        ...
        if (' ' != bData && '\t' != bData) {
            ...
            if (bData > ' ') {
                ...
                while (bData != ',' && bData != '=' && bData != ' ' && bData != '\t' && bData != '\n' && bData != '\r'
                       && mame_feof(fp) == 0) {
                    ++dwFilePos;
 ...
 ****************************************************************************/
static UINT8
ParseOpen(const char* pszFilename) {
 ...
 ****************************************************************************/
static UINT8
ParseSeek(INT64 offset, int whence) {
...
static int
ci_strcmp(const char* s1, const char* s2) {
    int c1, c2;
 ...
 **************************************************************************/
static int
ci_strncmp(const char* s1, const char* s2, size_t n) {
    while (n) {
        int c1, c2;
 ...
 **************************************************************************/
static int
index_datafile(struct tDatafileIndex** _index) {
 ...
 **************************************************************************/
static int
load_datafile_text(const struct GameDriver* drv, char* buffer, int bufsize, struct tDatafileIndex* idx,
                   const char* tag) {
    int offset = 0;
    int found = 0;
    UINT32 token = TOKEN_SYMBOL;
    ...
    *buffer = '\0';
    ...
    while (TOKEN_INVALID != token) {
        ...
        if (found) {
            ...
            if (TOKEN_SYMBOL == token && DATAFILE_TAG == s[0] && TOKEN_LINEBREAK == prev_token)
                break;
 ...
 **************************************************************************/
int
load_driver_history(const struct GameDriver* drv, char* buffer, int bufsize) {
