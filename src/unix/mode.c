...
struct rc_option mode_opts[] = {
    ...
    {"Video Mode Selection Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"keepaspect", "ka", rc_bool, &normal_use_aspect_ratio, "1", 0, 0, NULL,
     "Try / don't try to keep the aspect ratio of a game when selecting the best videomode"},
    {"displayaspectratio", "dar", rc_float, &display_aspect_ratio, "1.33", 0.75, 1.33, NULL,
     "Set the display aspect ratio of your monitor. This is used for -keepaspect The default = 1.33 (4/3). Use 0.75 "
     "(3/4) for a portrait monitor"},
    {"disablemode", "dm", rc_use_function, NULL, NULL, 0, 0, mode_disable,
     "Don't use mode XRESxYRESxDEPTH this can be used to disable specific video modes which don't work on your system. "
     "The xDEPTH part of the string is optional. This option may be used more then once"},
...
static int
mode_disable(struct rc_option* option, const char* s, int priority) {
    ...
    if (sscanf(s, "%dx%dx%d", &disabled_modes[disabled_modes_count].width, &disabled_modes[disabled_modes_count].height,
...
int
mode_disabled(int width, int height, int depth) {
    int i;
    for (i = 0; i < disabled_modes_count; i++) {
        if (disabled_modes[i].width == width && disabled_modes[i].height == height) {
            switch (disabled_modes[disabled_modes_count].depth) {
                case 0:
                    return TRUE;
                case 256:
...
void
mode_perfect(int* width, int* height) {
    ...
    if (use_aspect_ratio) {
        ...
        if (Machine->drv->video_attributes & VIDEO_TYPE_VECTOR) {
            ...
            pixel_aspect_ratio =
                (visual_width * widthscale) / (yarbsize ? yarbsize : (visual_height * heightscale * aspect_ratio));
        }
...
int
mode_match(int width, int height) {
    int wanted_width, wanted_height;
    mode_perfect(&wanted_width, &wanted_height);

    /* does the game fit at all ? */
    if (width < (visual_width * widthscale) || height < (yarbsize ? yarbsize : (visual_height * heightscale)))
        return 0;

    return (100 * ((float)wanted_width / (abs(width - wanted_width) + wanted_width))
