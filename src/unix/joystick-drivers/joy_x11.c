...
#ifdef X11_JOYSTICK
    {"x11joyname", NULL, rc_string, &x11joyname, X11_JOYNAME, 0, 0, NULL,
...
void
process_x11_joy_event(XEvent* event) {
    ...
    if ((event->type == devicebuttonpress) || (event->type == devicebuttonrelease)) {
        ...
        for (i = 0; i < joy_data[0].num_axis; i++)
            joy_data[0].axis[i].val = joy_data[0].axis[i].center + dbe->axis_data[i];
    }
    if ((event->type == devicemotionnotify)) {
        ...
        for (i = 0; i < joy_data[0].num_axis; i++)
            joy_data[0].axis[i].val = joy_data[0].axis[i].center + dme->axis_data[i];
    }
#endif
}
...
joy_x11_init(void) {
    ...
    for (i = j = 0; i < result; i++, list++)
        if (!strcmp(x11joyname, list->name)) {
            j = 1;
            break;
        }
    if (!j) {
    ...
    for (j = 0; j < list->num_classes; j++) {
        switch (any->class) {
            case ButtonClass:
                ...
                if ((joy_data[0].num_buttons = binfo->num_buttons) >= 2)
                    result |= 0x01;
                fprintf(stderr_file, "%s: %d buttons\n", x11joyname, joy_data[0].num_buttons);
                if (joy_data[0].num_buttons > JOY_BUTTONS)
                    joy_data[0].num_buttons = JOY_BUTTONS;
                break;
            case ValuatorClass:
                vinfo = (XValuatorInfoPtr)any;
                if ((joy_data[0].num_axis = vinfo->num_axes) >= 2)
                    result |= 0x02;
                fprintf(stderr_file, "%s: %d axes\n", x11joyname, joy_data[0].num_axis);
                if (joy_data[0].num_axis > JOY_AXIS)
                    joy_data[0].num_axis = JOY_AXIS;
                for (i = 0; i < joy_data[0].num_axis; i++) {
    ...
    for (j = 0; j < xstate->num_classes; j++) {
        switch (any->class) {
            ...
            case ValuatorClass:
                ...
                for (i = 0; i < joy_data[0].num_axis; i++)
                    joy_data[0].axis[i].val = joy_data[0].axis[i].center + vinfo->valuators[i];
                break;
            case KeyClass: /* no sense to use a extended key device */
            default:
                break; /* unknown class: ignore */
        }
