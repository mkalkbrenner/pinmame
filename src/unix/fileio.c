...
#ifndef MESS
    {"rompath", "rp", rc_string, &pathlist[FILETYPE_ROM].rawpath, XMAMEROOT "/roms", 0, 0, NULL,
     "Search path for rom files"},
#else
    {"biospath", "bp", rc_string, &pathlist[FILETYPE_ROM].rawpath, XMAMEROOT "/bios", 0, 0, NULL,
     "Search path for BIOS sets"},
    {"softwarepath", "swp", rc_string, &pathlist[FILETYPE_IMAGE].rawpath, XMAMEROOT "/software", 0, 0, NULL,
     "Search path for software"},
    {"CRC_directory", "crc", rc_string, &pathlist[FILETYPE_CRC].rawpath, XMAMEROOT "/crc", 0, 0, NULL,
     "Directory containing CRC files"},
#endif
    {"samplepath", "sp", rc_string, &pathlist[FILETYPE_SAMPLE].rawpath, XMAMEROOT "/samples", 0, 0, NULL,
     "Search path for sample files"},
    {"inipath", NULL, rc_string, &pathlist[FILETYPE_INI].rawpath, XMAMEROOT "/ini", 0, 0, NULL,
     "Search path for ini files"},
    {"cfg_directory", NULL, rc_string, &pathlist[FILETYPE_CONFIG].rawpath, "$HOME/." NAME "/cfg", 0, 0, NULL,
     "Directory to save configurations"},
    {"nvram_directory", NULL, rc_string, &pathlist[FILETYPE_NVRAM].rawpath, "$HOME/." NAME "/nvram", 0, 0, NULL,
     "Directory to save nvram contents"},
    {"memcard_directory", NULL, rc_string, &pathlist[FILETYPE_MEMCARD].rawpath, "$HOME/." NAME "/memcard", 0, 0, NULL,
     "Directory to save memory card contents"},
    {"input_directory", NULL, rc_string, &pathlist[FILETYPE_INPUTLOG].rawpath, "$HOME/." NAME "/inp", 0, 0, NULL,
     "Directory to save input device logs"},
    {"hiscore_directory", NULL, rc_string, &pathlist[FILETYPE_HIGHSCORE].rawpath, "$HOME/." NAME "/hi", 0, 0, NULL,
     "Directory to save hiscores"},
    {"state_directory", NULL, rc_string, &pathlist[FILETYPE_STATE].rawpath, "$HOME/." NAME "/sta", 0, 0, NULL,
     "Directory to save states"},
    {"artwork_directory", NULL, rc_string, &pathlist[FILETYPE_ARTWORK].rawpath, XMAMEROOT "/artwork", 0, 0, NULL,
     "Directory for Artwork (Overlays etc.)"},
    {"snapshot_directory", NULL, rc_string, &pathlist[FILETYPE_SCREENSHOT].rawpath, XMAMEROOT "/snap", 0, 0, NULL,
     "Directory for screenshots (.png format)"},
    {"diff_directory", NULL, rc_string, &pathlist[FILETYPE_IMAGE_DIFF].rawpath, "$HOME/." NAME "/diff", 0, 0, NULL,
     "Directory for hard drive image difference files"},
    {"ctrlr_directory", NULL, rc_string, &pathlist[FILETYPE_CTRLR].rawpath, XMAMEROOT "/ctrlr", 0, 0, NULL,
...
static char*
find_reverse_path_sep(char* name) {
...
static void
create_path(char* path, int has_filename) {
...
static const char*
parse_variable(const char** start, const char* end) {
...
static char*
copy_and_expand_variables(const char* path, int len) {
    char *dst, *result;
    const char* src;
    int length = 0;
...
static void
expand_pathlist(struct pathdata* list) {
...
static const char*
get_path_for_filetype(int filetype, int pathindex, int* count) {
...
static void
compose_path(char* output, int pathtype, int pathindex, const char* filename) {
...
int
osd_get_path_count(int pathtype) {
    int count;
...
int
osd_get_path_info(int pathtype, int pathindex, const char* filename) {
    struct stat buf;
    char fullpath[1024];
...
osd_file*
osd_fopen(int pathtype, int pathindex, const char* filename, const char* mode) {
    char fullpath[1024];
    osd_file* file;
    int i;
    int offs;
...
int
osd_fseek(osd_file* file, INT64 offset, int whence) {
...
UINT32
osd_fread(osd_file* file, void* buffer, UINT32 length) {
...
UINT32
osd_fwrite(osd_file* file, const void* buffer, UINT32 length) {
    int result;
...
int
osd_create_directory(int pathtype, int pathindex, const char* dirname) {
    char fullpath[1024];
...
void
build_crc_database_filename(int game_index) {
...
int
osd_select_file(mess_image* img, char* filename) {
    return 0;
}

void
osd_begin_final_unloading(void) {}

void
osd_image_load_status_changed(mess_image* img, int is_final_unload) {}
#endif
