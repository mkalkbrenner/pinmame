...
struct rc_option input_opts[] = {
    ...
    {"joytype", "jt", rc_int, &joytype, "0", 0, 7, NULL,
...
static int joy_list_equiv[][2] = {{JOY_AXIS_CODE(0, 0, 0), JOYCODE_1_LEFT},
...
static void
ugci_callback(int id, enum ugci_event_type type, int value) {
...
osd_input_initpre(void) {
    ...
    for (i = 0; i < JOY; i++) {
        ...
        for (j = 0; j < JOY_AXIS; j++) {
            ...
            for (k = 0; k < JOY_DIRS; k++) {
                snprintf(joy_list_names[joy_list_count], JOY_NAME_LEN, "Joy %d axis %d %s", i + 1, j + 1,
                         (k) ? "pos" : "neg");
                joy_list_count++;
            }
        }
        for (j = 0; j < JOY_BUTTONS; j++) {
            snprintf(joy_list_names[joy_list_count], JOY_NAME_LEN, "Joy %d button %d", i + 1, j + 1);
            joy_list_count++;
        }
    }
    ...
    for (i = 0; i < MOUSE; i++) {
        for (j = 0; j < MOUSE_BUTTONS; j++) {
            snprintf(joy_list_names[joy_list_count], JOY_NAME_LEN, "Mouse %d button %d", i + 1, j + 1);
            joy_list_count++;
        }
    }
...
osd_input_initpost(void) {
    ...
    switch (joytype) {
        ...
        default:
            fprintf(stderr_file,
                    "OSD: Warning: unknown joytype: %d, or joytype not compiled in.\n"
...
int
get_rapidfire_speed(int joy_num, int button_num) {
...
void
set_rapidfire_speed(int joy_num, int button_num, int speed) {
    if (joy_num < 0 || 3 < joy_num)
        return;
    if (button_num < 0 || 9 < button_num)
        return;
...
int
is_rapidfire_ctrl_button(int joy_num, int button_num) {
    if (joy_num < 0 || 3 < joy_num)
        return 0;
    if (button_num < 0 || 9 < button_num)
        return 0;
...
int
no_rapidfire_ctrl_button(int joy_num) {
    if (joy_num < 0 || 3 < joy_num)
        return 0;

    if (rapidfire_data[joy_num].ctrl_button < 0 || rapidfire_data[joy_num].ctrl_button > 9)
        return 1;
    else
        return 0;
}
...
void
set_rapidfire_ctrl_button(int joy_num, int button_num) {
    if (joy_num < 0 || 3 < joy_num)
        return;
    if (button_num < 0 || 9 < button_num)
        return;
...
void
unset_rapidfire_ctrl_button(int joy_num) {
    if (joy_num < 0 || 3 < joy_num)
        return;
...
int
setrapidfire(struct mame_bitmap* bitmap, int selected) {
...
joystick_rapidfire(void) {
    ...
    for (joy_num = 0; joy_num < 4 && joy_data[joy_num].fd >= 0; joy_num++) {
        ...
        if (ctrl >= 0 && ctrl < 10 && ctrl < joy_data[joy_num].num_buttons) {
            if (joy_data[joy_num].buttons[ctrl] && rapidfire_data[joy_num].ctrl_prev_status == 0) {
        ...
        if (rapidfire_data[joy_num].enable) {
            for (button_num = 0; button_num < 10 && button_num < joy_data[joy_num].num_buttons; button_num++) {
                if (rapidfire_data[joy_num].setting[button_num] & 0xff
                    ...
                    if (joy_data[joy_num].buttons[button_num] != 0) {
                        ...
                        rapidfire_data[joy_num].status[button_num] += rapidfire_data[joy_num].setting[button_num];
