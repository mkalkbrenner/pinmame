...
static int
sound_set_options(struct rc_option* option, const char* arg, int priority) {
    if (sound_enabled) {
...
struct rc_option sound_opts[] = {
    ...
    {"fakesound", "fsnd", rc_set_int, &sound_fake, NULL, 1, 0, sound_set_options,
     "Generate sound even when sound is disabled, this is needed for some games which won't run without sound"},
    {"samplefreq", "sf", rc_int, &sound_samplerate, "44100", 8000, 96000, sound_set_options,
...
void
osd_sound_enable(int enable_it) {
    if (sound_stream && enable_it) {
        sound_enabled = 1;
        if (!sound_dsp) {
            if (!(sound_dsp = sysdep_dsp_create(NULL, sound_dsp_device, &options.samplerate, &type,
                ...
                if (!(sound_stream = sound_stream_create(sound_dsp, type, sound_samples_per_frame, 3))) {
...
osd_start_audio_stream(int stereo) {
    ...
    if (sound_enabled) {
        ...
        sound_samples_per_frame = Machine->sample_rate / Machine->drv->frames_per_second;
#ifdef SOUND_DEBUG
    ...
    sound_mixer = sysdep_mixer_create(NULL, sound_mixer_device, SYSDEP_MIXER_RESTORE_SETTINS_ON_EXIT);
...
osd_update_audio_stream(INT16* buffer) {
    ...
    if (sound_enabled) {
        ...
        if (type & SYSDEP_DSP_STEREO)
            bytes =
                lame_encode_buffer_interleaved(fl, buffer, sound_samples_per_frame, audio_outbuf, sizeof(audio_outbuf));
        else
            bytes = lame_encode_buffer(fl, buffer, buffer, sound_samples_per_frame, audio_outbuf, sizeof(audio_outbuf));
    ...
    /* sound enabled ? */
    if (sound_enabled)
        sound_stream_write(sound_stream, (unsigned char*)buffer, sound_samples_per_frame);
