...
static int
barath_skip_this_frame(void) {
    return (modframe >= FRAMESKIP_LEVELS);
}

int
barath_skip_next_frame(void) {
    ...
    if (throttle) {
        ...
        else if (autoframeskip && (frameskip < max_autoframeskip) && (frames_skipped < 1))
...
barath_show_fps(char* buffer) {
    ...
    if (showme++ > 5) {
        ...
        chars_filled =
            sprintf(buffer, "%2d %d %s%s%s%2d %3d%%(%3d/%3d)", debug_value, sysload, throttle ? "T" : "",
                    (throttle && should_sleep_idle()) ? "S" : "", (throttle && autoframeskip) ? "A" : "F", frameskip,
                    (int)(speed * 100 + .5), (int)(fps / slow_speed), (int)(video_fps / slow_speed));
