...
#define ALSA_SCAN_CHECK(f)                                                                                             \
    alsa_stat = f;                                                                                                     \
    if (alsa_stat) {                                                                                                   \
        ALSA_error(alsa_stat);                                                                                         \
        return 0;                                                                                                      \
    }
#define ALSA_INIT_CHECK(f)                                                                                             \
    alsa_stat = f;                                                                                                     \
    if (alsa_stat)                                                                                                     \
...
int
prioritize_level3(int* bits, const int b_flags) {
...
int
prioritize_level2(int* rate, int* bits, const int min, const int max, const int b_flags) {
...
int
prioritize(int* rate, int* bits, int* channels, const int min, const int max, const int b_flags, const int max_chan) {
...
int
insert(choice list[], int* list_max, int card, int pcm, int rate, int bits, int channels, const int min, const int max,
       const int b_flags, const int max_chan) {
    choice temp;
...
scan_cards(choice list[]) {
    ...
    int bits = (sound_8bit) ? SND_PCM_PINFO_8BITONLY : SND_PCM_PINFO_16BITONLY; /* Are 8 or 16 bits desired? */
    ...
    for (cur_card = 0; cur_card < max_cards; cur_card++) {
        ...
        for (cur_pcm = 0; cur_pcm < info.pcmdevs; cur_pcm++) {
            ...
            if (id.flags & SND_PCM_INFO_PLAYBACK) {
                ...
                insert(list, &num_choices, cur_card, cur_pcm, options.samplerate, bits, voices, temp.min_rate,
                       temp.max_rate, bit_mask, temp.max_channels);

                /* Relay information to console */
                fprintf(stderr_file, "%s detected, %d-%dHz, ", id.name, temp.min_rate, temp.max_rate);
                switch (temp.flags) {
                    case SND_PCM_PINFO_8BITONLY:
                        fprintf(stderr_file, "8 bit ");
                        break;
                    case SND_PCM_PINFO_16BITONLY:
                        fprintf(stderr_file, "16 bit ");
                        break;
                    default:
                        fprintf(stderr_file, "8/16 bit ");
                };
...
sysdep_audio_init(void) {
    ...
    if (!sound_8bit)
        corrected_frag_size += corrected_frag_size;
    if (sound_stereo)
        corrected_frag_size += corrected_frag_size;
    ...
    fmt.format = (list[i].bits == SND_PCM_PINFO_8BITONLY) ? SND_PCM_SFMT_U8 : SND_PCM_SFMT_S16_LE;
    ...
    frag_size = (temp.max_fragment_size < corrected_frag_size) ? temp.max_fragment_size : corrected_frag_size;
    /*    frag_size = 2048; */
    ...
    fprintf(stderr_file, "%d fragments of size %d... ", num_frags, frag_size);

    fprintf(stderr_file, "OK.\nSetting sound format... %dHz, %s %s... ", fmt.rate, (sound_8bit) ? "8-bit" : "16-bit",
