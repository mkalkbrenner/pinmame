...
Parserc(FILE* fd) {
    ...
    while (ReadLine(fd, aux) != EOF) {
        ...
        /* CAMBIAR!!! */
        if (strcmp(iden, "mamebin") == 0)
            strcpy(mamebin, value);

        if (strcmp(iden, "srcromsdir") == 0)
            strcpy(srcbasedir, value);
        if (strcmp(iden, "srclistdir") == 0)
            strcpy(srclistdir, value);

        if (strcmp(iden, "tmpdir") == 0)
            strcpy(tmpdir, value);
        if (strcmp(iden, "romsdir") == 0)
            strcpy(romsdir, value);
        if (strcmp(iden, "mamedir") == 0)
            strcpy(mamedir, value);
        if (strcmp(iden, "neogeodir") == 0)
            strcpy(neogeodir, value);
        if (strcmp(iden, "samplesdir") == 0)
            strcpy(samplesdir, value);
        if (strcmp(iden, "artworkdir") == 0)
            strcpy(artworkdir, value);
        if (strcmp(iden, "listdir") == 0)
            strcpy(listdir, value);
        if (strcmp(iden, "imagesdir") == 0)
            strcpy(imagesdir, value);
        if (strcmp(iden, "titlesdir") == 0)
            strcpy(titlesdir, value);
    }
            ...
            if ((strcmp(argv[i], "--size") == 0) || (strcmp(argv[i], "-s") == 0)) {
                ...
                while (argv[i][j] != '\0') {
                    if (isalpha(argv[i][j])) {
                        ...
                        if (argv[i][j] == 'K')
                            multiplier = MULTIPLIERK;
                        if (argv[i][j] == 'M')
                            multiplier = MULTIPLIERM;
                        if (argv[i][j] == 'm')
                            multiplier = MULTIPLIERm;
                        break;
                    } else {
...
main(int argc, char** argv) {
    ...
    if (!generatecds)
        printf("Don't generate ISO CDs\n");
    if (!generatelist)
        printf("Don't generate game list\n");
    if (generatefulllist)
        printf("Create full game list\n");

    printf("Size of CD's     %lu bytes (%lu Kb, %lu Mb, %lu')\n\n", tamanyocd / MULTIPLIERB, tamanyocd / MULTIPLIERK,
           tamanyocd / MULTIPLIERM, tamanyocd / MULTIPLIERm);
