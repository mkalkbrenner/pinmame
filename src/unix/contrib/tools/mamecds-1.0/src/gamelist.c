...
int
cuenta_juegos(struct mamegame* lista, int tipo, int ncd) {
    int i;
    ...
    if (tipo == MAME) {
        ...
        for (i = GAME_1; i <= GAME_Z; i++) {
            if (i == GAME_1)
                printf("Numero de juegos 1-9 \t%i\n", njuegos_letra[i]);
            else
                printf("Numero de juegos %c \t%i\n", i + 0x60, njuegos_letra[i]);
        }
#endif
        return (njuegos_mame);
    }
...
/************************************************************/
int
ComparaJuegos(struct mamegame* game1, struct mamegame* game2) {
...
MakeGameList(char* mame) {
    ...
    fscanf(pmame, "%s %s %s %s %s %s\n", aux1, basura, basura, aux2, aux3,
           aux4); /* xmame (SDL) version 0.37 BETA 13 (Mar 31 2001) */
    ...
    while (1) {
        ...
        if (strcmp(todos[i].base, "Total") == 0)
            break;
...
/************************************************************/
int
PrintGameList(struct mamegame* lista) {
    int i;
    ...
    while (actual != NULL) {
        if (actual->tipo == NEOGEO)
            printf(" Neogeo");
        if (actual->tipo == MAME)
            printf("   Mame");
        printf(" %10i  %02i  %i%i%i%i%i%i%i %8s %8s %5s %s %s\n", actual->tamanyorom, actual->numerocd, actual->hasgame,
               actual->isclone, actual->hasclones, actual->hastitle, actual->hasimage, actual->hassamples,
               actual->hasartwork, actual->base, actual->cloneof, actual->anyo, actual->nombre, actual->fabricante);
