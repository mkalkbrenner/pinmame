...
int
CreateLinks(struct mamegame* juego, int tipo, int ncd) {
    char src[MAXCAD];
    char dst[MAXCAD];
...
int
InsertGame(struct mamegame* lista, struct mamegame* juego, int tipo, int ncd, int tamanyoparcial) {
    int tamanyo, i;
...
int
MakeCD(struct mamegame* lista, int tipo, int ncd) {
    char aux[MAXCAD];
    char src[MAXCAD];
    ...
    if (tipo == MAME) {
        ...
        sprintf(srcroms, "%s/%s", srcbasedir, mamedir); /* /home/carlitos/roms/mame				*/
        sprintf(srcsamples, "%s/%s/%s", srcbasedir, romsdir,
                samplesdir); /* /home/carlitos/roms/samples			*/
        sprintf(srcartwork, "%s/%s/%s", srcbasedir, romsdir,
                artworkdir); /* /home/carlitos/roms/artwork			*/
    }
    ...
    if (generatelist) {
        ...
        mkdir(baseimages, S_IRWXU | S_IRWXG | S_IRWXO);
        ; /* ./tmp/cddir.ncd/lista/images			*/
...
int
CreateCDS(struct mamegame* lista, int tipo) {
    int ncd; /* CD number */
    int ret;
