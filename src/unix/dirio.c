...
#ifdef MESS
int
osd_num_devices(void) {
    return 0; /* unix doesn't have devices ( a device = a: b: etc) */
}

void
osd_change_device(const char* device) {}

const char*
osd_get_device_name(int idx) {
    return "";
}
...
void*
osd_dir_open(const char* dirname, const char* filemask) {
...
#ifndef __QNXNTO__
static int
fnmatch(const char* f1, const char* f2) {
    while (*f1 && *f2) {
        if (*f1 == '*') {
...
int
osd_dir_get_entry(void* dir, char* name, int namelength, int* is_dir) {
    struct osd_dir* my_dir = dir;
...
void
osd_change_directory(const char* directory) {
    chdir(directory);
}
...
char*
osd_dirname(const char* filename) {
    char* dirname;
    char* c;
...
/*============================================================ */
int
osd_is_absolute_path(const char* path) {
    int result;
