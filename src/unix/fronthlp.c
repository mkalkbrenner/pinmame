...
struct rc_option frontend_opts[] = {
    ...
    {"list", "l", rc_set_int, &list, NULL, LIST_LIST, 0, NULL,
    ...
    {"listgamelist", "lgl", rc_set_int, &list, NULL, LIST_GAMELIST, 0, NULL,
...
#ifdef MESS
    {"listdevices", "ldev", rc_set_int, &list, NULL, LIST_DEVICES, 0, NULL,
     "Like -list, with devices and image file extensions supported"},
#endif
    {"listromsize", "lrs", rc_set_int, &list, NULL, LIST_ROMSIZE, 0, NULL,
     "Like -list, with the year and size of the roms used"},
    {"listpalettesize", "lps", rc_set_int, &list, NULL, LIST_ROMSIZE, 0, NULL,
    ...
    {"listsamdir", "lsd", rc_set_int, &list, NULL, LIST_SAMDIR, 0, NULL,
     "Like -list, but lists dir where samples are taken from"},
#endif
    {"verifyroms", "vr", rc_set_int, &list, NULL, VERIFY_ROMS, 0, NULL,
...
#if (HAS_SAMPLES)
    {"verifysamples", "vs", rc_set_int, &list, NULL, VERIFY_SAMPLES, 0, NULL,
     "Like -verifyroms but verify audio samples instead"},
    {"verifysamplesets", "vss", rc_set_int, &list, NULL, VERIFY_SAMPLESETS, 0, NULL,
     "Like -verifysamples, but less verbose"},
#endif
    {"clones", "cl", rc_bool, &showclones, "1", 0, 0, NULL,
     "Show / don't show bootlegs/clones in the above list commands"},
    {"listclones", "lcl", rc_set_int, &list, NULL, LIST_CLONES, 0, NULL,
    ...
    {"listmissingroms", "lmr", rc_set_int, &list, NULL, LIST_MISSINGROMS, 0, NULL,
     "Like -list, but lists ROMs missing"},
#endif
    {"listdupcrc", "ldc", rc_set_int, &list, NULL, LIST_DUPCRC, 0, NULL,
     "Like -list, but lists ROMs with identical crc"},
    {"listwrongorientation", "lwo", rc_set_int, &list, NULL, LIST_WRONGORIENTATION, 0, NULL,
     "Like -list, but lists any games which use the orientation flags wrongly"},
    {"listwrongmerge", "lwm", rc_set_int, &list, NULL, LIST_WRONGMERGE, 0, NULL,
     "Like -list, but lists any games which use the clone_of field wrongly"},
    {"listwrongfps", "lwf", rc_set_int, &list, NULL, LIST_WRONGFPS, 0, NULL,
...
int CLIB_DECL
compare_names(const void* elem1, const void* elem2) {
...
int CLIB_DECL
compare_driver_names(const void* elem1, const void* elem2) {
    struct GameDriver* drv1 = *(struct GameDriver**)elem1;
    struct GameDriver* drv2 = *(struct GameDriver**)elem2;
...
int
strwildcmp(const char* sp1, const char* sp2) {
    char s1[9], s2[9];
    int i, l1, l2;
    char* p;
...
int
frontend_help(const char* gamename) {
    return 0;
}

static void
frontend_verify(int driver, int rom) {
    int status;
    ...
    if (verbose)
        fprintf(stdout_file, "%s %s ", rom ? "romset" : "sampleset", drivers[driver]->name);
    else
...
static int
frontend_uses_roms(int driver) {
    const struct RomModule *region, *rom;
    int total_roms = 0;

    for (region = rom_first_region(drivers[driver]); region; region = rom_next_region(region)) {
        for (rom = rom_first_file(region); rom && (ROM_GETNAME(rom) || ROM_GETOFFSET(rom) || ROM_GETLENGTH(rom));
...
frontend_list(char* gamename) {
    ...
    const char* header[] = {
        ...
        "---- ------------------------------------ --------------------------------\n",
        /* listdetails      */
        " romname driver     cpu 1    cpu 2    cpu 3    cpu 4    cpu 5    cpu 6    cpu 7    cpu 8    sound 1     sound "
        "2     sound 3     sound 4     sound 5     name\n"
        "-------- ---------- -------- -------- -------- -------- -------- -------- -------- -------- ----------- "
        "----------- ----------- ----------- ----------- --------------------------\n",
        /* listgamelist     */ "",
        /* listsourcefile   */
        ...
        "--------  ------\n",
        /* listextensions   */
        "name      device      image file extensions supported\n"
        "--------  ----------  -------------------------------\n",
        /* listromsize      */
        "name    \tyear \tsize\n"
        "--------\t-----\t----\n",
        ...
        "--------  --------  -----------\n",
        /* listdupcrc       */
        "CRC      filename1    name1        filename2    name2\n"
        "-------- ------------ --------     ------------ --------\n",
        /* wrongorientation */ "",
        /* wrongmerge       */ "",
        /* wrongfps         */
    ...
    for (i = 0; drivers[i]; i++) {
        ...
        if ((showclones || drivers[i]->clone_of == 0 || (drivers[i]->clone_of->flags & NOT_A_DRIVER))
            ...
            switch (list) {
                ...
                case LIST_FULL: /* games list with descriptions */
                    fprintf(stdout_file, "%-10s\"%s\"\n", drivers[i]->name, get_description(i));
                    break;
                case LIST_GAMES: {
                ...
                case LIST_SOURCEFILE:
                    ...
                    break;
                case LIST_COLORS:
                    fprintf(stdout_file, "%-8s  %d\n", drivers[i]->name, drv.total_colors);
                    break;
#ifdef MESS
                case LIST_DEVICES: /* list devices */
                    if (device_first(drivers[i])) {
                        ...
                        while (dev) {
                            ...
                            if (!j) /* first time ? */
                                fprintf(stdout_file, "%-10s  ", device_typename(dev->type));
                            else
                                fprintf(stdout_file, "%-8s  %-10s  ", "", device_typename(dev->type));
                ...
                case LIST_MISSINGROMS:
                    if (RomsetMissing(i)) {
                        fprintf(stdout_file, "%-10s%-10s%s\n", drivers[i]->name,
                                (drivers[i]->clone_of) ? drivers[i]->clone_of->name : "", get_description(i));
                        not_found++;
                    }
                    break;
                case LIST_DUPCRC: {
                        ...
                        for (rom = rom_first_file(region); rom; rom = rom_next_file(rom))
                            if (!hash_data_has_info(ROM_GETHASHDATA(rom), HASH_INFO_NO_DUMP))
                                for (j = i + 1; drivers[j]; j++) {
                                    ...
                                    for (region1 = rom_first_region(drivers[j]); region1;
                                         region1 = rom_next_region(region1))
                ...
                case LIST_WRONGORIENTATION: /* list drivers which incorrectly use the orientation and visible area fields */
                    if (!(drv.video_attributes & VIDEO_TYPE_VECTOR)
                        ...
                        strcmp(drivers[i]->name, "crater") && strcmp(drivers[i]->name, "mpatrol")
                        && strcmp(drivers[i]->name, "troangel") && strcmp(drivers[i]->name, "travrusa")
                        && strcmp(drivers[i]->name, "kungfum") && strcmp(drivers[i]->name, "battroad")
                        && strcmp(drivers[i]->name, "vigilant") && strcmp(drivers[i]->name, "sonson")
                        && strcmp(drivers[i]->name, "brkthru") && strcmp(drivers[i]->name, "darwin")
                        && strcmp(drivers[i]->name, "exprraid") && strcmp(drivers[i]->name, "sidetrac")
                        && strcmp(drivers[i]->name, "targ") && strcmp(drivers[i]->name, "spectar")
                        && strcmp(drivers[i]->name, "venture") && strcmp(drivers[i]->name, "mtrap")
                        && strcmp(drivers[i]->name, "pepper2") && strcmp(drivers[i]->name, "hardhat")
                        && strcmp(drivers[i]->name, "fax") && strcmp(drivers[i]->name, "circus")
                        && strcmp(drivers[i]->name, "robotbwl") && strcmp(drivers[i]->name, "crash")
                        && strcmp(drivers[i]->name, "ripcord") && strcmp(drivers[i]->name, "starfire")
                        && strcmp(drivers[i]->name, "fireone") && strcmp(drivers[i]->name, "renegade")
                        && strcmp(drivers[i]->name, "battlane") && strcmp(drivers[i]->name, "megatack")
                        && strcmp(drivers[i]->name, "killcom") && strcmp(drivers[i]->name, "challeng")
                        && strcmp(drivers[i]->name, "kaos") && strcmp(drivers[i]->name, "formatz")
                        && strcmp(drivers[i]->name, "bankp") && strcmp(drivers[i]->name, "liberatr")
                        && strcmp(drivers[i]->name, "toki") && strcmp(drivers[i]->name, "stactics")
                        && strcmp(drivers[i]->name, "sprint1") && strcmp(drivers[i]->name, "sprint2")
                        && strcmp(drivers[i]->name, "nitedrvr") && strcmp(drivers[i]->name, "punchout")
                        && strcmp(drivers[i]->name, "spnchout") && strcmp(drivers[i]->name, "armwrest")
                        && strcmp(drivers[i]->name, "route16") && strcmp(drivers[i]->name, "stratvox")
                        && strcmp(drivers[i]->name, "irobot") && strcmp(drivers[i]->name, "leprechn")
                        && strcmp(drivers[i]->name, "starcrus") && strcmp(drivers[i]->name, "astrof")
                        && strcmp(drivers[i]->name, "tomahawk") && strcmp(drivers[i]->name, "astrocde")
                        && strcmp(drivers[i]->name, "vic20") && strcmp(drivers[i]->name, "vc20")
                        && strcmp(drivers[i]->name, "p2000t") && strcmp(drivers[i]->name, "kim1")) {
                                            ...
                                            for (rom1 = rom_first_file(region1); rom1; rom1 = rom_next_file(rom1)) {
                                                if (!strcmp(ROM_GETNAME(rom), ROM_GETNAME(rom1))) {
                                                    if (!hash_data_has_info(ROM_GETHASHDATA(rom1), HASH_INFO_NO_DUMP)
                                                        && !hash_data_is_equal(ROM_GETHASHDATA(rom),
                                                                               ROM_GETHASHDATA(rom1), 0)) {
                                                        char temp[512];
                ...
                case LIST_WRONGFPS: /* list drivers with too high frame rate */
                    if ((drv.video_attributes & VIDEO_TYPE_VECTOR) == 0
                        && (drivers[i]->clone_of == 0 || (drivers[i]->clone_of->flags & NOT_A_DRIVER))
    ...
    if (skipped)
        fprintf(stdout_file, "Displayed: %d, Skipped: %d, because they don't use any roms/samples/devices\n",
                matching - skipped, skipped);
    if (correct + incorrect)
        fprintf(stdout_file, "Found: %d, of which %d correct and %d incorrect\n", correct + incorrect, correct,
                incorrect);
    if (not_found)
        fprintf(stdout_file, "Not found: %d\n", not_found);
    fflush(stdout_file);
...
static int
frontend_list_clones(char* gamename) {
    /* listclones is a special case since the strwildcmp */
    /* also has to be done on clone_of. */
    int i;
...
static int
frontend_list_gamelistheader(void) {
    fprintf(stdout_file, "This is the complete list of games supported by %s %s.\n", NAME, build_version);
    if (!showclones)
        fprintf(stdout_file, "Variants of the same game are not included, you can use the -listclones command\n"
