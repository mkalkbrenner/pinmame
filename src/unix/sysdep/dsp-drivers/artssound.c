...
struct rc_option arts_dsp_opts[] = {
    ...
    {"artsBufferTime", "abt", rc_int, &btime, "10", 1, 1000, NULL, "aRts buffer delay time"},
...
arts_dsp_create(const void* flags) {
    ...
    arts_init();
    priv->stream = arts_play_stream(dsp->hw_info.samplerate, (dsp->hw_info.type & SYSDEP_DSP_16BIT) ? 16 : 8,
                                    (dsp->hw_info.type & SYSDEP_DSP_STEREO) ? 2 : 1, "xmame arts");
