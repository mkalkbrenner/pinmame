...
/* public methods (static but exported through the sysdep_dsp or plugin
   struct) */
static void*
sol_dsp_create(const void* flags) {
    int i, j;
    ...
    info.play.buffer_size = dsp->hw_info.bufsize =
        priv->buffer_samples * sol_dsp_bytes_per_sample[dsp->hw_info.type]; /* this seems to have no effect */
    if (ioctl(priv->fd, AUDIO_SETINFO, &info) < 0) {
        perror("error: AUDIO_SETINFO\n");
        sol_dsp_destroy(dsp);
        return NULL;
    ...
    fprintf(stderr, "info: audiodevice %s set to %dbit linear %s %dHz\n", dev.name, info.play.precision,
            (info.play.channels & 2) ? "stereo" : "mono", info.play.sample_rate);

    return dsp;
}

static void
sol_dsp_destroy(struct sysdep_dsp_struct* dsp) {
...
static int
sol_dsp_get_freespace(struct sysdep_dsp_struct* dsp) {
    audio_info_t info;
    struct sol_dsp_priv_data* priv = dsp->_priv;
...
static int
sol_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count) {
    int result;
    struct sol_dsp_priv_data* priv = dsp->_priv;
