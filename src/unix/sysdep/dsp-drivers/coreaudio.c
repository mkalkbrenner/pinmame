...
static OSStatus
coreaudio_dsp_play(AudioDeviceID device, const AudioTimeStamp* now_time, const AudioBufferList* in_data,
                   const AudioTimeStamp* in_time, AudioBufferList* out_data, const AudioTimeStamp* out_time,
                   void* data) {
    int i;
    struct coreaudio_private* priv = (struct coreaudio_private*)data;
    for (i = 0; i < out_data->mNumberBuffers; i++)
        dequeue_audio_block(&(priv->queue), out_data->mBuffers[i].mData,
                            out_data->mBuffers[i].mDataByteSize / sizeof(float));
    return noErr;
}
...
static int
coreaudio_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count) {
    struct coreaudio_private* priv = (struct coreaudio_private*)dsp->_priv;
...
coreaudio_dsp_create(const void* flags) {
    ...
    audio_err = AudioHardwareGetProperty(kAudioHardwarePropertyDefaultOutputDevice, &audio_count, &(priv->device));
    if (audio_err != noErr) {
    ...
    audio_err =
        AudioDeviceGetProperty(priv->device, 0, false, kAudioDevicePropertyDeviceName, &audio_count, audio_device);
    if (audio_err != noErr) {
    ...
    audio_err =
        AudioDeviceGetProperty(priv->device, 0, false, kAudioDevicePropertyStreamFormat, &audio_count, &default_format);
    if (audio_err != noErr) {
    ...
    new_format.mChannelsPerFrame = (params->type & SYSDEP_DSP_STEREO) ? 2 : 1;
    /*
    ...
    audio_count = sizeof(AudioStreamBasicDescription);
    audio_err =
        AudioDeviceGetProperty(priv->device, 0, false, kAudioDevicePropertyStreamFormat, &audio_count, &new_format);
    if (audio_err != noErr) {
        fprintf(stderr, "error %ld getting basic device parameters\n", audio_err);
        return NULL;
    }
    ...
    priv->rate_mult = ((int)(new_format.mSampleRate + 0.5)) / params->samplerate;
    if (((int)(new_format.mSampleRate + 0.5)) != (params->samplerate * priv->rate_mult)) {
    ...
    audio_err = AudioDeviceAddIOProc(priv->device, coreaudio_dsp_play, (void*)priv);
    if (audio_err != noErr) {
