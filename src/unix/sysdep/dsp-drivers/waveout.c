...
static void*
waveout_dsp_create(const void* flags) {
    ...
    fprintf(stderr, "info: Writing sound output to file \"%s\" in %dHz/%d/%c PCM format\n", priv->name,
            dsp->hw_info.samplerate, (dsp->hw_info.type & SYSDEP_DSP_16BIT) ? 16 : 8,
...
static void
waveout_dsp_destroy(struct sysdep_dsp_struct* dsp) {
...
static int
waveout_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count) {
    if (count > 0) {
        struct waveout_dsp_priv_data* priv;
        size_t result;
