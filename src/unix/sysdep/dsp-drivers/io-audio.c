...
struct rc_option alsa_opts[] = {{"QNX Audio related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
                                {"audio-preferred", "audio-primary", rc_bool, &preferred_device, "1", 0, 0, NULL,
                                 "Use the preferred device or use the primary device."},
...
alsa_dsp_create(const void* flags) {
    ...
    if (!(priv = calloc(1, sizeof(struct alsa_dsp_priv_data)))) {
        fprintf(stderr, "error: malloc failed for struct dsp_priv_data\n");
    ...
    if (preferred_device) {
        if ((err = snd_pcm_open_preferred(&(priv->audio_dev.m_AudioHandle), &priv->audio_dev.m_Acard,
                                          &priv->audio_dev.m_Adevice, SND_PCM_OPEN_PLAYBACK))
            < 0) {
    ...
    /* number of frags */
    j = ((dsp->hw_info.samplerate * alsa_dsp_bytes_per_sample[dsp->hw_info.type] * params->bufsize) / (0x01 << i)) + 1;
    ...
    dsp->hw_info.bufsize =
        priv->audio_dev.m_Asetup.buf.stream.queue_size / alsa_dsp_bytes_per_sample[dsp->hw_info.type];
#if 0
