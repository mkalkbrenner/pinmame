...
static int netbsd_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count);
...
/* Public methods (static but exported through the sysdep_dsp or plugin
   struct.) */
static void*
netbsd_dsp_create(const void* flags) {
    ...
    if (!(priv = calloc(1, sizeof(struct netbsd_dsp_priv_data)))) {
        fprintf(stderr, "error malloc failed for struct netbsd_dsp_priv_data\n");
    ...
    if (a_info.play.encoding != desired_encoding || a_info.play.precision != desired_precision) {
        ...
        if (a_info.play.encoding != AUDIO_ENCODING_ULINEAR || a_info.play.precision != 8) {
    ...
    blocks =
        ((dsp->hw_info.samplerate * netbsd_dsp_bytes_per_sample[dsp->hw_info.type] * params->bufsize) / block_size) + 1;
    ...
    dsp->hw_info.bufsize = block_size * blocks / netbsd_dsp_bytes_per_sample[dsp->hw_info.type];
...
static void
netbsd_dsp_destroy(struct sysdep_dsp_struct* dsp) {
...
static int
netbsd_dsp_get_freespace(struct sysdep_dsp_struct* dsp) {
    audio_info_t a_info;
    struct netbsd_dsp_priv_data* priv = dsp->_priv;
    ...
    return dsp->hw_info.bufsize - a_info.play.seek / netbsd_dsp_bytes_per_sample[dsp->hw_info.type];
}

static int
netbsd_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count) {
    int result;
