...
static int esound_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count);
...
/* public methods (static but exported through the sysdep_dsp or plugin
   struct) */
static void*
esound_dsp_create(const void* flags) {
#ifdef ESOUND_DEBUG
    int server_fd;
    ...
    if (!(priv = calloc(1, sizeof(struct esound_dsp_priv_data)))) {
        fprintf(stderr, "error malloc failed for struct esound_dsp_priv_data\n");
    ...
    esd_format |= (dsp->hw_info.type & SYSDEP_DSP_16BIT) ? ESD_BITS16 : ESD_BITS8;
    esd_format |= (dsp->hw_info.type & SYSDEP_DSP_STEREO) ? ESD_STEREO : ESD_MONO;
    ...
    if ((priv->fd = esd_play_stream(esd_format, dsp->hw_info.samplerate, params->device, "mame esound")) < 0) {
...
static void
esound_dsp_destroy(struct sysdep_dsp_struct* dsp) {
...
static int
esound_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count) {
    int result;
    struct esound_dsp_priv_data* priv = dsp->_priv;

    result = write(priv->fd, data, count * esound_dsp_bytes_per_sample[dsp->hw_info.type]);
