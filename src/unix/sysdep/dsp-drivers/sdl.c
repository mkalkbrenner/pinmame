...
static int sdl_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count);
...
/* public methods (static but exported through the sysdep_dsp or plugin
   struct) */
static void*
sdl_dsp_create(const void* flags) {
...
static void
sdl_dsp_destroy(struct sysdep_dsp_struct* dsp) {
    SDL_CloseAudio();
...
static int
sdl_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count) {
...
/* Private method */
static void
sdl_fill_sound(void* unused, Uint8* stream, int len) {
    int result;
    Uint8* dst;
    SDL_LockAudio();
