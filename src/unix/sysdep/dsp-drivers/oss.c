...
static int oss_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count);
...
/* public methods (static but exported through the sysdep_dsp or plugin
   struct) */
static void*
oss_dsp_create(const void* flags) {
    int i, j;
    audio_buf_info info;
    ...
    if (!(priv = calloc(1, sizeof(struct oss_dsp_priv_data)))) {
        fprintf(stderr, "error malloc failed for struct oss_dsp_priv_data\n");
        oss_dsp_destroy(dsp);
        return NULL;
    }
    ...
    /* number of frags */
    j = ((dsp->hw_info.samplerate * oss_dsp_bytes_per_sample[dsp->hw_info.type] * params->bufsize) / (0x01 << i)) + 1;
    ...
    if (ioctl(priv->fd, SNDCTL_DSP_GETOSPACE, &info) < 0) {
        ...
        i = 1 << (j & 0x0000FFFF);
        j = j >> 16;
        if ((info.fragsize < (i / 2)) || (info.fragsize > (i * 2)) || (info.fragstotal < (j - 2))
            || (info.fragstotal > (j + 2))) {
            ...
            dsp->hw_info.bufsize =
                (info.fragsize * (info.fragstotal + 1)) / oss_dsp_bytes_per_sample[dsp->hw_info.type];
    }
...
static void
oss_dsp_destroy(struct sysdep_dsp_struct* dsp) {
...
static int
oss_dsp_get_freespace(struct sysdep_dsp_struct* dsp) {
    audio_buf_info info;
    struct oss_dsp_priv_data* priv = dsp->_priv;
...
static int
oss_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count) {
    int result;
    struct oss_dsp_priv_data* priv = dsp->_priv;

    result = write(priv->fd, data, count * oss_dsp_bytes_per_sample[dsp->hw_info.type]);
