...
static int irix_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count);
...
irix_dsp_create(const void* flags) {
    ...
    fprintf(stderr, "Setting sound to %dHz, %d bit, %s\n", dsp->hw_info.samplerate, tempbits * 8,
            (tempchan == 2) ? "stereo" : "mono");
...
irix_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count) {
    ...
    for (; outbuf < (data + count); outbuf += priv->sampwidth)
        *outbuf ^= 0x80;
