...
static int sysdep_mixer_list_plugins(struct rc_option* option, const char* arg, int priority);
...
static struct rc_option sysdep_mixer_opts[] = {
    ...
    {"Sound mixer related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"sound-mixer-plugin", "smp", rc_string, &sysdep_mixer_plugin, NULL, 0, 0, NULL,
     "Select which plugin to use for the sound mixer"},
    {"list-mixer-plugins", "lmp", rc_use_function_no_arg, NULL, NULL, 0, 0, sysdep_mixer_list_plugins,
...
/* private methods */
static int
sysdep_mixer_list_plugins(struct rc_option* option, const char* arg, int priority) {
...
/* public methods */
int
sysdep_mixer_init(struct rc_struct* rc, const char* plugin_path) {
    if (!sysdep_mixer_rc) {
    ...
    if (!sysdep_mixer_plugin_manager) {
        if (!(sysdep_mixer_plugin_manager = plugin_manager_create("sysdep_mixer", rc))) {
            sysdep_mixer_exit();
            return -1;
        }
        ...
        plugin_manager_register(sysdep_mixer_plugin_manager, sysdep_mixer_plugins);
...
sysdep_mixer_create(const char* plugin, const char* device, int flags) {
    int i;
    ...
    /* create the instance */
    if (!(mixer = plugin_manager_create_instance(sysdep_mixer_plugin_manager, plugin ? plugin : sysdep_mixer_plugin,
                                                 &params)))
        return NULL;
...
#endif
            if (mixer->get(mixer, i, &mixer->cache_left[i], &mixer->cache_right[i])) {
...
void
sysdep_mixer_destroy(struct sysdep_mixer_struct* mixer) {
    int i, left, right;
...
#endif
            sysdep_mixer_set(mixer, i, mixer->orig_left[i], mixer->orig_right[i]);
        }
    }
    mixer->destroy(mixer);
}
...
int
sysdep_mixer_channel_available(struct sysdep_mixer_struct* mixer, int channel) {
    return mixer->channel_available[channel];
}

int
sysdep_mixer_set(struct sysdep_mixer_struct* mixer, int channel, int left, int right) {
...
int
sysdep_mixer_get(struct sysdep_mixer_struct* mixer, int channel, int* left, int* right) {
    if (!mixer->channel_available[channel])
        return -1;
    ...
    if ((*left >= (mixer->cache_left[channel] - 5)) && (*left <= (mixer->cache_left[channel] + 5))
        && (*right >= (mixer->cache_right[channel] - 5)) && (*right <= (mixer->cache_right[channel] + 5))) {
#ifdef SYSDEP_MIXER_DEBUG
