...
#define FIFO_PROTOTYPES(PREFIX, NAME, TYPE)                                                                            \
    ...
    PREFIX int NAME##_fifo_put(struct NAME##_fifo_struct* fifo, TYPE data);                                            \
    PREFIX int NAME##_fifo_get(struct NAME##_fifo_struct* fifo, TYPE* data);                                           \
    PREFIX int NAME##_fifo_peek(struct NAME##_fifo_struct* fifo, TYPE* data);                                          \
    PREFIX void NAME##_fifo_empty(struct NAME##_fifo_struct* fifo);                                                    \
    PREFIX int NAME##_fifo_in_use(struct NAME##_fifo_struct* fifo);                                                    \
    PREFIX int NAME##_fifo_get_freespace(struct NAME##_fifo_struct* fifo);
...
#define FIFO_CREATE(PREFIX, NAME, TYPE)                                                                                \
    PREFIX struct NAME##_fifo_struct* NAME##_fifo_create(int size) {                                                   \
        ...
        if (!(fifo = calloc(1, sizeof(struct NAME##_fifo_struct)))) {                                                  \
            fprintf(stderr, "error malloc failed for struct xxx_fifo_struct\n");                                       \
            return NULL;                                                                                               \
        }                                                                                                              \
                                                                                                                       \
        /* allocate the buffer */                                                                                      \
        if (!(fifo->buffer = calloc(size, sizeof(TYPE)))) {                                                            \
            fprintf(stderr, "error malloc failed for xxx_fifo buffer\n");                                              \
...
#define FIFO_EMPTY(PREFIX, NAME, TYPE)                                                                                 \
    PREFIX void NAME##_fifo_empty(struct NAME##_fifo_struct* fifo) { fifo->head = fifo->in_use = 0; }

#define FIFO_IN_USE(PREFIX, NAME, TYPE)                                                                                \
    PREFIX int NAME##_fifo_in_use(struct NAME##_fifo_struct* fifo) { return fifo->in_use; }

#define FIFO_GET_FREESPACE(PREFIX, NAME, TYPE)                                                                         \
    PREFIX int NAME##_fifo_get_freespace(struct NAME##_fifo_struct* fifo) { return fifo->size - fifo->in_use; }
