...
switch (use_dirty) {
    case 0: /* non dirty */
    {
        ...
        SRC_PIXEL* line_end = (SRC_PIXEL*)bitmap->line[src_y + src_height] + src_x + src_width;
...
#endif
#ifdef PUT_IMAGE
        PUT_IMAGE(src_x, src_y, dest_x, dest_y, SCALE_X(src_width), SCALE_Y(src_height))
#endif
        break;
    }
    case 1: /* normal dirty */
                ...
                for (x = src_x >> 3; x < max_x; x++) {
                    if (bitmap->dirty_blocks[y][x]) {
                        ...
                        for (; h < max_h; h++, block_dest += DEST_SCALE_Y(DEST_WIDTH))
                            COPY_LINE((SRC_PIXEL*)bitmap->line[h] + min_x, (SRC_PIXEL*)bitmap->line[h] + max_x,
                                      block_dest)
#endif
#ifdef PUT_IMAGE
                        PUT_IMAGE(min_x, y << 3, dest_x + SCALE_X(min_x - src_x), dest_y + SCALE_Y((y << 3) - src_y),
                                  SCALE_X((x << 3) - min_x), SCALE_Y(8))
#endif
                    }
                }
