...
static int sysdep_dsp_list_plugins(struct rc_option* option, const char* arg, int priority);
...
static struct rc_option sysdep_dsp_opts[] = {
    ...
    {"dsp-plugin", "dp", rc_string, &sysdep_dsp_plugin, NULL, 0, 0, NULL,
     "Select which plugin to use for digital sound"},
    {"list-dsp-plugins", "ldp", rc_use_function_no_arg, NULL, NULL, 0, 0, sysdep_dsp_list_plugins,
     "List available sound-dsp plugins"},
    {"timer", "ti", rc_bool, &sysdep_dsp_use_timer, "0", 0, 0, NULL,
...
/* private methods */
static int
sysdep_dsp_list_plugins(struct rc_option* option, const char* arg, int priority) {
...
int
sysdep_dsp_init(struct rc_struct* rc, const char* plugin_path) {
    if (!sysdep_dsp_rc) {
    ...
    if (!sysdep_dsp_plugin_manager) {
        if (!(sysdep_dsp_plugin_manager = plugin_manager_create("sysdep_dsp", rc))) {
            sysdep_dsp_exit();
            return -1;
        }
...
sysdep_dsp_create(const char* plugin, const char* device, int* samplerate, int* type, float bufsize, int flags) {
    ...
    /* create the instance */
    if (!(dsp = plugin_manager_create_instance(sysdep_dsp_plugin_manager, plugin ? plugin : sysdep_dsp_plugin,
                                               &params))) {
        return NULL;
    }
    ...
    if (memcmp(&dsp->emu_info, &dsp->hw_info, sizeof(struct sysdep_dsp_info))) {
        if (!(dsp->convert_buf = malloc(dsp->hw_info.bufsize * sysdep_dsp_bytes_per_sample[dsp->hw_info.type]))) {
            fprintf(stderr, "error malloc failed for dsp convert buffer\n");
...
void
sysdep_dsp_destroy(struct sysdep_dsp_struct* dsp) {
    if (dsp->convert_buf)
...
int
sysdep_dsp_get_freespace(struct sysdep_dsp_struct* dsp) {
    uclock_t t;
    int result;
    ...
    if (dsp->last_update == 0)
        ...
        result = ((t - dsp->last_update) * dsp->emu_info.samplerate) / UCLOCKS_PER_SEC;
...
int
sysdep_dsp_write(struct sysdep_dsp_struct* dsp, unsigned char* data, int count) {
    if (!count)
        return 0;
    ...
    if (memcmp(&dsp->emu_info, &dsp->hw_info, sizeof(struct sysdep_dsp_info))) {
        ...
        switch ((dsp->emu_info.type << 4) | dsp->hw_info.type) {
            ...
            case 0x20: /* 8 bit mono */
                for (i = 0; i < count; i++)
                    convert_buf[i] = (((int)data[i * 2] + (int)data[i * 2 + 1]) >> 1);
                break;
            case 0x21: /* 16bit mono */
                for (i = 0; i < count; i++)
                    convert_buf_large[i] = (((int)data[i * 2] + (int)data[i * 2 + 1]) << 7) - 32768;
                break;
            case 0x23: /* 16bit stereo */
                for (i = 0; i < count; i++) {
                    convert_buf_large[i * 2] = ((int)data[i * 2] << 8) - 32768;
                    convert_buf_large[i * 2 + 1] = ((int)data[i * 2 + 1] << 8) - 32768;
                }
                break;
            ...
            case 0x30: /* 8 bit mono */
                for (i = 0; i < count; i++)
                    convert_buf[i] = (((int)data_large[i * 2] + (int)data_large[i * 2 + 1]) >> 9) + 128;
                break;
            case 0x31: /* 16bit mono */
                for (i = 0; i < count; i++)
                    convert_buf_large[i] = (((int)data_large[i * 2] + (int)data_large[i * 2 + 1]) >> 1);
                break;
...
int
sysdep_dsp_get_max_freespace(struct sysdep_dsp_struct* dsp) {
