...
static int oss_mixer_set(struct sysdep_mixer_struct* mixer, int channel, int left, int right);
static int oss_mixer_get(struct sysdep_mixer_struct* mixer, int channel, int* left, int* right);
...
/* public methods (static but exported through the sysdep_mixer or plugin
   struct) */
static void*
oss_mixer_create(const void* flags) {
    int i, j;
    ...
    if (!(priv = calloc(1, sizeof(struct oss_mixer_priv_data)))) {
        fprintf(stderr, "error malloc failed for struct oss_mixer_priv_data\n");
...
static void
oss_mixer_destroy(struct sysdep_mixer_struct* mixer) {
...
static int
oss_mixer_set(struct sysdep_mixer_struct* mixer, int channel, int left, int right) {
    int i = left | right << 8;
    struct oss_mixer_priv_data* priv = mixer->_priv;
...
static int
oss_mixer_get(struct sysdep_mixer_struct* mixer, int channel, int* left, int* right) {
    int value;
    struct oss_mixer_priv_data* priv = mixer->_priv;

    if (ioctl(priv->fd, MIXER_READ(oss_mixer_sysdep_to_oss[channel]), &value)) {
