...
static int alsa_mixer_set(struct sysdep_mixer_struct* mixer, int channel, int left, int right);
static int alsa_mixer_get(struct sysdep_mixer_struct* mixer, int channel, int* left, int* right);
...
struct rc_option alsa_mixer_opts[] = {
    ...
    {"alsa-mixer", "amixer", rc_string, &mixer_name, "PCM", 0, 0, NULL, "Select which simple mixer control to use"},
...
/* public methods (static but exported through the sysdep_mixer or plugin
   struct) */
static void*
alsa_mixer_create(const void* flags) {
    int err;
    ...
    if (!(mixer = calloc(1, sizeof(struct sysdep_mixer_struct)))) {
        fprintf(stderr, "error malloc failed for struct sysdep_mixer_struct\n");
        return NULL;
    }

    /* alloc private data */
    if (!(priv = calloc(1, sizeof(struct alsa_mixer_priv_data)))) {
        fprintf(stderr, "error malloc failed for struct alsa_mixer_priv_data\n");
...
static void
alsa_mixer_destroy(struct sysdep_mixer_struct* mixer) {
...
static int
alsa_mixer_set(struct sysdep_mixer_struct* mixer, int channel, int left, int right) {
    int err;
    float vol;
    struct alsa_mixer_priv_data* priv = mixer->_priv;
...
static int
alsa_mixer_get(struct sysdep_mixer_struct* mixer, int channel, int* left, int* right) {
    long vol;
    struct alsa_mixer_priv_data* priv = mixer->_priv;
