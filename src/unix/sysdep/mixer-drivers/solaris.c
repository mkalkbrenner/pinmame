...
/* public methods (static but exported through the sysdep_mixer or plugin
   struct) */
static void*
sol_mixer_create(const void* flags) {
...
static void
sol_mixer_destroy(struct sysdep_mixer_struct* mixer) {
...
static int
sol_mixer_set(struct sysdep_mixer_struct* mixer, int channel, int left, int right) {
    audio_info_t info;
    struct sol_mixer_priv_data* priv = mixer->_priv;
...
static int
sol_mixer_get(struct sysdep_mixer_struct* mixer, int channel, int* left, int* right) {
    audio_info_t info;
    struct sol_mixer_priv_data* priv = mixer->_priv;
