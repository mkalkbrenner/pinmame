...
/* private methods */
static int
sysdep_palette_make_pen_from_info(struct sysdep_palette_info* info, unsigned char red, unsigned char green,
                                  unsigned char blue) {
    int pen = 0;

    /* are the shifts initialised ? */
    if (!info->red_shift) {
        for (pen = 1 << (8 * sizeof(pen) - 1); pen && (!(pen & info->red_mask)); pen >>= 1, info->red_shift++)
            ;

        for (pen = 1 << (8 * sizeof(pen) - 1); pen && (!(pen & info->green_mask)); pen >>= 1, info->green_shift++)
            ;

        for (pen = 1 << (8 * sizeof(pen) - 1); pen && (!(pen & info->blue_mask)); pen >>= 1, info->blue_shift++)
            ;
    }
...
sysdep_palette_create(int depth, int writable_colors) {
    int r, g, b;
    ...
    if (display_palette_info.depth < depth) {
        fprintf(stderr, "error in sysdep_palette_create: %d bpp requested on a %d bpp display\n", depth,
...
/* If the display gets recreated this must be done again, but the
   ...
   if (display_palette_info.depth == 8)
    ...
    if (!(palette = calloc(1, sizeof(struct sysdep_palette_struct)))) {
        fprintf(stderr, "error malloc failed for struct sysdep_palette_struct\n");
        return NULL;
    }
        ...
        for (g = 0; g < 32; g++)
            for (b = 0; b < 32; b++)
                palette->lookup[r * 1024 + g * 32 + b] =
                    sysdep_palette_make_pen_from_info(&display_palette_info, r * 8, g * 8, b * 8);
...
/* destructor */
void
sysdep_palette_destroy(struct sysdep_palette_struct* palette) {
    free(palette->lookup);
    free(palette);
}
...
int
sysdep_palette_set_pen(struct sysdep_palette_struct* palette, int pen, unsigned char red, unsigned char green,
                       unsigned char blue) {
    palette->lookup[pen] = sysdep_palette_make_pen_from_info(&display_palette_info, red, green, blue);
...
int
sysdep_palette_make_pen(struct sysdep_palette_struct* palette, unsigned char red, unsigned char green,
                        unsigned char blue) {
    return sysdep_palette_make_pen_from_info(&palette->emulated, red, green, blue);
}
...
int
sysdep_palette_change_display(struct sysdep_palette_struct** palette) {
    struct sysdep_palette_struct* new_palette = NULL;

    if (!(new_palette = sysdep_palette_create((*palette)->emulated.depth, (*palette)->emulated.writable_colors)))
        return -1;
...
void
sysdep_palette_mark_dirty(struct sysdep_palette_struct* palette) {
    palette->dirty = 1;
}

void
sysdep_palette_clear_dirty(struct sysdep_palette_struct* palette) {
    palette->dirty = 0;
}
