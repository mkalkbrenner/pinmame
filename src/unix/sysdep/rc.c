...
/* private methods */
static int
rc_verify(struct rc_option* option, float value) {
...
static int
rc_set_defaults(struct rc_option* option) {
    int i;
    ...
    for (i = 0; option[i].type; i++) {
        if (option[i].type == rc_link) {
            if (rc_set_defaults(option[i].dest))
                return -1;
        } else if (option[i].deflt && rc_set_option3(option + i, option[i].deflt, option[i].priority))
            return -1;
    }
...
static void
rc_free_stuff(struct rc_option* option) {
    int i;
...
void
rc_destroy(struct rc_struct* rc) {
    if (rc->option) {
...
int
rc_register(struct rc_struct* rc, struct rc_option* option) {
    int i;
    ...
    if (i >= (rc->option_size - 1)) {
        struct rc_option* tmp = realloc(rc->option, (rc->option_size + BUF_SIZE) * sizeof(struct rc_option));
        if (!tmp) {
            fprintf(stderr, "error: malloc failed in rc_register_option\n");
            return -1;
        }
        rc->option = tmp;
        memset(rc->option + rc->option_size, 0, BUF_SIZE * sizeof(struct rc_option));
...
int
rc_unregister(struct rc_struct* rc, struct rc_option* option) {
    int i;
...
int
rc_load(struct rc_struct* rc, const char* name, int priority, int continue_on_errors) {
    FILE* f;
...
int
rc_save(struct rc_struct* rc, const char* name, int append) {
    FILE* f;
...
int
osd_rc_read(struct rc_struct* rc, mame_file* f, const char* description, int priority, int continue_on_errors) {
    char buf[BUF_SIZE];
    int line = 0;
    ...
    while (mame_fgets(buf, BUF_SIZE, f)) {
        ...
        if (!(option = rc_get_option2(rc->option, name))) {
            ...
            fprintf(stderr, "error: %s requires an argument, on line %d of file: %s\n", name, line, description);
        } else if ((tmp = strtok(NULL, " \t\r\n")) && (tmp[0] != '#')) {
            fprintf(stderr, "error: trailing garbage: \"%s\" on line: %d of file: %s\n", tmp, line, description);
...
int
rc_read(struct rc_struct* rc, FILE* f, const char* description, int priority, int continue_on_errors) {
    char buf[BUF_SIZE];
    int line = 0;
    ...
    while (fgets(buf, BUF_SIZE, f)) {
        ...
        if (!(option = rc_get_option2(rc->option, name))) {
            fprintf(stderr, "error: %s(%d): unknown option %s", description, line, name);
        } else if (rc_requires_arg[option->type] && !(arg = strtok(NULL, " \t\r\n"))) {
            fprintf(stderr, "error: %s(%d): %s requires an argument", description, line, name);
        } else if ((tmp = strtok(NULL, " \t\r\n")) && (tmp[0] != '#')) {
            fprintf(stderr, "error: %s(%d): trailing garbage: \"%s\"", description, line, tmp);
        } else if (rc_set_option3(option, arg, priority))
            errin3 = 1;
        else
            continue;
...
static int
rc_real_write(struct rc_option* option, FILE* f, const char* description) {
    int i;
...
static int
real_rc_write(struct rc_option* option, mame_file* f, const char* description) {
    int i;
...
int
osd_rc_write(struct rc_struct* rc, mame_file* f, const char* description) {
    return real_rc_write(rc->option, f, description);
}

int
rc_write(struct rc_struct* rc, FILE* f, const char* description) {
    return rc_real_write(rc->option, f, description);
}

int
rc_parse_commandline(struct rc_struct* rc, int argc, char* argv[], int priority, int (*arg_callback)(char* arg)) {
    int i;
    ...
    for (i = 1; i < argc; i++) {
        if (argv[i][0] == '-') {
            ...
            } else if (!strncmp(argv[i] + start, "no", 2) && (option = rc_get_option2(rc->option, argv[i] + start + 2))
                       && (option->type == rc_bool)) {
            ...
            if (rc->args_registered >= (rc->arg_size)) {
                char** tmp = realloc(rc->arg, (rc->arg_size + BUF_SIZE) * sizeof(char*));
                if (!tmp) {
                    fprintf(stderr, "error: malloc failed in rc_parse_commadline\n");
                    return -1;
                }
                rc->arg = tmp;
...
int
rc_get_non_option_args(struct rc_struct* rc, int* argc, char** argv[]) {
    *argv = rc->arg;
...
/* needed to walk the tree */
static void
rc_real_print_help(struct rc_option* option, FILE* f) {
    int i;
    char buf[BUF_SIZE];
    static const char* type_name[] = {"",       "", " <string>", " <int>", " <float>", "", "", " <filename>",
                                      " <arg>", "", ""};
    ...
    for (i = 0; option[i].type; i++) {
        switch (option[i].type) {
            ...
            case rc_link:
                ...
                break;
            default:
                snprintf(buf, BUF_SIZE, "-%s%s%s%s%s%s", (option[i].type == rc_bool) ? "[no]" : "", option[i].name,
                         (option[i].shortname) ? " / -" : "",
                         (option[i].shortname && (option[i].type == rc_bool)) ? "[no]" : "",
                         (option[i].shortname) ? option[i].shortname : "", type_name[option[i].type]);
                fprint_columns(f, buf, (option[i].help) ? option[i].help : "no help available");
        }
    }
}

void
rc_print_help(struct rc_struct* rc, FILE* f) {
    rc_real_print_help(rc->option, f);
}

/* needed to walk the tree */
static void
rc_real_print_man_options(struct rc_option* option, FILE* f) {
    int i;
    static const char* type_name[] = {"", "", " Ar string", " Ar int", " Ar float", "", "", " Ar filename", " Ar arg",
                                      "", ""};
    ...
    for (i = 0; option[i].type; i++) {
        switch (option[i].type) {
            ...
            case rc_link:
                ...
                break;
            default:
                fprintf(f, ".It Fl %s%s%s%s%s%s\n%s\n", (option[i].type == rc_bool) ? "[no]" : "", option[i].name,
                        (option[i].shortname) ? " , " : "",
                        (option[i].shortname && (option[i].type == rc_bool)) ? "[no]" : "",
                        (option[i].shortname) ? option[i].shortname : "", type_name[option[i].type],
...
void
rc_print_man_options(struct rc_struct* rc, FILE* f) {
    rc_real_print_man_options(rc->option, f);
}

int
rc_verify_power_of_2(struct rc_option* option, const char* arg, int priority) {
    int i, value;
...
int
rc_option_requires_arg(struct rc_struct* rc, const char* name) {
    return rc_option_requires_arg2(rc->option, name);
}

int
rc_option_requires_arg2(struct rc_option* option, const char* name) {
...
int
rc_option_requires_arg3(struct rc_option* option) {
    return rc_requires_arg[option->type];
}

int
rc_get_priority(struct rc_struct* rc, const char* name) {
    return rc_get_priority2(rc->option, name);
}

int
rc_get_priority2(struct rc_option* option, const char* name) {
    struct rc_option* my_option;
...
int
rc_get_priority3(struct rc_option* option) {
    return option->priority;
}

int
rc_set_option(struct rc_struct* rc, const char* name, const char* arg, int priority) {
    return rc_set_option2(rc->option, name, arg, priority);
}

int
rc_set_option2(struct rc_option* option, const char* name, const char* arg, int priority) {
    struct rc_option* my_option;
...
int
rc_set_option3(struct rc_option* option, const char* arg, int priority) {
    char* end;
    ...
    switch (option->type) {
        ...
        case rc_use_function_no_arg:
            break;
        default:
            fprintf(stderr, "error: unknown option type: %d, this should not happen!\n", option->type);
            return -1;
    }
...
struct rc_option*
rc_get_option(struct rc_struct* rc, const char* name) {
    return rc_get_option2(rc->option, name);
}

struct rc_option*
rc_get_option2(struct rc_option* option, const char* name) {
    int i;
    ...
    for (i = 0; option[i].type; i++) {
        switch (option[i].type) {
            ...
            case rc_link:
                ...
                break;
            default:
                if (!strcmp(name, option[i].name) || (option[i].shortname && !strcmp(name, option[i].shortname)))
                    return &option[i];
        }
    }
    return NULL;
...
struct rc_option*
rc_get_options(struct rc_struct* rc) {
    return rc->option;
}
 ...
 */
int
rc_check_and_create_dir(const char* name) {
