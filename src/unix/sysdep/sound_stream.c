...
sound_stream_create(struct sysdep_dsp_struct* dsp, int type, int buf_size, int buf_count) {
    ...
    if (!(stream = calloc(1, sizeof(struct sound_stream_struct)))) {
        fprintf(stderr, "error malloc failed for struct sound_stream_struct\n");
        return NULL;
    }
    ...
    if (!(stream->output_buf = calloc(stream->output_buf_size, stream->bytes_per_sample))) {
        fprintf(stderr, "error malloc failed for sound_stream output buffer\n");
        sound_stream_destroy(stream);
        return NULL;
    }

    /* create the fifo's */
    if (!(stream->sample_buf_fifo = sample_buf_fifo_create(stream->sample_buf_count))) {
        sound_stream_destroy(stream);
        return NULL;
    }
    if (!(stream->empty_sample_buf_fifo = sample_buf_fifo_create(stream->sample_buf_count))) {
        sound_stream_destroy(stream);
        return NULL;
    }

    /* create sample_buf_count sample_buf structs */
    if (!(stream->sample_buf = calloc(stream->sample_buf_count, sizeof(struct sound_stream_sample_buf)))) {
        fprintf(stderr, "error malloc failed for struct sound_stream_sample_buf\n");
        sound_stream_destroy(stream);
        return NULL;
    }
    ...
    for (i = 0; i < stream->sample_buf_count; i++) {
        if (!(stream->sample_buf[i].data = calloc(stream->sample_buf_size, stream->bytes_per_sample))) {
            fprintf(stderr, "error malloc failed for sample_buf data\n");
            sound_stream_destroy(stream);
            return NULL;
        }
        sample_buf_fifo_put(stream->empty_sample_buf_fifo, &stream->sample_buf[i]);
    }
...
void
sound_stream_destroy(struct sound_stream_struct* stream) {
    int i;
...
void
sound_stream_write(struct sound_stream_struct* stream, unsigned char* data, int samples) {
    ...
    while (samples) {
        if (sample_buf_fifo_get(stream->empty_sample_buf_fifo, &sample_buf) == 0) {
            ...
            sample_buf->pos = 0;
            memcpy(sample_buf->data, data, samples_this_loop * stream->bytes_per_sample);
...
void
sound_stream_update(struct sound_stream_struct* stream) {
    int freespace;
    struct sound_stream_sample_buf* sample_buf;
    ...
    while (freespace > 0) {
        ...
        result = sysdep_dsp_write(stream->dsp, sample_buf->data + (sample_buf->pos * stream->bytes_per_sample),
                                  samples_this_loop);
...
#ifdef SOUND_STREAM_WARNING
            fprintf(stderr, "warning: sound_stream: sysdep_dsp_write returned %d, expected %d\n", result,
                    samples_this_loop);
#endif
            return;
        }
