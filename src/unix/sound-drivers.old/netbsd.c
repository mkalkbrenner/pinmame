    ...
    if (play_sound) {
         ...
         */
                ...
                fprintf(stderr_file, "Audio device %s set to %dbit linear %s %dHz\n", audiodevice,
                        (sound_8bit) ? 8 : 16, (sound_stereo) ? "stereo" : "mono", options.samplerate);
            }
        }
    }
    return OSD_OK;
...
static void
setblocksize(int fd, struct audio_info* info) /* from netbsd's ossaudio lib */
{
...
static int
setfragment(int fd, int* frag) /* from netbsd's ossaudio lib */
{
