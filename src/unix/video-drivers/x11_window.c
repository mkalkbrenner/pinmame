...
#ifdef USE_MITSHM
    {"mitshm", "ms", rc_bool, &use_mit_shm, "1", 0, 0, NULL,
...
/*	{ "xvext", "xv", rc_bool, &use_xv,
 ...
 "Use/don't use Xv extension for hardware scaling (if available and compiled in))" },*/
    ...
    {"yv12", NULL, rc_bool, &hwscale_yv12, "0", 0, 0, NULL,
...
#endif
    {"xsync", "xs", rc_bool, &use_xsync, "1", 0, 0, NULL,
    ...
    {"xil", "x", rc_bool, &use_xil, "1", 0, 0, NULL,
    ...
    {"run-in-root-window", "root", rc_bool, &run_in_root_window, "0", 0, 0, NULL,
     "Enable/disable running in root window"},
    {"root_window_id", "rid", rc_int, &root_window_id, "0", 0, 0, NULL,
...
int
test_mit_shm(Display* display, XErrorEvent* error) {
    char msg[256];
...
#ifdef USE_XV
int
FindXvPort(Display* dpy, long format, int* port) {
...
int
FindRGBXvFormat(Display* dpy, int* port, long* format, int* bpp) {
    int i, j, p, ret, num_formats;
    unsigned int num_adaptors;
    XvAdaptorInfo* ai;
    XvImageFormatValues* fo;
...
static Cursor
create_invisible_cursor(Display* display, Window win) {
    Pixmap cursormask;
    XGCValues xgc;
    XColor dummycolour;
    Cursor cursor;
...
static int
x11_find_best_visual(int bitmap_depth) {
    ...
    if (bitmap_depth == 8 && XMatchVisualInfo(display, screen_no, 8, PseudoColor, &visualinfo)) {
        xvisual = visualinfo.visual;
        depth = 8;
        return 0;
    }
...
int
x11_window_create_display(int bitmap_depth) {
...
#ifdef USE_HWSCALE
    use_xv = (x11_video_mode == X11_XV_WINDOW || x11_video_mode == X11_XV_FULLSCREEN);
    ...
    if (use_xv) /* look for available Xv extensions */
    {
        ...
        if (XvQueryExtension(display, &p_version, &p_release, &p_request_base, &p_event_base, &p_error_base)
            == Success) {
    ...
    if (run_in_root_window) {
        ...
        i = XWMGeometry(display, screen_no, geometry, NULL, 0, &hints, &hints.x, &hints.y, &geom_width, &geom_height,
                        &hints.win_gravity);
        ...
        window = XCreateWindow(
            display, root_window_id, hints.x, hints.y, window_width, window_height, 0, depth, InputOutput, xvisual,
            (CWBorderPixel | CWBackPixel | CWBitGravity | CWWinGravity | CWBackingStore | CWOverrideRedirect
        ...
        event_mask = FocusChangeMask | ExposureMask | EnterWindowMask | LeaveWindowMask | KeyPressMask | KeyReleaseMask;
        if (use_mouse) {
    ...
    switch (x11_window_update_method) {
        ...
        case X11_MITSHM:
            ...
            image = XShmCreateImage(display, xvisual, depth, ZPixmap, NULL, &shm_info, image_width, image_height);
            if (image) {
        ...
        case X11_XV:
            ...
            /* Create an XV MITSHM image. */
            {
                ...
                xvimage = XvShmCreateImage(display, xv_port, hwscale_format, 0, image_width, image_height, &shm_info);
                if (xvimage) {
                    shm_info.shmid = shmget(IPC_PRIVATE, xvimage->data_size, (IPC_CREAT | 0777));
                    if (shm_info.shmid < 0) {
                        fprintf(stderr_file, "\nError: failed to create MITSHM block.\n");
                        return OSD_NOT_OK;
                    }
                    ...
                    /* And allocate the bitmap buffer. */
                    /* new pen color code force double buffering in every cases */
                    xvimage->data = shm_info.shmaddr = (char*)shmat(shm_info.shmid, 0, 0);
        ...
        case X11_NORMAL:
            ...
            image = XCreateImage(display, xvisual, depth, ZPixmap, 0, (char*)scaled_buffer_ptr, image_width,
                                 image_height, 32, /* image_width always is a multiple of 8 */
                                 0);
            ...
            switch (hwscale_format) {
                ...
                case FOURCC_YV12:
                    ...
                    if (widthscale == 1 && heightscale == 1)
                        ...
                        x11_window_update_display_func = x11_window_update_32_to_YV12_direct_perfect;
                    else {
            ...
            switch (hwscale_format) {
                ...
                case FOURCC_YV12:
                    ...
                    if (widthscale == 1 && heightscale == 1)
                        ...
                        x11_window_update_display_func = x11_window_update_16_to_YV12_perfect;
                    else {
                        fprintf(stderr_file, "\nScaling different from 1 or 2"
                                             " is useless and unsupported\n");
                        return OSD_NOT_OK;
...
x11_window_close_display(void) {
    ...
    if (pseudo_color_lookup) {
        if (pseudo_color_use_rw_palette) {
            XFreeColors(display, colormap, pseudo_color_lookup, display_palette_info.writable_colors, 0);
        } else {
 ...
 */
int
x11_window_alloc_palette(int writable_colors) {
    int i;
    ...
    if (XAllocColorCells(display, colormap, 0, 0, 0, pseudo_color_lookup, writable_colors)) {
        pseudo_color_use_rw_palette = 1;
        fprintf(stderr_file, "Using r/w palette entries to speed up, good\n");
        for (i = 0; i < writable_colors; i++)
            if (pseudo_color_lookup[i] != i)
                break;
    } else {
        if (!(pseudo_color_allocated = calloc(writable_colors, sizeof(char)))) {
            fprintf(stderr_file, "X11-window: Error: Malloc failed for pseudo color lookup table\n");
            XFreeColors(display, colormap, pseudo_color_lookup, writable_colors, 0);
            free(pseudo_color_lookup);
            pseudo_color_lookup = NULL;
            return -1;
        }
...
int
x11_window_modify_pen(int pen, unsigned char red, unsigned char green, unsigned char blue) {
    XColor color;
...
#define SQRT(x) ((float)(x) * (x))
                float diff =
                    SQRT(my_red - colors[i].red) + SQRT(my_green - colors[i].green) + SQRT(my_blue - colors[i].blue);
                if (diff < best_diff) {
            ...
            if (XAllocColor(display, colormap, &color)) {
                ...
                if (pseudo_color_warn_low_on_colors) {
                    ...
                    fprintf(stderr_file, "X11-palette: Warning: Closest color match alloc failed\n"
...
void
x11_window_update_display(struct mame_bitmap* bitmap) {
#ifdef USE_HWSCALE
    if (use_hwscale && hwscale_yuv)
    ...
    if (use_mouse && keyboard_pressed(KEYCODE_LALT) && keyboard_pressed_memory(KEYCODE_PGDN)) {
        if (x11_grab_mouse) {
            XUngrabPointer(display, CurrentTime);
            if (show_cursor)
                XDefineCursor(display, window, normal_cursor);
    ...
    if (keyboard_pressed(KEYCODE_LALT) && keyboard_pressed_memory(KEYCODE_PGUP)) {
        if (x11_grab_keyboard) {
            XUngrabKeyboard(display, CurrentTime);
...
void
x11_window_refresh_screen(void) {
    switch (x11_window_update_method) {
        case X11_XIL:
#ifdef USE_XIL
            refresh_xil_screen();
...
#ifdef USE_XV
        {
            ...
            XvShmPutImage(display, xv_port, window, gc, xvimage, 0, 0, HWSCALE_WIDTH, HWSCALE_HEIGHT, (_w - pw) / 2,
                          (_h - ph) / 2, pw, ph, True);
        }
#endif
        break;
...
INLINE void
x11_window_put_image(int x, int y, int width, int height) {
    switch (x11_window_update_method) {
        case X11_XV:
            break;
        case X11_XIL:
        ...
        case X11_MITSHM:
#ifdef USE_MITSHM
            XShmPutImage(display, window, gc, image, x, y, x + startx, y + starty, width, height, FALSE);
#endif
            break;
        case X11_NORMAL:
...
static void
x11_window_update_16_to_YV12(struct mame_bitmap* bitmap) {
    int _x, _y;
    char* dest_y;
    char* dest_u;
    char* dest_v;
...
static void
x11_window_update_16_to_YV12_perfect(struct mame_bitmap* bitmap) { /* this one is used when scale==2 */
    unsigned int _x, _y;
    char* dest_y;
    char* dest_u;
    char* dest_v;
...
static void
x11_window_update_32_to_YV12_direct(struct mame_bitmap* bitmap) {
    int _x, _y, r, g, b;
    char* dest_y;
    char* dest_u;
    char* dest_v;
...
static void
x11_window_update_32_to_YV12_direct_perfect(struct mame_bitmap* bitmap) { /* This one is used when scale == 2 */
    int _x, _y, r, g, b;
    char* dest_y;
    char* dest_u;
    char* dest_v;
...
#ifdef USE_HWSCALE
static void
x11_window_update_16_to_YUY2(struct mame_bitmap* bitmap) {
#define RMASK      0xff0000;
#define GMASK      0x00ff00;
#define BMASK      0x0000ff;
...
static void
x11_window_update_16_to_16bpp(struct mame_bitmap* bitmap) {
#ifdef USE_HWSCALE
...
static void
x11_window_update_16_to_24bpp(struct mame_bitmap* bitmap) {
#define PACK_BITS
#include "blit.h"
#undef PACK_BITS
}
...
static void
x11_window_update_16_to_32bpp(struct mame_bitmap* bitmap) {
#include "blit.h"
}
...
static void
x11_window_update_32_to_32bpp_direct(struct mame_bitmap* bitmap) {
#include "blit.h"
}

#ifdef USE_HWSCALE
static void
x11_window_update_32_to_YUY2_direct(struct mame_bitmap* bitmap) {
#define RMASK 0xff0000;
#define GMASK 0x00ff00;
#define BMASK 0x0000ff;
#undef DEST_PIXEL
