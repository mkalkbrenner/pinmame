...
extern GLdouble vx_cscr_p1, vy_cscr_p1, vz_cscr_p1, vx_cscr_p2, vy_cscr_p2, vz_cscr_p2, vx_cscr_p3, vy_cscr_p3,
    vz_cscr_p3, vx_cscr_p4, vy_cscr_p4, vz_cscr_p4;

/* Skip until we hit whitespace */

char*
SkipToSpace(char* buf) {
...
char*
ParseVec4(char* buf, GLdouble* x, GLdouble* y, GLdouble* z, GLdouble* a) {
    *x = atof(buf);
...
char*
ParseVec3(char* buf, GLdouble* x, GLdouble* y, GLdouble* z) {
    *x = atof(buf);
...
char*
ParseVec2(char* buf, GLdouble* x, GLdouble* y) {
    *x = atof(buf);
...
void
ParsePan(char* buf, PanType type) {
    if (pannum == numpans) {
...
ParseLine(char* buf) {
    ...
    if (!strncasecmp(buf, "startgeom", 9)) {
        ...
        if (ingeom)
            printf("GLError (cab): second call to startgeom\n");
        ingeom = 1;
        ...
        if (!inpan)
            printf("GLError (cab): pan command outside of camerapan\n");
        else
            ParsePan(buf + 4, pan_goto);
    } else if (!strncasecmp(buf, "moveto", 6)) {
        if (!inpan)
            printf("GLError (cab): pan command outside of camerapan\n");
        else
            ParsePan(buf + 6, pan_moveto);
        ...
        if (!ingeom) {
            ...
            if (!strncasecmp(buf + 6, "points", 6)) {
                ...
                printf("GLError (cab): Invalid object type -- %s", buf + 6);
            ...
            printf("GLError (cab): Invalid command -- %s", buf);
    }
}
...
LoadCabinet(char* cabname) {
    ...
    if (strncasecmp(buf, "cabv1.0", 7) && strncasecmp(buf, "cabv1.1", 7)) {
