...
void
setup_xil_images(int xsize, int ysize) {
...
static void
scale_screen(XilImage source) {
    xil_scale(source, window_image, "nearest", window_width / (float)draw_image_xsize,
              window_height / (float)draw_image_ysize);
}

void
update_xil_window_size(int width, int height) {
    window_width = width;
