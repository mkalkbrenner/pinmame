...
struct rc_option x11_input_opts[] = {
    ...
    {"grabmouse", "gm", rc_bool, &x11_grab_mouse, "0", 0, 0, NULL,
     "Enable/disable mousegrabbing (also alt + pagedown)"},
    {"grabkeyboard", "gkb", rc_bool, &x11_grab_keyboard, "0", 0, 0, NULL,
        ...
        while (XPending(display)) {
            ...
            switch (E.type) {
                ...
                case ConfigureNotify:
                    if (E.xany.window == window) {
                        if (xgl_aspect_resize_action == 0
                            && (abs(winwidth - E.xconfigure.width) > 50 || abs(winheight - E.xconfigure.height) > 50)) {
                ...
                case FocusIn:
                    ...
                    if (old_grab_mouse) {
                        if (!XGrabPointer(display, window, True, 0, GrabModeAsync, GrabModeAsync, window, None,
                                          CurrentTime)) {
                            if (show_cursor)
                ...
                case LeaveNotify:
                    if (use_private_cmap)
                        XInstallColormap(display, DefaultColormapOfScreen(screen));
                    break;
#ifdef USE_XIL
                case ConfigureNotify:
 ...
 */
static int
x11_mapkey(struct rc_option* option, const char* arg, int priority) {
...
sysdep_mouse_poll(void) {
    ...
    if (x11_video_mode == X11_DGA) {
        ...
        if (x11_grab_mouse) {
            XWarpPointer(display, None, window, 0, 0, 0, 0, visual_width / 2, visual_height / 2);
