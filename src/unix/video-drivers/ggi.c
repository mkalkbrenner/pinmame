...
struct rc_option display_opts[] = {
    ...
    {"GGI Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"linear", NULL, rc_bool, &use_linear, "0", 0, 0, NULL, "Enable/disable use of linear framebuffer (fast)"},
    {"xres", NULL, rc_int, &force_x, "0", 0, 0, NULL, "Force the X resolution"},
    {"yres", NULL, rc_int, &force_y, "0", 0, 0, NULL, "Force the Y resolution"},
...
static struct mode_list ggimodes[] = {{320, 200}, {320, 240},  {360, 240},   {360, 400},  {360, 480}, {320, 400},
                                      {320, 480}, {400, 300},  {600, 400},   {640, 200},  {640, 400}, {640, 480},
                                      {800, 600}, {1024, 768}, {1280, 1024}, {2048, 1536}};
 ...
 */
static int
ggi_check_mode(ggi_visual_t vis, int w, int h, int depth, ggi_graphtype* type) {
    ggi_mode mode;
    ...
    if ((depth == 8) && (!ggiCheckSimpleMode(vis, w, h, GGI_AUTO, GT_8BIT, &mode))) {
        *type = GT_8BIT;
        return (TRUE);
    }
...
set_video_mode(int depth) {
    ...
    updater_t updaters[] = {
        /* linear updaters */
        NULL, NULL, NULL, NULL, NULL, ggi_update_linear_16_to_16bpp, ggi_update_linear_16_to_24bpp,
        ggi_update_linear_16_to_32bpp,
        /* non-linear updaters */
        NULL, NULL, NULL, NULL, NULL, ggi_update_16_to_16bpp, ggi_update_16_to_24bpp, ggi_update_16_to_32bpp,
        /* scaled non-linear updaters */
        NULL, NULL, NULL, NULL, NULL, ggi_update_16_to_16bpp_scaled, ggi_update_16_to_24bpp, ggi_update_16_to_32bpp};
    int updater = 0;
    ...
    if (video_height < scaled_visual_height || video_width < scaled_visual_width) {
        fprintf(stderr_file, "Forced resolution %dx%d < needed resolution %dx%d -- aborting...\n", video_width,
                video_height, scaled_visual_width, scaled_visual_height);
        return (FALSE);
    }
    ...
    if (!ggi_check_mode(vis, video_width, video_height, depth, &best_type) && (!force_x && !force_y)) {
        int w, h, score;
        ...
        if (!best_score) {
            fprintf(stderr_file,
                    "GGI: Couldn't find a suitable mode for a resolution of %dx%d\n"
    ...
    if (ggiCheckSimpleMode(vis, video_width, video_height, GGI_AUTO, best_type, &mode) != 0)
        return (FALSE);
...
mode_set:
    ...
    if ((mode.visible.x < scaled_visual_width) || (mode.visible.y < scaled_visual_height)) {
        fprintf(stderr_file, "Fatal: cannot get big enough mode %dx%d\n", scaled_visual_width, scaled_visual_height);
        return (FALSE);
    }
    if ((mode.visible.x != scaled_visual_width) || (mode.visible.y != scaled_visual_height)) {
        fprintf(stderr_file, "Notice: cannot get ideal mode %dx%d, setting to %dx%d\n", scaled_visual_width,
                scaled_visual_height, mode.visible.x, mode.visible.y);
    }
    ...
    /* can we do linear ? */
    if (use_linear && (direct_buf = ggiDBGetBuffer(vis, 0)) && (direct_buf->type & GGI_DB_SIMPLE_PLB)) {
        if ((widthscale > 1 || heightscale > 2)) {
            doublebuffer_buffer = malloc(scaled_visual_width * GT_SIZE(mode.graphtype) / 8);
        ...
        video_mem += screen_starty * video_width * GT_SIZE(mode.graphtype) / 8;
#ifdef GGI_DEBUG
        fprintf(stderr_file, "ggi.c: set_video_mode: using %d bit linear update\n", GT_SIZE(mode.graphtype));
#endif
    } else {
        ...
        if ((widthscale > 1) || (heightscale > 1) || (depth == 16) || (depth != GT_SIZE(mode.graphtype))) {
            doublebuffer_buffer = malloc(scaled_visual_width * scaled_visual_height * GT_SIZE(mode.graphtype) / 8);
            if (!doublebuffer_buffer) {
                fprintf(stderr_file, "GGI: Error: Couldn't allocate doublebuffer buffer\n");
                return FALSE;
            }
...
#endif
    if (!vis)
        return -1; /* visual not initialized yet -- do nothing */
...
static void
ggi_update_16_to_16bpp(struct mame_bitmap* bitmap) {
...
#define PUT_IMAGE(X, Y, WIDTH, HEIGHT)                                                                                 \
    {                                                                                                                  \
        ...
        for (_i = 0; _i < HEIGHT; _i++) {                                                                              \
            ggiPutHLine(vis, screen_startx + (X), screen_starty + (Y) + _i, WIDTH,                                     \
                        bitmap->line[(Y) + visual.min_y + _i] + (visual.min_x + (X)) * 2);                             \
        }                                                                                                              \
    }
#include "blit.h"
#undef PUT_IMAGE
...
void
ggi_update_16_to_16bpp_scaled(struct mame_bitmap* bitmap) {
...
#define PUT_IMAGE(X, Y, WIDTH, HEIGHT)                                                                                 \
    {                                                                                                                  \
        ...
        for (_i = 0; _i < HEIGHT; _i++) {                                                                              \
            ggiPutHLine(vis, screen_startx + (X), screen_starty + (Y) + _i, WIDTH,                                     \
                        DEST + (X) * sizeof(DEST_PIXEL) + DEST_WIDTH * sizeof(DEST_PIXEL) * (_i + (Y)));               \
        }                                                                                                              \
    }
...
static void
ggi_update_16_to_24bpp(struct mame_bitmap* bitmap) {
#define INDIRECT   current_palette->lookup
#define SRC_PIXEL  unsigned short
...
static void
ggi_update_16_to_32bpp(struct mame_bitmap* bitmap) {
#define INDIRECT   current_palette->lookup
#define SRC_PIXEL  unsigned short
#define DEST_PIXEL unsigned int
#define DEST       doublebuffer_buffer
...
static void
ggi_update_linear_16_to_16bpp(struct mame_bitmap* bitmap) {
#define DEST_PIXEL unsigned short
#define SRC_PIXEL  unsigned short
...
static void
ggi_update_linear_16_to_24bpp(struct mame_bitmap* bitmap) {
#define INDIRECT   current_palette->lookup
#define DEST_PIXEL unsigned int
#define SRC_PIXEL  unsigned short
#define PACK_BITS
...
static void
ggi_update_linear_16_to_32bpp(struct mame_bitmap* bitmap) {
#define INDIRECT   current_palette->lookup
#define DEST_PIXEL unsigned int
#define SRC_PIXEL  unsigned short
#define DEST       video_mem
...
int
ggi_key(ggi_event* ev) {
    unsigned int keycode = KEY_NONE;
    int label = ev->key.label;

#ifdef KEY_DEBUG
    fprintf(stderr_file, "Keyevent detected: sym = 0x%02x, code = 0x%02x, label = 0x%02x\n", ev->key.sym,
            ev->key.button, label);
#endif
        ...
        case GII_KT_LATIN1:
            switch (label) { /* for now, the simple way */
                ...
                case GIIUC_Percent:
                    keycode = KEY_5;
                    break;
                case GIIUC_Ampersand:
                    keycode = KEY_7;
                    break;
                case GIIUC_Apostrophe:
                    keycode = KEY_QUOTE;
                    break;
                case GIIUC_ParenLeft:
                    keycode = KEY_9;
                    break;
                case GIIUC_ParenRight:
                    keycode = KEY_0;
                    break;
                case GIIUC_Asterisk:
                ...
                case GIIUC_z:
                    keycode = KEY_Z;
                    break;
                case GIIUC_BraceLeft:
                    keycode = KEY_OPENBRACE;
                    break;
                case GIIUC_Pipe:
                    keycode = KEY_BACKSLASH;
                    break;
                case GIIUC_BraceRight:
                    keycode = KEY_CLOSEBRACE;
                    break;
                case GIIUC_Tilde:
                    keycode = KEY_TILDE;
        ...
        case GII_KT_PAD:
            switch (label) { /* for now, the simple way */
                ...
                case GIIK_PDecimal:
                    keycode = KEY_DEL_PAD;
                    break;
                case GIIK_PParenLeft:
                    keycode = KEY_9_PAD;
                    break;
                case GIIK_PParenRight:
                    keycode = KEY_0_PAD;
                    break;
                case GIIK_PSpace:
                    keycode = KEY_SPACE;
