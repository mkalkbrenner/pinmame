...
struct rc_option display_opts[] = {
    ...
    {"tweak", NULL, rc_bool, &use_tweak, "0", 0, 0, NULL, "Enable/disable svgalib tweaked video modes"},
...
static update_func update_functions[8] = {svgalib_update_linear,    svgalib_update_planar,         svgalib_update_gl,
                                          svgalib_update_gl_scaled, svgalib_update_linear_16bpp,   NULL,
                                          svgalib_update_gl_16bpp,  svgalib_update_gl_scaled_16bpp};
...
static struct tweaked_mode_struct tweaked_modes[] = {
    {384, 256, scr384x256, NULL, 1, 0}, {384, 240, scr384x240, NULL, 1, 0},
    {384, 224, scr384x224, NULL, 1, 0}, {336, 240, scr336x240, NULL, 1, 0},
    {320, 240, scr320x240, NULL, 1, 0}, {320, 204, scr320x204, NULL, 0, 0},
    {288, 224, scr288x224, NULL, 0, 0}, {256, 240, scr256x240, NULL, 0, 0},
    {256, 256, scr256x256, NULL, 0, 0}, {256, 256, scr256x256ver, NULL, 0, 1},
    {240, 256, scr240x256, NULL, 0, 0}, {224, 288, scr224x288, scr224x288scanlines, 0, 0},
    {200, 320, scr200x320, NULL, 0, 0}, {0, 0, NULL, NULL, 0, 0}};

static void
set_tweaked_mode(void) {
...
sysdep_display_16bpp_capable(void) {
    ...
    for (i = 1; (my_modeinfo = vga_getmodeinfo(i)); i++)
        if ((my_modeinfo->colors == 32768) || (my_modeinfo->colors == 65536))
            return 1;
...
sysdep_set_video_mode(void) {
    ...
    if (video_modeinfo.flags & IS_MODEX)
        ...
        i = video_modeinfo.width * video_modeinfo.height * video_modeinfo.bytesperpixel;
        if (i <= 65536 || (video_modeinfo.flags & IS_LINEAR)
            || (use_linear && (video_modeinfo.flags && CAPABLE_LINEAR) && vga_setlinearaddressing() >= i)) {
            video_mem = vga_getgraphmem();
            video_mem += startx * video_modeinfo.bytesperpixel;
            video_mem += starty * video_modeinfo.width * video_modeinfo.bytesperpixel;
            if ((widthscale > 1 || heightscale > 1 || yarbsize) && doublebuffer_buffer == NULL) {
                doublebuffer_buffer = malloc(scaled_visual_width * video_modeinfo.bytesperpixel);
...
#endif
        {
            ...
            could be paletised */
            if (((widthscale > 1) || (heightscale > 1) || (yarbsize) || (video_modeinfo.bytesperpixel == 2))
                && !doublebuffer_buffer) {
                doublebuffer_buffer = malloc(scaled_visual_width * scaled_visual_height * video_modeinfo.bytesperpixel);
                if (!doublebuffer_buffer) {
                    fprintf(stderr_file, "Svgalib: Error: Couldn't allocate doublebuffer buffer\n");
                    return OSD_NOT_OK;
                }
...
sysdep_create_display(int depth) {
    ...
    for (i = 1; (my_modeinfo = vga_getmodeinfo(i)); i++) {
        if (depth == 16) {
            ...
            if (my_modeinfo->colors != 256)
                continue;
            if ((my_modeinfo->flags & IS_MODEX) && (!use_planar || widthscale != 1 || heightscale != 1 || yarbsize))
                continue;
        }
        if (!vga_hasmode(i))
            continue;
    ...
    if (use_tweak && depth == 8 && vga_hasmode(G320x200x256) && vga_hasmode(G320x240x256)) {
        ...
        for (i = 0; tweaked_modes[i].width; i++) {
            if (mode_disabled(tweaked_modes[i].width, tweaked_modes[i].height, depth))
                continue;
            if ((tweaked_modes[i].planar) && (!use_planar || widthscale != 1 || heightscale != 1 || yarbsize))
                continue;
            ...
            if (score && score >= best_score) {
                ...
                video_modeinfo.width = video_modeinfo.linewidth = tweaked_modes[i].width;
    ...
    else if (video_modeinfo.colors == 32768) {
...
int
sysdep_display_set_pen(int pen, unsigned char red, unsigned char green, unsigned char blue) {
...
static void
svgalib_update_linear(struct mame_bitmap* bitmap) {
...
static void
svgalib_update_planar(struct mame_bitmap* bitmap) {
    ...
    if (page)
        ...
        page = 131072;
    vga_copytoplanar256(bitmap->line[visual.min_y] + visual.min_x, bitmap->line[1] - bitmap->line[0],
                        (starty * video_modeinfo.width + startx + page) / 4, video_modeinfo.width / 4, visual_width,
                        visual_height);
    vga_setdisplaystart(page);
}

static void
svgalib_update_gl(struct mame_bitmap* bitmap) {
    int bitmap_linewidth = (bitmap->line[1] - bitmap->line[0]) / video_modeinfo.bytesperpixel;
#define PUT_IMAGE(X, Y, WIDTH, HEIGHT)                                                                                 \
    gl_putboxpart(startx + X, starty + Y, WIDTH, HEIGHT, bitmap_linewidth, bitmap->height, bitmap->line[0],            \
                  X + visual.min_x, Y + visual.min_y);
...
static void
svgalib_update_gl_scaled(struct mame_bitmap* bitmap) {
#define SRC_PIXEL  unsigned char
#define DEST_PIXEL unsigned char
#define DEST       doublebuffer_buffer
#define DEST_WIDTH scaled_visual_width
#define PUT_IMAGE(X, Y, WIDTH, HEIGHT)                                                                                 \
    gl_putboxpart(startx + X, starty + Y, WIDTH, HEIGHT, scaled_visual_width, scaled_visual_height,                    \
...
static void
svgalib_update_linear_16bpp(struct mame_bitmap* bitmap) {
...
static void
svgalib_update_gl_16bpp(struct mame_bitmap* bitmap) {
    if (current_palette->lookup) {
...
static void
svgalib_update_gl_scaled_16bpp(struct mame_bitmap* bitmap) {
#define SRC_PIXEL  unsigned short
#define DEST_PIXEL unsigned short
#define DEST       doublebuffer_buffer
#define DEST_WIDTH scaled_visual_width
