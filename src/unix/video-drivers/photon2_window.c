...
struct rc_option ph_window_opts[] = {
    ...
    {"Photon-window Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"cursor", "nocursor", rc_bool, &show_cursor, "1", 0, 0, NULL, "Show / don't show the cursor."},
    {NULL, NULL, rc_end, NULL, NULL, 0, 0, NULL, NULL}};

struct rc_option ph_ovr_opts[] = {

    {"Photon-overlay Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"fullscreen", "window", rc_bool, &phovr_fullscreen, "1", 0, 0, NULL, "Full Screen/Windowed"},
    /*
 ...
 */
int
I_GetEvent(PtWidget_t* widget, void* data, PtCallbackInfo_t* cbinfo, int bitmap_depth) {
    int mask = FALSE;
...
int
ph_window_16bpp_capable(void) {
    return 1;
}

int
ph_window_create_display(int bitmap_depth) {
    PtArg_t arg[9];
    PhRect_t rect;
    ...
    PtSetArg(&arg[4], Pt_ARG_WINDOW_RENDER_FLAGS, Pt_FALSE,
             Ph_WM_RENDER_MENU | Ph_WM_RENDER_CLOSE | Ph_WM_RENDER_MAX | Ph_WM_RENDER_MIN | Ph_WM_RENDER_COLLAPSE
                 | Ph_WM_RENDER_RESIZE);
    ...
    PtAddEventHandler(P_mainWindow,
                      Ph_EV_BUT_PRESS | Ph_EV_BUT_RELEASE | Ph_EV_BOUNDARY | Ph_EV_EXPOSE | Ph_EV_PTR_MOTION | Ph_EV_KEY
                          | Ph_EV_INFO,
                      I_GetEvent, NULL);
...
int
ph_window_alloc_palette(int writable_colors) {
#if 0
   int i;
...
int
ph_window_modify_pen(int pen, unsigned char red, unsigned char green, unsigned char blue) {
#if 0
   PgColor_t color;
...
void
ph_window_update_display(struct mame_bitmap* bitmap) {
    PhRegion_t region_info;
...
INLINE void
ph_window_put_image(int x, int y, int width, int height) {
    PhArea_t sarea, darea;
    ...
    switch (ph_window_update_method) {
        case PH_NORMAL:
            ...
            darea.pos.x = x + startx;
            darea.pos.y = y + starty;
...
static void
ph_window_update_16_to_16bpp(struct mame_bitmap* bitmap) {
...
static void
ph_window_update_16_to_24bpp(struct mame_bitmap* bitmap) {
#define PACK_BITS
#include "blit.h"
#undef PACK_BITS
}
...
static void
ph_window_update_16_to_32bpp(struct mame_bitmap* bitmap) {
#include "blit.h"
}
...
static void
ph_window_update_32_to_32bpp_direct(struct mame_bitmap* bitmap) {
#include "blit.h"
}
