...
struct rc_option display_opts[] = {
    ...
    {"Photon Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"render-mode", NULL, rc_int, &ph_video_mode, "0", 0, PH_MODE_COUNT - 1, NULL,
     "Select Photon rendering video mode:\n0 Normal window  (hotkey left-alt + insert)\n1 Fullscreen Video Overlay "
     "(hotkey left-alt + home)"},
    {NULL, NULL, rc_link, ph_window_opts, NULL, 0, 0, NULL, NULL},
    /*
   { NULL,		NULL,			rc_link,	ph_ovr_opts,
     NULL,		0,			0,		NULL,
     NULL },
     */
    {NULL, NULL, rc_link, ph_input_opts, NULL, 0, 0, NULL, NULL},
...
struct ph_func_struct ph_func[PH_MODE_COUNT] = {
    {NULL, ph_window_create_display, ph_window_close_display, ph_window_update_display, ph_window_alloc_palette,
     ph_window_modify_pen, ph_window_16bpp_capable},
#if 1
/*
{ NULL,
...
sysdep_display_16bpp_capable(void) {
    if (ph_video_mode >= PH_MODE_COUNT) {
        fprintf(stderr_file, "info: photon-mode %d does not exist, falling back to normal window code\n",
                ph_video_mode);
        ph_video_mode = PH_WINDOW;
    }

    if (!mode_available[ph_video_mode]) {
        fprintf(stderr_file, "info: photon-mode %d not available, falling back to normal window code\n", ph_video_mode);
        ph_video_mode = PH_WINDOW;
    }
...
sysdep_update_display(struct mame_bitmap* bitmap) {
    ...
    if (new_video_mode != ph_video_mode && mode_available[new_video_mode]) {
        ...
        if ((*ph_func[new_video_mode].create_display)(bitmap_depth) != OSD_OK) {
            fprintf(stderr_file, "warning: could not create display for new photon-mode\n"
                ...
                goto barf;
            {
                ...
                osd_exit();
                sysdep_close();
                fprintf(stderr_file, "error: could not create new photon display while switching display modes\n");
...
barf:
    ...
    osd_exit();
    sysdep_close();
    fprintf(stderr_file, "error: could not create new display while switching display modes\n");
    exit(1); /* ugly, anyone know a better way ? */
}
