...
static int
xf86_dga_vidmode_find_best_vidmode(int bitmap_depth) {
    int bestmode = 0;
    ...
    for (i = 0; i < modecount; i++) {
...
#ifdef TDFX_DGA_WORKAROUND
        if (xf86ctx.modes[i].viewportWidth == modeline.hdisplay && xf86ctx.modes[i].viewportHeight == modeline.vdisplay)
        ...
        if (xf86ctx.modes[i].viewportWidth & 7)
            continue;
...
static int
xf86_dga_vidmode_setup_mode_restore(void) {
    Display* disp;
    int status;
    pid_t pid;
...
int
xf86_dga2_alloc_palette(int writable_colors) {
    XColor color;
    int i;

    if (xf86ctx.device->mode.depth != 8) {
        xf86ctx.cmap = XDGACreateColormap(display, xf86ctx.screen, xf86ctx.device, AllocNone);
    } else {
        xf86ctx.cmap = XDGACreateColormap(display, xf86ctx.screen, xf86ctx.device, AllocAll);
...
static int
xf86_dga_setup_graphics(XDGAMode modeinfo, int bitmap_depth) {
    int sizeof_pixel;

    if (bitmap_depth == 32) {
        if (depth == 32) {
            xf86ctx.xf86_dga_update_display_func = xf86_dga_update_display_32_to_32bpp_direct;
        }
    } else if (bitmap_depth == 16) {
        switch (depth) {
            case 16:
                xf86ctx.xf86_dga_update_display_func = xf86_dga_update_display_16_to_16bpp;
                break;
            case 24:
                xf86ctx.xf86_dga_update_display_func = xf86_dga_update_display_16_to_24bpp;
                break;
            case 32:
                xf86ctx.xf86_dga_update_display_func = xf86_dga_update_display_16_to_32bpp;
                break;
        }
    }
...
#if 1
    xf86ctx.addr += (((modeinfo.viewportWidth - visual_width * widthscale) / 2) & ~7) * sizeof_pixel;
    if (yarbsize)
        xf86ctx.addr += ((modeinfo.viewportHeight - yarbsize) / 2) * modeinfo.bytesPerScanline;
    else
        xf86ctx.addr += ((modeinfo.viewportHeight - visual_height * heightscale) / 2) * modeinfo.bytesPerScanline;
#endif
...
int
xf86_dga2_create_display(int bitmap_depth) {
    int bestmode;
    ...
    xf86ctx.width = xf86ctx.device->mode.bytesPerScanline * 8 / xf86ctx.device->mode.bitsPerPixel;
    ...
    if (widthscale != 1 || heightscale != 1 || yarbsize > visual_height) {
    ...
    if (use_mouse) {
        if (XGrabPointer(display, window, True, PointerMotionMask | ButtonPressMask | ButtonReleaseMask, GrabModeAsync,
    ...
    if (xf86ctx.device->mode.flags & XDGASolidFillRect) {
        XDGAFillRectangle(display, xf86ctx.screen, 0, 0, DisplayWidth(display, xf86ctx.screen),
                          DisplayHeight(display, xf86ctx.screen), BlackPixel(display, xf86ctx.screen));
        XDGASync(display, xf86ctx.screen);
    } else {
        memset(xf86ctx.base_addr, 0, xf86ctx.device->mode.bytesPerScanline * xf86ctx.device->mode.imageHeight);
    }
...
int
xf86_dga2_modify_pen(int pen, unsigned char red, unsigned char green, unsigned char blue) {
    XColor color;
    color.pixel = pen;
    color.red = red << 8;
    color.green = green << 8;
...
static void
xf86_dga_update_display_16_to_16bpp(struct mame_bitmap* bitmap) {
...
static void
xf86_dga_update_display_16_to_24bpp(struct mame_bitmap* bitmap) {
#define PACK_BITS
#include "blit.h"
#undef PACK_BITS
}
...
static void
xf86_dga_update_display_16_to_32bpp(struct mame_bitmap* bitmap) {
#include "blit.h"
}
...
static void
xf86_dga_update_display_32_to_32bpp_direct(struct mame_bitmap* bitmap) {
#include "blit.h"
}
...
void
xf86_dga2_update_display(struct mame_bitmap* bitmap) {
    (*xf86ctx.xf86_dga_update_display_func)(bitmap);
    XDGASync(display, xf86ctx.screen);
}

void
xf86_dga2_close_display(void) {
    XDGASync(display, xf86ctx.screen);
    if (xf86ctx.device) {
