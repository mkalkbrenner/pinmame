...
int
vector_vh_start(void) {
    vecwidth = (float)(Machine->drv->default_visible_area.max_x - Machine->drv->default_visible_area.min_x);

    vecheight = (float)(Machine->drv->default_visible_area.max_y - Machine->drv->default_visible_area.min_y);
...
void
PointConvert(int x, int y, float* sx, float* sy) {
    float dx, dy, tmp;
...
void
vector_add_point(int x, int y, int color, int intensity) {
    GrVertex* vert;
    FxU32 pen;
...
void
vector_vh_update(struct mame_bitmap* bitmap, int full_refresh) {}
