...
void LIBAPIENTRY
print_gl_error(const char* msg, const char* file, int line, GLenum errorcode) {
...
void LIBAPIENTRY
__sglBegin(const char* file, int line, GLenum mode) {
...
void LIBAPIENTRY
__sglEnd(const char* file, int line) {
    if (gl_begin_ctr != 1) {
 ...
 */
void LIBAPIENTRY
checkGlBeginEndBalance(const char* file, int line) {
    if (gl_begin_ctr != 0) {
...
void LIBAPIENTRY
showGlBeginEndBalance(const char* file, int line) {
...
int LIBAPIENTRY
loadGLLibrary(const char* libGLName, const char* libGLUName) {
...
#ifdef _MAC_OS9_
    returnError = GetSharedLibrary("\pOpenGLLibrary", kPowerPCCFragArch, kReferenceCFrag, &glLibConnectId,
                                   &glLibMainAddr, errName);
...
void* LIBAPIENTRY
getGLProcAddressHelper(const char* libGLName, const char* libGLUName, const char* func, int* method, int debug,
                       int verbose) {
    void* funcPtr = NULL;
    int lmethod;
    ...
    if (disp__wglGetProcAddress == NULL && __firstAccess) {
        disp__wglGetProcAddress = (PROC(CALLBACK*)(LPCSTR))GetProcAddress(hDLL_OPENGL32, "wglGetProcAddress");
   ...
   */
    ...
    if (disp__glXGetProcAddress == NULL && __firstAccess) {
        ...
        if (disp__glXGetProcAddress == NULL) {
            printf("GLINFO: cannot find glXGetProcAddress* in OpenGL library %s\n", libGLName);
            fflush(NULL);
            if (libHandleGLX != NULL) {
    ...
    funcPtr = (void*)SeekSymbolNamesByConnection(glLibConnectId, funcName);
#endif

    if (funcPtr == NULL) {
        returnError = FindSymbol(glLibConnectId, funcName, &funcPtr, &glLibSymClass);
        lmethod = 2;
    }
#ifndef NDEBUG
    else
...
void LIBAPIENTRY
fetch_GL_FUNCS(const char* libGLName, const char* libGLUName, int force) {
...
static void
PrintSymbolNamesByConnection(CFragConnectionID myConnID) {
    long myIndex;
    ...
    if (!myErr)
        for (myIndex = 1; myIndex <= myCount; myIndex++) {
            myErr = GetIndSymbol(myConnID, myIndex, myName, &myAddr, &myClass);
            if (!myErr) {
...
static Ptr
SeekSymbolNamesByConnection(CFragConnectionID myConnID, Str255 name) {
    long myIndex;
    long myCount; /*number of exported symbols in fragment*/
    OSErr myErr;
    Str255 myName;            /*symbol name*/
