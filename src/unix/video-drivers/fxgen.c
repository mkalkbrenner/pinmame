...
#define _32TO16(p) (UINT16)((((p)&0x00F80000) >> 9) | (((p)&0x0000F800) >> 6) | (((p)&0x000000F8) >> 3))

#define PIXELCOLOR(x)                                                                                                  \
    fxdepth == 15                                                                                                      \
        ? (UINT16)(x)                                                                                                  \
        : (fxdepth == 16 ? color_values[(UINT16)(x)] : (fxdepth == 24 || fxdepth == 32 ? _32TO16((UINT32)(x)) : 0))
...
static int SetResolution(struct rc_option* option, const char* arg, int priority);
...
float cscrx1, cscry1, cscrz1, cscrx2, cscry2, cscrz2, cscrx3, cscry3, cscrz3, cscrx4, cscry4, cscrz4;
float cscrwdx, cscrwdy, cscrwdz;
float cscrhdx, cscrhdy, cscrhdz;

struct rc_option fx_opts[] = {
    /* name, shortname, type, dest, deflt, min, max, func, help */
    {"FX (Glide) Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"resolution", "res", rc_use_function, NULL, "640x480", 0, 0, SetResolution,
     "Specify the resolution/ windowsize to use in the form of XRESxYRES"},
    {"keepaspect", NULL, rc_bool, &normal_use_aspect_ratio, "1", 0, 0, NULL,
...
void
CalcPoint(GrVertex* vert, int x, int y) {
    vert->x = vscrntlx + (float)x * texpercx * vscrnwidth;
    if (vert->x > vscrntlx + vscrnwidth)
        vert->x = vscrntlx + vscrnwidth;
...
InitTextures(void) {
    ...
    grTexCombine(GR_TMU0, GR_COMBINE_FUNCTION_LOCAL, GR_COMBINE_FACTOR_NONE, GR_COMBINE_FUNCTION_NONE,
    ...
    if (texnumx * texsize != visual_width)
        texnumx++;
    texnumy = visual_height / texsize;
    if (texnumy * texsize != visual_height)
        texnumy++;
    ...
    texgrid = (struct TexSquare*)malloc(texnumx * texnumy * sizeof(struct TexSquare));
...
static int
SetResolution(struct rc_option* option, const char* arg, int priority) {
    ...
    fprintf(stderr,
            ...
            arg);
    for (match = 0; match < resTableSize; match++) {
        fprintf(stderr_file, "   \"%dx%d\"", resTable[match].width, resTable[match].height);
        if (match && (match % 5) == 0)
            fprintf(stderr_file, "\n");
    }
    return -1;
...
InitVScreen(void) {
    ...
    if (!grSstWinOpen(0, Gr_resolution, GR_REFRESH_60Hz, GR_COLORFORMAT_ABGR, GR_ORIGIN_LOWER_LEFT, 2, 1)) {
        fprintf(stderr_file,
                "error opening Glide window, do you have enough memory on your 3dfx for the selected mode?\n");
        return OSD_NOT_OK;
    }
    fprintf(stderr_file, "info: screen resolution set to %dx%d\n", fxwidth, fxheight);
                    ...
                    if (emulation_paused) {
                        for (i = 0; i < width; i++) {
                            square->texture[texline * texsize + i] =
                                ((UINT16*)(bitmap->line[y]))[visual.min_x + ofs + i];
                        }
                    } else {
                        for (i = 0; i < width; i++) {
                            square->texture[texline * texsize + i] =
                                PAUSEDCOLOR((((UINT16*)(bitmap->line[y]))[visual.min_x + ofs + i]));
                        }
                    }
                }
            }
...
DrawFlatBitmap(void) {
    ...
    for (y = 0; y < texnumy; y++) {
        for (x = 0; x < texnumx; x++) {
            ...
            grTexDownloadMipMapLevel(GR_TMU0, square->texadd, GR_LOD_256, GR_LOD_256, GR_ASPECT_1x1, Gr_format,
                                     GR_MIPMAPLEVELMASK_BOTH, texinfo.data);

            grTexSource(GR_TMU0, square->texadd, GR_MIPMAPLEVELMASK_BOTH, &texinfo);
...
void
UpdateFXDisplay(struct mame_bitmap* bitmap) {
    if (bitmap)
