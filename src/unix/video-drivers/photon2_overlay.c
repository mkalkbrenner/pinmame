...
unsigned char*
setup_overlay(int src_width, int src_height, int dst_width, int dst_height, int dst_x, int dst_y) {
    void* p;
    int i = 0;
    int bCont;
...
int
ph_ovr_create_display(int bitmap_depth) {
    PtArg_t arg[8];
    PhRect_t rect;
    ...
    PtSetArg(&arg[6], Pt_ARG_WINDOW_STATE, Pt_TRUE,
             Ph_WM_STATE_ISALTKEY | Ph_WM_STATE_ISFRONT | Ph_WM_STATE_ISMAX | Ph_WM_STATE_ISFOCUS);
    ...
    PtAddEventHandler(P_mainWindow,
                      Ph_EV_BUT_PRESS | Ph_EV_BUT_RELEASE | Ph_EV_BOUNDARY | Ph_EV_EXPOSE | Ph_EV_PTR_MOTION
                          | Ph_EV_KEY,
                      I_GetEvent, NULL);
    ...
    scaled_buffer_ptr = setup_overlay(view_size.w, view_size.h, vscrnwidth, vscrnheight, vscrntlx, vscrntly);
 ...
 */
int
ph_ovr_alloc_palette(int writable_colors) {
    return 0;
}

int
ph_ovr_modify_pen(int pen, unsigned char red, unsigned char green, unsigned char blue) {
    return 0;
}

//* invoked by main tree code to update bitmap into screen */
void
ph_ovr_update_display(struct mame_bitmap* bitmap) {
    PhRegion_t region_info;
...
void
ph_ovr_refresh_screen(void) {}

INLINE void
ph_ovr_put_image(int x, int y, int width, int height) {}
...
static void
ph_ovr_update_16_to_16bpp(struct mame_bitmap* bitmap) {
...
static void
ph_ovr_update_16_to_24bpp(struct mame_bitmap* bitmap) {
#define PACK_BITS
#include "blit.h"
#undef PACK_BITS
}
...
static void
ph_ovr_update_16_to_32bpp(struct mame_bitmap* bitmap) {
#include "blit.h"
}
