...
struct rc_option display_opts[] = {
    ...
    {"x11-mode", "x11", rc_int, &x11_video_mode, "0", 0, X11_MODE_COUNT - 1, NULL,
     "Select x11 video mode: (if compiled in)\n0 Normal windowed (hotkey left-alt + insert)\n1 DGA fullscreen (hotkey "
     "left-alt + home)\n2 Xv windowed\n3 Xv fullscreen"},
...
static struct x_func_struct x_func[X11_MODE_COUNT] = {
    {NULL, x11_window_create_display, x11_window_close_display, x11_window_update_display, x11_window_alloc_palette,
     x11_window_modify_pen, x11_window_16bpp_capable},
#ifdef USE_DGA
    {xf86_dga_init, xf86_dga_create_display, xf86_dga_close_display, xf86_dga_update_display, xf86_dga_alloc_palette,
     xf86_dga_modify_pen, xf86_dga_16bpp_capable},
#else
...
sysdep_display_16bpp_capable(void) {
    if (x11_video_mode >= X11_MODE_COUNT) {
        fprintf(stderr_file, "X11-mode %d does not exist, falling back to normal window code\n", x11_video_mode);
        x11_video_mode = X11_WINDOW;
    }

    if (!mode_available[x11_video_mode]) {
        fprintf(stderr_file, "X11-mode %d not available, falling back to normal window code\n", x11_video_mode);
        x11_video_mode = X11_WINDOW;
    }
...
sysdep_update_display(struct mame_bitmap* bitmap) {
    ...
    if (new_video_mode != x11_video_mode && mode_available[new_video_mode]) {
        ...
        if ((*x_func[x11_video_mode].create_display)(bitmap_depth) != OSD_OK) {
            fprintf(stderr_file, "X11: Warning: Couldn't create display for new x11-mode\n"
...
barf:
    ...
    fprintf(stderr_file, "X11: Error: couldn't create new display while switching display modes\n");
