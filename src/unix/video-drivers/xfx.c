...
int
sysdep_init(void) {
    fprintf(stderr, "info: using FXmame v0.5 driver for xmame, written by Mike Oliphant\n");
...
sysdep_create_display(int depth) {
    ...
    if (!XMatchVisualInfo(display, myscreen, 8, PseudoColor, &myvisual)) {
        ...
        fprintf(stderr, "Using 8bpp PseudoColor Visual. Good!\n");

    colormap = XCreateColormap(display, RootWindow(display, myvisual.screen), myvisual.visual, AllocNone);
    ...
    window = XCreateWindow(
        display, RootWindowOfScreen(screen), 0, 0, fxwidth, fxheight, 0, myvisual.depth, InputOutput, myvisual.visual,
