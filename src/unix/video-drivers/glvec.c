...
VIDEO_START(vector) {
    ...
    printf("GLINFO (vec): min %d/%d, max %d/%d, cent %d/%d,\n\t vecsize %dx%d\n", vec_min_x, vec_min_y, vec_max_x,
           vec_max_y, vec_cent_x, vec_cent_y, vecwidth, vecheight);
...
int
PointConvert(int x, int y, GLdouble* sx, GLdouble* sy, GLdouble* sz) {
    int x1, y1;
    GLdouble dx, dy, tmp;
    ...
    if (!cabview) {
        ...
        *sy = dy;
        ...
        CalcCabPointbyViewpoint(dx * s__cscr_w_view, dy * s__cscr_h_view, sx, sy, sz);
    }

    if (0 <= *sx && *sx <= 1.0 && 0 <= *sy && *sy <= 1.0)
        return 1;
