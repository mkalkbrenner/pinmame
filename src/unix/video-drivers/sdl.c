...
struct rc_option display_opts[] = {
    ...
    {"fullscreen", NULL, rc_bool, &start_fullscreen, "0", 0, 0, NULL, "Start fullscreen"},
    {"listmodes", NULL, rc_use_function_no_arg, NULL, NULL, 0, 0, list_sdl_modes, "List all posible fullscreen modes"},
    {"modenumber", NULL, rc_int, &mode_number, "-1", 0, 0, NULL,
...
sysdep_create_display(int depth) {
    ...
    fprintf(stderr,
            ...
            video_info->blit_hw, video_info->blit_sw, video_info->video_mem, video_info->vfmt->BitsPerPixel);
#endif
 ...
 */
static int
sdl_mapkey(struct rc_option* option, const char* arg, int priority) {
...
/* Update routines */
void
sdl_update_16_to_16bpp(struct mame_bitmap* bitmap) {
...
void
sdl_update_16_to_24bpp(struct mame_bitmap* bitmap) {
#define SRC_PIXEL  unsigned short
...
void
sdl_update_16_to_32bpp(struct mame_bitmap* bitmap) {
#define INDIRECT   current_palette->lookup
#define SRC_PIXEL  unsigned short
#define DEST_PIXEL unsigned int
#define DEST       Offscreen_surface->pixels
...
void
sdl_update_rgb_direct_32bpp(struct mame_bitmap* bitmap) {
...
sysdep_update_display(struct mame_bitmap* bitmap) {
    ...
    switch (use_dirty) {
        ...
        case 0:
            ...
            Hermes_ConverterCopy(H_ConverterHandle, bitmap->line[0], 0, 0, Vid_width, Vid_height,
                                 bitmap->line[1] - bitmap->line[0], Surface->pixels, 0, 0, Vid_width, Vid_height,
                                 Vid_width << 1);
                ...
                for (y = 0; y < h; y++) {
                    for (i = 0; i < 8; i++) {
                        ...
                        for (x = 0; x < w; x++) {
                            for (j = 0; j < 8; j++) {
                                int col_off = ((x << 3) + j);
                                if (*(bak_bitmap + (line_off * (bitmap->line[1] - bitmap->line[0])) + col_off)
                                    != *(*(bitmap->line + line_off) + col_off)) {
                                    if (!dirty_blocks[y][x]) {
                ...
                if (dirty_lines[y]) {
                    ...
                    for (x = 0; x < w; x++) {
                        if (dirty_blocks[y][x]) {
                            ...
                            Hermes_ConverterCopy(H_ConverterHandle, bitmap->line[0], col_min, line_min, col_amount,
                                                 line_amount, bitmap->line[1] - bitmap->line[0], Surface->pixels,
                                                 col_min, line_min, col_amount, line_amount, Vid_width << 1);
 ...
 */
int
sysdep_display_alloc_palette(int totalcolors) {
    int ncolors;
    int i;
    ncolors = totalcolors;
...
int
list_sdl_modes(struct rc_option* option, const char* arg, int priority) {
    SDL_Rect** vid_modes;
    int vid_modes_i;
    ...
    while (*(vid_modes + vid_modes_i)) {
        printf("\t%d) Mode %d x %d\n", vid_modes_i, (*(vid_modes + vid_modes_i))->w, (*(vid_modes + vid_modes_i))->h);
