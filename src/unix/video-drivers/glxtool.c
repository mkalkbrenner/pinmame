 ...
 */
void LIBAPIENTRY
fetch_GLX_FUNCS(const char* libGLName, const char* libGLUName, int force) {
 ...
 */
int LIBAPIENTRY
get_GC(Display* display, Window win, XVisualInfo* visual, GLXContext* gc, GLXContext gc_share, int verbose) {
    int trial = 2;
...
int LIBAPIENTRY
setVisualAttribListByGLCapabilities(int visualAttribList[/*>=32*/], GLCapabilities* glCaps) {
    int i = 0;
 ...
 */
VisualGC LIBAPIENTRY
findVisualGlX(Display* display, Window rootWin, Window* pWin, int width, int height, GLCapabilities* glCaps,
              int* pOwnWin, XSetWindowAttributes* pOwnWinAttr, unsigned long ownWinmask, GLXContext shareWith,
              int offscreen, Pixmap* pix, int verbose) {
    Window newWin = 0;
    ...
    do {
		 ...
		 */
            vgc.visual = findVisualIdByID(&visualList, (int)(glCaps->nativeVisualID), display, *pWin, verbose);
        }
        ...
        if (tryChooseVisual && vgc.visual == NULL) {
            vgc.visual = disp__glXChooseVisual(display, DefaultScreen(display), visualAttribList);
            if (verbose) {
                if (vgc.visual != NULL) {
                    fprintf(stdout, "findVisualGlX.glXChooseVisual: found visual(ID:%d(0x%X))\n",
                            (int)vgc.visual->visualid, (int)vgc.visual->visualid);
        ...
        if (vgc.visual == NULL) {
            vgc.visual = findVisualIdByFeature(&visualList, display, *pWin, glCaps, verbose);
        }
        ...
        if (ownwin && vgc.visual != NULL) {
            ...
            newWin = createOwnOverlayWin(display, rootWin, *pWin /* the parent */, pOwnWinAttr, ownWinmask, vgc.visual,
                                         width, height);
        }
        ...
        if (offscreen && vgc.visual != NULL) {
            ...
            if (pix != NULL && vgc.visual != NULL)
                *pix = XCreatePixmap(display, rootWin, width, height, vgc.visual->depth);
            if (pix != NULL && *pix != 0) {
        ...
        if (ownwin && newWin != 0 && gc_ret == -100 /* just a poss. test stage */
            && (gc_ret = get_GC(display, newWin, vgc.visual, &(vgc.gc), shareWith, verbose)) == 0) {
            vgc.success = 1;
            *pWin = newWin;
        } else if (vgc.visual != NULL && !ownwin && *pWin != 0
                   && (gc_ret = get_GC(display, *pWin, vgc.visual, &(vgc.gc), shareWith, verbose)) == 0) {
            vgc.success = 1;
        } else {
            j++; /* trial counter */
            if (verbose) {
            ...
            if (ownwin && newWin != 0) {
                ...
                destroyOwnOverlayWin(display, &newWin, pOwnWinAttr);
            }
            ...
            } else if (glCaps->alphaBits > 0 || glCaps->accumAlphaBits > 0) {
                glCaps->alphaBits = 0;
                glCaps->accumAlphaBits = 0;
            } else if (accumTestDone == 0
                       && (glCaps->accumRedBits == 0 || glCaps->accumGreenBits == 0 || glCaps->accumBlueBits == 0)) {
                glCaps->accumRedBits = 1;
                glCaps->accumGreenBits = 1;
                glCaps->accumBlueBits = 1;
            } else if (glCaps->accumRedBits > 0 || glCaps->accumGreenBits > 0 || glCaps->accumBlueBits > 0) {
                glCaps->accumRedBits = 0;
                glCaps->accumGreenBits = 0;
                glCaps->accumBlueBits = 0;
                accumTestDone = 1;
            } else if (glCaps->buffer == BUFFER_SINGLE && offscreen) {
    ...
    if (vgc.success == 1 && verbose) {
        ...
        fprintf(stderr,
                ...
                "shared with %d\n",
                (int)vgc.visual->visualid, (int)vgc.visual->screen, (int)vgc.visual->depth, (int)vgc.visual->class,
                (int)vgc.visual->colormap_size, (int)vgc.visual->bits_per_rgb, (int)shareWith);
    }

    return vgc;
}

XVisualInfo* LIBAPIENTRY
findVisualIdByID(XVisualInfo** visualList, int visualID, Display* disp, Window win, int verbose) {
    XVisualInfo* vi = 0;
    ...
    *visualList = XGetVisualInfo(disp, VisualScreenMask, &viTemplate, &numReturns);
...
XVisualInfo* LIBAPIENTRY
findVisualIdByFeature(XVisualInfo** visualList, Display* disp, Window win, GLCapabilities* glCaps, int verbose) {
    XVisualInfo* vi = 0;
    XVisualInfo viTemplate;
    int i, numReturns;
    XWindowAttributes xwa;
...
int LIBAPIENTRY
setGLCapabilities(Display* disp, XVisualInfo* visual, GLCapabilities* glCaps) {
    int iValue = 0;
    int iValue1 = 0;
    int iValue2 = 0;
    int iValue3 = 0;
...
int LIBAPIENTRY
testVisualInfo(Display* display, XVisualInfo* vi, GLCapabilities* glCaps, int verbose) {
    GLCapabilities _glCaps;
    ...
    */
    ...
    if (_glCaps.gl_supported != glCaps->gl_supported)
        return 0;

    if (_glCaps.buffer != glCaps->buffer)
        return 0;

    if (_glCaps.color != glCaps->color)
        return 0;

    if (_glCaps.stereo != glCaps->stereo)
        return 0;

    if ((glCaps->stencilBits > 0 && _glCaps.stencilBits < glCaps->stencilBits)
        || (glCaps->stencilBits == 0 && _glCaps.stencilBits != glCaps->stencilBits))
        return 0;

    if ((glCaps->accumRedBits > 0 && _glCaps.accumRedBits < glCaps->accumRedBits)
        || (glCaps->accumRedBits == 0 && _glCaps.accumRedBits != glCaps->accumRedBits))
        return 0;

    if ((glCaps->accumGreenBits > 0 && _glCaps.accumGreenBits < glCaps->accumGreenBits)
        || (glCaps->accumGreenBits == 0 && _glCaps.accumGreenBits != glCaps->accumGreenBits))
        return 0;

    if ((glCaps->accumBlueBits > 0 && _glCaps.accumBlueBits < glCaps->accumBlueBits)
        || (glCaps->accumBlueBits == 0 && _glCaps.accumBlueBits != glCaps->accumBlueBits))
        return 0;

    if ((glCaps->alphaBits > 0 && _glCaps.alphaBits < glCaps->alphaBits)
        || (glCaps->alphaBits == 0 && _glCaps.alphaBits != glCaps->alphaBits))
        return 0;

    if (glCaps->alphaBits > 0) {
        if ((glCaps->accumAlphaBits > 0 && _glCaps.accumAlphaBits < glCaps->accumAlphaBits)
            || (glCaps->accumAlphaBits == 0 && _glCaps.accumAlphaBits != glCaps->accumAlphaBits))
            return 0;
    }
...
void LIBAPIENTRY
printVisualInfo(Display* display, XVisualInfo* vi) {
    GLCapabilities glCaps;

    setGLCapabilities(display, vi, &glCaps);

    fprintf(stdout, "\nvi(ID:%d(0x%X)): \n screen %d, depth %d, class %d,\n clrmapsz %d, bitsPerRGB %d\n",
            (int)vi->visualid, (int)vi->visualid, (int)vi->screen, (int)vi->depth, (int)vi->class,
            (int)vi->colormap_size, (int)vi->bits_per_rgb);

    printGLCapabilities(&glCaps);
}

void LIBAPIENTRY
printAllVisualInfo(Display* disp, Window win, int verbose) {
    ...
    visualInfo = XGetVisualInfo(disp, VisualScreenMask, &viTemplate, &numReturns);
...
Window LIBAPIENTRY
createOwnOverlayWin(Display* display, Window rootwini, Window parentWin, XSetWindowAttributes* pOwnWinAttr,
                    unsigned long ownWinmask, XVisualInfo* visual, int width, int height) {
    /*
  //------------------------------------------------------------------------//
  ...
  */
    if (attribs.colormap == 0 && (winmask & CWColormap) > 0)
        attribs.colormap = XCreateColormap(display, rootwini, visual->visual, AllocNone);
  ...
  */
    ...
    //----------------------------------------------------------------------//
  */
    window = XCreateWindow(display, parentWin, 0, 0, width, height, 0, visual->depth, InputOutput, visual->visual,
                           winmask, &attribs);
...
void LIBAPIENTRY
destroyOwnOverlayWin(Display* display, Window* newWin, XSetWindowAttributes* pOwnWinAttr) {
    if (newWin != NULL) {
...
int LIBAPIENTRY
x11gl_myErrorHandler(Display* pDisp, XErrorEvent* p_error) {
    char err_msg[80];
...
int LIBAPIENTRY
x11gl_myIOErrorHandler(Display* pDisp) {
