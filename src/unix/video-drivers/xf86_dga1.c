...
xf86_dga1_init(void) {
    ...
    else if (!XF86DGAGetVideo(display, xf86ctx.screen, &xf86ctx.base_addr, &xf86ctx.width, &xf86ctx.bank_size,
                              &xf86ctx.ram_size))
...
xf86_dga_vidmode_find_best_vidmode(int depth) {
    XF86VidModeModeInfo **modes, *bestmode = NULL;
    int score, best_score = 0;
    int i, modecount = 0;

    if (!XF86VidModeGetAllModeLines(display, xf86ctx.screen, &modecount, &modes)) {
...
static Bool
xf86_dga_vidmode_getmodeinfo(XF86VidModeModeInfo* modeinfo) {
...
static void
xf86_dga_vidmode_restoremode(Display* disp) {
...
int
xf86_dga1_alloc_palette(int writable_colors) {
    XColor color;
    int i;
...
static int
xf86_dga_setup_graphics(XF86VidModeModeInfo* modeinfo, int bitmap_depth) {
    int sizeof_pixel;
    ...
    xf86ctx.addr += (((modeinfo->hdisplay - visual_width * widthscale) / 2) & ~7) * sizeof_pixel;
    if (yarbsize)
        xf86ctx.addr += ((modeinfo->vdisplay - yarbsize) / 2) * xf86ctx.width * sizeof_pixel;
    else
        xf86ctx.addr += ((modeinfo->vdisplay - visual_height * heightscale) / 2) * xf86ctx.width * sizeof_pixel;
...
int
xf86_dga1_create_display(int bitmap_depth) {
    int i, count;
    ...
    if (!XF86DGADirectVideo(display, xf86ctx.screen, XF86DGADirectGraphics | XF86DGADirectMouse | XF86DGADirectKeyb)) {
...
int
xf86_dga1_modify_pen(int pen, unsigned char red, unsigned char green, unsigned char blue) {
    XColor color;
    color.pixel = pen;
    color.red = red << 8;
    color.green = green << 8;
...
void
xf86_dga1_update_display(struct mame_bitmap* bitmap) {
