...
/* some prototypes */
static int video_handle_scale(struct rc_option* option, const char* arg, int priority);
static int video_verify_beam(struct rc_option* option, const char* arg, int priority);
static int video_verify_flicker(struct rc_option* option, const char* arg, int priority);
static int video_verify_intensity(struct rc_option* option, const char* arg, int priority);
static int video_verify_bpp(struct rc_option* option, const char* arg, int priority);
static int video_verify_vectorres(struct rc_option* option, const char* arg, int priority);

#ifndef xgl
static void adjust_bitmap_and_update_display(struct mame_bitmap* srcbitmap, struct rectangle bounds);
#endif
...
struct rc_option video_opts[] = {
    ...
    {"Video Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"bpp", "b", rc_int, &options.color_depth, "0", 0, 0, video_verify_bpp,
     "Specify the colordepth the core should render, one of: auto(0), 8, 16"},
    {"arbheight", "ah", rc_int, &yarbsize, "0", 0, 4096, NULL, "Scale video to exactly this height (0 = disable)"},
    {"heightscale", "hs", rc_int, &normal_heightscale, "1", 1, 8, NULL, "Set Y-Scale aspect ratio"},
    {"widthscale", "ws", rc_int, &normal_widthscale, "1", 1, 8, NULL, "Set X-Scale aspect ratio"},
    {"scale", "s", rc_use_function, NULL, NULL, 0, 0, video_handle_scale,
     "Set X-Y Scale to the same aspect ratio. For vector games scale (and also width- and heightscale) may have "
     "value's like 1.5 and even 0.5. For scaling of regular games this will be rounded to an int"},
    {"effect", "ef", rc_int, &effect, EFFECT_NONE, EFFECT_NONE, EFFECT_LAST, NULL,
     "Video effect:\n"
     "0 = none (default)\n"
    ...
    {"autodouble", "adb", rc_bool, &use_auto_double, "1", 0, 0, NULL,
     "Enable/disable automatic scale doubling for 1:2 pixel aspect ratio games"},
    {"scanlines", "sl", rc_bool, &use_scanlines, "0", 0, 0, NULL, "Enable/disable displaying simulated scanlines"},
    {"artwork", "art", rc_bool, &use_artwork, "1", 0, 0, NULL,
    ...
    {"frameskipper", "fsr", rc_int, &frameskipper, "1", 0, FRAMESKIP_DRIVER_COUNT - 1, NULL,
    ...
    {"throttle", "th", rc_bool, &throttle, "1", 0, 0, NULL, "Enable/disable throttle"},
    {"frames_to_run", "ftr", rc_int, &frames_to_display, "0", 0, 0, NULL,
     "Sets the number of frames to run within the game"},
    {"sleepidle", "si", rc_bool, &sleep_idle, "0", 0, 0, NULL, "Enable/disable sleep during idle"},
    {"autoframeskip", "afs", rc_bool, &autoframeskip, "1", 0, 0, NULL, "Enable/disable autoframeskip"},
    {"maxautoframeskip", "mafs", rc_int, &max_autoframeskip, "8", 0, FRAMESKIP_LEVELS - 1, NULL,
     "Set highest allowed frameskip for autoframeskip"},
    {"frameskip", "fs", rc_int, &frameskip, "0", 0, FRAMESKIP_LEVELS - 1, NULL,
     "Set frameskip when not using autoframeskip"},
    {"brightness", "brt", rc_float, &options.brightness, "1.0", 0.5, 2.0, NULL,
    ...
    {"gamma", "gc", rc_float, &options.gamma, "1.0", 0.5, 2.0, NULL, "Set the gamma correction (0.5 - 2.0)"},
    {"norotate", "nr", rc_bool, &video_norotate, "0", 0, 0, NULL, "Do not apply rotation"},
    {"ror", "rr", rc_bool, &video_ror, "0", 0, 0, NULL, "Rotate screen clockwise"},
    {"rol", "rl", rc_bool, &video_rol, "0", 0, 0, NULL, "Rotate screen counter-clockwise"},
    {"autoror", NULL, rc_bool, &video_autoror, "0", 0, 0, NULL,
     "Automatically rotate screen clockwise for vertical games"},
    {"autorol", NULL, rc_bool, &video_autorol, "0", 0, 0, NULL,
     "Automatically rotate screen counter-clockwise for vertical games"},
    {"flipx", "fx", rc_bool, &video_flipx, "0", 0, 0, NULL, "Flip screen left-right"},
    {"flipy", "fy", rc_bool, &video_flipy, "0", 0, 0, NULL, "Flip screen upside-down"},
    {"Vector Games Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"vectorres", "vres", rc_string, &vector_res, NULL, 0, 0, video_verify_vectorres,
    ...
    {"intensity", NULL, rc_float, &f_intensity, "1.5", 0.5, 3.0, video_verify_intensity,
     "Set intensity in vector games"},
    {"antialias", "aa", rc_bool, &options.antialias, "1", 0, 0, NULL, "Enable/disable antialiasing"},
    {"translucency", "t", rc_bool, &options.translucency, "1", 0, 0, NULL, "Enable/disable tranlucency"},
    {NULL, NULL, rc_link, display_opts, NULL, 0, 0, NULL, NULL},
    {NULL, NULL, rc_end, NULL, NULL, 0, 0, NULL, NULL}};

#ifndef LISY_VIDEO
static int
video_handle_scale(struct rc_option* option, const char* arg, int priority) {
...
static int
video_verify_beam(struct rc_option* option, const char* arg, int priority) {
...
static int
video_verify_flicker(struct rc_option* option, const char* arg, int priority) {
...
static int
video_verify_intensity(struct rc_option* option, const char* arg, int priority) {
...
static int
video_verify_bpp(struct rc_option* option, const char* arg, int priority) {
    if (options.color_depth != 0 && options.color_depth != 8 && options.color_depth != 15 && options.color_depth != 16
...
static int
video_verify_vectorres(struct rc_option* option, const char* arg, int priority) {
...
osd_video_initpre() {
    ...
    if (video_ror) {
        /* if only one of the components is inverted, switch them */
        if ((orientation & ROT180) == ORIENTATION_FLIP_X || (orientation & ROT180) == ORIENTATION_FLIP_Y)
            orientation ^= ROT180;
...
void
orient_rect(struct rectangle* rect) {
    int temp;
...
int
osd_create_display(const struct osd_create_params* params, UINT32* rgb_components) {
    int r, g, b;
    ...
    if (use_auto_double) {
        if ((params->video_attributes & VIDEO_PIXEL_ASPECT_RATIO_MASK) == VIDEO_PIXEL_ASPECT_RATIO_1_2) {
        ...
        if ((params->video_attributes & VIDEO_PIXEL_ASPECT_RATIO_MASK) == VIDEO_PIXEL_ASPECT_RATIO_2_1) {
            if (params->orientation & ORIENTATION_SWAP_XY)
                normal_heightscale *= 2;
            else
                normal_widthscale *= 2;
    ...
    if (using_15bpp_rgb_direct) {
        for (r = 0; r < 32; r++)
            for (g = 0; g < 32; g++)
                for (b = 0; b < 32; b++) {
                    int idx = (r << 10) | (g << 5) | b;
                    sysdep_palette_set_pen(normal_palette, idx, (r << 3) | (r >> 2), (g << 3) | (g >> 2),
                                           (b << 3) | (b >> 2));
                }
    } else {
        for (r = 0; r < 32; r++)
...
osd_close_display(void) {
    ...
    if (frames_displayed != 0) {
        ...
        fprintf(stderr_file, "Average FPS: %f (%d frames)\n", (double)cps / (end_time - start_time) * frames_displayed,
                frames_displayed);
    }
}

static void
osd_change_display_settings(struct rectangle* new_visual, struct sysdep_palette_struct* new_palette, int new_widthscale,
    ...
    if (visual_width != new_visual_width || visual_height != new_visual_height || widthscale != new_widthscale
        || heightscale != new_heightscale || use_aspect_ratio != new_use_aspect_ratio) {
        sysdep_display_close();
...
static void
round_rectangle_to_8(struct rectangle* rect) {
    if (rect->min_x & 7) {
...
static void
update_visible_area(struct mame_display* display) {
    ...
    if (!debugger_has_focus)
        osd_change_display_settings(&normal_visual, normal_palette, normal_widthscale, normal_heightscale,
                                    normal_use_aspect_ratio);

    set_ui_visarea(display->game_visible_area.min_x, display->game_visible_area.min_y, display->game_visible_area.max_x,
                   display->game_visible_area.max_y);
}

static void
update_palette(struct mame_display* display, int force_dirty) {
    int i, j;
        ...
        if (dirtyflags || force_dirty) {
            ...
            for (j = 0; (j < 32) && (i + j < display->game_palette_entries); j++, dirtyflags >>= 1)
                if (((dirtyflags & 1) || force_dirty) && (i + j < display->game_palette_entries)) {
                    ...
                    sysdep_palette_set_pen(current_palette, i + j, r, g, b);
...
static void
update_debug_display(struct mame_display* display) {
    ...
    if (!debug_palette) {
        ...
        for (i = 0; i < DEBUGGER_TOTAL_COLORS; i++) {
            ...
            sysdep_palette_set_pen(debug_palette, i, r, g, b);
        }
    }
...
static skip_next_frame_func skip_next_frame_functions[FRAMESKIP_DRIVER_COUNT] = {dos_skip_next_frame,
                                                                                 barath_skip_next_frame};

typedef int (*show_fps_frame_func)(char* buffer);
static show_fps_frame_func show_fps_frame_functions[FRAMESKIP_DRIVER_COUNT] = {dos_show_fps, barath_show_fps};

int
osd_skip_this_frame(void) {
    return skip_next_frame;
}

int
osd_get_frameskip(void) {
    return autoframeskip ? -(frameskip + 1) : frameskip;
}

void
change_debugger_focus(int new_debugger_focus) {
    if ((!debugger_has_focus && new_debugger_focus) || (debugger_has_focus && !new_debugger_focus)) {
        if (new_debugger_focus)
            osd_change_display_settings(&debug_visual, debug_palette, 1, 1, 0);
        else
            osd_change_display_settings(&normal_visual, normal_palette, normal_widthscale, normal_heightscale,
                                        normal_use_aspect_ratio);
...
/* Update the display. */
void
osd_update_video_and_audio(struct mame_display* display) {
    int skip_this_frame;
    cycles_t curr;
    ...
    if (input_ui_pressed(IPT_UI_THROTTLE)) {
        if (!keyboard_pressed(KEYCODE_LSHIFT) && !keyboard_pressed(KEYCODE_RSHIFT)) {
			 ...
			 */
            frames_displayed = 0;
        } else if (keyboard_pressed(KEYCODE_RSHIFT) || keyboard_pressed(KEYCODE_LSHIFT))
            sleep_idle ^= 1;
    }
    ...
    if (skip_this_frame == 0 && (display->changed_flags & GAME_BITMAP_CHANGED) && !debugger_has_focus) {
...
#else
        adjust_bitmap_and_update_display(display->game_bitmap, updatebounds);
#endif
...
#ifndef xgl
void
adjust_bitmap_and_update_display(struct mame_bitmap* srcbitmap, struct rectangle bounds) {
    orient_rect(&bounds);
...
osd_override_snapshot(struct mame_bitmap* bitmap, struct rectangle* bounds) {
    ...
    if (blit_swapxy) {
        t = newbounds.min_x;
        newbounds.min_x = newbounds.min_y;
        newbounds.min_y = t;
        t = newbounds.max_x;
        newbounds.max_x = newbounds.max_y;
        newbounds.max_y = t;
    }
...
void
osd_pause(int paused) {
    emulation_paused = paused;
}

const char*
osd_get_fps_text(const struct performance_info* performance) {
    static char buffer[1024];
    char* dest = buffer;

    int chars_filled = (*show_fps_frame_functions[frameskipper])(dest);

    if (chars_filled)
        dest += chars_filled;
    else {
        /* display the FPS, frameskip, percent, fps and target fps */
        dest += sprintf(dest, "%s%s%s%2d%4d%%%4d/%d fps", throttle ? "T " : "", (throttle && sleep_idle) ? "S " : "",
                        autoframeskip ? "auto" : "fskp", frameskip, (int)(performance->game_speed_percent + 0.5),
                        (int)(performance->frames_per_second + 0.5), (int)(Machine->drv->frames_per_second + 0.5));
    }
...
void
throttle_speed_part(int part, int totalparts) {
