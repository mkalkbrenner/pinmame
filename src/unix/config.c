...
static int
specify_ram(struct rc_option* option, const char* arg, int priority) {
...
#ifdef MAME_DEBUG
static int config_handle_debug_size(struct rc_option* option, const char* arg, int priority);
#endif
...
static struct rc_option opts[] = {
    ...
    {"ramsize", "ram", rc_string, &mess_opts, NULL, 0, 0, specify_ram,
     "Specifies size of RAM (if supported by driver)"},
#else
    {"MAME Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"defaultgame", "def", rc_string, &defaultgamename, "pacman", 0, 0, NULL,
    ...
    {"fuzzycmp", "fc", rc_bool, &use_fuzzycmp, "1", 0, 0, NULL,
    ...
    {"skip_disclaimer", NULL, rc_bool, &options.skip_disclaimer, "0", 0, 0, NULL,
    ...
    {"debug-size", "ds", rc_use_function, NULL, "640x480", 0, 0, config_handle_debug_size,
     "Specify the resolution/window size to use for the debugger (window) in the form of XRESxYRES (minimum size = "
     "640x480)"},
#endif
    ...
    {"manhelp", "mh", rc_set_int, &showmanusage, NULL, 1, 0, NULL,
...
static int
fuzzycmp(const char* s, const char* l) {
    int gaps = 0;
    int match = 0;
    int last = 1;
...
static int
config_printf(const char* fmt, ...) {
    return 0;
}
#endif

static int
config_handle_arg(char* arg) {
...
#ifdef MAME_DEBUG
static int
config_handle_debug_size(struct rc_option* option, const char* arg, int priority) {
    int width, height;
    ...
    fprintf(stderr, "error: invalid debugger size or too small (minimum size = 640x480): \"%s\".\n", arg);
    return -1;
}
#endif /* MAME_DEBUG */
...
config_init(int argc, char* argv[]) {
    ...
    if (loadconfig) {
        ...
        if (rc_load(rc, buffer, 1, 1))
            return OSD_NOT_OK;
        snprintf(buffer, BUF_SIZE, "%s/.%s/%s-%src", home_dir, NAME, NAME, DISPLAY_METHOD);
        if (rc_load(rc, buffer, 1, 1))
            return OSD_NOT_OK;
    }
...
#else
    {
        ...
        for (i = 0; drivers[i]; i++) {
            if (strncasecmp(begin, drivers[i]->name, len) == 0 && len == strlen(drivers[i]->name)) {
    ...
    if ((game_index == -1) && use_fuzzycmp) {
        ...
        if (game_index != -1)
            fprintf(stdout_file, "fuzzy name compare, running %s\n", drivers[game_index]->name);
    }
    ...
    if (loadconfig) {
        snprintf(buffer, BUF_SIZE, "%s/rc/%src", XMAMEROOT, drivers[game_index]->name);
        if (rc_load(rc, buffer, 1, 1))
            return OSD_NOT_OK;
        snprintf(buffer, BUF_SIZE, "%s/.%s/rc/%src", home_dir, NAME, drivers[game_index]->name);
        if (rc_load(rc, buffer, 1, 1))
            return OSD_NOT_OK;
    }
...
show_usage(void) {
    ...
    fprint_columns(stdout_file, XMAMEROOT "/" NAME "rc", "Global configuration config file");
    fprint_columns(stdout_file, "${HOME}/." NAME "/" NAME "rc", "User configuration config file");
    fprint_columns(stdout_file, XMAMEROOT "/" NAME "-" DISPLAY_METHOD "rc", "Global per display method config file");
    fprint_columns(stdout_file, "${HOME}/." NAME "/" NAME "-" DISPLAY_METHOD "rc",
                   "User per display method config file");
    fprint_columns(stdout_file, XMAMEROOT "/rc/<game>rc", "Global per game config file");
    fprint_columns(stdout_file, "${HOME}/." NAME "/rc/<game>rc", "User per game config file");
 ...
 */
static int
add_device(struct rc_option* option, const char* arg, int priority) {
    int id;
