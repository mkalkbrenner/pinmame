...
#ifdef __GNUC__
#if PROTOCOL_DEBUG >= 3
#define _CHATTY_LOG(format, args...) fprintf(stderr_file, "%s:%d: " format "\n", __FUNCTION__, __LINE__, ##args)
#else
#define _CHATTY_LOG(format, args...) fprintf(stderr_file, format "\n", ##args)
#endif
#define _CHATTY_ERRNO_LOG(format, args...) _CHATTY_LOG(format " (errno %d: %s)", ##args, errno, strerror(errno))
#else
/* Fix this at some point */
#error "GCC required to compile NetMAME"
#endif

#define _PTR_SWAP(a, b)                                                                                                \
    do {                                                                                                               \
        void* _scratch_ptr = a;                                                                                        \
        a = b;                                                                                                         \
        b = _scratch_ptr;                                                                                              \
...
enum net_block_type { NVRAM_BLOCK = 1, MACHINESTATE_BLOCK };
...
enum net_peer_state { INACTIVE = 1, UNSYNCED, SYNCED };

enum net_protocol_state { REGISTRATION = 1, INPUT_EXCHANGE, ACKNOWLEDGEMENT, BLOCK_TRANSFER, QUITTING };
        ...
        *port_str = 0;
        ...
        if (port_str[0] != 0) {
            ...
            if (scratch < 1 || scratch > 65535) {
                ...
                _master_info.addr.sin_port = htons((unsigned short)atoi(port_str));
                port_ok = TRUE;
            }
        }
    } else {
        _master_info.addr.sin_port = htons((unsigned short)atoi(DEFAULT_BIND_PORT));
        port_ok = TRUE;
    }
...
#ifdef XMAME_NET
    {"Network Related", NULL, rc_seperator, NULL, NULL, 0, 0, NULL, NULL},
    {"master", NULL, rc_int, &_original_player_count, NULL, 2, NET_MAX_PLAYERS, NULL,
     "Enable master mode. Set number of players"},
    {"slave", NULL, rc_use_function, NULL, NULL, 0, 0, _parse_master_addr,
     "Enable slave mode. Set master hostname[:port]"},
    {"bind", NULL, rc_int, &_bind_port, DEFAULT_BIND_PORT, 1, 65535, NULL,
     "Specify a UDP port on which to accept messages"},
    {"netmapkey", NULL, rc_bool, &_input_remap, "1", 0, 0, NULL,
     "Makes player 1 keys control whichever player number you're actually assigned by the master (for slave only)"},
    {"parallelsync", NULL, rc_bool, &_parallel_sync, "1", 0, 0, NULL,
     "Perform network input sync in advance:  Causes ~ 16 ms input delay but more suitable for relatively slow "
     "machines/networks (for master only)"},
    {"statedebug", NULL, rc_bool, &_machine_state_debug, "0", 0, 0, NULL,
...
static unsigned _minimum_frame_for_valid_sma = SYNC_TIME_SMA_IGNORE_COUNT + SYNC_TIME_SMA_POINTS;
...
_init_sockets(void) {
    ...
    if (_socket_fd = socket(AF_INET, SOCK_DGRAM, 0), _socket_fd < 0) {
        _CHATTY_ERRNO_LOG("Can't create socket");
    } else if (scratch = fcntl(_socket_fd, F_SETFL, O_NONBLOCK), scratch == -1) {
        ...
        /* bind socket */
        if (bind(_socket_fd, (struct sockaddr*)&input_addr, sizeof(input_addr)) == -1) {
...
static void
_prime_msg(struct net_msg_info* msg_info, enum net_msg_type msg_type, UINT32 sequence) {
...
static int
_write_to_msg(struct net_msg_info* msg_info, const void* new_data, unsigned new_data_len) {
    int result = FALSE;
...
static void
_copy_msg(struct net_msg_info* src_msg, struct net_msg_info* dest_msg) {
    memcpy(dest_msg, src_msg, sizeof(struct net_msg_info));
    if (src_msg->data_start != NULL) {
        dest_msg->data_start = dest_msg->msg + (src_msg->data_start - src_msg->msg);
    }
}
...
_build_block_prompt_msg(struct net_msg_info* msg_info) {
    ...
    UINT32 block_part = 0;
    unsigned total_parts_requested = (_block_info.remaining_part_count <= MAX_REQUESTED_PARTS)
    ...
    while (block_part < _block_info.total_part_count && parts_requested < total_parts_requested) {
...
static int
_send_msg(struct net_msg_info* msg_info, struct sockaddr_in* dest) {
    int result = FALSE;
    if (sendto(_socket_fd, msg_info->msg, (msg_info->data_start - msg_info->msg) + msg_info->data_len, 0,
...
_new_peer_with_addr(struct sockaddr_in* addr) {
    unsigned char i = 0;
    struct net_peer_info* result = NULL;
    while (i < _original_player_count - 1 && _peer_info[i].state != INACTIVE) {
        i += 1;
    }
    if (i < _original_player_count - 1) {
        memcpy(&(_peer_info[i].addr), addr, sizeof(_peer_info[i].addr));
        result = &(_peer_info[i]);
    }
    return result;
}
...
static int
_send_msg_to_peer(struct net_msg_info* msg_info, struct net_peer_info* dest) {
    int result = FALSE;
...
static int
_ok_to_resend(struct net_msg_info* msg_info, struct net_peer_info* dest, unsigned minimum_time_lapse) {
    int result = FALSE;
...
static int
_send_block_part_to_peer(unsigned block_part, struct net_peer_info* dest) {
    unsigned part_size = _block_part_size(block_part);
    UINT32 scratch = (UINT32)ntohl(block_part);
    _prime_msg(&_outbound_scratch_msg_info, BLOCK_PART, _current_sync_sequence);
    _write_to_msg(&_outbound_scratch_msg_info, &scratch, sizeof(scratch));
    _write_to_msg(&_outbound_scratch_msg_info, _block_info.buffer + block_part * MAX_BLOCK_PART_SIZE, part_size);
#if PROTOCOL_DEBUG >= 3
    _CHATTY_LOG("Sending block part %d, seq %d to slave \"%s\"", block_part, _outbound_scratch_msg_info.sequence,
                dest->name);
#endif
...
static int
_rcv_msg(struct net_msg_info* msg_info, struct sockaddr_in* source_addr, int timeout, unsigned* time_used_p) {
    int result = FALSE;
    int sel_result;
    unsigned time_used;
    unsigned socklen = 0;
    ...
    do {
        ...
        sel_result = select(_socket_fd + 1, &scratch_fds, NULL, NULL, timeout_tv_p);
        if (sel_result < 0) {
            _CHATTY_ERRNO_LOG("select() failure");
        } else if (sel_result == 0) {
            timeout = 0;
        } else if (msg_len = recvfrom(_socket_fd, msg_info->msg, sizeof(msg_info->msg), 0,
	   ...
	   they are assumed to be unrelated traffic */
        else if (msg_len >= MSG_HEADER_LEN && memcmp(msg_info->msg, MSG_MAGIC, MSG_MAGIC_LEN) == 0) {
            msg_info->msg_type = msg_info->msg[3];
            msg_info->counter = (UINT32)ntohl(*((UINT32*)(msg_info->msg + 4)));
            msg_info->sequence = (UINT32)ntohl(*((UINT32*)(msg_info->msg + 8)));
            msg_info->data_start = msg_info->msg + MSG_HEADER_LEN;
...
static void
_send_reminder_msg(struct net_peer_info* dest, unsigned retry_timeout) {
    ...
    if (_current_net_role == SLAVE && dest == &(_master_info)) {
        switch (_protocol_state) {
            ...
            case ACKNOWLEDGEMENT:
                _prime_msg(&_outbound_scratch_msg_info, ACK, _current_sync_sequence - 1);
    ...
    if (reminder_msg != NULL && _ok_to_resend(reminder_msg, dest, retry_timeout)) {
#if PROTOCOL_DEBUG >= 1
#if PROTOCOL_DEBUG >= 2
        _CHATTY_LOG("Sending reminder msg"
                    " type %d, sequence %d/counter %d to peer \"%s\"",
                    reminder_msg->msg_type, reminder_msg->sequence, dest->outbound_counter, dest->name);
#endif
        _reminder_count += 1;
#endif
...
static struct net_peer_info*
_rcv_msg_from_peer(struct net_msg_info* msg, unsigned timeout, unsigned* time_used, int allow_join) {
...
static struct net_peer_info*
_pushy_rcv_msg_from_peer(struct net_msg_info* msg, int retry_timeout, int giveup_timeout, unsigned* time_used,
                         int allow_join) {
    struct net_peer_info* source_peer = NULL;
    ...
    *time_used = 0;
    do {
        ...
        source_peer = _rcv_msg_from_peer(msg, rcv_timeout, &try_time_used, allow_join);
        ...
        if (source_peer == NULL && giveup_timeout != 0) {
            ...
            for (i = 0; i < _original_player_count - 1; i++) {
                if (_peer_info[i].state == UNSYNCED) {
                    _send_reminder_msg(&(_peer_info[i]), retry_timeout);
                }
            }
        }
...
_record_sync_end() {
    ...
    if (!_non_state_msg_since_sync) {
	   ...
	   deviation */
        if (_frame_count > SYNC_TIME_SMA_IGNORE_COUNT + SYNC_TIME_SMA_POINTS && sync_time > 4 * _sync_time_sma) {
            is_spike = TRUE;
        }

        if (_current_net_role == MASTER && _frame_count > SYNC_TIME_SMA_IGNORE_COUNT && !is_spike) {
            _sync_time_sma -= (double)_sync_time[_sync_time_cursor] / (double)SYNC_TIME_SMA_POINTS;
            _sync_time[_sync_time_cursor] = sync_time;
            _sync_time_sma += (double)_sync_time[_sync_time_cursor] / (double)SYNC_TIME_SMA_POINTS;
#if PROTOCOL_DEBUG >= 3
            _CHATTY_LOG("Sync time: %d ms; _sync_time_sma now == %f ms", sync_time, _sync_time_sma);
#endif
            _sync_time_cursor += 1;
...
static void
_handle_duplicate_msg(struct net_msg_info* duplicate_msg_info, struct net_peer_info* source_peer,
                      struct net_msg_info* counterpart, unsigned retry_timeout)

{
    ...
    _CHATTY_LOG("Received duplicate message"
                ...
                duplicate_msg_info->msg_type, duplicate_msg_info->sequence, duplicate_msg_info->counter,
                source_peer->name);
#endif
...
#if PROTOCOL_DEBUG >= 2
        _CHATTY_LOG("... re-sending message type %d sequence %d", counterpart->msg_type, counterpart->sequence);
#endif
...
static void
_handle_early_msg(struct net_msg_info* early_msg_info, struct net_peer_info* source_peer, unsigned retry_timeout) {
    ...
    if (early_msg_info->sequence > source_peer->early_inbound_msg_info.sequence) {
        _copy_msg(early_msg_info, &(source_peer->early_inbound_msg_info));
    }
}

static void
_process_new_state_msg(struct net_msg_info* msg_to_process, struct net_peer_info* source_peer,
                       unsigned short* input_port_deviations) {
    unsigned i;
    for (i = 0; i < MAX_INPUT_PORTS; i++) {
        /* mask in the default deviations from each peer */
        input_port_deviations[i] |= ntohs(((unsigned short*)msg_to_process->data_start)[i]);
    }
    assert(_protocol_state == INPUT_EXCHANGE);
    source_peer->state = SYNCED;
    _unsynced_peer_count -= 1;
    if (_unsynced_peer_count == 0 && (_current_net_role == MASTER || PROTOCOL_DEBUG >= 1)) {
        _record_sync_end();
    }
#if PROTOCOL_DEBUG >= 3
    _CHATTY_LOG("State of peer \"%s\" is now %d", source_peer->name, source_peer->state);
#endif
}
...
_read_nvram(char** buffer, unsigned* len) {
    ...
    file = mame_fopen(Machine->gamedrv->name, NULL, FILETYPE_NVRAM, FALSE);
    if (file != NULL) {
        *len = mame_fsize(file);
...
_write_nvram(char* buffer, unsigned len) {
    int result = FALSE;
    void* file = mame_fopen(Machine->gamedrv->name, NULL, FILETYPE_NVRAM, TRUE);
    if (file != NULL) {
...
static void
_process_new_block_part_msg(struct net_msg_info* msg_to_process, struct net_peer_info* source_peer) {
    ...
    if (!(_block_info.bitmap[block_part / 8] & (1 << (block_part % 8)))) {
        memcpy(_block_info.buffer + MAX_BLOCK_PART_SIZE * block_part, msg_to_process->data_start + 4,
               msg_to_process->data_len - 4);
        _block_info.bitmap[block_part / 8] |= (1 << (block_part % 8));
        _block_info.remaining_part_count -= 1;
#if PROTOCOL_DEBUG >= 3
        _CHATTY_LOG("Got block part %d, seq %d; %d parts left", block_part, msg_to_process->sequence,
                    _block_info.remaining_part_count);
#endif
        if (_block_info.remaining_part_count == 0) {
            ...
            _prime_msg(&_outbound_scratch_msg_info, ACK, _current_sync_sequence);
...
static void
_process_new_block_prompt_msg(struct net_msg_info* msg_to_process, struct net_peer_info* source_peer) {
    unsigned block_count = msg_to_process->data_len / sizeof(UINT32);
    unsigned i;
    for (i = 0; i < block_count; i++) {
        unsigned block_part = ntohl(((UINT32*)msg_to_process->data_start)[i]);
...
static void
_build_start_msg_for_peer(struct net_msg_info* msg, struct net_peer_info* peer, unsigned sync_sequence) {
    unsigned i;
    ...
    scratch_msg_data[1] =
        (_parallel_sync << CONFIG_BITMAP_PARALLELSYNC) | (_machine_state_debug << CONFIG_BITMAP_MACHINE_STATE_DEBUG);
    ...
    for (i = 0; i < _current_player_count - 1; i++) {
	   ...
	   IP address for every *other* slave */
        if (_peer_info[i].state != INACTIVE && _peer_info[i].player_number != peer->player_number) {
            char* ip_string = inet_ntoa(_peer_info[i].addr.sin_addr);
            char scratch[10];
            sprintf(scratch, ":%d", _peer_info[i].addr.sin_port);
            _write_to_msg(msg, _peer_info[i].name, strlen(_peer_info[i].name) + 1);
            _write_to_msg(msg, ip_string, strlen(ip_string));
    ...
    if (_current_player_count == 1) {
	   ...
	   all pick the same one without needing to sync */
        ...
        for (i = 0; i < _original_player_count; i++) {
            if (_peer_info[i].state != INACTIVE && _peer_info[i].player_number < lowest_player_number) {
...
static void
_process_inbound_msg(struct net_msg_info* msg_to_process, struct net_peer_info* source_peer,
                     unsigned short* input_port_deviations, unsigned retry_timeout) {

#if PROTOCOL_DEBUG >= 3
    _CHATTY_LOG("Processing message type %d, sequence %d/counter %d"
                " from peer \"%s\" (_current_sync_sequence == %d, "
                "_min_state_sequence == %d)",
                msg_to_process->msg_type, msg_to_process->sequence, msg_to_process->counter, source_peer->name,
                _current_sync_sequence, _min_state_sequence);
#endif

    if (msg_to_process->msg_type == QUIT && msg_to_process->sequence <= _current_sync_sequence)
    /* This is the only acceptable case from an inactive peer */
    {
        _prime_msg(&_outbound_scratch_msg_info, QUIT, msg_to_process->sequence);
        if (source_peer->state != INACTIVE) {
            ...
            _handle_duplicate_msg(msg_to_process, source_peer, &_outbound_scratch_msg_info, retry_timeout);
        }
	   ...
	   Log an error anyways?  Maybe depending on PROTOCOL_DEBUG? */
        ...
        if (msg_to_process->sequence >= _prev_min_state_sequence && msg_to_process->sequence < _min_state_sequence) {
            assert(_previous_outbound_msg_info->sequence < _current_sync_sequence);
            _handle_duplicate_msg(msg_to_process, source_peer, _previous_outbound_msg_info, retry_timeout);
        } else if (msg_to_process->sequence >= _min_state_sequence) {
            if (_current_net_role == MASTER && _protocol_state == ACKNOWLEDGEMENT) {
		   ...
		   so state is for current frame */
                if (source_peer->state == SYNCED) {
                    _handle_duplicate_msg(msg_to_process, source_peer, _current_outbound_msg_info, retry_timeout);
                } else {
                    if (_current_net_role == SLAVE) {
                        _protocol_state = INPUT_EXCHANGE;
                    }
                    _process_new_state_msg(msg_to_process, source_peer, input_port_deviations);
                }
            }
	   ...
	   out of order */
        if (msg_to_process->sequence == _current_sync_sequence && _protocol_state != QUITTING) {
            _process_new_block_part_msg(msg_to_process, source_peer);
            _non_state_msg_since_sync = TRUE;
        }
    } else if (msg_to_process->msg_type == START || msg_to_process->msg_type == BLOCK_INIT
               || msg_to_process->msg_type == SET_SYNC_SKIP) {
        /* All ACKables handled here */
        assert(_current_net_role == SLAVE && source_peer == &(_master_info));
        if (msg_to_process->sequence == _current_sync_sequence - 1) {
            _prime_msg(&_outbound_scratch_msg_info, ACK, msg_to_process->sequence);
            _handle_duplicate_msg(msg_to_process, source_peer, &_outbound_scratch_msg_info, retry_timeout);
        } else if (msg_to_process->sequence == _current_sync_sequence && _protocol_state != QUITTING) {
            /* All ACKables handled here */
        ...
        if (msg_to_process->sequence == _current_sync_sequence - 1) {
            if (_protocol_state == REGISTRATION) {
                _build_start_msg_for_peer(&_outbound_scratch_msg_info, source_peer, _current_sync_sequence - 1);
                _handle_duplicate_msg(msg_to_process, source_peer, &_outbound_scratch_msg_info, retry_timeout);
            } else {
                _handle_duplicate_msg(msg_to_process, source_peer, _previous_outbound_msg_info, retry_timeout);
            }
	   ...
	   transfer was initiated during this frame */
        ...
        if (msg_to_process->counter > source_peer->last_inbound_counter) {
            _process_new_block_prompt_msg(msg_to_process, source_peer);
            _non_state_msg_since_sync = TRUE;
        }
    } else {
...
_inbound_presync(unsigned short input_port_deviations[MAX_INPUT_PORTS]) {
    ...
    while (source_peer = _rcv_msg_from_peer(&_inbound_scratch_msg_info, 0, NULL, FALSE), source_peer != NULL) {
        _process_inbound_msg(&_inbound_scratch_msg_info, source_peer, input_port_deviations, SYNC_RETRY_TIMEOUT);
    }
}
...
_inbound_sync(unsigned short input_port_deviations[MAX_INPUT_PORTS]) {
    ...
    for (i = 0; i < _original_player_count - 1; i++) {
        if (_peer_info[i].state == UNSYNCED) {
            struct net_msg_info* early_msg = &(_peer_info[i].early_inbound_msg_info);
            if ((_protocol_state == INPUT_EXCHANGE && early_msg->sequence >= _min_state_sequence
                 && early_msg->sequence <= _current_sync_sequence)
                || (early_msg->sequence == _current_sync_sequence)) {
                _process_inbound_msg(&(_peer_info[i].early_inbound_msg_info), &(_peer_info[i]), input_port_deviations,
                                     SYNC_RETRY_TIMEOUT);
            }
        }
    }
    ...
    while (_unsynced_peer_count > 0 && time_before_giveup > 0) {
        unsigned time_used;
        if (_current_net_role == SLAVE) {
            source_peer = _pushy_rcv_msg_from_peer(&_inbound_scratch_msg_info, SYNC_RETRY_TIMEOUT, time_before_giveup,
                                                   &time_used, FALSE);
        } else {
            /* Master never sends reminders due to timeouts */
            source_peer = _rcv_msg_from_peer(&_inbound_scratch_msg_info, time_before_giveup, &time_used, FALSE);
        }
...
#if PROTOCOL_DEBUG >= 3
    _CHATTY_LOG("_current_sync_sequence incremented to %d", _current_sync_sequence);
#endif
}
...
_register_to_master(void) {
    ...
    _write_to_msg(_current_outbound_msg_info, scratch, strlen(scratch) + 1);
    _write_to_msg(_current_outbound_msg_info, _truncated_build_version, strlen(_truncated_build_version) + 1);
    _write_to_msg(_current_outbound_msg_info, PROTOCOL_VERSION, strlen(PROTOCOL_VERSION) + 1);
    _write_to_msg(_current_outbound_msg_info, Machine->gamedrv->name, strlen(Machine->gamedrv->name) + 1);
    ...
    if (!_send_msg_to_peer(_current_outbound_msg_info, &(_master_info))) {
	   ...
	   are the only two peers involved */
        ...
        while (peer = _pushy_rcv_msg_from_peer(&_inbound_scratch_msg_info, REGISTRATION_RETRY_TIMEOUT,
                                               time_before_giveup, &time_used, FALSE),
               peer != NULL && _inbound_scratch_msg_info.msg_type != REFUSE
                   && _inbound_scratch_msg_info.msg_type != START) {
            /* NVRAM override happens here */
            _process_inbound_msg(&_inbound_scratch_msg_info, &(_master_info), NULL, REGISTRATION_RETRY_TIMEOUT);
        ...
        if (peer == NULL) {
            ...
            if (_inbound_scratch_msg_info.data_len > 0) {
                _CHATTY_LOG("Reason given:  %.50s", _inbound_scratch_msg_info.data_start);
            } else {
            ...
            _parallel_sync = (_inbound_scratch_msg_info.data_start[1] & (1 << CONFIG_BITMAP_PARALLELSYNC));
            _machine_state_debug = (_inbound_scratch_msg_info.data_start[1] & (1 << CONFIG_BITMAP_MACHINE_STATE_DEBUG));
            ...
            while (msg_data_left > 0) {
                ...
                strncpy(_peer_info[peer_index].name, msg_read_pointer, msg_data_left);
                ...
                if (msg_data_left == 0) {
                    _CHATTY_LOG("Master returned malformed accept message:"
                                ...
                                peer_player_number);
                } else if (!_parse_hostport(msg_read_pointer, &(_peer_info[peer_index].addr))) {
                    _CHATTY_LOG("Master returned malformed accept message:"
                                " bogus peer host:port \"%s\" for player %d",
                                msg_read_pointer, peer_player_number);
                    break;
                } else {
                    msg_data_left -= (strlen(msg_read_pointer) + 1);
                    msg_read_pointer += (strlen(msg_read_pointer) + 1);
            ...
            if (msg_data_left == 0) {
                ...
                _prime_msg(&_outbound_scratch_msg_info, ACK, _current_sync_sequence);
                _send_msg_to_peer(&_outbound_scratch_msg_info, &(_master_info));
                _protocol_state = ACKNOWLEDGEMENT;
                _current_sync_sequence += 1;
                ...
                _CHATTY_LOG("Protocol config:"
                            ...
                            (_parallel_sync ? "enabled" : "disabled"), (_machine_state_debug ? "enabled" : "disabled"));
                result = TRUE;
            }
        }
    }
...
static int
_approve_join_msg(struct net_msg_info* msg_info, struct net_peer_info* source_peer) {
    int result = FALSE;
    const char* comparison_string[4];
    const char* comparison_string_name[] = {"build version", "protocol version", "game name"};
    unsigned i = 0;
    ...
    while (comparison_string[i] != NULL) {
        if (strncmp(msg_read_pointer, comparison_string[i], msg_data_left) != 0) {
            sprintf(scratch_msg_data, "Wrong %s; need %s not %.10s", comparison_string_name[i], comparison_string[i],
                    msg_read_pointer);

            _prime_msg(&_outbound_scratch_msg_info, REFUSE, 0);
            _write_to_msg(&_outbound_scratch_msg_info, scratch_msg_data, strlen(scratch_msg_data));
...
_await_slave_registrations(void) {
    ...
    _CHATTY_LOG("Master Mode: Waiting for %d more player%s.", _original_player_count - 1,
                (_original_player_count - 1 == 1) ? "" : "s");

    _current_player_count = 1; /* Count only master to being with */

    for (slave_index = 0; slave_index < _original_player_count - 1; slave_index++) {
        memset(&(_peer_info[slave_index].addr.sin_addr), 0, sizeof(&(_peer_info[slave_index].addr.sin_addr)));
    ...
    if (_read_nvram(&_block_info.buffer, &_block_info.size)) {
        ...
        _write_to_msg(_current_outbound_msg_info, &scratch, sizeof(scratch));
    ...
    while (expected_ack_count > 0 || _current_player_count < _original_player_count) {
        struct net_peer_info* source_peer;
        source_peer = _rcv_msg_from_peer(&_inbound_scratch_msg_info, -1, NULL, TRUE);

        if (_inbound_scratch_msg_info.msg_type != JOIN && source_peer->state != INACTIVE) {
            /* BLOCK_INIT for the NVRAM transfer happens here */
            _process_inbound_msg(&_inbound_scratch_msg_info, source_peer, NULL, REGISTRATION_RETRY_TIMEOUT);
            if (source_peer->state == SYNCED) {
            ...
            if (_approve_join_msg(&_inbound_scratch_msg_info, source_peer)) {
                ...
                if (source_peer->state == INACTIVE) {
                    ...
                    if (_block_info.buffer != NULL) {
                        _send_msg_to_peer(_current_outbound_msg_info, source_peer);
                        expected_ack_count += 1;
                    }
                } else {
                    if (_block_info.buffer != NULL) {
                        _handle_duplicate_msg(&_inbound_scratch_msg_info, source_peer, _current_outbound_msg_info,
                                              REGISTRATION_RETRY_TIMEOUT);
                    }
                }
                source_peer->last_inbound_counter = _inbound_scratch_msg_info.counter;
            } else {
                if (source_peer->state != INACTIVE) {
                    /* Existing peer re-registration failed */
                    if (_block_info.buffer != NULL && source_peer->state == UNSYNCED) {
                        expected_ack_count -= 1;
                    }
    ...
    if (_block_info.buffer != NULL) {
        _current_sync_sequence += 1;
        _protocol_state = BLOCK_TRANSFER;
        for (slave_index = 0; slave_index < _current_player_count - 1; slave_index++) {
    ...
    for (slave_index = 0; slave_index < _current_player_count - 1; slave_index++) {
        ...
        _build_start_msg_for_peer(&_outbound_scratch_msg_info, &(_peer_info[slave_index]), _current_sync_sequence);

        if (!_send_msg_to_peer(&_outbound_scratch_msg_info, &(_peer_info[slave_index]))) {
	       ...
	       out */
            _CHATTY_ERRNO_LOG("Error: socket error sending START message"
                              " to slave \"%s\" (player %d)",
                              _peer_info[slave_index].name, _peer_info[slave_index].player_number);
        } else {
            _CHATTY_LOG("\"%s\" registration accepted as player %d.", _peer_info[slave_index].name,
...
static int
_map_input_port(unsigned unmapped_bit_index, struct net_input_bit_id* mapped_bit_id) {
    ...
    if (Machine->gamedrv->input_ports[0].type != IPT_PORT) {
        ...
        if ((unmapped_type & IPF_PLAYERMASK) == input_map_reference[_player_number - 2].playermask) {
        ...
        if (target_type != IPT_UNKNOWN) {
            ...
            while (
                ...
                && (candidate_type = Machine->gamedrv->input_ports[candidate_bit_index].type, candidate_type != IPT_END)
                && !result) {
...
_build_net_keymap(void) {
    ...
    if (Machine->gamedrv->input_ports[0].type != IPT_PORT) {
        ...
        while (unmapped_port_index < MAX_INPUT_PORTS
               && (current_type = Machine->gamedrv->input_ports[unmapped_bit_index].type, current_type != IPT_END)) {
	       ...
	       and player 1 start in Gauntlet) */
            ...
            while (previous_mapping_index < _input_mapping_count && !duplicate_found) {
                unsigned previously_mapped_port_index = _input_map[previous_mapping_index].source.port_index;
                unsigned short previously_mapped_mask = _input_map[previous_mapping_index].source.mask;
            ...
            if (!duplicate_found && _map_input_port(unmapped_bit_index, &(_input_map[_input_mapping_count].dest))) {
                _input_map[_input_mapping_count].source.port_index = unmapped_port_index;
                _input_map[_input_mapping_count].source.mask = Machine->gamedrv->input_ports[unmapped_bit_index].mask;
...
_remap_input_state(unsigned short input_state[MAX_INPUT_PORTS]) {
    ...
    for (mapping_index = 0; mapping_index < _input_mapping_count; mapping_index++) {
        unsigned short source_mask = _input_map[mapping_index].source.mask;
        unsigned short source_bit = input_state[_input_map[mapping_index].source.port_index] & source_mask;
        unsigned short dest_mask = _input_map[mapping_index].dest.mask;
        unsigned short dest_bit = input_state[_input_map[mapping_index].dest.port_index] & dest_mask;
...
_log_port_array(unsigned short* port_values, unsigned port_count) {
    ...
    for (i = 0; i < port_count; i++) {
        fprintf(stderr_file, "%x%s", port_values[i], (i < port_count - 1) ? " " : "\n");
    }
}
#endif
...
#if PROTOCOL_DEBUG >= 3
    fprintf(stderr_file, "Frame %d outbound/saved deviations: ", _frame_count);
...
#if PROTOCOL_DEBUG >= 3
    _CHATTY_LOG("Sending message type %d sequence %d to peers", msg->msg_type, msg->sequence);
#endif
    if (_current_net_role == MASTER || PROTOCOL_DEBUG >= 1) {
        _record_sync_start();
    }

    for (peer_index = 0; peer_index < _original_player_count - 1; peer_index++) {
        if (_peer_info[peer_index].state != INACTIVE) {
            _send_msg_to_peer(_current_outbound_msg_info, &(_peer_info[peer_index]));
            _peer_info[peer_index].state = UNSYNCED;
#if PROTOCOL_DEBUG >= 3
            _CHATTY_LOG("State of peer \"%s\" is now %d", _peer_info[peer_index].name, _peer_info[peer_index].state);
#endif
        }
    }
    _unsynced_peer_count = _current_player_count - 1;
...
_accumulate_new_input(unsigned short input_port_values[MAX_INPUT_PORTS],
    ...
    for (i = 0; i < MAX_INPUT_PORTS; i++) {
        _new_input_state[i] |= (_outbound_input_state[i] ^ (input_port_values[i] ^ input_port_defaults[i]));
    }
}
...
_sync_skip_adjustment() {
    ...
    if (_frame_count >= _minimum_frame_for_valid_sma) {
        float ms_per_frame = (float)1000 / (float)Machine->drv->frames_per_second;
        float lower_boundary = (float)_sync_skip * ms_per_frame - (float)SYNC_SKIP_TOLERANCE;
        float upper_boundary = (float)(_sync_skip + 1) * ms_per_frame + (float)SYNC_SKIP_TOLERANCE;
        if (_sync_skip > 0 && _sync_time_sma < lower_boundary) {
            adjusted_skip -= 1;
#if PROTOCOL_DEBUG >= 2
            _CHATTY_LOG("Sync time SMA is now %.2f (< %.2f)", _sync_time_sma, lower_boundary);
#endif
        } else if (_sync_skip < MAX_SYNC_SKIP && _sync_time_sma > upper_boundary) {
            adjusted_skip += 1;
#if PROTOCOL_DEBUG >= 2
            _CHATTY_LOG("Sync time SMA is now %.2f (> %.2f)", _sync_time_sma, upper_boundary);
#endif
        }
    }
...
void
osd_net_sync(unsigned short input_port_values[MAX_INPUT_PORTS], unsigned short input_port_defaults[MAX_INPUT_PORTS]) {
    if (_current_net_role != NONE) {
        ...
        if (skip_this_frame) {
            ...
            for (i = 0; i < MAX_INPUT_PORTS; i++) {
                _saved_input_state[i] = _outbound_input_state[i] | _inbound_input_state[i];
                _outbound_input_state[i] ^= _new_input_state[i];
            }
            memset(_new_input_state, 0, MAX_INPUT_PORTS * sizeof(input_port_values[0]));
            memset(_inbound_input_state, 0, MAX_INPUT_PORTS * sizeof(input_port_values[0]));
            ...
            if (_new_sync_skip != _sync_skip) {
                _CHATTY_LOG("Now setting skip to %d (_frame_count == %d)", _new_sync_skip, _frame_count);
                _sync_skip = _new_sync_skip;
                _minimum_frame_for_valid_sma = _frame_count + SYNC_TIME_SMA_POINTS;
            } else if (_current_net_role == MASTER
                       && (_new_sync_skip = _sync_skip_adjustment(), _new_sync_skip != _sync_skip)) {
        ...
        for (i = 0; i < MAX_INPUT_PORTS; i++) {
            input_port_values[i] = _saved_input_state[i] ^ input_port_defaults[i];
        }

#if PROTOCOL_DEBUG >= 3
        fprintf(stderr_file, "Frame %d final values: ", _frame_count);
    ...
    if (_current_net_role == NONE) {
	   ...
	   the handshake to match MAME versions -- is it worth it? */
        ...
        if (build_version_end == NULL) {
            ...
            strncpy(_truncated_build_version, build_version, (build_version_end - build_version));
        }
        ...
        if (_init_sockets()) {
            ...
            memset(_saved_input_state, 0, MAX_INPUT_PORTS * sizeof(_saved_input_state[0]));
            memset(_inbound_input_state, 0, MAX_INPUT_PORTS * sizeof(_inbound_input_state[0]));
            memset(_outbound_input_state, 0, MAX_INPUT_PORTS * sizeof(_outbound_input_state[0]));
            memset(_new_input_state, 0, MAX_INPUT_PORTS * sizeof(_new_input_state[0]));
...
_bid_farewell(void) {
    ...
    for (peer_index = 0; peer_index < _original_player_count - 1; peer_index++) {
        if (_peer_info[peer_index].state != INACTIVE) {
            _prime_msg(&_outbound_scratch_msg_info, QUIT, _current_sync_sequence);
            _send_msg_to_peer(&_outbound_scratch_msg_info, &(_peer_info[peer_index]));
            _peer_info[peer_index].state = UNSYNCED;
        }
    }
    _inbound_sync(NULL);
...
_net_shutdown(void) {
    ...
    _CHATTY_LOG("Duplicate inbound messages:  %d, reminder messages sent: %d", _inbound_duplicate_count,
                _reminder_count);
    _CHATTY_LOG("Total sync time:  %02dm%02d.%ds (%02dm%02d.%ds real time elapsed)", MINUTES_PART(_total_sync_time),
                SECONDS_PART(_total_sync_time), MILLISECONDS_PART(_total_sync_time), MINUTES_PART(total_time),
                SECONDS_PART(total_time), MILLISECONDS_PART(total_time));
    _CHATTY_LOG("Average sync time:  %d ms", _total_sync_time / _current_sync_sequence);
