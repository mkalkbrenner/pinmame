...
void ui_displaymenu(struct mame_bitmap* bitmap, const char** items, const char** subitems, char* flag, int selected,
...
#ifdef __GNUC__
void CLIB_DECL usrintf_showmessage(const char* text, ...) __attribute__((format(printf, 1, 2)));

void CLIB_DECL usrintf_showmessage_secs(int seconds, const char* text, ...) __attribute__((format(printf, 2, 3)));
#else
