...
#if !defined(_MSC_VER) && !defined(LIBPINMAME)                                                                         \
    && !defined(XMAME) //!! as not included in PinMAMEs standard makefile build yet
...
/* artwork callbacks */
#ifndef MESS
static struct artwork_callbacks mame_artwork_callbacks = {NULL, artwork_load_artwork_file};
#endif
...
static struct hard_disk_interface mame_hard_disk_interface = {mame_hard_disk_open, mame_hard_disk_close,
                                                              mame_hard_disk_read, mame_hard_disk_write};
...
INLINE void
bail_and_print(const char* message) {
    if (!bailing) {
        bailing = 1;
        printf("%s\n", message);
    }
...
void
expand_machine_driver(void (*constructor)(struct InternalMachineDriver*), struct InternalMachineDriver* output) {
...
vh_open(void) {
    ...
    set_visible_area(Machine->drv->default_visible_area.min_x, Machine->drv->default_visible_area.max_x,
                     Machine->drv->default_visible_area.min_y, Machine->drv->default_visible_area.max_y);
...
static void
compute_aspect_ratio(const struct InternalMachineDriver* drv, int* aspect_x, int* aspect_y) {
...
static int
decode_graphics(const struct GfxDecodeInfo* gfxdecodeinfo) {
    int i;
    ...
    for (i = 0; i < MAX_GFX_ELEMENTS && gfxdecodeinfo[i].memory_region != -1; i++) {
        ...
        if (IS_FRAC(glcopy.total))
            glcopy.total =
                (UINT32)(region_length / glcopy.charincrement * FRAC_NUM(glcopy.total) / FRAC_DEN(glcopy.total));

        /* loop over all the planes, converting fractions */
        for (j = 0; j < MAX_GFX_PLANES; j++) {
            int value = glcopy.planeoffset[j];
            if (IS_FRAC(value))
                glcopy.planeoffset[j] =
                    (UINT32)(FRAC_OFFSET(value) + region_length * FRAC_NUM(value) / FRAC_DEN(value));
        }
...
static void
scale_vectorgames(int gfx_width, int gfx_height, int* width, int* height) {
...
void
set_visible_area(int min_x, int max_x, int min_y, int max_y) {
    if (Machine->visible_area.min_x == min_x && Machine->visible_area.max_x == max_x
        && Machine->visible_area.min_y == min_y && Machine->visible_area.max_y == max_y)
        return;
...
void
force_partial_update(int scanline) {
...
const struct performance_info*
mame_get_performance_info(void) {
    return &performance;
}
...
int
mame_find_cpu_index(const char* tag) {
    int cpunum;
...
struct MachineCPU*
machine_add_cpu(struct InternalMachineDriver* machine, const char* tag, int type, double cpuclock) {
    int cpunum;
...
struct MachineCPU*
machine_find_cpu(struct InternalMachineDriver* machine, const char* tag) {
    int cpunum;
...
void
machine_remove_cpu(struct InternalMachineDriver* machine, const char* tag) {
    int cpunum;
...
struct MachineSound*
machine_add_sound(struct InternalMachineDriver* machine, const char* tag, int type, void* sndintf) {
    int soundnum;
...
struct MachineSound*
machine_find_sound(struct InternalMachineDriver* machine, const char* tag) {
    int soundnum;
...
void
machine_remove_sound(struct InternalMachineDriver* machine, const char* tag) {
    int soundnum;

    for (soundnum = 0; soundnum < MAX_SOUND; soundnum++)
        if (machine->sound[soundnum].tag && strcmp(machine->sound[soundnum].tag, tag) == 0) {
            memmove(&machine->sound[soundnum], &machine->sound[soundnum + 1],
                    sizeof(machine->sound[0]) * (MAX_SOUND - soundnum - 1));
...
void*
mame_hard_disk_open(const char* filename, const char* mode) {
...
void
mame_hard_disk_close(void* file) {
...
UINT32
mame_hard_disk_read(void* file, UINT64 offset, UINT32 count, void* buffer) {
...
UINT32
mame_hard_disk_write(void* file, UINT64 offset, UINT32 count, const void* buffer) {
    mame_fseek((mame_file*)file, offset, SEEK_SET);
...
validitychecks(void) {
    ...
    b = a + 1;
    if (b > a) {
        printf("UINT8 must be 8 bits\n");
        error = 1;
    }

    if (sizeof(INT8) != 1) {
        printf("INT8 must be 8 bits\n");
        error = 1;
    }
    if (sizeof(UINT8) != 1) {
        printf("UINT8 must be 8 bits\n");
    ...
    if (sizeof(INT16) != 2) {
        printf("INT16 must be 16 bits\n");
        error = 1;
    }
    if (sizeof(UINT16) != 2) {
        printf("UINT16 must be 16 bits\n");
        error = 1;
    }
    if (sizeof(INT32) != 4) {
        printf("INT32 must be 32 bits\n");
        error = 1;
    }
    if (sizeof(UINT32) != 4) {
        printf("UINT32 must be 32 bits\n");
        error = 1;
    }
    if (sizeof(INT64) != 8) {
        printf("INT64 must be 64 bits\n");
        error = 1;
    }
    if (sizeof(UINT64) != 8) {
        printf("UINT64 must be 64 bits\n");
        error = 1;
    }
        ...
        for (j = i + 1; drivers[j]; j++) {
            if (!strcmp(drivers[i]->name, drivers[j]->name)) {
                printf("%s: %s is a duplicate name (%s, %s)\n", drivers[i]->source_file, drivers[i]->name,
                       drivers[i]->source_file, drivers[j]->source_file);
                error = 1;
            }
            if (!strcmp(drivers[i]->description, drivers[j]->description)) {
                printf("%s: %s is a duplicate description (%s, %s)\n", drivers[i]->description, drivers[i]->source_file,
                       drivers[i]->name, drivers[j]->name);
                error = 1;
            }
#ifndef MESS
            if (drivers[i]->rom && drivers[i]->rom == drivers[j]->rom && (drivers[i]->flags & NOT_A_DRIVER) == 0
                    ...
                    if (drv.cpu[cpu].memory_read) {
                        ...
                        switch (databus_width) {
                            case 8:
                                if ((mra->end & MEMPORT_WIDTH_MASK) != MEMPORT_WIDTH_8) {
                                    printf("%s: %s cpu #%d uses wrong data width memory handlers! (width = %d, memory "
                                           "= %08x)\n",
                        ...
                        while (!IS_MEMPORT_END(mra)) {
                            if (!IS_MEMPORT_MARKER(mra)) {
                                ...
                                if ((mra->start & (alignunit - 1)) != 0
                                    || (mra->end & (alignunit - 1)) != (alignunit - 1)) {
                    ...
                    if (drv.cpu[cpu].memory_write) {
                        ...
                        while (!IS_MEMPORT_END(mwa)) {
                            if (!IS_MEMPORT_MARKER(mwa)) {
                                ...
                                if ((mwa->start & (alignunit - 1)) != 0
                                    || (mwa->end & (alignunit - 1)) != (alignunit - 1)) {
                    ...
                    if (drv.cpu[cpu].port_read) {
                        ...
                        switch (databus_width) {
                            case 8:
                                if ((pra->end & MEMPORT_WIDTH_MASK) != MEMPORT_WIDTH_8) {
                                    printf("%s: %s cpu #%d uses wrong data width port handlers! (width = %d, port = "
                                           "%08x)\n",
                        ...
                        while (!IS_MEMPORT_END(pra)) {
                            if (!IS_MEMPORT_MARKER(pra)) {
                                ...
                                if ((pra->start & (alignunit - 1)) != 0
                                    || (pra->end & (alignunit - 1)) != (alignunit - 1)) {
                    ...
                    if (drv.cpu[cpu].port_write) {
                        ...
                        while (!IS_MEMPORT_END(pwa)) {
                            if (!IS_MEMPORT_MARKER(pwa)) {
                                ...
                                if ((pwa->start & (alignunit - 1)) != 0
                                    || (pwa->end & (alignunit - 1)) != (alignunit - 1)) {
...
*/
                    ...
                    if (!IS_FRAC(drv.gfxdecodeinfo[j].gfxlayout->total)) {
                        ...
                        len = drv.gfxdecodeinfo[j].gfxlayout->total * drv.gfxdecodeinfo[j].gfxlayout->charincrement;
                        avail =
                            region_length[type]
            ...
            while (inp->type != IPT_END) {
                if (inp->name && inp->name != IP_NAME_DEFAULT) {
                    ...
                    for (j = 0; j < STR_TOTAL; j++) {
                        if (inp->name == ipdn_defaultstrings[j])
                            break;
