...
static const char*
xml_normalize_string(const int OUTPUT_XML, const char* string) {
...
static void
print_free_string(int OUTPUT_XML, FILE* out, const char* s) {
    if (!OUTPUT_XML) {
        fprintf(out, "\"");
        if (s) {
            while (*s) {
...
static void
print_statement_string(int OUTPUT_XML, FILE* out, const char* s) {
    if (OUTPUT_XML) {
...
static void
print_game_switch(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
...
static void
print_game_input(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
    const struct InputPortTiny* input = game->input_ports;
    int nplayer = 0;
...
static void
print_game_bios(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
    ...
    while (!BIOSENTRY_ISEND(thisbios)) {
        ...
        if (thisbios->_description)
            fprintf(out, SELECT(L2P "description \"%s\"" L2N, " description=\"%s\""),
                    xml_normalize_string(OUTPUT_XML, thisbios->_description));
        if (thisbios->value == 0)
...
static void
print_game_rom(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
    ...
    for (region = rom_first_region(game); region; region = rom_next_region(region))
        for (rom = rom_first_file(region); rom; rom = rom_next_file(rom)) {
            ...
            if (!is_disk && in_parent)
                fprintf(out, SELECT(L2P "merge %s" L2N, " merge=\"%s\""),
                        xml_normalize_string(OUTPUT_XML, ROM_GETNAME(fprom)));
            ...
            switch (ROMREGION_GETTYPE(region)) {
                case REGION_CPU1:
                    fprintf(out, SELECT(L2P "region cpu1" L2N, " region=\"cpu1\""));
                    break;
                case REGION_CPU2:
                    fprintf(out, SELECT(L2P "region cpu2" L2N, " region=\"cpu2\""));
                    break;
                case REGION_CPU3:
                    fprintf(out, SELECT(L2P "region cpu3" L2N, " region=\"cpu3\""));
                    break;
                case REGION_CPU4:
                    fprintf(out, SELECT(L2P "region cpu4" L2N, " region=\"cpu4\""));
                    break;
                case REGION_CPU5:
                    fprintf(out, SELECT(L2P "region cpu5" L2N, " region=\"cpu5\""));
                    break;
                case REGION_CPU6:
                    fprintf(out, SELECT(L2P "region cpu6" L2N, " region=\"cpu6\""));
                    break;
                case REGION_CPU7:
                    fprintf(out, SELECT(L2P "region cpu7" L2N, " region=\"cpu7\""));
                    break;
                case REGION_CPU8:
                    fprintf(out, SELECT(L2P "region cpu8" L2N, " region=\"cpu8\""));
                    break;
                case REGION_GFX1:
                    fprintf(out, SELECT(L2P "region gfx1" L2N, " region=\"gfx1\""));
                    break;
                case REGION_GFX2:
                    fprintf(out, SELECT(L2P "region gfx2" L2N, " region=\"gfx2\""));
                    break;
                case REGION_GFX3:
                    fprintf(out, SELECT(L2P "region gfx3" L2N, " region=\"gfx3\""));
                    break;
                case REGION_GFX4:
                    fprintf(out, SELECT(L2P "region gfx4" L2N, " region=\"gfx4\""));
                    break;
                case REGION_GFX5:
                    fprintf(out, SELECT(L2P "region gfx5" L2N, " region=\"gfx5\""));
                    break;
                case REGION_GFX6:
                    fprintf(out, SELECT(L2P "region gfx6" L2N, " region=\"gfx6\""));
                    break;
                case REGION_GFX7:
                    fprintf(out, SELECT(L2P "region gfx7" L2N, " region=\"gfx7\""));
                    break;
                case REGION_GFX8:
                    fprintf(out, SELECT(L2P "region gfx8" L2N, " region=\"gfx8\""));
                    break;
                case REGION_PROMS:
                    fprintf(out, SELECT(L2P "region proms" L2N, " region=\"proms\""));
                    break;
                case REGION_SOUND1:
                    fprintf(out, SELECT(L2P "region sound1" L2N, " region=\"sound1\""));
                    break;
                case REGION_SOUND2:
                    fprintf(out, SELECT(L2P "region sound2" L2N, " region=\"sound2\""));
                    break;
                case REGION_SOUND3:
                    fprintf(out, SELECT(L2P "region sound3" L2N, " region=\"sound3\""));
                    break;
                case REGION_SOUND4:
                    fprintf(out, SELECT(L2P "region sound4" L2N, " region=\"sound4\""));
                    break;
                case REGION_SOUND5:
                    fprintf(out, SELECT(L2P "region sound5" L2N, " region=\"sound5\""));
                    break;
                case REGION_SOUND6:
                    fprintf(out, SELECT(L2P "region sound6" L2N, " region=\"sound6\""));
                    break;
                case REGION_SOUND7:
                    fprintf(out, SELECT(L2P "region sound7" L2N, " region=\"sound7\""));
                    break;
                case REGION_SOUND8:
                    fprintf(out, SELECT(L2P "region sound8" L2N, " region=\"sound8\""));
                    break;
                case REGION_USER1:
                    fprintf(out, SELECT(L2P "region user1" L2N, " region=\"user1\""));
                    break;
                case REGION_USER2:
                    fprintf(out, SELECT(L2P "region user2" L2N, " region=\"user2\""));
                    break;
                case REGION_USER3:
                    fprintf(out, SELECT(L2P "region user3" L2N, " region=\"user3\""));
                    break;
                case REGION_USER4:
                    fprintf(out, SELECT(L2P "region user4" L2N, " region=\"user4\""));
                    break;
                case REGION_USER5:
                    fprintf(out, SELECT(L2P "region user5" L2N, " region=\"user5\""));
                    break;
                case REGION_USER6:
                    fprintf(out, SELECT(L2P "region user6" L2N, " region=\"user6\""));
                    break;
                case REGION_USER7:
                    fprintf(out, SELECT(L2P "region user7" L2N, " region=\"user7\""));
                    break;
                case REGION_USER8:
                    fprintf(out, SELECT(L2P "region user8" L2N, " region=\"user8\""));
                    break;
                case REGION_DISKS:
                    fprintf(out, SELECT(L2P "region disks" L2N, " region=\"disks\""));
                    break;
                default:
                    fprintf(out, SELECT(L2P "region 0x%x" L2N, " region=\"0x%x\""), ROMREGION_GETTYPE(region));
            }
...
static void
print_game_sampleof(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
#if (HAS_SAMPLES)
        ...
        if (samplenames != 0 && samplenames[0] != 0) {
            ...
            if (samplenames[k][0] == '*') {
                ...
                if (strcmp(samplenames[k] + 1, game->name) != 0)
                    fprintf(out, SELECT(L1P "sampleof %s" L1N, " sampleof=\"%s\""),
                            xml_normalize_string(OUTPUT_XML, samplenames[k] + 1));
                ++k;
            }
        }
    }
...
static void
print_game_sample(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
#if (HAS_SAMPLES)
    struct InternalMachineDriver drv;
    int i;
...
static void
print_game_micro(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
...
static void
print_game_video(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
    struct InternalMachineDriver driver;
...
static void
print_game_sound(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
    struct InternalMachineDriver driver;
    const struct MachineCPU* cpu;
    const struct MachineSound* sound;
...
static void
print_game_history(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
...
static void
print_game_driver(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
    struct InternalMachineDriver driver;
...
#ifdef MESS
static void
print_game_device(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
...
static void
print_game_info(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
    extern struct GameDriver driver_0;
    ...
    if (game->clone_of && !(game->clone_of->flags & NOT_A_DRIVER))
        fprintf(out, SELECT(L1P "cloneof %s" L1N, " cloneof=\"%s\""),
                xml_normalize_string(OUTPUT_XML, game->clone_of->name));

    if (game->clone_of && game->clone_of != &driver_0)
        fprintf(out, SELECT(L1P "romof %s" L1N, " romof=\"%s\""),
                xml_normalize_string(OUTPUT_XML, game->clone_of->name));
...
static void
print_resource_info(int OUTPUT_XML, FILE* out, const struct GameDriver* game) {
...
static void
print_mame_data(int OUTPUT_XML, FILE* out, const struct GameDriver* games[]) {
    int j;
...
void
print_mame_xml(FILE* out, const struct GameDriver* games[]) {
    fprintf(out, "<?xml version=\"1.0\"?>\n"
                 "<!DOCTYPE " XML_ROOT " [\n"
                 "<!ELEMENT " XML_ROOT " (" XML_TOP "+)>\n"
#ifdef MESS
                 "\t<!ELEMENT " XML_TOP " (description, year?, manufacturer, history?, biosset*, rom*, disk*, sample*, "
                                        "chip*, video?, sound?, input?, dipswitch*, driver?, device*)>\n"
#else
                 "\t<!ELEMENT " XML_TOP " (description, year?, manufacturer, history?, biosset*, rom*, disk*, sample*, "
                                        "chip*, video?, sound?, input?, dipswitch*, driver?)>\n"
#endif
...
void
print_mame_info(FILE* out, const struct GameDriver* games[]) {
