...
extern "C" void
OnSolenoid(int nSolenoid, int IsActive) {
    if (!m_pController)
        return;

    // post the call to the event window (which is created in the thread of the host app)
    if (IsWindow(m_pController->m_hEventWnd))
        PostMessage(m_pController->m_hEventWnd, RegisterWindowMessage(VPINMAMEONEVENTMSG), EVENT_ONSOLENOID,
                    (WPARAM)((IsActive ? 0x10000 : 0x00000) + nSolenoid));
}

extern "C" void
OnStateChange(int nState) {
    if (!m_pController)
        return;
    ...
    // post the call to the event window (which is created in the thread of the host app)
    if (IsWindow(m_pController->m_hEventWnd))
        PostMessage(m_pController->m_hEventWnd, RegisterWindowMessage(VPINMAMEONEVENTMSG), EVENT_ONSTATECHANGE,
                    (WPARAM)nState);
}

DWORD FAR PASCAL
CController::RunController(CController* pController) {
    if (!pController)
        return 0;
    ...
    if (fIsSupported == VARIANT_FALSE) {
        MessageBox(pController->m_hParentWnd, "This game is not supported by Visual PinMAME", "Failed to start",
...
osd_init(void) {
    ...
    SetWindowPos(win_video_window, HWND_TOPMOST, 0, 0, 0, 0, SWP_NOSIZE | SWP_NOMOVE | SWP_NOACTIVATE);
    ...
    if (cabinetMode)
        maxX = (maxX + 40) * 2 + 40;
    int maxY = GetSystemMetrics(cabinetMode ? 79 /*SM_CYVIRTUALSCREEN*/ : SM_CYSCREEN) - windowRect.bottom
               + (cabinetMode ? 40 : 0);
...
LRESULT CALLBACK
EventWindowProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
...
void
CreateEventWindow(CController* pController) {
    if (!pController)
        return;
    ...
    pController->m_hEventWnd =
        CreateWindow("VPINMAMEEVENTWND", "", 0, 0, 0, 0, 0, NULL, NULL, _Module.m_hInst, (LPVOID*)pController);
}

void
DestroyEventWindow(CController* pController) {
    if (!pController)
        return;
...
void
AdjustWindowPosition(HWND hWnd, CController* pController) {
    int xpos, ypos;
...
void
SaveWindowPosition(HWND hWnd, CController* pController) {
    RECT Rect;
...
void
SetWindowStyle(HWND hWnd, int iWindowStyle) {
...
osd_hook(HWND wnd, UINT message, WPARAM wparam, LPARAM lparam, BOOL* pfhandled) {
    LRESULT iResult = 0;
    ...
    switch (message) {
        ...
        case WM_MOUSEMOVE:
            if (fMoveWindow) {
                ...
                SetWindowPos(wnd, 0, pt.x - iWindowMovedX, pt.y - iWindowMovedY, 0, 0,
