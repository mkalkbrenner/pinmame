...
class ATL_NO_VTABLE CEnumGames : public CComObjectRootEx<CComMultiThreadModel>,
  ...
  public:
    ...
    STDMETHOD(InterfaceSupportsErrorInfo)(REFIID riid) {
        static const IID* arr[] = {&IID_IEnumGames};
...
STDMETHODIMP
CGames::InterfaceSupportsErrorInfo(REFIID riid) {
    static const IID* arr[] = {&IID_IGames};
    for (int i = 0; i < sizeof(arr) / sizeof(arr[0]); i++) {
        if (InlineIsEqualGUID(*arr[i], riid))
            return S_OK;
    }
...
STDMETHODIMP
CGames::get_Count(long* pnCount) {
    *pnCount = m_lGames;
    return S_OK;
}

STDMETHODIMP
CGames::get_Item(VARIANT* pKey, IGame** pGame) {
    HRESULT hr = S_FALSE;
    *pGame = NULL;
...
STDMETHODIMP
CGames::get__NewEnum(IUnknown** ppunkEnum) {
    *ppunkEnum = 0;
