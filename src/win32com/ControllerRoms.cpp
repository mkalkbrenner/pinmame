...
class ATL_NO_VTABLE CEnumRoms : public CComObjectRootEx<CComMultiThreadModel>,
  ...
  public:
    ...
    STDMETHOD(InterfaceSupportsErrorInfo)(REFIID riid) {
        static const IID* arr[] = {&IID_IEnumRoms};
  ...
  public:
    STDMETHODIMP
    Next(ULONG celt, VARIANT __RPC_FAR* rgVar, ULONG __RPC_FAR* pCeltFetched) {
        HRESULT hr;
    ...
    STDMETHODIMP
    Clone(IUnknown __RPC_FAR* __RPC_FAR* ppEnum) {
...
STDMETHODIMP
CRoms::Init(const struct GameDriver* gamedrv) {
    if (!gamedrv) {
        m_lRoms = 0;
        m_pRomsList = NULL;
        m_gamedrv = NULL;
...
STDMETHODIMP
CRoms::InterfaceSupportsErrorInfo(REFIID riid) {
    static const IID* arr[] = {&IID_IRoms};
    for (int i = 0; i < sizeof(arr) / sizeof(arr[0]); i++) {
        if (InlineIsEqualGUID(*arr[i], riid))
            return S_OK;
    }
...
STDMETHODIMP
CRoms::get_Count(long* pnCount) {
    *pnCount = m_lRoms;
    return S_OK;
}

STDMETHODIMP
CRoms::get_Item(VARIANT* pKey, IRom** pRom) {
    HRESULT hr = S_FALSE;
    *pRom = NULL;
...
STDMETHODIMP
CRoms::get__NewEnum(IUnknown** ppunkEnum) {
    *ppunkEnum = 0;
...
STDMETHODIMP
CRoms::get_Available(VARIANT_BOOL* pVal) {
    if (!m_gamedrv)
        return S_FALSE;
    ...
    *pVal = VARIANT_TRUE;
    if (!mame_faccess(m_gamedrv->name, FILETYPE_ROM)) {
        ...
        if (m_gamedrv->clone_of == 0 || !mame_faccess(m_gamedrv->clone_of->name, FILETYPE_ROM))
            *pVal = VARIANT_FALSE;
    }
...
STDMETHODIMP
CRoms::get_StateDescription(BSTR* pVal) {
    long lState;
    get_State(&lState);
...
STDMETHODIMP
CRoms::Audit(VARIANT_BOOL fStrict) {
    int i = 0;
    while (i < m_lRoms)
