...
/***********************/
STDMETHODIMP
CController::InterfaceSupportsErrorInfo(REFIID riid) {
 ...
 ***************************************/
void
CController::GetProductVersion(int* nVersionNo0, int* nVersionNo1, int* nVersionNo2, int* nVersionNo3) {
    DWORD dwVerHandle;
    DWORD dwInfoSize;
    HANDLE hVersionInfo;
    LPVOID lpEntryInfo;
...
CController::CController() {
    ...
    if (GameUsedTheFirstTime("")) {
        ...
        MessageBox(GetActiveWindow(),
                   "Welcome to Visual PinMAME! \n Please setup the default game options by clicking \"OK\"!", "Notice!",
 ...
 *****************************************/
STDMETHODIMP
CController::Run(/*[in]*/ LONG_PTR hParentWnd, /*[in,defaultvalue(100)]*/ int nMinVersion) {
    ...
    m_pGame->ShowInfoDlg(0x8000 | CHECKOPTIONS_SHOWRESULTSIFFAIL
                             | ((vValue.boolVal == VARIANT_TRUE) ? 0x0000 : CHECKOPTIONS_IGNORESOUNDROMS),
                         (LONG_PTR)m_hParentWnd, &iCheckVal);
    ...
    if (GameWasNeverStarted(m_szROM)) {
        ...
        if (!cabinetMode) {
            if (fFirstTime) {
                char szTemp[256];
                sprintf(szTemp,
                        "This is the first time you use: %s\nPlease specify options for this game by clicking \"OK\"!",
                        drivers[m_nGameNo]->description);
    ...
    if (!cabinetMode) {
        ...
        if (drivers[m_nGameNo]->flags & GAME_NOT_WORKING) {
            if (MessageBox(GetActiveWindow(),
                           ...
                           "Notice!", MB_YESNO | MB_ICONWARNING)
                == IDNO)
                return 1;
        }
        ...
        if ((iCheckVal != IDOK) && (drivers[m_nGameNo]->flags & GAME_NOCRC)) {
            MessageBox(GetActiveWindow(), "This game can only run with the EXACT romset required!", "Notice!",
                       MB_OK | MB_ICONINFORMATION);
    ...
    DWORD dwThreadID;
    m_hThreadRun =
        /*_beginthreadex*/ CreateThread(NULL, 0, (LPTHREAD_START_ROUTINE)RunController, (LPVOID)this, 0, &dwThreadID);
    ...
    sprintf(szTemp, "Machine '%s' (%s) terminated before initialized, check the rom path or rom file!", m_szROM,
            drivers[m_nGameNo]->description);
    return Error(TEXT(szTemp));
}
 ...
 ****************************************************************/
STDMETHODIMP
CController::get_Lamp(int nLamp, VARIANT_BOOL* pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 ************************************************************************/
STDMETHODIMP
CController::get_Solenoid(int nSolenoid, VARIANT_BOOL* pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 ************************************************************************/
STDMETHODIMP
CController::get_Switch(int nSwitchNo, VARIANT_BOOL* pVal) {
    if (pVal) {
        if (WaitForSingleObject(m_hEmuIsRunning, 0) == WAIT_TIMEOUT)
            *pVal = false;
        else
...
STDMETHODIMP
CController::put_Switch(int nSwitchNo, VARIANT_BOOL newVal) {
    if (WaitForSingleObject(m_hEmuIsRunning, 0) == WAIT_TIMEOUT)
        return S_OK;
    else
 ...
 *************************************************/
STDMETHODIMP
CController::get_Pause(VARIANT_BOOL* pVal) {
    *pVal = (WaitForSingleObject(m_hEmuIsRunning, 0) == WAIT_OBJECT_0) ? (g_fPause ? VARIANT_TRUE : VARIANT_FALSE)
                                                                       : VARIANT_FALSE;
    return S_OK;
}

STDMETHODIMP
CController::put_Pause(VARIANT_BOOL newVal) {
    if (WaitForSingleObject(m_hEmuIsRunning, 0) == WAIT_TIMEOUT)
        return S_OK;
 ...
 ***************************************************************************************/
STDMETHODIMP
CController::get_WPCNumbering(/*[out, retval]*/ VARIANT_BOOL* pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 *********************************************************************/
STDMETHODIMP
CController::get_Lamps(VARIANT* pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 ************************************************************************/
STDMETHODIMP
CController::get_RawDmdWidth(int* pVal) {
    *pVal = g_raw_dmdx;
    return S_OK;
}
 ...
 **************************************************************************/
STDMETHODIMP
CController::get_RawDmdHeight(int* pVal) {
    *pVal = g_raw_dmdy;
    return S_OK;
}
...
***************************************************************************/
STDMETHODIMP
CController::get_NVRAM(VARIANT* pVal) {
...
***************************************************************/
STDMETHODIMP
CController::get_ChangedNVRAM(VARIANT* pVal) {
    if (!pVal)
        return S_FALSE;
    ...
    if (oldNVRAMname == 0
        || strstr(Machine->gamedrv->name, oldNVRAMname) == 0) // detect initial VPM start or game change
    {
        ...
        mame_fclose(nvram_file);
        pVal->vt = 0;
        return S_OK; //!! for now, as too many changes initially!?!
    } else {
        uCount = 0;
 ...
 ************************************************************************************************/
STDMETHODIMP
CController::get_RawDmdPixels(VARIANT* pVal) {
...
*******************************************************************************************************/
STDMETHODIMP
CController::get_RawDmdColoredPixels(VARIANT* pVal) {
    if (Machine && g_needs_DMD_update && (int)g_raw_dmdx > 0 && (int)g_raw_dmdy > 0 && pVal) {
        SAFEARRAY* psa = SafeArrayCreateVector(VT_VARIANT, 0, g_raw_dmdx * g_raw_dmdy);
 ...
 ************************************************************************/
STDMETHODIMP
CController::get_DmdWidth(int* pVal) {
    *pVal = current_display_ptr
                ? current_display_ptr->game_visible_area.max_x - current_display_ptr->game_visible_area.min_x + 1
                : 0;
    return S_OK;
}
 ...
 **************************************************************************/
STDMETHODIMP
CController::get_DmdHeight(int* pVal) {
    *pVal = current_display_ptr
                ? current_display_ptr->game_visible_area.max_y - current_display_ptr->game_visible_area.min_y + 1
                : 0;
    return S_OK;
}
 ...
 *************************************************************************/
STDMETHODIMP
CController::get_DmdPixel(int x, int y, int* pVal) {
 ...
 *******************************************************************************/
STDMETHODIMP
CController::get_updateDmdPixels(int** buf, int width, int height, int* pVal) {
    if (!buf) {
        *pVal = 0;
        return S_FALSE;
    }
    ...
    if (Machine && current_display_ptr && btm) {
        if (width != (current_display_ptr->game_visible_area.max_x - current_display_ptr->game_visible_area.min_x + 1)
            || height
                   != (current_display_ptr->game_visible_area.max_y - current_display_ptr->game_visible_area.min_y
                       + 1)) {
            *pVal = 0;
            return S_OK;
        }
        ...
        if (btm->depth == 8) {
            for (int j = current_display_ptr->game_visible_area.max_y;
                 j >= current_display_ptr->game_visible_area.min_y; j--) {
                UINT8* src = (UINT8*)btm->line[j] + current_display_ptr->game_visible_area.min_x;
                for (int i = current_display_ptr->game_visible_area.min_x;
                     i <= current_display_ptr->game_visible_area.max_x; i++) {
                    UINT8 r, g, b;
 ...
 *****************************************************************************************/
STDMETHODIMP
CController::get_ChangedLampsState(int** buf, int* pVal) {
    if (!buf) {
        *pVal = 0;
        return S_FALSE;
    }
 ...
 **************************************************************************/
STDMETHODIMP
CController::get_LampsState(int** buf, int* pVal) {
    if (!buf) {
        *pVal = 0;
        return S_FALSE;
    }
 ...
 *************************************************************************************************/
STDMETHODIMP
CController::get_ChangedSolenoidsState(int** buf, int* pVal) {
    if (!buf) {
        *pVal = 0;
        return S_FALSE;
    }
 ...
 **********************************************************************************/
STDMETHODIMP
CController::get_SolenoidsState(int** buf, int* pVal) {
    if (!buf) {
        *pVal = 0;
        return S_FALSE;
    }
 ...
 *************************************************************************************/
STDMETHODIMP
CController::get_ChangedGIsState(int** buf, int* pVal) {
    if (!buf) {
        *pVal = 0;
        return S_FALSE;
    }
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_Switches(VARIANT* pVal) {
    if (!pVal)
        return S_FALSE;
...
STDMETHODIMP
CController::put_Switches(VARIANT newVal) {
    if (WaitForSingleObject(m_hEmuIsRunning, 0) == WAIT_TIMEOUT)
        return S_OK;
 ...
 ******************************************************/
STDMETHODIMP
CController::get_GameName(BSTR* pVal) {
...
STDMETHODIMP
CController::put_GameName(BSTR newVal) {
    if (m_hThreadRun != INVALID_HANDLE_VALUE) {
        if (WaitForSingleObject(m_hThreadRun, 0) == WAIT_TIMEOUT)
 ...
 ******************************************************/
STDMETHODIMP
CController::get_ROMName(BSTR* pVal) {
 ...
 *************************************************************/
STDMETHODIMP
CController::get_HandleKeyboard(VARIANT_BOOL* pVal) {
    if (pVal)
...
STDMETHODIMP
CController::put_HandleKeyboard(VARIANT_BOOL newVal) {
 ...
 ***************************************************/
STDMETHODIMP
CController::get_Machines(BSTR sMachine, VARIANT* pVal) {
    char szMachine[256];
 ...
 *************************************************************************/
STDMETHODIMP
CController::get_Running(VARIANT_BOOL* pVal) {
    if (pVal)
 ...
 ***************************************************************/
STDMETHODIMP
CController::get_ChangedLamps(VARIANT* pVal) {
    vp_tChgLamps chgLamps;
...
STDMETHODIMP
CController::get_ChangedLEDs(int nHigh, int nLow, int nnHigh, int nnLow, VARIANT* pVal) {
 ...
 *****************************************************************************************/
STDMETHODIMP
CController::get_ChangedLEDsState(int nHigh, int nLow, int nnHigh, int nnLow, int** buf, int* pVal) {
    const UINT64 mask = (((UINT64)nHigh) << 32) | ((UINT32)nLow);
    const UINT64 mask2 = (((UINT64)nnHigh) << 32) | ((UINT32)nnLow);
    vp_tChgLED chgLED;
 ...
 ******************************************************/
STDMETHODIMP
CController::ShowAboutDialog(LONG_PTR hParentWnd) {
    switch (hParentWnd) {
        case 0:
            break;
 ...
 ***************************************************************/
STDMETHODIMP
CController::get_HandleMechanics(int* pVal) {
    if (pVal)
...
STDMETHODIMP
CController::put_HandleMechanics(int newVal) {
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_GetMech(int mechNo, int* pVal) {
    if (!pVal)
        return S_FALSE;
...
STDMETHODIMP
CController::put_Mech(int mechNo, int newVal) {
 ...
 **************************************************************************/
STDMETHODIMP
CController::get_GIString(int nString, int* pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 ***************************************************************/
STDMETHODIMP
CController::get_ChangedGIStrings(VARIANT* pVal) {
    vp_tChgGIs chgGI;
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_ChangedSolenoids(VARIANT* pVal) {
    vp_tChgSols chgSol;
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_SplashInfoLine(BSTR* pVal) {
...
STDMETHODIMP
CController::put_SplashInfoLine(BSTR newVal) {
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_Solenoids(VARIANT* pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_Dip(int nNo, int* pVal) {
...
STDMETHODIMP
CController::put_Dip(int nNo, int newVal) {
    // TODO: Add your implementation code here. DONE
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_GIStrings(VARIANT* pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_LockDisplay(VARIANT_BOOL* pVal) {
    if (pVal)
...
STDMETHODIMP
CController::put_LockDisplay(VARIANT_BOOL newVal) {
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_SolMask(int nLow, long* pVal) {
...
STDMETHODIMP
CController::put_SolMask(int nLow, long newVal) {
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_Version(BSTR* pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_Games(IGames** pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 ******************************************************************************/
STDMETHODIMP
CController::get_Settings(IControllerSettings** pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 *************************************************************************/
STDMETHODIMP
CController::get_NewSoundCommands(VARIANT* pVal) {
 ...
 ***************************************************************/
STDMETHODIMP
CController::SetDisplayPosition(int x, int y, LONG_PTR hParentWindow) {
 ...
 *****************************************************************/
STDMETHODIMP
CController::ShowOptsDialog(LONG_PTR hParentWnd) {
 ...
 ************************************************************/
STDMETHODIMP
CController::get_ShowDMDOnly(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_ShowDMDOnly(VARIANT_BOOL newVal) {
 ...
 ******************************************************/
STDMETHODIMP
CController::get_ShowTitle(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_ShowTitle(VARIANT_BOOL newVal) {
 ...
 *******************************************************/
STDMETHODIMP
CController::get_ShowFrame(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_ShowFrame(VARIANT_BOOL newVal) {
 ...
 ************************************************************/
STDMETHODIMP
CController::get_DoubleSize(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_DoubleSize(VARIANT_BOOL newVal) {
 ...
 *********************************************************************/
STDMETHODIMP
CController::CheckROMS(/*[in,defaultvalue(0)]*/ int nShowOptions, /*[in,defaultvalue(0)]*/ LONG_PTR hParentWnd,
 ...
 ****************************************************************************/
STDMETHODIMP
CController::ShowPathesDialog(LONG_PTR hParentWnd) {
    switch (hParentWnd) {
        case 0:
            break;
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_Hidden(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_Hidden(VARIANT_BOOL newVal) {
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_Game(IGame** pVal) {
    if (!pVal)
        return S_FALSE;
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_MechSamples(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_MechSamples(VARIANT_BOOL newVal) {
 ...
 ****************************************************************************/
STDMETHODIMP
CController::GetWindowRect(LONG_PTR hWnd, VARIANT* pVal) {
    RECT rect;
 ...
 ****************************************************************************/
STDMETHODIMP
CController::GetClientRect(LONG_PTR hWnd, VARIANT* pVal) {
    RECT rect;
    if (IsWindow((HWND)hWnd)) {
 ...
 *********************************************************/
STDMETHODIMP
CController::get_MasterVolume(int* pVal) {
    if (pVal)
...
STDMETHODIMP
CController::put_MasterVolume(int newVal) {
...
************************************************************************************/
STDMETHODIMP
CController::get_EnumAudioDevices(int* pVal) {
    if (pVal)
...
**************************************************************************/
STDMETHODIMP
CController::get_AudioDevicesCount(int* pVal) {
    if (pVal)
...
***********************************************************************************/
STDMETHODIMP
CController::get_AudioDeviceDescription(int num, BSTR* pVal) {
    if (!pVal)
        return S_FALSE;
...
****************************************************************************/
STDMETHODIMP
CController::get_AudioDeviceModule(int num, BSTR* pVal) {
    if (!pVal)
        return S_FALSE;
...
*******************************************/
STDMETHODIMP
CController::get_CurrentAudioDevice(int* pVal) {
    if (pVal)
...
STDMETHODIMP
CController::put_CurrentAudioDevice(int newVal) {
...
STDMETHODIMP
CController::get_FastFrames(int* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_FastFrames(int newVal) {
...
STDMETHODIMP
CController::get_IgnoreRomCrc(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_IgnoreRomCrc(VARIANT_BOOL newVal) {
...
STDMETHODIMP
CController::get_SoundMode(int* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_SoundMode(int newVal) {
...
STDMETHODIMP
CController::get_CabinetMode(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_CabinetMode(VARIANT_BOOL newVal) {
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_ShowPinDMD(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_ShowPinDMD(VARIANT_BOOL newVal) {
 ...
 ****************************************************************************/
STDMETHODIMP
CController::get_ShowWinDMD(VARIANT_BOOL* pVal) {
    if (!pVal)
        return E_POINTER;
...
STDMETHODIMP
CController::put_ShowWinDMD(VARIANT_BOOL newVal) {
