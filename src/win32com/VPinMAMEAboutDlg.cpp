...
class CAboutDlg : public CDialogImpl<CAboutDlg> {
  ...
  private:
    int
    GetVersionResourceEntry(LPCTSTR lpszFilename, LPCTSTR lpszResourceEntry, LPTSTR lpszBuffer, WORD wMaxSize) {
        DWORD dwVerHandle;
        DWORD dwInfoSize;
        HANDLE hVersionInfo;
        LPVOID lpEntryInfo;
    ...
    OnInitDialog(UINT, WPARAM, LPARAM, BOOL&) {
        ...
        //Hyperlink stuff
        {
            ...
            hDC = GetDC();
            nHeight = -MulDiv(8, GetDeviceCaps(hDC, LOGPIXELSY),
                              72); //This line comes from the Win32API help for specifying point size!
            ...
            SetClassLongPtr(GetDlgItem(IDC_HOMEPAGELINK), GCL_HCURSOR,
                            (LONG_PTR)LoadCursor(NULL, MAKEINTRESOURCE(32649)));
        }
        //MUST RETURN 1
        return 1;
    }
    ...
    LRESULT
    OnHomePageLink(WORD, UINT, HWND, BOOL&) {
    ...
    /*Dialog Destroy*/
    LRESULT
    OnDestroyDialog(UINT, WPARAM, LPARAM, BOOL&) {
    ...
    /*Coloring*/
    LRESULT
    OnColorStaticDialog(UINT, WPARAM wParam, LPARAM lParam, BOOL&) {
...
void
ShowAboutDlg(HWND hParent) {
    CAboutDlg AboutDlg;
    AboutDlg.DoModal(hParent);
}

char*
GetBuildDateString(void) {
