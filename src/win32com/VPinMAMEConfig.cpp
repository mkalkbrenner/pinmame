...
static struct rc_option vpinmame_opts[] = {
    ...
    {"dmd_doublesize", NULL, rc_int, &dmd_scalefactor, "0", 0, 10000, NULL,
     "DMD display size (1/2 = 2x, 3,4,.. = 3x,4x,..)"},
    {"threadpriority", NULL, rc_int, &threadpriority, "1", 0, 2, NULL, "priority of the worker thread"},
    {"synclevel", NULL, rc_int, &synclevel, "0", -50, 60, NULL,
     "Sync. of frame rate for external programs (fps)"}, //SJE: Default synclevel is 0 now.. 10/01/03
    ...
    {"resampling_quality", NULL, rc_int, &resampling_quality, "0", 0, 1, NULL,
     "Quality of the resampling implementation (0=Fast,1=Normal)"},
#if defined(VPINMAME_ALTSOUND) || defined(VPINMAME_PINSOUND)
    {"sound_mode", NULL, rc_int, &sound_mode, "0", 0, 3, NULL,
     "Sound processing mode (PinMAME, Alternative, PinSound, PinSound + Recordings)"},
#endif
    ...
    {"low_latency_throttle", NULL, rc_bool, &g_low_latency_throttle, "1", 0, 0, NULL,
     "Distribute CPU execution across one emulated frame to minimize flipper latency"},

    {"vgmwrite", NULL, rc_bool, &g_vgmwrite, "0", 0, 0, NULL,
     "Enable to write a VGM of the current session (name is based on romname)"},
...
static const char* GlobalSettings[] = {
    // fileio_opts
    "rompath", "samplepath", "cfg_directory", "nvram_directory", "memcard_directory", "input_directory",
    "hiscore_directory", "state_directory", "artwork_directory", "snapshot_directory", "diff_directory",
    "wave_directory", "cheat_file", "history_file", "mameinfo_file",

    // input_opts
    "hotrod", "hotrodse", "mouse", "joystick", "steadykey",

    // video_opts
    "screen", "window",

    // performance opts
    "cpu_affinity_mask", "low_latency_throttle",
...
const static char* IgnoredSettings[] = {"resolution", "debug_resolution", "maximize", "throttle", "sleep", "log",

                                        NULL};

const static char* RunningGameSettings[] = {
    // fileio_opts
    "dmd_pos_x", "dmd_pos_y", "dmd_doublesize", "dmd_border", "dmd_title", "dmd_width", "dmd_height",

    "dmd_colorize", "dmd_red66", "dmd_green66", "dmd_blue66", "dmd_red33", "dmd_green33", "dmd_blue33", "dmd_red0",
    "dmd_green0", "dmd_blue0", "dmd_opacity", "resampling_quality",
#if defined(VPINMAME_ALTSOUND) || defined(VPINMAME_PINSOUND)
    "sound_mode",
#endif

    "showpindmd", "showwindmd",
...
int
set_option(const char* const name, const char* const arg, int priority) {
    return rc_set_option(rc, name, arg, priority);
}

void*
get_option(const char* const name) {
...
BOOL
FindSettingInList(const char* pList[], const char* const pszName) {
    if (!pszName || !*pszName)
        return FALSE;
...
BOOL
IsGlobalSetting(const char* const pszName) {
    return FindSettingInList(GlobalSettings, pszName);
}

BOOL
IsPathOrFile(const char* const pszName) {
    return FindSettingInList(PathOrFileSettings, pszName);
}

BOOL
IgnoreSetting(const char* const pszName) {
    return FindSettingInList(IgnoredSettings, pszName);
}

BOOL
SettingAffectsRunningGame(const char* const pszName) {
    return FindSettingInList(RunningGameSettings, pszName);
}

bool
RegLoadOpts(HKEY hKey, rc_option* pOpt, char* pszDefault, char* pszValue) {
    if (!pszValue)
        return false;
...
bool
RegSaveOpts(HKEY hKey, rc_option* pOpt, void* pValue) {
    bool fFailed = true;
    char* pszValue;
    DWORD dwValue;
    char szTemp[256];
    ...
    switch (pOpt->type) {
        case rc_string:
            ...
            if (pszValue)
                fFailed = (RegSetValueEx(hKey, pOpt->name, 0, REG_SZ, (LPBYTE)pszValue, strlen(pszValue) + 1)
                           != ERROR_SUCCESS);
            else {
                lstrcpy(szTemp, "");
                fFailed =
                    (RegSetValueEx(hKey, pOpt->name, 0, REG_SZ, (LPBYTE)&szTemp, strlen(szTemp) + 1) != ERROR_SUCCESS);
            }
            break;
        ...
        case rc_bool:
            ...
            fFailed =
                (RegSetValueEx(hKey, pOpt->name, 0, REG_DWORD, (LPBYTE)&dwValue, sizeof(dwValue)) != ERROR_SUCCESS);
            break;
...
SaveGlobalSettings() {
    ...
    HKEY hKey;
    DWORD dwDisposition;
    if (RegCreateKeyEx(HKEY_CURRENT_USER, szKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &hKey,
                       &dwDisposition)
        != ERROR_SUCCESS)
        return;
...
void
SaveGameSettings(const char* const pszGameName) {
    char szKey[MAX_PATH];
    lstrcpy(szKey, REG_BASEKEY);
    lstrcat(szKey, "\\");
...
void
LoadGameSettings(const char* const pszGameName) {
    bool fNew = false;
...
void
DeleteGameSettings(const char* const pszGameName) {
    char szKey[MAX_PATH];
    lstrcpy(szKey, REG_BASEKEY);
...
BOOL
GetSetting(const char* const pszGameName, const char* const pszName, VARIANT* pVal) {
    if (!pszName || !*pszName)
        return FALSE;
...
BOOL
PutSetting(const char* const pszGameName, const char* const pszName, VARIANT vValue) {
    if (!pszName || !*pszName)
        return FALSE;
...
BOOL
WriteRegistry(const char* const pszKey, const char* const pszName, DWORD dwValue) {
    HKEY hKey;
    DWORD dwDisposition;
    ...
    if (RegCreateKeyEx(HKEY_CURRENT_USER, pszKey, 0, NULL, REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, NULL, &hKey,
                       &dwDisposition)
        != ERROR_SUCCESS)
        return FALSE;
...
DWORD
ReadRegistry(const char* const pszKey, const char* const pszName, DWORD dwDefault) {
...
char*
GetInstallDir(char* const pszInstallDir, int iSize) {
    if (!pszInstallDir)
        return NULL;
