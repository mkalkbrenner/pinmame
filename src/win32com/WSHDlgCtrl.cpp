...
STDMETHODIMP
CWSHDlgCtrl::InterfaceSupportsErrorInfo(REFIID riid) {
    static const IID* arr[] = {&IID_IWSHDlgCtrl};
...
const char* pszCtrlTypes[] = {"cmdbtn", "okbtn", "cancelbtn", "chkbox", "optbtn", "frame", "label", 0};
...
STDMETHODIMP
CWSHDlgCtrl::get_Type(BSTR* pVal) {
    if (!pVal)
        return S_FALSE;
...
STDMETHODIMP
CWSHDlgCtrl::get_Value(VARIANT* pVal) {
    if (!pVal)
        return S_FALSE;
...
STDMETHODIMP
CWSHDlgCtrl::put_Value(VARIANT newVal) {
    switch (m_iType) {
...
STDMETHODIMP
CWSHDlgCtrl::get_Title(BSTR* pVal) {
    if (!pVal)
        return S_FALSE;
...
STDMETHODIMP
CWSHDlgCtrl::put_Title(BSTR newVal) {
...
STDMETHODIMP
CWSHDlgCtrl::Init(BSTR sType, long x, long y, long w, long h, BSTR sTitle) {
