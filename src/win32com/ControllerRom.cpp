...
DWORD
CRom::GetChecksumFromHash(char* szHash) {
    DWORD dwChecksum = 0;
    if (szHash && *szHash)
...
STDMETHODIMP
CRom::InterfaceSupportsErrorInfo(REFIID riid) {
    static const IID* arr[] = {&IID_IRom};
...
HRESULT
CRom::Init(const struct GameDriver* gamedrv, const struct RomModule* region, const struct RomModule* rom) {
...
STDMETHODIMP
CRom::get_StateDescription(BSTR* pVal) {
...
STDMETHODIMP
CRom::Audit(VARIANT_BOOL fStrict) {
    int err;
    char szHash[256];
...
STDMETHODIMP
CRom::get_ExpChecksum(long* pVal) {
