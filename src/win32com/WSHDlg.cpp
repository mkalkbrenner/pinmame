...
STDMETHODIMP
CWSHDlg::InterfaceSupportsErrorInfo(REFIID riid) {
    static const IID* arr[] = {&IID_IWSHDlg};
...
STDMETHODIMP
CWSHDlg::AddCtrl(BSTR sType, long x, long y, long w, long h, BSTR sTitle, IUnknown** pRetVal) {
    if (!pRetVal)
        return S_FALSE;
    *pRetVal = NULL;
...
STDMETHODIMP
CWSHDlg::get_Ctrls(IWSHDlgCtrls** ppVal) {
    if (!ppVal)
        return S_FALSE;
...
STDMETHODIMP
CWSHDlg::Show(LONG_PTR hParentWnd, VARIANT* RetVal) {
    if (!RetVal)
        return S_FALSE;
...
#pragma pack()
    ...
    Template.DlgTemplate.style = DS_SETFONT | DS_ABSALIGN | DS_MODALFRAME | DS_SETFOREGROUND | DS_3DLOOK | DS_CENTER
    ...
    RetVal->vt = VT_I4;
    RetVal->lVal =
        DialogBoxIndirectParam(_Module.m_hInst, &Template.DlgTemplate, (HWND)hParentWnd, WSHDlgProc, (LPARAM)this);
...
void
SaveDlgValues(HWND hDlg, CWSHDlgCtrls* pWSHDlgCtrls) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG:
            ...
            for (i = 0; i < pWSHDlgCtrls->m_lCount; i++) {
                ...
                int iCommonOptions =
                    WS_CHILD | WS_VISIBLE
                    | ((iPreviousType != pWSHDlgCtrls->m_pCtrlList[i]->m_iType) ? WS_GROUP | WS_TABSTOP : 0);
                ...
                switch (pWSHDlgCtrls->m_pCtrlList[i]->m_iType) {
                    case CTRLTYPE_PUSHBUTTON:
                        hWnd = CreateWindow("button", pWSHDlgCtrls->m_pCtrlList[i]->m_szTitle,
                                            BS_PUSHBUTTON | iCommonOptions, x, y, w, h, hDlg, (HMENU)(CTRLID_START + i),
                    ...
                    case CTRLTYPE_OKBUTTON:
                        hWnd = CreateWindow("button", pWSHDlgCtrls->m_pCtrlList[i]->m_szTitle,
                                            BS_DEFPUSHBUTTON | iCommonOptions, x, y, w, h, hDlg,
                                            (HMENU)(CTRLID_START + i), _Module.m_hInst, NULL);
                    ...
                    case CTRLTYPE_CHECKBOX:
                        hWnd = CreateWindow("button", pWSHDlgCtrls->m_pCtrlList[i]->m_szTitle,
                                            BS_AUTOCHECKBOX | iCommonOptions, x, y, w, h, hDlg,
                                            (HMENU)(CTRLID_START + i), _Module.m_hInst, NULL);
                        if (hWnd)
                    ...
                    case CTRLTYPE_RADIONBUTTON:
                        hWnd = CreateWindow("button", pWSHDlgCtrls->m_pCtrlList[i]->m_szTitle,
                                            BS_AUTORADIOBUTTON | iCommonOptions, x, y, w, h, hDlg,
                                            (HMENU)(CTRLID_START + i), _Module.m_hInst, NULL);
                        if (hWnd)
                            SendMessage(hWnd, BM_SETCHECK, pWSHDlgCtrls->m_pCtrlList[i]->m_vValue.lVal, 0);
                    ...
                    case CTRLTYPE_GROUPBOX:
                        hWnd = CreateWindow("button", pWSHDlgCtrls->m_pCtrlList[i]->m_szTitle,
                                            BS_GROUPBOX | iCommonOptions & ~WS_TABSTOP, x, y, w, h, hDlg,
                                            (HMENU)(CTRLID_START + i), _Module.m_hInst, NULL);
                    ...
                    case CTRLTYPE_LABEL:
                        hWnd = CreateWindow("static", pWSHDlgCtrls->m_pCtrlList[i]->m_szTitle,
                                            iCommonOptions & ~WS_TABSTOP, x, y, w, h, hDlg, (HMENU)(CTRLID_START + i),
                                            _Module.m_hInst, NULL);
            ...
            if (fPlaceStandardButtons) {
                ...
                if (iIDOkButton != -1) {
                    ...
                    SetWindowPos(GetDlgItem(hDlg, iIDOkButton), 0, iPosX, Rect.bottom, 0, 0, SWP_NOZORDER | SWP_NOSIZE);
                ...
                if (iIDCancelButton != -1) {
                    ...
                    SetWindowPos(GetDlgItem(hDlg, iIDCancelButton), 0, iPosX, Rect.bottom, 0, 0,
                    ...
                    default:
                        ...
                        switch (pWSHDlgCtrls->m_pCtrlList[LOWORD(wParam) - CTRLID_START]->m_iType) {
                            ...
                            case CTRLTYPE_OKBUTTON:
                                SaveDlgValues(hDlg, pWSHDlgCtrls);
                                EndDialog(hDlg,
                                          pWSHDlgCtrls->m_pCtrlList[LOWORD(wParam) - CTRLID_START]->m_vValue.lVal);
                                break;
