...
class CSplashWnd : public CWindowImpl<CSplashWnd> {
  ...
  private:
    ...
    LRESULT
    OnCreate(UINT, WPARAM, LPARAM lParam, BOOL&) {
        ...
        HDC hDC = GetDC();
        m_hFont = CreateFont(-MulDiv(8, GetDeviceCaps(hDC, LOGPIXELSY), 72), 0, 0, 0, iWeight, 0, 0, 0, ANSI_CHARSET, 0,
                             0, 0, FF_DONTCARE, "Microsoft Sans Serife");
        ReleaseDC(hDC);
    ...
    LRESULT
    OnClick(UINT, WPARAM, LPARAM, BOOL&) {
        DestroyWindow();
        return 1;
    }

    LRESULT
    OnChar(UINT, WPARAM wParam, LPARAM, BOOL&) {
        if (wParam != 'a')
            DestroyWindow();
    ...
    LRESULT
    OnPaint(UINT, WPARAM, LPARAM, BOOL&) {
        RECT Rect;
...
void
CreateSplashWnd(void** ppData, char* pszCredits) {
    if (!ppData)
        return;
...
void
WaitForSplashWndToClose(void** ppData) {
    if (!ppData || !*ppData)
        return;
