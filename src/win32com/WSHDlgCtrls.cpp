...
class ATL_NO_VTABLE CEnumWSHDlgCtrls
    : public CComObjectRootEx<CComMultiThreadModel>,
      ...
      public IEnumVARIANT {
  public:
    CEnumWSHDlgCtrls() { m_pWSHDlgCtrls = NULL; }
    ...
    STDMETHOD(InterfaceSupportsErrorInfo)(REFIID riid) {
        static const IID* arr[] = {&IID_IEnumWSHDlgCtrls};
  ...
  public:
    ...
    void
    Init(IWSHDlgCtrls* pWSHDlgCtrls) {
...
STDMETHODIMP
CWSHDlgCtrls::InterfaceSupportsErrorInfo(REFIID riid) {
    static const IID* arr[] = {&IID_IWSHDlgCtrls};
    for (int i = 0; i < sizeof(arr) / sizeof(arr[0]); i++) {
        if (InlineIsEqualGUID(*arr[i], riid))
            return S_OK;
    }
...
STDMETHODIMP
CWSHDlgCtrls::get_Count(long* pnCount) {
    *pnCount = m_lCount;
    return S_OK;
}

STDMETHODIMP
CWSHDlgCtrls::get_Item(VARIANT* pKey, IWSHDlgCtrl** ppWSHDlgCtrl) {
    HRESULT hr = S_FALSE;
    *ppWSHDlgCtrl = NULL;
...
STDMETHODIMP
CWSHDlgCtrls::get__NewEnum(IUnknown** ppunkEnum) {
    *ppunkEnum = 0;
...
STDMETHODIMP
CWSHDlgCtrls::add_Item(IWSHDlgCtrl* pWSHDlgCtrl) {
    if (!pWSHDlgCtrl)
        return S_FALSE;
