...
class CGameSettingsDlg : public CDialogImpl<CGameSettingsDlg> {
  ...
  private:
    ...
    GetControlValues() {
        ...
        pGameSettings->put_Value(CComBSTR("dmd_doublesize"),
                                 CComVariant((int)GetDlgItemInt(IDC_DOUBLESIZE, NULL, TRUE)));
        ...
        pGameSettings->put_Value(CComBSTR("resampling_quality"),
                                 CComVariant((int)GetDlgItemInt(IDC_RESAMPLEQ, NULL, TRUE)));
    ...
    /***********************************************************************/
    void
    GetDMD_RGB_Color(int* r, int* g, int* b, int* p2, int* p3, int* p4) {
        UINT value;
    ...
    void
    SetChanged(bool fChanged) {
        m_fChanged = fChanged;
    }

    // message handlers
    LRESULT
    OnInitDialog(UINT, WPARAM, LPARAM lParam, BOOL&) {
        CenterWindow();
    ...
    LRESULT
    OnDestroy(UINT, WPARAM, LPARAM, BOOL&) {
        if (m_hBrushDMDColor)
    ...
    LRESULT
    OnCtrlColorStatic(UINT, WPARAM wParam, LPARAM lParam, BOOL&) {
        if (::GetDlgCtrlID((HWND)lParam) == IDC_DMDSHOW1 || ::GetDlgCtrlID((HWND)lParam) == IDC_DMDSHOW2
            || ::GetDlgCtrlID((HWND)lParam) == IDC_DMDSHOW3 || ::GetDlgCtrlID((HWND)lParam) == IDC_DMDSHOW4) {
    ...
    BOOL
    click_in_color(POINT pt, int id, RGB& color) {
        RECT rc;
    ...
    LRESULT
    OnLeftButtonDown(UINT, WPARAM, LPARAM lpar, BOOL&) {
        if (IsDlgButtonChecked(IDC_DMD_COLORIZE)) {
            ...
            if (click_in_color(pt, IDC_DMDSHOW1, dmd100)) {
                ...
                return 0;
            } else if (click_in_color(pt, IDC_DMDSHOW2, m_dmd66) || click_in_color(pt, IDC_DMDSHOW3, m_dmd33)
    ...
    // command handlers
    LRESULT
    OnCheckBox(WORD, UINT, HWND, BOOL&) {
        SetChanged(true);
        return 0;
    }

    LRESULT
    OnEditCtrlChanged(WORD, UINT, HWND, BOOL&) {
        SetChanged(true);
        return 0;
    }

    LRESULT
    OnOK(WORD, UINT, HWND, BOOL&) {
        if (m_fChanged) {
            ...
            if (IsEmulationRunning())
                MessageBox("Your changes will not take effect until you restart Visual PinMAME!", "Notice!",
    ...
    LRESULT
    OnCancel(WORD, UINT, HWND, BOOL&) {
        EndDialog(IDCANCEL);
        return 0;
    }

    LRESULT
    OnAbout(WORD, UINT, HWND, BOOL&) {
        ShowAboutDlg(m_hWnd);
        return 0;
    }

    LRESULT
    OnResetToDefault(WORD, UINT, HWND, BOOL&) {
    ...
    LRESULT
    OnGetColor(WORD, UINT uCtrlId, HWND, BOOL&) {
        int r, g, b, p2, p3, p4;
    ...
    void
    show_hide_colorize_ctls() {
        int shade_ctls[] = {IDC_DMDRED,      IDC_DMDGREEN,    IDC_DMDBLUE,     IDGETCOLOR,
                            IDC_DMDPERC66,   IDC_DMDPERC33,   IDC_DMDPERC0,    IDC_DMD_STATIC1,
                            IDC_DMD_STATIC2, IDC_DMD_STATIC3, IDC_DMD_STATIC4, IDC_DMD_STATIC5,
                            IDC_DMD_STATIC6, IDC_DMD_STATIC7, IDC_DMD_STATIC8, 0};
        int colorize_ctls[] = {IDC_DMD_STATIC1A, 0};
    ...
    LRESULT
    OnDmdColorize(WORD, UINT uCtrlId, HWND, BOOL&) {
        show_hide_colorize_ctls();
...
STDMETHODIMP
CGameSettings::InterfaceSupportsErrorInfo(REFIID riid) {
    static const IID* arr[] = {&IID_IGameSettings};
...
void
CGameSettings::Init(IGame* pGame) {
    if (!pGame)
        return;
...
STDMETHODIMP
CGameSettings::ShowSettingsDlg(LONG_PTR hParentWnd) {
    switch (hParentWnd) {
        case 0:
            break;
...
STDMETHODIMP
CGameSettings::get_Value(BSTR sName, VARIANT* pVal) {
    char szName[4096];
...
STDMETHODIMP
CGameSettings::put_Value(BSTR sName, VARIANT newVal) {
    char szName[4096];
    WideCharToMultiByte(CP_ACP, 0, sName, -1, szName, sizeof szName, NULL, NULL);
...
STDMETHODIMP
CGameSettings::SetDisplayPosition(VARIANT newValX, VARIANT newValY, LONG_PTR hWnd) {
