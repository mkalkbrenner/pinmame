  ...
  private:
    ...
    OnInitDialog(UINT, WPARAM, LPARAM lParam, BOOL&) {
        ...
        while (SUCCEEDED(pEnumRoms->Next(1, &vRom, &uFetched)) && uFetched) {
            ...
            sprintf(szLine, "%s\t%s\t%i\t%i\t0x%08X\t0x%08X\t", szName, szType, lExpLength, lLength, lExpChecksum,
                    lChecksum);
            ...
            if (!(m_nShowOptions & CHECKOPTIONS_IGNORESOUNDROMS) || !(lFlags & ROMREGION_SOUNDONLY)) {
                ...
                fMaybeOK &= (lState == AUD_ROM_GOOD) || (lState == AUD_ROM_NEED_DUMP) || (lState == AUD_ROM_NEED_REDUMP)
                            || (lState == AUD_NOT_AVAILABLE) || (lState == AUD_LENGTH_MISMATCH)
                            || (lState == AUD_BAD_CHECKSUM);
            }
        ...
        switch (iError) {
            case 0:
                if (fOK) {
                    ...
                    strcpy(szLine, "There are CRC and/or length errors! VPinMAME may or may not successfully be able "
    ...
    LRESULT
    OnDontCare(WORD, UINT, HWND, BOOL&) {
        EndDialog(IDIGNORE);
        return 0;
    }
...
STDMETHODIMP
CGame::InterfaceSupportsErrorInfo(REFIID riid) {
    static const IID* arr[] = {&IID_IGame};
...
HRESULT
CGame::Init(const struct GameDriver* gamedrv) {
    m_gamedrv = gamedrv;
...
STDMETHODIMP
CGame::get_Description(BSTR* pVal) {
    if (!m_gamedrv) {
...
STDMETHODIMP
CGame::get_Manufacturer(BSTR* pVal) {
    if (!m_gamedrv) {
...
STDMETHODIMP
CGame::get_Settings(IGameSettings** pVal) {
...
char*
GetGameRegistryKey(char* pszRegistryKey, const char* const pszROMName) {
    if (!pszRegistryKey)
        return nullptr;
...
BOOL
GameUsedTheFirstTime(const char* const pszROMName) {
    char szKey[MAX_PATH];
    ...
    if (RegOpenKeyEx(HKEY_CURRENT_USER, szKey, 0, KEY_QUERY_VALUE, &hKey) != ERROR_SUCCESS) {
        DWORD dwDisposition;
        if (RegCreateKeyEx(HKEY_CURRENT_USER, szKey, 0, nullptr, REG_OPTION_NON_VOLATILE, KEY_SET_VALUE, nullptr, &hKey,
                           &dwDisposition)
            == ERROR_SUCCESS)
            RegCloseKey(hKey);
        return true;
...
BOOL
GameWasNeverStarted(char* pszROMName) {
    char szKey[MAX_PATH];
    GetGameRegistryKey(szKey, pszROMName);
...
void
SetGameWasStarted(char* pszROMName) {
    char szKey[MAX_PATH];
    GetGameRegistryKey(szKey, pszROMName);
...
STDMETHODIMP
CGame::ShowInfoDlg(int nShowOptions, LONG_PTR hParentWnd, int* pVal) {
    if (!pVal)
        return S_FALSE;
...
STDMETHODIMP
CGame::get_IsSupported(VARIANT_BOOL* pVal) {
    if (!pVal)
        return S_FALSE;
