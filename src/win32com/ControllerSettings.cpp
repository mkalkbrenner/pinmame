...
class CControllerSettingsDlg : public CDialogImpl<CControllerSettingsDlg> {
  ...
  private:
    ...
    SetControlValues() {
        ...
        CheckDlgButton(IDC_FULLSCREEN,
                       (vValue.boolVal == VARIANT_TRUE) ? BST_UNCHECKED : BST_CHECKED); // NOTE: Inverted
        VariantClear(&vValue);
    ...
    GetControlValues() {
        ...
        pControllerSettings->put_Value(CComBSTR("window"),
                                       CComVariant((BOOL)!IsDlgButtonChecked(IDC_FULLSCREEN))); // NOTE: Inverted
    ...
    /***************************************************************/
    BOOL
    BrowseForDirectory(char* pResult) {
        BOOL bResult = FALSE;
        LPMALLOC piMalloc;
        BROWSEINFO Info;
    ...
    LRESULT
    OnDestroy(UINT, WPARAM, LPARAM lParam, BOOL&) {
        CleanupDisplayComboBox();
    ...
    LRESULT
    OnBrowseForDir(WORD, UINT uCtrlId, HWND, BOOL&) {
        char szDir[MAX_PATH];
...
STDMETHODIMP
CControllerSettings::InterfaceSupportsErrorInfo(REFIID riid) {
    static const IID* arr[] = {&IID_IControllerSettings};
...
STDMETHODIMP
CControllerSettings::ShowSettingsDlg(LONG_PTR hParentWnd) {
...
STDMETHODIMP
CControllerSettings::get_Value(BSTR sName, VARIANT* pVal) {
    char szName[4096];
...
STDMETHODIMP
CControllerSettings::put_Value(BSTR sName, VARIANT newVal) {
    char szName[4096];
    WideCharToMultiByte(CP_ACP, 0, sName, -1, szName, sizeof szName, NULL, NULL);
...
STDMETHODIMP
CControllerSettings::get_InstallDir(BSTR* pVal) {
