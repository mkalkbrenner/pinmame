...
#endif
#if (HAS_TMS9900 || HAS_TMS9940 || HAS_TMS9980 || HAS_TMS9985 || HAS_TMS9989 || HAS_TMS9995 || HAS_TMS99105A           \
     || HAS_TMS99110A)
...
#define CPU0(cpu, name, nirq, dirq, oc, datawidth, mem, shift, bits, endian, align, maxinst)                           \
    {                                                                                                                  \
        CPU_##cpu, name##_init, name##_reset, name##_exit, name##_execute, NULL, name##_get_context,                   \
            name##_set_context, NULL, NULL, name##_get_reg, name##_set_reg, name##_set_irq_line,                       \
            name##_set_irq_callback, name##_info, name##_dasm, nirq, dirq, &name##_ICount, oc, datawidth,              \
            (mem_read_handler)cpu_readmem##mem, (mem_write_handler)cpu_writemem##mem, NULL, NULL, 0,                   \
            cpu_setopbase##mem, shift, bits, CPU_IS_##endian, align, maxinst                                           \
    }

/* CPUs which have the _burn function */
#define CPU1(cpu, name, nirq, dirq, oc, datawidth, mem, shift, bits, endian, align, maxinst)                           \
    {                                                                                                                  \
        CPU_##cpu, name##_init, name##_reset, name##_exit, name##_execute, name##_burn, name##_get_context,            \
            name##_set_context, name##_get_cycle_table, name##_set_cycle_table, name##_get_reg, name##_set_reg,        \
            name##_set_irq_line, name##_set_irq_callback, name##_info, name##_dasm, nirq, dirq, &name##_ICount, oc,    \
            datawidth, (mem_read_handler)cpu_readmem##mem, (mem_write_handler)cpu_writemem##mem, NULL, NULL, 0,        \
            cpu_setopbase##mem, shift, bits, CPU_IS_##endian, align, maxinst                                           \
    }
...
#define CPU3(cpu, name, nirq, dirq, oc, datawidth, mem, shift, bits, endian, align, maxinst)                           \
    {                                                                                                                  \
        CPU_##cpu, name##_init, name##_reset, name##_exit, name##_execute, NULL, name##_get_context,                   \
            name##_set_context, NULL, NULL, name##_get_reg, name##_set_reg, name##_set_irq_line,                       \
            name##_set_irq_callback, name##_info, name##_dasm, nirq, dirq, &name##_icount, oc, datawidth,              \
            (mem_read_handler)cpu_readmem##mem, (mem_write_handler)cpu_writemem##mem, NULL, NULL, cpu##_PGM_OFFSET,    \
            cpu_setopbase##mem, shift, bits, CPU_IS_##endian, align, maxinst                                           \
    }
...
#define CPU4(cpu, name, nirq, dirq, oc, datawidth, mem, shift, bits, endian, align, maxinst)                           \
    {                                                                                                                  \
        CPU_##cpu, name##_init, name##_reset, name##_exit, name##_execute, NULL, name##_get_context,                   \
            name##_set_context, NULL, NULL, name##_get_reg, name##_set_reg, name##_set_irq_line,                       \
            name##_set_irq_callback, name##_info, name##_dasm, nirq, dirq, &name##_icount, oc, datawidth,              \
            (mem_read_handler)cpu_readmem##mem, (mem_write_handler)cpu_writemem##mem,                                  \
            (mem_read_handler)name##_internal_r, (mem_write_handler)name##_internal_w, 0, cpu_setopbase##mem, shift,   \
            bits, CPU_IS_##endian, align, maxinst                                                                      \
    }
...
void
cpuintrf_push_context(int cpunum) {
...
void
activecpu_set_irq_line(int irqline, int state) {
...
void
activecpu_set_op_base(unsigned val) {
...
unsigned
activecpu_dasm(char* buffer, unsigned pc) {
...
int
cpunum_execute(int cpunum, int cycles) {
    int ran;
...
void
cpunum_reset(int cpunum, void* param, int (*irqack)(int)) {
...
data8_t
cpunum_read_byte(int cpunum, offs_t address) {
    int result;
...
void
cpunum_write_byte(int cpunum, offs_t address, data8_t data) {
...
void*
cpunum_get_context_ptr(int cpunum) {
...
const void*
cpunum_get_cycle_table(int cpunum, int which) {
    const void* result;
...
void
cpunum_set_cycle_tbl(int cpunum, int which, void* new_table) {
...
unsigned
cpunum_get_reg(int cpunum, int regnum) {
    unsigned result;
...
void
cpunum_set_reg(int cpunum, int regnum, unsigned val) {
...
void
cpunum_set_op_base(int cpunum, unsigned val) {
...
unsigned
cpunum_dasm(int cpunum, char* buffer, unsigned pc) {
    unsigned result;
...
const char*
cpunum_dump_reg(int cpunum, int regnum) {
    const char* result;
...
static void
dummy_reset(void* param) {}

static void
dummy_exit(void) {}

static int
dummy_execute(int cycles) {
    return cycles;
}

static unsigned
dummy_get_context(void* regs) {
    return 0;
}

static void
dummy_set_context(void* regs) {}

static unsigned
dummy_get_reg(int regnum) {
    return 0;
}

static void
dummy_set_reg(int regnum, unsigned val) {}

static void
dummy_set_irq_line(int irqline, int state) {}

static void
dummy_set_irq_callback(int (*callback)(int irqline)) {}

static const char*
dummy_info(void* context, int regnum) {
    switch (regnum) {
        case CPU_INFO_NAME:
            return "";
...
static unsigned
dummy_dasm(char* buffer, unsigned pc) {
    strcpy(buffer, "???");
    return 1;
}
...
#if (HAS_M68000 || HAS_M68010 || HAS_M68020 || HAS_M68EC020 || HAS_M68306)
void
cpu_set_m68k_reset(int cpunum, void (*resetfn)(void)) {
