...
code_init(void) {
    ...
    assert(__code_key_first == 0 && __code_key_last + 1 == __code_joy_first && __code_joy_last + 1 == __code_proc_first
           && __code_proc_last + 1 == __code_max);
#else  /* PINMAME && PROC_SUPPORT */
    assert(__code_key_first == 0 && __code_key_last + 1 == __code_joy_first && __code_joy_last + 1 == __code_max);
...
INLINE const struct KeyboardInfo*
internal_oscode_find_keyboard(unsigned oscode) {
...
INLINE const struct JoystickInfo*
internal_oscode_find_joystick(unsigned oscode) {
...
#if defined(PINMAME) && defined(PROC_SUPPORT)
INLINE const struct PROCInfo*
internal_oscode_find_proc(unsigned oscode) {
...
static int
internal_oscode_find(unsigned oscode, unsigned type) {
    unsigned i;
    const struct KeyboardInfo* keyinfo;
    const struct JoystickInfo* joyinfo;
#if defined(PINMAME) && defined(PROC_SUPPORT)
...
static int
internal_oscode_add(unsigned oscode, unsigned type) {
...
INLINE const struct KeyboardInfo*
internal_code_find_keyboard(InputCode code) {
    const struct KeyboardInfo* keyinfo;
    keyinfo = osd_get_key_list();
...
INLINE const struct JoystickInfo*
internal_code_find_joystick(InputCode code) {
    const struct JoystickInfo* joyinfo;
    joyinfo = osd_get_joy_list();
...
#if defined(PINMAME) && defined(PROC_SUPPORT)
INLINE const struct PROCInfo*
internal_code_find_proc(InputCode code) {
    const struct PROCInfo* procinfo;
    procinfo = osd_get_proc_list();
...
static int
internal_code_pressed(InputCode code) {
    assert(code < code_mac);
...
static const char*
internal_code_name(InputCode code) {
    const struct KeyboardInfo* keyinfo;
    const struct JoystickInfo* joyinfo;
#if defined(PINMAME) && defined(PROC_SUPPORT)
    const struct PROCInfo* procinfo;
...
InputCode
keyoscode_to_code(unsigned oscode) {
    InputCode code;
...
InputCode
joyoscode_to_code(unsigned oscode) {
...
InputCode
procoscode_to_code(unsigned oscode) {
...
InputCode
savecode_to_code(unsigned savecode) {
...
unsigned
code_to_savecode(InputCode code) {
...
#if defined(PINMAME) && defined(PROC_SUPPORT)
        case CODE_TYPE_PROC:
            return code_map[code].oscode | SAVECODE_FLAGS_TYPE_PROC;
#endif /* PINMAME && PROC_SUPPORT */
    }
...
int
code_pressed_memory(InputCode code) {
    int pressed;
...
static int
code_pressed_not_memorized(InputCode code) {
    int pressed;
...
int
code_pressed_memory_repeat(InputCode code, int speed) {
    static int counter;
    static int keydelay;
    int pressed;
...
void
seq_set_1(InputSeq* a, InputCode code) {
    int j;
    (*a)[0] = code;
    for (j = 1; j < SEQ_MAX; ++j)
        (*a)[j] = CODE_NONE;
...
void
seq_set_2(InputSeq* a, InputCode code1, InputCode code2) {
    int j;
    (*a)[0] = code1;
    (*a)[1] = code2;
    for (j = 2; j < SEQ_MAX; ++j)
...
void
seq_set_3(InputSeq* a, InputCode code1, InputCode code2, InputCode code3) {
    int j;
    (*a)[0] = code1;
    (*a)[1] = code2;
    (*a)[2] = code3;
...
void
seq_set_4(InputSeq* a, InputCode code1, InputCode code2, InputCode code3, InputCode code4) {
    int j;
    (*a)[0] = code1;
    (*a)[1] = code2;
    (*a)[2] = code3;
...
void
seq_set_5(InputSeq* a, InputCode code1, InputCode code2, InputCode code3, InputCode code4, InputCode code5) {
    int j;
    (*a)[0] = code1;
    (*a)[1] = code2;
    (*a)[2] = code3;
...
void
seq_copy(InputSeq* a, InputSeq* b) {
    int j;
    for (j = 0; j < SEQ_MAX; ++j)
        (*a)[j] = (*b)[j];
}
...
void
seq_name(InputSeq* code, char* buffer, size_t max) {
    int j;
    char* dest = buffer;
    for (j = 0; j < SEQ_MAX; ++j) {
        const char* name;
...
*/
int
seq_read_async(InputSeq* seq, int first) {
    InputCode newkey;
...
int
input_ui_pressed_repeat(int code, int speed) {
...
int
is_joystick_axis_code(unsigned code) {
    assert(code < code_mac);
...
int
return_os_joycode(InputCode code) {
    assert(code < code_mac);
