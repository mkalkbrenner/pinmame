...
struct rc_option frontend_opts[] = {
    ...
    /* list options follow */
    {"list", "ls", rc_set_int, &list, NULL, LIST_SHORT, 0, NULL,
    ...
    {"verifysets", NULL, rc_set_int, &verify, NULL, VERIFY_ROMS | VERIFY_VERBOSE | VERIFY_TERSE, 0, NULL,
     "verify checksums of romsets (terse)"},
    {"vset", NULL, rc_set_int, &verify, NULL, VERIFY_ROMS | VERIFY_VERBOSE, 0, NULL,
     "verify checksums of a romset (verbose)"},
    {"verifysamples", NULL, rc_set_int, &verify, NULL, VERIFY_SAMPLES | VERIFY_VERBOSE, 0, NULL,
...
static void
namecopy(char* name_ref, const char* desc) {
    char name[200];
...
static void
match_roms(const struct GameDriver* driver, const char* hash, int* found) {
...
void
identify_rom(const char* name, const char* hash, int length) {
    int found = 0;
...
void
identify_zip(const char* zipname) {
    struct zipent* ent;
...
void
identify_dir(const char* dirname) {
    DIR* dir;
    struct dirent* ent;
...
void
romident(const char* name, int enter_dirs) {
    struct stat s;
    ...
    switch (list) /* front-end utilities ;) */
    {
        ...
        case LIST_SHORT: /* simple games list */
            ...
            for (i = j = 0; drivers[i]; i++)
                if ((listclones || drivers[i]->clone_of == 0 || (drivers[i]->clone_of->flags & NOT_A_DRIVER))
        ...
        case LIST_LMR: {
            ...
            for (i = 0; drivers[i]; i++) {
                ...
                if (RomsetMissing(i)) {
                    ...
                    printf("%-10s%-10s%s\n", drivers[i]->name, (drivers[i]->clone_of) ? drivers[i]->clone_of->name : "",
        ...
        case LIST_WRONGORIENTATION: /* list drivers which incorrectly use the orientation and visible area fields */
            for (i = 0; drivers[i]; i++) {
                ...
                if ((drv.video_attributes & VIDEO_TYPE_VECTOR) == 0
                    ...
                    if (strcmp(drivers[i]->name, "crater") && strcmp(drivers[i]->name, "mpatrol")
                        ...
                        && strcmp(drivers[i]->name, "tomahawk") && 1)
                        printf("%s %dx%d\n", drivers[i]->name,
                               drv.default_visible_area.max_x - drv.default_visible_area.min_x + 1,
                               drv.default_visible_area.max_y - drv.default_visible_area.min_y + 1);
                }
...
#define MAXCOUNT 8
            ...
            for (i = 0; drivers[i]; i++) {
                if (drivers[i]->clone_of == 0 || (drivers[i]->clone_of->flags & NOT_A_DRIVER)) {
                    ...
                    for (j = 0; drivers[j]; j++) {
                        if (drivers[j]->clone_of == 0 || (drivers[j]->clone_of->flags & NOT_A_DRIVER)) {
            ...
            for (i = 0; drivers[i]; i++) {
                ...
                for (region = rom_first_region(drivers[i]); region; region = rom_next_region(region)) {
                    for (rom = rom_first_file(region); rom; rom = rom_next_file(rom)) {
                        if (!hash_data_has_info(ROM_GETHASHDATA(rom), HASH_INFO_NO_DUMP)) {
                            for (j = 0; drivers[j]; j++) {
                                if (j != i && drivers[j]->clone_of && (drivers[j]->clone_of->flags & NOT_A_DRIVER) == 0
                                        ...
                                        for (rom1 = rom_first_file(region1); rom1; rom1 = rom_next_file(rom1)) {
                                            if (!strcmp(ROM_GETNAME(rom), ROM_GETNAME(rom1))) {
                                                if (!hash_data_has_info(ROM_GETHASHDATA(rom1), HASH_INFO_NO_DUMP)
                                                    && !hash_data_is_equal(ROM_GETHASHDATA(rom), ROM_GETHASHDATA(rom1),
                                                                           0)) {
                                                    char temp[512];
                        ...
                        for (rom = rom_first_file(region); rom; rom = rom_next_file(rom)) {
                            for (chunk = rom_first_chunk(rom); chunk; chunk = rom_next_chunk(chunk)) {
                                ...
                                if (type >= REGION_CPU1 && type <= REGION_CPU8)
                                    romcpu += ROM_GETLENGTH(chunk);
                                if (type >= REGION_GFX1 && type <= REGION_GFX8)
                                    romgfx += ROM_GETLENGTH(chunk);
                                if (type >= REGION_SOUND1 && type <= REGION_SOUND8)
                                    romsound += ROM_GETLENGTH(chunk);
                            }
                        }
                    }
...
#undef MAXCOUNT
    ...
    if (verify) /* "verify" utilities */
    {
        ...
        if (correct + incorrect == 0) {
            ...
            printf("%d %s found, %d were OK.\n", correct + incorrect, (verify & VERIFY_ROMS) ? "romsets" : "samplesets",
                   correct);
            if (incorrect > 0)
                return 2;
            else
