...
DWORD filterExclusion[NUM_EXCLUSIONS] = {IDC_FILTER_CLONES,  IDC_FILTER_ORIGINALS,   IDC_FILTER_NONWORKING,
                                         IDC_FILTER_WORKING, IDC_FILTER_UNAVAILABLE, IDC_FILTER_AVAILABLE,
                                         IDC_FILTER_RASTER,  IDC_FILTER_VECTOR};

static void DisableFilterControls(HWND hWnd, LPFOLDERDATA lpFilterRecord, LPFILTER_ITEM lpFilterItem, DWORD dwFlags);
...
INT_PTR CALLBACK
ResetDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
...
INT_PTR CALLBACK
InterfaceDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
...
INT_PTR CALLBACK
FilterDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
...
AboutDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
    switch (Msg) {
        case WM_INITDIALOG: {
            HBITMAP hBmp;
            hBmp =
                (HBITMAP)LoadImage(GetModuleHandle(NULL), MAKEINTRESOURCE(IDB_ABOUTS), IMAGE_BITMAP, 0, 0, LR_SHARED);
...
INT_PTR CALLBACK
AddCustomFileDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (Msg) {
        case WM_INITDIALOG: {
            ...
            SetWindowText(GetDlgItem(hDlg, IDC_CUSTOMFILE_GAME), ModifyThe(drivers[driver_index]->description));
...
INT_PTR CALLBACK
DirectXDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
    HWND hEdit;

    const char* directx_help = MAME32NAME " requires DirectX version 3 or later, which is a set of operating\r\n"
    ...
    switch (Msg) {
        ...
        case WM_COMMAND:
            if (LOWORD(wParam) == IDB_WEB_PAGE)
                ShellExecute(GetMainWindow(), NULL, "http://www.microsoft.com/directx", NULL, NULL, SW_SHOWNORMAL);
...
static void
DisableFilterControls(HWND hWnd, LPFOLDERDATA lpFilterRecord, LPFILTER_ITEM lpFilterItem, DWORD dwFlags) {
...
static void
EnableFilterExclusions(HWND hWnd, DWORD dwCtrlID) {
    int i;
    DWORD id;
...
static DWORD
ValidateFilters(LPFOLDERDATA lpFilterRecord, DWORD dwFlags) {
    DWORD dwFilters;
