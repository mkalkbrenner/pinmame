...
static BOOL WINAPI DDEnumInfo(GUID FAR* lpGUID, LPSTR lpDriverDescription, LPSTR lpDriverName, LPVOID lpContext,
                              HMONITOR hm);

static BOOL WINAPI DDEnumOldInfo(GUID FAR* lpGUID, LPSTR lpDriverDescription, LPSTR lpDriverName, LPVOID lpContext);
...
DirectDraw_GetDisplayModes(void) {
...
int
DirectDraw_GetNumDisplays(void) {
    return g_nNumDisplays;
}

BOOL
DirectDraw_HasHWStretch(void) {
    return g_bHWStretch;
}

BOOL
DirectDraw_HasRefresh(void) {
    return g_bRefresh;
}

const char*
DirectDraw_GetDisplayName(int num_display) {
...
static BOOL WINAPI
DDEnumInfo(GUID FAR* lpGUID, LPSTR lpDriverDescription, LPSTR lpDriverName, LPVOID lpContext, HMONITOR hm) {
...
static BOOL WINAPI
DDEnumOldInfo(GUID FAR* lpGUID, LPSTR lpDriverDescription, LPSTR lpDriverName, LPVOID lpContext) {
    return DDEnumInfo(lpGUID, lpDriverDescription, lpDriverName, lpContext, NULL);
}

static HRESULT CALLBACK
EnumDisplayModesCallback(LPDDSURFACEDESC pddsd, LPVOID Context) {
...
static HRESULT CALLBACK
EnumDisplayModesCallback2(DDSURFACEDESC2* pddsd2, LPVOID Context) {
    struct tDisplayModes* pDisplayModes = (struct tDisplayModes*)Context;
...
static void
CalculateDisplayModes(void) {
    g_DisplayModes.m_nNumModes = 0;

    if (g_pDirectDraw4)
        IDirectDraw4_EnumDisplayModes(g_pDirectDraw4, DDEDM_REFRESHRATES, NULL, &g_DisplayModes,
