...
void
CalcSplitter(HWND hWnd, LPHZSPLITTER lpSplitter) {
    POINT p = {0, 0};
...
void
AdjustSplitter2Rect(HWND hWnd, LPRECT lpRect) {
    RECT pRect;
...
void
AdjustSplitter1Rect(HWND hWnd, LPRECT lpRect) {}

void
RecalcSplitters(void) {
    int i;
...
void
AddSplitter(HWND hWnd, HWND hWndLeft, HWND hWndRight, void (*func)(HWND hWnd, LPRECT lpRect)) {
...
void
OnInvertTracker(HWND hWnd, const RECT* rect) {
    HDC hDC = GetDC(hWnd);
...
void
StartTracking(HWND hWnd, UINT hitArea) {
...
UINT
SplitterHitTest(HWND hWnd, POINTS p) {
    RECT rect;
    POINT pt;
    int i;
...
void
OnMouseMove(HWND hWnd, UINT nFlags, POINTS p) {
...
void
OnLButtonDown(HWND hWnd, UINT nFlags, POINTS p) {
...
void
OnLButtonUp(HWND hWnd, UINT nFlags, POINTS p) {
    if (bTracking) {
        StopTracking(hWnd);
    }
}
