...
DIJoystick_init(void) {
    ...
    hr = IDirectInput_EnumDevices(di, DIDEVTYPE_JOYSTICK, (LPDIENUMDEVICESCALLBACK)DIJoystick_EnumDeviceProc, NULL,
                                  DIEDFL_ATTACHEDONLY);
    if (FAILED(hr)) {
...
static void
DIJoystick_poll_joysticks(void) {
    HRESULT hr;
    DWORD i;
...
static int
DIJoystick_is_joy_pressed(int joycode) {
    int joy_num;
    int stick;
    int axis;
    int dir;
...
char*
DIJoystick_GetPhysicalJoystickName(int num_joystick) {
    return This.joysticks[num_joystick].name;
}

int
DIJoystick_GetNumPhysicalJoystickAxes(int num_joystick) {
    return This.joysticks[num_joystick].num_axes;
}

char*
DIJoystick_GetPhysicalJoystickAxisName(int num_joystick, int num_axis) {
...
BOOL CALLBACK
DIJoystick_EnumDeviceProc(LPDIDEVICEINSTANCE pdidi, LPVOID pv) {
    char buffer[5000];
...
DIJoystick_EnumAxisObjectsProc(LPCDIDEVICEOBJECTINSTANCE lpddoi, LPVOID pvRef) {
...
static BOOL CALLBACK
DIJoystick_EnumPOVObjectsProc(LPCDIDEVICEOBJECTINSTANCE lpddoi, LPVOID pvRef) {
    joystick_type* joystick = (joystick_type*)pvRef;
    joystick->num_pov++;
...
static BOOL CALLBACK
DIJoystick_EnumButtonObjectsProc(LPCDIDEVICEOBJECTINSTANCE lpddoi, LPVOID pvRef) {
    joystick_type* joystick = (joystick_type*)pvRef;
...
static void
ClearJoyState(DIJOYSTATE* pdijs) {
...
static void
InitJoystick(joystick_type* joystick) {
    LPDIRECTINPUTDEVICE didTemp;
    HRESULT hr;
    LPDIRECTINPUT di = GetDirectInput();
...
static void
ExitJoystick(joystick_type* joystick) {
    DWORD i;
