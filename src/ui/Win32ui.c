...
void SendMessageToProcess(LPPROCESS_INFORMATION lpProcessInformation, UINT Msg, WPARAM wParam, LPARAM lParam);
...
static TBBUTTON tbb[] = {{0, ID_VIEW_FOLDERS, TBSTATE_ENABLED, TBSTYLE_CHECK, {0, 0}, 0, 0},
...
static char szTbStrings[NUM_TOOLTIPS + 1][30] = {"Toggle Folder List",
                                                 "Toggle Screen Shot",
                                                 "Large Icons",
                                                 "Small Icons",
                                                 "List",
...
static int CommandToString[] = {
    ID_VIEW_FOLDERS, ID_VIEW_PICTURE_AREA, ID_VIEW_LARGE_ICON, ID_VIEW_SMALL_ICON, ID_VIEW_LIST_MENU,
    ID_VIEW_DETAIL,  ID_VIEW_GROUPED,      ID_HELP_ABOUT,      ID_HELP_CONTENTS,   -1};

/* How to resize main window */
static ResizeItem main_resize_items[] = {{RA_HWND, {0}, RA_LEFT | RA_RIGHT | RA_TOP, NULL},
...
const char* column_names[COLUMN_MAX] = {"Game",   "ROMs",         "Samples", "Directory", "Type",   "Trackball",
                                        "Played", "Manufacturer", "Year",    "Clone Of",  "Source", "Play Time"};

/* a tiny compile is without Neogeo games */
#if (defined(NEOFREE) || defined(TINY_COMPILE)) && !defined(NEOMAME)
struct GameDriver driver_neogeo = {__FILE__, 0, "Neo-Geo Fake driver", 0, 0, 0, 0, 0, 0, 0, 0, NOT_A_DRIVER};
#else
...
static void
CreateCommandLine(int nGameIndex, char* pCmdLine) {
...
static BOOL
WaitWithMessageLoop(HANDLE hEvent) {
    DWORD dwRet;
    MSG msg;
...
RunMAME(int nGameIndex) {
    ...
    if (!CreateProcess(NULL, pCmdLine, NULL, /* Process handle not inheritable. */
...
int
Mame32Main(HINSTANCE hInstance, LPSTR lpCmdLine, int nCmdShow) {
    MSG msg;
 ...
 */
HICON
FormatICOInMemoryToHICON(PBYTE ptrBuffer, UINT nBufferSize) {
    ICONIMAGE IconImage;
    ...
    hIcon = CreateIconFromResourceEx(IconImage.lpBits, IconImage.dwNumBytes, TRUE, 0x00030000,
                                     ...
                                     (*(LPBITMAPINFOHEADER)(IconImage.lpBits)).biHeight / 2, 0);
...
HICON
LoadIconFromFile(const char* iconname) {
    HICON hIcon = 0;
    ...
    if (stat(tmpStr, &file_stat) != 0 || (hIcon = ExtractIcon(hInst, tmpStr, 0)) == 0) {
...
static void
ResizeTreeAndListViews(BOOL bResizeHidden) {
    int i;
    int nLastWidth = 0;
    int nLastWidth2 = 0;
    ...
    for (i = 0; g_splitterInfo[i].nSplitterWindow; i++) {
        bVisible =
            GetWindowLongPtr(GetDlgItem(hMain, g_splitterInfo[i].nLeftWindow), GWL_STYLE) & WS_VISIBLE ? TRUE : FALSE;
        if (bResizeHidden || bVisible) {
            ...
            MoveWindow(GetDlgItem(hMain, g_splitterInfo[i].nLeftWindow), nLastWidth, rect.top + 2, nLeftWindowWidth,
...
UpdateScreenShot(void) {
    ...
    if (GetShowScreenShot()) {
        ...
        showing_history = (have_history && GetCurrentTab() == TAB_SCREENSHOT && GetShowTab(TAB_HISTORY) == FALSE);
        ...
        ShowWindow(GetDlgItem(hMain, IDC_SSPICTURE), (GetCurrentTab() != TAB_HISTORY) ? SW_SHOW : SW_HIDE);
...
void
ResizePickerControls(HWND hWnd) {
    RECT frameRect;
    RECT rect, sRect;
    ...
    if (bShowTabCtrl) {
        MoveWindow(GetDlgItem(hWnd, IDC_SSTAB), nListWidth + 4, rect.top + 2, nScreenShotWidth - 2, rect.top + 20,
                   doSSControls);
        rect.top += 20;
    }

    /* resize the Screen shot frame */
    MoveWindow(GetDlgItem(hWnd, IDC_SSFRAME), nListWidth + 4, rect.top + 2, nScreenShotWidth - 2,
               (rect.bottom - rect.top) - 4, doSSControls);
    ...
    MoveWindow(GetDlgItem(hWnd, IDC_HISTORY), sRect.left, sRect.top, sRect.right - sRect.left, sRect.bottom,
               doSSControls);
...
MYBITMAPINFO*
GetBackgroundInfo(void) {
    return &bmDesc;
}

int
GetMinimumScreenShotWindowWidth(void) {
    BITMAP bmp;
...
int
GetDriverIndex(const struct GameDriver* driver) {
    return GetGameNameIndex(driver->name);
}

int
GetGameNameIndex(const char* name) {
    driver_data_type* driver_index_info;
    driver_data_type key;
    key.name = name;

    // uses our sorted array of driver names to get the index in log time
    driver_index_info = bsearch(&key, sorted_drivers, game_count, sizeof(driver_data_type), DriverDataCompareFunc);
...
int
GetIndexFromSortedIndex(int sorted_index) {
...
int CLIB_DECL
DriverDataCompareFunc(const void* arg1, const void* arg2) {
    return strcmp(((driver_data_type*)arg1)->name, ((driver_data_type*)arg2)->name);
}

static void
ResetTabControl(void) {
    TC_ITEM tci;
    int i;
...
static BOOL
Win32UI_init(HINSTANCE hInstance, LPSTR lpCmdLine, int nCmdShow) {
    WNDCLASS wndclass;
    RECT rect;
    int i, nSplitterCount;
...
MameWindowProc(HWND hWnd, UINT message, UINT_PTR wParam, LONG_PTR lParam) {
    MINMAXINFO* mminfo;
    int i;
    POINTS p;
    ...
    switch (message) {
        ...
        case WM_CLOSE: {
            ...
            SetWindowLongPtr(hwndList, GWL_STYLE, (GetWindowLongPtr(hwndList, GWL_STYLE) & ~LVS_TYPEMASK) | LVS_REPORT);
...
static void
OnSize(HWND hWnd, UINT nState, int nWidth, int nHeight) {
    static BOOL firstTime = TRUE;
...
static void
ResizeWindow(HWND hParent, Resize* r) {
...
ProgressBarStep() {
    char tmp[80];
    sprintf(tmp, "Game search %d%% complete", ((game_index + 1) * 100) / game_count);
...
InitProgressBar(HWND hParent) {
    ...
    return CreateWindowEx(WS_EX_STATICEDGE, PROGRESS_CLASS, "Progress Bar", WS_CHILD | PBS_SMOOTH, rect.left, rect.top,
                          rect.right - rect.left, rect.bottom - rect.top, hParent, NULL, hInst, NULL);
}

static void
CopyToolTipText(LPTOOLTIPTEXT lpttt) {
    int i;
...
static HWND
InitToolbar(HWND hParent) {
    return CreateToolbarEx(hParent, WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | CCS_TOP | TBSTYLE_FLAT | TBSTYLE_TOOLTIPS,
                           1, 8, hInst, IDB_TOOLBAR, tbb, NUM_TOOLBUTTONS, 16, 16, 0, 0, sizeof(TBBUTTON));
}

static HWND
InitStatusBar(HWND hParent) {
    ...
    return CreateStatusWindow(WS_CHILD | WS_VISIBLE | WS_CLIPSIBLINGS | CCS_BOTTOM | SBARS_SIZEGRIP, "Ready", hParent,
                              2);
}
...
static void
EnableSelection(int nGame) {
    char buf[200];
    const char* pText;
    MENUITEMINFO mmi;
    HMENU hMenu = GetMenu(hMain);

    snprintf(buf, sizeof(buf) / sizeof(buf[0]), g_szPlayGameString,
...
static void
PaintBackgroundImage(HWND hWnd, HRGN hRgn, int x, int y) {
    RECT rcClient;
    HRGN rgnBitmap;
    HPALETTE hPAL;
    HDC hDC = GetDC(hWnd);
                ...
                switch (GetRealColumnFromViewColumn(pDispInfo->item.iSubItem)) {
                    ...
                    case COLUMN_SAMPLES:
                        ...
                        if (DriverUsesSamples(nItem)) {
                            pDispInfo->item.pszText = (char*)GetAuditString(GetSampleAuditResults(nItem));
                        } else {
...
static int
GetTabFromTabIndex(int tab_index) {
    int shown_tabs = -1;
    int i;
...
static BOOL
HeaderOnContextMenu(HWND hWnd, WPARAM wParam, LPARAM lParam) {
    HMENU hMenuLoad;
    HMENU hMenu;
...
char*
ConvertAmpersandString(const char* s) {
...
PollGUIJoystick() {
    ...
    if (g_pJoyGUI->is_joy_pressed(
            JOYCODE(GetUIJoyPageUp(0), GetUIJoyPageUp(1), GetUIJoyPageUp(2), GetUIJoyPageUp(3)))) {
        SetFocus(hwndList);
    ...
    if (g_pJoyGUI->is_joy_pressed(
            JOYCODE(GetUIJoyPageDown(0), GetUIJoyPageDown(1), GetUIJoyPageDown(2), GetUIJoyPageDown(3)))) {
        SetFocus(hwndList);
    ...
    if (g_pJoyGUI->is_joy_pressed(
            JOYCODE(GetUIJoySSChange(0), GetUIJoySSChange(1), GetUIJoySSChange(2), GetUIJoySSChange(3)))) {
        SendMessage(hMain, WM_COMMAND, IDC_SSFRAME, 0);
    }

    // User pressed SCROLL HISTORY UP
    if (g_pJoyGUI->is_joy_pressed(
            JOYCODE(GetUIJoyHistoryUp(0), GetUIJoyHistoryUp(1), GetUIJoyHistoryUp(2), GetUIJoyHistoryUp(3)))) {
    ...
    if (g_pJoyGUI->is_joy_pressed(
            JOYCODE(GetUIJoyHistoryDown(0), GetUIJoyHistoryDown(1), GetUIJoyHistoryDown(2), GetUIJoyHistoryDown(3)))) {
        HWND hHistory = GetDlgItem(hMain, IDC_HISTORY);
...
SetView(int menu_id, int listview_style) {
    ...
    SetWindowLongPtr(hwndList, GWL_STYLE, (GetWindowLongPtr(hwndList, GWL_STYLE) & ~LVS_TYPEMASK) | listview_style);
...
static BOOL
MameCommand(HWND hwnd, int id, HWND hwndCtl, UINT codeNotify) {
    int i;
    switch (id) {
        case ID_FILE_PLAY:
            MamePlayGame();
		...
		*/
        ...
        case ID_CONTEXT_ADD_CUSTOM: {
            /*INT_PTR  nResult =*/DialogBoxParam(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_CUSTOM_FILE), hMain,
                                                 AddCustomFileDialogProc, GetSelectedPickItem());
            SetFocus(hwndList);
            break;
        }
        ...
        case ID_OPTIONS_LANGUAGE:
            DialogBox(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_LANGUAGE), hMain, LanguageDialogProc);
            return TRUE;
...
static void
ResetColumnDisplay(BOOL first_time) {
    LV_COLUMN lvc;
    int i;
    int nColumn;
    int widths[COLUMN_MAX];
    ...
    ListView_SetExtendedListViewStyle(hwndList, LVS_EX_FULLROWSELECT | LVS_EX_HEADERDRAGDROP | LVS_EX_UNDERLINEHOT
                                                    | LVS_EX_UNDERLINECOLD /* | LVS_EX_LABELTIP*/);
...
static int
GetRealColumnFromViewColumn(int view_column) {
    return realColumn[view_column];
}

static int
GetViewColumnFromRealColumn(int real_column) {
    int i;
...
static void
AddDriverIcon(int nItem, int default_icon_index) {
    HICON hIcon = 0;
...
CreateIcons(void) {
    ...
    hSmall =
        ImageList_Create(GetShellSmallIconSize(), GetShellSmallIconSize(), ILC_COLORDDB | ILC_MASK, icon_count, 500);
    hLarge = ImageList_Create(dwLargeIconSize, dwLargeIconSize, ILC_COLORDDB | ILC_MASK, icon_count, 500);
...
static int CALLBACK
ListCompareFunc(LPARAM index1, LPARAM index2, int sort_subitem) {
    ...
    if (DriverIsClone((int)index1) && DriverIsClone((int)index2)) {
        ...
        return BasicCompareFunc(GetDriverIndex(drivers[index1]->clone_of), GetDriverIndex(drivers[index2]->clone_of),
                                sort_subitem);
    }
...
static int
BasicCompareFunc(LPARAM index1, LPARAM index2, int sort_subitem) {
    int value;
    ...
    switch (sort_subitem) {
        case COLUMN_GAMES:
            value = _stricmp(ModifyThe(drivers[index1]->description), ModifyThe(drivers[index2]->description));
            break;
        ...
        case COLUMN_TYPE: {
            ...
            if ((drv1.video_attributes & VIDEO_TYPE_VECTOR) == (drv2.video_attributes & VIDEO_TYPE_VECTOR))
                return BasicCompareFunc(index1, index2, COLUMN_GAMES);

            value = (drv1.video_attributes & VIDEO_TYPE_VECTOR) - (drv2.video_attributes & VIDEO_TYPE_VECTOR);
            break;
        }
        case COLUMN_TRACKBALL:
...
static void
SetSelectedPick(int new_index) {
    if (new_index < 0)
        new_index = 0;

    ListView_SetItemState(hwndList, new_index, LVIS_FOCUSED | LVIS_SELECTED, LVIS_FOCUSED | LVIS_SELECTED);
    ListView_EnsureVisible(hwndList, new_index, FALSE);
}

static void
SetSelectedPickItem(int val) {
    int i;
    LV_FINDINFO lvfi;
...
static BOOL
CommonFileDialog(common_file_dialog_proc cfd, char* filename, BOOL bZip) {
    BOOL success;
...
static BOOL
SelectLanguageFile(HWND hWnd, TCHAR* filename) {
    OPENFILENAME of;
...
LanguageDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
    TCHAR pLangFile[MAX_PATH];
    DWORD dwHelpIDs[] = {IDC_LANGUAGECHECK, HIDC_LANGUAGECHECK, IDC_LANGUAGEEDIT, HIDC_LANGUAGEEDIT, 0, 0};
    ...
    switch (Msg) {
        ...
        case WM_HELP:
            HelpFunction(((LPHELPINFO)lParam)->hItemHandle, MAME32CONTEXTHELP, HH_TP_HELP_WM_HELP,
                         (DWORD_PTR)dwHelpIDs);
            break;
...
static void
MamePlayGameWithOptions(int nGame) {
...
static void
EnablePlayOptions(int nIndex, options_type* o) {
    if (!DIJoystick.Available())
        o->use_joystick = FALSE;
}

static int
FindIconIndex(int nIconResource) {
    int i;
    for (i = 0; g_iconData[i].icon_name; i++) {
...
static BOOL
HandleTreeContextMenu(HWND hWnd, WPARAM wParam, LPARAM lParam) {
    HMENU hTreeMenu;
    HMENU hMenu;
    TVHITTESTINFO hti;
    POINT pt;
...
static BOOL
HandleContextMenu(HWND hWnd, WPARAM wParam, LPARAM lParam) {
    HMENU hMenuLoad;
    HMENU hMenu;
    POINT pt;
...
static BOOL
HandleScreenShotContextMenu(HWND hWnd, WPARAM wParam, LPARAM lParam) {
    HMENU hMenuLoad;
    HMENU hMenu;
    POINT pt;
...
UpdateMenu(HMENU hMenu) {
    ...
    if (have_selection) {
        snprintf(buf, sizeof(buf), g_szPlayGameString, ConvertAmpersandString(ModifyThe(drivers[nGame]->description)));
    ...
    CheckMenuRadioItem(hMenu, ID_VIEW_TAB_SCREENSHOT, ID_VIEW_TAB_HISTORY, ID_VIEW_TAB_SCREENSHOT + GetCurrentTab(),
                       MF_BYCOMMAND);
...
void
InitTreeContextMenu(HMENU hTreeMenu) {
    MENUITEMINFO mii;
    HMENU hMenu;
    int i;
    extern FOLDERDATA g_folderData[];
...
static LPCTSTR
MakeShortString(HDC hDC, LPCTSTR lpszLong, int nColumnLen, int nOffset) {
...
HistoryWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    if (hBackground) {
        switch (uMsg) {
            case WM_MOUSEMOVE: {
                if (MouseHasBeenMoved())
...
static LRESULT CALLBACK
PictureFrameWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_MOUSEMOVE: {
            if (MouseHasBeenMoved())
                ShowCursor(TRUE);
        ...
        case WM_NCHITTEST: {
            ...
            if (have_history
                && ((GetCurrentTab() == TAB_HISTORY)
                    || ((GetCurrentTab() == TAB_SCREENSHOT && GetShowTab(TAB_HISTORY) == FALSE) && (rect.top - 6) < pt.y
                        && pt.y < (rect.bottom + 6)))) {
...
static LRESULT CALLBACK
PictureWndProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    switch (uMsg) {
        case WM_ERASEBKGND:
            return TRUE;
        case WM_PAINT: {
            ...
            StretchBlt(hdc, 0, 0, rect.right - rect.left, rect.bottom - rect.top, hdc_temp, 0, 0, width, height,
                       SRCCOPY);
...
DrawItem(LPDRAWITEMSTRUCT lpDrawItemStruct) {
    ...
    bSelected =
        ((lvi.state & LVIS_DROPHILITED)
         || ((lvi.state & LVIS_SELECTED) && ((bFocus) || (GetWindowLongPtr(hwndList, GWL_STYLE) & LVS_SHOWSELALWAYS))));
    ...
    if (hImageList) {
        ...
        if (rcIcon.left + 16 + indent_space < rcItem.right) {
            ImageList_DrawEx(hImageList, lvi.iImage, hDC, rcIcon.left, rcIcon.top, 16, 16, GetSysColor(COLOR_WINDOW),
...
static BOOL
ListCtrlOnErase(HWND hWnd, HDC hDC) {
    RECT rcClient;
    HRGN rgnBitmap;
    HPALETTE hPAL;
...
static BOOL
ListCtrlOnPaint(HWND hWnd, UINT uMsg) {
    PAINTSTRUCT ps;
    HDC hDC;
    ...
    BitBlt(memDC, rcClip.left, rcClip.top, rcClip.right - rcClip.left, rcClip.bottom - rcClip.top, hDC, rcClip.left,
           rcClip.top, SRCCOPY);
    ...
    /* Draw bitmap in the background */
    {
        ...
        BitBlt(maskDC, 0, 0, rcClient.right - rcClient.left, rcClient.bottom - rcClient.top, memDC, rcClient.left,
               rcClient.top, SRCCOPY);
		...
		*/
        ...
        BitBlt(memDC, rcClip.left, rcClip.top, rcClip.right - rcClip.left, rcClip.bottom - rcClip.top, maskDC,
               rcClip.left, rcClip.top, SRCAND);

        /* Set the foreground to black. See comment above. */
        SetBkColor(imageDC, RGB(255, 255, 255));
        SetTextColor(imageDC, RGB(0, 0, 0));
        BitBlt(imageDC, rcClip.left, rcClip.top, rcClip.right - rcClip.left, rcClip.bottom - rcClip.top, maskDC,
               rcClip.left, rcClip.top, SRCAND);

        /* Combine the foreground with the background */
        BitBlt(imageDC, rcClip.left, rcClip.top, rcClip.right - rcClip.left, rcClip.bottom - rcClip.top, memDC,
               rcClip.left, rcClip.top, SRCPAINT);

        /* Draw the final image to the screen */
        BitBlt(hDC, rcClip.left, rcClip.top, rcClip.right - rcClip.left, rcClip.bottom - rcClip.top, imageDC,
...
int
osd_display_loading_rom_message(const char* name, struct rom_load_data* romdata) {
    int retval;
...
static INT_PTR CALLBACK
LoadProgressDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
    switch (Msg) {
        case WM_INITDIALOG: {
            char buf[256];
...
int
UpdateLoadProgress(const char* name, const struct rom_load_data* romdata) {
    ...
    if (hWndLoad == NULL) {
        hWndLoad =
            CreateDialog(GetModuleHandle(NULL), MAKEINTRESOURCE(IDD_LOAD_PROGRESS), hMain, LoadProgressDialogProc);
    ...
    SetWindowText(GetDlgItem(hWndLoad, IDC_LOAD_STATUS), ConvertToWindowsNewlines(romdata->errorbuf));
        ...
        if (romdata->errors > 0 || romdata->warnings > 0) {
			...
			*/
            ...
            if (romdata->errors)
                ...
                SetWindowText(GetDlgItem(hWndLoad, IDC_ERROR_TEXT), "WARNING: the game might not run correctly.");
        }
    } else
...
void
RemoveCurrentGameCustomFolder(void) {
    RemoveGameCustomFolder(GetSelectedPickItem());
}

void
RemoveGameCustomFolder(int driver_index) {
    int i;
    TREEFOLDER** folders;
    int num_folders;
...
void
BeginListViewDrag(NM_LISTVIEW* pnmv) {
    LV_ITEM lvi;
    POINT pt;
...
void
MouseMoveListViewDrag(POINTS p) {
    HTREEITEM htiTarget;
    TV_HITTESTINFO tvht;
...
void
CalculateBestScreenShotRect(HWND hWnd, RECT* pRect, BOOL restrict_height) {
    int destX, destY;
    int destW, destH;
    RECT rect;
    /* for scaling */
...
void
SendIconToProcess(LPPROCESS_INFORMATION pi, int nGameIndex) {
    HICON hIcon;
...
EnumWindowCallBack(HWND hwnd, LPARAM lParam) {
    ...
    if (ProcessId == pfwhs->ProcessInfo->dwProcessId && strncmp(buffer, MAMENAME, strlen(MAMENAME)) == 0) {
