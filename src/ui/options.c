...
static REG_OPTION regSettings[] = {
    ...
    {"current_tab", RO_ENCODE, &settings.current_tab, CurrentTabEncodeString, CurrentTabDecodeString},
    ...
    {"hide_folders", RO_ENCODE, &settings.show_folder_flags, FolderFlagsEncodeString, FolderFlagsDecodeString},

    {"show_tabs", RO_BOOL, &settings.show_tabctrl, 0, 0},
    {"hide_tabs", RO_ENCODE, &settings.show_tab_flags, TabFlagsEncodeString, TabFlagsDecodeString},
...
static REG_OPTION regGameOpts[] = {
    ...
    {"cleanstretch", RO_ENCODE, &gOpts.clean_stretch, CleanStretchEncodeString, CleanStretchDecodeString},
    ...
    {"d3deffect", RO_ENCODE, &gOpts.d3d_effect, D3DEffectEncodeString, D3DEffectDecodeString},
    {"d3dprescale", RO_ENCODE, &gOpts.d3d_prescale, D3DPrescaleEncodeString, D3DPrescaleDecodeString},
...
static GAMEVARIABLE_OPTION gamevariable_options[] = {
    ...
    {"samples_audit", RO_INT, offsetof(game_variables_type, samples_audit_results), DriverUsesSamples,
     (const void*)UNKNOWN},
#ifdef MESS
...
const char* image_tabs_long_name[MAX_TAB_TYPES] = {
    "Snapshot ", "Flyer ", "Cabinet ", "Marquee", "Title", "Control Panel", "History ",
};

const char* image_tabs_short_name[MAX_TAB_TYPES] = {
    "snapshot", "flyer", "cabinet", "marquee", "title", "cpanel", "history",
};
...
const char* d3d_effects_short_name[MAX_D3D_EFFECTS] = {
    "none",       "auto",         "aperturegrille", "dotmedmask", "dotmedbright", "rgbminmask",
    "rgbmedmask", "rgbmaxbright", "rgbmicro",       "rgbtiny",    "rgb3",         "rgb4",
    "rgb6",       "rgb16",        "scan25",         "scan50",     "scan75",
};
...
const char* d3d_filter_long_name[MAX_D3D_FILTERS] = {
    "None", "Bilinear", "Cubic (flat kernel)", "Cubic (gaussian kernel)", "Anisotropic",
};

const char* clean_stretch_long_name[MAX_CLEAN_STRETCH] = {
    "None", "Auto", "Full", "Horizontal", "Vertical",
};
...
OptionsInit() {
    ...
    settings.show_tab_flags = (1 << TAB_SCREENSHOT) | (1 << TAB_FLYER) | (1 << TAB_CABINET) | (1 << TAB_MARQUEE)
                              | (1 << TAB_TITLE) | (1 << TAB_CONTROL_PANEL);
...
void
FreeGameOptions(options_type* o) {
    int i;

    for (i = 0; i < NUM_GAME_OPTIONS; i++) {
        if (regGameOpts[i].m_iType == RO_STRING) {
            char** string_to_free = (char**)((char*)o + ((char*)regGameOpts[i].m_vpData - (char*)&gOpts));
...
void
CopyGameOptions(options_type* source, options_type* dest) {
    int i;
    ...
    *dest = *source;
    ...
    for (i = 0; i < NUM_GAME_OPTIONS; i++) {
        if (regGameOpts[i].m_iType == RO_STRING) {
            char** string_to_copy = (char**)((char*)dest + ((char*)regGameOpts[i].m_vpData - (char*)&gOpts));
...
options_type*
GetGameOptions(int driver_index) {
...
BOOL
GetGameUsesDefaults(int driver_index) {
    if (driver_index < 0) {
...
void
SetGameUsesDefaults(int driver_index, BOOL use_defaults) {
    if (driver_index < 0) {
...
void
LoadFolderFilter(int folder_index, int filters) {
    //dprintf("loaded folder filter %i %i",folder_index,filters);

    if (num_folder_filters == size_folder_filters) {
        size_folder_filters *= 2;
        folder_filters = (folder_filter_type*)realloc(folder_filters, size_folder_filters * sizeof(folder_filter_type));
    }
...
const char*
GetImageTabLongName(int tab_index) {
    return image_tabs_long_name[tab_index];
}

const char*
GetImageTabShortName(int tab_index) {
    return image_tabs_short_name[tab_index];
}

const char*
GetD3DEffectLongName(int d3d_effect) {
    return d3d_effects_long_name[d3d_effect];
}

const char*
GetD3DEffectShortName(int d3d_effect) {
    return d3d_effects_short_name[d3d_effect];
}

const char*
GetD3DPrescaleLongName(int d3d_prescale) {
    return d3d_prescale_long_name[d3d_prescale];
}

const char*
GetD3DPrescaleShortName(int d3d_prescale) {
    return d3d_prescale_short_name[d3d_prescale];
}

const char*
GetD3DFilterLongName(int d3d_filter) {
    return d3d_filter_long_name[d3d_filter];
}

const char*
GetCleanStretchLongName(int clean_stretch) {
    return clean_stretch_long_name[clean_stretch];
}

const char*
GetCleanStretchShortName(int clean_stretch) {
...
void
SetCycleScreenshot(int cycle_screenshot) {
    settings.cycle_screenshot = cycle_screenshot;
}

int
GetCycleScreenshot(void) {
    return settings.cycle_screenshot;
}

void
SetStretchScreenShotLarger(BOOL stretch) {
    settings.stretch_screenshot_larger = stretch;
}

BOOL
GetStretchScreenShotLarger(void) {
...
void
SetShowStatusBar(BOOL val) {
    settings.show_statusbar = val;
}

BOOL
GetShowStatusBar(void) {
    return settings.show_statusbar;
}

void
SetShowTabCtrl(BOOL val) {
...
void
SetDefaultGame(const char* name) {
    FreeIfAllocated(&settings.default_game);
...
void
SetListCloneColor(COLORREF uColor) {
...
BOOL
AllowedToSetShowTab(int tab, BOOL show) {
...
void
SetSplitterPos(int splitterId, int pos) {
    if (splitterId < GetSplitterCount())
        settings.splitter[splitterId] = pos;
}

int
GetSplitterPos(int splitterId) {
    if (splitterId < GetSplitterCount())
...
void
SetSampleDirs(const char* paths) {
    FreeIfAllocated(&settings.sampledirs);
...
const char*
GetControlPanelDir(void) {
    return settings.cpaneldir;
}

void
SetControlPanelDir(const char* path) {
    FreeIfAllocated(&settings.cpaneldir);
    if (path != NULL)
...
const char*
GetCheatFileName(void) {
    return settings.cheat_filename;
}

void
SetCheatFileName(const char* path) {
    FreeIfAllocated(&settings.cheat_filename);
...
const char*
GetHistoryFileName(void) {
    return settings.history_filename;
}

void
SetHistoryFileName(const char* path) {
    FreeIfAllocated(&settings.history_filename);
...
const char*
GetMAMEInfoFileName(void) {
    return settings.mameinfo_filename;
}

void
SetMAMEInfoFileName(const char* path) {
    FreeIfAllocated(&settings.mameinfo_filename);
...
void
ResetGameOptions(int driver_index) {
    assert(0 <= driver_index && driver_index < num_games);
...
void
ResetGameDefaults(void) {
    save_default_options = FALSE;
}

void
ResetAllGameOptions(void) {
    int i;
...
int
GetRomAuditResults(int driver_index) {
    assert(0 <= driver_index && driver_index < num_games);
...
void
SetRomAuditResults(int driver_index, int audit_results) {
    assert(0 <= driver_index && driver_index < num_games);
...
int
GetSampleAuditResults(int driver_index) {
    assert(0 <= driver_index && driver_index < num_games);
...
void
SetSampleAuditResults(int driver_index, int audit_results) {
    assert(0 <= driver_index && driver_index < num_games);
...
void
IncrementPlayCount(int driver_index) {
    assert(0 <= driver_index && driver_index < num_games);
...
void
IncrementPlayTime(int driver_index, int playtime) {
    assert(0 <= driver_index && driver_index < num_games);
    game_variables[driver_index].play_time += playtime;
}

void
GetTextPlayTime(int driver_index, char* buf) {
...
void
SetUIJoyUp(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyDown(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyLeft(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
int
GetUIJoyRight(int joycodeIndex) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyRight(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
int
GetUIJoyStart(int joycodeIndex) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyStart(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
int
GetUIJoyPageUp(int joycodeIndex) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyPageUp(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
int
GetUIJoyPageDown(int joycodeIndex) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyPageDown(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyHome(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyEnd(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
int
GetUIJoySSChange(int joycodeIndex) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoySSChange(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
int
GetUIJoyHistoryUp(int joycodeIndex) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyHistoryUp(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
int
GetUIJoyHistoryDown(int joycodeIndex) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyHistoryDown(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetUIJoyExec(int joycodeIndex, int val) {
    assert(0 <= joycodeIndex && joycodeIndex < 4);
...
void
SetExecWait(int wait) {
    settings.exec_wait = wait;
}

BOOL
GetHideMouseOnStartup(void) {
    return settings.hide_mouse;
}

void
SetHideMouseOnStartup(BOOL hide) {
    settings.hide_mouse = hide;
}

BOOL
GetRunFullScreen(void) {
    return settings.full_screen;
}

void
SetRunFullScreen(BOOL fullScreen) {
...
static void
ColumnEncodeStringWithCount(void* data, char* str, int count) {
    int* value = (int*)data;
    int i;
    char buffer[100];
...
static void
ColumnDecodeStringWithCount(const char* str, void* data, int count) {
    int* value = (int*)data;
    int i;
    char *s, *p;
    char tmpStr[100];
...
static void
ColumnEncodeString(void* data, char* str) {
    ColumnEncodeStringWithCount(data, str, COLUMN_MAX);
}

static void
ColumnDecodeString(const char* str, void* data) {
    ColumnDecodeStringWithCount(str, data, COLUMN_MAX);
}

static void
JoyInfoEncodeString(void* data, char* str) {
    ColumnEncodeStringWithCount(data, str, 4);
}

static void
JoyInfoDecodeString(const char* str, void* data) {
    ColumnDecodeStringWithCount(str, data, 4);
}

static void
ColumnDecodeWidths(const char* str, void* data) {
    ColumnDecodeString(str, data);
}

static void
SplitterEncodeString(void* data, char* str) {
    int* value = (int*)data;
    int i;
    char tmpStr[100];
...
static void
SplitterDecodeString(const char* str, void* data) {
    int* value = (int*)data;
    int i;
    char *s, *p;
    char tmpStr[100];
...
static void
ListDecodeString(const char* str, void* data) {
    int* value = (int*)data;
    int i;
...
static void
ListEncodeString(void* data, char* str) {
    int* value = (int*)data;
...
static void
FontDecodeString(const char* str, void* data) {
    LOGFONT* f = (LOGFONT*)data;
    char* ptr;

    sscanf(str, "%li,%li,%li,%li,%li,%i,%i,%i,%i,%i,%i,%i,%i", &f->lfHeight, &f->lfWidth, &f->lfEscapement,
           &f->lfOrientation, &f->lfWeight, (int*)&f->lfItalic, (int*)&f->lfUnderline, (int*)&f->lfStrikeOut,
           (int*)&f->lfCharSet, (int*)&f->lfOutPrecision, (int*)&f->lfClipPrecision, (int*)&f->lfQuality,
...
static void
FontEncodeString(void* data, char* str) {
    LOGFONT* f = (LOGFONT*)data;

    sprintf(str, "%li,%li,%li,%li,%li,%i,%i,%i,%i,%i,%i,%i,%i,%s", f->lfHeight, f->lfWidth, f->lfEscapement,
            f->lfOrientation, f->lfWeight, f->lfItalic, f->lfUnderline, f->lfStrikeOut, f->lfCharSet, f->lfOutPrecision,
            f->lfClipPrecision, f->lfQuality, f->lfPitchAndFamily, f->lfFaceName);
}

static void
D3DEffectEncodeString(void* data, char* str) {
...
static void
D3DEffectDecodeString(const char* str, void* data) {
    int i;
...
static void
D3DPrescaleEncodeString(void* data, char* str) {
...
static void
D3DPrescaleDecodeString(const char* str, void* data) {
    int i;
...
static void
CleanStretchEncodeString(void* data, char* str) {
...
static void
CleanStretchDecodeString(const char* str, void* data) {
    int i;
...
static void
CurrentTabEncodeString(void* data, char* str) {
...
static void
CurrentTabDecodeString(const char* str, void* data) {
    int i;
...
static void
FolderFlagsEncodeString(void* data, char* str) {
    int i;
    int num_saved = 0;
...
static void
FolderFlagsDecodeString(const char* str, void* data) {
    char s[2000];
    extern FOLDERDATA g_folderData[];
    char* token;
...
static void
TabFlagsEncodeString(void* data, char* str) {
    int i;
    int num_saved = 0;
...
static void
TabFlagsDecodeString(const char* str, void* data) {
    char s[2000];
    char* token;
...
static REG_OPTION*
GetOption(REG_OPTION* option_array, int num_options, const char* key) {
    int i;
...
static void
LoadOption(REG_OPTION* option, const char* value_str) {
...
static BOOL
LoadGameVariableOrFolderFilter(char* key, const char* value) {
...
static void
ParseKeyValueStrings(char* buffer, char** key, char** value) {
    char* ptr;
    BOOL quoted;
...
void
LoadGameOptions(int driver_index) {
    char buffer[512];
...
static BOOL
LoadOptions(const char* filename, options_type* o, BOOL load_global_game_options) {
    FILE* fptr;
    char buffer[512];
...
DWORD
GetFolderFlags(int folder_index) {
    int i;
                    ...
                    switch (gamevariable_options[j].m_iType) {
                        ...
                        case RO_BOOL:
                            ...
                            if (nValue != (int)gamevariable_options[j].m_vpDefault) {
                                fprintf(fptr, "%s_%s %i\n", drivers[driver_index]->name, gamevariable_options[j].name,
                                        nValue);
                            }
                            break;
                        ...
                        case RO_STRING:
                            ...
                            if (strcmp(pValue, (const char*)gamevariable_options[j].m_vpDefault)) {
                                fprintf(fptr, "%s_%s \"%s\"\n", drivers[driver_index]->name,
                                        gamevariable_options[j].name, pValue);
                            }
                            break;
...
void
SaveGameOptions(int driver_index) {
    int i;
    FILE* fptr;
    char buffer[512];
...
static void
WriteStringOptionToFile(FILE* fptr, const char* key, const char* value) {
...
static void
WriteIntOptionToFile(FILE* fptr, const char* key, int value) {
    fprintf(fptr, "%s %i\n", key, value);
}

static void
WriteBoolOptionToFile(FILE* fptr, const char* key, BOOL value) {
    fprintf(fptr, "%s %i\n", key, value ? 1 : 0);
}

static void
WriteColorOptionToFile(FILE* fptr, const char* key, COLORREF value) {
    fprintf(fptr, "%s %i,%i,%i\n", key, (int)(value & 0xff), (int)((value >> 8) & 0xff), (int)((value >> 16) & 0xff));
}

static BOOL
IsOptionEqual(int option_index, options_type* o1, options_type* o2) {
...
static void
WriteOptionToFile(FILE* fptr, REG_OPTION* regOpt) {
    BOOL* pBool;
    int* pInt;
    char* pString;
    double* pDouble;
