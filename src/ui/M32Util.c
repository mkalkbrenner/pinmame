...
void
DisplayTextFile(HWND hWnd, const char* cName) {
    HINSTANCE hErr;
    const char* msg = 0;
...
char*
MyStrStrI(const char* pFirst, const char* pSrch) {
    char* cp = (char*)pFirst;
    char* s1;
    char* s2;
...
char*
ConvertToWindowsNewlines(const char* source) {
...
BOOL
DriverHasOptionalBIOS(int driver_index) {
    const struct GameDriver* gamedrv = drivers[driver_index];
...
BOOL
DriverIsStereo(int driver_index) {
...
BOOL
DriverIsVector(int driver_index) {
    struct InternalMachineDriver drv;
    expand_machine_driver(drivers[driver_index]->drv, &drv);
...
BOOL
DriverUsesRoms(int driver_index) {
    const struct GameDriver* gamedrv = drivers[driver_index];
...
BOOL
DriverUsesSamples(int driver_index) {
...
BOOL
DriverUsesTrackball(int driver_index) {
...
BOOL
DriverUsesLightGun(int driver_index) {
    const struct InputPortTiny* input_ports;
...
void
FlushFileCaches(void) {
    unzip_cache_clear();
}

void
FreeIfAllocated(char** s) {
    if (*s)
        free(*s);
    *s = NULL;
}
...
BOOL
StringIsSuffixedBy(const char* s, const char* suffix) {
