...
INT_PTR CALLBACK
DirectoriesDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
    BOOL bReturn = FALSE;
...
static BOOL
IsMultiDir(int nType) {
    return g_directoryInfo[nType].bMulti;
}

static void
DirInfo_SetDir(tDirInfo* pInfo, int nType, int nItem, const char* pText) {
    char* s;
    char* pOldText;
...
static char*
DirInfo_Dir(tDirInfo* pInfo, int nType) {
    assert(!IsMultiDir(nType));
    return pInfo[nType].m_Directory;
}

static char*
DirInfo_Path(tDirInfo* pInfo, int nType, int nItem) {
    return pInfo[nType].m_Path->m_Directories[nItem];
}

static void
DirInfo_SetModified(tDirInfo* pInfo, int nType, BOOL bModified) {
    assert(IsMultiDir(nType));
    pInfo[nType].m_Path->m_bModified = bModified;
}

static BOOL
DirInfo_Modified(tDirInfo* pInfo, int nType) {
    assert(IsMultiDir(nType));
...
static void
Directories_OnSelChange(HWND hDlg) {
...
static BOOL
Directories_OnInitDialog(HWND hDlg, HWND hwndFocus, LPARAM lParam) {
    RECT rectClient;
    LVCOLUMN LVCol;
    int i;
    int nDirInfoCount;
...
static void
Directories_OnDestroy(HWND hDlg) {
    int nDirInfoCount, i;
...
static void
Directories_OnClose(HWND hDlg) {
    EndDialog(hDlg, IDCANCEL);
}

static int
RetrieveDirList(int nDir, int nFlagResult, void (*SetTheseDirs)(const char* s)) {
    int i;
    int nResult = 0;
    int nPaths;
    char buf[MAX_PATH * MAX_DIRS];
...
static void
Directories_OnCancel(HWND hDlg) {
    EndDialog(hDlg, IDCANCEL);
}

static void
Directories_OnInsert(HWND hDlg) {
    int nItem;
    char buf[MAX_PATH];
    HWND hList;
    ...
    if (BrowseForDirectory(hDlg, NULL, buf) == TRUE) {
        ...
        if (IsMultiDir(nType)) {
            ...
            for (i = DirInfo_NumDir(g_pDirInfo, nType); nItem < i; i--)
                strcpy(DirInfo_Path(g_pDirInfo, nType, i), DirInfo_Path(g_pDirInfo, nType, i - 1));
...
static void
Directories_OnBrowse(HWND hDlg) {
    int nType;
    int nItem;
    char inbuf[MAX_PATH];
...
static void
Directories_OnDelete(HWND hDlg) {
    int nType;
    int nCount;
    int nSelect;
    int i;
...
static BOOL
Directories_OnBeginLabelEdit(HWND hDlg, NMHDR* pNMHDR) {
    int nType;
    BOOL bResult = FALSE;
...
static BOOL
Directories_OnEndLabelEdit(HWND hDlg, NMHDR* pNMHDR) {
    BOOL bResult = FALSE;
    NMLVDISPINFO* pDispInfo = (NMLVDISPINFO*)pNMHDR;
    LVITEM* pItem = &pDispInfo->item;
    ...
    if (pItem->pszText != NULL) {
        ...
        if (stat(pItem->pszText, &file_stat) == 0 && (file_stat.st_mode & S_IFDIR)) {
            bResult = TRUE;
        } else {
    ...
    if (bResult == TRUE) {
        ...
        UpdateDirectoryList(hDlg);
        ListView_SetItemState(GetDlgItem(hDlg, IDC_DIR_LIST), pItem->iItem, LVIS_FOCUSED | LVIS_SELECTED,
...
static void
Directories_OnCommand(HWND hDlg, int id, HWND hwndCtl, UINT codeNotify) {
    switch (id) {
        case IDOK:
...
static BOOL
Directories_OnNotify(HWND hDlg, int id, NMHDR* pNMHDR) {
    switch (id) {
        case IDC_DIR_LIST:
            switch (pNMHDR->code) {
...
BOOL
BrowseForDirectory(HWND hwnd, const char* pStartDir, char* pResult) {
    BOOL bResult = FALSE;
    IMalloc* piMalloc = 0;
    BROWSEINFO Info;
