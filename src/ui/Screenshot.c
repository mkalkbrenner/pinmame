 ...
 */
static BOOL
DrawDIB(HWND hWnd, HDC hDC, HGLOBAL hDIB, HPALETTE hPal) {
...
#ifdef MESS
static BOOL
LoadSoftwareScreenShot(const struct GameDriver* drv, LPCSTR lpSoftwareName, int nType) {
...
#ifdef MESS
BOOL
LoadScreenShotEx(int nGame, LPCSTR lpSoftwareName, int nType)
#else  /* !MESS */
BOOL
LoadScreenShot(int nGame, int nType)
#endif /* MESS */
{
    BOOL loaded = FALSE;
    ...
    if (!loaded && (drivers[nGame]->clone_of != NULL) && !(drivers[nGame]->clone_of->flags & NOT_A_DRIVER)) {
...
int
GetScreenShotWidth(void) {
    return ((LPBITMAPINFO)m_hDIB)->bmiHeader.biWidth;
}

int
GetScreenShotHeight(void) {
...
BOOL
LoadDIB(LPCTSTR filename, HGLOBAL* phDIB, HPALETTE* pPal, int pic_type) {
    mame_file* mfile;
    BOOL success;
...
HBITMAP
DIBToDDB(HDC hDC, HANDLE hDIB, LPMYBITMAPINFO desc) {
...
static void
store_pixels(UINT8* buf, int len) {
...
BOOL
AllocatePNG(struct png_info* p, HGLOBAL* phDIB, HPALETTE* pPal) {
    int dibSize;
    HGLOBAL hDIB;
    BITMAPINFOHEADER bi;
    LPBITMAPINFOHEADER lpbi;
...
static int
png_read_bitmap(LPVOID mfile, HGLOBAL* phDIB, HPALETTE* pPAL) {
    struct png_info p;
    UINT32 i;
    int bytespp;
