...
static TREEICON treeIconNames[] = {{IDI_FOLDER_OPEN, "foldopen"},
                                   {IDI_FOLDER, "folder"},
...
void
InitTree(LPFOLDERDATA lpFolderData, LPFILTER_ITEM lpFilterList) {
...
void
SetCurrentFolder(LPTREEFOLDER lpFolder) {
...
BOOL
GameFiltered(int nGame, DWORD dwMask) {
    int i;
    ...
    // Filter out clones?
    if (dwMask & F_CLONES && !(drivers[nGame]->clone_of->flags & NOT_A_DRIVER))
        return TRUE;
...
void
CreateSourceFolders(int parent_index) {
    int i, jj;
    int nGames = GetNumGames();
...
CreateManufacturerFolders(int parent_index) {
    ...
    LPTREEFOLDER lpTemp;
    lpTemp = NewFolder("Romstar", next_folder_id++, parent_index, IDI_MANUFACTURER, GetFolderFlags(numFolders));
    AddFolder(lpTemp);
...
CreateCPUFolders(int parent_index) {
    ...
    for (i = 1; i < CPU_COUNT; i++) {
        ...
        lpTemp = NewFolder(cputype_name(i), next_folder_id++, parent_index, IDI_CPU, GetFolderFlags(numFolders));
        AddFolder(lpTemp);
...
CreateSoundFolders(int parent_index) {
    ...
    for (i = 1; i < SOUND_COUNT; i++) {
        ...
        lpTemp = NewFolder(sndintf[i].name, next_folder_id++, parent_index, IDI_CPU, GetFolderFlags(numFolders));
        AddFolder(lpTemp);
        map[i] = treeFolders[nFolder++];
    }
...
void
CreateOrientationFolders(int parent_index) {
    int jj;
    int nGames = GetNumGames();
    LPTREEFOLDER lpFolder = treeFolders[parent_index];
    ...
    lpVert = NewFolder("Vertical", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
    AddFolder(lpVert);
    lpHorz = NewFolder("Horizontal", next_folder_id++, parent_index, IDI_FOLDER, GetFolderFlags(numFolders));
    AddFolder(lpHorz);
...
void
SelectTreeViewFolder(int folder_id) {
    HWND hTreeView = GetTreeView();
    HTREEITEM hti;
    TVITEM tvi;
...
static BOOL
AddFolder(LPTREEFOLDER lpFolder) {
...
static BOOL
RemoveFolder(LPTREEFOLDER lpFolder) {
    int found = -1;
    UINT i = 0;
...
static LPTREEFOLDER
NewFolder(const char* lpTitle, UINT nFolderId, int nParent, UINT nIconId, DWORD dwFlags) {
...
static void
DeleteFolder(LPTREEFOLDER lpFolder) {
    if (lpFolder) {
...
/* Make a reasonable name out of the one found in the driver array */
static const char*
LicenseManufacturer(const char* s) {
    static char tmp[40];
    char *ptr, *t;
...
InitFolders(void) {
    ...
    for (i = 0; i < numExtraFolders; i++) {
        ...
        AddFolder(
            NewFolder(fExData->m_szTitle, fExData->m_nFolderId, fExData->m_nParent, fExData->m_nIconId, dwFolderFlags));
    }
...
CreateTreeIcons() {
    ...
    if (ImageList_GetImageCount(hTreeSmall) < ICON_MAX) {
        ErrorMsg("Error with icon list--too few images.  %i < %i", ImageList_GetImageCount(hTreeSmall), ICON_MAX);
        return FALSE;
    }
...
static void
TreeCtrlOnPaint(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    PAINTSTRUCT ps;
    HDC hDC;
...
static void
SetExtraIcons(char* name, int* id) {
    char* p = strchr(name, '.');
...
BOOL
TryAddExtraFolderAndChildren(int parent_index) {
    FILE* fp = NULL;
    char fname[MAX_PATH];
    char readbuf[256];
    char* p;
            ...
            *p = '\0';
            ...
            if (strcmp(name, "FOLDER_SETTINGS") == 0) {
                ...
                if (!strcmp(name, "ROOT_FOLDER")) {
                    ...
                    lpTemp = NewFolder(name, current_id, parent_index, ExtraFolderData[id]->m_nSubIconId,
...
void
GetFolders(TREEFOLDER*** folders, int* num_folders) {
    *folders = treeFolders;
    *num_folders = numFolders;
}

BOOL
TryRenameCustomFolder(LPTREEFOLDER lpFolder, const char* new_name) {
    BOOL retval;
    ...
    if (retval) {
        ...
        char buf[500];
        snprintf(buf, sizeof(buf), "Error while renaming custom file %s to %s", filename, new_filename);
...
void
AddToCustomFolder(LPTREEFOLDER lpFolder, int driver_index) {
    if ((lpFolder->m_dwFlags & F_CUSTOM) == 0) {
        MessageBox(GetMainWindow(), "Unable to add game to non-custom folder", MAME32NAME, MB_OK | MB_ICONERROR);
        return;
    }
...
void
RemoveFromCustomFolder(LPTREEFOLDER lpFolder, int driver_index) {
    if ((lpFolder->m_dwFlags & F_CUSTOM) == 0) {
        MessageBox(GetMainWindow(), "Unable to remove game from non-custom folder", MAME32NAME, MB_OK | MB_ICONERROR);
        return;
    }
...
BOOL
TrySaveExtraFolder(LPTREEFOLDER lpFolder) {
    char fname[MAX_PATH];
    FILE* fp;
    BOOL error = FALSE;
    int i, j;
...
HIMAGELIST
GetTreeViewIconList(void) {
    return hTreeSmall;
}

int
GetTreeViewIconIndex(int icon_id) {
    int i;
