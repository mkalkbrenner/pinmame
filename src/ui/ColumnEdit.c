...
int
DoExchangeItem(HWND hFrom, HWND hTo, int nMinItem) {
    LV_ITEM lvi;
    char buf[80];
    int nFrom, nTo;
...
void
DoMoveItem(HWND hWnd, BOOL bDown) {
    LV_ITEM lvi;
    char buf[80];
    int nMaxpos;
    ...
    if (lvi.iItem == -1 || (lvi.iItem < 2 && bDown == FALSE) || // Disallow moving First column
...
INT_PTR
InternalColumnDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam, int nColumnMax, int* shown, int* order,
    ...
    switch (Msg) {
        case WM_INITDIALOG:
            ...
            if ((dwShowStyle & LVS_TYPEMASK) != dwView)
                SetWindowLongPtr(hShown, GWL_STYLE, (dwShowStyle & ~LVS_TYPEMASK) | dwView);
            if ((dwAvailableStyle & LVS_TYPEMASK) != dwView)
                SetWindowLongPtr(hAvailable, GWL_STYLE, (dwAvailableStyle & ~LVS_TYPEMASK) | dwView);
                ...
                case LVN_ITEMCHANGED:
                    ...
                    if ( //!(pnmv->uOldState & LVIS_SELECTED) &&
                        ...
                        if (pnmv->iItem == 0 && pnmv->hdr.idFrom == IDC_LISTSHOWCOLUMNS) {
                            ...
                            ListView_SetItemState(hShown, pnmv->iItem, LVIS_FOCUSED | LVIS_SELECTED,
                    ...
                    if (pnmv->uOldState & LVIS_SELECTED && pnmv->iItem == 0
                        && pnmv->hdr.idFrom == IDC_LISTSHOWCOLUMNS) {
...
static void
GetColumnInfo(int* order, int* shown) {
    GetColumnOrder(order);
    GetColumnShown(shown);
}

static void
SetColumnInfo(int* order, int* shown) {
    SetColumnOrder(order);
    SetColumnShown(shown);
}

INT_PTR CALLBACK
ColumnDialogProc(HWND hDlg, UINT Msg, WPARAM wParam, LPARAM lParam) {
    ...
    return InternalColumnDialogProc(hDlg, Msg, wParam, lParam, COLUMN_MAX, shown, order, column_names,
                                    GetRealColumnOrder, GetColumnInfo, SetColumnInfo);
}
