...
void
DataMapAdd(DWORD dwCtrlId, int dmType, int cType, void* var, int encoded_type, void* encoded_var, int min, int max,
           void (*func)(HWND hWnd)) {
    DATA_MAP* lpDmap = 0;
    int i;
    int ctrlNum = -1;
 ...
 */
BOOL
ReadControl(HWND hWnd, DWORD dwCtrlId) {
    int i;
    HWND hCtrl;
    DATA_MAP* lpDmap = 0;
...
static BOOL
validate(HWND hCtrl, DATA_MAP* lpDmap) {
...
int
GetControlID(HWND hDlg, HWND hwnd) {
    int i;
...
int
GetControlType(HWND hDlg, HWND hwnd) {
    int i;
...
BOOL
IsControlDifferent(HWND hDlg, HWND hwnd_ctrl, options_type* o, options_type* base) {
    int i;
    DATA_MAP* data_item;
    ...
    for (i = 0; i < numCtrls; i++) {
        ...
        switch (data_item->encoded_type) {
            ...
            case DM_STRING: {
                ...
                if (strcmp(value, *(char**)((char*)base + ((char*)data_item->encoded_var - (char*)o))) != 0)
                    return TRUE;
                return FALSE;
            }
