...
void
errormsg(const char* extmsg, const char* usermsg, const char* zipname) {
    char temp[250];
    sprintf(temp, "Error in zipfile %s\n%s\n%s\n", zipname, usermsg,
            "(This message may appear several more times, please be patient)");
...
*/
static int
ecd_find_sig(char* buffer, int buflen, int* offset) {
...
*/
ZIP*
openzip(int pathtype, int pathindex, const char* zipfile) {
    /* allocate */
...
*/
int
seekcompresszip(ZIP* zip, struct zipent* ent) {
    char buf[ZIPNAME];
    long offset;
...
*/
static int
inflate_file(osd_file* in_file, unsigned in_size, unsigned char* out_data, unsigned out_size) {
    int err;
...
*/
int
readcompresszip(ZIP* zip, struct zipent* ent, char* data) {
...
*/
int
readuncompresszip(ZIP* zip, struct zipent* ent, char* data) {
...
static ZIP*
cache_openzip(int pathtype, int pathindex, const char* zipfile) {
    ZIP* zip;
    unsigned i;

    /* search in the cache buffer */
    for (i = 0; i < ZIP_CACHE_MAX; ++i) {
        if (zip_cache_map[i] && zip_cache_map[i]->pathtype == pathtype && zip_cache_map[i]->pathindex == pathindex
            && strcmp(zip_cache_map[i]->zip, zipfile) == 0) {
            /* found */
            unsigned j;
...
*/
static int
equal_filename(const char* zipfile, const char* file) {
    const char* s1 = file;
...
int /* error */
load_zipped_file(int pathtype, int pathindex, const char* zipfile, const char* filename, unsigned char** buf,
                 unsigned int* length) {
    ZIP* zip;
    struct zipent* ent;
    ...
    while (readzip(zip)) {
        ...
        if (equal_filename(ent->name, filename) || (ent->crc32 && !strcmp(crc, filename))) {
...
int /* error */
checksum_zipped_file(int pathtype, int pathindex, const char* zipfile, const char* filename, unsigned int* length,
                     unsigned int* sum) {
    ZIP* zip;
    struct zipent* ent;
