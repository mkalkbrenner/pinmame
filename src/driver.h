...
#define MDRV_DIPS(no)            machine->pinmame.coreDips = (no);

#define SWITCH_UPDATE(name)      void name##_switch_update(int* inports)

#define MDRV_SWITCH_UPDATE(name) machine->pinmame.updSw = name##_switch_update;
#define MDRV_DIAGNOSTIC_LEDH(no) machine->pinmame.diagLEDs = (no);
#define MDRV_DIAGNOSTIC_LEDV(no) machine->pinmame.diagLEDs = (no) | DIAGLED_VERTICAL;
#define MDRV_DIAGNOSTIC_LED7     machine->pinmame.diagLEDs = 0xff;
...
#define MDRV_TIMER_ADD(func, rate) machine_add_timer(machine, func, rate);
...
#define MACHINE_DRIVER_EXTERN(game) void construct_##game(struct InternalMachineDriver* machine)
...
#define MDRV_IMPORT_FROM(game)             construct_##game(machine);

/* add/modify/remove/replace CPUs */
#define MDRV_CPU_ADD_TAG(tag, type, clock) cpu = machine_add_cpu(machine, (tag), CPU_##type, (clock));

#define MDRV_CPU_ADD(type, clock)          MDRV_CPU_ADD_TAG(NULL, type, clock)

#define MDRV_CPU_MODIFY(tag)               cpu = machine_find_cpu(machine, tag);
...
#define MDRV_VIDEO_ATTRIBUTES(flags)   machine->video_attributes = (flags);
...
#define MDRV_SOUND_ATTRIBUTES(flags)             machine->sound_attributes = (flags);

/* add/remove/replace sounds */
#define MDRV_SOUND_ADD_TAG(tag, type, interface) machine_add_sound(machine, (tag), SOUND_##type, &(interface));

#define MDRV_SOUND_ADD(type, interface)          MDRV_SOUND_ADD_TAG(NULL, type, interface)
