...
static mame_file* generic_fopen(int pathtype, const char* gamename, const char* filename, const char* hash,
                                UINT32 flags);
...
mame_file*
mame_fopen(const char* gamename, const char* filename, int filetype, int openforwrite) {
    ...
    switch (filetype) {
        ...
        /* cheat file */
        case FILETYPE_CHEAT:
            return generic_fopen(filetype, NULL, filename, 0,
                                 FILEFLAG_OPENREAD | (openforwrite ? FILEFLAG_OPENWRITE : 0));
        ...
        /* ctrlr files */
        case FILETYPE_CTRLR:
            return generic_fopen(filetype, gamename, filename, 0,
                                 openforwrite ? FILEFLAG_OPENWRITE : FILEFLAG_OPENREAD);
...
mame_file*
mame_fopen_rom(const char* gamename, const char* filename, const char* exphash) {
...
int
mame_faccess(const char* filename, int filetype) {
...
UINT32
mame_fread(mame_file* file, void* buffer, size_t length) {
    /* switch off the file type */
    switch (file->type) {
        case PLAIN_FILE:
...
UINT32
mame_fwrite(mame_file* file, const void* buffer, size_t length) {
    /* switch off the file type */
    switch (file->type) {
        case PLAIN_FILE:
...
int
mame_fseek(mame_file* file, INT64 offset, int whence) {
    int err = 0;
...
int
mame_fchecksum(const char* gamename, const char* filename, unsigned int* length, char* hash) {
    mame_file* file;
    ...
    file =
        generic_fopen(FILETYPE_ROM, gamename, filename, hash, FILEFLAG_OPENREAD | FILEFLAG_HASH | FILEFLAG_VERIFY_ONLY);
...
int
mame_ungetc(int c, mame_file* file) {
    /* switch off the file type */
    switch (file->type) {
        case PLAIN_FILE:
...
char*
mame_fgets(char* s, int n, mame_file* file) {
    char* cur = s;
...
UINT32
mame_fread_swap(mame_file* file, void* buffer, size_t length) {
    UINT8* buf;
    int res, i;
...
UINT32
mame_fwrite_swap(mame_file* file, const void* buffer, size_t length) {
    UINT8* buf;
    UINT8 temp;
    int res;
    size_t i;
...
INLINE void
compose_path(char* output, const char* gamename, const char* filename, const char* extension) {
...
static const char*
get_extension_for_filetype(int filetype) {
...
static mame_file*
generic_fopen(int pathtype, const char* gamename, const char* filename, const char* hash, UINT32 flags) {
            ...
            if (osd_get_path_info(pathtype, pathindex, name) == PATH_IS_FILE) {
                ...
                if (flags & FILEFLAG_VERIFY_ONLY) {
                    ...
                    if (hash && hash_data_extract_binary_checksum(hash, HASH_CRC, crcs) != 0) {
                        ...
                        crc = ((unsigned long)crcs[0] << 24) | ((unsigned long)crcs[1] << 16)
...
static int
checksum_file(int pathtype, int pathindex, const char* file, UINT8** p, UINT64* size, char* hash) {
    UINT64 length;
    UINT8* data;
    osd_file* f;
...
int
mame_fputs(mame_file* f, const char* s) {
...
int
mame_vfprintf(mame_file* f, const char* fmt, va_list va) {
    char buf[512];
...
int CLIB_DECL
mame_fprintf(mame_file* f, const char* fmt, ...) {
    int rc;
    va_list va;
    va_start(va, fmt);
