...
int (*cpu_irq_callbacks[MAX_CPU])(int) = {cpu_0_irq_callback, cpu_1_irq_callback, cpu_2_irq_callback,
                                          cpu_3_irq_callback, cpu_4_irq_callback, cpu_5_irq_callback,
                                          cpu_6_irq_callback, cpu_7_irq_callback};
...
cpuint_init(void) {
    ...
    for (cpunum = 0; cpunum < cpu_gettotalcpu(); cpunum++) {
        ...
        for (irqline = 0; irqline < MAX_IRQ_LINES; irqline++) {
            ...
            interrupt_vector[cpunum][irqline] = irq_line_vector[cpunum][irqline] = cpunum_default_irq_vector(cpunum);
        }
...
void
cpu_set_irq_callback(int cpunum, int (*callback)(int)) {
...
INLINE int
cpu_irq_callback(int cpunum, int irqline) {
...
static int
cpu_0_irq_callback(int irqline) {
    return cpu_irq_callback(0, irqline);
}

static int
cpu_1_irq_callback(int irqline) {
    return cpu_irq_callback(1, irqline);
}

static int
cpu_2_irq_callback(int irqline) {
    return cpu_irq_callback(2, irqline);
}

static int
cpu_3_irq_callback(int irqline) {
    return cpu_irq_callback(3, irqline);
}

static int
cpu_4_irq_callback(int irqline) {
    return cpu_irq_callback(4, irqline);
}

static int
cpu_5_irq_callback(int irqline) {
    return cpu_irq_callback(5, irqline);
}

static int
cpu_6_irq_callback(int irqline) {
    return cpu_irq_callback(6, irqline);
}

static int
cpu_7_irq_callback(int irqline) {
...
void
cpu_irq_line_vector_w(int cpunum, int irqline, int vector) {
...
static void
cpu_empty_event_queue(int cpunum) {
    int i;
...
void
cpu_set_irq_line(int cpunum, int irqline, int state) {
...
void
cpu_set_irq_line_and_vector(int cpunum, int irqline, int state, int vector) {
...
static void
cpu_clearintcallback(int cpunum) {
...
void
cpu_interrupt_enable(int cpunum, int enabled) {
