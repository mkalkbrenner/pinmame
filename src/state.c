...
/* Available flags */
enum { SS_NO_SOUND = 0x01, SS_MSB_FIRST = 0x02 };
...
static void (*ss_conv[])(unsigned char*, unsigned) = {0, 0, ss_c2, ss_c2, ss_c4, ss_c4, 0, ss_c8, ss_c4};
...
static ss_entry*
ss_register_entry(const char* module, int instance, const char* name, int type, void* data, unsigned size) {
...
void
state_save_register_UINT8(const char* module, int instance, const char* name, UINT8* val, unsigned size) {
    ss_register_entry(module, instance, name, SS_UINT8, val, size);
}

void
state_save_register_INT8(const char* module, int instance, const char* name, INT8* val, unsigned size) {
    ss_register_entry(module, instance, name, SS_INT8, val, size);
}

void
state_save_register_UINT16(const char* module, int instance, const char* name, UINT16* val, unsigned size) {
    ss_register_entry(module, instance, name, SS_UINT16, val, size);
}

void
state_save_register_INT16(const char* module, int instance, const char* name, INT16* val, unsigned size) {
    ss_register_entry(module, instance, name, SS_INT16, val, size);
}

void
state_save_register_UINT32(const char* module, int instance, const char* name, UINT32* val, unsigned size) {
    ss_register_entry(module, instance, name, SS_UINT32, val, size);
}

void
state_save_register_INT32(const char* module, int instance, const char* name, INT32* val, unsigned size) {
    ss_register_entry(module, instance, name, SS_INT32, val, size);
}

void
state_save_register_int(const char* module, int instance, const char* name, int* val) {
    ss_register_entry(module, instance, name, SS_INT, val, 1);
}

void
state_save_register_double(const char* module, int instance, const char* name, double* val, unsigned size) {
    ss_register_entry(module, instance, name, SS_DOUBLE, val, size);
}

void
state_save_register_float(const char* module, int instance, const char* name, float* val, unsigned size) {
    ss_register_entry(module, instance, name, SS_FLOAT, val, size);
}

static void
ss_register_func(ss_func** root, void (*func)(void)) {
    ss_func* next = *root;
    while (next) {
...
void
state_save_register_func_presave(void (*func)(void)) {
    ss_register_func(&ss_prefunc_reg, func);
}

void
state_save_register_func_postload(void (*func)(void)) {
    ss_register_func(&ss_postfunc_reg, func);
}

void
state_save_set_current_tag(int tag) {
    ss_current_tag = tag;
}

static void
ss_c2(unsigned char* data, unsigned size) {
    unsigned i;
    for (i = 0; i < size; i++) {
        unsigned char v;
        v = data[0];
...
static void
ss_c4(unsigned char* data, unsigned size) {
    unsigned i;
    for (i = 0; i < size; i++) {
        unsigned char v;
        v = data[0];
...
static void
ss_c8(unsigned char* data, unsigned size) {
    unsigned i;
    for (i = 0; i < size; i++) {
        unsigned char v;
        v = data[0];
...
void
state_save_save_begin(mame_file* file) {
    ss_module* m;
            ...
            for (e = m->instances[i]; e; e = e->next)
                if (e->tag == ss_current_tag) {
                    if (e->type == SS_INT) {
                        ...
                        TRACE(logerror("    %s.%d.%s: %x..%x\n", m->name, i, e->name, e->offset,
                                       e->offset + ss_size[e->type] * e->size - 1));
                    }
                }
        }
    }
...
int
state_save_load_begin(mame_file* file) {
    ss_module* m;
    unsigned int offset;
    ...
    if (ss_dump_array[8] != 1) {
        usrintf_showmessage("Error: Wrong version in save file (%d, 1 expected)", ss_dump_array[8]);
        goto bad;
    }

    file_sig =
        ss_dump_array[0x14] | (ss_dump_array[0x15] << 8) | (ss_dump_array[0x16] << 16) | (ss_dump_array[0x17] << 24);

    if (file_sig != signature) {
        usrintf_showmessage("Error: Incompatible save file (signature %08x, expected %08x)", file_sig, signature);
        goto bad;
    }
        ...
        for (i = 0; i < MAX_INSTANCES; i++) {
            ...
            for (e = m->instances[i]; e; e = e->next)
                if (e->tag == ss_current_tag) {
                    if (e->type == SS_INT) {
                        int v;
                        v = ss_dump_array[e->offset] | (ss_dump_array[e->offset + 1] << 8)
                            | (ss_dump_array[e->offset + 2] << 16) | (ss_dump_array[e->offset + 3] << 24);
                        TRACE(logerror("    %s.%d.%s: %x..%x\n", m->name, i, e->name, e->offset, e->offset + 3));
                        *(int*)(e->data) = v;
                    } else {
