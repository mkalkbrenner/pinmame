...
static void
play1s_init(struct sndbrdData* brdData) {
...
static void
play2s_init(struct sndbrdData* brdData) {
    memset(&sndlocals, 0, sizeof sndlocals);
...
static WRITE_HANDLER(play2s_ctrl_w) {
    if (!sndlocals.enSn && (data & 1)) { // sound on to full volume
        timer_adjust(sndlocals.timer, TIME_NEVER, 0, 0);
        if (!mixer_is_sample_playing(sndlocals.channel)) {
            mixer_play_sample(sndlocals.channel, (signed char*)squareWave, sizeof(squareWave),
                              2950000. / 4. / (sndlocals.freq + 1), 1);
        }
        sndlocals.volume = 100;
        mixer_set_volume(sndlocals.channel, sndlocals.volume);
...
static void
play3s_init(struct sndbrdData* brdData) {
    memset(&sndlocals, 0, sizeof sndlocals);
    tms5220_reset();
...
static WRITE_HANDLER(play3s_data_w) { sndlocals.sndCmd = data; }

static WRITE_HANDLER(play3s_ctrl_w) { sndlocals.ef[2] = data & 1; }
...
static void
play4s_timer_callback(int n) {
    cpu_set_irq_line(PLAYMATIC_SCPU, CDP1802_INPUT_LINE_INT, CLEAR_LINE);
    cpu_set_irq_line(PLAYMATIC_SCPU, CDP1802_INPUT_LINE_INT, ASSERT_LINE);
}

static void
play4s_init(struct sndbrdData* brdData) {
    memset(&sndlocals, 0, sizeof sndlocals);
    sndlocals.timer = timer_alloc(play4s_timer_callback);
    timer_adjust(sndlocals.timer, TIME_IN_HZ(3496), 0,
                 TIME_IN_HZ(3496)); // ought to be correct according to 1863 datasheet
}
...
/ exported interfaces
/--------------------*/
const struct sndbrdIntf play1sIntf = {"PLAY1",       play1s_init, NULL, NULL, play1s_data_w,
                                      play1s_data_w, NULL,        NULL, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
const struct sndbrdIntf play2sIntf = {
    "PLAY2",       play2s_init, NULL,          NULL, play2s_man_w,
    play2s_data_w, NULL,        play2s_ctrl_w, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
const struct sndbrdIntf play3sIntf = {
    "PLAY3",       play3s_init, NULL,          NULL, play3s_man_w,
    play3s_data_w, NULL,        play3s_ctrl_w, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
const struct sndbrdIntf play4sIntf = {
    "PLAY4",       play4s_init, NULL,          NULL, play4s_man_w,
    play4s_data_w, NULL,        play4s_ctrl_w, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
...
static DISCRETE_SOUND_START(play_tones) DISCRETE_INPUT(NODE_01, 1, 0x000f, 0) // Input handlers, mostly for enable
    DISCRETE_INPUT(NODE_02, 2, 0x000f, 0) DISCRETE_INPUT(NODE_04, 4, 0x000f, 0) DISCRETE_INPUT(NODE_08, 8, 0x000f, 0)
    ...
    MACHINE_DRIVER_START(PLAYMATICS1) MDRV_SOUND_ADD(DISCRETE, play_tones) MACHINE_DRIVER_END
...
static READ_HANDLER(in_snd_3) { return sndlocals.sndCmd; }

static WRITE_HANDLER(out_snd_3) { tms5220_data_w(0, data); }
...
static MEMORY_READ_START(playsound_readmem3){0x0000, 0x1fff, MRA_ROM}, {0x2000, 0x201f, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(playsound_writemem3){0x2000, 0x201f, MWA_RAM},
    MEMORY_END

    static PORT_READ_START(playsound_readport3){0x02, 0x02, in_snd_3},
    MEMORY_END

    static PORT_WRITE_START(playsound_writeport3){0x01, 0x01, out_snd_3},
    MEMORY_END

    static void play_5220Irq(int state) {
    cpu_set_irq_line(PLAYMATIC_SCPU, CDP1802_INPUT_LINE_INT, state ? CLEAR_LINE : ASSERT_LINE);
}
static struct TMS5220interface play3s_5220Int = {640000, 75, play_5220Irq};
...
struct AY8910interface play4s_8910Int = {2,                                                       /* 2 chips */
...
static WRITE_HANDLER(ay0_w) { AY8910Write(0, offset % 2, data); }

static WRITE_HANDLER(ay1_w) { AY8910Write(1, offset % 2, data); }
...
static WRITE_HANDLER(clk_snd) {
    ...
    timer_adjust(sndlocals.timer, TIME_IN_HZ((3579545 >> 6) / (data + 1)), 0,
                 TIME_IN_HZ((3579545 >> 6) / (data + 1))); // too fast? but sounds right!
}
...
static MEMORY_READ_START(playsound_readmem4){0x0000, 0x3fff, MRA_ROM}, {0x4000, 0x5fff, AY8910_read_port_0_r},
    {0x6000, 0x7fff, AY8910_read_port_1_r}, {0x8000, 0x80ff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(playsound_writemem4){0x4000, 0x5fff, ay0_w},
    {0x6000, 0x7fff, ay1_w}, {0x8000, 0x80ff, MWA_RAM},
    MEMORY_END

    static PORT_READ_START(playsound_readport4){0x02, 0x02, in_snd_4},
    MEMORY_END

    static PORT_WRITE_START(playsound_writeport4){0x01, 0x01, clk_snd},
    MEMORY_END

    MACHINE_DRIVER_START(PLAYMATICS4) MDRV_CPU_ADD_TAG("scpu", CDP1802, 3579545) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_CONFIG(playsound_config4) MDRV_CPU_MEMORY(playsound_readmem4, playsound_writemem4)
            MDRV_CPU_PORTS(playsound_readport4, playsound_writeport4) MDRV_SOUND_ADD(AY8910, play4s_8910Int)
...
static WRITE_HANDLER(ay_data_w) { sndlocals.aydata = data; }

static WRITE_HANDLER(ay_ctrl_w) {
    sndlocals.ayctrl = data & 3;
    switch (sndlocals.ayctrl) {
        case 1:
            AY8910_write_port_0_w(0, sndlocals.aydata);
            break;
        case 2:
            sndlocals.aydata = AY8910Read(0);
            break;
        case 3:
            AY8910_control_port_0_w(0, sndlocals.aydata);
            break;
    }
}

static READ_HANDLER(ay_data_r) { return sndlocals.aydata; }

static WRITE_HANDLER(ay8910_z_porta_w) { coreGlobals.lampMatrix[8] = coreGlobals.tmpLampMatrix[8] = ~data; }

static WRITE_HANDLER(ay8910_z_portb_w) { coreGlobals.lampMatrix[9] = coreGlobals.tmpLampMatrix[9] = ~data; }
struct AY8910interface playzs_8910Int = {
    1,       /* 1 chip */
    2012160, /* 2.01216 MHz quartz on pic! */
    {25},    /* Volume */
    {0},     {0}, {ay8910_z_porta_w}, {ay8910_z_portb_w},
};

static WRITE_HANDLER(romsel_w) { cpu_setbank(1, memory_region(PLAYMATIC_MEMREG_SCPU) + ((data & 8) ? 0x400 : 0)); }

static MEMORY_READ_START(playsound_readmemz){0x0000, 0x03ff, MRA_BANKNO(1)},
    MEMORY_END

    static MEMORY_WRITE_START(playsound_writememz) MEMORY_END

    static PORT_READ_START(playsound_readportz){COP400_PORT_L, COP400_PORT_L, ay_data_r},
    {COP400_PORT_IN, COP400_PORT_IN, in_snd_z},
    MEMORY_END

    static PORT_WRITE_START(playsound_writeportz){COP400_PORT_L, COP400_PORT_L, ay_data_w},
    {COP400_PORT_G, COP400_PORT_G, ay_ctrl_w}, {COP400_PORT_D, COP400_PORT_D, romsel_w},
    MEMORY_END
...
static WRITE_HANDLER(playzs_ctrl_w) { sndlocals.sndCmd = data; }

const struct sndbrdIntf playzsIntf = {"PLAYZ", playzs_init, NULL,          NULL, playzs_ctrl_w,
                                      NULL,    NULL,        playzs_ctrl_w, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
