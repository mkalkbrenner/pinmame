...
static NVRAM_HANDLER(byVP) { core_nvram(file, read_or_write, byVP_CMOS, 0x100, 0xff); }
...
static void
byVP_lampStrobe(int board, int lampadr) {
    if (lampadr != 0x0f) {
...
static WRITE_HANDLER(pia0a_w) {
    ...
    if ((locals.p1_a & 0x04) == 0)
        soundlatch2_w(0, data ^ 0x0f);
...
static READ_HANDLER(pia0b_r) {
    ...
    if ((locals.p1_a & 0x08) == 0)
        return ~locals.p2_a & 0x03;
    // Enable Output must be low to return data..
    if ((locals.p1_a & 0x02) == 0)
        return soundlatch_r(0);

    if (locals.p0_a & 0x20)
        return core_getDip(0); // DIP#1-8
    if (locals.p0_a & 0x40)
        return core_getDip(1); // DIP#9-16
    if (locals.p0_a & 0x80)
        return core_getDip(2); // DIP#17-24
    if (locals.p0_cb2)
        return core_getDip(3); // DIP#25-32
...
static WRITE_HANDLER(pia0cb2_w) {
    if (locals.p0_cb2 & ~data)
        locals.lampadr1 = locals.p0_a & 0x0f;
    locals.p0_cb2 = data;
}
...
static READ_HANDLER(pia2b_r) {
    ...
    if (locals.p2_a & 0x40)
        return (coreGlobals.swMatrix[6] & 0x1)
               << 7; //Power Button for Granny - Mapped to switch #41, since only 40 switches are used in the game.
    return 0;
}
...
static SWITCH_UPDATE(byVP) {
    ...
    if (core_getSw(BYVP_SWCPUDIAG))
        cpu_set_nmi_line(0, PULSE_LINE);
    if (core_getSw(BYVP_SWVIDEODIAG))
        cpu_set_nmi_line(BYVP_VCPUNO, PULSE_LINE);
...
static struct pia6821_interface piaIntf[] = {{/* I:  A/B,CA1/B1,CA2/B2 */ 0, pia0b_r, 0, 0, 0, 0,
...
static INTERRUPT_GEN(byVP_irq) { pia_set_input_ca1(BYVP_PIA1, locals.irqstate = !locals.irqstate); }
...
static VIDEO_START(byVP) {
    if ((TMS9928A_start(0, TMS99x8A, 0x4000)) || (core_gameData->hw.display && TMS9928A_start(1, TMS99x8A, 0x4000)))
        return 1;
    TMS9928A_int_callback(0, by_vdp_interrupt);
    return 0;
}

static VIDEO_STOP(byVP) { TMS9928A_stop(core_gameData->hw.display ? 2 : 1); }
...
/------------------------------------*/
static MEMORY_READ_START(byVP_readmem){0x0000, 0x0080, MRA_RAM}, /* U7 128 Byte Ram*/
    ...
    {0x1000, 0x1fff, MRA_ROM}, {0x5000, 0x5fff, MRA_ROM}, {0xf000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(byVP_writemem){0x0000, 0x0080, MWA_RAM}, /* U7 128 Byte Ram*/
    ...
    {0x1000, 0x1fff, MWA_ROM}, {0x5000, 0x5fff, MWA_ROM}, {0xf000, 0xffff, MWA_ROM},
    MEMORY_END

    /*----------------------------------------------------
/  Memory map for VIDEO CPU (Located on Vidiot Board)
/----------------------------------------------------*/
    static MEMORY_READ_START(byVP_video_readmem){0x0000, 0x1fff, soundlatch2_r},
    ...
    static MEMORY_WRITE_START(byVP_video_writemem){0x0000, 0x1fff, soundlatch_w},
...
/------------------------------------------------------------------*/
    static MEMORY_READ_START(byVPGG_video_readmem){0x0000, 0x0001, soundlatch2_r},
    ...
    static MEMORY_WRITE_START(byVPGG_video_writemem){0x0000, 0x0001, soundlatch_w},
    ...
    const struct core_dispLayout byVP_dispBabyPac[] = {{0, 0, 192, 256, CORE_VIDEO, (genf*)byVP_update, NULL}, {0}};
const struct core_dispLayout byVP_dispGranny[] = {{0, 0, 192, 256, CORE_VIDEO, (genf*)byVP_update, NULL}, {0}};
/*--------------------*/
/* Machine Definition */
/*--------------------*/
/* BABYPAC HARDWARE */
