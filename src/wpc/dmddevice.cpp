...
typedef void (*DmdDev_Render_16_Shades_with_Raw_t)(UINT16 width, UINT16 height, UINT8* currbuffer, UINT32 noOfRawFrames,
                                                   UINT8* rawbuffer);
typedef void (*DmdDev_Render_4_Shades_with_Raw_t)(UINT16 width, UINT16 height, UINT8* currbuffer, UINT32 noOfRawFrames,
                                                  UINT8* rawbuffer);
typedef void (*DmdDev_render_PM_Alphanumeric_Frame_t)(layout_t layout, const UINT16* const seg_data,
                                                      const UINT16* const seg_data2);
typedef void (*DmdDev_render_PM_Alphanumeric_Dim_Frame_t)(layout_t layout, const UINT16* const seg_data,
                                                          const char* const seg_dim, const UINT16* const seg_data2);
...
int
RcvConsoleInput(UINT8* buf, int size) {
    int ret = at91_receive_serial(1, buf, size);
    return ret;
}

int
pindmdInit(const char* GameName, UINT64 HardwareGeneration, const tPMoptions* Options) {
    ...
    DmdDev_hModule =
        LoadLibraryEx(DmdDev_filename, NULL, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
    ...
    DmdScr_hModule =
        LoadLibraryEx(DmdScr_filename, NULL, LOAD_LIBRARY_SEARCH_DLL_LOAD_DIR | LOAD_LIBRARY_SEARCH_DEFAULT_DIRS);
    ...
    if (DmdDev_hModule) {
        ...
        DmdDev_Render_4_Shades_with_Raw =
            (DmdDev_Render_4_Shades_with_Raw_t)GetProcAddress(DmdDev_hModule, "Render_4_Shades_with_Raw");
        DmdDev_Render_16_Shades_with_Raw =
            (DmdDev_Render_16_Shades_with_Raw_t)GetProcAddress(DmdDev_hModule, "Render_16_Shades_with_Raw");
        DmdDev_render_PM_Alphanumeric_Frame =
            (DmdDev_render_PM_Alphanumeric_Frame_t)GetProcAddress(DmdDev_hModule, "Render_PM_Alphanumeric_Frame");
        DmdDev_render_PM_Alphanumeric_Dim_Frame = (DmdDev_render_PM_Alphanumeric_Dim_Frame_t)GetProcAddress(
            DmdDev_hModule, "Render_PM_Alphanumeric_Dim_Frame");
        DmdDev_Set_4_Colors_Palette =
            (DmdDev_Set_4_Colors_Palette_t)GetProcAddress(DmdDev_hModule, "Set_4_Colors_Palette");
        ...
        if (!DmdDev_Open || !DmdDev_Close || !DmdDev_PM_GameSettings || !DmdDev_Render_4_Shades
            || !DmdDev_Render_16_Shades || !DmdDev_render_PM_Alphanumeric_Frame) {
            DmdDev = false;
        } else {
            DmdDev = true;
        }
    ...
    if (DmdScr_hModule) {
        ...
        DmdScr_Render_4_Shades_with_Raw =
            (DmdDev_Render_4_Shades_with_Raw_t)GetProcAddress(DmdScr_hModule, "Render_4_Shades_with_Raw");
        DmdScr_Render_16_Shades_with_Raw =
            (DmdDev_Render_16_Shades_with_Raw_t)GetProcAddress(DmdScr_hModule, "Render_16_Shades_with_Raw");
        DmdScr_render_PM_Alphanumeric_Frame =
            (DmdDev_render_PM_Alphanumeric_Frame_t)GetProcAddress(DmdScr_hModule, "Render_PM_Alphanumeric_Frame");
        DmdScr_render_PM_Alphanumeric_Dim_Frame = (DmdDev_render_PM_Alphanumeric_Dim_Frame_t)GetProcAddress(
            DmdScr_hModule, "Render_PM_Alphanumeric_Dim_Frame");
        DmdScr_Set_4_Colors_Palette =
            (DmdDev_Set_4_Colors_Palette_t)GetProcAddress(DmdScr_hModule, "Set_4_Colors_Palette");
        ...
        if (!DmdScr_Open || !DmdScr_Close || !DmdScr_PM_GameSettings || !DmdScr_Render_4_Shades
            || !DmdScr_Render_16_Shades || !DmdScr_render_PM_Alphanumeric_Frame) {
            DmdScr = false;
        } else {
            DmdScr = true;
        }
    ...
    if (!DmdScr && !DmdDev) {
        MessageBox(NULL, "No external DMD driver found or DMD driver functions not found", "Visual PinMame Error",
                   MB_ICONERROR);
        return 0;
    } else
        return 1;
...
void
renderDMDFrame(UINT64 gen, UINT16 width, UINT16 height, UINT8* currbuffer, UINT8 doDumpFrame, const char* GameName,
               UINT32 noOfRawFrames, UINT8* rawbuffer) {
...
void
render2ndDMDFrame(UINT64 gen, UINT16 width, UINT16 height, UINT8* currbuffer, UINT8 doDumpFrame, const char* GameName,
                  UINT32 noOfRawFrames, UINT8* rawbuffer) {
...
void
renderAlphanumericFrame(UINT64 gen, UINT16* seg_data, char* seg_dim, UINT8 total_disp, UINT8* disp_num_segs) {
