...
static int
INDER_sw2m(int no) {
    return (no / 10 - 4) * 8 + no % 10;
}

static int
INDER_m2sw(int col, int row) {
    return 40 + col * 10 + row;
}

static INTERRUPT_GEN(INDER_irq) { cpu_set_irq_line(INDER_CPU, 0, PULSE_LINE); }
...
static SWITCH_UPDATE(INDER) {
    ...
    for (i = 0; i < 5; i++)
        locals.swCol[i] = ~coreGlobals.swMatrix[i + 1];
}

static READ_HANDLER(dip_r) { return ~core_getDip(3 - offset); }
...
static WRITE_HANDLER(sol_w) { locals.solenoids |= data; }

static READ_HANDLER(sndcmd_r) { return locals.sndCmd; }
...
static NVRAM_HANDLER(INDER) { core_nvram(file, read_or_write, INDER_CMOS, 0x100, 0xff); }
...
/ PIA definitions
/-------------------*/
static READ_HANDLER(ci20_porta_r) {
    logerror("UNDOCUMENTED: ci20_porta_r\n");
    return 0;
}

static READ_HANDLER(ci20_portb_r) {
    logerror("UNDOCUMENTED: ci20_portb_r\n");
    return 0;
}
...
static READ_HANDLER(ci21_porta_r) {
    logerror("UNDOCUMENTED: ci21_porta_r\n");
    return 0;
}

static READ_HANDLER(ci21_portb_r) {
    logerror("UNDOCUMENTED: ci21_portb_r\n");
    return 0;
}

static READ_HANDLER(ci21_portc_r) {
    logerror("UNDOCUMENTED: ci21_portc_r\n");
    return 0;
}

static READ_HANDLER(ci22_porta_r) {
    logerror("UNDOCUMENTED: ci22_porta_r\n");
    return 0;
}

static READ_HANDLER(ci22_portb_r) {
    logerror("UNDOCUMENTED: ci22_portb_r\n");
    return 0;
}

static READ_HANDLER(ci22_portc_r) {
    logerror("UNDOCUMENTED: ci22_portc_r\n");
    return 0;
}

static READ_HANDLER(ci23_porta_r) {
    logerror("UNDOCUMENTED: ci23_porta_r\n");
    return 0;
}

static READ_HANDLER(ci23_portb_r) {
    logerror("UNDOCUMENTED: ci23_portb_r\n");
    return 0;
}

static READ_HANDLER(ci23_portc_r) {
    logerror("UNDOCUMENTED: ci23_portc_r\n");
    return 0;
}
...
// (always 00?)
static WRITE_HANDLER(ci20_portb_w) { locals.solenoids |= data << 16; }

// (always ff?)
static WRITE_HANDLER(ci20_portc_w) { coreGlobals.lampMatrix[7] = data ^ 0xff; }

// solenoids
static WRITE_HANDLER(ci21_porta_w) { locals.solenoids |= data ^ 0xee; }

// lamps
static WRITE_HANDLER(ci21_portb_w) { coreGlobals.lampMatrix[0] = data; }
...
// lamps
static WRITE_HANDLER(ci22_porta_w) { coreGlobals.lampMatrix[2] = data; }

// lamps
static WRITE_HANDLER(ci22_portb_w) { coreGlobals.lampMatrix[6] = data; }

// lamps
static WRITE_HANDLER(ci22_portc_w) { coreGlobals.lampMatrix[3] = data; }

// lamps
static WRITE_HANDLER(ci23_porta_w) { coreGlobals.lampMatrix[4] = data; }

// lamps
static WRITE_HANDLER(ci23_portb_w) { coreGlobals.lampMatrix[5] = data; }
...
// display data
static WRITE_HANDLER(disp16_w) { locals.dispSeg[offset] = data; }

// sound
static WRITE_HANDLER(snd_w) { locals.sndCmd = data; }

// unknown, 0x21 on game start, 0x20 on game over
static WRITE_HANDLER(X6ce0_w) {
    if (data)
        logerror("Unknown write to 0x6ce0: %02x\n", data);
...
static MEMORY_READ_START(INDER_readmem){0x0000, 0x3fff, MRA_ROM}, {0x4000, 0x43ff, MRA_RAM},
    {0x4400, 0x44ff, INDER_CMOS_r}, {0x6000, 0x6003, ppi8255_0_r}, {0x6400, 0x6403, ppi8255_1_r},
    {0x6800, 0x6803, ppi8255_2_r}, {0x6c00, 0x6c03, ppi8255_3_r},
    MEMORY_END

    static MEMORY_WRITE_START(INDER_writemem){0x4000, 0x43ff, MWA_RAM},
    {0x4400, 0x44ff, INDER_CMOS_w, &INDER_CMOS},
    //{0x4900,0x4900, MWA_NOP}, // unknown stuff here
    {0x6000, 0x6003, ppi8255_0_w}, {0x6400, 0x6403, ppi8255_1_w}, {0x6800, 0x6803, ppi8255_2_w},
    {0x6c00, 0x6c03, ppi8255_3_w}, {0x6c20, 0x6c20, snd_w}, {0x6c60, 0x6c66, disp16_w}, {0x6ce0, 0x6ce0, X6ce0_w},
    MEMORY_END
...
static MACHINE_DRIVER_START(INDER) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", Z80, 2500000)
    MDRV_CPU_MEMORY(INDER_readmem, INDER_writemem) MDRV_CPU_VBLANK_INT(INDER_vblank, 1)
        MDRV_CPU_PERIODIC_INT(INDER_irq, 250) MDRV_CORE_INIT_RESET_STOP(INDER, NULL, NULL) MDRV_NVRAM_HANDLER(INDER)
            MDRV_DIPS(24) MDRV_SWITCH_CONV(INDER_sw2m, INDER_m2sw) MDRV_SWITCH_UPDATE(INDER)
                MDRV_DIAGNOSTIC_LEDH(1) MACHINE_DRIVER_END

    static MEMORY_READ_START(INDER0_readmem){0x0000, 0x1fff, MRA_ROM},
    {0x4000, 0x43ff, MRA_RAM}, {0x4400, 0x44ff, INDER_CMOS_r}, {0x4800, 0x4802, dip_r}, {0x4805, 0x4809, sw_r},
    MEMORY_END

    static MEMORY_WRITE_START(INDER0_writemem){0x2000, 0x20ff, disp_w},
    {0x4000, 0x43ff, MWA_RAM}, {0x4400, 0x44ff, INDER_CMOS_w, &INDER_CMOS},
    //{0x4800,0x4805, MWA_NOP}, // unknown stuff here
    {0x4806, 0x480a, sw_w}, {0x4900, 0x4900, sol_w}, {0x4901, 0x4907, lamp_w}, {0x4b00, 0x4b00, ti76489_0_w},
    MEMORY_END

    MACHINE_DRIVER_START(INDER0) MDRV_IMPORT_FROM(INDER) MDRV_CPU_MODIFY("mcpu")
        MDRV_CPU_MEMORY(INDER0_readmem, INDER0_writemem) MDRV_SOUND_ADD(SN76489, INDER_ti76489Int) MACHINE_DRIVER_END

    static MEMORY_READ_START(INDER1_readmem){0x0000, 0x1fff, MRA_ROM},
    {0x4000, 0x43ff, MRA_RAM}, {0x4400, 0x44ff, INDER_CMOS_r}, {0x4800, 0x4802, dip_r}, {0x4805, 0x4809, sw_r},
    {0x4b01, 0x4b01, ay8910_0_r},
    MEMORY_END

    static MEMORY_WRITE_START(INDER1_writemem){0x2000, 0x20ff, disp_w},
    {0x4000, 0x43ff, MWA_RAM}, {0x4400, 0x44ff, INDER_CMOS_w, &INDER_CMOS},
    //{0x4800,0x4805, MWA_NOP}, // unknown stuff here
    {0x4806, 0x480a, sw_w}, {0x4900, 0x4900, sol_w}, {0x4901, 0x4907, lamp_w}, {0x4b00, 0x4b00, ay8910_0_ctrl_w},
    {0x4b02, 0x4b02, ay8910_0_data_w},
    MEMORY_END

    MACHINE_DRIVER_START(INDER1) MDRV_IMPORT_FROM(INDER) MDRV_CPU_MODIFY("mcpu")
        MDRV_CPU_MEMORY(INDER1_readmem, INDER1_writemem) MDRV_SOUND_ADD(AY8910, INDER_ay8910Int) MACHINE_DRIVER_END

    static INTERRUPT_GEN(inder2_snd_irq) {
    cpu_set_irq_line(INDER_SND_CPU, 0, PULSE_LINE);
}

static MEMORY_WRITE_START(INDER2_writemem){0x2000, 0x20ff, disp_w}, {0x4000, 0x43ff, MWA_RAM},
    {0x4400, 0x44ff, INDER_CMOS_w, &INDER_CMOS},
    //{0x4800,0x4805, MWA_NOP}, // unknown stuff here
    {0x4806, 0x480a, sw_w}, {0x4900, 0x4900, sol_w}, {0x4901, 0x4907, lamp_w}, {0x4a00, 0x4a00, snd2_w},
    {0x4a01, 0x4a01, disp2_w},
    //{0x4b00,0x4b00, MWA_NOP}, // unknown stuff here
    MEMORY_END

    static MEMORY_READ_START(inder2_snd_readmem){0x0000, 0x1fff, MRA_ROM},
    {0x8000, 0x87ff, MRA_RAM}, {0x9001, 0x9001, ay8910_0_r}, {0xa001, 0xa001, ay8910_1_r},
    MEMORY_END

    static MEMORY_WRITE_START(inder2_snd_writemem){0x8000, 0x87ff, MWA_RAM},
    {0x9000, 0x9000, ay8910_0_ctrl_w}, {0x9002, 0x9002, ay8910_0_data_w}, {0xa000, 0xa000, ay8910_1_ctrl_w},
...
static MACHINE_STOP(INDER2) { sndbrd_0_exit(); }
...
static READ_HANDLER(snd_porta_r) {
    logerror(("SND1_PORTA_R\n"));
    return 0;
}

static READ_HANDLER(snd_portb_r) {
    logerror(("SND1_PORTB_R\n"));
    return 0;
}

static READ_HANDLER(snd_portc_r) { return !sndlocals.PC0; }

static WRITE_HANDLER(snd_porta_w) { sndlocals.ALO = data; }

static WRITE_HANDLER(snd_portb_w) { sndlocals.AHI = data; }
...
static WRITE_HANDLER(sndctrl_w) { sndlocals.CS = core_BitColToNum(data ^ 0xff); }
...
static MEMORY_READ_START(indersnd_readmem){0x0000, 0x1fff, MRA_ROM}, {0x2000, 0x2fff, MRA_RAM},
    {0x4000, 0x4003, ppi8255_4_r}, {0x8000, 0x8000, sndcmd_r},
    MEMORY_END

    static MEMORY_WRITE_START(indersnd_writemem){0x2000, 0x2fff, MWA_RAM},
    {0x4000, 0x4900, ppi8255_4_w}, {0x6000, 0x6000, sndctrl_w},
    MEMORY_END

    MACHINE_DRIVER_START(INDERS1) MDRV_IMPORT_FROM(INDER) MDRV_CPU_MODIFY("mcpu")
        MDRV_CPU_PERIODIC_INT(INDER_irq, 180) // any higher, and switches behave erratic

    MDRV_CPU_ADD_TAG("scpu", Z80, 2500000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CORE_INIT_RESET_STOP(INDERS1, NULL, INDER2) MDRV_CPU_MEMORY(indersnd_readmem, indersnd_writemem)

            MDRV_INTERLEAVE(50) MDRV_SOUND_ADD(MSM5205, INDER_msm5205Int) MACHINE_DRIVER_END
...
static WRITE_HANDLER(ci21_portb_w0) { coreGlobals.lampMatrix[2] = data; }

static WRITE_HANDLER(ci21_portc_w0) { coreGlobals.lampMatrix[3] = data; }

static WRITE_HANDLER(ci22_porta_w0) { coreGlobals.lampMatrix[5] = ~data; }

static WRITE_HANDLER(ci22_portb_w0) { locals.solenoids |= data ^ 0xee; }
...
static WRITE_HANDLER(ci23_porta_w0) { coreGlobals.lampMatrix[1] = data ^ 0x80; }

static WRITE_HANDLER(ci23_portb_w0) { coreGlobals.lampMatrix[0] = data; }
...
static READ_HANDLER(ci20_portb_r2) { return 0x02; }
...
static READ_HANDLER(INDER_S1_MSM5205_READROM) {
    int addr, data;
    addr = (sndlocals2.S1_CS1 << 18) | (sndlocals2.S1_A17 << 17) | (sndlocals2.S1_A16 << 16) | (sndlocals2.S1_AHI << 8)
           | sndlocals2.S1_ALO;
...
static READ_HANDLER(INDER_S2_MSM5205_READROM) {
    int addr, data;
    addr = (sndlocals2.S2_CS1 << 18) | (sndlocals2.S2_A17 << 17) | (sndlocals2.S2_A16 << 16) | (sndlocals2.S2_AHI << 8)
           | sndlocals2.S2_ALO;
    data = (UINT8) * (memory_region(REGION_USER2) + addr);
    return data;
}

static WRITE_HANDLER(INDER_S1_MSM5205_w) { sndlocals2.S1_MSMDATA = data; }

static WRITE_HANDLER(INDER_S2_MSM5205_w) { sndlocals2.S2_MSMDATA = data; }

/* MSM5205 interrupt callback */
static void
INDER_S1_msmIrq(int data) {
    //Write data
...
static WRITE_HANDLER(extra_w) { coreGlobals.lampMatrix[8 + offset] = data; }

static MEMORY_READ_START(INDERS2_readmem){0x0000, 0x3fff, MRA_ROM}, {0x4000, 0x45ff, MRA_RAM},
    {0x6000, 0x6003, ppi8255_0_r}, {0x6400, 0x6403, ppi8255_1_r}, {0x6800, 0x6803, ppi8255_2_r},
    {0x6c00, 0x6c03, ppi8255_3_r},
    MEMORY_END

    static MEMORY_WRITE_START(INDERS2_writemem){0x4000, 0x43ff, MWA_RAM},
    {0x4400, 0x45ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    //{0x4900,0x4900, MWA_NOP}, // unknown stuff here
    {0x6000, 0x6003, ppi8255_0_w}, {0x6400, 0x6403, ppi8255_1_w}, {0x6800, 0x6803, ppi8255_2_w},
    {0x6c00, 0x6c03, ppi8255_3_w}, {0x6c20, 0x6c20, snd_w}, {0x6c40, 0x6c45, extra_w}, {0x6c60, 0x6c66, disp16_w},
    {0x6ce0, 0x6ce0, X6ce0_w},
    MEMORY_END

    //CPU #1 - SOUND EFFECTS SAMPLES
    static MEMORY_READ_START(indersnd1_readmem){0x0000, 0x1fff, MRA_ROM},
    {0x2000, 0x3fff, MRA_RAM}, {0x4000, 0x4003, ppi8255_4_r},
    {0x8000, 0x8000, sndcmd_r}, MEMORY_END static MEMORY_WRITE_START(indersnd1_writemem){0x0000, 0x1fff, MWA_ROM},
    {0x2000, 0x3fff, MWA_RAM}, {0x4000, 0x4003, ppi8255_4_w},
    //{ 0x4900, 0x4900, MWA_NOP}, // unknown stuff here
    {0x6000, 0x6000, sndctrl_1_w},
    MEMORY_END

    //CPU #2 - MUSIC SAMPLES
    static MEMORY_READ_START(indersnd2_readmem){0x0000, 0x1fff, MRA_ROM},
    {0x2000, 0x3fff, MRA_RAM}, {0x4000, 0x4003, ppi8255_5_r},
    {0x8000, 0x8000, sndcmd_r}, MEMORY_END static MEMORY_WRITE_START(indersnd2_writemem){0x0000, 0x1fff, MWA_ROM},
    {0x2000, 0x3fff, MWA_RAM}, {0x4000, 0x4003, ppi8255_5_w},
    //{ 0x4900, 0x4900, MWA_NOP}, // unknown stuff here
    {0x6000, 0x6000, sndctrl_2_w},
    MEMORY_END

    MACHINE_DRIVER_START(INDERS2) MDRV_IMPORT_FROM(INDER) MDRV_CPU_MODIFY("mcpu")
        MDRV_CPU_MEMORY(INDERS2_readmem, INDERS2_writemem)
            MDRV_CPU_PERIODIC_INT(INDER_irq, 180) // adjustable on real machine
    MDRV_CORE_INIT_RESET_STOP(INDERS2, NULL, INDER2) MDRV_NVRAM_HANDLER(generic_0fill)

        MDRV_CPU_ADD_TAG("scpu1", Z80, 5000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
            MDRV_CPU_MEMORY(indersnd1_readmem, indersnd1_writemem)

                MDRV_CPU_ADD_TAG("scpu2", Z80, 5000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
                    MDRV_CPU_MEMORY(indersnd2_readmem, indersnd2_writemem)

                        MDRV_INTERLEAVE(50) MDRV_SOUND_ADD(MSM5205, inder_msm5205Int2) MACHINE_DRIVER_END

    static struct MSM5205interface INDER_msm5205IntRana = {1,
