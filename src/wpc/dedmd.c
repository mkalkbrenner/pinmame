...
const struct sndbrdIntf dedmd32Intf = {NULL,       dmd32_init, NULL,         NULL,         NULL,
...
static MEMORY_READ_START(dmd32_readmem){0x0000, 0x1fff, MRA_RAM},
    {0x2000, 0x2fff, MRA_RAM}, /* DMD RAM PAGE 0-7 512 bytes each */
    {0x3000, 0x3000, crtc6845_register_0_r}, {0x3001, 0x3002, MRA_NOP}, {0x3003, 0x3003, dmd32_latch_r},
    {0x3004, 0x3fff, MRA_NOP}, {0x4000, 0x7fff, MRA_BANKNO(DMD32_BANK0)}, /* Banked ROM */
    {0x8000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(dmd32_writemem){0x0000, 0x1fff, MWA_RAM},
    {0x2000, 0x2fff, MWA_RAM, &dmd32RAM}, /* DMD RAM PAGE 0-7 512 bytes each*/
    {0x3000, 0x3000, crtc6845_address_0_w}, {0x3001, 0x3001, crtc6845_register_0_w},
    {0x3002, 0x3002, dmd32_bank_w},                              /* DMD Bank Switching*/
    {0x3003, 0x3fff, MWA_NOP}, {0x4000, 0x4000, dmd32_status_w}, /* DMD Status*/
    {0x4001, 0xffff, MWA_NOP},
    MEMORY_END

    MACHINE_DRIVER_START(de_dmd32_hack) MDRV_CPU_ADD(M6809, 2000000) // 8000000/4
    MDRV_CPU_MEMORY(dmd32_readmem, dmd32_writemem) MDRV_CPU_PERIODIC_INT(dmd32_firq, DMD32_FIRQFREQ_HACK)
        MDRV_INTERLEAVE(50) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(de_dmd32) MDRV_CPU_ADD(M6809, 2000000) // 8000000/4
    MDRV_CPU_MEMORY(dmd32_readmem, dmd32_writemem) MDRV_CPU_PERIODIC_INT(dmd32_firq, DMD32_FIRQFREQ_DE)
        MDRV_INTERLEAVE(50) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(se_dmd32) MDRV_CPU_ADD(M6809, 2000000) // 8000000/4
    MDRV_CPU_MEMORY(dmd32_readmem, dmd32_writemem) MDRV_CPU_PERIODIC_INT(dmd32_firq, DMD32_FIRQFREQ_SE)
        MDRV_INTERLEAVE(50) MACHINE_DRIVER_END
...
static WRITE_HANDLER(dmd32_bank_w) { cpu_setbank(DMD32_BANK0, dmdlocals.brdData.romRegion + (data & 0x1f) * 0x4000); }
...
static INTERRUPT_GEN(dmd32_firq) { cpu_set_irq_line(dmdlocals.brdData.cpuNo, M6809_FIRQ_LINE, HOLD_LINE); }
...
const struct sndbrdIntf dedmd64Intf = {NULL,       dmd64_init, NULL,         NULL,         NULL,
...
static MEMORY_READ16_START(dmd64_readmem){0x00000000, 0x000fffff, MRA16_ROM}, /* ROM (2 X 512K)*/
    ...
    static MEMORY_WRITE16_START(dmd64_writemem){0x00000000, 0x000fffff, MWA16_ROM}, /* ROM (2 X 512K)*/
    {0x00800000, 0x0080ffff, MWA16_RAM, &dmd64RAM}, /* RAM - 0x800000 Page 0, 0x801000 Page 1*/
    {0x00c00010, 0x00c00011, crtc6845_msb_address_w}, {0x00c00012, 0x00c00013, crtc6845_msb_register_w},
    {0x00c00020, 0x00c00021, dmd64_status_w}, /* Set the Status Line*/
    MEMORY_END

    MACHINE_DRIVER_START(de_dmd64) MDRV_CPU_ADD(M68000, 12000000) // schematics
    MDRV_CPU_MEMORY(dmd64_readmem, dmd64_writemem) MDRV_CPU_PERIODIC_INT(dmd64_irq2, DMD64_IRQ2FREQ)
        MDRV_INTERLEAVE(50) MACHINE_DRIVER_END
...
static INTERRUPT_GEN(dmd64_irq2) { cpu_set_irq_line(dmdlocals.brdData.cpuNo, MC68000_IRQ_2, HOLD_LINE); }

static WRITE16_HANDLER(crtc6845_msb_address_w) {
    if (ACCESSING_MSB)
        crtc6845_address_0_w(offset, data >> 8);
}

static WRITE16_HANDLER(crtc6845_msb_register_w) {
    if (ACCESSING_MSB)
        crtc6845_register_0_w(offset, data >> 8);
}
...
const struct sndbrdIntf dedmd16Intf = {NULL,       dmd16_init, NULL,         NULL,         NULL,
...
static MEMORY_READ_START(dmd16_readmem){0x0000, 0x3fff, MRA_ROM}, /* Z80 ROM CODE*/
    {0x4000, 0x7fff, MRA_BANKNO(DMD16_BANK0)},                    /* ROM BANK*/
    {0x8000, 0x9fff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(dmd16_writemem){0x0000, 0x3fff, MWA_ROM},
    {0x4000, 0x7fff, MWA_ROM}, {0x8000, 0x9fff, MWA_RAM},
    MEMORY_END

    static PORT_READ_START(dmd16_readport){0x00, 0xff, dmd16_port_r},
    PORT_END

    static PORT_WRITE_START(dmd16_writeport){0x00, 0xff, dmd16_port_w},
    PORT_END

    MACHINE_DRIVER_START(de_dmd16) MDRV_CPU_ADD(Z80, 4000000) MDRV_CPU_MEMORY(dmd16_readmem, dmd16_writemem)
        MDRV_CPU_PORTS(dmd16_readport, dmd16_writeport) MDRV_CPU_PERIODIC_INT(dmd16_nmi, DMD16_NMIFREQ)
            MDRV_INTERLEAVE(50) MACHINE_DRIVER_END
...
static void
dmd16_init(struct sndbrdData* brdData) {
    memset(&dmdlocals, 0, sizeof(dmdlocals));
    dmdlocals.brdData = *brdData;
...
static READ_HANDLER(dmd16_port_r) {
    if ((offset & 0x84) == 0x80) {
        dmd16_setbusy(BUSY_CLR, 0);
        dmd16_setbusy(BUSY_CLR, 1);
        return dmdlocals.cmd;
    }
...
dmd16_dmdout(void) {
    ...
    if (dmdlocals.hv5222 == 0)
        dmdlocals.frame = !dmdlocals.frame;
    frame = &dmdlocals.framedata[dmdlocals.frame * 0x80];

    while (row) {
        if (row & 0x01) {
            frame[0] = dmdlocals.hv5408;
            frame[1] = dmdlocals.hv5308;
...
static WRITE_HANDLER(dmd16_port_w) {
    switch (offset & 0x84) {
        case 0x00: // COCLK
            ...
            break;
        case 0x04: // CLATCH
            dmdlocals.hv5408 = dmdlocals.hv5408s;
            dmdlocals.hv5308 = dmdlocals.hv5308s;
            if (dmdlocals.blnk)
                dmd16_dmdout();
            break;
        case 0x80:
            break; // IDAT (ignored on write)
        case 0x84:
            data &= 0x01;
            switch (offset & 0xdc) {
                case 0x84: // Bank0
                    dmd16_setbank(0x01, !data);
                    break;
                case 0x8c: // Bank1
                    dmd16_setbank(0x02, !data);
                    break;
                case 0x94: // Bank2
                    dmd16_setbank(0x04, !data);
                    break;
                case 0xc4: // Status
                    sndbrd_ctrl_cb(dmdlocals.brdData.boardNo, dmdlocals.status = data);
                    break;
                case 0xdc: // Test
                    dmd16_setbusy(BUSY_SET, data);
                    break;
                case 0x9c: // Blanking
                    dmdlocals.blnk = data;
                    if (data)
...
static WRITE_HANDLER(dmd16_ctrl_w) {
    ...
    if (~data & dmdlocals.ctrl & 0x02) {
        ...
        dmd16_setbank(0x07, 0x07);
        dmd16_setbusy(BUSY_SET, 0);
        dmdlocals.rowdata = dmdlocals.blnk = 0;
        cpu_set_reset_line(dmdlocals.brdData.cpuNo, PULSE_LINE);
        dmdlocals.hv5408 = dmdlocals.hv5408s = dmdlocals.hv5308 = dmdlocals.hv5308s = dmdlocals.hv5222 =
            dmdlocals.lasthv5222 = dmdlocals.rowclk = dmdlocals.frame = 0;
    }
    dmdlocals.ctrl = data;
}

static void
dmd16_setbusy(int bit, int value) {
  ...
  */
    if (~newstat & laststat & BUSY_CLR)
        dmdlocals.busy = 0;
    else if (~newstat & laststat & BUSY_SET)
        dmdlocals.busy = 1;
    else if ((newstat & (BUSY_CLR | BUSY_SET)) == (BUSY_CLR | BUSY_SET)) {
        if (newstat & ~laststat & BUSY_CLK)
            dmdlocals.busy = 1;
    }
#else
    switch (newstat & 0x03) {
...
static void
dmd16_setbank(int bit, int value) {
