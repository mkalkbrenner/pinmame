...
static MACHINE_RESET(MAC) { memset(&locals, 0x00, sizeof(locals)); }
...
static WRITE_HANDLER(ay8910_0_portb_w) { coreGlobals.solenoids = (coreGlobals.solenoids & 0xfff00) | (data ^ 0xff); }
...
struct AY8910interface MAC_ay8910Int = {
    ...
    1900000,  /* 2 MHz */
    {30, 50}, /* Volume */
    {0, 0},   {0, ay8910_1_portb_r}, {ay8910_0_porta_w, ay8910_1_porta_w}, {ay8910_0_portb_w, ay8910_1_portb_w},
};
...
static READ_HANDLER(i8279_r) {
    ...
    if ((locals.i8279cmd & 0xe0) == 0x40) {
        ...
        if (row < 2)
            locals.i8279data = core_getDip(row); // read dips
        else
            locals.i8279data = coreGlobals.swMatrix[row - 1]; // read switches
    } else if ((locals.i8279cmd & 0xe0) == 0x60)
        locals.i8279data = locals.i8279ram[locals.i8279reg]; // read display ram
    else
        logerror("i8279 r:%02x\n", locals.i8279cmd);
    if (locals.i8279cmd & 0x10)
        locals.i8279reg = (locals.i8279reg + 1) % 16; // auto-increase if register is set
...
static WRITE_HANDLER(i8279_w) {
    if (offset) { // command
        ...
        if ((locals.i8279cmd & 0xe0) == 0x40)
            ...
            printf("i8279 w%d:%02x\n", offset, data);
#endif
        if (locals.i8279cmd & 0x10)
            locals.i8279reg = data & 0x0f;      // reset data for auto-increment
    } else {                                    // data
...
static MEMORY_READ_START(mac_readmem){0x0000, 0x7fff, MRA_ROM}, {0xc000, 0xc7ff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(mac_writemem){0x0000, 0x7fff, MWA_NOP},
    {0xc000, 0xc7ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static PORT_READ_START(mac_readport){0x09, 0x09, ay8910_0_r},
    {0x29, 0x29, ay8910_1_r}, {0x40, 0x40, i8279_r},
    PORT_END

    static PORT_WRITE_START(mac_writeport){0x08, 0x08, ay8910_0_ctrl_w},
    {0x0a, 0x0a, ay8910_0_data_w}, {0x28, 0x28, ay8910_1_ctrl_w}, {0x2a, 0x2a, ay8910_1_data_w}, {0x40, 0x41, i8279_w},
    {0x60, 0x61, msm_w},
    PORT_END

    static MACHINE_DRIVER_START(mac) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", Z80, 3800000)
        MDRV_CPU_MEMORY(mac_readmem, mac_writemem) MDRV_CPU_PORTS(mac_readport, mac_writeport)
            MDRV_CPU_VBLANK_INT(mac_vblank, 1) MDRV_TIMER_ADD(mac_nmi, 120) MDRV_CORE_INIT_RESET_STOP(NULL, MAC, NULL)
                MDRV_DIPS(16) MDRV_SWITCH_UPDATE(MAC) MDRV_NVRAM_HANDLER(generic_0fill)
                    MDRV_SOUND_ADD(AY8910, MAC_ay8910Int) MACHINE_DRIVER_END

    static MEMORY_READ_START(mac0_readmem){0x0000, 0x3fff, MRA_ROM},
    {0x4000, 0x47ff, MRA_RAM}, {0x6000, 0x6000, i8279_r},
    MEMORY_END

    static MEMORY_WRITE_START(mac0_writemem){0x0000, 0x3fff, MWA_NOP},
    {0x4000, 0x47ff, MWA_RAM, &generic_nvram, &generic_nvram_size}, {0x6000, 0x6001, i8279_w},
    MEMORY_END

    static PORT_READ_START(mac0_readport){0x09, 0x09, ay8910_0_r},
    {0x19, 0x19, ay8910_1_r},
    PORT_END

    static PORT_WRITE_START(mac0_writeport){0x08, 0x08, ay8910_0_ctrl_w},
    {0x0a, 0x0a, ay8910_0_data_w}, {0x18, 0x18, ay8910_1_ctrl_w}, {0x1a, 0x1a, ay8910_1_data_w},
    PORT_END

    MACHINE_DRIVER_START(mac0) MDRV_IMPORT_FROM(mac) MDRV_CPU_MODIFY("mcpu")
        MDRV_CPU_MEMORY(mac0_readmem, mac0_writemem) MDRV_CPU_PORTS(mac0_readport, mac0_writeport) MACHINE_DRIVER_END
...
#define MAC_COMPORTS(game, balls)                                                                                      \
    ...
    SIM_PORTS(balls)                                                                                                   \
    PORT_START /* 0 */                                                                                                 \
        COREPORT_BIT(0x0001, "Start", KEYCODE_1) COREPORT_BITIMP(0x0002, "Coin 1", KEYCODE_3) COREPORT_BITIMP(         \
            0x0004, "Coin 2", KEYCODE_5) COREPORT_BIT(0x0020, "Coin 3", KEYCODE_9) COREPORT_BIT(0x0008, "Tilt",        \
                                                                                                KEYCODE_INSERT)        \
            COREPORT_BIT(0x0010, "Slam Tilt", KEYCODE_HOME) COREPORT_BIT(0x0040, "Reset RAM", KEYCODE_0) COREPORT_BIT( \
                0x4000, "SW1", KEYCODE_DEL) COREPORT_BITTOG(0x8000, "40V Line", KEYCODE_END) PORT_START /* 1 */        \
                COREPORT_DIPNAME(0x0001, 0x0001, "Balls per Game") COREPORT_DIPSET(0x0001, "3") COREPORT_DIPSET(       \
                    0x0000, "5") COREPORT_DIPNAME(0x0006, 0x0006, "Credits f. sm. / big Coins")                        \
                    COREPORT_DIPSET(0x0000, "1/2 / 3") COREPORT_DIPSET(0x0002, "4/6 / 4") COREPORT_DIPSET(             \
                        0x0006, "5/4 / 5") COREPORT_DIPSET(0x0004, "3/2 / 6") COREPORT_DIPNAME(0x0008, 0x0000,         \
                                                                                               "3-Ball Multiball")     \
                        COREPORT_DIPSET(0x0008, DEF_STR(Off)) COREPORT_DIPSET(0x0000, DEF_STR(On)) COREPORT_DIPNAME(   \
                            0x0010, 0x0000,                                                                            \
                            DEF_STR(Unused)) COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x0010,             \
                                                                                                   DEF_STR(On))        \
                            COREPORT_DIPNAME(0x0020, 0x0020, "In-Game Music") COREPORT_DIPSET(                         \
                                0x0000,                                                                                \
                                DEF_STR(                                                                               \
                                    No)) COREPORT_DIPSET(0x0020,                                                       \
                                                         DEF_STR(                                                      \
                                                             Yes)) COREPORT_DIPNAME(0x0040, 0x0040, "Attract Sound")   \
                                COREPORT_DIPSET(0x0000, DEF_STR(No)) COREPORT_DIPSET(                                  \
                                    0x0040,                                                                            \
                                    DEF_STR(                                                                           \
                                        Yes)) COREPORT_DIPNAME(0x0080, 0x0000,                                         \
                                                               "Unused, always off") COREPORT_DIPSET(0x0000,           \
                                                                                                     DEF_STR(Off))     \
                                    COREPORT_DIPNAME(0x0f00, 0x0600, "Ball/RP1/2/HSTD") COREPORT_DIPSET(               \
                                        0x0000,                                                                        \
                                        "300K/450K/630K/1M") COREPORT_DIPSET(0x0100, "325K/500K/690K/1.1M")            \
                                        COREPORT_DIPSET(0x0200, "350K/570K/740K/1.15M") COREPORT_DIPSET(               \
                                            0x0300,                                                                    \
                                            "375K/610K/790K/1.25M") COREPORT_DIPSET(0x0400, "400K/650K/850K/1.35M")    \
                                            COREPORT_DIPSET(0x0500, "425K/690K/900K/1.4M") COREPORT_DIPSET(            \
                                                0x0600,                                                                \
                                                "450K/730K/950K/1.5M") COREPORT_DIPSET(0x0700,                         \
                                                                                       "500K/810K/1.05M/1.65M")        \
                                                COREPORT_DIPSET(0x0800, "550K/890K/1.15M/1.8M") COREPORT_DIPSET(       \
                                                    0x0900,                                                            \
                                                    "600K/950K/1.25M/2M") COREPORT_DIPSET(0x0a00,                      \
                                                                                          "650K/1.05M/1.35M/2.15M")    \
                                                    COREPORT_DIPSET(0x0b00, "700K/1.15M/1.5M/2.3M") COREPORT_DIPSET(   \
                                                        0x0c00,                                                        \
                                                        "750K/1.25M/1.6M/2.5M") COREPORT_DIPSET(0x0d00, "800K/1.3M/"   \
                                                                                                        "1.7M/2.7M")   \
                                                        COREPORT_DIPSET(0x0e00, "900K/1.45M/1.9M/3M") COREPORT_DIPSET( \
                                                            0x0f00,                                                    \
                                                            "1M/1.65M/2.15M/3.3M")                                     \
                                                            COREPORT_DIPNAME(0x1000, 0x0000, "Auto-adjust")            \
                                                                COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET( \
                                                                    0x1000,                                            \
                                                                    DEF_STR(                                           \
                                                                        On)) COREPORT_DIPNAME(0x2000, 0x2000,          \
                                                                                              "Multiple Extra Balls")  \
                                                                    COREPORT_DIPSET(                                   \
                                                                        0x0000,                                        \
                                                                        DEF_STR(Off)) COREPORT_DIPSET(0x2000,          \
                                                                                                      DEF_STR(On))     \
                                                                        COREPORT_DIPNAME(0x4000, 0x4000,               \
                                                                                         "Multiple Replays")           \
...
static core_tLCDLayout dispMAC[] = {{0, 0, 0, 6, CORE_SEG8},   {0, 16, 16, 6, CORE_SEG8},
                                    {3, 0, 6, 6, CORE_SEG8},   {3, 16, 22, 6, CORE_SEG8},
                                    {6, 0, 15, 1, CORE_SEG8},  {6, 4, 14, 1, CORE_SEG8},
                                    {6, 8, 13, 1, CORE_SEG8},  {6, 10, 12, 1, CORE_SEG8},
                                    {6, 16, 28, 4, CORE_SEG8}, {6, 24, 31, 1, CORE_SEG8},
                                    {6, 26, 31, 1, CORE_SEG8}, {6, 2, 32, 1, CORE_SEG8},
                                    {6, 6, 32, 1, CORE_SEG8},  {0}};
        ...
        if ((locals.i8279cmd & 0xe0) == 0x80) { // write display ram
            ...
            if (locals.i8279reg == 6 || locals.i8279reg > 12) {
                ...
                if (locals.i8279reg == 0 || locals.i8279reg == 3 || locals.i8279reg == 7 || locals.i8279reg == 10) {
                    if (coreGlobals.segments[locals.i8279reg].w)
                        coreGlobals.segments[locals.i8279reg].w |= 0x80;
                    if (coreGlobals.segments[16 + locals.i8279reg].w)
                        coreGlobals.segments[16 + locals.i8279reg].w |= 0x80;
                }
            }
...
static MEMORY_WRITE_START(cic_writemem){0x0000, 0x3fff, MWA_NOP},
    {0x4000, 0x47ff, MWA_RAM, &generic_nvram, &generic_nvram_size}, {0x6000, 0x6001, i8279_w_cic},
    MEMORY_END

    MACHINE_DRIVER_START(cic) MDRV_IMPORT_FROM(mac) MDRV_CPU_MODIFY("mcpu") MDRV_CPU_MEMORY(mac0_readmem, cic_writemem)
        MDRV_CPU_PORTS(mac0_readport, mac0_writeport) MDRV_CORE_INIT_RESET_STOP(NULL, CIC, NULL) MACHINE_DRIVER_END
...
#define CIC_COMPORTS(game, balls)                                                                                      \
    ...
    PORT_START /* 0 */                                                                                                 \
        COREPORT_BIT(0x0001, "Start", KEYCODE_1) COREPORT_BITIMP(0x0002, "Coin 1", KEYCODE_3) COREPORT_BITIMP(         \
            0x0004, "Coin 2", KEYCODE_5) COREPORT_BIT(0x0010, "Tilt", KEYCODE_INSERT)                                  \
            COREPORT_BIT(0x0008, "Slam Tilt", KEYCODE_HOME) COREPORT_BIT(0x4000, "SW1", KEYCODE_DEL) COREPORT_BITTOG(  \
                0x8000, "40V Line", KEYCODE_END) PORT_START /* 1 */                                                    \
                COREPORT_DIPNAME(0x0001, 0x0001, "Balls per Game") COREPORT_DIPSET(0x0001, "3") COREPORT_DIPSET(       \
                    0x0000, "5") COREPORT_DIPNAME(0x0006, 0x0006, "Credits f. sm. / big Coins")                        \
                    COREPORT_DIPSET(0x0006, "5/4 / 5") COREPORT_DIPSET(0x0004, "3/2 / 6") COREPORT_DIPSET(             \
                        0x0002, "9/4 / 9") COREPORT_DIPSET(0x0000, "5/2 / 10") COREPORT_DIPNAME(0x0008, 0x0000,        \
                                                                                                "Arrows reward")       \
                        COREPORT_DIPSET(0x0000, "30K") COREPORT_DIPSET(0x0008, "50K") COREPORT_DIPNAME(                \
                            0x0030, 0x0010, "Replay 1/2 thresholds") COREPORT_DIPSET(0x0000, "500K/650K")              \
                            COREPORT_DIPSET(0x0020, "500K/750K") COREPORT_DIPSET(0x0010, "550K/750K") COREPORT_DIPSET( \
                                0x0030, "600K/800K") COREPORT_DIPNAME(0x0040, 0x0000, DEF_STR(Unknown))                \
                                COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x0040, DEF_STR(On))             \
                                    COREPORT_DIPNAME(0x0080, 0x0000, "Unused, always off") COREPORT_DIPSET(            \
                                        0x0000, DEF_STR(Off)) COREPORT_DIPNAME(0x0300, 0x0000, "Extra Ball threshold") \
                                        COREPORT_DIPSET(0x0000, "350K") COREPORT_DIPSET(                               \
                                            0x0200, "400K") COREPORT_DIPSET(0x0100, "450K") COREPORT_DIPSET(0x0300,    \
                                                                                                            "-")       \
                                            COREPORT_DIPNAME(0x0400, 0x0000, DEF_STR(Unknown)) COREPORT_DIPSET(        \
                                                0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x0400, DEF_STR(On))             \
                                                COREPORT_DIPNAME(0x0800, 0x0000, DEF_STR(Unknown)) COREPORT_DIPSET(    \
                                                    0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x0800, DEF_STR(On))         \
                                                    COREPORT_DIPNAME(0x1000, 0x0000, DEF_STR(Unknown))                 \
                                                        COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(         \
                                                            0x1000,                                                    \
                                                            DEF_STR(On)) COREPORT_DIPNAME(0x2000, 0x0000,              \
                                                                                          DEF_STR(Unknown))            \
                                                            COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(     \
                                                                0x2000, DEF_STR(On))                                   \
    ...
    static core_tLCDLayout dispCIC[] = {
        {0, 0, 0, 1, CORE_SEG10},  {0, 2, 1, 2, CORE_SEG9},    {0, 6, 3, 1, CORE_SEG10},   {0, 8, 4, 2, CORE_SEG9},
        {0, 12, 32, 1, CORE_SEG9}, {0, 16, 16, 1, CORE_SEG10}, {0, 18, 17, 2, CORE_SEG9},  {0, 22, 19, 1, CORE_SEG10},
        {0, 24, 20, 2, CORE_SEG9}, {0, 28, 32, 1, CORE_SEG9},  {3, 0, 7, 1, CORE_SEG10},   {3, 2, 8, 2, CORE_SEG9},
        {3, 6, 10, 1, CORE_SEG10}, {3, 8, 11, 2, CORE_SEG9},   {3, 12, 32, 1, CORE_SEG9},  {3, 16, 23, 1, CORE_SEG10},
        {3, 18, 24, 2, CORE_SEG9}, {3, 22, 26, 1, CORE_SEG10}, {3, 24, 27, 2, CORE_SEG9},  {3, 28, 32, 1, CORE_SEG9},
        {6, 0, 14, 2, CORE_SEG7S}, {6, 6, 6, 1, CORE_SEG7S},   {6, 10, 13, 1, CORE_SEG7S}, {0}};
