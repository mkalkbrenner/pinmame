...
static WRITE_HANDLER(port4_w) { logerror("%04x: p4 write:%02x\n", activecpu_get_previouspc(), data); }
    ...
    if (data < 0x04) {
        if (core_gameData->hw.gameSpecific1) { // FG solenoids 15 & 16
            ...
            coreGlobals.pulsedSolState =
                (coreGlobals.pulsedSolState & 0xffffe7ff) | ((data & 0x01) << 12) | ((data & 0x02) << 10);
        }
    } else
...
static WRITE_HANDLER(pp1_a_w) { logerror("%04x: PIA 1 A WRITE = %02x\n", activecpu_get_previouspc(), data); }
...
static WRITE_HANDLER(pp1_b_w) { // periphal data
    static int solOrder[2][4][4] = {{
                                    ...
                                    {{0, 2, 5, 1}, {9, 3, 7, 16}, {11, 13, 10, 17}, {8, 12, 4, 6}}};
    ...
    if (locals.lampCol != 0x0f) {
        ...
        if (locals.solEnable && countBits(0x0f ^ (data >> 4)) == 1 && countBits(0x0f ^ (data & 0x0f)) == 1) {
            UINT8 solData = ~data;
            locals.solenoids |= 1 << solOrder[core_gameData->hw.gameSpecific1][core_BitColToNum(solData & 0x0f)]
                                             [core_BitColToNum(solData >> 4)];
            locals.solEnable = 0;
        }
...
static INTERRUPT_GEN(by68701_irq) { pia_set_input_cb1(BY68701_PIA0, locals.irq = !locals.irq); }

static void
by68701_zeroCross(int data) {
    locals.zc = !locals.zc;
...
static MACHINE_STOP(by68701) { sndbrd_0_exit(); }
...
static WRITE_HANDLER(by68701_m0800_w) {
    ...
    switch (offset) {
        ...
        case 5:
        case 6:
        case 7: // player display data
            num = (offset - 4) * 8 + 7 - digit;
            locals.segments[num].w = (data & 0x7f) | ((data & 0x80) << 1) | ((data & 0x80) << 2);
            if (locals.commas && locals.segments[num].w && (num % 8 == 1 || num % 8 == 4))
                locals.segments[num].w |= 0x80;
            break;
...
static PORT_READ_START(by68701_readport){M6803_PORT1, M6803_PORT2, MRA_NOP},
    PORT_END

    static PORT_WRITE_START(by68701_writeport){M6803_PORT1, M6803_PORT1, port1_w},
    {M6803_PORT2, M6803_PORT2, port2_w}, {M6803_PORT4, M6803_PORT4, port4_w},
    PORT_END
...
*/
    ...
    {0x3000, 0x301f, by68701_m3000_r}, {0x7000, 0xffff, MRA_ROM}, /*ROM */
    MEMORY_END
...
*/
    ...
    MACHINE_DRIVER_START(by68701_61S) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(by68701, by68701, by68701)
        MDRV_CPU_ADD_TAG("mcpu", M6803, 3579545. / 4.) MDRV_CPU_MEMORY(by68701_readmem, by68701_writemem)
            MDRV_CPU_PORTS(by68701_readport, by68701_writeport) MDRV_CPU_VBLANK_INT(by68701_vblank, 1)
                MDRV_CPU_PERIODIC_INT(by68701_irq, 4000) MDRV_TIMER_ADD(by68701_zeroCross, 120)
                    MDRV_NVRAM_HANDLER(generic_0fill) MDRV_SWITCH_UPDATE(by68701)
                        MDRV_DIPS(1) // needed for extra inports
    MDRV_DIAGNOSTIC_LEDH(1) MDRV_IMPORT_FROM(by61) MACHINE_DRIVER_END

#define BY68701_INPUT_PORTS_START(name)                                                                                \
    INPUT_PORTS_START(name)                                                                                            \
    CORE_PORTS                                                                                                         \
    SIM_PORTS(1)                                                                                                       \
    PORT_START /* 2 */ /* Switch Column 0 */                                                                           \
        COREPORT_BIT(0x0010, "CPU diagnostics", KEYCODE_9)                                                             \
            COREPORT_BIT(0x0020, "Sound diagnostics", KEYCODE_0) /* Switch Column 4 */                                 \
        COREPORT_BIT(0x0008, "KP: Game", KEYCODE_6) COREPORT_BIT(0x0001, "KP: Enter", KEYCODE_7) COREPORT_BIT(         \
            0x0004, "KP: Clear", KEYCODE_8) COREPORT_BIT(0x0002, "KP: 0", KEYCODE_0_PAD)                               \
            PORT_START /* 3 */ /* Switch Columns 5 to 8 */                                                             \
                COREPORT_BIT(0x0004, "KP: 1", KEYCODE_1_PAD) COREPORT_BIT(0x0002, "KP: 2", KEYCODE_2_PAD)              \
                    COREPORT_BIT(0x0001, "KP: 3", KEYCODE_3_PAD) COREPORT_BIT(0x0040, "KP: 4", KEYCODE_4_PAD)          \
                        COREPORT_BIT(0x0020, "KP: 5", KEYCODE_5_PAD) COREPORT_BIT(0x0010, "KP: 6", KEYCODE_6_PAD)      \
                            COREPORT_BIT(0x0400, "KP: 7", KEYCODE_7_PAD) COREPORT_BIT(0x0200, "KP: 8", KEYCODE_8_PAD)  \
                                COREPORT_BIT(0x0100, "KP: 9", KEYCODE_9_PAD) COREPORT_BIT(0x0008, "KP: A", KEYCODE_V)  \
                                    COREPORT_BIT(0x0080, "KP: B", KEYCODE_B) COREPORT_BIT(0x0800, "KP: C", KEYCODE_C)  \
    ...
    static const core_tLCDLayout dispFG[] = {{0, 0, 1, 7, CORE_SEG98},
                                             {0, 18, 9, 7, CORE_SEG98},
                                             {6, 0, 17, 7, CORE_SEG98},
                                             {6, 18, 25, 7, CORE_SEG98},
                                             {3, 24, 35, 1, CORE_SEG9},
                                             {3, 26, 36, 1, CORE_SEG10},
                                             {3, 30, 38, 2, CORE_SEG9},
                                             {3, 20, 33, 1, CORE_SEG10},
                                             {3, 22, 34, 1, CORE_SEG9},
                                             {3, 28, 37, 1, CORE_SEG9},
                                             {0}};

static const core_tLCDLayout dispEBD[] = {{0, 0, 1, 7, CORE_SEG98},
                                          {0, 18, 9, 7, CORE_SEG98},
                                          {6, 0, 17, 7, CORE_SEG98},
                                          {6, 18, 25, 7, CORE_SEG98},
                                          {3, 30, 35, 2, CORE_SEG7S},
                                          {3, 36, 38, 2, CORE_SEG7S},
                                          {0}};
...
BY68701_ROMSTART_CA8(flashgdp, "fg6801.bin", CRC(8af7bf77) SHA1(fd65578b2340eb207b2e197765e6721473340565) BAD_DUMP,
                     "xxx-xx.u10", CRC(3e9fb30f) SHA1(173cd9e55e9c954944aa504308564e4842646e55), "xxx-xx.u11",
                     CRC(8b0ae6d8) SHA1(2380bd6d354c204153fd44534d617f7be000e46f), "xxx-xx.u12",
                     CRC(57406a1f) SHA1(01986e8d879071374d6f94ae6fce5832eb89f160))
BY61_SOUNDROM0xx0("834-20_2.532", CRC(2f8ced3e) SHA1(ecdeb07c31c22ec313b55774f4358a9923c5e9e7), "834-18_5.532",
                  CRC(8799e80e) SHA1(f255b4e7964967c82cfc2de20ebe4b8d501e3cb0)) ROM_END
...
#define input_ports_flashgp2 input_ports_flashgdp
        BY68701_ROMSTART_CA8(flashgp2, "fg6801.bin",
                             CRC(8af7bf77) SHA1(fd65578b2340eb207b2e197765e6721473340565) BAD_DUMP, "xxx-xx2.u10",
                             CRC(0fff825c) SHA1(3c567aa8ec04a8ff9a09b530b6d324fdbe363ab6), "xxx-xx2.u11",
                             CRC(e34b113a) SHA1(b2860d284995db0ec59b22b434ccb9f6721e7d9d), "xxx-xx2.u12",
                             CRC(12bf4e19) SHA1(b0acf069d86f7472728610834ef3ab6da83b4b67))
            BY61_SOUNDROM0xx0("834-20_2.532", CRC(2f8ced3e) SHA1(ecdeb07c31c22ec313b55774f4358a9923c5e9e7),
                              "834-18_5.532", CRC(8799e80e) SHA1(f255b4e7964967c82cfc2de20ebe4b8d501e3cb0)) ROM_END
    CORE_CLONEDEFNV(flashgp2, flashgdn, "Flash Gordon (Prototype rev. 2)", 1981, "Bally", by68701_61S, 0)

    /*------------------
/ Eight Ball Deluxe
/------------------*/
    INITGAMEP(eballdp1, FLIP_SW(FLIP_L), SNDBRD_BY61, 0, dispEBD) BY68701_ROMSTART_DC7A(
        eballdp1, "ebd68701.1", CRC(2c693091) SHA1(93ae424d6a43424e8ea023ef555f6a4fcd06b32f),
        ...
        "720-61.u10", CRC(ac646e58) SHA1(85694264a739118ed249d97c04fe8e9f6edfdd33), "720-62.u14",
        CRC(b6476a9b) SHA1(1dc92125422908e829ce17aaed5ad49b0dbda0e5), "720-63.u13",
        CRC(f5d751fd) SHA1(4ab5975d52cdde0e05f2bbea7dcd732882fb1dd5), "838-19.u12",
        CRC(dfba8976) SHA1(70c19237842ee73cfc8e9607df79a59424d90d99))
        BY61_SOUNDROMx080("838-08_3.532", CRC(c39478d7) SHA1(8148aca7c4113921ab882da32d6d88e66abb22cc), "838-09_4.716",
                          CRC(518ea89e) SHA1(a387274ef530bb57f31819733b35615a39260126), "838-16_5.532",
                          CRC(63d92025) SHA1(2f8e8435326a39064b99b9971b0d8944586571fb)) ROM_END
...
#define input_ports_eballdp2 input_ports_eballdp1
        BY68701_ROMSTART_DC7A(eballdp2, "ebd68701.1", CRC(2c693091) SHA1(93ae424d6a43424e8ea023ef555f6a4fcd06b32f),
                              "720-56.u10", CRC(65a3a02b) SHA1(6fa8667509d314f521dce63d9f1b7fc132d85a1f), "720-57.u14",
                              CRC(a7d96074) SHA1(04726af863a2c7589308725f3183112b5e1f84ac), "720-58.u13",
                              CRC(c9585f1f) SHA1(a38b059bb7ef15fccb54bec58d88dd15182b66a6), "838-18.u12",
            ...
            BY61_SOUNDROMx080("838-08_3.532", CRC(c39478d7) SHA1(8148aca7c4113921ab882da32d6d88e66abb22cc),
                              ...
                              "838-16_5.532", CRC(63d92025) SHA1(2f8e8435326a39064b99b9971b0d8944586571fb)) ROM_END
...
#define input_ports_eballdp3 input_ports_eballdp1
        BY68701_ROMSTART_DC7A(eballdp3, "ebd68701.1", CRC(2c693091) SHA1(93ae424d6a43424e8ea023ef555f6a4fcd06b32f),
                              "720-xx.u10", CRC(6da34581) SHA1(6e005ceda9a4a23603d5243dfca85ccd3f0e425a), "720-xx.u14",
                              CRC(7079648a) SHA1(9d91cd18fb68f165498de8ac51c1bc2a35bd9468), "xxx-xx.u13",
                              CRC(bda2c78b) SHA1(d5e7d0dd3d44d63b9d4b43bf5f63917b80a7ce23), "838-18.u12",
                              CRC(20fa35e5) SHA1(d8808aa357d2a20fc235da7c80f78c8e5d805ac3))
            BY61_SOUNDROMx080("838-08_3.532", CRC(c39478d7) SHA1(8148aca7c4113921ab882da32d6d88e66abb22cc),
                              "838-09_4.716", CRC(518ea89e) SHA1(a387274ef530bb57f31819733b35615a39260126),
                              "838-16_5.532", CRC(63d92025) SHA1(2f8e8435326a39064b99b9971b0d8944586571fb)) ROM_END
...
#define input_ports_eballdp4 input_ports_eballdp1
        BY68701_ROMSTART_DCA(eballdp4, "ebd68701.1", CRC(2c693091) SHA1(93ae424d6a43424e8ea023ef555f6a4fcd06b32f),
                             "720-54.u10", CRC(9facc547) SHA1(a10d7747918b3a1d87bd9caa19e87739631a7566), "720-55.u14",
                             CRC(99080832) SHA1(e1d416b4910ed31b40bde0860e698f0cbe46cc57), "838-17.u12",
                             CRC(43ebffc6) SHA1(bffd41c68430889b3926db9b05c5991185c28053))
            BY61_SOUNDROMx080("838-08_3.532", CRC(c39478d7) SHA1(8148aca7c4113921ab882da32d6d88e66abb22cc),
                              "838-09_4.716", CRC(518ea89e) SHA1(a387274ef530bb57f31819733b35615a39260126),
                              "838-16_5.532", CRC(63d92025) SHA1(2f8e8435326a39064b99b9971b0d8944586571fb)) ROM_END
