...
static int
ZAC_m2sw(int col, int row) {
    return col * 8 + row - 8;
}

static int
irq_callback_old(int int_level) {
    return core_getSw(-2) ? 0x10 : 0x18;
}

static void
timer_callback_old(int n) {
    cpu_set_irq_line(ZAC_CPUNO, 0, PULSE_LINE);
}

static int
irq_callback(int int_level) {
    return 0xbf;
}

static void
timer_callback(int n) {
    cpu_set_irq_line(ZAC_CPUNO, 0, PULSE_LINE);
}
...
static NVRAM_HANDLER(ZAC1) { core_nvram(file, read_or_write, ram1, 0x400, 0x00); }

static UINT8* ram;

static NVRAM_HANDLER(ZAC2) { core_nvram(file, read_or_write, ram, 0x800, 0x00); }
...
static MACHINE_INIT(ZAC1) {
    ...
    if (ram1[0xf7] != 0x05 && ram1[0xf8] != 0x0a) { // data is invalid
        ...
        for (i = 0xc1; i < 0xd6; i++)
            ram1[i] = 0x01; // enable match & coin slots
        ram1[0xf7] = 0x05;
        ram1[0xf8] = 0x0a; // validate data
    }
...
static READ_HANDLER(ctrl_port_r) {
    if (locals.gen == 1) {
        switch (locals.swCol) {
            case 0x80:
                return ~core_getDip(2); // 8 dip switches B
            case 0x40:
                return ~core_getDip(1); // 8 dip switches A
            default:
                return ~core_getSwCol(locals.swCol); // switch columns 1-6
        }
    } else {
...
static WRITE_HANDLER(flag_w) {
    ...
    if (bitno == 8 && startbit > 1)
        if (locals.printfile)
            mame_fwrite(locals.printfile, printdata, 1);
    if (bitno > 7) {
        printdata[0] = 0;
        bitno = 0;
        startbit = 0;
...
static READ_HANDLER(ram_r) { return ram[offset]; }
...
static WRITE_HANDLER(ram_w) {
    ...
    if (offset > 0x43f && offset < 0x460) {
        ...
        if (offset % 8 == 1 || offset % 8 == 4)
            locals.segments[offset].w |= 0x80;
    }
...
static READ_HANDLER(ram1_r) { return ram1[offset]; }
...
*/
static MEMORY_READ_START(ZAC_readmem){0x0000, 0x17ff, MRA_ROM}, {0x1800, 0x1bff, ram1_r}, /* RAM */
    {0x1c00, 0x1fff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(ZAC_writemem){0x0000, 0x17ff, MWA_ROM},
    {0x1800, 0x1bff, ram1_w, &ram1}, /* RAM */
    {0x1c00, 0x1fff, MWA_ROM},
    MEMORY_END

    /* bigger RAM area from 0x1800 to 0x1fff */
    static MEMORY_READ_START(ZAC_readmem2){0x0000, 0x17ff, MRA_ROM},
    {0x1800, 0x1fff, ram_r},                            /* RAM */
    {0x2000, 0x37ff, MRA_ROM}, {0x3800, 0x3fff, ram_r}, /* RAM Mirror */
    {0x4000, 0x57ff, MRA_ROM}, {0x5800, 0x5fff, ram_r}, /* RAM Mirror  */
    {0x6000, 0x77ff, MRA_ROM}, {0x7800, 0x7fff, ram_r}, /* RAM Mirror */
    MEMORY_END

    static MEMORY_WRITE_START(ZAC_writemem2){0x0000, 0x17ff, MWA_ROM},
    {0x1800, 0x1fff, ram_w},                                  /* RAM */
    {0x2000, 0x37ff, MWA_ROM}, {0x3800, 0x3fff, ram_w, &ram}, /* RAM Mirror */
    {0x4000, 0x57ff, MWA_ROM}, {0x5800, 0x5fff, ram_w},       /* RAM Mirror */
    {0x6000, 0x77ff, MWA_ROM}, {0x7800, 0x7fff, ram_w},       /* RAM Mirror */
    MEMORY_END

    /* bigger RAM area from 0x1400 to 0x1bff */
    static MEMORY_READ_START(ZAC_readmem0){0x0000, 0x13ff, MRA_ROM},
    {0x1400, 0x17ff, ram_r},  /* RAM */
    {0x1800, 0x1bff, ram1_r}, /* RAM */
    {0x1c00, 0x1fff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(ZAC_writemem0){0x0000, 0x13ff, MWA_ROM},
   ...
   FLAG WRITE = Write Serial Output (hooked to printer)
*/
    static PORT_WRITE_START(ZAC_writeport){S2650_CTRL_PORT, S2650_CTRL_PORT, ctrl_port_w},
    {S2650_DATA_PORT, S2650_DATA_PORT, data_port_w}, {S2650_SENSE_PORT, S2650_SENSE_PORT, flag_w},
    PORT_END

    static PORT_READ_START(ZAC_readport){S2650_CTRL_PORT, S2650_CTRL_PORT, ctrl_port_r},
    {S2650_DATA_PORT, S2650_DATA_PORT, data_port_r}, {S2650_SENSE_PORT, S2650_SENSE_PORT, sense_r},
    PORT_END

    MACHINE_DRIVER_START(ZAC) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(ZAC1, NULL, ZAC)
        MDRV_CPU_ADD_TAG("mcpu", S2650, 6000000. / 2.) MDRV_CPU_MEMORY(ZAC_readmem, ZAC_writemem)
            MDRV_CPU_PORTS(ZAC_readport, ZAC_writeport) MDRV_CPU_VBLANK_INT(ZAC_vblank_old, 1) MDRV_NVRAM_HANDLER(ZAC1)
                MDRV_DIPS(4 + 2 * 8) MDRV_SWITCH_UPDATE(ZAC1) MDRV_DIAGNOSTIC_LEDH(3)
                    MDRV_SWITCH_CONV(ZAC_sw2m, ZAC_m2sw) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(ZAC0) MDRV_IMPORT_FROM(ZAC) MDRV_CPU_MODIFY("mcpu")
        MDRV_CPU_MEMORY(ZAC_readmem0, ZAC_writemem0) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(ZAC1311) MDRV_IMPORT_FROM(ZAC0) MDRV_IMPORT_FROM(zac1311) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(ZAC1125) MDRV_IMPORT_FROM(ZAC0) MDRV_IMPORT_FROM(zac1125) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(ZAC1144) MDRV_IMPORT_FROM(ZAC0) MDRV_IMPORT_FROM(wmssnd_s67s) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(ZAC1346) MDRV_IMPORT_FROM(ZAC0) MDRV_IMPORT_FROM(zac1346) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(ZAC1146) MDRV_IMPORT_FROM(ZAC) MDRV_IMPORT_FROM(zac1146) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(ZAC2) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(ZAC2, NULL, ZAC)
        MDRV_CPU_ADD_TAG("mcpu", S2650, 6000000. / 4.) MDRV_CPU_MEMORY(ZAC_readmem2, ZAC_writemem2)
            MDRV_CPU_PORTS(ZAC_readport, ZAC_writeport) MDRV_CPU_VBLANK_INT(ZAC_vblank, 1) MDRV_NVRAM_HANDLER(ZAC2)
                MDRV_DIPS(4) MDRV_SWITCH_UPDATE(ZAC2) MDRV_DIAGNOSTIC_LEDH(3)
                    MDRV_SWITCH_CONV(ZAC_sw2m, ZAC_m2sw) MACHINE_DRIVER_END

    //Sound board 1370
    MACHINE_DRIVER_START(ZAC2A) MDRV_IMPORT_FROM(ZAC2) MDRV_IMPORT_FROM(zac1370) MACHINE_DRIVER_END

    //Sound board 13136
    MACHINE_DRIVER_START(ZAC2X) MDRV_IMPORT_FROM(ZAC2) MDRV_DIAGNOSTIC_LEDH(3)
        MDRV_IMPORT_FROM(zac13136) MACHINE_DRIVER_END

    //Sound board 11178
    MACHINE_DRIVER_START(ZAC2XS) MDRV_IMPORT_FROM(ZAC2) MDRV_IMPORT_FROM(zac11178) MACHINE_DRIVER_END

    //Sound boards 11178 + 13181
    MACHINE_DRIVER_START(ZAC2XS2) MDRV_IMPORT_FROM(ZAC2) MDRV_IMPORT_FROM(zac11178_13181) MACHINE_DRIVER_END

    //Sound boards 11178 + 11181
    MACHINE_DRIVER_START(ZAC2XS2A) MDRV_IMPORT_FROM(ZAC2) MDRV_IMPORT_FROM(zac11178_11181) MACHINE_DRIVER_END

    //Sound board 11183
    MACHINE_DRIVER_START(ZAC2XS3) MDRV_IMPORT_FROM(ZAC2) MDRV_IMPORT_FROM(zac11183) MACHINE_DRIVER_END

    //Technoplay sound board
    MACHINE_DRIVER_START(TECHNO) MDRV_IMPORT_FROM(ZAC2) MDRV_IMPORT_FROM(techno) MACHINE_DRIVER_END
