...
static INTERRUPT_GEN(IDSA_irq) { cpu_set_irq_line(0, 0, PULSE_LINE); }
...
idsa2seg7(UINT8 data) {
    switch (data & 0x1f) {
        ...
        case 0x14:
            return 0x38; // L !
        case 0x15:
            return 0x56; // m ? pretty much impossible with 7 segs...
        case 0x16:
            return 0x54; // n !
        case 0x17:
            return 0x55; // n with tilde ?
        case 0x18:
            return 0x5c; // o !
        case 0x19:
            return 0x73; // P !
        ...
        case 0x1f:
            return 0;
        default:
            return core_bcd2seg7[data & 0x0f];
    }
}
...
static void
lrq_callback(int state) {
    locals.lrq = !state;
}
struct sp0256_interface IDSA_sp0256Int = {100,     /* volume */
                                          3120000, /* clock */
                                          lrq_callback, NULL, REGION_SOUND1};

static READ_HANDLER(sw_r) { return ~coreGlobals.swMatrix[1 + (offset >> 4)]; }

static READ_HANDLER(dip_r) { return ~core_getDip(offset >> 4); }

static READ_HANDLER(port_bd_r) { return 0x01 ^ coreGlobals.swMatrix[0]; }

static WRITE_HANDLER(row_w) { locals.dispRow++; }
...
static NVRAM_HANDLER(IDSA) { core_nvram(file, read_or_write, IDSA_CMOS, 0x800, 0x00); }
...
static READ_HANDLER(IDSA_CMOS_r) { return IDSA_CMOS[offset]; }

static MEMORY_READ_START(IDSA_readmem){0x0000, 0x7fff, MRA_ROM}, {0x8000, 0x87ff, IDSA_CMOS_r},
    MEMORY_END

    static MEMORY_WRITE_START(IDSA_writemem){0x0000, 0x7fff, MWA_NOP},
    {0x8000, 0x87ff, IDSA_CMOS_w, &IDSA_CMOS},
    MEMORY_END

    static PORT_READ_START(IDSA_readport){0x00, 0x50, sw_r},
    {0x60, 0x70, dip_r}, {0xb0, 0xb3, snd_status_r}, {0xbd, 0xbd, port_bd_r},
    MEMORY_END

    static PORT_WRITE_START(IDSA_writeport){0x80, 0x8f, row_w},
    {0x90, 0x90, disp_w}, {0xd0, 0xd0, sp0256_ALD_w}, {0xe0, 0xe0, AY8910_control_port_0_w},
    {0xe1, 0xe1, AY8910_write_port_0_w}, {0xf0, 0xf0, AY8910_control_port_1_w}, {0xf1, 0xf1, AY8910_write_port_1_w},
    MEMORY_END
...
static MACHINE_RESET(IDSA) { reset_common(); }
...
SIM_PORTS(1)
PORT_START /* 0 */
    COREPORT_BIT(0x0040, "Start", KEYCODE_1) COREPORT_BIT(0x0020, "Coin 1", KEYCODE_3)
        COREPORT_BIT(0x0010, "Coin 2", KEYCODE_4) COREPORT_BIT(0x0008, "Coin 3", KEYCODE_5)
            COREPORT_BIT(0x0200, "Tilt", KEYCODE_INSERT)
                COREPORT_BIT(0x1000, "Slam Tilt", KEYCODE_HOME) PORT_START /* 1 */
    ...
    core_tLCDLayout v1_disp[] = {{0, 0, 20, 6, CORE_SEG7}, {0, 20, 14, 6, CORE_SEG7}, {3, 0, 8, 6, CORE_SEG7},
                                 {3, 20, 2, 6, CORE_SEG7}, {3, 14, 0, 2, CORE_SEG7},  {0}};
...
core_tLCDLayout idsa_disp[] = {{0, 0, 0, 7, CORE_SEG7},   {0, 16, 7, 7, CORE_SEG7},
                               {3, 0, 16, 7, CORE_SEG7},  {3, 16, 23, 7, CORE_SEG7},
                               {6, 6, 36, 1, CORE_SEG7},  {6, 10, 38, 2, CORE_SEG7},
                               {6, 16, 41, 1, CORE_SEG7}, {6, 20, 43, 2, CORE_SEG7},
                               {9, 0, 48, 2, CORE_SEG7},  {0}};
