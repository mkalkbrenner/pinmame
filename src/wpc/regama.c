...
static WRITE_HANDLER(riot_porta_w) {
    ...
    if (data == 0x0a)
        coreGlobals.diagnosticLed = 2; // indicates fault, CPU is looped until reset or NMI
            ...
            case 2:
                if (locals.bitCnt > 15) {
                    strobe =
                        (latch & 0x1000)
                            ? 1
                            : 0; // this is probably how additional displays are accessed, needs other games to confirm
...
static i8155_interface i8155_intf = {
    1, {0}, {0}, {0}, {riot_porta_w}, {riot_portb_w}, {riot_portc_w}, {0} // {riot_timer_w}
};
...
static MACHINE_RESET(regama) {
    i8155_reset(0);
    AY8910_reset(0);
    timer_set(TIME_IN_USEC(1), 0,
              install_sod_cb); // needs to be installed after reset is through because setting CPU context overrides it!
}
...
static SWITCH_UPDATE(regama) {
    ...
    cpu_set_irq_line(0, I8085_INTR_LINE,
                     coreGlobals.swMatrix[7] & 0x40 ? ASSERT_LINE : CLEAR_LINE);    // INTR stalls the machine
...
static MEMORY_READ_START(readmem){0x0000, 0x7fff, MRA_ROM}, {0x8000, 0x80ff, MRA_RAM}, {0x8800, 0x88ff, MRA_RAM},
    {0x8900, 0x8907, i8155_0_r}, {0x9000, 0x9000, AY8910_read_port_0_r},
    MEMORY_END

    static MEMORY_WRITE_START(writemem){0x0000, 0x7fff, MWA_NOP},
    {0x8000, 0x80ff, MWA_RAM, &generic_nvram, &generic_nvram_size}, {0x8800, 0x88ff, MWA_RAM},
    {0x8900, 0x8907, i8155_0_w}, {0x9001, 0x9001, AY8910_control_port_0_w}, {0x9002, 0x9002, AY8910_write_port_0_w},
    MEMORY_END
...
static READ_HANDLER(ay8910_portb_r) { return ~coreGlobals.swMatrix[1 + locals.swCol]; }

static struct AY8910interface ay8910Int = {
    1, 6144000 / 2, {30}, {ay8910_porta_r}, {ay8910_portb_r},
};
...
SIM_PORTS(1)
PORT_START /* 0 */
    COREPORT_BIT(0x0200, "Coin #1", KEYCODE_3) COREPORT_BIT(0x0400, "Coin #2", KEYCODE_5)
        COREPORT_BIT(0x0004, "Start", KEYCODE_1) COREPORT_BIT(0x8000, "Tilt", KEYCODE_DEL)
            COREPORT_BITTOG(0x0008, "Coin Door", KEYCODE_END) COREPORT_BIT(0x0010, "Bookkeeping", KEYCODE_7)
                COREPORT_BIT(0x0100, "Clear NVRAM", KEYCODE_0) PORT_START /* 1 */
    COREPORT_DIPNAME(0x0001, 0x0000, "Credits for coin #1") COREPORT_DIPSET(0x0000, "1") COREPORT_DIPSET(0x0001, "2")
        COREPORT_DIPNAME(0x0006, 0x0000, "Credits for coin #2") COREPORT_DIPSET(0x0000, "4")
            COREPORT_DIPSET(0x0002, "5") COREPORT_DIPSET(0x0004, "6") COREPORT_DIPSET(0x0006, "7")
                COREPORT_DIPNAME(0x0018, 0x0000, "Balls per game") COREPORT_DIPSET(0x0000, "3")
                    COREPORT_DIPSET(0x0008, "4") COREPORT_DIPSET(0x0010, "5") COREPORT_DIPSET(0x0018, "5")
                        COREPORT_DIPNAME(0x0060, 0x0000, "Replay scores") COREPORT_DIPSET(0x0000, "600K / 1.2M")
                            COREPORT_DIPSET(0x0020, "700K / 1.4M") COREPORT_DIPSET(0x0040, "800K / 1.6M")
                                COREPORT_DIPSET(0x0060, "900K / 1.8M")
                                    COREPORT_DIPNAME(0x0080, 0x0080, "Repetitive Special")
                                        COREPORT_DIPSET(0x0080, DEF_STR(Off))
                                            COREPORT_DIPSET(0x0000, DEF_STR(On)) INPUT_PORTS_END

    static core_tLCDLayout disp[] = {{3, 0, 0, 6, CORE_SEG7}, {0, 16, 8, 1, CORE_SEG7}, {0, 20, 10, 2, CORE_SEG7}, {0}};
