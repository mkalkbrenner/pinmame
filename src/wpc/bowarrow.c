...
static READ_HANDLER(piap0b_r) {
    ...
    if (locals.a0 & 0x10)
        sw = core_getDip(0); // DIP#1 1-8
    else if (locals.a0 & 0x20)
        sw = core_getDip(1); // DIP#2 9-16
    else if (locals.a0 & 0x40)
        sw = core_getDip(2); // DIP#3 17-24
    else if (locals.a0 & 0x80)
        sw = core_getDip(3); // DIP#4 25-32
    else if (locals.a0)
        sw = core_getSwCol(locals.a0 & 0x0f);
    return core_revbyte(sw);
}

// writes out credits when NMI is fired, so buffer it
static WRITE_HANDLER(piap0b_w) { memory_region(REGION_CPU1)[0x200] = data; }
...
// set display row
static WRITE_HANDLER(piap1a_w) { locals.a1 = data; }
...
//diag. LED
static WRITE_HANDLER(piap1ca2_w) { coreGlobals.diagnosticLed = data; }

// solenoid control?
static WRITE_HANDLER(piap1cb2_w) { locals.cb21 = data; }

static READ_HANDLER(piap0ca1_r) { return locals.ca10; }

static READ_HANDLER(piap1ca1_r) { return locals.ca11; }

static struct pia6821_interface by35Proto_pia[] = {{/* I:  A/B,CA1/B1,CA2/B2 */ 0, piap0b_r, piap0ca1_r, 0, 0, 0,
                                                    /* O:  A/B,CA2/B2        */ piap0a_w, piap0b_w, piap0ca2_w, 0,
                                                    /* IRQ: A/B              */ piaIrq0, 0},
                                                   {/* I:  A/B,CA1/B1,CA2/B2 */ 0, 0, piap1ca1_r, 0, 0, 0,
                                                    /* O:  A/B,CA2/B2        */ piap1a_w, piap1b_w, piap1ca2_w,
                                                    piap1cb2_w,
                                                    /* IRQ: A/B              */ piaIrq1, 0}};
...
static MEMORY_READ_START(by35_readmem){0x0000, 0x007f, MRA_RAM}, /* U7 128 Byte Ram*/
    ...
    MACHINE_DRIVER_START(byProto) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(by35Proto, by35, by35Proto)
        MDRV_CPU_ADD_TAG("mcpu", M6800, 560000) MDRV_CPU_MEMORY(by35_readmem, by35_writemem)
            MDRV_CPU_VBLANK_INT(by35_vblank, 1) MDRV_CPU_PERIODIC_INT(byProto_irq, 317 * 2)
                MDRV_NVRAM_HANDLER(generic_1fill) MDRV_DIPS(32) MDRV_SWITCH_UPDATE(by35) MDRV_DIAGNOSTIC_LEDH(1)
                    MDRV_TIMER_ADD(by35p_zeroCross, 120 * 2) // won't work with 100Hz, ie. not in Europe!
    MACHINE_DRIVER_END

#define BY35PROTO_COMPORTS                                                                                                                   \
    PORT_START /* 0 */                                    /* Switch Column 0 */                                                              \
        COREPORT_BIT(0x1000, "Clear Credits", KEYCODE_0)  /* Switch Column 3 */                                                              \
        COREPORT_BIT(0x0100, "Ball Tilt", KEYCODE_INSERT) /* Switch Column 4 */                                                              \
        COREPORT_BITDEF(0x0001, IPT_START1, IP_KEY_DEFAULT) COREPORT_BIT(0x0002, "Self Test", KEYCODE_7) COREPORT_BITDEF(                    \
            0x0004,                                                                                                                          \
            IPT_COIN1, IP_KEY_DEFAULT) COREPORT_BITDEF(0x0008,                                                                               \
                                                       IPT_COIN2, IP_KEY_DEFAULT) COREPORT_BIT(0x0010, "Slam Tilt",                          \
                                                                                               KEYCODE_HOME) PORT_START /* 1 */              \
            COREPORT_DIPNAME(0x0003, 0x0000, "Chute #1 Coins") COREPORT_DIPSET(0x0000, "1") COREPORT_DIPSET(0x0002, "2") COREPORT_DIPSET(    \
                0x0001,                                                                                                                      \
                "3") COREPORT_DIPSET(0x0003,                                                                                                 \
                                     "4") COREPORT_DIPNAME(0x000c, 0x0000,                                                                   \
                                                           "Chute #1 Credits") COREPORT_DIPSET(0x0000,                                       \
                                                                                               "1") COREPORT_DIPSET(0x0008,                  \
                                                                                                                    "2")                     \
                COREPORT_DIPSET(0x0004, "3") COREPORT_DIPSET(0x000c, "4") COREPORT_DIPNAME(0x00f0, 0x00f0, "Replay #2/#1+") COREPORT_DIPSET( \
                    0x0000,                                                                                                                  \
                    "0") COREPORT_DIPSET(0x0080,                                                                                             \
                                         "4k") COREPORT_DIPSET(0x0040,                                                                       \
                                                               "8k") COREPORT_DIPSET(0x00c0,                                                 \
                                                                                     "12k") COREPORT_DIPSET(0x0020, "16k")                   \
                    COREPORT_DIPSET(0x00a0, "20k") COREPORT_DIPSET(0x0060, "24k") COREPORT_DIPSET(0x00e0, "28k") COREPORT_DIPSET(            \
                        0x0010,                                                                                                              \
                        "32k") COREPORT_DIPSET(0x0090,                                                                                       \
                                               "36k") COREPORT_DIPSET(0x0050,                                                                \
                                                                      "40k") COREPORT_DIPSET(0x00d0,                                         \
                                                                                             "44k") COREPORT_DIPSET(0x0030, "48k")           \
                        COREPORT_DIPSET(0x00b0, "52k") COREPORT_DIPSET(0x0070, "56k") COREPORT_DIPSET(0x00f0, "60k") COREPORT_DIPNAME(       \
                            0x0300,                                                                                                          \
                            0x0000,                                                                                                          \
                            "Chute #2 Coins") COREPORT_DIPSET(0x0000,                                                                        \
                                                              "1") COREPORT_DIPSET(0x0200,                                                   \
                                                                                   "2") COREPORT_DIPSET(0x0100, "3")                         \
                            COREPORT_DIPSET(0x0300, "4") COREPORT_DIPNAME(0x0c00, 0x0000, "Chute #2 Credits") COREPORT_DIPSET(               \
                                0x0000,                                                                                                      \
                                "1") COREPORT_DIPSET(0x0800,                                                                                 \
                                                     "2") COREPORT_DIPSET(0x0400,                                                            \
                                                                          "3") COREPORT_DIPSET(0x0c00, "4")                                  \
                                COREPORT_DIPNAME(0xf000, 0xf000, "Replay #3/#2+") COREPORT_DIPSET(                                           \
                                    0x0000,                                                                                                  \
                                    "0") COREPORT_DIPSET(0x8000,                                                                             \
                                                         "4k") COREPORT_DIPSET(0x4000,                                                       \
                                                                               "8k") COREPORT_DIPSET(0xc000,                                 \
                                                                                                     "12k") COREPORT_DIPSET(0x2000, "16k")   \
                                    COREPORT_DIPSET(0xa000, "20k") COREPORT_DIPSET(0x6000, "24k") COREPORT_DIPSET(                           \
                                        0xe000,                                                                                              \
                                        "28k") COREPORT_DIPSET(0x1000,                                                                       \
                                                               "32k") COREPORT_DIPSET(0x9000,                                                \
                                                                                      "36k") COREPORT_DIPSET(0x5000,                         \
                                                                                                             "40k")                          \
                                        COREPORT_DIPSET(0xd000, "44k") COREPORT_DIPSET(0x3000, "48k") COREPORT_DIPSET(                       \
                                            0xb000,                                                                                          \
                                            "52k") COREPORT_DIPSET(0x7000,                                                                   \
                                                                   "56k") COREPORT_DIPSET(0xf000,                                            \
                                                                                          "60k") PORT_START /* 2 */                          \
                                            COREPORT_DIPNAME(0x0001, 0x0000, "Blink display") COREPORT_DIPSET(                               \
                                                0x0000,                                                                                      \
                                                "0") COREPORT_DIPSET(0x0001,                                                                 \
                                                                     "1") COREPORT_DIPNAME(0x0002, 0x0002,                                   \
                                                                                           "Melody") COREPORT_DIPSET(0x0000,                 \
                                                                                                                     "0")                    \
                                                COREPORT_DIPSET(0x0002, "1") COREPORT_DIPNAME(0x0004, 0x0004, "Incentive") COREPORT_DIPSET(  \
                                                    0x0000,                                                                                  \
                                                    "0") COREPORT_DIPSET(0x0004,                                                             \
                                                                         "1") COREPORT_DIPNAME(0x0008, 0x0008,                               \
                                                                                               "Score award")                                \
                                                    COREPORT_DIPSET(0x0000, "Extra Ball") COREPORT_DIPSET(0x0008, "Cr"                       \
                                                                                                                  "ed"                       \
                                                                                                                  "i"                        \
                                                                                                                  "t") COREPORT_DIPNAME(     \
                                                        0x0010,                                                                              \
                                                        0x0010,                                                                              \
                                                        "Special award") COREPORT_DIPSET(0x0000, "Extra Ball")                               \
                                                        COREPORT_DIPSET(0x0010, "Credit") COREPORT_DIPNAME(                                  \
                                                            0x00e0,                                                                          \
                                                            0x0020,                                                                          \
                                                            "Balls per game") COREPORT_DIPSET(0x0000,                                        \
                                                                                              "1") COREPORT_DIPSET(0x0080,                   \
                                                                                                                   "2")                      \
                                                            COREPORT_DIPSET(0x0040, "3") COREPORT_DIPSET(0x00c0, "4") COREPORT_DIPSET(       \
                                                                0x0020,                                                                      \
                                                                "5") COREPORT_DIPSET(0x00a0,                                                 \
                                                                                     "6") COREPORT_DIPSET(0x0060, "7")                       \
                                                                COREPORT_DIPSET(0x00e0, "8") COREPORT_DIPNAME(                               \
                                                                    0x0100,                                                                  \
                                                                    0x0100,                                                                  \
                                                                    "Match feature") COREPORT_DIPSET(0x0000, "0")                            \
                                                                    COREPORT_DIPSET(0x0100, "1") COREPORT_DIPNAME(                           \
                                                                        0x0200,                                                              \
                                                                        0x0200,                                                              \
                                                                        "Replay #1+128k") COREPORT_DIPSET(0x0000, "0")                       \
                                                                        COREPORT_DIPSET(0x0200, "1") COREPORT_DIPNAME(                       \
                                                                            0x0400,                                                          \
                                                                            0x0000,                                                          \
                                                                            "Replay #1+64k") COREPORT_DIPSET(0x0000,                         \
                                                                                                             "0")                            \
                                                                            COREPORT_DIPSET(0x0400, "1") COREPORT_DIPNAME(                   \
                                                                                0x0800,                                                      \
                                                                                0x0000,                                                      \
                                                                                "Replay #1+32k") COREPORT_DIPSET(0x0000,                     \
                                                                                                                 "0")                        \
                                                                                COREPORT_DIPSET(0x0800, "1") COREPORT_DIPNAME(               \
                                                                                    0x1000,                                                  \
                                                                                    0x0000,                                                  \
                                                                                    "Replay #1+16k") COREPORT_DIPSET(0x0000,                 \
                                                                                                                     "0")                    \
    ...
    static const core_tLCDLayout dispBA[] = {{0, 0, 2, 6, CORE_SEG7},
                                             {0, 14, 10, 6, CORE_SEG7},
                                             {2, 0, 18, 6, CORE_SEG7},
                                             {2, 14, 26, 6, CORE_SEG7},
                                             {4, 2, 34, 2, CORE_SEG7},
