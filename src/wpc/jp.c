...
static INTERRUPT_GEN(JP_irq) { cpu_set_irq_line(JP_CPU, 0, PULSE_LINE); }
...
dispStrobe(void) {
    ...
    static int pos[32] = {31, 30, 29, 28, 26, 25, 24, 23, 22, 19, 18, 17, 16, 15, 12, 11,
                          10, 9,  8,  5,  4,  3,  2,  1,  33, 34, 35, 36, 21, 14, 7,  0};
    int i, data = locals.sixColumns ? (!(locals.dispData >> 24) ? 8 : core_BitColToNum(locals.dispData >> 24))
                                    : (locals.dispData >> 24) & 0x0f;
    ...
    switch (data) {
        ...
        default:
            ...
            for (i = 0; i < 32; i++)
                if (locals.dispData & (1 << i))
                    locals.segments[pos[i]].w &= ~(1 << (locals.sixColumns ? data : 6 - data));
...
static WRITE_HANDLER(parallel_0_out) { locals.dispData = (locals.dispData & 0xffffff00) | data; }

static WRITE_HANDLER(parallel_1_out) { locals.dispData = (locals.dispData & 0xffff00ff) | (data << 8); }

static WRITE_HANDLER(parallel_2_out) { locals.dispData = (locals.dispData & 0xff00ffff) | (data << 16); }

static WRITE_HANDLER(parallel_3_out) { locals.dispData = (locals.dispData & 0x00ffffff) | (data << 24); }

static WRITE_HANDLER(qs1pin_0_out) { HC4094_data_w(1, data); }

static WRITE_HANDLER(qs1pin_1_out) { HC4094_data_w(2, data); }

static WRITE_HANDLER(qs1pin_2_out) { HC4094_data_w(3, data); }

static HC4094interface hc4094jp = {4, // 4 chips
...
static MEMORY_READ_START(JP_readmem){0x0000, 0x3fff, MRA_ROM}, {0x4000, 0x47ff, MRA_RAM}, {0x6001, 0x6001, ay8910_r},
    MEMORY_END

    static MEMORY_WRITE_START(JP_writemem){0x0000, 0x3fff, MWA_NOP},
    {0x4000, 0x47ff, MWA_RAM, &generic_nvram, &generic_nvram_size}, {0x6000, 0x6000, ay8910_ctrl_w},
    {0x6002, 0x6002, ay8910_data_w}, {0xa000, 0xa000, sol_w}, {0xa001, 0xa001, disp_w}, {0xa002, 0xa007, lamp1_w},
...
static MACHINE_INIT(JPS) { machine_init_JP(); }
...
static INTERRUPT_GEN(JPS_irq) { cpu_set_irq_line(1, 0, PULSE_LINE); }
...
static MEMORY_READ_START(jpsnd_readmem){0x0000, 0x3fff, MRA_ROM}, {0x4000, 0x47ff, MRA_RAM},
    {0x8000, 0xffff, MRA_BANKNO(1)},
    MEMORY_END

    static MEMORY_WRITE_START(jpsnd_writemem){0x4000, 0x47ff, MWA_RAM},
    {0x5000, 0x5000, bank_w}, {0x6000, 0x6000, snd_w}, {0x7000, 0x7000, enable_w},
    MEMORY_END

    MACHINE_DRIVER_START(JPS) MDRV_IMPORT_FROM(JP) MDRV_CPU_ADD_TAG("scpu", Z80, JP_CPUFREQ)
        MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(jpsnd_readmem, jpsnd_writemem)
            MDRV_CPU_PERIODIC_INT(JPS_irq, 4000) MDRV_CORE_INIT_RESET_STOP(JPS, NULL, JP) MDRV_INTERLEAVE(50)
                MDRV_SOUND_ADD(MSM5205, JP_msm5205Int) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(JP1) MDRV_IMPORT_FROM(JP) MDRV_CORE_INIT_RESET_STOP(JP1, NULL, JP) MACHINE_DRIVER_END
...
static READ_HANDLER(i8279_r) {
    ...
    if ((locals.i8279cmd & 0xe0) == 0x40)
        lastData = coreGlobals.swMatrix[1 + (locals.i8279cmd & 0x07)]; // read switches
...
static WRITE_HANDLER(i8279_w) {
    ...
    if (offset) { // command
        ...
        if ((locals.i8279cmd & 0xe0) == 0x40)
            ...
            logerror("i8279 w1:%d:%02x ", offset, data);
        if (locals.i8279cmd & 0x10)
            locals.i8279reg = 0;                // reset data for auto-increment
    } else {                                    // data
        if ((locals.i8279cmd & 0xe0) == 0x80) { // write display ram
            ...
            logerror("i8279 w0:%d:%02x\n", offset, data);
        if (locals.i8279cmd & 0x10)
            locals.i8279reg = (locals.i8279reg + 1) % 16; // auto-increase if register is set
    }
}

static MEMORY_READ_START(JP2_readmem){0x0000, 0x3fff, MRA_ROM}, {0x6000, 0x67ff, MRA_RAM}, {0x8000, 0x8000, i8279_r},
    MEMORY_END

    static MEMORY_WRITE_START(JP2_writemem){0x6000, 0x67ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    ...
    switch (offset) {
        case 0:
            return (~coreGlobals.swMatrix[5] & 0x40) | (~core_getDip(0) & 0xbf);
        case 1:
            return (~coreGlobals.swMatrix[6] & 0xc0) | (~core_getDip(1) & 0x3f);
        default:
            return ~core_getDip(offset);
    }
}

static PORT_READ_START(JP2_readport){0x08, 0x0a, sw2_r}, {0x0d, 0x0d, AY8910_read_port_0_r},
    MEMORY_END
...
static WRITE_HANDLER(sol2_w) { locals.solenoids = (locals.solenoids & 0xffff00ff) | (data << 8); }

static PORT_WRITE_START(JP2_writeport){0x0c, 0x0c, AY8910_control_port_0_w}, {0x0e, 0x0e, AY8910_write_port_0_w},
    {0x10, 0x10, AY8910_control_port_1_w}, {0x12, 0x12, AY8910_write_port_1_w}, {0x14, 0x14, sol1_w},
    {0x18, 0x18, sol2_w},
    MEMORY_END

    MACHINE_DRIVER_START(JP2) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", Z80, 5000000) // taken from schematics
    MDRV_CPU_MEMORY(JP2_readmem, JP2_writemem) MDRV_CPU_PORTS(JP2_readport, JP2_writeport)
        MDRV_CPU_VBLANK_INT(JP2_vblank, 1)
            MDRV_CPU_PERIODIC_INT(JP_irq, 400) // guessed, schematics show IRQ not connected?
    MDRV_CORE_INIT_RESET_STOP(JP, NULL, NULL) MDRV_NVRAM_HANDLER(generic_0fill) MDRV_SWITCH_UPDATE(JP2) MDRV_DIPS(24)
        MDRV_SOUND_ADD(AY8910, JP2_ay8910Int2) MDRV_SOUND_ATTRIBUTES(SOUND_SUPPORTS_STEREO) MACHINE_DRIVER_END
