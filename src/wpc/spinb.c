...
#define SPINB_NMIFREQ                                                                                                  \
    1440 /* Z80 NMI frequency (confirmed by Jolly Park schematics) (should probably be INTFRQ*8=2000) */
...
/* Sound board */
struct sndbrdIntf spinbIntf = {"SPINB", NULL, NULL, NULL, spinb_sndCmd_w, NULL, NULL, NULL, NULL, SNDBRD_NODATASYNC};

/* -- Manual IRQ Timing Adjustments for debugging -- */
#ifdef MAME_DEBUG
static void
adjust_timer(int offset) {
    static char s[8];
    SPINBlocals.irqfreq += offset;
    if (SPINBlocals.irqfreq < 1)
        SPINBlocals.irqfreq = 1;
...
dmd_serial_callback(int data) {
    ...
    if (r5 >= 6 && r5 <= 9)
        SPINBlocals.DMDPage = 0x600;
    if (r5 >= 10 && r5 <= 13)
        SPINBlocals.DMDPage = 0xa00;
    if (r5 >= 14 && r5 <= 17)
        SPINBlocals.DMDPage = 0xe00;
    if (r5 >= 18 && r5 <= 21)
        SPINBlocals.DMDPage = 0x1200;
    if (r5 >= 22 && r5 <= 25)
        SPINBlocals.DMDPage = 0x1600;
    if (r5 >= 26 && r5 <= 29)
        SPINBlocals.DMDPage = 0x1a00;
    if (r5 >= 30 && r5 <= 33)
        SPINBlocals.DMDPage = 0x1e00;
#else
    if ((SPINBlocals.DMDFrame >= SPINBlocals.dmdframes) || (SPINBlocals.DMDRow >= 32) || (SPINBlocals.DMDCol >= 16)) {
        logerror("bad DMD position, frame=%d, row=%d, col=%d", SPINBlocals.DMDFrame, SPINBlocals.DMDRow,
                 SPINBlocals.DMDCol);
    } else {
...
READ32_HANDLER(dmd_eram_address) {
    ...
    if (!SPINBlocals.DMDRamEnabled && !SPINBlocals.DMDRom1Enabled && !SPINBlocals.DMDRom2Enabled)
        ...
        if (SPINBlocals.DMDRom1Enabled || SPINBlocals.DMDRom2Enabled) {
            addr |= (SPINBlocals.DMDA16Enabled << 16) | (SPINBlocals.DMDA17Enabled << 17);
...
static WRITE_HANDLER(solenoid_w) {
    switch (offset) {
        case 0:
            SPINBlocals.solenoids |= coreGlobals.pulsedSolState = (coreGlobals.pulsedSolState & 0xFFFFFF00) | data;
            break;
        case 1:
            SPINBlocals.solenoids |= coreGlobals.pulsedSolState =
                (coreGlobals.pulsedSolState & 0xFFFF00FF) | (data << 8);
            break;
        case 2:
            SPINBlocals.solenoids |= coreGlobals.pulsedSolState =
                (coreGlobals.pulsedSolState & 0xFF00FFFF) | (data << 16);
            break;
        default:
...
READ_HANDLER(ci20_porta_r) {
    LOG(("UNDOCUMENTED: ci20_porta_r\n"));
    return 0;
}

READ_HANDLER(ci20_portb_r) {
    LOG(("UNDOCUMENTED: ci20_portb_r\n"));
    return 0;
}
...
READ_HANDLER(ci20_portc_r) {
    ...
    if (SPINBlocals.swCol > 2) //Switch Column Strobe
        data =
            coreGlobals.swMatrix
                [SPINBlocals.swCol
                 - 2]; //so we begin by reading column 1 of input matrix instead of 0 which is used for special switches in many drivers
...
READ_HANDLER(ci23_porta_r) {
    LOG(("UNDOCUMENTED: ci23_porta_r\n"));
    return 0;
}

READ_HANDLER(ci23_portb_r) {
    LOG(("UNDOCUMENTED: ci23_portb_r\n"));
    return 0;
}
...
READ_HANDLER(ci22_porta_r) {
    LOG(("UNDOCUMENTED: ci22_porta_r\n"));
    return 0;
}

READ_HANDLER(ci22_portb_r) {
    LOG(("UNDOCUMENTED: ci22_portb_r\n"));
    return 0;
}

READ_HANDLER(ci22_portc_r) {
    LOG(("UNDOCUMENTED: ci22_portc_r\n"));
    return 0;
}

READ_HANDLER(ci21_porta_r) {
    LOG(("UNDOCUMENTED: ci21_porta_r\n"));
    return 0;
}

READ_HANDLER(ci21_portb_r) {
    LOG(("UNDOCUMENTED: ci21_portb_r\n"));
    return 0;
}

READ_HANDLER(ci21_portc_r) {
    LOG(("UNDOCUMENTED: ci21_portc_r\n"));
    return 0;
}
...
(out) P3-P7: J2 - Pins 6 - 10 (Marked Nivel C03-C07) - Switch Strobe (5 lines)
*/
WRITE_HANDLER(ci20_porta_w) { SPINBlocals.swCol = core_BitColToNum(data); }
...
WRITE_HANDLER(ci20_portb_w) {
    if (data)
        SPINBlocals.swCol = 8 + core_BitColToNum(data & 0x0f);
}
...
static WRITE_HANDLER(lamp_ex_w) { coreGlobals.tmpLampMatrix[6 + offset] |= data; }
...
WRITE_HANDLER(ci22_porta_w) {
    if (SPINBlocals.L16isGameOn) {
        SPINBlocals.solenoids |= coreGlobals.pulsedSolState =
            (coreGlobals.pulsedSolState & 0xFEFFFFFF) | ((data & 0x80) << 17);
        data &= 0x7f;
    }
    if (!SPINBlocals.nmiSeries)
...
WRITE_HANDLER(ci21_porta_w) {
    if (data)
        solenoid_w(0, data ^ ~SPINBlocals.solInv0);
}
...
WRITE_HANDLER(ci21_portc_w) {
    if (data)
        solenoid_w(1, data ^ ~SPINBlocals.solInv1);
}
...
static SWITCH_UPDATE(spinb) {
    ...
    if (inports) {
        ...
        coreGlobals.swMatrix[1] =
            (coreGlobals.swMatrix[1] & 0x06) | (inports[SPINB_COMINPORT] & 0x00f9); //Column 1 Switches
    }
    SPINBlocals.TestContactos = (core_getSw(SPINB_SWTEST) > 0 ? 1 : 0);
}

//Send a sound command to the sound board
WRITE_HANDLER(spinb_sndCmd_w) { soundbd_w(0, data); }
...
static MACHINE_RESET(spinb) { SPINBlocals.volume = 122; }

static MACHINE_STOP(spinb) { sndbrd_0_exit(); }
...
P1_update(int data) {
...
#ifdef MAME_DEBUG
    {
        ...
        if (newstat != prvstat && newstat == ERROR)
            logerror("DMD reports error\n");
        prvstat = newstat;
    }
#endif
...
WRITE_HANDLER(digvol_w) {
    if (data && SPINBlocals.volume < 142)
        SPINBlocals.volume++;
    if (!data && SPINBlocals.volume > 0)
        SPINBlocals.volume--;
...
static READ_HANDLER(SPINB_S1_MSM5205_READROM) {
    int addr, data;
    addr = (SPINBlocals.S1_CS2 << 20) | (SPINBlocals.S1_CS1 << 19) | (SPINBlocals.S1_A18 << 18)
           | (SPINBlocals.S1_A17 << 17) | (SPINBlocals.S1_A16 << 16) | (SPINBlocals.S1_AHI << 8) | (SPINBlocals.S1_ALO);
...
static READ_HANDLER(SPINB_S2_MSM5205_READROM) {
    int addr, data;
    addr = (SPINBlocals.S2_CS2 << 20) | (SPINBlocals.S2_CS1 << 19) | (SPINBlocals.S2_A18 << 18)
           | (SPINBlocals.S2_A17 << 17) | (SPINBlocals.S2_A16 << 16) | (SPINBlocals.S2_AHI << 8) | (SPINBlocals.S2_ALO);
    data = (UINT8) * (memory_region(REGION_USER2) + addr);
    return data;
}

static WRITE_HANDLER(SPINB_S1_MSM5205_w) { SPINBlocals.S1_MSMDATA = data; }

static WRITE_HANDLER(SPINB_S2_MSM5205_w) { SPINBlocals.S2_MSMDATA = data; }

/* MSM5205 interrupt callback */
static void
SPINB_S1_msmIrq(int data) {
    //Write data
...
/* MAIN CPU MEMORY MAP */
/* ------------------- */
static MEMORY_READ_START(spinb_readmem){0x0000, 0x3fff, MRA_ROM}, {0x4000, 0x5fff, MRA_RAM},
    {0x6000, 0x6003, ppi8255_0_r}, {0x6400, 0x6403, ppi8255_1_r}, {0x6800, 0x6803, ppi8255_2_r},
    {0x6c00, 0x6c03, ppi8255_3_r},
    MEMORY_END

    static MEMORY_WRITE_START(spinb_writemem){0x0000, 0x3fff, MWA_ROM},
    {0x4000, 0x43ff, MWA_RAM}, {0x4400, 0x45ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    {0x4600, 0x5fff, MWA_RAM}, {0x6000, 0x6003, ppi8255_0_w}, {0x6400, 0x6403, ppi8255_1_w},
    {0x6800, 0x6803, ppi8255_2_w}, {0x6c00, 0x6c03, ppi8255_3_w}, {0x6c20, 0x6c20, soundbd_w},
    {0x6c40, 0x6c45, lamp_ex_w}, {0x6c60, 0x6c60, dmdbd_w}, {0x6ce0, 0x6ce0, X6ce0_w},
    MEMORY_END

    static MEMORY_READ_START(spinb_readmem2){0x0000, 0x7fff, MRA_ROM},
    {0x8000, 0x85ff, MRA_RAM}, {0x8600, 0x86ff, MRA_RAM}, {0xc000, 0xc003, ppi8255_0_r}, {0xc400, 0xc403, ppi8255_1_r},
    {0xc800, 0xc803, ppi8255_2_r}, {0xcc00, 0xcc03, ppi8255_3_r},
    MEMORY_END

    static MEMORY_WRITE_START(spinb_writemem2){0x8000, 0x83ff, MWA_RAM},
    {0x8400, 0x85ff, MWA_RAM, &generic_nvram, &generic_nvram_size}, {0x8600, 0x86ff, MWA_RAM},
    {0xc000, 0xc003, ppi8255_0_w}, {0xc400, 0xc403, ppi8255_1_w}, {0xc800, 0xc803, ppi8255_2_w},
    {0xcc00, 0xcc03, ppi8255_3_w}, {0xcc20, 0xcc20, soundbd_w}, {0xcc60, 0xcc60, dmdbd_w},
    MEMORY_END
    ...
    static MEMORY_READ_START(spinbdmd_readmem){0x00000, 0x0ffff, MRA_ROM},
    {0x10000, 0x13ffe, MRA_RAM}, {0x13fff, 0x13fff, dmd_readcmd}, {0x14000, 0xfffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(spinbdmd_writemem){
        0x00000, 0x0ffff,
        MWA_ROM}, //This area can never really be accessed by the cpu core but we'll put this here anyway
#if DMD_FROM_RAM
    ...
    static PORT_READ_START(spinbdmd_readport){0x00, 0xff, i8031_port_read},
    PORT_END

    static PORT_WRITE_START(spinbdmd_writeport){0x00, 0xff, i8031_port_write},
    PORT_END
    ...
    static MEMORY_READ_START(spinbsnd1_readmem){0x0000, 0x1fff, MRA_ROM},
    {0x2000, 0x3fff, MRA_RAM}, {0x4000, 0x4003, ppi8255_4_r},
    {0x8000, 0x8000, sndcmd_r}, MEMORY_END static MEMORY_WRITE_START(spinbsnd1_writemem){0x0000, 0x1fff, MWA_ROM},
    {0x2000, 0x3fff, MWA_RAM}, {0x4000, 0x4003, ppi8255_4_w}, {0x6000, 0x6000, sndctrl_1_w},
    MEMORY_END

    //CPU #2 - MUSIC SAMPLES
    static MEMORY_READ_START(spinbsnd2_readmem){0x0000, 0x1fff, MRA_ROM},
    {0x2000, 0x3fff, MRA_RAM}, {0x4000, 0x4003, ppi8255_5_r},
    {0x8000, 0x8000, sndcmd_r}, MEMORY_END static MEMORY_WRITE_START(spinbsnd2_writemem){0x0000, 0x1fff, MWA_ROM},
    {0x2000, 0x3fff, MWA_RAM}, {0x4000, 0x4003, ppi8255_5_w}, {0x6000, 0x6000, sndctrl_2_w}, {0xa000, 0xa000, digvol_w},
    MEMORY_END

    /* DMD DRIVER */
    static MACHINE_DRIVER_START(spinbdmd) MDRV_CPU_ADD(I8051, SPINB_8051CPU_FREQ) /*16 Mhz*/
    MDRV_CPU_MEMORY(spinbdmd_readmem, spinbdmd_writemem) MDRV_CPU_PORTS(spinbdmd_readport, spinbdmd_writeport)
        MDRV_INTERLEAVE(50) MACHINE_DRIVER_END
    ...
    MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(spinbsnd1_readmem, spinbsnd1_writemem)
        MDRV_CPU_ADD(Z80, 6000000) //Schem shows 6/2 = 2.5/3Mhz, but sound is distorted otherwise
    MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(spinbsnd2_readmem, spinbsnd2_writemem) MDRV_INTERLEAVE(50)
        MDRV_SOUND_ADD(MSM5205, SPINB_msm5205Int) MACHINE_DRIVER_END
    ...
    MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(spinbsnd1_readmem, spinbsnd1_writemem)
        MDRV_CPU_ADD(Z80, 6000000) //Schem shows 6/2 = 2.5/3Mhz, but sound is distorted otherwise
    MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(spinbsnd2_readmem, spinbsnd2_writemem) MDRV_INTERLEAVE(50)
        MDRV_SOUND_ADD(MSM5205, SPINB_msm6585Int) MACHINE_DRIVER_END

    //Main Machine Driver (Main CPU Only)
    MACHINE_DRIVER_START(spinb) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(spinb, spinb, spinb)
        MDRV_NVRAM_HANDLER(generic_0fill) MDRV_CPU_ADD_TAG("mcpu", Z80, SPINB_Z80CPU_FREQ)
            MDRV_CPU_MEMORY(spinb_readmem, spinb_writemem) MDRV_CPU_VBLANK_INT(spinb_vblank, 1)
                MDRV_SWITCH_UPDATE(spinb) MDRV_SWITCH_CONV(spinb_sw2m, spinb_m2sw) MACHINE_DRIVER_END

    //Main CPU without NMI, DMD, Sound hardware Driver
    MACHINE_DRIVER_START(spinbs1) MDRV_IMPORT_FROM(spinb) MDRV_IMPORT_FROM(spinbdmd) MDRV_IMPORT_FROM(spinbs5205)
        MDRV_SOUND_CMD(spinb_sndCmd_w) MDRV_SOUND_CMDHEADING("spinb") MACHINE_DRIVER_END

    //Main CPU with NMI, DMD, Sound hardware Driver
    MACHINE_DRIVER_START(spinbs1n) MDRV_IMPORT_FROM(spinb) MDRV_CPU_MODIFY("mcpu")
        MDRV_CORE_INIT_RESET_STOP(spinbnmi, spinb, spinb) MDRV_CPU_PERIODIC_INT(spinb_z80nmi, SPINB_NMIFREQ)
            MDRV_IMPORT_FROM(spinbdmd) MDRV_IMPORT_FROM(spinbs6585) MDRV_SOUND_CMD(spinb_sndCmd_w)
                MDRV_SOUND_CMDHEADING("spinb") MACHINE_DRIVER_END

    //Main CPU with NMI, DMD, Sound hardware Driver, extended DMD memory
    MACHINE_DRIVER_START(spinbs1n2) MDRV_IMPORT_FROM(spinb) MDRV_CPU_MODIFY("mcpu")
        MDRV_CORE_INIT_RESET_STOP(spinbnmi2, spinb, spinb) MDRV_CPU_MEMORY(spinb_readmem2, spinb_writemem2)
            MDRV_CPU_PERIODIC_INT(spinb_z80nmi, SPINB_NMIFREQ) MDRV_IMPORT_FROM(spinbdmd)
                MDRV_CPU_ADD(Z80, 6000000) //Schem shows 5 or 6/2 = 2.5/3Mhz, but sound is distorted otherwise
    MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(spinbsnd1_readmem, spinbsnd1_writemem)
        MDRV_CPU_ADD(Z80, 6000000) //Schem shows 6/2 = 2.5/3Mhz, but sound is distorted otherwise
    MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(spinbsnd2_readmem, spinbsnd2_writemem) MDRV_INTERLEAVE(50)
        MDRV_SOUND_ADD(MSM5205, SPINB_msm6585Int2) MDRV_SOUND_CMD(spinb_sndCmd_w)
            MDRV_SOUND_CMDHEADING("spinb") MACHINE_DRIVER_END
...
//DRAW DMD FROM RAM OPTION
    ...
    static const int intens[3][4] = {{0, 3, 3, 3}, {0, 1, 3, 3}, {0, 1, 2, 3}};
