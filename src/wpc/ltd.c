...
static WRITE_HANDLER(ay8910_0_ctrl_w) {
    AY8910_set_volume(0, ALL_8910_CHANNELS, 50);
    AY8910Write(0, 0, data);
}

static WRITE_HANDLER(ay8910_0_data_w) {
    AY8910_set_volume(0, ALL_8910_CHANNELS, 50);
...
static WRITE_HANDLER(ay8910_1_ctrl_w) {
    AY8910_set_volume(1, ALL_8910_CHANNELS, 50);
    AY8910Write(1, 0, data);
}

static WRITE_HANDLER(ay8910_1_data_w) {
    AY8910_set_volume(1, ALL_8910_CHANNELS, 50);
...
static WRITE_HANDLER(ay8910_01_ctrl_w) {
    ay8910_0_ctrl_w(offset, data);
    ay8910_1_ctrl_w(offset, data);
}

static WRITE_HANDLER(ay8910_01_data_w) {
    ay8910_0_data_w(offset, data);
    ay8910_1_data_w(offset, data);
}

static WRITE_HANDLER(ay8910_01_reset) {
    AY8910_reset(0);
    AY8910_reset(1);
}
...
static INTERRUPT_GEN(LTD_irq) { cpu_set_irq_line(LTD_CPU, M6800_IRQ_LINE, PULSE_LINE); }
...
static WRITE_HANDLER(peri_w) {
    ...
    if (offset < 0x06) {
        ...
        if (core_gameData->hw.gameSpecific1 & (1 << seg)) {
            if (seg & 1)
                coreGlobals.tmpLampMatrix[8 + seg / 2 * 2] =
                    (coreGlobals.tmpLampMatrix[8 + seg / 2 * 2] & 0xf0) | (data >> 4);
            else
                coreGlobals.tmpLampMatrix[8 + seg / 2 * 2] =
                    (coreGlobals.tmpLampMatrix[8 + seg / 2 * 2] & 0x0f) | (data & 0xf0);
        }
        if (core_gameData->hw.gameSpecific2 & (1 << seg)) {
            if (seg & 1)
                coreGlobals.tmpLampMatrix[9 + seg / 2 * 2] =
                    (coreGlobals.tmpLampMatrix[9 + seg / 2 * 2] & 0xf0) | (data & 0x0f);
            else
                coreGlobals.tmpLampMatrix[9 + seg / 2 * 2] =
                    (coreGlobals.tmpLampMatrix[9 + seg / 2 * 2] & 0x0f) | (data << 4);
        }
    } else if (offset == 0x09) {
        strobe = data;
        if (data & 0x10) {
            ...
            timer_set(0.1, 2, snd_stop);
               ...
               && !strncasecmp(Machine->gamedrv->name, "force", 5)) { // drop target single reset solenoids
...
static WRITE_HANDLER(ram_w) {
    ...
    if (offset >= 0x70 && offset < 0x80)
        peri_w(offset - 0x70, data);
}

static READ_HANDLER(sw_r) { return ~(coreGlobals.swMatrix[offset + 1] & 0x3f); }

static READ_HANDLER(ff_r) { return 0xff; }

static MACHINE_INIT(LTD) { memset(&locals, 0, sizeof locals); }

/*-----------------------------------------
/  Memory map for system III CPU board
/------------------------------------------*/
static MEMORY_READ_START(LTD_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x0087, sw_r},
    {0x00ff, 0x00ff,
     ff_r}, // spcpoker code flaw: lda $00FF instead of lda #$FF, read from unmapped space -> all bits should be high
    {0xc000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(LTD_writemem){0x0000, 0x007f, ram_w, &generic_nvram, &generic_nvram_size},
    {0x0800, 0x0800, ay8910_01_data_w}, {0x0c00, 0x0c00, ay8910_01_ctrl_w}, {0x1800, 0x1800, ay8910_0_data_w},
    {0x1c00, 0x1c00, ay8910_0_ctrl_w}, {0x2800, 0x2800, ay8910_1_data_w}, {0x2c00, 0x2c00, ay8910_1_ctrl_w},
    {0xb000, 0xb000, ay8910_01_reset},
    MEMORY_END

    MACHINE_DRIVER_START(LTD3) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", M6802, LTD_CPUFREQ)
        MDRV_CPU_MEMORY(LTD_readmem, LTD_writemem) MDRV_CPU_VBLANK_INT(LTD_vblank, 1)
            MDRV_CPU_PERIODIC_INT(LTD_irq, 260) MDRV_CORE_INIT_RESET_STOP(LTD, NULL, NULL)
                MDRV_NVRAM_HANDLER(generic_1fill) MDRV_DIAGNOSTIC_LEDH(1) MDRV_SWITCH_UPDATE(LTD)
                    MDRV_SOUND_ADD(DISCRETE, ltd3_discInt) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(LTD3A) MDRV_IMPORT_FROM(LTD3) MDRV_CPU_MODIFY("mcpu") MDRV_CPU_PERIODIC_INT(LTD_irq, 1111)

        MDRV_SOUND_ADD(AY8910, LTD_ay8910Int) MDRV_SOUND_ATTRIBUTES(SOUND_SUPPORTS_STEREO) MACHINE_DRIVER_END
...
static READ_HANDLER(sw4_r) { return (~locals.port2 & 0x10) ? ~coreGlobals.swMatrix[locals.swCol + 1] : 0xff; }

/* convert the segments to fit the usual core.c layout */
static UINT8
convDisp(UINT8 data) {
    return (data & 0x88) | ((data & 1) << 6) | ((data & 2) << 4) | ((data & 4) << 2) | ((data & 0x10) >> 2)
           | ((data & 0x20) >> 4) | ((data & 0x40) >> 6);
}
    ...
    if (locals.port2 & 0x10) {
        switch (locals.cycle) {
            case 0:
                clear = data != 0xff;
                if (clear)
                    lampCol = (1 + core_BitColToNum(data)) % 8;
                break;
            case 1:
                if (locals.isHH) {
                    if ((data & 0x0f) != 0x0f)
                        locals.solenoids |= 0x10 << (data & 0x0f);
                    if ((data & 0xf0) != 0xf0)
                        locals.solenoids |= 0x4000 << (data >> 4);
                    coreGlobals.solenoids = locals.solenoids;
                } else
                    solBank = core_BitColToNum(data);
                break;
            case 2:
                locals.solenoids |= (data >> 4) << (solBank * 4);
            ...
            case 6:
                ...
                locals.segments[15 - (data & 0x0f)].w = dispData[1];
                break;
            case 7:
                if (clear)
                    dispData[0] = convDisp(data);
                break;
            case 8:
                if (clear)
                    dispData[1] = convDisp(data);
                break;
            case 9:
                if (clear)
                    coreGlobals.tmpLampMatrix[(lampCol % 2 ? 8 : 12) + lampCol / 2] = locals.auxData;
                break;
            case 10:
                if (clear)
                    coreGlobals.tmpLampMatrix[lampCol] = data;
                break;
            default:
                logerror("peri_%d_w = %02x\n", locals.cycle, data);
        }
    }
}

static READ_HANDLER(tilt_r) { return coreGlobals.swMatrix[0]; }

static WRITE_HANDLER(cycle_w) {
    if (~locals.port2 & data & 0x10)
        locals.cycle++;
    if (locals.port2 == 0x04)
        locals.cycle = 0;
    locals.port2 = data;
}
...
/------------------------------------------*/
static MEMORY_READ_START(LTD4_readmem){0x0000, 0x001f, m6803_internal_registers_r}, {0x0080, 0x00ff, MRA_RAM},
    {0x0100, 0x01ff, MRA_RAM}, {0xc000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(LTD4_writemem){0x0000, 0x001f, m6803_internal_registers_w},
    {0x0080, 0x00ff, MWA_RAM}, {0x0100, 0x01ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    {0x0800, 0x0800, ay8910_1_ctrl_w}, {0x0c00, 0x0c00, ay8910_1_mute}, {0x1000, 0x1000, ay8910_0_ctrl_w},
    {0x1400, 0x1400, ay8910_0_mute}, {0x1800, 0x1800, ay8910_01_ctrl_w}, {0x1c00, 0x1c00, ay8910_01_reset},
    {0x2800, 0x2800, ay8910_1_data_w}, {0x3000, 0x3000, ay8910_0_data_w}, {0x3800, 0x3800, ay8910_01_data_w},
    MEMORY_END

    static PORT_READ_START(LTD4_readport){M6803_PORT1, M6803_PORT1, sw4_r},
    {M6803_PORT2, M6803_PORT2, tilt_r},
    PORT_END

    static PORT_WRITE_START(LTD4_writeport){M6803_PORT1, M6803_PORT1, peri4_w},
    {M6803_PORT2, M6803_PORT2, cycle_w},
    PORT_END

    MACHINE_DRIVER_START(LTD4) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", M6803, LTD_CPUFREQ)
        MDRV_CPU_MEMORY(LTD4_readmem, LTD4_writemem) MDRV_CPU_PORTS(LTD4_readport, LTD4_writeport)
            MDRV_CPU_VBLANK_INT(LTD4_vblank, 1) MDRV_CORE_INIT_RESET_STOP(LTD, NULL, NULL)
                MDRV_NVRAM_HANDLER(generic_0fill) MDRV_SWITCH_UPDATE(LTD4) MDRV_SOUND_ADD(AY8910, LTD_ay8910Int)
                    MDRV_SOUND_ATTRIBUTES(SOUND_SUPPORTS_STEREO) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(LTD4HH) MDRV_IMPORT_FROM(LTD4) MDRV_CORE_INIT_RESET_STOP(LTDHH, NULL, NULL) MACHINE_DRIVER_END
