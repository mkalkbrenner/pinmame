...
const struct sndbrdIntf capcomsIntf = {
    "TMS320AV120", capcoms_init,     NULL, NULL, capcoms_sndCmd_w, NULL, NULL, NULL,
    NULL,          SNDBRD_NODATASYNC
...
static struct { struct sndbrdData brdData; } locals_cap;
...
void
set_cts_line_to_8752(int data) {
    locals.cts = data;
    //PRINTF(("setting cts = %x\n",data));
}

int
get_rts_line_from_8752(void) {
    return locals.rts;
}

void
send_data_to_8752(int data) {
    locals.to_8752 = data;
...
//Track state of SREQ
void
cap_sreq(int chipnum, int state) {
...
static MEMORY_READ_START(capcoms_readmem){0x000000, 0x001fff, MRA_ROM}, //Internal ROM
    ...
    MEMORY_END static MEMORY_WRITE_START(capcoms_writemem){0x000000, 0x001fff, MWA_ROM}, //Internal ROM
    ...
    static PORT_READ_START(capcoms_readport){0x00, 0xff, port_r},
    PORT_END static PORT_WRITE_START(capcoms_writeport){0x00, 0xff, port_w},
    PORT_END

    //Driver template with 8752 cpu
    MACHINE_DRIVER_START(capcoms) MDRV_CPU_ADD(I8752, 12000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(capcoms_readmem, capcoms_writemem) MDRV_CPU_PORTS(capcoms_readport, capcoms_writeport)
    //MDRV_INTERLEAVE(1500)
    MDRV_INTERLEAVE(50) MACHINE_DRIVER_END

    //Driver with 1 x TMS320av120
    MACHINE_DRIVER_START(capcom1s) MDRV_IMPORT_FROM(capcoms)
        MDRV_SOUND_ADD_TAG("tms320av120", TMS320AV120, capcoms_TMS320AV120Int1) MACHINE_DRIVER_END

    //Driver with 2 x TMS320av120
    MACHINE_DRIVER_START(capcom2s) MDRV_IMPORT_FROM(capcoms)
        MDRV_SOUND_ADD_TAG("tms320av120", TMS320AV120, capcoms_TMS320AV120Int2) MACHINE_DRIVER_END
...
//Set up timer to force feed data to the TMS chips from the ROMS
    ...
    if (locals.buffTimer)
        timer_remove(locals.buffTimer);

    /*-- Create timer to fill our buffer --*/
    locals.buffTimer = timer_alloc(cap_FillBuff);

    /*-- start the timer --*/
    timer_adjust(locals.buffTimer, 0, 0,
                 TIME_IN_HZ(30)); // Send 30 Frames per second ( 32Khz sample rate ~ 27.8 Frames per second)
#endif
}

//Initialize anything that cannot be reset here..
static void
capcoms_init(struct sndbrdData* brdData) {
...
static int
get_one_byte(int num, UINT8* byte, UINT32 pos) {
