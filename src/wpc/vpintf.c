...
/-------------------------------------*/
int
vp_getLamp(int lampNo) {
    if (coreData->lamp2m)
        lampNo = coreData->lamp2m(lampNo) - 8;
...
/-------------------------------------*/
int
vp_getChangedLamps(vp_tChgLamps chgStat) {
...
/-------------------------------------*/
int
vp_getChangedSolenoids(vp_tChgSols chgStat) {
...
/-------------------------------------*/
int
vp_getChangedGI(vp_tChgGIs chgStat) {
...
/-----------*/
void
vp_setDIP(int dipBank, int value) {
...
void
vp_setMechData(int para, int data) {
    if (para == 0) {
        mech_add(MECH_MAXMECH / 2 + data - 1, &locals.md);
        memset(&locals.md, 0, sizeof(locals.md));
    } else if (para == 1)
        locals.md.sol1 = data;
    else if (para == 2)
        locals.md.sol2 = data;
    else if (para == 3)
        locals.md.length = data;
    else if (para == 4)
        locals.md.steps = data;
    else if (para == 5)
        locals.md.type = (locals.md.type & 0xfffffe00) | data;
    else if (para == 6)
        locals.md.type = (locals.md.type & 0xff0001ff) | (data << 9);
    else if (para == 7)
        locals.md.type = (locals.md.type & 0x00ffffff) | (data << 24);
    else if (para == 8)
        locals.md.initialpos = data + 1;
    else if (para % 10 == 0)
        locals.md.sw[para / 10 - 1].swNo = data;
    else if (para % 10 == 1)
        locals.md.sw[para / 10 - 1].startPos = data;
    else if (para % 10 == 2)
        locals.md.sw[para / 10 - 1].endPos = data;
    else if (para % 10 == 3)
        locals.md.sw[para / 10 - 1].pulse = data;
}
...
/------------------------------------------------*/
int
vp_getNewSoundCommands(vp_tChgSound chgSound) {
...
/------------------------------------------------*/
int
vp_getChangedLEDs(vp_tChgLED chgStat, UINT64 mask, UINT64 mask2) {
    int idx = 0;
    int ii;
