...
static MEMORY_READ_START(nuova_readmem){0x0000, 0x007f, MRA_RAM}, /* Internal 128 Byte Ram, this needs all 8 bits */
    ...
    static MEMORY_WRITE_START(nuova_writemem){0x0000, 0x007f,
                                              MWA_RAM}, /* Internal 128 Byte Ram, this needs all 8 bits */
...
static const struct pia6821_interface nuova_pia = {
    /*i: A/B,CA/B1,CA/B2 */ nuova_pia_a_r,
    0,
    PIA_UNUSED_VAL(1),
    nuova_pia_cb1_r,
    nuova_pia_ca2_r,
    PIA_UNUSED_VAL(0),
    /*o: A/B,CA/B2       */ nuova_pia_a_w,
    nuova_pia_b_w,
    0,
    0,
    /*irq: A/B           */ nuova_irq,
...
static void
nuova_init(struct sndbrdData* brdData) {
...
static void
nuova_diag(int button) {
    cpu_set_nmi_line(locals.brdData.cpuNo, button ? ASSERT_LINE : CLEAR_LINE);
}

static WRITE_HANDLER(nuova_data_w) { locals.sndCmd = data & 0x0f; }
...
static READ_HANDLER(snd_cmd_r) { return ((~locals.sndCmd & 0x07) << 1) | ((~locals.sndCmd & 0x08) >> 3); }

static MEMORY_READ_START(snd_readmem){0x0000, 0x001f, m6803_internal_registers_r}, {0x0080, 0x00ff, MRA_RAM},
    {0x8000, 0xffff, MRA_BANKNO(1)},
    MEMORY_END

    static MEMORY_WRITE_START(snd_writemem){0x0000, 0x001f, m6803_internal_registers_w},
    {0x0080, 0x00ff, MWA_RAM}, {0x8000, 0xffff, bank_w},
    MEMORY_END

    static PORT_READ_START(snd_readport){M6803_PORT2, M6803_PORT2, snd_cmd_r},
    PORT_END

    static PORT_WRITE_START(snd_writeport){M6803_PORT1, M6803_PORT1, dac_w},
...
const struct sndbrdIntf nuovaIntf = {
    "NUOVA",      nuova_init, NULL,         nuova_diag, nuova_man_w,
    nuova_data_w, NULL,       nuova_ctrl_w, NULL,       SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
...
static core_tLCDLayout dispNB[] = {{0, 0, 2, 7, CORE_SEG87F},
                                   {0, 16, 10, 7, CORE_SEG87F},
                                   {2, 0, 18, 7, CORE_SEG87F},
                                   {2, 16, 26, 7, CORE_SEG87F},
                                   {4, 4, 35, 2, CORE_SEG7},
                                   {4, 10, 38, 2, CORE_SEG7},
                                   {0}};
static core_tLCDLayout dispBy7[] = {{0, 0, 1, 7, CORE_SEG87F},
                                    ...
                                    {4, 10, 38, 2, CORE_SEG7},
                                    {0}};
static core_tLCDLayout dispAlpha[] = {{0, 0, 0, 16, CORE_SEG16S}, {4, 0, 16, 16, CORE_SEG16S}, {0}};
...
BY35_ROMSTARTx00(suprbowl, "sbowlu2.732", CRC(bc497a13) SHA1(f428373bde72f0302c45c326aebbe56e8b09c2d6), "sbowlu6.732",
                 CRC(a9c92719) SHA1(972da0cf87863b637b88575c329f1d8162098d6f))
    BY56_SOUNDROM("suprbowl.snd", CRC(97fc0f7a) SHA1(595aa080a6d2c1ab7e718974c4d01e846e142cc1)) BY35_ROMEND
...
/-------------------------------*/
    ROM_START(tigerrag) NORMALREGION(0x10000, REGION_CPU1)
        ROM_LOAD("tigerrag.mpu", 0xe000, 0x2000, CRC(3eb389ba) SHA1(bdfdcf00f4a2200d39d7e469fe633e0b7b8f1676))
            ROM_COPY(REGION_CPU1, 0xe000, 0x1000, 0x0800) ROM_COPY(REGION_CPU1, 0xe800, 0x5000, 0x0800)
                ROM_COPY(REGION_CPU1, 0xf000, 0x1800, 0x0800) ROM_COPY(REGION_CPU1, 0xf800, 0x5800, 0x0800)
                    BY45_SOUNDROM11("kngsu3.snd", CRC(11b02dca) SHA1(464eee1aa1fd9b6e26d4ba635777fffad0222106),
                                    "kngsu4.snd", CRC(f3e4d2f6) SHA1(93f4e9e1348b1225bc02db38c994e3338afb175c)) ROM_END
...
/-------------------------------*/
    INITGAMENB(cosflash, GEN_BY35, dispNB, FLIP_SW(FLIP_L), 8, SNDBRD_BY61, 0)
        BY35_ROMSTARTx00(cosflash, "cf2d.532", CRC(939e941d) SHA1(889862043f351762e8c866aefb36a9ea75cbf828), "cf6d.532",
                         CRC(7af93d2f) SHA1(2d939b14f7fe79f836e12926f44b70037630cd3f))
...
/-------------------------------*/
    INITGAMENB(saturn2, GEN_BY35, dispNB, FLIP_SW(FLIP_L), 8, SNDBRD_BY45, 0)
        BY35_ROMSTARTx00(saturn2, "spy-2732.u2", CRC(9e930f2d) SHA1(fb48ce0d8d8f8a695827c0eea57510b53daa7c39),
            ...
            BY45_SOUNDROMx2("sat2_snd.764", CRC(6bf15541) SHA1(dcdd4e8f662818584de9b1ed7ae69d57362ebadb)) BY35_ROMEND
    ...
    CORE_CLONEDEFNV(saturn2, spyhuntr, "Saturn 2", 1985, "Bell Games", by35_mBY35_45S, 0)
        ...
        INITGAMENB(saturnfp, GEN_BY35, dispNB, FLIP_SW(FLIP_L), 8, SNDBRD_BY45, 0)
            BY35_ROMSTARTx00(saturnfp, "saturn2_freeplay.u2",
                             CRC(422f190e) SHA1(595b0f237c80acd174de44bb0a5ad7b36a60d967), "saturn2_freeplay.u6",
                             CRC(cae795c1) SHA1(586b7b341e6f0292e7d3c8bb0b4e2c05dd19d2d7))
                BY45_SOUNDROMx2("sat2_snd.764",
                                CRC(6bf15541) SHA1(dcdd4e8f662818584de9b1ed7ae69d57362ebadb)) BY35_ROMEND
...
/-------------------------------*/
    INITGAMENB(newwave, GEN_BY35, dispNB, FLIP_SW(FLIP_L), 8, SNDBRD_BY45, 0)
        BY35_ROMSTARTx00(newwave, "blkp2732.u2", CRC(600535b0) SHA1(33d080f4430ad9c33ee9de1bfbb5cfde50f0776e),
            ...
            BY45_SOUNDROM11("bp_u3.532", CRC(a5005067) SHA1(bd460a20a6e8f33746880d72241d6776b85126cf), "newwu4.532",
                            CRC(6f4f2a95) SHA1(a7a375827c0429b8b3d2ee9e471f557152492993)) BY35_ROMEND
...
/ World Defender
/-------------------------------*/
    ROM_START(worlddef) NORMALREGION(0x10000, REGION_CPU1)
        ROM_LOAD("worlddef.764", 0xe000, 0x2000, CRC(ad1a7ba3) SHA1(d799b6d1cd252cd6d9fb72586099c43de7c22a00))
            ROM_COPY(REGION_CPU1, 0xe000, 0x1000, 0x0800) ROM_COPY(REGION_CPU1, 0xe800, 0x5000, 0x0800)
                ROM_COPY(REGION_CPU1, 0xf000, 0x1800, 0x0800) ROM_COPY(REGION_CPU1, 0xf800, 0x5800, 0x0800)
                    BY45_SOUNDROMx2("wodefsnd.764",
                                    CRC(b8d4dc20) SHA1(5aecac4a2deb7ea8e0ff0600ea459ef272dcd5f0)) ROM_END
...
/-------------------------------*/
    ROM_START(worlddfp) NORMALREGION(0x10000, REGION_CPU1)
...
/ Space Hawks
/-------------------------------*/
    ROM_START(spacehaw) NORMALREGION(0x10000, REGION_CPU1)
        ROM_LOAD("spacehaw.bin", 0xe000, 0x2000, CRC(f070b2c3) SHA1(405d959e6e3976d50470594deadd3e3625fc1791))
            ROM_COPY(REGION_CPU1, 0xe000, 0x1000, 0x0800) ROM_COPY(REGION_CPU1, 0xe800, 0x5000, 0x0800)
                ROM_COPY(REGION_CPU1, 0xf000, 0x1800, 0x0800) ROM_COPY(REGION_CPU1, 0xf800, 0x5800, 0x0800)
                    BY45_SOUNDROM2x("sh_sound.264",
                                    CRC(2b548d24) SHA1(83ac9b75ae9c1960ad73abcf40adc2bc46827568)) ROM_END
...
/-------------------------------*/
    ROM_START(spchawfp) NORMALREGION(0x10000, REGION_CPU1)
...
/ Dark Shadow
/-------------------------------*/
    ROM_START(darkshad) NORMALREGION(0x10000, REGION_CPU1)
        ROM_LOAD("cpu_u7.bin", 0xe000, 0x2000, CRC(8d04c546) SHA1(951e75d9867b85a0bf9f04fe9aa647a53b6830bc))
            ROM_COPY(REGION_CPU1, 0xe000, 0x1000, 0x0800) ROM_COPY(REGION_CPU1, 0xe800, 0x1800, 0x0800)
                ROM_COPY(REGION_CPU1, 0xf000, 0x5000, 0x0800) ROM_COPY(REGION_CPU1, 0xf800, 0x5800, 0x0800)
                    BY45_SOUNDROM2x("darkshad.snd",
                                    CRC(9fd6ee82) SHA1(6486fa56c663152e565e160b8f517be824338a9a)) ROM_END
...
static MEMORY_READ_START(snd_readmem_skflight){0x0000, 0x001f, m6803_internal_registers_r}, {0x0000, 0x00ff, MRA_RAM},
    {0x4000, 0x4003, pia_r(2)}, {0x8000, 0xffff, MRA_BANKNO(1)},
    MEMORY_END

    static MEMORY_WRITE_START(snd_writemem_skflight){0x0000, 0x001f, m6803_internal_registers_w},
    {0x0000, 0x00ff, MWA_RAM}, {0x4000, 0x4003, pia_w(2)}, {0x8000, 0xffff, MWA_NOP},
    MEMORY_END

    static PORT_READ_START(snd_readport_skflight){M6803_PORT2, M6803_PORT2, snd_cmd_r_skflight},
    PORT_END

    static PORT_WRITE_START(snd_writeport_skflight){M6803_PORT1, M6803_PORT1, dac_w},
    {M6803_PORT2, M6803_PORT2, bank_w_skflight},
    PORT_END

    static void nuova_5220Irq(int state) {
    pia_set_input_cb1(2, state);
}

static void
nuova_5220Rdy(int state) {
    pia_set_input_ca2(2, state);
}
...
BY35_INPUT_PORTS_START(skflight, 3)
BY35_INPUT_PORTS_END CORE_GAMEDEFNV(skflight, "Skill Flight", 1986, "Nuova Bell Games", skflight, 0)

    /*--------------------------------
/ Cobra
/-------------------------------*/
    ROM_START(cobra) NORMALREGION(0x10000, REGION_CPU1)
        ROM_LOAD("cpu_u7.256", 0xc000, 0x4000, CRC(c0f89577) SHA1(16d351f2bf642bf886e808b58173b3e699a44fd6))
            ROM_COPY(REGION_CPU1, 0xc000, 0x1000, 0x1000) ROM_COPY(REGION_CPU1, 0xd000, 0x5000, 0x1000)
                ROM_COPY(REGION_CPU1, 0xe000, 0x9000, 0x1000) ROM_COPY(REGION_CPU1, 0xf000, 0xd000, 0x1000)

                    NORMALREGION(0x20000, REGION_SOUND1)
                        ROM_LOAD("snd_u8.256", 0x00000, 0x8000,
                                 CRC(cdf2a28d) SHA1(d4969370109b4c7f31f48a3ebd8925268caf9c44))
                            ROM_LOAD("snd_u9.256", 0x08000, 0x8000,
                                     CRC(08bd0db9) SHA1(af851b8c993649b61645a414459000c206516bec))
                                ROM_LOAD("snd_u10.256", 0x10000, 0x8000,
                                         CRC(634bc64c) SHA1(8389fda08ee7bf0e5002153cec22e219bf786993))
                                    ROM_LOAD("snd_u11.256", 0x18000, 0x8000,
                                             CRC(d4da383c) SHA1(032a4a425936d5c822fba6e46483f03a87c1a6ec))
                                        NORMALREGION(0x10000, REGION_CPU2)
                                            ROM_COPY(REGION_SOUND1, 0x0000, 0x8000, 0x8000) ROM_END
...
/ Future Queen
/-------------------------------*/
    ROM_START(futrquen) NORMALREGION(0x10000, REGION_CPU1)
        ROM_LOAD("mpu_u2.bin", 0xc000, 0x4000, CRC(bc66b636) SHA1(65f3e6461a1eca8542bbbc5b8c7cd1fca1b3011f))
            ROM_COPY(REGION_CPU1, 0xc000, 0x1000, 0x0800) ROM_COPY(REGION_CPU1, 0xd000, 0x1800, 0x0800)
                ROM_COPY(REGION_CPU1, 0xc800, 0x5000, 0x0800) ROM_COPY(REGION_CPU1, 0xd800, 0x5800, 0x0800)
                    ROM_COPY(REGION_CPU1, 0xe000, 0x9000, 0x0800) ROM_COPY(REGION_CPU1, 0xf000, 0x9800, 0x0800)
                        ROM_COPY(REGION_CPU1, 0xe800, 0xd000, 0x0800) ROM_COPY(REGION_CPU1, 0xf800, 0xd800, 0x0800)

                            NORMALREGION(0x20000, REGION_SOUND1)
                                ROM_LOAD("snd_u8.bin", 0x00000, 0x8000,
                                         CRC(3d254d89) SHA1(2b4aa3387179e2c0fbf18684128761d3f778dcb2))
                                    ROM_LOAD("snd_u9.bin", 0x08000, 0x8000,
                                             CRC(9560f2c3) SHA1(3de6d074e2a3d3c8377fa330d4562b2d266bbfff))
                                        ROM_LOAD("snd_u10.bin", 0x10000, 0x8000,
                                                 CRC(70f440bc) SHA1(9fa4d33cc6174ce8f43f030487171bfbacf65537))
                                            ROM_LOAD("snd_u11.bin", 0x18000, 0x8000,
                                                     CRC(71d98d17) SHA1(9575b80a91a67b1644e909f70d364e0a75f73b02))
                                                NORMALREGION(0x10000, REGION_CPU2)
                                                    ROM_COPY(REGION_SOUND1, 0x0000, 0x8000, 0x8000) ROM_END
...
static MEMORY_READ_START(snd_readmem_f1gp){0x0000, 0x001f, m6803_internal_registers_r}, {0x0080, 0x00ff, MRA_RAM},
    {0x8000, 0xffff, MRA_BANKNO(2)},
    MEMORY_END

    static MEMORY_WRITE_START(snd_writemem_f1gp){0x0000, 0x001f, m6803_internal_registers_w},
    {0x0080, 0x00ff, MWA_RAM}, {0x8000, 0xffff, bank_w_f1gp},
    MEMORY_END

    static PORT_WRITE_START(snd_writeport_f1gp){M6803_PORT1, M6803_PORT1, dac_w_f1gp},
...
BY35_INPUT_PORTS_START(f1gp, 3)
BY35_INPUT_PORTS_END CORE_GAMEDEFNV(f1gp, "F1 Grand Prix", 1987, "Nuova Bell Games", f1gp, 0)
...
static MEMORY_READ_START(snd_readmem_toppin){0x0000, 0x001f, m6803_internal_registers_r}, {0x0080, 0x00ff, MRA_RAM},
    {0x8000, 0xbfff, MRA_BANKNO(1)}, {0xc000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(snd_writemem_toppin){0x0000, 0x001f, m6803_internal_registers_w},
    {0x0080, 0x00ff, MWA_RAM}, {0x8000, 0xffff, bank_w_toppin},
    MEMORY_END

    MACHINE_DRIVER_START(toppin) MDRV_IMPORT_FROM(nuova)
        MDRV_CPU_REPLACE("scpu", M6803, 1000000) // faster clock to match sound samples
    MDRV_CPU_MEMORY(snd_readmem_toppin, snd_writemem_toppin) MACHINE_DRIVER_END

    ROM_START(toppin) NORMALREGION(0x10000, REGION_CPU1)
                ...
                ROM_COPY(REGION_CPU1, 0xc800, 0x5000, 0x0800) ROM_COPY(REGION_CPU1, 0xd800, 0x5800, 0x0800)
                    ROM_COPY(REGION_CPU1, 0xe000, 0x9000, 0x0800) ROM_COPY(REGION_CPU1, 0xf000, 0x9800, 0x0800)
                        ROM_COPY(REGION_CPU1, 0xe800, 0xd000, 0x0800) ROM_COPY(REGION_CPU1, 0xf800, 0xd800, 0x0800)
                            ...
                            NORMALREGION(0x20000, REGION_SOUND1)
                                ROM_LOAD("snd_u8.bin", 0x00000, 0x8000,
                                         CRC(2cb9c931) SHA1(2537976c890ceff857b9aaf204c48ab014aad94e))
                                    ROM_LOAD("snd_u9.bin", 0x08000, 0x8000,
                                             CRC(72690344) SHA1(c2a13aa59f0c605eb616256cd288b79cceca003b))
                                        ROM_LOAD("snd_u10.bin", 0x10000, 0x8000,
                                                 CRC(bca9a805) SHA1(0deb3172b5c8fc91c4b02b21b1e3794ed7adef13))
                                            ROM_LOAD("snd_u11.bin", 0x18000, 0x8000,
                                                     CRC(513d06a9) SHA1(3785398649fde5579b5a0461b52360ef83d71323))
                                                NORMALREGION(0x10000, REGION_CPU2)
                                                    ROM_COPY(REGION_SOUND1, 0x0000, 0x8000, 0x8000) ROM_END
...
/-------------------------------*/
    ...
    static MEMORY_READ_START(snd_readmem2){0x00000, 0x0ffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(snd_writemem2) MEMORY_END
...
static WRITE_HANDLER(port_w) {
    ...
    switch (offset) {
        case 1:
            logerror("port   1 W %02x\n", data);
            break;
        case 2:
            logerror(" port  2 W %02x\n", data);
            break;
        case 3:
            logerror("  port 3 W %02x\n", data);
            break;
    }
}

static PORT_READ_START(snd_readport2){0, 3, port_r},
    PORT_END

    static PORT_WRITE_START(snd_writeport2){0, 3, port_w},
    PORT_END
...
BY35_INPUT_PORTS_START(uboat65, 3)
BY35_INPUT_PORTS_END CORE_GAMEDEFNV(uboat65, "U-Boat 65", 1988, "Nuova Bell Games", uboat, GAME_IMPERFECT_SOUND)
