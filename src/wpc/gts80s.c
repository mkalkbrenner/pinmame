...
static struct { int stream; } stream_locals;
...
static WRITE_HANDLER(gts80s_riot6530_0_ram_w) {
    ...
    pMem[0x0000] = pMem[0x0040] = pMem[0x0080] = pMem[0x00c0] = pMem[0x0100] = pMem[0x0140] = pMem[0x0180] =
        pMem[0x01c0] = pMem[0x1000] = pMem[0x1040] = pMem[0x1080] = pMem[0x10c0] = data;
}

/*--------------
/  Memory map
/---------------*/
MEMORY_READ_START(GTS80S_readmem){0x0000, 0x01ff, MRA_RAM}, {0x0200, 0x03ff, riot6530_0_r}, {0x0400, 0x0fff, MRA_ROM},
    {0x1000, 0x10ff, MRA_RAM}, {0xf800, 0xffff, MRA_ROM},
    MEMORY_END

    MEMORY_WRITE_START(GTS80S_writemem){0x0000, 0x01ff, gts80s_riot6530_0_ram_w},
    {0x0200, 0x03ff, riot6530_0_w}, {0x0400, 0x0fff, MWA_ROM}, {0x1000, 0x10ff, gts80s_riot6530_0_ram_w},
    {0xf800, 0xffff, MWA_ROM},
    MEMORY_END

    static void GTS80S_Update(
        int num, INT16* const buffer,
        int length) // El Dorado City of Gold, Black Hole (Sound Only), Volcano (Sound Only), Panthera, etc
{
    ...
    while (length) {
        filter_insert(GTS80S_locals.filter_f, GTS80S_locals.filter_state,
                      GTS80S_locals.buffer[i] * (float)(1.0 / 32768.0));
...
void
gts80s_init(struct sndbrdData* brdData) {
    int i;
    UINT8* pMem;
    ...
    /* init dips */
    GTS80S_locals.dips = ((core_getDip(5) & 0x01) ? 0x00 : 0x80) | /* S1: Sound/Tones        */
...
static int
s80s_sh_start(const struct MachineSound* msound) {
...
const struct sndbrdIntf gts80sIntf = {
    "GTS80",       gts80s_init, gts80s_exit, gts80s_diag, gts80s_data_w,
    gts80s_data_w, NULL,        NULL,        NULL,        SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
...
MDRV_CPU_ADD_TAG(
    "scpu", M6502,
    904977.376) //!! somewhere between 893000 - 1000000, maybe even make this dependent on release date of game?? (earlier = higher)
...
MDRV_CPU_REPLACE(
    "scpu", M6502,
    961538.462) //!! somewhere between 680000 - 895000, but weirdly 960kHz also sounds good for some games?!?
MACHINE_DRIVER_END
...
static READ_HANDLER(GTS80SS_riot3a_r) { return GTS80SS_locals.riot3a; }
...
static READ_HANDLER(GTS80SS_riot6532_3_ram_r) { return GTS80SS_locals.pRIOT6532_3_ram[offset % 0x80]; }
...
/  Memory map
/---------------*/
MEMORY_READ_START(GTS80SS_readmem){0x0000, 0x01ff, GTS80SS_riot6532_3_ram_r}, {0x0200, 0x0fff, riot6532_3_r},
    {0x4000, 0x4fff, GTS80SS_ext_board_1_r}, {0x5000, 0x5fff, GTS80SS_ext_board_2_r},
    {0x6000, 0x6fff, GTS80SS_ext_board_3_r}, {0x7000, 0x7fff, MRA_ROM}, {0x8000, 0x81ff, GTS80SS_riot6532_3_ram_r},
    {0x8200, 0x8fff, riot6532_3_r}, {0xc000, 0xcfff, GTS80SS_ext_board_1_r}, {0xd000, 0xdfff, GTS80SS_ext_board_2_r},
    {0xe000, 0xefff, GTS80SS_ext_board_3_r}, {0xf000, 0xffff, MRA_ROM},
    MEMORY_END

    MEMORY_WRITE_START(GTS80SS_writemem){0x0000, 0x01ff, GTS80SS_riot6532_3_ram_w},
    {0x0200, 0x0fff, riot6532_3_w}, {0x1000, 0x1fff, GTS80SS_da1_latch_w}, {0x2000, 0x2fff, GTS80SS_vs_latch_w},
    {0x3000, 0x3fff, GTS80SS_da2_latch_w}, {0x4000, 0x4fff, GTS80SS_ext_board_1_w},
    {0x5000, 0x5fff, GTS80SS_ext_board_2_w}, {0x6000, 0x6fff, GTS80SS_ext_board_3_w},
    {0x7000, 0x7fff, empty_w}, // the soundboard does fake writes to the ROM area (used for a delay function)
    {0x8000, 0x81ff, GTS80SS_riot6532_3_ram_w}, {0x8200, 0x8fff, riot6532_3_w}, {0x9000, 0x9fff, GTS80SS_da1_latch_w},
    {0xa000, 0xafff, GTS80SS_vs_latch_w}, {0xb000, 0xbfff, GTS80SS_da2_latch_w},
    {0xc000, 0xcfff, GTS80SS_ext_board_1_w}, {0xd000, 0xdfff, GTS80SS_ext_board_2_w},
    ...
    static void GTS80SS_Update(int num, INT16* const buffer,
                               int length) // Mars - God of War, Black Hole, Haunted House, etc
{
    ...
    if (length
        > DAC_SAMPLE_RATE
              / 384) // there was quite some time since the last sound update? -> we're entering or leaving silence
    {
        if (GTS80SS_locals.last_sound
            != 0) // clock did not update next_value since the last update -> fade to silence (resolves clicks and simulates real DAC kinda)
        {
    ...
    } else // clock does the update (or the sound buffer catches up) -> fill buffer with curr_value, then next_value
    {
        ...
        for (i = 0; i < length; i++, t += diff)
            buffer_f[i] =
                (t < GTS80SS_locals.last_clock)
                    ? GTS80SS_locals.curr_value
                    : GTS80SS_locals
                          .next_value; // lerping between the values leads to noise for whatever reason, so raw values only!
    }
...
/---------------*/
void
gts80ss_init(struct sndbrdData* brdData) {
    int i;
...
static int
s80ss_sh_start(const struct MachineSound* msound) {
    if (Machine->gamedrv->flags & GAME_NO_SOUND) {
        return -1;
    }
...
struct VOTRAXSC01interface GTS80SS_votrax_sc01_interface = {
    1, /* 1 chip */
    {100},
    /* master volume */ // OLD_VOTRAX 75
    {720000},
    /* initial sampling frequency */ //!! or ~895kHz? or MAME astrocde's 756000? // OLD_VOTRAX: 7000
    {&GTS80SS_nmi}                   /* set NMI when busy signal get's low */
};

const struct sndbrdIntf gts80ssIntf = {
    "GTS80SS",      gts80ss_init, gts80ss_exit, gts80ss_diag, gts80ss_data_w,
    gts80ss_data_w, NULL,         NULL,         NULL,         SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
...
static WRITE_HANDLER(sp0250_latch) { GTS80BS_locals.sp0250_latch = data; }

// Latch data for AY chips
static WRITE_HANDLER(s80bs_ay8910_latch_w) { GTS80BS_locals.ay_latch = data; }

//NMI Timer - Setup the next frequency for the timer to fire, and Trigger an NMI if enabled
static void
nmi_callback(int param) {
...
WRITE_HANDLER(s80bs_sh_w) {
    ...
    if (core_gameData->hw.soundBoard & SNDBRD_GTS3) {
        ...
        clear_irq =
            ((data & 0x0f)
             == 0x0f); /* interrupt trigered by four low bits (not all 1's) - Not sure if this is TRUE, comes from the MAME driver*/
...
static READ_HANDLER(s80bs1_sound_input_r) {
    ...
    if (GTS80BS_locals.speechboard_drq)
        data |= 0x80;
    return data;
}

//Common to All Generations - Set NMI Timer Enable
static WRITE_HANDLER(common_sound_control_w) { GTS80BS_locals.nmi_enable = data & 0x01; }
...
static WRITE_HANDLER(s80bs_ym2151_w) { YM2151_word_0_w(GTS80BS_locals.ym2151_port, data); }
...
WRITE_HANDLER(gts80b_data_w) {
    ...
    if (!GTS80BS_locals.firstCmd) {
        GTS80BS_locals.firstCmd = 1;
        return;
    }
...
/***************************/
//Y-CPU
MEMORY_READ_START(GTS80BS1_readmem){0x0000, 0x07ff, MRA_RAM}, {0x6000, 0x6000, s80bs1_sound_input_r},
    {0xa800, 0xa800, soundlatch_r}, {0xb000, 0xb000, s80bs_cause_dac_nmi_r}, /*Trigger D-CPU NMI*/
    {0xc000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(GTS80BS1_writemem){0x0000, 0x07ff, MWA_RAM},
    ...
    {0x4000, 0x4000, s80bs1_sound_control_w}, {0x8000, 0x8000, s80bs_ay8910_latch_w},
    {0xa000, 0xa000, s80bs_nmi_rate_w},      /* set Y-CPU NMI rate */
    {0xb000, 0xb000, s80bs_cause_dac_nmi_w}, /*Trigger D-CPU NMI*/
    MEMORY_END
    //D-CPU
    MEMORY_READ_START(GTS80BS1_readmem2){0x0000, 0x07ff, MRA_RAM},
    {0x8000, 0x8000, soundlatch_r},
    {0xe000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(GTS80BS1_writemem2){0x0000, 0x07ff, MWA_RAM},
    {0x4000, 0x4000, s80bs_dac_vol_w}, {0x4001, 0x4001, s80bs_dac_data_w},
    MEMORY_END
    /***************************/
    ...
    /* Y-CPU */
    /*********/
    MEMORY_READ_START(GTS80BS2_readmem){0x0000, 0x07ff, MRA_RAM},
    {0x4000, 0x4000, s80bs1_sound_input_r}, // at least Victory is using the test dip switch!
    {0x6800, 0x6800, soundlatch_r}, {0x7000, 0x7000, s80bs_cause_dac_nmi_r},
    {0x8000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(GTS80BS2_writemem){0x0000, 0x07ff, MWA_RAM},
    {0x2000, 0x2000, MWA_NOP}, {0x6000, 0x6000, s80bs_nmi_rate_w}, {0x8000, 0x8000, s80bs_ay8910_latch_w},
    {0xa000, 0xa000, s80bs1_sound_control_w}, {0xa001, 0xffff, MWA_ROM},
    MEMORY_END
    /*********/
    /* D-CPU */
    /*********/
    MEMORY_READ_START(GTS80BS2_readmem2){0x0000, 0x07ff, MRA_RAM},
    {0x4000, 0x4000, soundlatch_r},
    {0x8000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(GTS80BS2_writemem2){0x0000, 0x07ff, MWA_RAM},
    {0x8000, 0x8000, s80bs_dac_vol_w}, {0x8001, 0x8001, s80bs_dac_data_w},
    MEMORY_END
    ...
    /* Y-CPU */
    /*********/
    MEMORY_READ_START(GTS80BS3_yreadmem){0x0000, 0x07ff, MRA_RAM},
    {0x6800, 0x6800, soundlatch_r}, {0x7000, 0x7000, s80bs_cause_dac_nmi_r},
    {0x8000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(GTS80BS3_ywritemem){0x0000, 0x07ff, MWA_RAM},
    {0x4000, 0x4000, s80bs_ym2151_w}, {0x6000, 0x6000, s80bs_nmi_rate_w}, {0xa000, 0xa000, s80bs3_sound_control_w},
    MEMORY_END
    /*********/
    /* D-CPU */
    /*********/
    MEMORY_READ_START(GTS80BS3_dreadmem){0x0000, 0x07ff, MRA_RAM},
    {0x4000, 0x4000, soundlatch_r},
    {0x8000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(GTS80BS3_dwritemem){0x0000, 0x07ff, MWA_RAM},
    {0x8000, 0x8000, s80bs_dac_vol_w},
    {0x8001, 0x8001, s80bs_dac_data_w}, MEMORY_END MEMORY_WRITE_START(GTS80BS3_d2writemem){0x0000, 0x07ff, MWA_RAM},
    {0x8000, 0x8000, s80bs_dac2_vol_w}, {0x8001, 0x8001, s80bs_dac2_data_w},
    MEMORY_END
...
static struct AY8910interface GTS80BS_ay8910Int = {2,        /* 2 chips */
                                                   2000000,  /* 2 MHz */
                                                   {25, 25}, /* Volume */
                                                   {0},      {0}, {0}, {0}};

static struct YM2151interface GTS80BS_ym2151Int = {1,       /* 1 chip */
                                                   4000000, /* 4 MHz */
...
static WRITE_HANDLER(sound_control_w) {
	...
	*/
    ...
    if (offset && GTS80BS_locals.enable_w && !hold_enable_w)
        oki6295_w();
}

// sound control with OKI chip
static WRITE_HANDLER(sound_control_oki) { sound_control_w(1, data); }
...
/* Y-CPU */
/*********/
MEMORY_READ_START(GTS3_yreadmem){0x0000, 0x07ff, MRA_RAM}, {0x6800, 0x6800, s3_soundlatch_y},
    {0x7000, 0x7000, s80bs_cause_dac_nmi_r},
    {0x8000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(GTS3_ywritemem){0x0000, 0x07ff, MWA_RAM},
    {0x4000, 0x4000, s80bs_ym2151_w}, {0x6000, 0x6000, s80bs_nmi_rate_w}, {0x7000, 0x7000, s80bs_cause_dac_nmi_w},
    {0x7800, 0x7800, u2latch_w}, {0x8000, 0x8000, MWA_NOP},
    {0xa000, 0xa000, sound_control_w}, MEMORY_END MEMORY_WRITE_START(GTS3_ywritemem_oki){0x0000, 0x07ff, MWA_RAM},
    {0x4000, 0x4000, s80bs_ym2151_w}, {0x6000, 0x6000, s80bs_nmi_rate_w}, {0x7000, 0x7000, s80bs_cause_dac_nmi_w},
    {0x7800, 0x7800, u2latch_w}, {0x8000, 0x8000, MWA_NOP}, {0xa000, 0xa000, sound_control_oki},
    MEMORY_END

    /*********/
    /* D-CPU */
    /*********/
    MEMORY_READ_START(GTS3_dreadmem){0x0000, 0x07ff, MRA_RAM},
    {0x4000, 0x4000, s3_soundlatch_d},
    {0x8000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(GTS3_dwritemem){0x0000, 0x07ff, MWA_RAM},
    {0x8000, 0x8000, s80bs_dac_vol_w}, {0x8001, 0x8001, s80bs_dac_data_w},
    MEMORY_END
...
static struct OKIM6295interface GTS3_okim6295_interface = {1,                   /* 1 chip */
...
// Init
void
gts80b_init(struct sndbrdData* brdData) {
...
const struct sndbrdIntf gts80bIntf = {
    "GTS80B",      gts80b_init, gts80b_exit, NULL, gts80b_data_w,
    gts80b_data_w, NULL,        NULL,        NULL, 0 //SNDBRD_NODATASYNC|SNDBRD_NOCTRLSYNC
};
...
// Latch data for AY chips
WRITE_HANDLER(techno_ay8910_latch_w) { techno_locals.ay_latch = data; }

//NMI Timer - Setup the next frequency for the timer to fire, and Trigger an NMI if enabled
static void
techno_nmi_callback(int param) {
    //Reset the timing frequency
    double interval;
    int cl1, cl2;
...
WRITE_HANDLER(techno_cause_dac_nmi_w) { cpu_set_nmi_line(techno_locals.brdData.cpuNo + 1, PULSE_LINE); }
...
WRITE_HANDLER(techno_sh_w) {
    ...
    if (~data & 0x40)
        cpu_set_irq_line(techno_locals.brdData.cpuNo + 2, TMS7000_IRQ1_LINE, ASSERT_LINE);
}
...
READ_HANDLER(techno_sound_input_r) {
    int data = 0x40;
    if (techno_locals.speechboard_drq)
        data |= 0x80;
    return data;
}
...
// Init
static void
tsns_init(struct sndbrdData* brdData) {
...
*/
WRITE_HANDLER(tms_portb_w) { cpu_setbank(1, techno_locals.brdData.romRegion + (0x10000) + ((data & 2) >> 1) * 0x8000); }
...
//Speechboard IRQ callback, will be set to 1 while speech is busy..
static void
techno_speechboard_drq_w(int level) {
    techno_locals.speechboard_drq = (level == ASSERT_LINE);
}

//Latch data to the SP0250
static WRITE_HANDLER(techno_sp0250_latch) { techno_locals.sp0250_latch = data; }
...
static WRITE_HANDLER(DAC_TMS_w) { DAC_data_w(1, data); }

static void
tsns_diag(int button) {
...
//6502 #1 CPU
MEMORY_READ_START(m6502_readmem){0x0000, 0x07ff, MRA_RAM}, {0x6000, 0x6000, techno_sound_input_r},
    {0xa800, 0xa800, techno_snd_r}, {0xb000, 0xb000, techno_cause_dac_nmi_r}, /* Trigger D-CPU NMI */
    {0xe000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(m6502_writemem){0x0000, 0x07ff, MWA_RAM},
    {0x2000, 0x2000, techno_sp0250_latch}, /* speech chip. The game sends strings */
                                           /* of 15 bytes (clocked by 4000). The chip also */
                                           /* checks a DATA REQUEST bit in 6000. */
    {0x4000, 0x4000, techno_sound_control_w}, {0x8000, 0x8000, techno_ay8910_latch_w},
    {0xa000, 0xa000, techno_nmi_rate_w},      /* set Y-CPU NMI rate */
    {0xb000, 0xb000, techno_cause_dac_nmi_w}, /* Trigger D-CPU NMI */
    MEMORY_END

    //6502 #2 CPU
    MEMORY_READ_START(m6502_b_readmem){0x0000, 0x07ff, MRA_RAM},
    {0x8000, 0x8000, techno_b_snd_r},
    {0xe000, 0xffff, MRA_ROM}, MEMORY_END MEMORY_WRITE_START(m6502_b_writemem){0x0000, 0x07ff, MWA_RAM},
    {0x4000, 0x4000, techno_dac_vol_w}, {0x4001, 0x4001, techno_dac_data_w},
    MEMORY_END

    //TMS7000 CPU
    static MEMORY_READ_START(tms_readmem){0x0000, 0x7fff, MRA_BANK1}, /* ROM BANK */
    {0x8000, 0xffff, MRA_ROM}, MEMORY_END static MEMORY_WRITE_START(tms_writemem){0x8000, 0x8000, DAC_TMS_w}, /* DAC */
    MEMORY_END

    static PORT_READ_START(tms_readport){TMS7000_PORTA, TMS7000_PORTA, tms_porta_r},
    {TMS7000_PORTB, TMS7000_PORTB, tms_portb_r}, {TMS7000_PORTA, TMS7000_PORTC, tms_portc_r},
    {TMS7000_PORTB, TMS7000_PORTD, tms_portd_r},
    PORT_END static PORT_WRITE_START(tms_writeport){TMS7000_PORTA, TMS7000_PORTA, tms_porta_w},
    {TMS7000_PORTB, TMS7000_PORTB, tms_portb_w}, {TMS7000_PORTA, TMS7000_PORTC, tms_portc_w},
    {TMS7000_PORTB, TMS7000_PORTD, tms_portd_w},
    PORT_END

    MACHINE_DRIVER_START(techno) MDRV_CPU_ADD(M6502, 1000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(m6502_readmem, m6502_writemem) MDRV_SOUND_ADD(AY8910, techno_ay8910Int)
            MDRV_SOUND_ADD(SP0250, techno_sp0250_interface)

                MDRV_CPU_ADD(M6502, 1000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
                    MDRV_CPU_MEMORY(m6502_b_readmem, m6502_b_writemem) MDRV_SOUND_ADD(DAC, techno_6502dacInt)

    //MDRV_CPU_ADD(TMS7000, 4000000)
    MDRV_CPU_ADD(TMS7000, 1500000) //!! Sounds much better at 1.5Mhz than 4Mhz
    MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(tms_readmem, tms_writemem)
        MDRV_CPU_PORTS(tms_readport, tms_writeport) MACHINE_DRIVER_END
