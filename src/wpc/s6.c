...
static WRITE_HANDLER(s6_alpha_w) {
    s6locals.segments[s6locals.alphapos].w |= s6locals.pseg[s6locals.alphapos].w = core_bcd2seg[data >> 4];
    s6locals.segments[20 + s6locals.alphapos].w |= s6locals.pseg[20 + s6locals.alphapos].w = core_bcd2seg[data & 0x0f];
}

static WRITE_HANDLER(s6_pa_w) {
    s6locals.a0 = data;
    if (s6locals.ca20)
        s6locals.diagnosticLed = ((data >> 3) & 0x02) | ((data >> 5) & 0x01);
...
setSSSol(int data, int solNo) {
    ...
    if (s6locals.ssEn & (~data & 1)) {
        coreGlobals.pulsedSolState |= bit;
        s6locals.solenoids |= bit;
    } else
        coreGlobals.pulsedSolState &= ~bit;
}
...
static MACHINE_RESET(s6) { pia_reset(); }

static MACHINE_STOP(s6) { sndbrd_0_exit(); }
...
/------------------------------------*/
static MEMORY_READ_START(s6_readmem){0x0000, 0x01ff, MRA_RAM},
    ...
    static MEMORY_WRITE_START(s6_writemem){0x0000, 0x0100, MWA_RAM},
    {0x0100, 0x01ff, s6_CMOS_w, &s6_CMOS}, {0x2200, 0x2203, pia_w(S6_PIA3)}, /*Solenoids + Sound Commands*/
    ...
    MACHINE_DRIVER_START(s6) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(s6, s6, s6)
        MDRV_CPU_ADD(M6808, 3579545. / 4.) // MAME: 3580000
    MDRV_CPU_MEMORY(s6_readmem, s6_writemem) MDRV_CPU_VBLANK_INT(s6_vblank, 1) MDRV_CPU_PERIODIC_INT(s6_irq, S6_IRQFREQ)
        MDRV_NVRAM_HANDLER(s6) MDRV_DIPS(8 + 16) MDRV_SWITCH_UPDATE(s6) MDRV_DIAGNOSTIC_LEDV(2) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(s6S) MDRV_IMPORT_FROM(s6) MDRV_IMPORT_FROM(wmssnd_s67s) MACHINE_DRIVER_END
