...
#define SAM_COMPORTS                                                                                                   \
    PORT_START /* 0 */ /*Switch Col. 0*/                                                                               \
        COREPORT_BITDEF(0x0010, IPT_TILT, KEYCODE_INSERT) COREPORT_BIT(0x0020, "Slam Tilt", KEYCODE_HOME)              \
            COREPORT_BIT(0x0040, "Ticket Notch", KEYCODE_K) COREPORT_BIT(0x0080, "Dedicated Sw#20", KEYCODE_L)         \
                COREPORT_BIT(0x0100, "Back", KEYCODE_7) COREPORT_BIT(0x0200, "Minus", KEYCODE_8)                       \
                    COREPORT_BIT(0x0400, "Plus", KEYCODE_9)                                                            \
                        COREPORT_BIT(0x0800, "Select", KEYCODE_0) /*Switch Col. 2*/                                    \
        COREPORT_BIT(0x8000, "Start Button", KEYCODE_1)                                                                \
            COREPORT_BIT(0x4000, "Tournament Start", KEYCODE_2) /*Switch Col. 9*/                                      \
        COREPORT_BITDEF(0x0001, IPT_COIN1, KEYCODE_3) COREPORT_BITDEF(0x0002, IPT_COIN2, KEYCODE_4)                    \
            COREPORT_BITDEF(0x0004, IPT_COIN3, KEYCODE_5) COREPORT_BITDEF(0x0008, IPT_COIN4, KEYCODE_6) /*None*/       \
        COREPORT_BITTOG(0x1000, "Coin Door", KEYCODE_END) PORT_START                                    /* 1 */        \
            COREPORT_DIPNAME(0x001f, 0x0000, "Country") COREPORT_DIPSET(0x0000, "USA") COREPORT_DIPSET(                \
                0x000d, "Australia") COREPORT_DIPSET(0x0001, "Austria") COREPORT_DIPSET(0x0002, "Belgium")             \
                COREPORT_DIPSET(0x0003, "Canada 1") COREPORT_DIPSET(0x001a, "Canada 2") COREPORT_DIPSET(               \
                    0x001c, "China") COREPORT_DIPSET(0x0013, "Chuck E. Cheese") COREPORT_DIPSET(0x0016, "Croatia")     \
                    COREPORT_DIPSET(0x0009, "Denmark") COREPORT_DIPSET(0x0005, "Finland") COREPORT_DIPSET(             \
                        0x0006, "France") COREPORT_DIPSET(0x0007, "Germany") COREPORT_DIPSET(0x000f, "Greece")         \
                        COREPORT_DIPSET(0x0008, "Italy") COREPORT_DIPSET(0x001b, "Lithuania")                          \
                            COREPORT_DIPSET(0x0015, "Japan") COREPORT_DIPSET(0x0017, "Middle East") COREPORT_DIPSET(   \
                                0x0004, "Netherlands") COREPORT_DIPSET(0x0010, "New Zealand")                          \
                                COREPORT_DIPSET(0x000a, "Norway") COREPORT_DIPSET(0x0011, "Portugal")                  \
                                    COREPORT_DIPSET(0x0019, "Russia") COREPORT_DIPSET(0x0014, "South Africa")          \
                                        COREPORT_DIPSET(0x0012, "Spain") COREPORT_DIPSET(0x000b, "Sweden")             \
                                            COREPORT_DIPSET(0x000c, "Switzerland") COREPORT_DIPSET(0x0018, "Taiwan")   \
                                                COREPORT_DIPSET(0x000e, "U.K.") COREPORT_DIPSET(0x001d,                \
                                                                                                "Unknown (00011101)")  \
                                                    COREPORT_DIPSET(0x001e, "Unknown (00011110)") COREPORT_DIPSET(     \
                                                        0x001f, "Unknown (00011111)")                                  \
                                                        COREPORT_DIPNAME(0x0020, 0x0000, "Dip #6")                     \
                                                            COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0020, "1")  \
...
static void
samsnd_init(struct sndbrdData* brdData) {}

//Sound Interface
const struct sndbrdIntf samIntf = {"SAM", samsnd_init, NULL, NULL, man3_w, scmd_w, NULL, NULL, NULL, SNDBRD_NODATASYNC};

static void
sam_sh_update(int num, INT16* buffer[2], int length) {
    if (length > 0) {
        int ii, channel;
...
static int
sam_sh_start(const struct MachineSound* msound) {
    ...
    return stream_init_multi(2, stream_name, volume, SAM_SOUNDFREQ, 0, sam_sh_update) < 0;
}
...
static READ32_HANDLER(samswitch_r) {
    ...
    if (offset >= 0x1100000 && offset <= 0x1100006) {
        ...
        switch (offset) {
            ...
            case 0x1100000: {
                ...
                data = MAKE16BIT(coreGlobals.swMatrix[2 + samlocals.sw_stb * 2],
                                 coreGlobals.swMatrix[1 + samlocals.sw_stb * 2]);
                data ^= 0xffff;
                break;
            }
    ...
    if (logit)
        LOG(("%08x: reading from: %08x = %08x (mask=%x)\n", activecpu_get_pc(), offset, data, mask));
...
static READ32_HANDLER(samxilinx_r) {
    ...
    if (mask == 0xff00) {
        ...
        //                                                   u1 = power switch
        data = /*(0x1C & 0xFB)*/ (7 << 3) | (samlocals.zc << 2)
               | (samlocals.coindoor != 0 ? 3 : 0); //!! which one is correct?
        data = data << 8;
    }
    if (logit)
...
/*****************************/
static MEMORY_READ32_START(sam_readmem){0x00000000, 0x000FFFFF, MRA32_RAM}, //Boot RAM
    ...
    {0x05000000, 0x057FFFFF, MRA32_RAM}, {0x05800000, 0x05FFFFFF, MRA32_BANK1}, {0x06000000, 0x067FFFFF, MRA32_RAM},
    {0x06800000, 0x06FFFFFF, MRA32_BANK1}, {0x07000000, 0x077FFFFF, MRA32_RAM}, {0x07800000, 0x07FFFFFF, MRA32_BANK1},
    MEMORY_END
...
static WRITE32_HANDLER(sam_io2_w) {
    ...
    LOG(("%08x: IO2 output to %05x=%02x\n", activecpu_get_pc(), offset,
         data)); //!! find out what is what for LE support (but even normal TWD uses it (and more??))
}
        ...
        switch (offset) {
                ...
                if (++samlocals.dataWrites[0] == 1) {
                    ...
                    for (ii = 0; ii <= 7; ii++) {
                        ...
                        if (ii + 9 >= SAM_SOL_FLIPSTART && ii + 9 <= SAM_SOL_FLIPEND) {
                            UINT8 value =
                                core_calc_modulated_light(samlocals.solenoidbits[ii + 8], 24,
                                                          &coreGlobals.modulatedSolenoids[CORE_MODSOL_PREV][ii + 8]);
            ...
            case 0x02400026:
                ...
                if (core_gameData->hw.gameSpecific1 & SAM_GAME_WPT) {
                    if (samlocals.miniDMDCol
                        == 1) // the DMD row is decided by the bits set in the first two columns that are written to each column!
                        samlocals.miniDMDRow =
                            rowMap[core_BitColToNum((data & 0x7F) | ((samlocals.auxdata & 0x7F) << 7))];
                    ...
                    if (data & ~samlocals.auxdata & 0x80) { // observe change low -> high to trigger the column reset
                        samlocals.miniDMDCol = 0;
                    ...
                    if ((test < 0x80) && (test >= 0)) // observe change low -> high to trigger the column reset
                    {
                        ...
                        if (options.usemodsol) // To protect the VP9 table from an onslaught of extra lights.
                        {
                            ...
                            for (i = 0; i < WOF_MINIDMD_MAX; i++) {
                                coreGlobals.RGBlamps[i + 60] = core_calc_modulated_light(
                                    samlocals.modulated_lights[i], 8, &samlocals.modulated_lights_prev_levels[i]);
                            }
                        }
                    }
                    ...
                    if (samlocals.WOF_minidmdflag) {
                        ...
                        if (samlocals.miniDMDCol > 1 && samlocals.miniDMDCol < 7) {
                            ...
                            {
                                core_update_modulated_light(
                                    &samlocals.modulated_lights[35 * samlocals.miniDMDRow
                                                                + (samlocals.miniDMDCol - 2) * 7 + i],
                                    data & (1u << (6 - i)));
                            }
                        }
                    } else {
            ...
            case 0x0240002B:
                ...
                if (((core_gameData->hw.gameSpecific1 & SAM_GAME_AUXSOL12) && (~data & 0x20))
                    ...
                    int ii;
                    for (ii = 0;
                         ii <= ((core_gameData->hw.gameSpecific1 & (SAM_GAME_AUXSOL8 | SAM_GAME_ACDC_FLAMES)) ? 7 : 5);
                         ii++) {
                        core_update_modulated_light(&samlocals.solenoidbits[ii + CORE_FIRSTCUSTSOL - 1],
                                                    samlocals.auxdata & (1u << ii));
                    }
                }
                if (((core_gameData->hw.gameSpecific1 & SAM_GAME_AUXSOL12) && (~data & 0x10))) {
                    int ii;
                    for (ii = 0; ii <= 5; ii++) {
                        core_update_modulated_light(&samlocals.solenoidbits[ii + CORE_FIRSTCUSTSOL + 8 - 1],
                                                    samlocals.auxdata & (1u << ii));
                    }
                }
                ...
                if (((core_gameData->hw.gameSpecific1 & SAM_GAME_METALLICA_MAGNET) && (~data & 0x08))) {
                    core_update_modulated_light(&samlocals.solenoidbits[6 + CORE_FIRSTCUSTSOL - 1],
                                                samlocals.auxdata & 0x80);
                    core_update_modulated_light(&samlocals.solenoidbits[7 + CORE_FIRSTCUSTSOL - 1],
                                                samlocals.auxdata & 0x40);
                }
...
static MEMORY_WRITE32_START(sam_writemem){0x00000000, 0x000FFFFF, MWA32_RAM, &sam_page0_ram}, // Boot RAM
    ...
    {0x05000000, 0x057FFFFF, MWA32_RAM}, {0x05800000, 0x05FFFFFF, MWA32_RAM}, {0x06000000, 0x067FFFFF, MWA32_RAM},
    {0x06800000, 0x06FFFFFF, MWA32_RAM}, {0x07000000, 0x077FFFFF, MWA32_RAM}, {0x07800000, 0x07FFFFFF, MWA32_RAM},
    MEMORY_END
...
static PORT_READ32_START(sam_readport){0x00, 0xFF, sam_port_r},
    PORT_END

    static PORT_WRITE32_START(sam_writeport){0x00, 0xFF, sam_port_w},
    PORT_END
...
static void
sam_transmit_serial(int usartno, data8_t* data, int size) {
#if 0 //def _DEBUG
	int i;
	for (i = 0; i < size; i++)
	{
        ...
        if (samlocals.led_row == -1) {
            ...
            if ((*data) == 0x80 && samlocals.prev_ch1 == 0x41) {
                if (samlocals.serchar_waiting == 2) {
                    memcpy(&samlocals.ext_leds[samlocals.target_row * samlocals.leds_per_string],
                           &samlocals.tmp_leds[0], samlocals.leds_per_string);
                }
            ...
            // Walking Dead LE
            if (((*data) == 0x80 || (*data) == 0xa0) && samlocals.prev_ch1 == 0x23
                && (samlocals.prev_ch2 == 0x83 || samlocals.prev_ch2 == 0x88)) {
                ...
                samlocals.leds_per_string = samlocals.prev_ch1;
                samlocals.led_row = (samlocals.prev_ch2 == 0x83) ? ((*data) == 0x80) ? 0 : 2
                                    : ((*data) == 0x80)          ? 1
                                                                 : 3;
            }
            // AC/DC or Metallica
            ...
            const int count = size > (samlocals.leds_per_string - samlocals.led_col)
                                  ? samlocals.leds_per_string - samlocals.led_col
                                  : size;
            int i;
            for (i = 0; i < count; i++) {
...
static INTERRUPT_GEN(sam_vblank) {
    ...
    switch (core_gameData->hw.gameSpecific1) {
        case SAM_GAME_TRON: {
            for (i = 0; i < 6; i++) {
                coreGlobals.RGBlamps[i + 20] = core_calc_modulated_light(samlocals.modulated_lights[i], 15,
                                                                         &samlocals.modulated_lights_prev_levels[i]);
            }
            break;
        }
        case SAM_GAME_WOF:
        ...
        default:
            ...
            for (i = 0; i < core_gameData->hw.lampCol - 2; i++) {
                coreGlobals.lampMatrix[10 + i] =
                    (coreGlobals.RGBlamps[8 * i] ? 1 : 0) | (coreGlobals.RGBlamps[8 * i + 1] ? 2 : 0)
    ...
    /*-- solenoids --*/
    {
        ...
        for (i = 0; i < CORE_MODSOL_MAX; i++) {
            ...
            value = core_calc_modulated_light(samlocals.solenoidbits[i], 24,
                                              &coreGlobals.modulatedSolenoids[CORE_MODSOL_PREV][i]);
...
static INTERRUPT_GEN(sam_irq) { at91_fire_irq(AT91_FIQ_IRQ); }

/*********************************************/
/* S.A.M. Generation #1 - Machine Definition */
/*********************************************/
static MACHINE_DRIVER_START(sam1) MDRV_IMPORT_FROM(PinMAME) MDRV_SWITCH_UPDATE(sam)
    MDRV_CPU_ADD(AT91, SAM_CPUFREQ) // AT91R40008
    MDRV_CPU_MEMORY(sam_readmem, sam_writemem) MDRV_CPU_PORTS(sam_readport, sam_writeport)
        MDRV_CPU_VBLANK_INT(sam_vblank, 1) MDRV_CPU_PERIODIC_INT(sam_irq, SAM_IRQFREQ)
            MDRV_CORE_INIT_RESET_STOP(sam, sam1, sam) MDRV_DIPS(8) MDRV_NVRAM_HANDLER(sam)
                MDRV_TIMER_ADD(sam_timer, SAM_ZC_FREQ) MDRV_SOUND_ADD(CUSTOM, samCustInt)
                    MDRV_SOUND_ATTRIBUTES(SOUND_SUPPORTS_STEREO) MDRV_DIAGNOSTIC_LEDH(2) MACHINE_DRIVER_END

    /*********************************************/
    /* S.A.M. Generation #2 - Machine Definition */
    /*********************************************/
    static MACHINE_DRIVER_START(sam2) MDRV_IMPORT_FROM(sam1)
        MDRV_CORE_INIT_RESET_STOP(sam, sam2, sam) MACHINE_DRIVER_END
...
--*/
    ...
    for (ii = 0; ii < 32; ii++) {
        ...
        for (jj = 0; jj < 128; jj++) {
            ...
            const UINT8 temp =
                (RAM2 & mix)
                | (RAM1 & (mix ^ 0xF)); //!! is this correct or is mix rather a multiplier/ratio/alphavalue??
            if ((mix != 0xF)
                && (mix
                    != 0x0)) //!! happens e.g. in POTC in extra ball explosion animation: RAM1 values triggering this: 223, 190, 175, 31, 25, 19, 17 with RAM2 being always 0. But is this just wrong game data (as its a converted animation)?!
...
static struct core_dispLayout sam_dmd128x32[] = {{0, 0, 32, 128, CORE_DMD | CORE_DMDNOAA, (genf*)samdmd_update}, {0}};
...
SAM1_BOOTFLASH(sam1_flashb_0102, "boot_102.bin", CRC(92c93cba) SHA1(aed7ba2f988df8c95e2ad08f70409152d5caa49a),
               0x00100000)
SAM1_BOOTFLASH(sam1_flashb_0106, "boot_106.bin", CRC(fe7bcece) SHA1(775590bbd52c24950db86cc231566ba3780030d8),
               0x000e8ac8)
SAM1_BOOTFLASH(sam1_flashb_0210, "boot_210.bin", CRC(0f3fd4a4) SHA1(115d0b73c40fcdb2d202a0a9065472d216ca89e0),
               0x000f0304)
SAM1_BOOTFLASH(sam1_flashb_0230, "boot_230.bin", CRC(a4258c49) SHA1(d865edf7d1c6d2c922980dd192222dc24bc092a0),
               0x000f0624)
SAM1_BOOTFLASH(sam1_flashb_0310, "boot_310.bin", CRC(de017f82) SHA1(e4a9a818fa3f1754374cd00b52b8a087d6c442a9),
               0x00100000)
...
CORE_CLONEDEFNV(scarn9nj, scarn200, "Simpsons Kooky Carnival, The (Redemption) (V0.90 New Jersey)", 2006, "Stern", sam1,
                0)
...
static mech_tInitData mechwofWheel = {WOF_WHEEL_SOL1,
                                      WOF_WHEEL_SOL2,
                                      MECH_CIRCLE | MECH_LINEAR | MECH_TWOSTEPSOL | MECH_FAST,
                                      398,
                                      360,
                                      {{WOF_WHEEL_OPTO_SW, 358, 360}, 0}};

static void
wof_handleMech(int mech) {}

static int
wof_getMech(int mechNo) {
    return mech_getPos(mechNo);
}

static void
wof_drawMech(BMTYPE** line) {
    core_textOutf(35, 10, BLACK, "Wheel Pos   : %3d", wof_getMech(0));
}

static core_tGameData wofGameData = {GEN_SAM,
                                     sammini2_dmd128x32,
                                     {FLIP_SW(FLIP_L) | FLIP_SOL(FLIP_L), 0, SAM_8COL, 16, 0, 0, SAM_GAME_WOF, 0,
                                      sam_getSol, wof_handleMech, wof_getMech, wof_drawMech}};
...
CORE_CLONEDEF(wof, 602h, 500, "Wheel of Fortune (V6.02 Home Rom)", 2009, "Stern", sam1,
              0) // unofficial version, but apparently compiled by Stern
/*
...
CORE_CLONEDEF(bdk, 300, 294, "Batman: The Dark Knight Home Edition/Costco (V3.00)", 2010, "Stern", sam1,
              0) // Standard model in Stern terms
#ifdef SAM_INCLUDE_COLORED
