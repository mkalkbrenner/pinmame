...
static READ_HANDLER(snd300_r) {
    logerror("%04x: snd300_r adress A%c data latch %02x give %02x \n", activecpu_get_previouspc(), debugms[offset],
             snddatst300.ax[offset], snddatst300.axb[offset]);
...
static WRITE_HANDLER(snd300_wex) { sndbrd_0_ctrl_w(0, data); }
...
by35_dispStrobe(int mask) {
    ...
    for (ii = 0; digit; ii++, digit >>= 1)
        if (digit & 0x01) {
            ...
            for (jj = 0; dispMask; jj++, dispMask >>= 1)
                if (dispMask & 0x01) {
                    locals.segments[jj * 8 + ii].w |= locals.pseg[jj * 8 + ii].w =
                        locals.bcd2seg[locals.bcd[jj] & 0x0f];
#ifdef LISY_SUPPORT
    ...
    if (core_gameData->hw.gameSpecific1 & BY35GD_FAKEZERO) {
        if (locals.segments[7].w)
            locals.segments[8].w = locals.pseg[8].w = locals.bcd2seg[0];
        if (locals.segments[15].w)
            locals.segments[16].w = locals.pseg[16].w = locals.bcd2seg[0];
        if (locals.segments[23].w)
            locals.segments[24].w = locals.pseg[24].w = locals.bcd2seg[0];
        if (locals.segments[31].w)
            locals.segments[32].w = locals.pseg[32].w = locals.bcd2seg[0];
    }
}

static void
by35_lampStrobe(int board, int lampadr) {
    if (lampadr != 0x0f) {
        int lampdata = (locals.a0 >> 4) ^ 0x0f;
#ifdef LISY_SUPPORT
        if (lampdata)
            lisy35_lamp_handler(0, board, lampadr, lampdata);
#endif
...
static WRITE_HANDLER(pia0a_w) {
    ...
    if (core_gameData->hw.gameSpecific1 & BY35GD_PHASE)
        ...
        if (core_gameData->hw.lampCol > 0)
            by35_lampStrobe(1, locals.lampadr2);
    }
}
...
static READ_HANDLER(pia0b_r) {
#ifndef LISY_SUPPORT
    if (locals.a0 & 0x20)
        return core_getDip(0); // DIP#1 1-8
    if (locals.a0 & 0x40)
        return core_getDip(1); // DIP#2 9-16
    if (locals.a0 & 0x80)
        return core_getDip(2); // DIP#3 17-24
    if ((locals.hw & BY35HW_DIP4) && locals.cb20)
        return core_getDip(3); // DIP#4 25-32
#else
    int ret;
    if (locals.a0 & 0x20) {
        ret = lisy35_get_mpudips(0);
        if (ret < 0)
    ...
    if ((locals.hw & BY35HW_DIP4) && locals.cb20) {
        ret = lisy35_get_mpudips(3);
        if (ret < 0)
            return core_getDip(3);
        else
            return ret;
    }                                        // DIP#1 25-32
#endif
    {
        ...
        UINT8 sw;
        if (core_gameData->hw.gameSpecific1 & BY35GD_SWVECTOR)
            col |= (locals.b1 & 0x10) << 1;
        else
            col |= (locals.b1 & 0x80) >> 2;
...
static WRITE_HANDLER(pia0ca2_w) {
    locals.ca20 = data;
    if (data && !(core_gameData->hw.gameSpecific1 & BY35GD_ALPHA))
        by35_dispStrobe(0x0f);
}
...
#ifdef LISY_SUPPORT
        if (locals.cb21)
            lisy35_sound_handler(LISY35_SOUND_HANDLER_IS_DATA, data & 0x0f);
#endif
    }
...
static READ_HANDLER(pia1ca1_r) { return locals.ca11; }

/* PIA1:CB2-W Solenoid/Sound select */
static WRITE_HANDLER(pia1cb2_w) {
    int sb = core_gameData->hw.soundBoard; // ok
    locals.cb21 = data;
    if (((locals.hw & BY35HW_SCTRL) == 0) && ((sb & 0xff00) != SNDBRD_ST300) && (sb != SNDBRD_ASTRO)
        && (sb & 0xff00) != SNDBRD_ST100)
        // ok
...
static READ_HANDLER(pia0ca2_r) { return locals.ca20; }
...
*/
static struct pia6821_interface by35_pia[] = {{/* I:  A/B,CA1/B1,CA2/B2 */ 0, pia0b_r, PIA_UNUSED_VAL(1), pia0cb1_r,
                                               pia0ca2_r, 0,
...
static INTERRUPT_GEN(by35_irq) { pia_set_input_ca1(BY35_PIA1, locals.ca11 = !locals.ca11); }

static void
by35_zeroCross(int data) {
...
static NVRAM_HANDLER(by35) {
    core_nvram(file, read_or_write, by35_CMOS, 0x100,
               (core_gameData->gen & (GEN_STMPU100 | GEN_STMPU200)) ? 0x00 : 0xff);
#ifdef LISY_SUPPORT
    // 0 = read; 1 = write
    // inform lisy about position of pinmame nvram memory
    if (read_or_write == 0)
        lisy35_nvram_handler(by35_CMOS);
#endif
}
...
static WRITE_HANDLER(sb_ctrl_cb) { coreGlobals.diagnosticLed = (locals.diagnosticLed |= (data << 1)); }
...
/* Roms: U1: 0x1000-0x1800
	 ...
	 U6: 0x5800-0x6000
*/
static MEMORY_READ_START(by35_readmem){0x0000, 0x007f, MRA_RAM}, /* U7 128 Byte Ram*/
    ...
    static MEMORY_WRITE_START(by35_writemem){0x0000, 0x007f, MWA_RAM}, /* U7 128 Byte Ram*/
    ...
    MACHINE_DRIVER_START(by35) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(by35, by35, by35)
        MDRV_CPU_ADD_TAG("mcpu", M6800, 500000) MDRV_CPU_MEMORY(by35_readmem, by35_writemem)
            MDRV_CPU_VBLANK_INT(by35_vblank, 1) MDRV_CPU_PERIODIC_INT(by35_irq, BY35_IRQFREQ * 2)
                MDRV_NVRAM_HANDLER(by35) MDRV_DIPS(32) MDRV_SWITCH_UPDATE(by35) MDRV_DIAGNOSTIC_LEDH(1)
                    MDRV_TIMER_ADD(by35_zeroCross, BY35_ZCFREQ * 2) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(st100s) MDRV_IMPORT_FROM(by35) MDRV_IMPORT_FROM(st100) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(st100bs) MDRV_IMPORT_FROM(by35) MDRV_IMPORT_FROM(st100b) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(by35_32S) MDRV_IMPORT_FROM(by35) MDRV_IMPORT_FROM(by32) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(by35_51S) MDRV_IMPORT_FROM(by35) MDRV_IMPORT_FROM(by51) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(by35_51NS) MDRV_IMPORT_FROM(by35) MDRV_IMPORT_FROM(by51N) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(by35_56S) MDRV_IMPORT_FROM(by35) MDRV_IMPORT_FROM(by56) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(by35_61S) MDRV_IMPORT_FROM(by35) MDRV_DIAGNOSTIC_LEDH(2)
        MDRV_IMPORT_FROM(by61) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(by35_61S2) MDRV_IMPORT_FROM(by35) MDRV_DIAGNOSTIC_LEDH(3)
        MDRV_IMPORT_FROM(by61x2) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(by35_45S) MDRV_IMPORT_FROM(by35) MDRV_DIAGNOSTIC_LEDH(2)
        MDRV_IMPORT_FROM(by45) MACHINE_DRIVER_END

#ifdef MAME_DEBUG
    MACHINE_DRIVER_START(by6802_61S) MDRV_IMPORT_FROM(by35) MDRV_CPU_REPLACE("mcpu", M6802, 375000)
        MDRV_CPU_PERIODIC_INT(by35_irq, BY35_6802IRQFREQ * 2) MDRV_DIAGNOSTIC_LEDH(2)
            MDRV_IMPORT_FROM(by61) MACHINE_DRIVER_END
#endif

    MACHINE_DRIVER_START(by6802_45S) MDRV_IMPORT_FROM(by35) MDRV_CPU_REPLACE("mcpu", M6802, 375000)
        MDRV_CPU_PERIODIC_INT(by35_irq, BY35_6802IRQFREQ * 2) MDRV_DIAGNOSTIC_LEDH(2)
            MDRV_IMPORT_FROM(by45) MACHINE_DRIVER_END

    static MACHINE_DRIVER_START(st200NS) MDRV_IMPORT_FROM(by35)
        MDRV_CPU_REPLACE("mcpu", M6800, 1000000) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(st200) MDRV_IMPORT_FROM(st200NS) MDRV_IMPORT_FROM(st300) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(st200s100b) MDRV_IMPORT_FROM(st200NS) MDRV_IMPORT_FROM(st100b) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(st200v) MDRV_IMPORT_FROM(st200NS) MDRV_IMPORT_FROM(st300v) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(hnk) MDRV_IMPORT_FROM(by35) MDRV_CPU_MODIFY("mcpu") MDRV_CPU_PERIODIC_INT(NULL, 0) // no irq
    MDRV_DIPS(24) MDRV_IMPORT_FROM(hnks) MACHINE_DRIVER_END
...
static READ_HANDLER(sam4b_r) {
    ...
    if (retVal == 0x1e || retVal == 0x1d || retVal == 0x1b || retVal == 0x17 || retVal == 0x0f)
        retVal = 0xff;
...
static WRITE_HANDLER(sam5a_w) { pia_set_input_a(5, data); }

static WRITE_HANDLER(sam5b_w) {
    pia_set_input_b(5, data);
    if (data)
        locals.pseg[43 - core_BitColToNum(data)].w = locals.segments[43 - core_BitColToNum(data)].w = pia_5_porta_r(0);
}
...
static struct pia6821_interface sam_pia[] = {{/* I:  A/B,CA1/B1,CA2/B2 */ 0, sam0b_r, PIA_UNUSED_VAL(1), pia0cb1_r,
                                              pia0ca2_r, 0,
...
static MEMORY_READ_START(sam_readmem){0x0000, 0x007f, MRA_RAM}, {0x0088, 0x008b, pia_0_r}, {0x0090, 0x0093, pia_1_r},
    {0x0200, 0x02ff, MRA_RAM}, {0x1000, 0x1fff, MRA_ROM}, {0x2000, 0x207f, MRA_RAM}, {0x2104, 0x2107, pia_2_r},
    {0x2108, 0x210b, pia_3_r}, {0x2110, 0x2113, pia_4_r}, {0x2120, 0x2123, pia_5_r}, {0x2140, 0x2143, pia_6_r},
    {0x2800, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(sam_writemem){0x0000, 0x007f, MWA_RAM},
    {0x0088, 0x008b, pia_0_w}, {0x0090, 0x0093, pia_1_w}, {0x0200, 0x02ff, by35_CMOS_w, &by35_CMOS},
    {0x1000, 0x1fff, MWA_NOP}, {0x2000, 0x207f, MWA_RAM}, {0x2104, 0x2107, pia_2_w}, {0x2108, 0x210b, pia_3_w},
    {0x2110, 0x2113, pia_4_w}, {0x2120, 0x2123, pia_5_w}, {0x2140, 0x2143, pia_6_w}, {0x2800, 0xffff, MWA_NOP},
    MEMORY_END

    MACHINE_DRIVER_START(st200sam) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(sam, by35, by35)
        MDRV_CPU_ADD_TAG("mcpu", M6800, 530000) MDRV_CPU_MEMORY(sam_readmem, sam_writemem)
            MDRV_CPU_VBLANK_INT(by35_vblank, 1) MDRV_CPU_PERIODIC_INT(sam_display_irq, BY35_IRQFREQ * 2)
                MDRV_TIMER_ADD(by35_zeroCross, BY35_ZCFREQ * 2) MDRV_SWITCH_UPDATE(by35) MDRV_NVRAM_HANDLER(by35)
                    MDRV_DIPS(32) MDRV_DIAGNOSTIC_LEDH(1) MDRV_IMPORT_FROM(st300) MACHINE_DRIVER_END
