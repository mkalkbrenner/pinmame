...
static WRITE_HANDLER(irq_enable) { locals.irqenable = data; }

static WRITE_HANDLER(zc_enable) { locals.zcenable = data; }

static WRITE_HANDLER(latch_w) { locals.latch = locals.digit; }

static WRITE_HANDLER(col_w) { locals.col = (locals.col & ~(1 << offset)) | ((data & 1) << offset); }

static WRITE_HANDLER(digit_w) { locals.digit = (locals.digit & ~(1 << offset)) | ((data & 1) << offset); }

static WRITE_HANDLER(bcd_w) { locals.bcd = (locals.bcd & ~(1 << offset)) | ((data & 1) << offset); }

static WRITE_HANDLER(panel_w) {
    if (data)
        locals.segments[offset * 8 + core_BitColToNum(~locals.latch & 0xff)].w = core_bcd2seg7[0x0f ^ locals.bcd];
}
...
static WRITE_HANDLER(enable_w) { coreGlobals.solenoids = (coreGlobals.solenoids & 0xfffeffff) | (data << 16); }
...
static READ_HANDLER(sw1_r) { return coreGlobals.swMatrix[offset]; }

static READ_HANDLER(sw6_r) { return offset ? 0 : coreGlobals.swMatrix[2 + core_BitColToNum(~locals.col & 0x1f)]; }

static READ_HANDLER(dip_r) { return core_getDip(offset); }

static MEMORY_READ_START(readmem){0x0000, 0x3bff, MRA_ROM}, {0x3c00, 0x3fff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(writemem){0x3c00, 0x3fff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static PORT_WRITE_START(writeport){0x10, 0x15, snd_w},
    {0x16, 0x16, latch_w}, {0x17, 0x19, out1a_w}, {0x1a, 0x1a, enable_w}, {0x1b, 0x1f, out1b_w}, {0x20, 0x27, out2a_w},
    {0x28, 0x2f, out2b_w}, {0x30, 0x37, out3a_w}, {0x3e, 0x3e, irq_enable}, {0x3f, 0x3f, zc_enable},
    {0x40, 0x47, digit_w}, {0x48, 0x4b, bcd_w}, {0x4c, 0x50, panel_w}, {0x51, 0x55, col_w}, {0x58, 0x5f, out5b_w},
    {0x60, 0x67, out6a_w}, {0x68, 0x6f, out6b_w}, {0x70, 0x74, out7a_w}, {0x75, 0x7f, sol_w},
    PORT_END

    static PORT_WRITE_START(writeport2){0x00, 0x07, out0a2_w},
    {0x08, 0x0f, out0b2_w}, {0x10, 0x17, out1a2_w}, {0x18, 0x1f, out1b2_w}, {0x20, 0x27, out2a2_w},
    {0x28, 0x2f, out2b2_w}, {0x30, 0x30, out3a2_w}, {0x31, 0x36, snd_w}, {0x37, 0x37, latch_w},
    {0x3e, 0x3e, irq_enable}, {0x3f, 0x3f, zc_enable}, {0x40, 0x47, out4a2_w}, {0x48, 0x4a, out4b2_w},
    {0x4b, 0x4b, enable_w}, {0x4c, 0x4f, out4c2_w}, {0x50, 0x55, col_w}, {0x57, 0x5a, bcd_w}, {0x5b, 0x5f, panel_w},
    {0x60, 0x67, digit_w}, {0x68, 0x6f, out6b2_w}, {0x70, 0x74, out7a2_w}, {0x75, 0x7f, sol_w},
    PORT_END

    static PORT_READ_START(readport){0x01, 0x02, sw1_r},
    {0x03, 0x05, dip_r}, {0x06, 0x07, sw6_r}, // Escape
...
static MACHINE_RESET(jvh) { memset(&locals, 0, sizeof(locals)); }
...
static WRITE_HANDLER(jvh_via_b_w) { sndlocals.via_b = data; }
...
static const struct via6522_interface jvh_via[] = {{/*i: A/B,CA/B1,CA/B2 */ jvh_via_a_r, 0, 0, 0, 0, 0,
...
static void
jvh_init(struct sndbrdData* brdData) {
    via_reset();
...
const struct sndbrdIntf jvhIntf = {"JVH", jvh_init, NULL, NULL, jvh_data_w, jvh_data_w};

static struct AY8910interface jvh_ay8912Int = {1, 1000000, {25}};

static MEMORY_READ_START(snd_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x008f, via_0_r}, {0xc000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(snd_writemem){0x0000, 0x007f, MWA_RAM},
...
static core_tLCDLayout dispJVH0[] = {{0, 0, 0, 7, CORE_SEG7},
                                     {0, 27, 8, 7, CORE_SEG7},
                                     {3, 0, 16, 7, CORE_SEG7},
                                     {3, 27, 24, 7, CORE_SEG7},
                                     {3, 16, 33, 2, CORE_SEG7},
...
static core_tLCDLayout dispJVH[] = {{0, 0, 7, 1, CORE_SEG7},
                                    {0, 2, 0, 7, CORE_SEG7},
                                    {0, 29, 15, 1, CORE_SEG7},
                                    {0, 31, 8, 7, CORE_SEG7},
                                    {3, 0, 23, 1, CORE_SEG7},
...
static core_tLCDLayout dispJVH2[] = {{0, 0, 7, 1, CORE_SEG7},
                                     {0, 2, 0, 7, CORE_SEG7},
                                     {0, 29, 15, 1, CORE_SEG7},
                                     {0, 31, 8, 7, CORE_SEG7},
                                     {3, 0, 23, 1, CORE_SEG7},
...
static MACHINE_INIT(jvh3) {
  ...
  static INTERRUPT_GEN(irq3) { cpu_set_irq_line(0, 1, PULSE_LINE); }
  ...
  static MEMORY_WRITE_START(writemem3){0xf800, 0xffff, MWA_RAM, &generic_nvram, &generic_nvram_size},
      MEMORY_END

      static MEMORY_READ_START(readmem3){0x0000, 0x7fff, MRA_ROM},
      ...
      if (offset <= 0x07)
          ...
          coreGlobals.lampMatrix[(offset - 0x20) / 8] =
              (coreGlobals.lampMatrix[(offset - 0x20) / 8] & ~(1 << (offset % 8))) | (data << (offset % 8));
  ...
  static READ_HANDLER(port_r) {
      switch (offset) {
          ...
          case 5:
              return coreGlobals.swMatrix[1 + core_BitColToNum(~locals.col & 0x7f)];
          case 6:
              return 0; // no idea about this one, maybe reserved for future use / more switches?
          default:
              return core_getDip(offset);
      }
  }

  static PORT_WRITE_START(writeport3){0x00, 0xff, port_w},
      PORT_END

      static PORT_READ_START(readport3){0x00, 0x06, port_r},
      PORT_END
  ...
  static READ_HANDLER(ready_r) { return YM2203_status_port_0_r(0) | YM3812_status_port_0_r(0); }

  static MEMORY_WRITE_START(snd_writemem3){0x2000, 0x2000, YM2203_control_port_0_w},
      {0x2001, 0x2001, YM2203_write_port_0_w}, {0x4000, 0x4000, YM3812_control_port_0_w},
      {0x4001, 0x4001, YM3812_write_port_0_w}, {0x6000, 0x63ff, MWA_RAM}, {0x8000, 0xffff, MWA_NOP},
      MEMORY_END

      static MEMORY_READ_START(snd_readmem3){0x0000, 0x0000, sndcmd_r},
      {0x2000, 0x2000, YM2203_status_port_0_r}, {0x4000, 0x4000, YM3812_status_port_0_r}, {0x6000, 0x63ff, MRA_RAM},
      {0xc000, 0xc000, ready_r}, {0x8000, 0xffff, MRA_ROM},
      MEMORY_END
...
*/
  ...
  static MEMORY_WRITE_START(snd_writemem4){0x0000, 0x000f, via_0_w},
      /*
  { 0x0004, 0x0005, vol_w },
  { 0x000f, 0x000f, DAC_0_data_w },
*/
      {0x0000, 0x00ff, MWA_RAM}, {0x2000, 0xffff, MWA_NOP},
      MEMORY_END

      static MEMORY_READ_START(snd_readmem4){0x0000, 0x000f, via_0_r},
      {0x0010, 0x0010, sndcmd2_r}, {0x0000, 0x00ff, MRA_RAM}, {0x2000, 0xffff, MRA_ROM},
      MEMORY_END
  ...
  static WRITE_HANDLER(ym2203_b_w) {
      ...
      if (old != data) {
          if (!ignore && !sndlocals.ignore) {
              ...
              if (!sndlocals.timer)
                  sndlocals.timer = timer_alloc(snd_stop);
              timer_reset(sndlocals.timer, 0.05);
          } else {
              if (ignore)
                  ignore--;
              else if (sndlocals.ignore)
                  sndlocals.ignore--;
          }
      }
      old = data;
  ...
  static struct YM2203interface ym2203_interface = {1,      2000000,      {YM2203_VOL(10, 10)}, {NULL},
                                                    {NULL}, {ym2203_a_w}, {ym2203_b_w},         {ym2203_irq}};

  static struct YM3812interface ym3812_interface = {1, 1000000, {60}};

  static struct DACinterface dac_interface = {1, {30}};

  static DISCRETE_SOUND_START(discInt) DISCRETE_INPUT(NODE_01, 1, 0x0003, 0) // tone
      DISCRETE_INPUT(NODE_02, 2, 0x0003, 0)                                  // enable
      DISCRETE_MULTADD(NODE_03, 1, NODE_01, 1, 200) DISCRETE_TRIANGLEWAVE(NODE_04, NODE_02, NODE_03, 20000, 10000, 0)
          DISCRETE_GAIN(NODE_05, NODE_04, 1) DISCRETE_OUTPUT(NODE_05, 20) DISCRETE_SOUND_END

      // driver section

      MACHINE_DRIVER_START(jvh3) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(jvh3, jvh3, NULL)

          MDRV_CPU_ADD_TAG("mcpu", TMS9995, 12000000) MDRV_CPU_MEMORY(readmem3, writemem3)
              MDRV_CPU_PORTS(readport3, writeport3) MDRV_CPU_VBLANK_INT(vblank3, 1) MDRV_CPU_PERIODIC_INT(irq3, 100)
                  MDRV_DIAGNOSTIC_LEDH(3) MDRV_SWITCH_UPDATE(jvh3) MDRV_DIPS(32) MDRV_NVRAM_HANDLER(generic_1fill)

                      MDRV_CPU_ADD_TAG("scpu", M6809, 2000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
                          MDRV_CPU_MEMORY(snd_readmem3, snd_writemem3) MDRV_SOUND_ADD(YM2203, ym2203_interface)
                              MDRV_SOUND_ADD(YM3812, ym3812_interface) MDRV_SOUND_ADD(DISCRETE, discInt)

                                  MDRV_CPU_ADD_TAG("scpu2", M6809, 2000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
                                      MDRV_CPU_MEMORY(snd_readmem4, snd_writemem4)
                                          MDRV_SOUND_ADD(DAC, dac_interface) MACHINE_DRIVER_END

      INPUT_PORTS_START(formula1)
  CORE_PORTS
  SIM_PORTS(1)
  PORT_START /* 0 */
      COREPORT_BITDEF(0x0004, IPT_COIN1, IP_KEY_DEFAULT) COREPORT_BITDEF(0x0002, IPT_COIN2, IP_KEY_DEFAULT)
          COREPORT_BITDEF(0x0001, IPT_COIN3, KEYCODE_3) COREPORT_BITDEF(0x0010, IPT_START1, IP_KEY_DEFAULT)
              COREPORT_BIT(0x0020, "Reset", KEYCODE_0) COREPORT_BIT(0x0008, "Self Test", KEYCODE_7)
                  COREPORT_BIT(0x0080, "LDIR", KEYCODE_8) COREPORT_BIT(0x8000, "RDIR", KEYCODE_9) PORT_START /* 1 */
      COREPORT_DIPNAME(0x0001, 0x0000, "S1") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0001, "1") COREPORT_DIPNAME(
          0x0002, 0x0000, "S2") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0002, "1")
          COREPORT_DIPNAME(0x0004, 0x0000, "S3") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0004, "1")
              COREPORT_DIPNAME(0x0008, 0x0000, "S4") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0008, "1")
                  COREPORT_DIPNAME(0x0010, 0x0000, "S5") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0010, "1")
                      COREPORT_DIPNAME(0x0020, 0x0000, "S6") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0020, "1")
                          COREPORT_DIPNAME(0x0040, 0x0000, "S7") COREPORT_DIPSET(0x0000, "0")
                              COREPORT_DIPSET(0x0040, "1") COREPORT_DIPNAME(0x0080, 0x0000,
                                                                            "S8") COREPORT_DIPSET(0x0000, "0")
                                  COREPORT_DIPSET(0x0080, "1") COREPORT_DIPNAME(0x0100, 0x0000,
                                                                                "S9") COREPORT_DIPSET(0x0000, "0")
                                      COREPORT_DIPSET(0x0100, "1") COREPORT_DIPNAME(0x0200, 0x0000, "S10")
                                          COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0200, "1")
                                              COREPORT_DIPNAME(0x0400, 0x0000, "S11") COREPORT_DIPSET(0x0000, "0")
                                                  COREPORT_DIPSET(0x0400, "1") COREPORT_DIPNAME(0x0800, 0x0000, "S12")
                                                      COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0800, "1")
                                                          COREPORT_DIPNAME(0x1000, 0x0000, "S13")
                                                              COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x1000, "1")
                                                                  COREPORT_DIPNAME(0x2000, 0x0000,
                                                                                   "S14") COREPORT_DIPSET(0x0000, "0")
                                                                      COREPORT_DIPSET(0x2000, "1")
                                                                          COREPORT_DIPNAME(0x4000, 0x0000, "S15")
                                                                              COREPORT_DIPSET(0x0000, "0")
                                                                                  COREPORT_DIPSET(0x4000, "1")
                                                                                      COREPORT_DIPNAME(0x8000, 0x0000,
                                                                                                       "S16")
                                                                                          COREPORT_DIPSET(0x0000, "0")
                                                                                              COREPORT_DIPSET(0x8000,
                                                                                                              "1")
                                                                                                  PORT_START /* 2 */
      COREPORT_DIPNAME(0x0001, 0x0000, "S17") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0001, "1")
          COREPORT_DIPNAME(0x0002, 0x0000, "S18") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0002, "1")
              COREPORT_DIPNAME(0x0004, 0x0000, "S19") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0004, "1")
                  COREPORT_DIPNAME(0x0008, 0x0000, "S20") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0008, "1")
                      COREPORT_DIPNAME(0x0010, 0x0000, "S21") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0010, "1")
                          COREPORT_DIPNAME(0x0020, 0x0000, "S22") COREPORT_DIPSET(0x0000, "0")
                              COREPORT_DIPSET(0x0020, "1") COREPORT_DIPNAME(0x0040, 0x0000, "S23")
                                  COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0040, "1") COREPORT_DIPNAME(
                                      0x0080, 0x0000, "S24") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0080, "1")
                                      COREPORT_DIPNAME(0x0100, 0x0000, "S25") COREPORT_DIPSET(0x0000, "0")
                                          COREPORT_DIPSET(0x0100, "1") COREPORT_DIPNAME(0x0200, 0x0000, "S26")
                                              COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0200, "1")
                                                  COREPORT_DIPNAME(0x0400, 0x0000, "S27") COREPORT_DIPSET(0x0000, "0")
                                                      COREPORT_DIPSET(0x0400, "1") COREPORT_DIPNAME(0x0800, 0x0000,
                                                                                                    "S28")
                                                          COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0800, "1")
                                                              COREPORT_DIPNAME(0x1000, 0x0000, "S29") COREPORT_DIPSET(
                                                                  0x0000, "0") COREPORT_DIPSET(0x1000, "1")
                                                                  COREPORT_DIPNAME(0x2000, 0x0000,
                                                                                   "S30") COREPORT_DIPSET(0x0000, "0")
                                                                      COREPORT_DIPSET(0x2000, "1")
      ...
      static core_tLCDLayout dispJVH3[] = {{0, 0, 0, 8, CORE_SEG16},
                                           {0, 18, 8, 8, CORE_SEG16},
                                           {3, 0, 16, 8, CORE_SEG7},
                                           {3, 18, 24, 8, CORE_SEG7},
#ifdef MAME_DEBUG
