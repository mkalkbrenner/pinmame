    ...
    if (GTS3locals.extra16led) { // used for alpha display digits on Vegas only
        if (data > 0 && data < 4 && GTS3locals.ax[4] == GTS3locals.ax[6])
            GTS3locals.segments[39 + data].w = GTS3locals.pseg[39 + data].w =
                (GTS3locals.ax[6] & 0x3f) | ((GTS3locals.ax[6] & 0xc0) << 3) | ((GTS3locals.ax[5] & 0x0f) << 11)
                | ((GTS3locals.ax[5] & 0x10) << 2) | ((GTS3locals.ax[5] & 0x20) << 3);
...
static WRITE_HANDLER(xvia_1_ca1_w) { logerror1("NOT USED!: via_1_ca1_w %x\n", data); }
#endif

//CPU LED
static WRITE_HANDLER(xvia_1_ca2_w) { GTS3locals.diagnosticLed = data; }
...
static struct via6522_interface via_0_interface = {
    /*inputs : A/B         */ xvia_0_a_r,
    xvia_0_b_r,
    /*inputs : CA1/B1,CA2/B2 */ 0,
    xvia_0_cb1_r,
    xvia_0_ca2_r,
    xvia_0_cb2_r,
    /*outputs: A/B,CA2/B2   */ xvia_0_a_w,
    xvia_0_b_w,
    xvia_0_ca2_w,
    xvia_0_cb2_w,
    /*irq                  */ 0 /* GTS3_irq */
...
static struct via6522_interface via_1_interface = {
    /*inputs : A/B         */ xvia_1_a_r,
    xvia_1_b_r,
    /*inputs : CA1/B1,CA2/B2 */ xvia_1_ca1_r,
    xvia_1_cb1_r,
    xvia_1_ca2_r,
    xvia_1_cb2_r,
    /*outputs: A/B,CA2/B2   */ xvia_1_a_w,
    xvia_1_b_w,
    xvia_1_ca2_w,
    xvia_1_cb2_w,
    /*irq                  */ 0 /* GTS3_irq */
...
static INTERRUPT_GEN(GTS3_vblank) {
  ...
  /--------------------------------*/
    ...
    if ((GTS3locals.vblankCount % GTS3_DISPLAYSMOOTH) == 0) {
        ...
        if (GTS3_dmdlocals[0].version == 2) {
            coreGlobals.diagnosticLed = GTS3locals.diagnosticLed | (GTS3_dmdlocals[0].diagnosticLed << 1)
                                        | (GTS3_dmdlocals[1].diagnosticLed << 2);
        } else {
            coreGlobals.diagnosticLed = (GTS3locals.diagnosticLed2 << 3) | (GTS3locals.diagnosticLed1 << 2)
                                        | (GTS3_dmdlocals[0].diagnosticLed << 1) | GTS3locals.diagnosticLed;
        }
    }
...
static int
gts3_m2sw(int col, int row) {
    return (col - 1) * 10 + row;
}

/*Alpha Numeric First Generation Init*/
static void
GTS3_alpha_common_init(void) {
...
gts3dmd_init(void) {
    ...
    if (memory_region(GTS3_MEMREG_DCPU1)) {
        memcpy(memory_region(GTS3_MEMREG_DCPU1) + 0x8000,
               memory_region(GTS3_MEMREG_DROM1) + (memory_region_length(GTS3_MEMREG_DROM1) - 0x8000), 0x8000);
    }
...
static MACHINE_STOP(gts3) { sndbrd_0_exit(); }
...
static void
dmdswitchbank(int which) {
    int addr = (GTS3_dmdlocals[which].pa0 * 0x04000) + (GTS3_dmdlocals[which].pa1 * 0x08000)
               + (GTS3_dmdlocals[which].pa2 * 0x10000) + (GTS3_dmdlocals[which].pa3 * 0x20000)
...
static WRITE_HANDLER(alpha_aux) {
    ...
    if (!GTS3locals.extra16led && offset)
        GTS3locals.extra16led = 1;
}
...
dmd_vblank(int which) {
    ...
    GTS3_dmdlocals[which].nextDMDFrame =
        (GTS3_dmdlocals[which].nextDMDFrame + 1)
        % (GTS3_dmdlocals[0].color_mode == 0
               ? GTS3DMD_FRAMES_4C_a
               : (GTS3_dmdlocals[0].color_mode == 1 ? GTS3DMD_FRAMES_4C_b : GTS3DMD_FRAMES_5C));
}
        ...
        if (++modsol_rate_counter == GTS3_MODSOL_SAMPLE) {
            ...
            for (i = 0; i < 32; i++) {
                ...
                coreGlobals.modulatedSolenoids[CORE_MODSOL_CUR][i] =
                    core_calc_modulated_light(GTS3locals.solenoidbits[i], GTS3_MODSOL_SMOOTH,
...
void
UpdateSoundLEDS(int num, UINT8 bit) {
    if (num == 0)
...
/-------------------------------------------------*/
static NVRAM_HANDLER(gts3) { core_nvram(file, read_or_write, memory_region(GTS3_MEMREG_CPU), 0x2000, 0x00); }

/*---------------------------
/  Memory map for main CPU
/----------------------------*/
static MEMORY_READ_START(GTS3_readmem){0x0000, 0x1fff, MRA_RAM}, {0x2000, 0x200f, via_0_r}, {0x2010, 0x201f, via_1_r},
    {0x2020, 0x2023, display_r}, {0x2050, 0x2057, aux1_r}, {0x4000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(GTS3_writemem){0x0000, 0x1fff, MWA_RAM},
    {0x2000, 0x200f, via_0_w}, {0x2010, 0x201f, via_1_w}, {0x2020, 0x2023, display_control},
    {0x2030, 0x2033, solenoid_w}, {0x2040, 0x2040, lds_w}, {0x2041, 0x2043, aux_w}, {0x2050, 0x2070, aux1_w},
...
/----------------------------*/
    static MEMORY_READ_START(GTS3_dmdreadmem){0x0000, 0x1fff, MRA_RAM},
    {0x2000, 0x2000, crtc6845_register_0_r}, {0x3000, 0x3000, dmdlatch_r}, /*Input Enable*/
    {0x4000, 0x7fff, MRA_BANK1}, {0x8000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(GTS3_dmdwritemem){0x0000, 0x0fff, MWA_RAM},
    {0x1000, 0x1fff, MWA_RAM}, /*DMD Display RAM*/
    {0x2800, 0x2800, crtc6845_address_0_w}, {0x2801, 0x2801, crtc6845_register_0_w},
    {0x3800, 0x3800, dmdoport}, /*Output Enable*/
    {0x4000, 0x7fff, MWA_BANK1}, {0x8000, 0xffff, MWA_ROM},
    MEMORY_END

    //NOTE: DMD #2 for Strikes N Spares - Identical to DMD #1 hardware & memory map
    static MEMORY_READ_START(GTS3_dmdreadmem2){0x0000, 0x1fff, MRA_RAM},
    {0x2000, 0x2000, crtc6845_register_1_r}, {0x3000, 0x3000, dmdlatch2_r}, /*Input Enable*/
    {0x4000, 0x7fff, MRA_BANK2}, {0x8000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(GTS3_dmdwritemem2){0x0000, 0x0fff, MWA_RAM},
    {0x1000, 0x1fff, MWA_RAM}, /*DMD Display RAM*/
    {0x2800, 0x2800, crtc6845_address_1_w}, {0x2801, 0x2801, crtc6845_register_1_w},
    {0x3800, 0x3800, dmdoport2}, /*Output Enable*/
    {0x4000, 0x7fff, MWA_BANK2}, {0x8000, 0xffff, MWA_ROM},
    MEMORY_END

    MACHINE_DRIVER_START(gts3) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD(M65C02, 2000000) // XTAL(4'000'000) / 2
    MDRV_CPU_MEMORY(GTS3_readmem, GTS3_writemem) MDRV_CPU_VBLANK_INT(GTS3_vblank, GTS3_VBLANKFREQ)
        MDRV_CPU_PERIODIC_INT(alphanmi, GTS3_ALPHANMIFREQ) MDRV_TIMER_ADD(GTS3_irq, GTS3_IRQFREQ)
            MDRV_NVRAM_HANDLER(gts3)

                MDRV_SWITCH_UPDATE(GTS3) MDRV_DIAGNOSTIC_LEDH(4) MDRV_SWITCH_CONV(gts3_sw2m, gts3_m2sw)
                    MDRV_LAMP_CONV(gts3_sw2m, gts3_m2sw) MDRV_SOUND_CMD(GTS3_sndCmd_w)
                        MDRV_SOUND_CMDHEADING("GTS3") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_1a) MDRV_IMPORT_FROM(gts3) MDRV_CORE_INIT_RESET_STOP(gts3, NULL, gts3)
        MDRV_SCREEN_SIZE(320, 200) MDRV_VISIBLE_AREA(0, 319, 0, 199) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_1as_no) MDRV_IMPORT_FROM(gts3_1a) MDRV_IMPORT_FROM(gts80s_s3_no) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_1as) MDRV_IMPORT_FROM(gts3_1a) MDRV_IMPORT_FROM(gts80s_s3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_1as80b2) MDRV_IMPORT_FROM(gts3_1a) MDRV_IMPORT_FROM(gts80s_b2) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_1as80b3) MDRV_IMPORT_FROM(gts3_1a) MDRV_IMPORT_FROM(gts80s_b3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_1b) MDRV_IMPORT_FROM(gts3) MDRV_CORE_INIT_RESET_STOP(gts3b, NULL, gts3)
        MDRV_SCREEN_SIZE(320, 200) MDRV_VISIBLE_AREA(0, 319, 0, 199) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_1bs) MDRV_IMPORT_FROM(gts3_1b) MDRV_IMPORT_FROM(gts80s_s3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_2_4c_a) MDRV_IMPORT_FROM(gts3) MDRV_CPU_ADD(M65C02, 3579545. / 2.)
        MDRV_CPU_MEMORY(GTS3_dmdreadmem, GTS3_dmdwritemem) MDRV_CORE_INIT_RESET_STOP(gts3dmd_4c_a, NULL, gts3)
            MDRV_IMPORT_FROM(gts80s_s3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_2_4c_b) MDRV_IMPORT_FROM(gts3_2_4c_a)
        MDRV_CORE_INIT_RESET_STOP(gts3dmd_4c_b, NULL, gts3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_2_5c) MDRV_IMPORT_FROM(gts3_2_4c_a)
        MDRV_CORE_INIT_RESET_STOP(gts3dmd_5c, NULL, gts3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_2a_4c_a) MDRV_IMPORT_FROM(gts3_2_4c_a)
        MDRV_CORE_INIT_RESET_STOP(gts3dmda_4c_a, NULL, gts3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_2a_4c_b) MDRV_IMPORT_FROM(gts3_2_4c_a)
        MDRV_CORE_INIT_RESET_STOP(gts3dmda_4c_b, NULL, gts3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts3_2a_5c) MDRV_IMPORT_FROM(gts3_2_4c_a)
        MDRV_CORE_INIT_RESET_STOP(gts3dmda_5c, NULL, gts3) MACHINE_DRIVER_END
