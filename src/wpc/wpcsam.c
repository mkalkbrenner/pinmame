...
static const char* pinmame_sample_names[] = {
    "*pinmame",     "knocker.wav",  "lsling.wav",   "rsling.wav",   "solenoid.wav",
    "popper.wav",   "ballrel.wav",  "diverter.wav", "flapopen.wav", "flapclos.wav",
    "lflipper.wav", "rflipper.wav", "jet1.wav",     "jet2.wav",     "jet3.wav",
    "motor1.wav",   "motor2.wav",   "solon.wav",    "soloff.wav",   0 /* end of array */
};

struct Samplesinterface samples_interface = {6, 100, /* 6 channels full volume */
                                             pinmame_sample_names, "Mech. sounds"};

/* Check if a sample is specified for changed solenoid */
void
proc_mechsounds(int sol, int newState) {
    if ((sol == sLRFlip) || ((sol == sURFlip) && (core_gameData->hw.flippers & FLIP_SOL(FLIP_UR))))
        sample_start(0, newState ? SAM_RFLIPPER : SAM_SOLENOID_OFF, 0);
    else if ((sol == sLLFlip) || ((sol == sULFlip) && (core_gameData->hw.flippers & FLIP_SOL(FLIP_UL))))
...
void
wpc_play_sample(int channel, int samplename) {
