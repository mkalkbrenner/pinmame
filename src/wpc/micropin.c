...
static WRITE_HANDLER(mp_pia0ca2_w) {
    if (!data && locals.col < 8)
        coreGlobals.lampMatrix[4] |= (1 << locals.col);
}
...
static WRITE_HANDLER(m510x_w) { pia_1_w(offset, data ^ 0xff); }

static READ_HANDLER(m510x_r) { return (pia_1_r(offset) ^ 0xff); }
...
static NVRAM_HANDLER(MICROPIN) { core_nvram(file, read_or_write, mp_CMOS, 0x200, 0x00); }
...
static MEMORY_WRITE_START(mp_writemem){0x0000, 0x01ff, mp_CMOS_w, &mp_CMOS}, {0x4000, 0x4005, m400x_w},
    {0x5000, 0x5003, pia_0_w}, {0x5100, 0x5103, m510x_w}, {0x5200, 0x5203, m520x_w},
    MEMORY_END

    static MEMORY_READ_START(mp_readmem){0x0000, 0x01ff, MRA_RAM},
    {0x4000, 0x4004, m400x_r}, {0x5000, 0x5003, pia_0_r}, {0x5100, 0x5103, m510x_r}, {0x6000, 0x7fff, MRA_ROM},
...
static INTERRUPT_GEN(mp_vblank) { core_updateSw(core_getSol(12)); }

static INTERRUPT_GEN(mp_irq) { cpu_set_irq_line(0, M6800_IRQ_LINE, PULSE_LINE); }
...
SIM_PORTS(1)
PORT_START /* 0 */
    COREPORT_BITDEF(0x0010, IPT_START1, IP_KEY_DEFAULT) COREPORT_BIT(0x0001, "Tilt #1", KEYCODE_COMMA)
        COREPORT_BIT(0x0008, "Tilt #2", KEYCODE_STOP) COREPORT_BIT(0x0040, "Tilt #3", KEYCODE_SLASH)
            COREPORT_BIT(0x0080, "Tilt #4", KEYCODE_DEL) COREPORT_BITDEF(0x1000, IPT_COIN1, IP_KEY_DEFAULT)
                COREPORT_BIT(0x2000, "Volume up", KEYCODE_7) COREPORT_BIT(0x4000, "Volume down", KEYCODE_8)
                    COREPORT_BIT(0x8000, "Alarm/Tilt", KEYCODE_9) INPUT_PORTS_END

    core_tLCDLayout mp_disp[] = {{0, 0, 33, 2, CORE_SEG7},
                                 {0, 6, 35, 1, CORE_SEG7},
                                 {0, 10, 17, 3, CORE_SEG7},
                                 {0, 16, 1, 3, CORE_SEG7},
                                 {0, 24, 4, 6, CORE_SEG7},
...
static INTERRUPT_GEN(mp2_vblank) {
    ...
    coreGlobals.lampMatrix[16] =
        (memory_region(REGION_CPU1)[0x23d6] & 0xf0) | (memory_region(REGION_CPU1)[0x23de] >> 4);
    coreGlobals.lampMatrix[17] =
        (memory_region(REGION_CPU1)[0x23cc] & 0xf0) | (memory_region(REGION_CPU1)[0x23de] & 0x0f);
...
static SWITCH_UPDATE(MICROPIN2) {
    ...
    for (sw = 0; sw < 4; sw++) {
        for (i = 0; i < 8; i++) {
            ...
            if (offset < 0x2194 || (offset > 0x219f && offset < 0x21a4))
                continue;
...
static WRITE_HANDLER(mxxxx_w) { logerror("%04x: Write to %04x: %02x\n", activecpu_get_pc(), offset, data); }

static MEMORY_WRITE_START(mp2_writemem){0x0000, 0x1fff, mxxxx_w},
    {0x2000, 0x23ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static MEMORY_READ_START(mp2_readmem){0x0000, 0x1fff, MRA_ROM},
...
static PORT_WRITE_START(mp2_writeport){0x00, 0x0f, mp2_out},
    PORT_END

    static PORT_READ_START(mp2_readport){0x00, 0x01, mp2_sw},
    {0x02, 0x05, mp2_dip},
    PORT_END

    MACHINE_DRIVER_START(pentacp2) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", 8085A, 2500000)
        MDRV_CPU_MEMORY(mp2_readmem, mp2_writemem) MDRV_CPU_PORTS(mp2_readport, mp2_writeport)
            MDRV_CPU_PERIODIC_INT(mp2_irq, 120) MDRV_CPU_VBLANK_INT(mp2_vblank, 1) MDRV_SWITCH_UPDATE(MICROPIN2)
                MDRV_DIPS(48) MDRV_NVRAM_HANDLER(generic_1fill) MDRV_DIAGNOSTIC_LEDH(1) MACHINE_DRIVER_END

#define COREPORT_BITINV(mask, name, key) PORT_BITX(mask, IP_ACTIVE_LOW, IPT_BUTTON1, name, key, IP_JOY_NONE)

    INPUT_PORTS_START(pentacp2)
CORE_PORTS
SIM_PORTS(1)
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "Reset", KEYCODE_HOME) COREPORT_BIT(0x0010, "Coin", KEYCODE_5)
        COREPORT_BITINV(0x0020, "Keep NVRAM", KEYCODE_0) COREPORT_BITINV(0x0040, "Show previous", KEYCODE_9)
            COREPORT_BIT(0x0080, "Tilt", KEYCODE_DEL) PORT_START /* 1 */
                        ...
                        COREPORT_DIPNAME(0x0020, 0x0000, "S6") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0020, "1")
                            COREPORT_DIPNAME(0x0040, 0x0000, "S7") COREPORT_DIPSET(0x0000, "0")
                                COREPORT_DIPSET(0x0040, "1") COREPORT_DIPNAME(0x0080, 0x0000, "S8")
                                    COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0080, "1") PORT_START /* 2 */
    COREPORT_DIPNAME(0xffff, 0x0004, "10 cents x") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0002, "1")
        COREPORT_DIPSET(0x0004, "2") COREPORT_DIPSET(0x0008, "3") COREPORT_DIPSET(0x0010, "4")
            COREPORT_DIPSET(0x0020, "5") COREPORT_DIPSET(0x0040, "6") COREPORT_DIPSET(0x0080, "7")
                COREPORT_DIPSET(0x0400, "8") COREPORT_DIPSET(0x0800, "9") PORT_START /* 3 */
    COREPORT_DIPNAME(0xffff, 0x0020, " 1 cent  x") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0002, "1")
        COREPORT_DIPSET(0x0004, "2") COREPORT_DIPSET(0x0008, "3") COREPORT_DIPSET(0x0010, "4")
            COREPORT_DIPSET(0x0020, "5") COREPORT_DIPSET(0x0040, "6") COREPORT_DIPSET(0x0080, "7")
                COREPORT_DIPSET(0x0100, "8") COREPORT_DIPSET(0x0200, "9") INPUT_PORTS_END

    core_tLCDLayout mp2_disp[] = {{0, 6, 58, 3, CORE_SEG7},
                                  {0, 13, 61, 3, CORE_SEG7},
                                  {0, 27, 12, 3, CORE_SEG7},
                                  {0, 34, 15, 3, CORE_SEG7},
                                  {3, 0, 0, 2, CORE_SEG7},
                                  ...
                                  {9, 42, 23, 3, CORE_SEG7},
#ifdef MAME_DEBUG
                                  {12, 14, 39, 7, CORE_SEG7},
                                  {12, 28, 19, 1, CORE_SEG7},
                                  {12, 30, 8, 1, CORE_SEG7},
                                  {12, 0, 64, 1, CORE_SEG7},
                                  {12, 4, 65, 1, CORE_SEG7},
                                  {12, 8, 66, 2, CORE_SEG7},
