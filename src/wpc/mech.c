...
mech_emuExit(void) {
    if (locals.mechTimer) {
        timer_remove(locals.mechTimer);
        locals.mechTimer = NULL;
    }
    locals.emuRunning = FALSE;
}

int
mech_getPos(int mechNo) {
    return locals.mechData[mechNo].pos;
}

int
mech_getSpeed(int mechNo) {
    return locals.mechData[mechNo].speed / locals.mechData[mechNo].ret;
}

void
mech_addLong(int mechNo, int sol1, int sol2, int type, int length, int steps, mech_tSwData sw[], int initialpos) {
...
mech_updateAll(int param) {
    ...
    if (mech == 0) {
        ...
        for (ii = 0; ii < MECH_MAXMECH / 2; ii++) {
            ...
            if ((mech & 0x01) && locals.mechData[ii].sol1 && ((locals.mechCounter == 0) || locals.mechData[ii].fast)) {
...
mech_update(int mechNo) {
    ...
    { /*-- check power direction -1, 0, 1 --*/
        if ((md->type & 0x70) == MECH_FOURSTEPSOL) {
            ...
            int sol = ((core_getPulsedSol(md->sol1) != 0) + (md->sol2 ? 2 * (core_getPulsedSol(md->sol2) != 0) : 0))
                      ^ md->solinv;
    ...
    { /*-- update speed --*/
        if (dir == 0) {
            ...
            speed -= (speed > 0) + md->ret;
            if (speed < -md->acc * md->ret)
                speed = -md->acc * md->ret;
        } else /* if (dir > 0) */
        {
            speed += (speed < 0) + md->ret;
    ...
    if (speed || (md->pos < 0)) {
        ...
        if (md->type & MECH_STOPEND) {
            if (anglePos >= md->length * MECH_STEP)
                anglePos = (md->length - 1) * MECH_STEP;
            else if (anglePos < 0)
                anglePos = 0;
        } else if (md->type & MECH_REVERSE) {
            if (anglePos >= md->length * 2 * MECH_STEP)
                anglePos -= md->length * 2 * MECH_STEP;
            else if (anglePos < 0)
                anglePos += md->length * 2 * MECH_STEP;
        } else { /* MECH_CIRCLE */
            if (anglePos >= md->length * MECH_STEP)
                anglePos -= md->length * MECH_STEP;
            else if (anglePos < 0)
                anglePos += md->length * MECH_STEP;
        }
        ...
        for (ii = 0; ii < MECH_MAXMECHSW && md->swPos[ii].swNo > 0; ii++) {
            if (md->swPos[ii].pulse)
                currPos %= md->swPos[ii].pulse;
            if ((currPos >= md->swPos[ii].startPos) && (currPos <= md->swPos[ii].endPos))
