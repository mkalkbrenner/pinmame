...
static WRITE_HANDLER(parallel_1_out) { coreGlobals.tmpLampMatrix[locals.strobe] = ~data; }
...
static WRITE_HANDLER(parallel_4_out) { locals.segments[locals.strobe].w = ~data; }

static WRITE_HANDLER(parallel_5_out) { locals.segments[8 + locals.strobe].w = ~data; }

static WRITE_HANDLER(parallel_6_out) { locals.segments[16 + locals.strobe].w = ~data; }

static WRITE_HANDLER(parallel_7_out) { locals.segments[24 + locals.strobe].w = ~data; }

static WRITE_HANDLER(parallel_8_out) { locals.segments[32 + locals.strobe].w = ~data; }
...
static HC4094interface hc4094nsm = {
    9, // 9 chips!
    {parallel_0_out, parallel_1_out, parallel_2_out, parallel_3_out, parallel_4_out, parallel_5_out, parallel_6_out,
     parallel_7_out, parallel_8_out},
...
static READ_HANDLER(read_ba) { return 0; }

static READ_HANDLER(read_uv) { return locals.uv ? 0 : 0x05; }

static READ_HANDLER(read_zc) { return locals.zc; }

static READ_HANDLER(read_diag1) { return coreGlobals.swMatrix[0] & 0x80 ? 0xff : 0; }

static READ_HANDLER(read_diag2) { return coreGlobals.swMatrix[0] & 0x40 ? 0 : 0xff; }

static READ_HANDLER(read_diag3) { return coreGlobals.swMatrix[0] & 0x20 ? 0 : 0xff; }
            ...
            return 0xcf
                   ^ core_revbyte(
                       coreGlobals.swMatrix
                           [9]); // bits 1 .. 6 are test returns from lamps and solenoids, bit 7 is SW 65 (inverted)
        case 0x0e:
        case 0x0f:
...
static MEMORY_READ_START(readmem){0x0000, 0x7fff, MRA_ROM}, {0xe000, 0xefff, MRA_RAM},
    MEMORY_END
    ...
    {0xffec, 0xffec, ay8912_0_ctrl_w}, {0xffed, 0xffed, ay8912_0_data_w}, {0xffee, 0xffee, ay8912_1_ctrl_w},
    {0xffef, 0xffef, ay8912_1_data_w},
    MEMORY_END

    static PORT_WRITE_START(writeport){0x0f70, 0x0f7f, MWA_NOP}, // internal registers
    {0x7f80, 0x7f80, w7f80},                                     // ???
    {0x7fb0, 0x7fbf, cru_w}, {0x7fc0, 0x7fc0, w7fc0},            // ???
    {0x7fd0, 0x7fd0, oe_w},
    PORT_END

    static PORT_READ_START(readport){0x00, 0x00,
                                     read_uv}, // undervolt perc. on bit 1, also needs bit 3 set to pass "OE 614" test
    ...
    {0xfe4, 0xfe4, keypad_r}, {0xff0, 0xfff, cru_r},
    PORT_END
...
static core_tLCDLayout dispNsm[] = {{0, 0, 0, 8, CORE_SEG8D},
                                    {0, 18, 8, 8, CORE_SEG8D},
                                    {3, 0, 16, 8, CORE_SEG8D},
                                    {3, 18, 24, 8, CORE_SEG8D},
#ifdef MAME_DEBUG
                                    {6, 9, 32, 8, CORE_SEG8D},
#else
                                     {6, 11, 33, 2, CORE_SEG8D},
                                     {6, 19, 38, 2, CORE_SEG8D},
#endif
                                    {0}};
...
SIM_PORTS(1)
PORT_START /* 0 */
    COREPORT_BIT(0x0002, "Start", KEYCODE_1) COREPORT_BIT(0x0080, "Coin 1", KEYCODE_3)
        COREPORT_BIT(0x0040, "Coin 2", KEYCODE_4) COREPORT_BIT(0x0020, "Coin 3", KEYCODE_5)
            COREPORT_BIT(0x0001, "Tilt", KEYCODE_DEL) COREPORT_BIT(0x8000, "Test 1", KEYCODE_7)
                COREPORT_BITTOG(0x4000, "Test 2", KEYCODE_8)
                    COREPORT_BITTOG(0x2000, "Test 3", KEYCODE_9) PORT_START /* 1 */
    COREPORT_BIT(0x0020, "Keypad 1", KEYCODE_1_PAD) COREPORT_BIT(0x0010, "Keypad 2", KEYCODE_2_PAD)
        COREPORT_BIT(0x0008, "Keypad 3", KEYCODE_3_PAD) COREPORT_BIT(0x0400, "Keypad 4", KEYCODE_4_PAD)
            COREPORT_BIT(0x0080, "Keypad 5", KEYCODE_5_PAD) COREPORT_BIT(0x0040, "Keypad 6", KEYCODE_6_PAD)
                COREPORT_BIT(0x2000, "Keypad 7", KEYCODE_7_PAD) COREPORT_BIT(0x1000, "Keypad 8", KEYCODE_8_PAD)
                    COREPORT_BIT(0x0800, "Keypad 9", KEYCODE_9_PAD) COREPORT_BIT(0x0002, "Keypad *", KEYCODE_ASTERISK)
                        COREPORT_BIT(0x8000, "Keypad 0", KEYCODE_0_PAD)
                            COREPORT_BIT(0x4000, "Keypad #", KEYCODE_ENTER_PAD) INPUT_PORTS_END
