...
static struct SN76477interface zac1125_sn76477Int = {
    1,
    {50},          /* mixing level */
                   /*                       pin description      */
...
/ exported interface
/--------------------*/
const struct sndbrdIntf zac1125Intf = {"ZAC1125",      zac1125_init, NULL, NULL, zac1125_data_w,
                                       zac1125_data_w, NULL,         NULL, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
...
static WRITE_HANDLER(zac1125_data_w) {
    static double states[8][5] = {
        // pins 7, 18, 20, 24, and optional timer interval
        {RES_K(468), RES_K(100), RES_M(1), RES_K(9.9)},        {RES_K(4.7), RES_K(32), RES_M(1), RES_K(136)},
        {RES_K(468), RES_K(25), RES_M(1), RES_K(9.9)},         {RES_K(468), RES_K(32), RES_M(1), RES_K(9.9)},
        {RES_K(192), RES_K(4.5), RES_M(1), RES_M(1.5), 0.015}, {RES_K(26.5), RES_K(4.5), RES_M(1), RES_K(9.9), 0.0003},
        {RES_M(1.5), RES_K(100), RES_M(1), RES_K(358)},        {RES_K(192), RES_K(9.1), RES_K(32), RES_K(94)}};
...
static void
zac1125_init(struct sndbrdData* brdData) {
    /* MIXER A & C = GND */
    SN76477_mixer_w(0, 0);
...
/ exported interface
/--------------------*/
const struct sndbrdIntf zac1346Intf = {"ZAC1346",     sp_init, NULL, sp_diag, sp1346_data_w,
                                       sp1346_data_w, NULL,    NULL, NULL,    0};
...
static void
sp_init(struct sndbrdData* brdData) {
...
static MEMORY_READ_START(i8035_readmem){0x0000, 0x07ff, MRA_ROM}, {0x0800, 0x087f, MRA_RAM}, {0x0880, 0x08ff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(i8035_writemem){0x0800, 0x087f, MWA_RAM},
    MEMORY_END
...
static PORT_READ_START(i8035_readport){0x00, 0x7f, ram_r}, {0x80, 0xff, sp1346_data_r}, {I8039_t1, I8039_t1, test_r},
    MEMORY_END

    static PORT_WRITE_START(i8035_writeport){0x00, 0x7f, ram_w},
    {I8039_p1, I8039_p1, dac_w},
    MEMORY_END

    MACHINE_DRIVER_START(zac1346) MDRV_CPU_ADD_TAG("scpu", I8035, 6000000. / 15.) // 8035 has internal divider by 15!
    MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(i8035_readmem, i8035_writemem)
        MDRV_CPU_PORTS(i8035_readport, i8035_writeport)

            MDRV_SOUND_ADD(DAC, sp1346_dacInt) MDRV_INTERLEAVE(500) MACHINE_DRIVER_END

    static struct SN76477interface zac1146_sn76477Int = {
        1,
        {30},          /* mixing level */
                       /*                       pin description      */
...
const struct sndbrdIntf zac1370Intf = {"ZAC1370",  sns_init, NULL, sns_diag, sns_data_w,
                                       sns_data_w, NULL,     NULL, NULL,     SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};

static struct TMS5220interface sns_tms5220Int = {640000, 75, sns_5220Irq,
                                                 sns_5220Rdy}; // the frequency may vary by up to 30 percent!!!
...
static struct AY8910interface sns2_ay8910Int = {2,   3579545 / 4, {25, 25},     {sns_8910a_r, sns2_8910a_r},
                                                {0}, {0},         {sns_8910b_w}};

static MEMORY_READ_START(sns_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x0083, pia_r(SNS_PIA0)},
    {0x0084, 0x0087, pia_r(SNS_PIA2)},                                 // 13136 only
    {0x0090, 0x0093, pia_r(SNS_PIA1)}, {0x1800, 0x1800, sns2_8910a_r}, // 13136 only
    {0x2000, 0x2000, sns_8910a_r}, {0x7000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(sns_writemem){0x0000, 0x007f, MWA_RAM},
    {0x0080, 0x0083, pia_w(SNS_PIA0)}, {0x0084, 0x0087, pia_w(SNS_PIA2)}, // 13136 only
    {0x0090, 0x0093, pia_w(SNS_PIA1)}, {0x1000, 0x1000, sns_dac_w},
    {0x4000, 0x4000, DAC_1_data_w}, // 13136 only (never accessed)
    MEMORY_END

    static MACHINE_DRIVER_START(zac1370_nosound) MDRV_CPU_ADD(M6802, 3579545. / 4.) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(sns_readmem, sns_writemem) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(zac1370) MDRV_IMPORT_FROM(zac1370_nosound)

        MDRV_INTERLEAVE(500) MDRV_SOUND_ADD(TMS5220, sns_tms5220Int) MDRV_SOUND_ADD(AY8910, sns_ay8910Int)
            MDRV_SOUND_ADD(DAC, sns_dacInt) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(zac13136) MDRV_IMPORT_FROM(zac1370_nosound)

        MDRV_INTERLEAVE(500) MDRV_SOUND_ADD(TMS5220, sns_tms5220Int) MDRV_SOUND_ADD(AY8910, sns2_ay8910Int)
            MDRV_SOUND_ADD(DAC, sns2_dacInt) MACHINE_DRIVER_END
...
static const struct pia6821_interface sns_pia[] = {
    ...
    {/*i: A/B,CA/B1,CA/B2 */ sns_pia1a_r, PIA_UNUSED_VAL(0x03), sns_pia1ca1_r, sns_pia1cb1_r, sns_pia1ca2_r,
     PIA_UNUSED_VAL(0),
...
initTMS(void) {
    tms5220_reset();
    if (core_gameData->hw.soundBoard == SNDBRD_ZAC1370 || core_gameData->hw.soundBoard == SNDBRD_ZAC11178
        || !strncasecmp(Machine->gamedrv->name, "tmac", 4) || !strncasecmp(Machine->gamedrv->name, "zankor", 6)) {
...
static void
sns_init(struct sndbrdData* brdData) {
...
sns_diag(int button) {
    if (core_gameData->hw.soundBoard & 0x02)
        return; // ignore 11178 - too bad subboards can't have their own sndbrdIntf!
    cpu_set_nmi_line(ZACSND_CPUA, button ? ASSERT_LINE : CLEAR_LINE);
}

static WRITE_HANDLER(sns_dac_w) {
    if (core_gameData->hw.soundBoard != SNDBRD_ZAC1370 || !snslocals.dacMute)
...
static WRITE_HANDLER(sns_pia0a_w) {
    ...
    if (snslocals.pia0b & 0x02)
        AY8910Write(0, snslocals.pia0b ^ 0x01, snslocals.pia0a);
}

static WRITE_HANDLER(sns_pia0b_w) {
    if ((data & 0xf0) != (snslocals.pia0b & 0xf0))
        logerror("DAC1408 modulation: %x\n", data >> 4);
    snslocals.pia0b = data;
    if (snslocals.pia0b & 0x02)
        AY8910Write(0, snslocals.pia0b ^ 0x01, snslocals.pia0a);
}

static WRITE_HANDLER(sns_pia0ca2_w) { UpdateZACSoundLED(1, data); } // diag led

static WRITE_HANDLER(sns_pia0cb2_w) { snslocals.dacMute = data; } // mute DAC
...
static WRITE_HANDLER(sns_pia1b_w) {
    ...
    if (!(core_gameData->hw.soundBoard & 0x02) && (data & 0xf0) != (snslocals.pia1b & 0xf0))
        logerror("TMS5200 modulation: %x\n", data >> 4);
    ...
    if (core_gameData->hw.soundBoard & 0x02) { // true for all 11178
        ...
        if ((data & 0x08) && (data & 0x10))
            snslocals.snot_ab4 = SW_TRUE; // pb3 and pb4 set -> snot_ab4 true
        if ((~data & 0x08) && (data & 0x10))
            snslocals.snot_ab3 = SW_TRUE;
        if ((data & 0x08) && (~data & 0x10))
            snslocals.snot_ab2 = SW_TRUE;
        if ((~data & 0x08) && (~data & 0x10))
            snslocals.snot_ab1 = SW_TRUE;
...
static READ_HANDLER(sns_pia1ca1_r) { return snslocals.pia1ca1; }

static READ_HANDLER(sns_pia1ca2_r) {
    //  logerror("sns_pia1ca2_r TMS5220 ready %x\n", snslocals.pia1ca2);
    // oliver
    if (snslocals.r500cmd)
...
static WRITE_HANDLER(sns_pia2a_w) {
    ...
    if (snslocals.pia2b & 0x02)
        AY8910Write(1, snslocals.pia2b ^ 0x01, snslocals.pia2a);
}
...
static WRITE_HANDLER(sns_pia2ca2_w) { UpdateZACSoundLED(2, data); } // diag led
...
static WRITE_HANDLER(sns_data_w) {
    ...
    switch (core_gameData->hw.soundBoard) {
        ...
        case SNDBRD_ZAC11178_13181:
            snslocals.pia1ca1 = data & 0x80 ? 1 : 0;
            pia_set_input_ca1(SNS_PIA1,
                              snslocals.pia1ca1); // CA1 should connect to GND according to schematics... or to DB7! :)
                                                  //      logerror("sns_data_ command stored %x\n", data);
            // cpu reads command from adress b0 after nmi !!!
            if (!(data & 0x40))
                cpu_set_nmi_line(ZACSND_CPUB, data & 0x80 ? ASSERT_LINE : CLEAR_LINE);
            break;
        case SNDBRD_ZAC13181x3:
            if (!(data & 0x40))
                cpu_set_nmi_line(ZACSND_CPUA, data & 0x80 ? ASSERT_LINE : CLEAR_LINE);
            if (!(data & 0x40))
                cpu_set_nmi_line(ZACSND_CPUB, data & 0x80 ? ASSERT_LINE : CLEAR_LINE);
            if (data & 0x40)
                cpu_set_nmi_line(ZACSND_CPUC, data & 0x80 ? ASSERT_LINE : CLEAR_LINE);
    }
}
...
static WRITE_HANDLER(sns_8910b_w) {
    ...
    if ((lastAy & 0x0f) != (data & 0x0f))
        logerror("AY8910  modulation: %x\n", data & 0x0f);
    lastAy = data;
}
...
static void
sns_5220Irq(int state) {
    if ((core_getDip(0) >> 4) != snslocals.tmsPitch)
        tms5220_set_frequency((93 + (snslocals.tmsPitch = (core_getDip(0) >> 4)))
                              * 6666.66666666666); // 0:620kHz .. 15:720kHz
...
static void
sns_5220Rdy(int state) {
    if (core_gameData->hw.soundBoard == SNDBRD_ZAC1370 || core_gameData->hw.soundBoard == SNDBRD_ZAC13136
        || core_gameData->hw.soundBoard & 0x02)
...
static int
sns_sh_start(const struct MachineSound* msound) {
    UINT8 i;
...
static MEMORY_READ_START(sns3_readmem){0x0000, 0x007f, MRA_RAM}, {0x0090, 0x0093, pia_r(SNS_PIA1)},
    {0x00b0, 0x00bf, readcmd}, {0x00f0, 0x00ff, readlatch}, {0x5000, 0x5000, MRA_NOP}, {0x8000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(sns3_writemem){0x0000, 0x007f, MWA_RAM},
    {0x0080, 0x0087, chip3h259}, {0x0090, 0x0093, pia_w(SNS_PIA1)}, {0x00a0, 0x00a7, chip3i259},
    {0x00c0, 0x00cf, dacxfer}, {0x00d0, 0x00df, storebyte1}, {0x00e0, 0x00ef, storebyte2}, {0x00f0, 0x00ff, storelatch},
    MEMORY_END

    MACHINE_DRIVER_START(zac11178) MDRV_CPU_ADD(M6802, 3579545. / 4.) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(sns3_readmem, sns3_writemem) MDRV_CPU_PERIODIC_INT(sns3_irq, SNS_11178_IRQFREQ)

            MDRV_INTERLEAVE(500) MDRV_SOUND_ADD(TMS5220, sns_tms5220Int)
                MDRV_SOUND_ADD(CUSTOM, sns_custInt) MACHINE_DRIVER_END
...
static INTERRUPT_GEN(sns3_irq) {
    ...
    if (keyboard_pressed_memory_repeat(KEYCODE_B, 10)) {
        pia_set_input_ca1(SNS_PIA1, 1);
...
static READ_HANDLER(readlatch) { return snslocals.daclatch; }

static WRITE_HANDLER(storelatch) { snslocals.daclatch = data; }

static WRITE_HANDLER(storebyte1) { snslocals.dacbyte1 = snslocals.daclatch; }

static WRITE_HANDLER(storebyte2) { snslocals.dacbyte2 = snslocals.daclatch; }
...
static MEMORY_READ_START(z80_readmem){0x0000, 0xfbff, MRA_ROM}, {0xfc00, 0xffff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(z80_writemem){0xfc00, 0xffff, MWA_RAM},
    MEMORY_END

    static WRITE_HANDLER(DAC_2_signed_data_w) {
...
static PORT_READ_START(z80_readport){0x01, 0x01, readcmd},
    PORT_END

    static PORT_WRITE_START(z80_writeport_a){0x00, 0x00, DAC_4_signed_data_w},
    {0x02, 0x02, snd_mod_w}, PORT_END static PORT_WRITE_START(z80_writeport_b){0x00, 0x00, DAC_0_signed_data_w},
    {0x02, 0x03, snd_mod_w}, {0x04, 0x04, DAC_1_signed_data_w}, {0x08, 0x08, DAC_2_signed_data_w},
    PORT_END

    static PORT_READ_START(z80_readport_c){0x01, 0x01, readcmd},
    {0x03, 0x03, tms5220_status_r}, PORT_END static PORT_WRITE_START(z80_writeport_c){0x00, 0x00, DAC_3_signed_data_w},
    {0x02, 0x02, snd_act_w}, {0x03, 0x03, tms5220_data_w},
    PORT_END

    static INTERRUPT_GEN(cpu_b_irq) {
    cpu_set_irq_line(ZACSND_CPUB, 0, PULSE_LINE);
}

static INTERRUPT_GEN(cpu_c_irq) { cpu_set_irq_line(ZACSND_CPUC, 0, PULSE_LINE); }

static MACHINE_DRIVER_START(zac11178_13181_nodac) MDRV_IMPORT_FROM(zac11178)

    MDRV_CPU_ADD(Z80, 4000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(z80_readmem, z80_writemem)
        MDRV_CPU_PORTS(z80_readport, z80_writeport_b) MDRV_CPU_PERIODIC_INT(cpu_b_irq, 60) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(zac11178_13181) MDRV_IMPORT_FROM(zac11178_13181_nodac)
        MDRV_SOUND_ADD(DAC, z80_1dacInt) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(zac11178_11181) MDRV_IMPORT_FROM(zac11178_13181_nodac)
        MDRV_SOUND_ADD(DAC, z80_3dacInt) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(zac11183) MDRV_CPU_ADD(Z80, 4000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(z80_readmem, z80_writemem) MDRV_CPU_PORTS(z80_readport, z80_writeport_a)

            MDRV_CPU_ADD(Z80, 4000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(z80_readmem, z80_writemem)
                MDRV_CPU_PORTS(z80_readport, z80_writeport_b)

                    MDRV_CPU_ADD(Z80, 4000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(z80_readmem, z80_writemem)
                        MDRV_CPU_PORTS(z80_readport_c, z80_writeport_c) MDRV_CPU_PERIODIC_INT(cpu_c_irq, 60)

                            MDRV_INTERLEAVE(500) MDRV_SOUND_ADD(TMS5220, sns_tms5220Int)
                                MDRV_SOUND_ADD(DAC, z80_5dacInt) MACHINE_DRIVER_END
