...
static int segMap[] = {4, 0, -4, 4, 0, -4, 4, 0, -4, 4, 0, -4, 0, 0};
...
static SWITCH_UPDATE(taito) {
    if (inports) {
        ...
        if (inports[TAITO_COMINPORT] & 0x8000)
            sndbrd_0_diag(1);
    }
}
...
static WRITE_HANDLER(switches_w) { logerror("switch write: %i %i\n", offset, data); }
...
static WRITE_HANDLER(dma_display) {
    ...
    if (offset < 12) {
        ...
        switch (offset) {
            ...
            case 14:
                // match
                TAITOlocals.segments[2 * 13 + segMap[13]].w =
                    (TAITOlocals.lampMatrix[7] & 0x80) ? core_bcd2seg7e[data & 0x0f] : 0;
                break;
...
static int
TAITO_m2sw(int col, int row) {
    return (row * 10) + (col - 1);
}

static MEMORY_READ_START(taito_readmem){0x0000, 0x27ff, MRA_ROM},
    {0x2800, 0x2808, switches_r}, /* some games use different locations */
    {0x2880, 0x2888, switches_r}, /* -"- */
    {0x28d8, 0x28df, switches_r}, /* -"- */
    {0x3e00, 0x3fff, MRA_RAM}, {0x4000, 0x40ff, MRA_RAM}, {0x4800, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(taito_writemem){0x0000, 0x3dff, MWA_NOP},
    {0x3e00, 0x3fff, MWA_RAM}, {0x4000, 0x407f, MWA_RAM}, {0x4080, 0x408f, dma_display}, {0x4090, 0x409f, dma_commands},
    {0x40a0, 0x40ff, MWA_RAM},
    MEMORY_END

    static MEMORY_READ_START(taito_readmem_old){0x0000, 0x0fff, MRA_ROM},
    {0x1000, 0x10ff, MRA_RAM}, {0x1400, 0x1408, switches_r}, {0x14d8, 0x14df, switches_r}, {0x1800, 0x1bff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(taito_writemem_old){0x0000, 0x0fff, MWA_NOP},
    {0x1000, 0x100f, dma_display}, {0x1010, 0x101f, dma_commands}, {0x1020, 0x10ff, MWA_RAM},
    {0x1400, 0x1408, switches_w},
    MEMORY_END

    MACHINE_DRIVER_START(taito) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(taito, NULL, taito)
        MDRV_CPU_ADD_TAG("mcpu", 8080, 17000000. / 9.) MDRV_CPU_MEMORY(taito_readmem, taito_writemem)
            MDRV_CPU_VBLANK_INT(taito_vblank, 1) MDRV_NVRAM_HANDLER(taito) MDRV_DIPS(8) MDRV_SWITCH_UPDATE(taito)
                MDRV_SWITCH_CONV(TAITO_sw2m, TAITO_m2sw) MDRV_LAMP_CONV(TAITO_lamp2m, TAITO_m2lamp)
                    MDRV_DIAGNOSTIC_LEDH(1) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(taito_sintetizador) MDRV_IMPORT_FROM(taito)

        MDRV_IMPORT_FROM(taitos_sintetizador) MDRV_SOUND_CMD(taito_sndCmd_w)
            MDRV_SOUND_CMDHEADING("taito") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(taito_sintetizador_nmi) MDRV_IMPORT_FROM(taito)

        MDRV_IMPORT_FROM(taitos_sintetizador_nmi) MDRV_SOUND_CMD(taito_sndCmd_w)
            MDRV_SOUND_CMDHEADING("taito") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(taito_sintetizadorpp) MDRV_IMPORT_FROM(taito)

        MDRV_IMPORT_FROM(taitos_sintetizadorpp) MDRV_SOUND_CMD(taito_sndCmd_w)
            MDRV_SOUND_CMDHEADING("taito") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(taito_sintetizadorpp_nmi) MDRV_IMPORT_FROM(taito)

        MDRV_IMPORT_FROM(taitos_sintetizadorpp_nmi) MDRV_SOUND_CMD(taito_sndCmd_w)
            MDRV_SOUND_CMDHEADING("taito") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(taito_sintevox) MDRV_IMPORT_FROM(taito)

        MDRV_IMPORT_FROM(taitos_sintevox) MDRV_SOUND_CMD(taito_sndCmd_w)
            MDRV_SOUND_CMDHEADING("taito") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(taito_sintevoxpp) MDRV_IMPORT_FROM(taito)

        MDRV_IMPORT_FROM(taitos_sintevoxpp) MDRV_SOUND_CMD(taito_sndCmd_w)
            MDRV_SOUND_CMDHEADING("taito") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(taito_old) MDRV_IMPORT_FROM(taito_sintetizador)

        MDRV_CORE_INIT_RESET_STOP(taito_old, NULL, taito) MDRV_CPU_REPLACE("mcpu", 8080, 19000000. / 9.) //!! ??
    MDRV_CPU_MEMORY(taito_readmem_old, taito_writemem_old) MDRV_NVRAM_HANDLER(taito_old) MACHINE_DRIVER_END
...
static WRITE_HANDLER(lamp_w) { coreGlobals.lampMatrix[z80locals.swCol] = data; }
...
static WRITE_HANDLER(p12_w) { logerror("Port 0x1%d write = %02x\n", offset + 2, data); }

static WRITE_HANDLER(p22_w) { logerror("Port 0x2%d write = %02x\n", offset + 2, data); }

static MEMORY_READ_START(z80_readmem){0x0000, 0x1fff, MRA_ROM}, {0x4800, 0x4bff, MRA_RAM},
    MEMORY_END static MEMORY_WRITE_START(z80_writemem){0x4800, 0x4bff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    {0x5800, 0x5800, col_w},
    MEMORY_END

    static PORT_READ_START(z80_readport){0x21, 0x21, sw_r},
    PORT_END static PORT_WRITE_START(z80_writeport){0x10, 0x10, disp_w}, {0x11, 0x11, taito_sndCmd_w},
    {0x12, 0x13, p12_w}, {0x20, 0x20, lamp_w}, {0x21, 0x21, sol_w}, {0x22, 0x23, p22_w},
    PORT_END
...
static MACHINE_STOP(taitoz80) { sndbrd_0_exit(); }
...
static core_tLCDLayout disp[] = {{0, 0, 26, 6, CORE_SEG7},
                                 {3, 0, 20, 6, CORE_SEG7},
                                 {6, 0, 14, 6, CORE_SEG7},
                                 {9, 0, 8, 6, CORE_SEG7},
                                 {13, 0, 7, 1, CORE_SEG7},
...
TAITO_ROMSTART2222(mrblkz80, "mb01z80.dat", CRC(7f883a70) SHA1(848783123b55ade769cac3c1b3d4a2c759a6c5b6), "mb02z80.dat",
                   CRC(68de8f50) SHA1(7076297060e927da1aefae8bf75c8cda18031660), "mb03z80.dat",
                   CRC(5a8e55e8) SHA1(b93102254004d258998bd6ab7d7b333361b37830), "mb04z80.dat",
                   CRC(ecf30c2f) SHA1(404c891bc420cfe540e829a1cd05ced10ea5a09c))
TAITO_SOUNDROMS444("mrbz80s1.bin", NO_DUMP, "mrbz80s2.bin", NO_DUMP, "mrbz80s3.bin", NO_DUMP)
TAITO_ROMEND
