...
/-----------------------------------------*/
void
sim_run(int* inports, int firstGameInport, int useSimKeys, int noOfBalls) {
    int ii;
  ...
  /-----------------------*/
    ...
    if (inports[CORE_SIMINPORT] & SIM_NEXTKEY)
        locals.currBall += 1;
    if (inports[CORE_SIMINPORT] & SIM_PREVKEY)
        locals.currBall -= 1;
    if (locals.currBall < 0)
        locals.currBall = noOfBalls - 1;
    if (locals.currBall >= noOfBalls)
        locals.currBall = 0;
        ...
        while (iData->mask) {
            if (!eventFound && (inports[iData->port + firstGameInport] & iData->mask) == iData->mask) {
                ...
                if ((iData->condition == SIM_STANY) || (inports[CORE_SIMINPORT] & SIM_IGNOREKEY)
                    ...
                    || ((iData->condition > 0) && (iData->condition < SIM_STATES) && (ball->state == stFree)
                        && core_getSw(iData->condition))
  ...
  /-----------------------------------------------*/
    if (simData->manShooter) {
        ...
        if (inports[CORE_SIMINPORT] & SIM_SHOOTERKEY) {
            ...
            if (locals.shooterRel == 1)
                locals.shooterSpeed = 0;
            if (locals.shooterRel > 0)
                locals.shooterRel -= 1;
            else
                locals.shooterRel = SIM_SHOOTRELTIME;
        }
    }
        ...
        for (ii = 0; ii < SIM_MAXBALLS; ii++) {
            ...
            while (ball->timer == 0) {
                ...
                if (ball->state != ball->nextState) {
                    ...
                    if ((sd->solSwNo) && !(sd->flags & (SIM_STSWON | SIM_STSWOFF)) && core_getSol(sd->solSwNo)) {
                        ...
                        for (jj = 0; jj < SIM_MAXBALLS; jj++) {
                            if ((jj != ii)
                                && ((locals.balls[jj].state == ball->nextState)
                                    || ((ball->state != stPending) && (locals.balls[jj].nextState == ball->nextState)
                    ...
                    if (sd->flags & SIM_STSHOOT)
                        ball->speed = locals.shooterSpeed;
                    if (sd->nextState) {
                        ball->nextState =
                            ((sd->altSolSw)
...
sim_draw(int firstRow) {
    ...
    if (simData->stateData) {
        for (ii = 0; ii < SIM_MAXBALLS; ii++)
            if (locals.balls[ii].state != stPending)
                core_textOutf(160, 10 * ii + 10, locals.balls[ii].current ? WHITE : BLACK, "%c %-20s",
                              locals.balls[ii].type ? '*' : ' ',
    ...
    if (simData->manShooter) {
        ...
        rec.min_x = 131 + locals.shooterSpeed / 2;
        rec.max_x = 155;
...
/---------------------------*/
int
sim_init(sim_tSimData* gameSimData, int* inports, int firstGameInport) {
    int ii;
...
/-------------------*/
static void
sim_startSpin(int swNo, int time) {
...
sim_updateSpin(void) {
    ...
    for (ii = 0; ii < SIM_MAXSPIN; ii++)
        if (locals.spinner[ii].swNo) {
            DBGLOG(("spin: count=%3d next=%3d\n", locals.spinner[ii].spinCount, locals.spinner[ii].nextSpin));
