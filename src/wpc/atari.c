...
static void
ATARI2_irq(int state) {
    cpu_set_irq_line(ATARI_CPU, M6800_IRQ_LINE, state ? ASSERT_LINE : CLEAR_LINE);
}

static INTERRUPT_GEN(ATARI2_irqhi) { ATARI2_irq(1); }

static WRITE_HANDLER(intack_w) { ATARI2_irq(0); }
...
static SWITCH_UPDATE(ATARI0) {
    if (inports) {
        coreGlobals.swMatrix[1] = (coreGlobals.swMatrix[1] & 0xc0) | (inports[ATARI_COMINPORT] & 0x0f)
                                  | ((inports[ATARI_COMINPORT] & 0x0300) >> 4);
...
// Gen 2
static READ_HANDLER(sw_r) { return ~coreGlobals.swMatrix[offset + 1]; }
...
static WRITE_HANDLER(ram_w1) { ram[offset + 0x81] = data; }

static WRITE_HANDLER(ram_w2) { ram[offset + 0x85] = data; }

static WRITE_HANDLER(ram_w3) { ram[offset + 0x89] = data; }

static WRITE_HANDLER(ram_w4) { ram[offset + 0x8d] = data; }
...
// Gen 2
static WRITE_HANDLER(disp0_w) { locals.pseg[offset].w = core_bcd2seg7a[data & 0x0f]; }
...
// Gen 1
static void
setLatch(int no, UINT8 data, UINT8 mask, int cnt) {
    /* table to adjust for original Atari sol numbers */
    static UINT32 solTable[] = {0x00001, 0x00010, 0x04000, 0x00040, 0x00002, 0x00020, 0x00008,
                                0x08000, 0x00800, 0x00004, 0x00080, 0x02000, 0x00400, 0x00200,
                                0x00100, 0x01000, 0x20000, 0x10000, 0x40000, 0x80000};
    latch[no] = data;
    if (data & mask) {
        int i;
        for (i = 0; i < cnt; i++) {
...
static WRITE_HANDLER(latch108c_w) { setLatch(3, data, 0xff, 8); }
...
static READ_HANDLER(latch1080_r) { return latch[0]; }

static READ_HANDLER(latch1084_r) { return latch[1]; }

static READ_HANDLER(latch1088_r) { return latch[2]; }

static READ_HANDLER(latch108c_r) { return latch[3]; }
...
static WRITE_HANDLER(sound1_w) { sndbrd_data_w(0, data); }

/*-----------------------------------------
/  Memory map for CPU board (GENERATION 1)
/------------------------------------------*/
static MEMORY_READ_START(ATARI1_readmem){0x0000, 0x01ff, ram_r}, /* RAM */
    ...
    static MEMORY_WRITE_START(ATARI1_writemem){0x0000, 0x01ff, ram_w, &ram}, /* RAM */
    ...
    static UINT8* atari_RAM,
    *atari_CMOS;

static NVRAM_HANDLER(ATARI) { core_nvram(file, read_or_write, atari_CMOS, 0x100, 0x00); }
...
/------------------------------------------*/
static MEMORY_READ_START(ATARI2_readmem){0x0000, 0x00ff, ram2_r}, /* RAM */
    ...
    static MEMORY_WRITE_START(ATARI2_writemem){0x0000, 0x00ff, ram2_w, &atari_RAM}, /* RAM */
...
static MACHINE_INIT(ATARI2) { init_common(); }

static MACHINE_STOP(ATARI) { sndbrd_0_exit(); }
...
/  Memory map for 4x4
/------------------------------------------*/
static MEMORY_READ_START(ATARI4_readmem){0x0000, 0x07ff, MRA_RAM}, {0x1000, 0x17ff, MRA_RAM}, {0x2000, 0x27ff, MRA_RAM},
    {0x3000, 0x37ff, MRA_RAM}, {0x8000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(ATARI4_writemem){0x0000, 0x07ff, MWA_RAM},
    {0x1000, 0x17ff, MWA_RAM}, {0x2000, 0x27ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
