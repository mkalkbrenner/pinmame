...
static SWITCH_UPDATE(tecno) {
    if (inports) {
        ...
        coreGlobals.swMatrix[2] =
            (coreGlobals.swMatrix[2] & 0xfc) | (inports[CORE_COREINPORT] & 0x0300) >> 8; //Column 1 Switches
    }
}
...
static READ16_HANDLER(input_key_r) {
    UINT8 switches =
        coreGlobals.swMatrix
            [locals.LampCol
             + 1]; //+1 so we begin by reading column 1 of input matrix instead of 0 which is used for special switches in many drivers
...
//Solenoids 17-32
static WRITE16_HANDLER(sol2_w) {
    coreGlobals.pulsedSolState = (coreGlobals.pulsedSolState & 0x0000FFFF) | (data << 16);
}
...
static NVRAM_HANDLER(tecno_nvram) { core_nvram(file, read_or_write, NVRAM, 0x2000, 0x00); }

//Memory Map for Main CPU
static MEMORY_READ16_START(readmem){0x000000, 0x003fff, MRA16_ROM}, {0x004000, 0x005fff, MRA16_RAM},
    {0x006000, 0x00ffff, MRA16_ROM}, {0x014000, 0x014001, input_key_r}, {0x014800, 0x014801, input_sound_r},
    {0x015000, 0x015001, rtrg_r}, {0x015800, 0x015801, rtrg2_r},
    MEMORY_END

    static MEMORY_WRITE16_START(writemem){0x000000, 0x003fff, MWA16_ROM},
    {0x004000, 0x005fff, MWA16_RAM, &NVRAM}, {0x014000, 0x014001, lamp1_w}, {0x014800, 0x014801, lamp2_w},
    {0x015000, 0x015001, sol1_w}, {0x015800, 0x015801, sol2_w}, {0x016000, 0x016001, sound_w},
    {0x016800, 0x016801, disp1_w}, {0x017000, 0x017001, disp2_w}, {0x017800, 0x017801, setout_w},
    MEMORY_END

    /* Manual starts with a switch # of 0 */
    static int tecno_sw2m(int no) {
    return no + 7 + 1;
}

static int
tecno_m2sw(int col, int row) {
...
static MEMORY_READ_START(snd_readmem){0x3000, 0x3000, Y8950_status_port_0_r}, {0x3001, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(snd_writemem){0x5800, 0x5800, Y8950_control_port_0_w},
    {0x5801, 0x5801, Y8950_write_port_0_w}, {0x6800, 0x6800, DAC_0_data_w}, {0x7000, 0x7000, m7000_w},
    MEMORY_END
...
static PORT_READ_START(snd_readport){TMS7000_PORTA, TMS7000_PORTD, tms_port_r},
    PORT_END

    static PORT_WRITE_START(snd_writeport){TMS7000_PORTA, TMS7000_PORTD, tms_port_w},
    PORT_END
...
static WRITE_HANDLER(key_w) { LOG(("Y8910 key_w %02x\n", data)); }
...
const struct sndbrdIntf tecnoplayIntf = {"TECNOPLAY",   NULL, NULL, NULL, tecsnd_data_w,
                                         tecsnd_data_w, NULL, NULL, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
...
PORT_START /* 0 */
    ...
    /* Switch Column 1 */
    COREPORT_BIT(0x0001, "Test Up", KEYCODE_7) COREPORT_BIT(0x0002, "Test Down", KEYCODE_8)
        COREPORT_BIT(0x0004, "Ball Tilt", KEYCODE_INSERT) COREPORT_BIT(0x0008, "Service", KEYCODE_9)
            COREPORT_BITDEF(0x0010, IPT_COIN1, KEYCODE_3) COREPORT_BITDEF(0x0020, IPT_COIN2, KEYCODE_4)
                COREPORT_BITDEF(0x0040, IPT_COIN3, KEYCODE_5)
                    COREPORT_BIT(0x0080, "Letter Sel ->", KEYCODE_PGUP) /* Switch Column 2 */
    COREPORT_BIT(0x0100, "Letter Sel <-", KEYCODE_PGDN)
        COREPORT_BITDEF(0x0200, IPT_START1, IP_KEY_DEFAULT) PORT_START /* 1 */
    ...
    static core_tLCDLayout disp[] = {{0, 0, 0, 16, CORE_SEG16S}, {3, 0, 16, 16, CORE_SEG16S}, {0}};
...
static core_tLCDLayout disp2[] = {{0, 0, 0, 1, CORE_SEG16S},
                                  {0, 2, 32, 15, CORE_SEG16S},
                                  {3, 0, 16, 1, CORE_SEG16S},
                                  {3, 2, 48, 15, CORE_SEG16S},
                                  {0}};
