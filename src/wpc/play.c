...
static WRITE_HANDLER(out2_n) {
    static int outports[4][7] = {{DISPCOL, DISPLAY, SOUND, SWITCH, DIAG, LAMPCOL, LAMP},
...
static UINT8
in_mode(void) {
    return locals.resetDone ? CDP1802_MODE_RUN : CDP1802_MODE_RESET;
}
...
static UINT8
in_ef(void) {
    return locals.ef[1] | (locals.ef[2] << 1) | (locals.ef[3] << 2) | (locals.ef[4] << 3);
}
...
static MACHINE_INIT(PLAYMATIC1) { init_common(0); }

static MACHINE_INIT(PLAYMATIC2) { init_common(1); }
...
static MACHINE_INIT(PLAYMATIC4) { init_common(3); }

static MACHINE_STOP(PLAYMATIC) { sndbrd_0_exit(); }

static PORT_READ_START(PLAYMATIC_readport1){0x00, 0x07, in1_n},
    MEMORY_END

    static PORT_READ_START(PLAYMATIC_readport2){0x00, 0x07, in2_n},
    MEMORY_END

    static PORT_WRITE_START(PLAYMATIC_writeport1){0x00, 0x07, out1_n},
    MEMORY_END

    static PORT_WRITE_START(PLAYMATIC_writeport2){0x00, 0x07, out2_n},
    MEMORY_END

    static MEMORY_READ_START(PLAYMATIC_readmem1){0x0000, 0x07ff, MRA_ROM},
    {0x0800, 0x081f, MRA_RAM}, {0x0c00, 0x0c1f, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(PLAYMATIC_writemem1){0x0800, 0x081f, MWA_RAM, &generic_nvram, &generic_nvram_size},
    {0x0c00, 0x0c1f, MWA_RAM},
    MEMORY_END

    static MEMORY_READ_START(PLAYMATIC_readmem1a){0x0000, 0x09ff, MRA_ROM},
    {0x0c00, 0x0c1f, MRA_RAM}, {0x0e00, 0x0e1f, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(PLAYMATIC_writemem1a){0x0c00, 0x0c1f, MWA_RAM},
    {0x0e00, 0x0e1f, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static MEMORY_READ_START(PLAYMATIC_readmem2){0x0000, 0x1fff, MRA_ROM},
    {0x2000, 0x20ff, MRA_RAM}, {0xa000, 0xafff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(PLAYMATIC_writemem2){0x0000, 0x00ff, MWA_NOP},
    {0x2000, 0x20ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static MEMORY_READ_START(PLAYMATIC_readmem3){0x0000, 0x7fff, MRA_ROM},
    {0x8000, 0x80ff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(PLAYMATIC_writemem3){0x0000, 0x00ff, MWA_NOP},
    {0x8000, 0x80ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static int play_sw2m(int no) {
    return 8 + (no / 10) * 8 + (no % 10 - 1);
}

static int
play_m2sw(int col, int row) {
...
MDRV_CPU_PERIODIC_INT(PLAYMATIC_irq1,
                      800) // actually 100 Hz (zc freq.) but needs uneven distribution of HI and LO states!
...
static MACHINE_DRIVER_START(PLAYMATIC2NS) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", CDP1802, 2950000)
    MDRV_CPU_MEMORY(PLAYMATIC_readmem2, PLAYMATIC_writemem2) MDRV_CPU_PORTS(PLAYMATIC_readport2, PLAYMATIC_writeport2)
        MDRV_CPU_CONFIG(play1802_config) MDRV_CPU_PERIODIC_INT(PLAYMATIC_irq2, 2950000 / 8192.)
            MDRV_TIMER_ADD(PLAYMATIC_zeroCross2, 100) MDRV_CPU_VBLANK_INT(PLAYMATIC_vblank2, 1)
                MDRV_CORE_INIT_RESET_STOP(PLAYMATIC2, NULL, PLAYMATIC) MDRV_SWITCH_UPDATE(PLAYMATIC2)
                    MDRV_SWITCH_CONV(play_sw2m, play_m2sw) MDRV_NVRAM_HANDLER(generic_0fill) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(PLAYMATIC2) MDRV_IMPORT_FROM(PLAYMATIC2NS) MDRV_IMPORT_FROM(PLAYMATICS2) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(PLAYMATIC2SZ) MDRV_IMPORT_FROM(PLAYMATIC2NS) MDRV_IMPORT_FROM(PLAYMATICSZ) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(PLAYMATIC2S3) MDRV_IMPORT_FROM(PLAYMATIC2NS) MDRV_IMPORT_FROM(PLAYMATICS3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(PLAYMATIC2S4) MDRV_IMPORT_FROM(PLAYMATIC2NS) MDRV_IMPORT_FROM(PLAYMATICS4) MACHINE_DRIVER_END

    static MACHINE_DRIVER_START(PLAYMATIC3) MDRV_IMPORT_FROM(PLAYMATIC2NS)
        MDRV_CORE_INIT_RESET_STOP(PLAYMATIC3, NULL, PLAYMATIC) MDRV_CPU_MODIFY("mcpu");
...
static INTERRUPT_GEN(PLAYMATIC_irqBingo) { cpu_set_irq_line(PLAYMATIC_CPU, CDP1802_INPUT_LINE_INT, ASSERT_LINE); }

static void
PLAYMATIC_zeroCrossBingo(int data) {
    locals.ef[1] = !locals.ef[1];
}
...
static PORT_READ_START(PLAYMATIC_readportBingo){0x00, 0x07, in_bingo},
    MEMORY_END

    static PORT_WRITE_START(PLAYMATIC_writeportBingo){0x00, 0x07, out_bingo},
    MEMORY_END

    MACHINE_DRIVER_START(PLAYMATICBINGO) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", CDP1802, 2950000)
        MDRV_CPU_MEMORY(PLAYMATIC_readmem3, PLAYMATIC_writemem3)
            MDRV_CPU_PORTS(PLAYMATIC_readportBingo, PLAYMATIC_writeportBingo) MDRV_CPU_CONFIG(play1802_config)
                MDRV_CPU_PERIODIC_INT(PLAYMATIC_irqBingo, 3579545 / 8192.) MDRV_TIMER_ADD(PLAYMATIC_zeroCrossBingo, 100)
                    MDRV_CPU_VBLANK_INT(PLAYMATIC_vblank2, 1) MDRV_CORE_INIT_RESET_STOP(PLAYMATICBINGO, NULL, PLAYMATIC)
                        MDRV_SWITCH_UPDATE(PLAYMATICBINGO) MDRV_NVRAM_HANDLER(generic_0fill) MACHINE_DRIVER_END
