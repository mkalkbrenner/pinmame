...
#ifdef VPINMAME
#define GAME_NOCRC                                                                                                     \
    0x2000 //This should allow mame team to add at least a few more flags to gamedrv before it's a problem
#else
#define GAME_NOCRC 0
#endif
...
typedef struct {
    int lampNo, currStat;
} vp_tChgLamps[CORE_MAXLAMPCOL * 8];
...
typedef struct {
    int giNo, currStat;
} vp_tChgGIs[CORE_MAXGI];

typedef struct {
    int ledNo, chgSeg, currStat;
} vp_tChgLED[128];

typedef struct {
    int sndNo;
} vp_tChgSound[MAX_CMD_LOG];

typedef struct {
    int nvramNo, oldStat, currStat;
} vp_tChgNVRAMs[CORE_MAXNVRAM];
...
/-------------------------------------*/
INLINE void
vp_putSwitch(int swNo, int newStat) {
    core_setSw(swNo, newStat);
}
...
/-------------------------------------*/
INLINE int
vp_getSwitch(int swNo) {
    return core_getSw(swNo);
}
