...
/*Interface*/
const struct sndbrdIntf alvgdmdIntf = {NULL,         dmd32_init, NULL,         NULL, NULL,
                                       dmd32_data_w, NULL,       dmd32_ctrl_w, NULL, SNDBRD_NOTSOUND};

/*Main CPU sends command to DMD*/
static WRITE_HANDLER(dmd32_data_w) { dmdlocals.cmd = data; }

static WRITE_HANDLER(control_w) {
    UINT16 tmpoffset = offset;
    tmpoffset +=
        0x8000; //Adjust the offset so we can work with the "actual address" as it really is (just makes it easier, it's not necessary to do this)
...
static MEMORY_READ_START(alvgdmd_readmem){0x000000, 0x007fff, MRA_ROM}, {0x008000, 0x00ffff, MRA_BANK1},
    {0x010000, 0x017fff, MRA_RAM}, {0x018000, 0x01ffff, control_r},
    MEMORY_END
    ...
    {0x010000, 0x017fff, MWA_RAM, &dmd32RAM}, {0x018000, 0x01ffff, control_w},
    MEMORY_END

    static PORT_READ_START(alvgdmd_readport){0x00, 0xff, port_r},
    PORT_END

    static PORT_WRITE_START(alvgdmd_writeport){0x00, 0xff, port_w},
    PORT_END

    // Al's Garage Band Goes On A World Tour
    MACHINE_DRIVER_START(alvgdmd1) MDRV_CPU_ADD(I8051, 12000000) /*12 Mhz*/
    MDRV_CPU_MEMORY(alvgdmd_readmem, alvgdmd_writemem) MDRV_CPU_PORTS(alvgdmd_readport, alvgdmd_writeport)
        MDRV_CPU_PERIODIC_INT(dmd32_firq, DMD32_FIRQFREQ) MDRV_INTERLEAVE(50) MACHINE_DRIVER_END

    // Pistol Poker
    MACHINE_DRIVER_START(alvgdmd2) MDRV_CPU_ADD(I8051, 12000000) /*12 Mhz*/
    MDRV_CPU_MEMORY(alvgdmd_readmem, alvgdmd_writemem) MDRV_CPU_PORTS(alvgdmd_readport, alvgdmd_writeport)
        MDRV_CPU_PERIODIC_INT(dmd32_firq, DMD32_FIRQFREQ) MDRV_INTERLEAVE(50) MACHINE_DRIVER_END
    ...
    MACHINE_DRIVER_START(alvgdmd3) MDRV_CPU_ADD(I8051, 24000000) /*24 Mhz*/ // retweak?
    MDRV_CPU_MEMORY(alvgdmd_readmem, alvgdmd_writemem) MDRV_CPU_PORTS(alvgdmd_readport, alvgdmd_writeport)
        MDRV_CPU_PERIODIC_INT(dmd32_firq, DMD32_FIRQFREQ) MDRV_INTERLEAVE(50) MACHINE_DRIVER_END

//Use only for testing the 8031 core emulation
#ifdef MAME_DEBUG
    MACHINE_DRIVER_START(test8031) MDRV_CPU_ADD(I8051, 12000000) /*12 Mhz*/
    MDRV_CPU_MEMORY(alvgdmd_readmem, alvgdmd_writemem) MDRV_CPU_PORTS(alvgdmd_readport, alvgdmd_writeport)
        MDRV_CPU_PERIODIC_INT(dmd32_firq, DMD32_FIRQFREQ) MACHINE_DRIVER_END
#endif
...
static void
pistol_poker__mystery_castle_dmd(void) {
    UINT8* RAM = ((UINT8*)dmd32RAM);
    int ii, jj;
    ...
    if (dmdlocals.planenable) {
        ...
        for (ii = 1; ii <= 32; ii++) {
            UINT8* line = &coreGlobals.dotCol[ii][0];
            for (jj = 0; jj < (128 / 8); jj++) {
                *line++ = level[((RAM[0] >> 7 & 0x01) + (RAM[0x200] >> 7 & 0x01) + (RAM[0x400] >> 7 & 0x01)
                                 + (RAM[0x600] >> 7 & 0x01))];
                *line++ = level[((RAM[0] >> 6 & 0x01) + (RAM[0x200] >> 6 & 0x01) + (RAM[0x400] >> 6 & 0x01)
                                 + (RAM[0x600] >> 6 & 0x01))];
                *line++ = level[((RAM[0] >> 5 & 0x01) + (RAM[0x200] >> 5 & 0x01) + (RAM[0x400] >> 5 & 0x01)
                                 + (RAM[0x600] >> 5 & 0x01))];
                *line++ = level[((RAM[0] >> 4 & 0x01) + (RAM[0x200] >> 4 & 0x01) + (RAM[0x400] >> 4 & 0x01)
                                 + (RAM[0x600] >> 4 & 0x01))];
                *line++ = level[((RAM[0] >> 3 & 0x01) + (RAM[0x200] >> 3 & 0x01) + (RAM[0x400] >> 3 & 0x01)
                                 + (RAM[0x600] >> 3 & 0x01))];
                *line++ = level[((RAM[0] >> 2 & 0x01) + (RAM[0x200] >> 2 & 0x01) + (RAM[0x400] >> 2 & 0x01)
                                 + (RAM[0x600] >> 2 & 0x01))];
                *line++ = level[((RAM[0] >> 1 & 0x01) + (RAM[0x200] >> 1 & 0x01) + (RAM[0x400] >> 1 & 0x01)
                                 + (RAM[0x600] >> 1 & 0x01))];
                *line++ = level[((RAM[0] /*>>0*/ & 0x01) + (RAM[0x200] /*>>0*/ & 0x01) + (RAM[0x400] /*>>0*/ & 0x01)
                                 + (RAM[0x600] /*>>0*/ & 0x01))];
#ifdef VPINMAME
        ...
        for (ii = 1; ii <= 32; ii++) {
            ...
            for (jj = 0; jj < (128 / 8); jj++) {
                ...
                *line++ = level[(RAM[0] << 2 & 0x04)];
#ifdef VPINMAME
                g_raw_gtswpc_dmd[i] = g_raw_gtswpc_dmd[0x200 + i] = g_raw_gtswpc_dmd[0x400 + i] =
