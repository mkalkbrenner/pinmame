...
/--------------------------------*/
static INTERRUPT_GEN(barni_vblank) { core_updateSw(core_getSol(17)); }
...
static WRITE_HANDLER(gram_w) { memory_region(REGION_CPU1)[offset] = memory_region(REGION_CPU2)[offset] = data; }

static READ_HANDLER(gram_r) { return memory_region(REGION_CPU1)[offset]; }
...
static WRITE_HANDLER(pia0a_w) { locals.strobe1 = core_BitColToNum(data); }

static WRITE_HANDLER(pia1a_w) { locals.strobe2 = core_BitColToNum(data & 0x0f); }

static READ_HANDLER(pia0b_r) { return coreGlobals.swMatrix[1 + locals.strobe1]; }
...
static void
showSegment(int num, UINT8 data) {
    int seg = 32 + num / 6;
    UINT8 bit = 1 << (num % 6);
            ...
            switch (~locals.via_a & 0x0f) {
                case 0:
                    if (core_gameData->hw.lampCol) {
                        ...
                        if (~data & 0x20)
                            coreGlobals.lampMatrix[8] = lampRow;
                        if (~data & 0x40)
                            coreGlobals.lampMatrix[9] = lampRow;
                    } else {
...
static const struct pia6821_interface pia[] = {{/*i: A/B,CA/B1,CA/B2 */ 0, pia0b_r, PIA_UNUSED_VAL(1),
                                                PIA_UNUSED_VAL(1), PIA_UNUSED_VAL(0), PIA_UNUSED_VAL(0),
                                                /*o: A/B,CA/B2       */ pia0a_w, 0, 0, 0,
                                                /*irq: A/B           */ 0, 0},
                                               {/*i: A/B,CA/B1,CA/B2 */ pia1a_r, pia1b_r, PIA_UNUSED_VAL(1),
                                                PIA_UNUSED_VAL(1), PIA_UNUSED_VAL(0), PIA_UNUSED_VAL(0),
...
static MEMORY_READ_START(barni_readmem1){0x0000, 0x03ff, gram_r}, {0x4000, 0x4000, firq_set}, {0x6000, 0x6000, cswd_r},
    {0x8000, 0x800f, via_0_r}, {0xa000, 0xa7ff, MRA_RAM}, {0xc000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(barni_writemem1){0x0000, 0x03ff, gram_w},
    {0x2000, 0x2000, pal_w}, {0x8000, 0x800f, via_0_w}, {0xa000, 0xa7ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static MEMORY_READ_START(barni_readmem2){0x0000, 0x03ff, gram_r},
    {0x2000, 0x2000, firq_clr}, {0x8000, 0x8003, pia_r(0)}, {0xa000, 0xa003, pia_r(1)}, {0xc000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(barni_writemem2){0x0000, 0x03ff, gram_w},
    {0x8000, 0x8003, pia_w(0)}, {0xa000, 0xa003, pia_w(1)},
    MEMORY_END
...
static WRITE_HANDLER(snd_pia0b_w) { logerror("snd PIA B: %02x\n", data); }
...
static READ_HANDLER(snd_pia1ca2_r) { return sndlocals.pia1ca2; }

static READ_HANDLER(snd_pia1cb1_r) { return sndlocals.pia1cb1; }
...
static const struct pia6821_interface snd_pia[] = {{/*i: A/B,CA/B1,CA/B2 */ PIA_UNUSED_VAL(0xff), PIA_UNUSED_VAL(0xff),
                                                    PIA_UNUSED_VAL(1), PIA_UNUSED_VAL(1), 0, PIA_UNUSED_VAL(0),
                                                    /*o: A/B,CA/B2       */ 0, snd_pia0b_w, 0, 0,
                                                    /*irq: A/B           */ snd_irq, snd_irq},
                                                   {/*i: A/B,CA/B1,CA/B2 */ snd_pia1a_r, 0, PIA_UNUSED_VAL(1),
                                                    snd_pia1cb1_r, snd_pia1ca2_r, PIA_UNUSED_VAL(0),
                                                    /*o: A/B,CA/B2       */ snd_pia1a_w, snd_pia1b_w, 0, 0,
                                                    /*irq: A/B           */ snd_irq, snd_irq}};

static void
snd_init(struct sndbrdData* brdData) {
...
const struct sndbrdIntf barniIntf = {"BARNI", snd_init, NULL, snd_diag, snd_data_w, snd_data_w, NULL, NULL, NULL, 0};

static READ_HANDLER(snd_cmd_r) { return sndlocals.lastcmd; }

static MEMORY_READ_START(snd_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x0083, pia_r(2)}, {0x0090, 0x0093, pia_r(3)},
    {0x2000, 0x2000, snd_cmd_r}, {0x1000, 0x1000, MRA_NOP}, {0xc000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(snd_writemem){0x0000, 0x007f, MWA_RAM},
    {0x0080, 0x0083, pia_w(2)}, {0x0090, 0x0093, pia_w(3)}, {0x1000, 0x1000, DAC_0_data_w},
    MEMORY_END

    static void snd_5220Irq(int state) {
    pia_set_input_cb1(3, (sndlocals.pia1cb1 = !state));
}

static void
snd_5220Rdy(int state) {
    pia_set_input_ca2(3, (sndlocals.pia1ca2 = state));
}
...
static MACHINE_STOP(barni) { sndbrd_0_exit(); }
...
static core_tLCDLayout dispAlpha[] = {{0, 0, 0, 6, CORE_SEG7},
                                      {0, 12, 32, 1, CORE_SEG7},
                                      {0, 26, 18, 6, CORE_SEG7},
                                      {0, 38, 35, 1, CORE_SEG7},
                                      {3, 0, 6, 6, CORE_SEG7},
                                      ...
                                      {3, 26, 24, 6, CORE_SEG7},
                                      {3, 38, 36, 1, CORE_SEG7},
                                      {2, 20, 12, 2, CORE_SEG7S},
                                      {2, 25, 14, 2, CORE_SEG7S},
                                      {2, 30, 16, 2, CORE_SEG7S},
#ifdef MAME_DEBUG
...
SIM_PORTS(1)
PORT_START /* 0 */
    COREPORT_BIT(0x0040, "Partida", KEYCODE_1) COREPORT_BIT(0x0010, "25 Ptas", KEYCODE_3)
        COREPORT_BIT(0x0020, "100 Ptas", KEYCODE_5) COREPORT_BIT(0x0080, "Tilt", KEYCODE_DEL)
            COREPORT_BIT(0x0002, "Test +", KEYCODE_7) COREPORT_BIT(0x0004, "Test -", KEYCODE_8)
                COREPORT_BIT(0x0001, "Sound test", KEYCODE_0) PORT_START /* 1 */
    COREPORT_DIPNAME(0x2000, 0x0000, "  800.000 puntos partida") COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(
        0x2000, DEF_STR(On)) COREPORT_DIPNAME(0x0200, 0x0200, "1.000.000 puntos partida") COREPORT_DIPSET(0x0000,
                                                                                                          DEF_STR(Off))
        COREPORT_DIPSET(0x0200, DEF_STR(On)) COREPORT_DIPNAME(0x0020, 0x0000, "1.200.000 puntos partida")
            COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x0020, DEF_STR(On)) COREPORT_DIPNAME(
                0x0002, 0x0000,
                "1.400.000 puntos partida") COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x0002, DEF_STR(On))
                COREPORT_DIPNAME(0x0001, 0x0000, "1.800.000 puntos partida") COREPORT_DIPSET(0x0000, DEF_STR(Off))
                    COREPORT_DIPSET(0x0001, DEF_STR(On)) COREPORT_DIPNAME(0x0010, 0x0010, "2.200.000 puntos partida")
                        COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x0010, DEF_STR(On)) COREPORT_DIPNAME(
                            0x0100, 0x0000, "2.600.000 puntos partida") COREPORT_DIPSET(0x0000, DEF_STR(Off))
                            COREPORT_DIPSET(0x0100, DEF_STR(On)) COREPORT_DIPNAME(0x1000, 0x1000,
                                                                                  "3.000.000 puntos partida")
                                COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x1000, DEF_STR(On))
                                    COREPORT_DIPNAME(0x8484, 0x0080, "Monedero") COREPORT_DIPSET(
                                        0x8000,
                                        "0,5/25 Ptas, 3/100 Ptas") COREPORT_DIPSET(0x0004, "  1/25 Ptas, 4/100 Ptas")
                                        COREPORT_DIPSET(0x0080, "  1/25 Ptas, 5/100 Ptas") COREPORT_DIPSET(
                                            0x0400,
                                            "  2/25 Ptas, 8/100 Ptas") COREPORT_DIPNAME(0x0008, 0x0008, "Musica Fondo")
                                            COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x0008, DEF_STR(On))
                                                COREPORT_DIPNAME(0x0040, 0x0000, "Loteria")
                                                    COREPORT_DIPSET(0x0000, "Mas") COREPORT_DIPSET(0x0040, "Menos")
                                                        COREPORT_DIPNAME(0x0800, 0x0000, "Bolas")
                                                            COREPORT_DIPSET(0x0000, "3") COREPORT_DIPSET(0x0800, "5")
                                                                COREPORT_DIPNAME(0x4000, 0x4000, "Voz Presentacion")
                                                                    COREPORT_DIPSET(0x0000, DEF_STR(Off))
                                                                        COREPORT_DIPSET(0x4000,
                                                                                        DEF_STR(On)) PORT_START /* 2 */
    COREPORT_DIPNAME(0x0001, 0x0000, "Display blanking") COREPORT_DIPSET(0x0000, DEF_STR(Off))
        COREPORT_DIPSET(0x0001, DEF_STR(On)) INPUT_PORTS_END
...
/ Champion
/-------------------------------*/
    ROM_START(champion) NORMALREGION(0x10000, REGION_CPU1)
            ...
            ROM_LOAD("chc.bin", 0xc000, 0x2000, CRC(6ab0f232) SHA1(0638d33f86c62ee93dff924a16a5b9309392d9e8))
                NORMALREGION(0x10000, REGION_CPU2)
                    ROM_LOAD("chan.bin", 0xe000, 0x2000, CRC(3f148587) SHA1(e44dc9cce15830f522dc781aaa13c659a43371f3))
                        NORMALREGION(0x10000, REGION_CPU3)
                            ROM_LOAD("voz1.bin", 0xf000, 0x1000,
                                     CRC(48665778) SHA1(c295dfe7f4a98756f508391eb326f37a5aac37ff))
                                ROM_LOAD("voz2.bin", 0xe000, 0x1000,
                                         CRC(30e7da5e) SHA1(3054cf9b09e0f89c242e1ad35bb31d9bd77248e4))
                                    ROM_LOAD("voz3.bin", 0xd000, 0x1000,
                                             CRC(3cd8058e) SHA1(fa4fd0cf4124263d4021c5a86033af9e5aa66eed))
                                        ROM_LOAD("voz4.bin", 0xc000, 0x1000,
                                                 CRC(0d00d8cc) SHA1(10f64d2fc3fc3e276bbd0e108815a3b395dcf0c9)) ROM_END
