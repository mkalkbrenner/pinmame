...
static INTERRUPT_GEN(EFO_vblank) { core_updateSw(core_getSol(1)); }
...
static READ_HANDLER(sw0_r) { return (~coreGlobals.swMatrix[0] & 0x7f) | (locals.zc ? 0x80 : 0); }

static READ_HANDLER(sw_r) { return ~coreGlobals.swMatrix[locals.col + 1]; }

static READ_HANDLER(u16_r) {
    return 0x9b; // we got lucky, this seems to work alright! :)
}

static WRITE_HANDLER(u16_w) { logerror("U16 WRITE @%04x: %02x\n", activecpu_get_previouspc(), data); }
...
static MEMORY_READ_START(cpu_readmem){0x0000, 0x7fff, MRA_ROM}, {0x8000, 0x87ff, MRA_RAM}, {0xa000, 0xa000, u16_r},
    MEMORY_END

    static MEMORY_WRITE_START(cpu_writemem){0x0000, 0x7fff, MWA_NOP},
    {0x8000, 0x87ff, MWA_RAM, &generic_nvram, &generic_nvram_size}, {0xa000, 0xa000, u16_w}, {0xc000, 0xc004, lamp_w},
    {0xc005, 0xc006, sol_w}, {0xe000, 0xe000, saa1099_write_port_0_w}, {0xe001, 0xe001, saa1099_control_port_0_w},
    MEMORY_END

    static PORT_READ_START(cpu_readport){0x00, 0x03, z80ctc_2_r},
    {0x04, 0x04, sw0_r}, {0x08, 0x08, sw_r},
    PORT_END

    static PORT_WRITE_START(cpu_writeport){0x00, 0x03, z80ctc_2_w},
    {0x0c, 0x0c, col_w}, {0x10, 0x10, disp_w}, {0x14, 0x14, row_w}, {0x18, 0x18, out_w}, {0x1c, 0x1c, latch_w},
    PORT_END

    static struct SAA1099_interface EFO_saa1099Int = {1, {{100, 100}}};
...
static int
efo_m2sw(int col, int row) {
    return col * 10 + row;
}

extern MACHINE_DRIVER_EXTERN(ZSU);
static MACHINE_DRIVER_START(EFO) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(EFO, EFO, EFO)
    MDRV_SWITCH_UPDATE(EFO) MDRV_SWITCH_CONV(efo_sw2m, efo_m2sw) MDRV_NVRAM_HANDLER(generic_0fill)

        MDRV_CPU_ADD_TAG("mcpu", Z80, 4000000) MDRV_CPU_CONFIG(EFO_DaisyChain)
            MDRV_CPU_MEMORY(cpu_readmem, cpu_writemem) MDRV_CPU_PORTS(cpu_readport, cpu_writeport)
                MDRV_CPU_VBLANK_INT(EFO_vblank, 1) MDRV_CPU_PERIODIC_INT(EFO_zc, 200)
                    MDRV_SOUND_ADD(SAA1099, EFO_saa1099Int)

                        MDRV_IMPORT_FROM(ZSU) MACHINE_DRIVER_END
...
#define EFO_COMPORTS(game, balls)                                                                                      \
    ...
    SIM_PORTS(balls)                                                                                                   \
    PORT_START /* 0 */                                                                                                 \
        COREPORT_BIT(0x0040, "Start", KEYCODE_1) COREPORT_BIT(0x0001, "Coin 1", KEYCODE_3)                             \
            COREPORT_BIT(0x0002, "Coin 2", KEYCODE_4) COREPORT_BIT(0x0004, "Coin 3", KEYCODE_5)                        \
                COREPORT_BIT(0x0008, "Tilt", KEYCODE_DEL) COREPORT_BITTOG(0x0010, "Coin Door", KEYCODE_END)            \
                    COREPORT_BIT(0x0020, "Test", KEYCODE_7) COREPORT_BIT(0x0100, "Port 1", KEYCODE_1_PAD)              \
                        COREPORT_BIT(0x0200, "Port 2", KEYCODE_2_PAD) COREPORT_BIT(0x0400, "Port 3", KEYCODE_3_PAD)    \
                            COREPORT_BIT(0x0800, "Port 4", KEYCODE_4_PAD)                                              \
                                COREPORT_BIT(0x1000, "Port 5", KEYCODE_5_PAD)                                          \
                                    COREPORT_BIT(0x2000, "Port 6", KEYCODE_6_PAD)                                      \
                                        COREPORT_BIT(0x4000, "Port 7 (Printer busy)", KEYCODE_7_PAD) INPUT_PORTS_END

    static core_tLCDLayout dispEB[] = {{0, 0, 24, 1, CORE_SEG7},
                                       {0, 2, 25, 1, CORE_SEG8D},
                                       {0, 4, 42, 1, CORE_SEG7},
                                       {0, 6, 27, 1, CORE_SEG7},
                                       {0, 8, 28, 1, CORE_SEG8D},
                                       ...
                                       {0, 14, 31, 1, CORE_SEG7},
                                       {0, 20, 16, 1, CORE_SEG7},
                                       {0, 22, 17, 1, CORE_SEG8D},
                                       {0, 24, 34, 1, CORE_SEG7},
                                       {0, 26, 19, 1, CORE_SEG7},
                                       {0, 28, 20, 1, CORE_SEG8D},
                                       {0, 30, 21, 1, CORE_SEG7},
                                       {0, 32, 38, 1, CORE_SEG7},
                                       {0, 34, 23, 1, CORE_SEG7},
                                       {6, 0, 8, 1, CORE_SEG7},
...
#ifdef MAME_DEBUG
                                       {3, 8, 50, 1, CORE_SEG7},
                                       {3, 10, 35, 3, CORE_SEG7}, // segments used in lamp test
#endif
                                       {0}};
...
static void
init_ebalchmb(void) {
    core_gameData = &ebalchmbGameData;
}
ROM_START(ebalchmb)
...
static core_tLCDLayout dispCB[] = {{0, 0, 36, 1, CORE_SEG8D},
                                   {0, 2, 29, 1, CORE_SEG7},
                                   {0, 4, 38, 1, CORE_SEG7},
                                   {0, 6, 24, 1, CORE_SEG8D},
                                   {0, 8, 25, 2, CORE_SEG7},
                                   {0, 12, 31, 1, CORE_SEG7},
                                   {0, 26, 28, 1, CORE_SEG8D},
                                   {0, 28, 21, 1, CORE_SEG7},
                                   {0, 30, 30, 1, CORE_SEG7},
                                   {0, 32, 16, 1, CORE_SEG8D},
                                   {0, 34, 17, 2, CORE_SEG7},
                                   {0, 38, 23, 1, CORE_SEG7},
                                   {4, 0, 20, 1, CORE_SEG8D},
                                   {4, 2, 13, 1, CORE_SEG7},
                                   ...
                                   {4, 38, 7, 1, CORE_SEG7},
                                   {2, 15, 46, 1, CORE_SEG7},
                                   {2, 17, 32, 1, CORE_SEG8D},
                                   {2, 21, 34, 1, CORE_SEG7},
                                   {2, 23, 39, 1, CORE_SEG7},
#ifdef MAME_DEBUG
                                   {2, 11, 44, 1, CORE_SEG8D},
                                   {2, 13, 37, 1, CORE_SEG7},
                                   {2, 19, 33, 1, CORE_SEG7}, // "invalid" segments, used in lamp test
...
static core_tLCDLayout dispCO[] = {{4, 0, 36, 1, CORE_SEG8D},
                                   {4, 2, 29, 1, CORE_SEG7},
                                   {4, 4, 38, 1, CORE_SEG7},
                                   {4, 6, 24, 1, CORE_SEG8D},
                                   {4, 8, 25, 2, CORE_SEG7},
                                   {4, 12, 31, 1, CORE_SEG7},
                                   {4, 16, 28, 1, CORE_SEG8D},
                                   {4, 18, 21, 1, CORE_SEG7},
                                   {4, 20, 30, 1, CORE_SEG7},
                                   {4, 22, 16, 1, CORE_SEG8D},
                                   {4, 24, 17, 2, CORE_SEG7},
                                   {4, 28, 23, 1, CORE_SEG7},
                                   {4, 32, 20, 1, CORE_SEG8D},
                                   {4, 34, 13, 1, CORE_SEG7},
                                   {4, 36, 22, 1, CORE_SEG7},
                                   {4, 38, 8, 1, CORE_SEG8D},
                                   {4, 40, 9, 2, CORE_SEG7},
                                   {4, 44, 15, 1, CORE_SEG7},
                                   {4, 48, 12, 1, CORE_SEG8D},
                                   {4, 50, 5, 1, CORE_SEG7},
                                   {4, 52, 14, 1, CORE_SEG7},
                                   {4, 54, 0, 1, CORE_SEG8D},
                                   {4, 56, 1, 2, CORE_SEG7},
...
static MACHINE_DRIVER_START(comeback) MDRV_IMPORT_FROM(EFO) MDRV_SCREEN_SIZE(640, 400)
...
static void
init_comeback(void) {
    core_gameData = &comebackGameData;
}
ROM_START(comeback)
NORMALREGION(0x10000, REGION_CPU1)
