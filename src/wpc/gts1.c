...
#endif
                    if (locals.strobe > 0)
                        lamp_w(locals.strobe - 1, locals.lampData);
                }
...
static READ_HANDLER(port_r) { return locals.accu; }

static WRITE_HANDLER(pgol_w) {
    data ^= 0x0f;
    if (offset)
        locals.pgolAddress = (locals.pgolAddress & 0x30f) | (data << 4);
    else
        locals.pgolAddress = (locals.pgolAddress & 0x3f0) | data;
}
...
/* port read / write */
static PORT_READ_START(GTS1_readport){0x000, 0x0ff, port_r}, {0x100, 0x100, pgol_r},
    PORT_END

    static PORT_WRITE_START(GTS1_writeport){0x000, 0x0ff, port_w},
    {0x100, 0x101, pgol_w},
    PORT_END
...
/------------------------------------------*/
    static MEMORY_READ_START(GTS1_readmem){0x0000, 0x0fff, MRA_ROM},
    {0x1000, 0x10ff, MRA_RAM}, {0x1800, 0x18ff, MRA_RAM}, {0x2000, 0x23ff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(GTS1_writemem){0x1000, 0x10ff, MWA_RAM},
...
static MACHINE_RESET(GTS1) { memset(&locals, 0, sizeof locals); }
...
static int type[1] = {0};
static DISCRETE_SOUND_START(b18555_discInt) DISCRETE_INPUT(NODE_01, 1, 0x0007, 0) DISCRETE_INPUT(NODE_02, 2, 0x0007, 0)
        ...
        DISCRETE_555_ASTABLE(NODE_10, NODE_01, 12.0, RES_K(2.7), RES_K(120), CAP_N(10), NODE_NC, type)
            DISCRETE_555_ASTABLE(NODE_20, NODE_02, 12.0, RES_K(2.7), RES_K(270), CAP_N(10), NODE_NC, type)
                DISCRETE_555_ASTABLE(NODE_30, NODE_03, 12.0, RES_K(2.7), RES_K(470), CAP_N(10), NODE_NC, type)
                    DISCRETE_GAIN(NODE_40, NODE_10, 1250) DISCRETE_GAIN(NODE_50, NODE_20, 1250)
                        DISCRETE_GAIN(NODE_60, NODE_30, 1250) DISCRETE_ADDER3(NODE_70, 1, NODE_40, NODE_50, NODE_60)
                            DISCRETE_OUTPUT(NODE_70, 50) DISCRETE_SOUND_END

    static MACHINE_DRIVER_START(GTS1NS) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", PPS4, 3579545. / 18.)
        MDRV_CPU_MEMORY(GTS1_readmem, GTS1_writemem) MDRV_CPU_PORTS(GTS1_readport, GTS1_writeport)
            MDRV_CPU_VBLANK_INT(GTS1_vblank, 1) MDRV_CORE_INIT_RESET_STOP(GTS1, GTS1, GTS1)
                MDRV_NVRAM_HANDLER(generic_0fill) MDRV_SWITCH_UPDATE(GTS1)
                    MDRV_SWITCH_CONV(GTS1_sw2m, GTS1_m2sw) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GTS1C) MDRV_IMPORT_FROM(GTS1NS) MDRV_DIPS(24)
        MDRV_SOUND_ADD(SAMPLES, samples_interface) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GTS1T) MDRV_IMPORT_FROM(GTS1NS) MDRV_CORE_INIT_RESET_STOP(GTS1T, GTS1T, GTS1) MDRV_DIPS(24)
        MDRV_SOUND_ADD(DISCRETE, b18555_discInt) MDRV_SOUND_ADD(SAMPLES, samples_interface) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GTS1S80) MDRV_IMPORT_FROM(GTS1NS) MDRV_DIPS(26) MDRV_IMPORT_FROM(gts80s_s) MACHINE_DRIVER_END
