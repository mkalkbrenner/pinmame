...
#define S4_IRQFREQ                                                                                                     \
    923 // NE556 Timer with 12 kOhm / 1.8 kOhm / 0.1 uF ~923 Hz // measured on a Sys3 and Sys4 as ~1110us -> ~901Hz
...
static WRITE_HANDLER(s4_alpha_w) {
    ...
    if (seg7)
        s4locals.segments[s4locals.alphapos].w = seg7;
    seg7 = core_bcd2seg[data & 15];
    if (seg7)
        s4locals.segments[20 + s4locals.alphapos].w = seg7;
}
...
/*****************/
static WRITE_HANDLER(s4_lampcol_w) {
    core_setLamp(coreGlobals.tmpLampMatrix, s4locals.lampColumn = data, s4locals.lampRow);
}

static WRITE_HANDLER(s4_lamprow_w) {
    core_setLamp(coreGlobals.tmpLampMatrix, s4locals.lampColumn, s4locals.lampRow = ~data);
}

#ifdef PROC_SUPPORT
// pass 0 to coil_index for C01-C08 and 8 for C09-C16
void
update_proc_coils(UINT8 cur_data, UINT8 last_data, int coil_index) {
    int i;
...
setSSSol(int data, int solNo) {
    ...
    if (s4locals.ssEn & (~data & 1)) {
        coreGlobals.pulsedSolState |= bit;
        s4locals.solenoids |= bit;
    } else
        coreGlobals.pulsedSolState &= ~bit;
}
...
static MACHINE_RESET(s4) { pia_reset(); }
...
/------------------------------------*/
static MEMORY_READ_START(s4_readmem){0x0000, 0x01ff, MRA_RAM},
    ...
    {0x6000, 0x8000, MRA_ROM}, {0x8000, 0xffff, MRA_ROM}, /*Doubled ROM region since only 15 address pins used!*/
    MEMORY_END

    static MEMORY_WRITE_START(s4_writemem){0x0000, 0x00ff, MWA_RAM},
    {0x0100, 0x01ff, s4_CMOS_w, &s4_CMOS}, {0x2200, 0x2203, pia_w(S4_PIA3)}, /*Solenoids + Sound Commands*/
    {0x2400, 0x2403, pia_w(S4_PIA2)},                                        /*Lamps*/
    {0x2800, 0x2803, pia_w(S4_PIA0)},                                        /*Display + Other*/
    {0x3000, 0x3003, pia_w(S4_PIA1)},                                        /*Switches*/
    {0x6000, 0x8000, MWA_ROM}, {0x8000, 0xffff, MWA_ROM}, /*Doubled ROM region since only 15 address pins used!*/
    MEMORY_END

    /*-- S4 without sound or S3 with chimes --*/
    MACHINE_DRIVER_START(s4) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(s4, s4, s4)
        MDRV_CPU_ADD(M6800, 3579545. / 4.) // MAME: 3580000
    MDRV_CPU_MEMORY(s4_readmem, s4_writemem) MDRV_CPU_VBLANK_INT(s4_vblank, 1) MDRV_CPU_PERIODIC_INT(s4_irq, S4_IRQFREQ)
        MDRV_NVRAM_HANDLER(s4) MDRV_DIPS(8 + 16) MDRV_SWITCH_UPDATE(s4) MDRV_DIAGNOSTIC_LEDV(2) MACHINE_DRIVER_END

    /*-- S3 with sound board --*/
    MACHINE_DRIVER_START(s3S) MDRV_IMPORT_FROM(s4) MDRV_IMPORT_FROM(wmssnd_s67s) MACHINE_DRIVER_END

    /*-- S4 with sound board --*/
    MACHINE_DRIVER_START(s4S) MDRV_IMPORT_FROM(s4) MDRV_IMPORT_FROM(wmssnd_s67s) MACHINE_DRIVER_END
