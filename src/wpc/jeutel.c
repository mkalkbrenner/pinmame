...
static INTERRUPT_GEN(jeutel_vblank) { core_updateSw(core_getSol(16)); }
...
static WRITE_HANDLER(m4000_w) { logerror("m4000_w: %02x\n", data); }

static MEMORY_READ_START(cpu_readmem1){0x0000, 0x0fff, MRA_ROM}, {0x2000, 0x2003, ppi8255_0_r},
    {0x3000, 0x3003, ppi8255_1_r}, {0x8000, 0x83ff, MRA_RAM}, {0xc000, 0xc3ff, shared_ram_r},
    MEMORY_END

    static MEMORY_WRITE_START(cpu_writemem1){0x2000, 0x2003, ppi8255_0_w},
    {0x3000, 0x3003, ppi8255_1_w}, {0x4000, 0x4000, m4000_w}, {0x8000, 0x83ff, MWA_RAM}, {0xc000, 0xc3ff, shared_ram_w},
    MEMORY_END

    static MEMORY_READ_START(cpu_readmem2){0x0000, 0x1fff, MRA_ROM},
    {0xc000, 0xc3ff, shared_ram_r}, {0xc400, 0xc7ff, MRA_RAM}, {0xe000, 0xe003, ppi8255_2_r},
    MEMORY_END

    static MEMORY_WRITE_START(cpu_writemem2){0xc000, 0xc3ff, shared_ram_w, &shared_RAM},
    {0xc400, 0xc7ff, MWA_RAM, &generic_nvram, &generic_nvram_size}, {0xe000, 0xe003, ppi8255_2_w},
    MEMORY_END
...
// lamp data
static WRITE_HANDLER(ppi0_portc_w) { locals.lampData = data; }

// switch strobe
static WRITE_HANDLER(ppi1_porta_w) { locals.swStrobe = ~data; }

// switch returns
static READ_HANDLER(ppi1_portb_r) { return ~coreGlobals.swMatrix[1 + core_BitColToNum(locals.swStrobe)]; }
...
// dip return
static READ_HANDLER(ppi2_portb_r) { return ~core_getDip(core_BitColToNum(locals.dipStrobe)); }

// sound command
static WRITE_HANDLER(ppi2_portc_w) { sndbrd_data_w(0, data); }
...
static MACHINE_INIT(JEUTEL) { sndbrd_0_init(core_gameData->hw.soundBoard, 2, memory_region(REGION_CPU3), NULL, NULL); }
...
static void
jeutel_init(struct sndbrdData* brdData) {
    memset(&sndlocals, 0x00, sizeof(sndlocals));
    tms5110_reset();
    AY8910_reset(0);
}

static WRITE_HANDLER(jeutel_data_w) { sndlocals.sndCmd = data; }
...
static WRITE_HANDLER(m8000_w) { sndlocals.tmsLatch = data; }

static MEMORY_READ_START(snd_readmem){0x0000, 0x0fff, MRA_ROM}, {0x4000, 0x43ff, MRA_RAM}, {0x6000, 0x7fff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(snd_writemem){0x4000, 0x43ff, MWA_RAM},
    {0x8000, 0x8000, m8000_w},
    MEMORY_END

    static PORT_READ_START(snd_readport){4, 4, AY8910_read_port_0_r},
    PORT_END

    static PORT_WRITE_START(snd_writeport){0, 0, AY8910_control_port_0_w},
 ...
 */
    ...
    logerror("8910 port A: %c%c%c%c%c%cxx\n", data & 0x80 ? '0' : ' ', data & 0x40 ? '1' : ' ', data & 0x20 ? 'K' : ' ',
             data & 0x10 ? ' ' : 'L', data & 0x08 ? ' ' : 'H', data & 0x04 ? 'J' : ' ');
...
static struct AY8910interface jeutel_8910Int = {
    1, 2000000, {20}, {NULL}, {ay8910_portb_r}, {ay8910_porta_w},
};

static void
tms5110_irq(int data) {
    cpu_set_irq_line(2, 0, tms5110_status_r(0) ? ASSERT_LINE : CLEAR_LINE);
}

static int
tms5110_callback(void) {
    int value;
...
const struct sndbrdIntf jeutelIntf = {"JEUTEL",        jeutel_init,   NULL, NULL,
                                      jeutel_manCmd_w, jeutel_data_w, NULL, jeutel_ctrl_w};
...
#define JEUTEL_COMPORTS(game, balls)                                                                                                              \
    ...
    SIM_PORTS(balls)                                                                                                                              \
    PORT_START /* 0 */                                                                                                                            \
        COREPORT_BITDEF(0x0002, IPT_START1, IP_KEY_DEFAULT) COREPORT_BITDEF(0x0004, IPT_COIN1, IP_KEY_DEFAULT) COREPORT_BITDEF(                   \
            0x0008,                                                                                                                               \
            IPT_COIN2, KEYCODE_3) COREPORT_BITDEF(0x0010, IPT_COIN3,                                                                              \
                                                  KEYCODE_4) COREPORT_BITDEF(0x0001,                                                              \
                                                                             IPT_TILT, KEYCODE_INSERT) COREPORT_BIT(0x0020,                       \
                                                                                                                    "Test",                       \
                                                                                                                    KEYCODE_7) PORT_START /* 1 */ \
            COREPORT_DIPNAME(0x0040, 0x0040, "S1/1") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(                                                \
                0x0040,                                                                                                                           \
                "1") COREPORT_DIPNAME(0x4000, 0x4000,                                                                                             \
                                      "S1/2") COREPORT_DIPSET(0x0000,                                                                             \
                                                              "0") COREPORT_DIPSET(0x4000,                                                        \
                                                                                   "1") COREPORT_DIPNAME(0x0080,                                  \
                                                                                                         0x0080,                                  \
                                                                                                         "S1/5")                                  \
                COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0080, "1") COREPORT_DIPNAME(0x8000, 0x0000, "S1/6") COREPORT_DIPSET(               \
                    0x0000,                                                                                                                       \
                    "0") COREPORT_DIPSET(0x8000,                                                                                                  \
                                         "1") COREPORT_DIPNAME(0x0010, 0x0000,                                                                    \
                                                               "S2/1") COREPORT_DIPSET(0x0000,                                                    \
                                                                                       "0") COREPORT_DIPSET(0x0010, "1")                          \
                    COREPORT_DIPNAME(0x1000, 0x0000, "S2/2") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(                                        \
                        0x1000,                                                                                                                   \
                        "1") COREPORT_DIPNAME(0x0020, 0x0000,                                                                                     \
                                              "S2/5") COREPORT_DIPSET(0x0000,                                                                     \
                                                                      "0") COREPORT_DIPSET(0x0020, "1")                                           \
                        COREPORT_DIPNAME(0x2000, 0x0000, "S2/6") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(                                    \
                            0x2000,                                                                                                               \
                            "1") COREPORT_DIPNAME(0x0004, 0x0000,                                                                                 \
                                                  "S3/1") COREPORT_DIPSET(0x0000,                                                                 \
                                                                          "0") COREPORT_DIPSET(0x0004, "1")                                       \
                            COREPORT_DIPNAME(0x0400, 0x0000, "S3/2") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(                                \
                                0x0400,                                                                                                           \
                                "1") COREPORT_DIPNAME(0x0008, 0x0000,                                                                             \
                                                      "S3/5") COREPORT_DIPSET(0x0000,                                                             \
                                                                              "0") COREPORT_DIPSET(0x0008, "1")                                   \
                                COREPORT_DIPNAME(0x0800, 0x0800, "S3/6") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(                            \
                                    0x0800,                                                                                                       \
                                    "1") COREPORT_DIPNAME(0x0001, 0x0001,                                                                         \
                                                          "S4/1") COREPORT_DIPSET(0x0000,                                                         \
                                                                                  "0") COREPORT_DIPSET(0x0001, "1")                               \
                                    COREPORT_DIPNAME(0x0100, 0x0100, "S4/2") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(                        \
                                        0x0100,                                                                                                   \
                                        "1") COREPORT_DIPNAME(0x0002, 0x0000,                                                                     \
                                                              "S4/5") COREPORT_DIPSET(0x0000,                                                     \
                                                                                      "0") COREPORT_DIPSET(0x0002,                                \
                                                                                                           "1")                                   \
                                        COREPORT_DIPNAME(0x0200, 0x0000, "S4/6") COREPORT_DIPSET(                                                 \
                                            0x0000,                                                                                               \
                                            "0") COREPORT_DIPSET(0x0200,                                                                          \
                                                                 "1") PORT_START /* 2 */                                                          \
                                            COREPORT_DIPNAME(0x0040, 0x0000, "S1/3") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(                \
                                                0x0040,                                                                                           \
                                                "1") COREPORT_DIPNAME(0x4000, 0x4000,                                                             \
                                                                      "S1/4") COREPORT_DIPSET(0x0000,                                             \
                                                                                              "0") COREPORT_DIPSET(0x4000,                        \
                                                                                                                   "1")                           \
                                                COREPORT_DIPNAME(0x0080, 0x0000, "S1/7") COREPORT_DIPSET(                                         \
                                                    0x0000,                                                                                       \
                                                    "0") COREPORT_DIPSET(0x0080,                                                                  \
                                                                         "1") COREPORT_DIPNAME(0x8000, 0x8000, "S1/8")                            \
                                                        ...
                                                        "1") COREPORT_DIPNAME(0x0010, 0x0000,                                                     \
                                                                              "S2/3") COREPORT_DIPSET(0x0000,                                     \
                                                                                                      "0") COREPORT_DIPSET(0x0010,                \
                                                                                                                           "1")                   \
                                                        COREPORT_DIPNAME(0x1000, 0x0000, "S2/4") COREPORT_DIPSET(                                 \
                                                            0x0000,                                                                               \
                                                            "0") COREPORT_DIPSET(0x1000,                                                          \
                                                                                 "1") COREPORT_DIPNAME(0x0020, 0x0000,                            \
                                                                                                       "S2/7")                                    \
                                                            COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(                                         \
                                                                0x0020,                                                                           \
                                                                "1") COREPORT_DIPNAME(0x2000, 0x0000,                                             \
                                                                                      "S2/8") COREPORT_DIPSET(0x0000,                             \
                                                                                                              "0")                                \
                                                                COREPORT_DIPSET(0x2000, "1") COREPORT_DIPNAME(                                    \
                                                                    0x0004,                                                                       \
                                                                    0x0000,                                                                       \
                                                                    "S3/3") COREPORT_DIPSET(0x0000, "0")                                          \
                                                                    COREPORT_DIPSET(0x0004, "1") COREPORT_DIPNAME(                                \
                                                                        0x0400,                                                                   \
                                                                        0x0000,                                                                   \
                                                                        "S3/4") COREPORT_DIPSET(0x0000, "0")                                      \
                                                                        COREPORT_DIPSET(0x0400, "1") COREPORT_DIPNAME(                            \
                                                                            0x0008,                                                               \
                                                                            0x0000,                                                               \
                                                                            "S3/7") COREPORT_DIPSET(0x0000, "0")                                  \
                                                                            COREPORT_DIPSET(0x0008, "1") COREPORT_DIPNAME(                        \
                                                                                0x0800,                                                           \
                                                                                0x0000,                                                           \
                                                                                "S3/8") COREPORT_DIPSET(0x0000, "0")                              \
                                                                                COREPORT_DIPSET(0x0800, "1") COREPORT_DIPNAME(                    \
                                                                                    0x0001,                                                       \
                                                                                    0x0000,                                                       \
                                                                                    "S4/3") COREPORT_DIPSET(0x0000, "0")                          \
                                                                                    COREPORT_DIPSET(0x0001, "1") COREPORT_DIPNAME(                \
                                                                                        0x0100,                                                   \
                                                                                        0x0000,                                                   \
                                                                                        "S4/4") COREPORT_DIPSET(0x0000, "0")                      \
                                                                                        COREPORT_DIPSET(                                          \
                                                                                            0x0100,                                               \
                                                                                            "1") COREPORT_DIPNAME(0x0002,                         \
                                                                                                                  0x0000,                         \
                                                                                                                  "S4"                            \
                                                                                                                  "/"                             \
                                                                                                                  "7")                            \
...
static core_tLCDLayout dispAlpha[] = {{0, 0, 10, 1, CORE_SEG10},
                                      {0, 2, 11, 2, CORE_SEG9},
                                      {0, 6, 13, 1, CORE_SEG10},
                                      {0, 8, 14, 2, CORE_SEG9},
                                      {0, 12, 1, 1, CORE_SEG9},
                                      ...
                                      {5, 8, 30, 2, CORE_SEG9},
                                      {5, 12, 17, 1, CORE_SEG9},
                                      {5, 16, 18, 1, CORE_SEG10},
                                      {5, 18, 19, 2, CORE_SEG9},
                                      {5, 22, 21, 1, CORE_SEG10},
                                      {5, 24, 22, 2, CORE_SEG9},
                                      {5, 28, 25, 1, CORE_SEG9},
                                      {3, 27, 37, 2, CORE_SEG7S},
                                      {3, 33, 39, 1, CORE_SEG7S},
                                      {3, 35, 41, 1, CORE_SEG7S},
#ifdef MAME_DEBUG
