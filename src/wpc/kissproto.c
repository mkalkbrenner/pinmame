...
static WRITE_HANDLER(by8035_ram_w) {
    UINT8* ram = memory_region(REGION_CPU1) + 0x5000 + 0x100 * locals.ramBank;
    ram[offset] = data;
    if (locals.ramBank == 0x0b)
        locals.segments[offset].w = (~data & 0x7f) | ((~data & 0x80) << 1) | ((~data & 0x80) << 2);
    if (locals.ramBank > 7)
        logerror("%04x: ram %03x write: %02x\n", activecpu_get_previouspc(), 0x100 * locals.ramBank + offset, data);
...
static MEMORY_READ_START(by8035_readmem){0x0000, 0x0fff, MRA_BANKNO(1)}, {0x1000, 0x4fff, MRA_ROM},
    {0x5000, 0x5fff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(by8035_writemem){0x0000, 0x0fff, MWA_BANKNO(1)},
    {0x1000, 0x4fff, MWA_ROM}, {0x5000, 0x5fff, MWA_RAM},
    MEMORY_END

    static PORT_READ_START(by8035_readport){0x00, 0xff, by8035_ram_r},
    {0x100, 0x107, by8035_port_r},
    MEMORY_END

    static PORT_WRITE_START(by8035_writeport){0x00, 0xff, by8035_ram_w},
    {0x100, 0x107, by8035_port_w},
    MEMORY_END

    static READ_HANDLER(u8_porta_r) {
    logerror("%04x: 8255 A read\n", activecpu_get_previouspc());
    return 0;
}

static READ_HANDLER(u8_portb_r) {
    logerror("%04x: 8255 B read\n", activecpu_get_previouspc());
    return 0;
}

static READ_HANDLER(u8_portc_r) {
    logerror("%04x: 8255 C read\n", activecpu_get_previouspc());
    return 0;
}
...
static const core_tLCDLayout dispBy6p[] = {
    {0, 0, 0, 16, CORE_SEG10}, {2, 0, 16, 16, CORE_SEG10}, {4, 0, 32, 16, CORE_SEG10}, {6, 0, 48, 16, CORE_SEG10}, {0}};
...
BY8035_ROMSTART(kissp, "kiss8755.bin", CRC(894c1052) SHA1(579ce3c8ec374f2cd17928ab92311f035ecee341), "kissprot.u5",
                CRC(38a2ef5a) SHA1(4ffdb2e9aa30417d506af3bc4b6835ba1dc80e4f), "kissprot.u6",
                CRC(bcdfaf1d) SHA1(d21bebbf702b400eb71f8c88be50a180a5ac260a), "kissprot.u7",
                CRC(d97da1d3) SHA1(da771a08969a12105c7adc9f9e3cbd1677971e79))
ROM_END
CORE_CLONEDEFNV(kissp, kiss, "Kiss (Prototype)", 1979, "Bally", by8035, GAME_NOT_WORKING)

INITGAMEP0(kissp2, FLIP_SW(FLIP_L), 0)
BY8035_ROMSTART(kissp2, "8755u8.dat", CRC(d2d04100) SHA1(fe81f3667cb5802c9780761a359660bad83862c2), "kissprot.u5",
                CRC(38a2ef5a) SHA1(4ffdb2e9aa30417d506af3bc4b6835ba1dc80e4f), "kissprot.u6",
                CRC(bcdfaf1d) SHA1(d21bebbf702b400eb71f8c88be50a180a5ac260a), "u7.dat",
                CRC(e224a9b0) SHA1(2a0e3afad8c566432ebe690ff1ce6fa92b68816f))
ROM_END
