...
static WRITE_HANDLER(pia1a_w) { locals.a1 = data; }
...
static WRITE_HANDLER(pia3b_w) { logerror("PIA3 B W %02x\n", data); }

static WRITE_HANDLER(pia3ca2_w) { logerror("PIA3 CA2 W %x\n", data); }
...
O: A0-A3  display address
   A4-A7  BCD data
*/
static struct pia6821_interface pia[] = {{/* I:  A/B,CA1/B1,CA2/B2 */ pia1a_r, pia1b_r, 0, 0, 0, 0,
...
static INTERRUPT_GEN(boomerang_vblank) { core_updateSw(core_getSol(2)); }

static INTERRUPT_GEN(boomerang_irq) { cpu_set_irq_line(0, M6800_IRQ_LINE, PULSE_LINE); }

static void
boomerang_zeroCross(int data) {
    locals.zc = !locals.zc;
    //  pia_set_input_cb2(PIA2, locals.zc);
}

static void
boomerang_dspIrq(int data) {
...
/------------------------------------*/
static MEMORY_READ_START(boomerang_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x0080, MRA_RAM}, /* fake NVRAM */
    {0x0084, 0x0087, pia_r(PIA1)}, {0x0088, 0x008b, pia_r(PIA2)}, {0x0090, 0x0093, pia_r(PIA3)},
    {0x0800, 0x0fff, MRA_ROM}, {0xfff0, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(boomerang_writemem){0x0000, 0x007f, MWA_RAM},
    {0x0080, 0x0080, MWA_RAM, &generic_nvram, &generic_nvram_size}, /* fake NVRAM */
    {0x0084, 0x0087, pia_w(PIA1)}, {0x0088, 0x008b, pia_w(PIA2)}, {0x0090, 0x0093, pia_w(PIA3)},
    {0x0800, 0xffff, MWA_NOP},
    MEMORY_END

    MACHINE_DRIVER_START(boomerang) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(boomerang, boomerang, NULL)
        MDRV_CPU_ADD_TAG("mcpu", M6800, 500000) // taken from patent
    MDRV_CPU_MEMORY(boomerang_readmem, boomerang_writemem) MDRV_CPU_VBLANK_INT(boomerang_vblank, 1)
        MDRV_CPU_PERIODIC_INT(boomerang_irq, 500)                                  // guessed
    MDRV_TIMER_ADD(boomerang_zeroCross, 120) MDRV_TIMER_ADD(boomerang_dspIrq, 720) // 360Hz x 2, taken from patent
    MDRV_SWITCH_UPDATE(boomerang) MDRV_NVRAM_HANDLER(generic_0fill) MDRV_DIPS(24) MACHINE_DRIVER_END

#define BOOMERANG_COMPORTS                                                                                             \
    PORT_START /* 0 */                                                                                                 \
        COREPORT_BIT(0x2000, "Start", KEYCODE_1) COREPORT_BIT(0x8000, "Coin #1", KEYCODE_3) COREPORT_BIT(              \
            0x4000, "Coin #2", KEYCODE_5) COREPORT_BIT(0x0080, "Tilt",                                                 \
                                                       KEYCODE_DEL) COREPORT_BIT(0x0001, "Slam Tilt",                  \
                                                                                 KEYCODE_HOME) PORT_START /* 1 */      \
            COREPORT_DIPNAME(0xc000, 0x0000, "Balls / game") COREPORT_DIPSET(0x0000, "3") COREPORT_DIPSET(0x4000, "4") \
                COREPORT_DIPSET(0x8000, "5") COREPORT_DIPSET(0xc000, "6") COREPORT_DIPNAME(                            \
                    0x00f0, 0x0060, "Score to beat x10,000") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0010, "1") \
                    COREPORT_DIPSET(0x0020, "2") COREPORT_DIPSET(0x0030, "3") COREPORT_DIPSET(                         \
                        0x0040, "4") COREPORT_DIPSET(0x0050, "5") COREPORT_DIPSET(0x0060, "6")                         \
                        COREPORT_DIPSET(0x0070, "7") COREPORT_DIPSET(0x0080, "8") COREPORT_DIPSET(                     \
                            0x0090, "9") COREPORT_DIPNAME(0x000f, 0x0000, "Score to beat  x1,000")                     \
                            COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0001, "1") COREPORT_DIPSET(                 \
                                0x0002, "2") COREPORT_DIPSET(0x0003, "3") COREPORT_DIPSET(0x0004, "4")                 \
                                COREPORT_DIPSET(0x0005, "5") COREPORT_DIPSET(0x0006, "6") COREPORT_DIPSET(             \
                                    0x0007, "7") COREPORT_DIPSET(0x0008, "8") COREPORT_DIPSET(0x0009, "9")             \
                                    COREPORT_DIPNAME(0x3800, 0x0800, "Coins for slot #1") COREPORT_DIPSET(             \
                                        0x0000, "0") COREPORT_DIPSET(0x0800, "1") COREPORT_DIPSET(0x1000, "2")         \
                                        COREPORT_DIPSET(0x1800, "3") COREPORT_DIPSET(0x2000, "4") COREPORT_DIPSET(     \
                                            0x2800, "5") COREPORT_DIPSET(0x3000,                                       \
                                                                         "6") COREPORT_DIPSET(0x3800, "7")             \
                                            COREPORT_DIPNAME(0x0700, 0x0100, "Credits for slot #1") COREPORT_DIPSET(   \
                                                0x0000,                                                                \
                                                "0") COREPORT_DIPSET(0x0100,                                           \
                                                                     "1") COREPORT_DIPSET(0x0200,                      \
                                                                                          "2") COREPORT_DIPSET(0x0300, \
                                                                                                               "3")    \
                                                COREPORT_DIPSET(0x0400, "4") COREPORT_DIPSET(                          \
                                                    0x0500,                                                            \
                                                    "5") COREPORT_DIPSET(0x0600,                                       \
                                                                         "6") COREPORT_DIPSET(0x0700,                  \
                                                                                              "7") PORT_START /* 2 */  \
                                                    COREPORT_DIPNAME(0x0038, 0x0008,                                   \
                                                                     "Coins for slot #2") COREPORT_DIPSET(0x0000, "0") \
                                                        COREPORT_DIPSET(0x0008, "1") COREPORT_DIPSET(0x0010, "2")      \
                                                            COREPORT_DIPSET(0x0018, "3") COREPORT_DIPSET(              \
                                                                0x0020,                                                \
                                                                "4") COREPORT_DIPSET(0x0028,                           \
                                                                                     "5") COREPORT_DIPSET(0x0030, "6") \
                                                                COREPORT_DIPSET(0x0038, "7") COREPORT_DIPNAME(         \
                                                                    0x0007,                                            \
                                                                    0x0003,                                            \
                                                                    "Credits for slot #2") COREPORT_DIPSET(0x0000,     \
                                                                                                           "0")        \
                                                                    COREPORT_DIPSET(0x0001, "1") COREPORT_DIPSET(      \
                                                                        0x0002,                                        \
                                                                        "2") COREPORT_DIPSET(0x0003, "3")              \
                                                                        COREPORT_DIPSET(0x0004, "4") COREPORT_DIPSET(  \
                                                                            0x0005,                                    \
                                                                            "5") COREPORT_DIPSET(0x0006, "6")          \
                                                                            COREPORT_DIPSET(                           \
                                                                                0x0007,                                \
                                                                                "7") COREPORT_DIPNAME(0x0040, 0x0000,  \
                                                                                                      DEF_STR(Unused)) \
    ...
    static const core_tLCDLayout dispBoom[] = {{0, 0, 6, 6, CORE_SEG7},
                                               {0, 16, 12, 6, CORE_SEG7},
                                               {3, 0, 18, 6, CORE_SEG7},
                                               {3, 16, 24, 6, CORE_SEG7},
                                               {6, 8, 30, 2, CORE_SEG7},
...
BOOMERANG_COMPORTS
INPUT_PORTS_END
CORE_GAMEDEFNV(boomrang, "Boomerang (Engineering Prototype, patched patent code)", 1975, "Bally", boomerang,
               GAME_USES_CHIMES)
...
CORE_CLONEDEFNV(boomranb, boomrang, "Boomerang (MOD Bugfixes patch)", 2017, "Bally / Gaston", boomerang,
                GAME_USES_CHIMES)
