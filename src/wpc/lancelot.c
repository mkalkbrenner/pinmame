...
static INTERRUPT_GEN(LANCELOT_irq) { cpu_set_irq_line(0, 0, PULSE_LINE); }
...
static WRITE_HANDLER(snd_w) {
    ...
    if (data > 0x80 && data < 0x90) { // music, probably won't play at all without the microcontroller's ROM
        locals.musicStart =
            memory_region(REGION_USER2)[data - 0x80] + ((UINT16)memory_region(REGION_USER2)[1 + data - 0xf80] << 8);
        locals.musicStop =
            memory_region(REGION_USER2)[4 + data - 0x80] + ((UINT16)memory_region(REGION_USER2)[5 + data - 0x80] << 8);
        timer_adjust(locals.sndtimer2, 0, 0, 0.001);
        return;
    }
    if (data >= 0xf0) { // music, probably won't play at all without the microcontroller's ROM
        locals.musicStart =
            memory_region(REGION_USER2)[data - 0xf0] + ((UINT16)memory_region(REGION_USER2)[1 + data - 0xf0] << 8);
        locals.musicStop =
            memory_region(REGION_USER2)[4 + data - 0xf0] + ((UINT16)memory_region(REGION_USER2)[5 + data - 0xf0] << 8);
        timer_adjust(locals.sndtimer2, 0, 0, 0.001);
        return;
    }
...
static READ_HANDLER(sw5_r) { return ~coreGlobals.swMatrix[5]; }

static READ_HANDLER(dip_r) { return ~core_getDip(offset); }

static WRITE_HANDLER(lamp_w) { coreGlobals.tmpLampMatrix[offset] = data; }

static WRITE_HANDLER(lamp2_w) { coreGlobals.tmpLampMatrix[8 + offset] = data; }
...
static WRITE_HANDLER(sol2_w) { coreGlobals.solenoids = (coreGlobals.solenoids & 0x000fffff) | (data << 20); }

static WRITE_HANDLER(disp_w) { locals.dispData[offset] = data; }
...
static MEMORY_READ_START(LANCELOT_readmem){0x0000, 0x7fff, MRA_ROM}, {0x8000, 0x87ff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(LANCELOT_writemem){0x8000, 0x87ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static PORT_READ_START(LANCELOT_readport){0x08, 0x0b, sw_r},
    {0x21, 0x22, dip_r}, {0x29, 0x29, sw5_r},
    PORT_END

    static PORT_WRITE_START(LANCELOT_writeport){0x00, 0x05, disp_w},
    {0x06, 0x06, lamp3_w}, {0x10, 0x17, lamp_w}, {0x18, 0x18, sol_w}, {0x2a, 0x2a, sol2_w}, {0x30, 0x33, lamp2_w},
    {0x34, 0x34, snd_w}, {0x40, 0x40, col_w}, {0x50, 0x50, p50_w},
    PORT_END
...
static core_tLCDLayout disp[] = {{0, 0, 0, 8, CORE_SEG8},      {3, 0, 8, 8, CORE_SEG8},
                                 {6, 0, 16, 8, CORE_SEG8},     {9, 0, 24, 8, CORE_SEG8},
                                 {0, 18, 39, 1, CORE_SEG8},    {0, 28, 37, 2, CORE_SEG7SC},
                                 {10, 23, 35, 1, CORE_SEG7SC}, {10, 26, 36, 1, CORE_SEG7SC},
                                 {3, 24, 45, 3, CORE_SEG7SC},  {3, 30, 48, 1, CORE_SEG7SC},
                                 {3, 32, 48, 1, CORE_SEG7SC},  {3, 34, 48, 1, CORE_SEG7SC},
                                 {3, 36, 48, 1, CORE_SEG7SC},  {3, 38, 48, 1, CORE_SEG7SC},
                                 {5, 24, 42, 3, CORE_SEG7SC},  {5, 30, 49, 1, CORE_SEG7SC},
                                 {5, 32, 49, 1, CORE_SEG7SC},  {5, 34, 49, 1, CORE_SEG7SC},
                                 {5, 36, 49, 1, CORE_SEG7SC},  {5, 38, 49, 1, CORE_SEG7SC},
                                 {7, 24, 34, 1, CORE_SEG7SC},  {7, 26, 40, 2, CORE_SEG7SC},
                                 {7, 30, 50, 1, CORE_SEG7SC},  {7, 32, 50, 1, CORE_SEG7SC},
                                 {7, 34, 50, 1, CORE_SEG7SC},  {7, 36, 50, 1, CORE_SEG7SC},
                                 {7, 38, 50, 1, CORE_SEG7SC},  {0}};
INPUT_PORTS_START(lancelot)
CORE_PORTS
SIM_PORTS(3)
PORT_START /* 0 */
    COREPORT_BITDEF(0x0010, IPT_START1, IP_KEY_DEFAULT) COREPORT_BITDEF(0x0020, IPT_COIN1, IP_KEY_DEFAULT)
        COREPORT_BITDEF(0x0040, IPT_COIN2, IP_KEY_DEFAULT)
            COREPORT_BITTOG(0x0080, "Run / Test", KEYCODE_0) PORT_START /* 1 */
    COREPORT_DIPNAME(0x8000, 0x0000, "SW1-1") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(
        0x8000, "1") COREPORT_DIPNAME(0x4000, 0x4000, "SW1-2") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x4000, "1")
        COREPORT_DIPNAME(0x2000, 0x0000, "SW1-3") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x2000, "1")
            COREPORT_DIPNAME(0x1000, 0x0000, "SW1-4") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x1000, "1")
                COREPORT_DIPNAME(0x0800, 0x0000, "SW1-5") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0800, "1")
                    COREPORT_DIPNAME(0x0400, 0x0400, "SW1-6") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0400, "1")
                        COREPORT_DIPNAME(0x0200, 0x0200, "SW1-7") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(
                            0x0200, "1") COREPORT_DIPNAME(0x0100, 0x0000, "SW1-8") COREPORT_DIPSET(0x0000, "0")
                            COREPORT_DIPSET(0x0100, "1") COREPORT_DIPNAME(0x0080, 0x0080,
                                                                          "SW2-1") COREPORT_DIPSET(0x0000, "0")
                                COREPORT_DIPSET(0x0080, "1") COREPORT_DIPNAME(0x0040, 0x0040,
                                                                              "SW2-2") COREPORT_DIPSET(0x0000, "0")
                                    COREPORT_DIPSET(0x0040, "1") COREPORT_DIPNAME(0x0020, 0x0020, "SW2-3")
                                        COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0020, "1")
                                            COREPORT_DIPNAME(0x0010, 0x0010, "SW2-4") COREPORT_DIPSET(0x0000, "0")
                                                COREPORT_DIPSET(0x0010, "1") COREPORT_DIPNAME(0x0008, 0x0008, "SW2-5")
                                                    COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0008, "1")
