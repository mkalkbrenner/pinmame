...
#define ALVGS1_SNDCPU_FREQ                                                                                             \
    2000000 //Schem shows an 8Mhz clock, but often we need to divide by 4 to make it work in MAME.
...
/* Sound board */
const struct sndbrdIntf alvgs1Intf = {"OKI",         alvgs1_init, NULL,          NULL,          alvg_sndCmd_w,
                                      alvgs1_data_w, NULL,        alvgs1_ctrl_w, alvgs1_ctrl_r, SNDBRD_NODATASYNC};
...
//PB0-7: (IN) - N.C.
static READ_HANDLER(xvia_2_b_r) {
    LOG(("WARNING: SOUND VIA -Port B Read\n"));
    return 0;
}

//CA1: (IN) - Sound Control
static READ_HANDLER(xvia_2_ca1_r) {
    LOG(("WARNING: SOUND VIA CA1 Read\n"));
    return 0;
}

//CB1: (IN) - N.C.
static READ_HANDLER(xvia_2_cb1_r) {
    LOG(("WARNING: SOUND VIA CB1 Read\n"));
    return 0;
}

//CA2:  (IN) - N.C.
static READ_HANDLER(xvia_2_ca2_r) {
    LOG(("WARNING: SOUND VIA CA2 Read\n"));
    return 0;
}

//CB2: (IN) - N.C.
static READ_HANDLER(xvia_2_cb2_r) {
    LOG(("WARNING: SOUND VIA CB2 Read\n"));
    return 0;
}
...
static MEMORY_READ_START(alvgs1_readmem){0x2000, 0x2000, YM3812_status_port_0_r}, {0x3000, 0x3fff, MRA_RAM},
    {0x4000, 0x4000, OKIM6295_status_0_r}, {0x5000, 0x500f, via_2_r}, {0x8000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(alvgs1_writemem){0x2000, 0x2000, YM3812_control_port_0_w},
    {0x2001, 0x2001, YM3812_write_port_0_w}, {0x3000, 0x3fff, MWA_RAM}, {0x4000, 0x4000, OKIM6295_data_0_w},
    {0x5000, 0x500f, via_2_w}, {0x6000, 0x6000, MWA_NOP}, //Watch dog?
    {0x8000, 0xffff, MWA_ROM},
    MEMORY_END

    MACHINE_DRIVER_START(alvg_s1) MDRV_CPU_ADD(M6809, ALVGS1_SNDCPU_FREQ) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(alvgs1_readmem, alvgs1_writemem) MDRV_INTERLEAVE(50) MDRV_SOUND_ADD(YM3812, alvgs1_ym3812_intf)
...
static void
alvgs1_init(struct sndbrdData* brdData) {
...
#define ALVGS2_SNDCPU_FREQ                                                                                             \
    2000000 //Schem shows an 8Mhz clock, but often we need to divide by 4 to make it work in MAME.
#define ALVGS_SNDFIRQ_FREQ                                                                                             \
    (ALVGS2_SNDCPU_FREQ                                                                                                \
     / 4096.) //Mystery Castle manual shows Jumper J103 set - which divides E signal from 6809 by 4096.
...
static struct BSMT2000interface alvgs_bsmt2000Int = {
    1, {24000000}, {12}, {ALVGS_ROMREGION}, {100},
    0, 0,          1 // should be BSMT2000 mode 5, as it does not trigger ADPCM commands //!! does not trigger reset, thus cannot set BSMT2000 mode itself currently
};

/* Sound board */
const struct sndbrdIntf alvgs2Intf = {"BSMT",       alvgs_init, NULL,         NULL,         alvg_sndCmd_w,
                                      alvgs_data_w, NULL,       alvgs_ctrl_w, alvgs_ctrl_r, SNDBRD_NODATASYNC};
...
static MEMORY_READ_START(alvgs_readmem){0x0100, 0x0100, bsmtready_r}, {0x0800, 0x0800, soundlatch_r},
    {0x2000, 0x3fff, MRA_RAM}, {0x4000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(alvgs_writemem){0x0100, 0x0100, watch_w},
    {0x0800, 0x0800, data_to_main_cpu}, {0x1000, 0x10ff, bsmt_write}, {0x2000, 0x3fff, MWA_RAM},
    {0x4000, 0xffff, MWA_ROM},
    MEMORY_END

    MACHINE_DRIVER_START(alvg_s2) MDRV_CPU_ADD(M6809, ALVGS2_SNDCPU_FREQ) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(alvgs_readmem, alvgs_writemem) MDRV_CPU_PERIODIC_INT(alvgs_firq, ALVGS_SNDFIRQ_FREQ)
            MDRV_INTERLEAVE(50) MDRV_SOUND_ADD_TAG("bsmt", BSMT2000, alvgs_bsmt2000Int)
                MDRV_SOUND_ATTRIBUTES(SOUND_SUPPORTS_STEREO) MACHINE_DRIVER_END
...
static INTERRUPT_GEN(alvgs_firq) { cpu_set_irq_line(ALVGS_CPUNO, M6809_FIRQ_LINE, PULSE_LINE); }
