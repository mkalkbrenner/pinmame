...
snd_cmd_init(void) {
    ...
    switch (locals.boards) {
        case 1:
            readCmds(-1, sndbrd_typestr(0));
            break;
        case 2:
            readCmds(-1, sndbrd_typestr(1));
            break;
        case 3:
            readCmds(0, sndbrd_typestr(0));
            readCmds(1, sndbrd_typestr(1));
            break;
    }
    for (ii = 0; ii < MAX_CMD_LENGTH * 2; ii++)
        locals.digits[ii] = 0x10;
    wave_init();
}
...
#ifdef VPINMAME_PINSOUND
static void
getPinSoundDirectory(char path_pinsound_cwd[2048]) {
    char* pos;
    char tmp_path[2048];
...
static BOOL
WriteSlot(const HANDLE hFile, const LPTSTR const lpszMessage) {
    DWORD cbWritten;
    const BOOL fResult = WriteFile(hFile, lpszMessage, (DWORD)((lstrlen(lpszMessage) + 1) * sizeof(TCHAR)), &cbWritten,
                                   (LPOVERLAPPED)NULL);
...
static BOOL
sendToSlot(const HANDLE hFile, const TCHAR msg_to_pinsound_studio[100]) {
...
static HANDLE
makeWriteSlot(const LPTSTR slotName) {
    const HANDLE hFile = CreateFile(slotName, GENERIC_WRITE, FILE_SHARE_READ, (LPSECURITY_ATTRIBUTES)NULL,
...
static HANDLE
makeReadSlot(const LPTSTR slotName) {
...
static BOOL
readSlot(const HANDLE hFile, char* msg) {
    DWORD msgSize;
    DWORD numRead;
    LPTSTR buffer;
...
reinit_pinSound(void) {
    ...
    if (!(pmoptions.sound_mode == 2 || pmoptions.sound_mode == 3)
        || options.samplerate
               == 0) // if not internal or external pinsound enabled, or if sound in general is disabled -> return
        return;
    ...
    sprintf(game_rom_system, "#ROM%s#SYSTEM%s", Machine->gamedrv->name,
...
static void
pinsound_handle(const int boardNo, const int cmd) {
        ...
        if (pmoptions.sound_mode == 3) {
            ...
            if (cd || le == ERROR_ALREADY_EXISTS) {
                ...
                if (fp_check) {
                    ...
                    do {
                        fclose(fp_check);
                        sprintf(machine_name, "%s/%.8s-%04d.psrec", path_pinsound_cwd, Machine->gamedrv->name,
                                ++cpt_filename);
                        fp_check = fopen(machine_name, "r");
                    } while (fp_check);
                }
...
/-----------------*/
void
snd_cmd_log(int boardNo, int cmd) {
#ifdef VPINMAME_ALTSOUND
    if (options.samplerate != 0 && pmoptions.sound_mode == 1)
    ...
    if (locals.soundMode || (locals.boards == 0))
        return; // Don't log from within sound commander
    if (locals.boards == 3) {
...
int
snd_get_cmd_log(int* last, int* buffer) {
    int count = 0;
...
/*-----------------------------------*/
static void
insert_char(char data) {
    locals.digits[locals.currDigit] = data;
    if (locals.rollover || (locals.currDigit < (MAX_CMD_LENGTH * 2 - 1))) {
        if (++locals.currDigit > (MAX_CMD_LENGTH * 2 - 1))
            locals.currDigit = 0;
    }
}

int
manual_sound_commands(struct mame_bitmap* bitmap) {
    int ii;
    ...
    /* Toggle command mode */
    if (keyboard_pressed_memory_repeat(SMDCMD_MODETOGGLE, REPEATKEY) && !playCmd(-1, NULL)) {
    ...
    if (locals.soundMode) {
        ...
        if (locals.digitMode) {
            ...
            for (ii = 0; ii < MAX_CMD_LENGTH; ii++)
                core_textOutf(SND_XROW + 13 * ii, 105, BLACK, (ii < locals.currCmd->length) ? "%02x" : "  ",
                              locals.currCmd->cmd[ii]);
        }
        core_textOutf(SND_XROW, 130, BLACK, "Last commands");
        for (ii = 0; ii < MAX_CMD_LOG; ii++)
            core_textOutf(SND_XROW + 13 * ii, 140, BLACK, "%02x", locals.cmdLog[(locals.firstLog + ii) % MAX_CMD_LOG]);
        playCmd(-1, NULL);
        return FALSE;
    }
...
static int
checkName(const char* buf, const char* name) {
    while (*name)
        if ((*buf == '\0') || (*(buf++) != *(name++)))
            return FALSE;
    return TRUE;
...
static void
readCmds(int boardNo, const char* head) {
                    ...
                    for (;;) {
                        ...
                        if (tmpCmd.length++ & 0x01) {
                            if ((getData == 2) && (boardNo >= 0)) {
                                tmpCmd.cmd[tmpCmd.length / 2 - 1] = boardNo;
                                tmpCmd.length += 2;
                            }
                ...
                if (head && checkName(buffer, head))
                    ...
                    getData = (boardNo != 0)
                              && (checkName(buffer, Machine->gamedrv->name)
                                  || (Machine->gamedrv->clone_of && !(Machine->gamedrv->clone_of->flags & NOT_A_DRIVER)
    ...
    if (locals.cmdIdx > 0) {
        ...
        if ((locals.cmdIdx % 4) == 2) { /* only send cmd every 4th frame */
            if (locals.nextCmd[locals.cmdIdx / 4] == -1)
                ...
                sndbrd_manCmd(locals.nextCmd[locals.cmdIdx / 4], locals.nextCmd[locals.cmdIdx / 4 + 1]);
                locals.cmdIdx += 4;
            } else
...
wave_next(void) {
    ...
    if (wavelocals.dumping == 1) {
        ...
        sprintf(dumpName, "0x%01X%01X%01X%01X-%s", locals.digits[MAX_CMD_LENGTH * 2 - 4],
                locals.digits[MAX_CMD_LENGTH * 2 - 3], locals.digits[MAX_CMD_LENGTH * 2 - 2],
                locals.digits[MAX_CMD_LENGTH * 2 - 1], Machine->gamedrv->name);

        if (locals.digits[MAX_CMD_LENGTH * 2 - 2] == 0x00 && locals.digits[MAX_CMD_LENGTH * 2 - 1] == 0x00)
            wavelocals.dumping = 0;
        else if (mame_faccess(dumpName, FILETYPE_WAVE)) {
            fclose(csv);
            playNextCmd();
        } else {
            fprintf(csv, "0x%01X%01X%01X%01X,", locals.digits[MAX_CMD_LENGTH * 2 - 4],
                    locals.digits[MAX_CMD_LENGTH * 2 - 3], locals.digits[MAX_CMD_LENGTH * 2 - 2],
                    locals.digits[MAX_CMD_LENGTH * 2 - 1]);
...
static int
is_silent(const INT16* const buf, const int size) {
    int i;
    ...
    for (i = 0; i < size; i++) {
        if (abs((int)buf[i] - old_value)
            > 32767
                  / 1000) //!! magic value, do not just check for 0 due to dithering and other side effects of some sound cores
        {
...
void
pm_wave_record(INT16* buffer, int samples) {
    int written;
    if (wavelocals.dumping == 1) {
...
void
pm_wave_disp(struct mame_bitmap* abitmap) {
    char buf[25];
    if (wavelocals.recording == 0) {
        if (wavelocals.spinner == 0)
            return;
        strcpy(buf, "                  ");
