...
static WRITE_HANDLER(pia2b_w) { core_setLamp(coreGlobals.tmpLampMatrix, s7locals.lampColumn = data, s7locals.lampRow); }
...
static WRITE_HANDLER(pia3ca2_w) { DBGLOG(("pia3ca2_w\n")); }
...
static WRITE_HANDLER(pia5a_w) {
    ...
    if (data & 0x40)
        s7locals.alphaSegs |= 0x100;
    if (data & 0x80)
        s7locals.alphaSegs |= 0x200;
}

static WRITE_HANDLER(pia5b_w) {
    if (data & 0x01)
        s7locals.alphaSegs |= 0x400;
    if (data & 0x02)
        s7locals.alphaSegs |= 0x40;
    if (data & 0x04)
        s7locals.alphaSegs |= 0x800;
    if (data & 0x08)
        s7locals.alphaSegs |= 0x4000;
    if (data & 0x10)
        s7locals.alphaSegs |= 0x2000;
    if (data & 0x20)
        s7locals.alphaSegs |= 0x1000;
    if (data & 0x40)
        s7locals.alphaSegs |= 0x80;
    if (data & 0x80)
        s7locals.alphaSegs |= 0x8000;
    s7locals.segments[40 + s7locals.digSel].w |= s7locals.alphaSegs;
}

static WRITE_HANDLER(pia5ca2_w) { DBGLOG(("pia5ca2_w\n")); }

static WRITE_HANDLER(pia5cb2_w) { DBGLOG(("pia5cb2_w\n")); }
...
int
dfndrCustSol(int solno) {
    return s7locals.custSol & (1 << (solno - CORE_FIRSTCUSTSOL));
}

static void
setSSSol(int data, int solNo) {
...
updsol(void) {
    ...
    coreGlobals.pulsedSolState =
        (coreGlobals.pulsedSolState & 0x01ff0000) | (s7locals.solBits2 << 8) | (s7locals.solBits1);
...
static WRITE_HANDLER(pia1a_w) {
    ...
    if (s7locals.lampColumn & 0x01)
        core_setLamp(coreGlobals.tmpLampMatrix, 0x100, ~data & 0x0f);
    if (s7locals.lampColumn & 0x02)
        core_setLamp(coreGlobals.tmpLampMatrix, 0x100, ~data << 4);
    if (s7locals.lampColumn & 0x04)
        core_setLamp(coreGlobals.tmpLampMatrix, 0x200, ~data & 0x0f);
    if (s7locals.lampColumn & 0x08)
        core_setLamp(coreGlobals.tmpLampMatrix, 0x200, ~data << 4);
    if (s7locals.lampColumn & 0x10)
        core_setLamp(coreGlobals.tmpLampMatrix, 0x400, ~data & 0x0f);
    if (s7locals.lampColumn & 0x20)
        core_setLamp(coreGlobals.tmpLampMatrix, 0x400, ~data << 4);
    if (s7locals.lampColumn & 0x40)
        core_setLamp(coreGlobals.tmpLampMatrix, 0x800, ~data & 0x0f);
    if (s7locals.lampColumn & 0x80)
        core_setLamp(coreGlobals.tmpLampMatrix, 0x800, ~data << 4);
}
...
static struct pia6821_interface s7_pia[] = {{/* PIA 0 (2100)
    PA0-4 Sound
    PB5   CA1
    PB6   Comma 3+4
    PB7   Comma 1+2
    ...
    CB2   SS7
    CA1,CB1 NC */
                                             /* in  : A/B,CA/B1,CA/B2 */ 0, PIA_UNUSED_VAL(0x3f), PIA_UNUSED_VAL(1),
                                             PIA_UNUSED_VAL(0), 0, 0,
...
static MACHINE_RESET(s7) { pia_reset(); }

static MACHINE_STOP(s7) { sndbrd_0_exit(); }
...
/*---------------------------------
  ...
  IC20 ROM (6800-6fff)
-----------------------------------*/
static MEMORY_READ_START(s7_readmem){0x0000, 0x00ff, MRA_BANKNO(S7_BANK0)}, {0x0100, 0x01ff, MRA_RAM},
    {0x1000, 0x13ff, MRA_RAM}, /* CMOS */
    {0x2100, 0x2103, pia_r(S7_PIA0)}, {0x2200, 0x2203, pia_r(S7_PIA1)}, {0x2400, 0x2403, pia_r(S7_PIA2)},
    {0x2800, 0x2803, pia_r(S7_PIA3)}, {0x3000, 0x3003, pia_r(S7_PIA4)}, {0x4000, 0x4003, pia_r(S7_PIA5)},
    {0x5000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(s7_writemem){0x0000, 0x00ff, MWA_BANKNO(S7_BANK0)},
    {0x0100, 0x01ff, s7_CMOS_w, &s7_CMOS}, {0x1000, 0x13ff, MWA_RAM, &s7_rambankptr}, /* CMOS */
    {0x2100, 0x2103, pia_w(S7_PIA0)}, {0x2200, 0x2203, pia_w(S7_PIA1)}, {0x2400, 0x2403, pia_w(S7_PIA2)},
    {0x2800, 0x2803, pia_w(S7_PIA3)}, {0x3000, 0x3003, pia_w(S7_PIA4)}, {0x4000, 0x4003, pia_w(S7_PIA5)},
...
/  Machine drivers
/------------------*/
    MACHINE_DRIVER_START(s7) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(s7, s7, s7)
        MDRV_CPU_ADD_TAG("mcpu", M6808, 3579545. / 4.) // MAME: 3580000
    MDRV_CPU_MEMORY(s7_readmem, s7_writemem) MDRV_CPU_VBLANK_INT(s7_vblank, 1) MDRV_CPU_PERIODIC_INT(s7_irq, S7_IRQFREQ)
        MDRV_NVRAM_HANDLER(s7) MDRV_DIPS(8 + 16) /* 0:sound & speech, 1:function dips, 2:data dips*/
    MDRV_SWITCH_UPDATE(s7) MDRV_DIAGNOSTIC_LED7 MACHINE_DRIVER_END

    MACHINE_DRIVER_START(s7S) MDRV_IMPORT_FROM(s7) MDRV_IMPORT_FROM(wmssnd_s67s) MDRV_SOUND_CMD(sndbrd_0_data_w)
        MDRV_SOUND_CMDHEADING("s7") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(s7S6) MDRV_IMPORT_FROM(s7) MDRV_CORE_INIT_RESET_STOP(s7S6, s7, s7)
        MDRV_IMPORT_FROM(wmssnd_s67s) MDRV_SOUND_CMD(sndbrd_0_data_w) MDRV_SOUND_CMDHEADING("s7") MACHINE_DRIVER_END
...
static MEMORY_READ_START(rr_readmem){0x0000, 0x07ff, MRA_RAM}, {0x2100, 0x2103, pia_r(S7_PIA0)},
    {0x2400, 0x2403, pia_r(S7_PIA2)}, {0x2800, 0x2803, pia_r(S7_PIA3)}, {0x3000, 0x3003, pia_r(S7_PIA4)},
    {0x5000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(rr_writemem){0x0000, 0x06ff, MWA_RAM},
    {0x0700, 0x07ff, MWA_RAM, &s7_CMOS}, {0x2100, 0x2103, pia_w(S7_PIA0)}, {0x2200, 0x2200, rr_sol_w},
    {0x2400, 0x2403, pia_w(S7_PIA2)}, {0x2800, 0x2803, pia_w(S7_PIA3)}, {0x3000, 0x3003, pia_w(S7_PIA4)},
    {0x5000, 0xffff, MWA_ROM},
    MEMORY_END
...
/-------------------------------------------------*/
static NVRAM_HANDLER(s7) { core_nvram(file, read_or_write, s7_CMOS, 0x0100, 0xff); }
