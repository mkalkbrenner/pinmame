...
void
sndbrd_init(int brdNo, int brdType, int cpuNo, UINT8* romRegion, WRITE_HANDLER((*data_cb)), WRITE_HANDLER((*ctrl_cb))) {
...
#else  // HAS_SAMPLES
    if ((brdType != SNDBRD_NONE) && ((b->flags & SNDBRD_NOTSOUND) || Machine->drv->sound[0].sound_type))
#endif // HAS_SAMPLES
    {
        brdData.boardNo = brdNo;
        brdData.subType = brdType & 0xff;
        brdData.cpuNo = cpuNo;
        brdData.romRegion = romRegion;
        i->brdIntf = b;
        i->type = brdType;
        i->data_cb = data_cb;
        i->ctrl_cb = ctrl_cb;
...
int
sndbrd_exists(int board) {
    return (intf[board].brdIntf && ((intf[board].brdIntf->flags & SNDBRD_NOTSOUND) == 0));
}
...
void
sndbrd_diag(int board, int button) {
    const struct sndbrdIntf* b = intf[board].brdIntf;
...
void
sndbrd_data_w(int board, int data) {
    const struct sndbrdIntf* b = intf[board].brdIntf;
...
void
sndbrd_ctrl_w(int board, int data) {
    const struct sndbrdIntf* b = intf[board].brdIntf;
...
void
sndbrd_ctrl_cb(int board, int data) {
...
void
sndbrd_data_cb(int board, int data) {
...
void
sndbrd_manCmd(int board, int cmd) {
    const struct sndbrdIntf* b = intf[board].brdIntf;
    if (b && (coreGlobals.soundEn || (b->flags & SNDBRD_NOTSOUND)) && b->manCmd_w) {
        if ((b->flags & SNDBRD_DOUBLECMD) && (intf[board].manCmdBuf < 0)) {
            intf[board].manCmdBuf = cmd;
            return;
        }
        b->manCmd_w(intf[board].manCmdBuf, cmd);
        intf[board].manCmdBuf = -1;
    }
}

void
sndbrd_setManCmd(int board, WRITE_HANDLER((*manCmd))) {
    struct sndbrdIntf* b = (struct sndbrdIntf*)intf[board].brdIntf;
    b->manCmd_w = manCmd;
}

void
sndbrd_0_init(int brdType, int cpuNo, UINT8* romRegion, WRITE_HANDLER((*data_cb)), WRITE_HANDLER((*ctrl_cb))) {
    sndbrd_init(0, brdType, cpuNo, romRegion, data_cb, ctrl_cb);
}

void
sndbrd_1_init(int brdType, int cpuNo, UINT8* romRegion, WRITE_HANDLER((*data_cb)), WRITE_HANDLER((*ctrl_cb))) {
...
void
sndbrd_0_diag(int button) {
    sndbrd_diag(0, button);
}

void
sndbrd_1_diag(int button) {
    sndbrd_diag(1, button);
}
...
int
sndbrd_type(int offset) {
    return intf[offset].type;
}
