...
static WRITE_HANDLER(GTS80_sndCmd_w) { sndbrd_0_data_w(0, data); }
...
static READ_HANDLER(riot6532_0a_r) {
    ...
    if (GTS80locals.riot1b & 0x80) {
        ...
        for (ii = 8; ii > 0; ii--) {
            bits <<= 1;
            if (coreGlobals.swMatrix[ii] & GTS80locals.riot0b)
                bits |= 1;
        }
#else
...
static READ_HANDLER(slam_sw_r) { return GTS80locals.slamSw; }
...
static WRITE_HANDLER(riot6532_1aBCD_w) {
    ...
    if (data & ~GTS80locals.riot1a & 0x40)
        GTS80locals.seg3 =
            dispdata < 12 ? core_bcd2seg9a[GTS80locals.riot1b & 0x0f] : core_bcd2seg7a[GTS80locals.riot1b & 0x0f];
    ...
    if ((GTS80locals.riot1b & 0x10) == 0)
        GTS80locals.seg1 |= core_bcd2seg9a[1];
    if ((GTS80locals.riot1b & 0x20) == 0)
        GTS80locals.seg2 |= core_bcd2seg9a[1];
    if ((GTS80locals.riot1b & 0x40) == 0)
        GTS80locals.seg3 |= dispdata < 12 ? core_bcd2seg9a[1] : core_bcd2seg7a[1];
...
static WRITE_HANDLER(riot6532_1b_w) {
    ...
    if (data & ~GTS80locals.riot1b & 0x10) {        // LD1 (falling edge)
        ...
        if (alpha == 0x01) {
            ...
            GTS80locals.segments[GTS80locals.segPos1].w |= GTS80locals.pseg[GTS80locals.segPos1].w =
                core_ascii2seg16[alpha];
        if (GTS80locals.segPos1 < 19)
            GTS80locals.segPos1 = (GTS80locals.segPos1 + 1) % 0x14;
        ...
        if (alpha == 0x01) {
            ...
            GTS80locals.segments[20 + GTS80locals.segPos2].w |= GTS80locals.pseg[20 + GTS80locals.segPos2].w =
                core_ascii2seg16[alpha];
...
static WRITE_HANDLER(riot6532_2a_w) {
    ...
    if (data & 0x20) /* solenoids 1-4 */
        GTS80locals.solenoids |= coreGlobals.pulsedSolState =
            (coreGlobals.pulsedSolState & 0xfffffff0) | (1 << (data & 0x03));
    if (data & 0x40) /* solenoid 5-8 */
        GTS80locals.solenoids |= coreGlobals.pulsedSolState =
            (coreGlobals.pulsedSolState & 0xffffff0f) | (0x10 << ((data >> 2) & 0x03));
    /* solenoid 9 */
    GTS80locals.solenoids |= coreGlobals.pulsedSolState =
        (coreGlobals.pulsedSolState & 0xfffffeff) | ((data & 0x80) << 1);

    if (core_gameData->hw.soundBoard == SNDBRD_GTS80B) {
        if (data & 0x10)
            GTS80_sndCmd_w(0, (coreGlobals.lampMatrix[0] & 0x10) | (data & 0x0f));
    } else {
...
static WRITE_HANDLER(riot6532_2b_w) {
    ...
    if (column >= 0) {
        ...
        if (core_gameData->hw.display & GTS80_DISPVIDEO) {
            if (column == 1)
                GTS80locals.vidPlayer = data;
            else if (column == 3)
                GTS80locals.vidCmd = data;
            else if ((column == 4) && (data & 0x01)) {
                soundlatch_w(0, GTS80locals.vidCmd);
...
static WRITE_HANDLER(ram_256w) {
    ...
    pMem[0x0000] = pMem[0x0100] = pMem[0x0200] = pMem[0x0300] = pMem[0x0400] = pMem[0x0500] = pMem[0x0600] =
        pMem[0x0700] = pMem[0x4000] = pMem[0x4100] = pMem[0x4200] = pMem[0x4300] = pMem[0x4400] = pMem[0x4500] =
            pMem[0x4600] = pMem[0x4700] = pMem[0x8000] = pMem[0x8100] = pMem[0x8200] = pMem[0x8300] = pMem[0x8400] =
                pMem[0x8500] = pMem[0x8600] = pMem[0x8700] = pMem[0xc000] = pMem[0xc100] = pMem[0xc200] = pMem[0xc300] =
...
/----------------------------*/
static MEMORY_READ_START(GTS80_readmem){0x0000, 0x007f, MRA_RAM},                           /*U4 - 6532 RAM*/
    {0x0080, 0x00ff, MRA_RAM},                                                              /*U5 - 6532 RAM*/
    {0x0100, 0x017f, MRA_RAM},                                                              /*U6 - 6532 RAM*/
    {0x01cb, 0x01cb, in_1cb_r}, {0x01e4, 0x01e4, in_1e4_r}, {0x0200, 0x027f, riot6532_0_r}, /*U4 - I/O*/
    ...
    static MEMORY_WRITE_START(GTS80_writemem){0x0000, 0x007f, riot6532_0_ram_w, &GTS80_riot0RAM}, /*U4 - 6532 RAM*/
    ...
    if (core_gameData->hw.display & GTS80_DISPVIDEO)
        pic8259_0_config(GTS80_VIDCPU, 0);
...
static MACHINE_DRIVER_START(gts80b_ns) MDRV_IMPORT_FROM(gts80) MDRV_SCREEN_SIZE(320, 200)
    MDRV_VISIBLE_AREA(0, 319, 0, 199) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts80b) MDRV_IMPORT_FROM(gts80b_ns) MDRV_IMPORT_FROM(gts80s_sp) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts80bs1) MDRV_IMPORT_FROM(gts80b_ns) MDRV_IMPORT_FROM(gts80s_b1) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts80bs2) MDRV_IMPORT_FROM(gts80b_ns) MDRV_IMPORT_FROM(gts80s_b2) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts80bs3) MDRV_IMPORT_FROM(gts80b_ns) MDRV_IMPORT_FROM(gts80s_b3) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(gts80bs3a) MDRV_IMPORT_FROM(gts80b_ns) MDRV_IMPORT_FROM(gts80s_b3a) MACHINE_DRIVER_END
...
static void
GTS80_vidStatus(int strobe, int data) {
    int ii;
    data <<= (strobe - 1);
...
static WRITE_HANDLER(port200w) {
    if (data)
        GTS80_vidStatus(1, data);
}

/* output to game switches, row 1 */
static WRITE_HANDLER(port300w) {
    if (data)
        GTS80_vidStatus(2, data);
}

// Check if the main CPU has read the data in the 8212s
static READ_HANDLER(port200r) {
    const int stat = ((GTS80locals.buf8212int & 0x01) << 7) | ((GTS80locals.buf8212int & 0x02) << 5);
    // reset switch lines
    if (GTS80locals.buf8212int & 0x01)
        GTS80_vidStatus(1, 0);
    if (GTS80locals.buf8212int & 0x02)
        GTS80_vidStatus(2, 0);
...
/* joystick + player */
static READ_HANDLER(port400r) { return (coreGlobals.swMatrix[0] & 0x0f) | (GTS80locals.vidPlayer << 4); }
...
static READ_HANDLER(in_1cb_r) {
    DBGLOG(("read Caveman input 1CB\n"));
    return 0;
}

static READ_HANDLER(in_1e4_r) {
    DBGLOG(("read Caveman input 1E4\n"));
    return 0;
}

static PORT_READ_START(video_readport){0x002, 0x002, port002r}, {0x102, 0x102, port102r}, {0x200, 0x200, port200r},
    {0x300, 0x300, soundlatch_r}, {0x400, 0x400, port400r},
    PORT_END

    static PORT_WRITE_START(video_writeport){0x000, 0x002, port00xw},
    {0x100, 0x102, port10xw}, {0x200, 0x200, port200w}, {0x300, 0x300, port300w}, {0x500, 0x506, port50xw},
    PORT_END

    static MEMORY_READ_START(video_readmem){0x00000, 0x0058e, MRA_RAM},
    {0x007cf, 0x007fe, MRA_RAM}, {0x02000, 0x05fff, MRA_RAM}, {0x08000, 0x0ffff, MRA_ROM}, {0xf8000, 0xfffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(video_writemem){0x00000, 0x0058e, MWA_RAM},
    {0x007cf, 0x007fe, MWA_RAM}, {0x02000, 0x05fff, vram_w, &GTS80_vRAM}, {0x08000, 0x0ffff, MWA_ROM},
    {0xf8000, 0xfffff, MWA_ROM},
    MEMORY_END

    /* 4 x 7 BCD + Ball,Credit */
    core_tLCDLayout GTS80_dispCaveman[] = {{0, 0, 2, 7, CORE_SEG98F},
                                           {0, 16, 9, 7, CORE_SEG98F},
                                           {4, 0, 22, 7, CORE_SEG98F},
                                           {4, 16, 29, 7, CORE_SEG98F},
                                           DISP_SEG_CREDIT(40, 41, CORE_SEG9),
                                           DISP_SEG_BALLS(42, 43, CORE_SEG9),
                                           {70, 0, 240, 256, CORE_VIDEO, (genf*)caveman_update, NULL},
                                           {0}};
