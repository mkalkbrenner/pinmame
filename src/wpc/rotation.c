...
static WRITE_HANDLER(MIDWAY_CMOS_w) { MIDWAY_CMOS[offset] = data; }

static NVRAM_HANDLER(MIDWAY) { core_nvram(file, read_or_write, MIDWAY_CMOS, 256, 0x00); }

static MACHINE_INIT(MIDWAY) { memset(&locals, 0, sizeof locals); }

static INTERRUPT_GEN(MIDWAY_nmihi) { cpu_set_nmi_line(0, PULSE_LINE); }
...
static READ_HANDLER(mem1800_r) { return 0x00; /* 0xc3 */ }

static READ_HANDLER(matrix_r) { return locals.tmpSwCol < 7 ? coreGlobals.swMatrix[locals.tmpSwCol + 1] : 0; }
...
static READ_HANDLER(keypad_r) {
    ...
    switch (locals.tmpSwCol) {
        // 3, 2, 1, 0
        case 0:
            return ((coreGlobals.swMatrix[8] & 0x07) << 3) | ((coreGlobals.swMatrix[8] & 0x80) >> 1);
        // 7, 6, 5, 4
        case 1:
            return coreGlobals.swMatrix[8] & 0x78;
            // set, dot, 9, 8
        case 2:
            return ((coreGlobals.swMatrix[0] & 0x07) << 3) | ((coreGlobals.swMatrix[0] & 0x80) >> 1);
        // test 3, test 2, test 1, game
        case 3:
            return coreGlobals.swMatrix[0] & 0x78;
            // test 7, test 6, test 5, test 4
        case 4:
            return ((coreGlobals.swMatrix[9] & 0x07) << 3) | ((coreGlobals.swMatrix[9] & 0x80) >> 1);
        // end, test 10, test 9, test 8
        case 5:
            return coreGlobals.swMatrix[9] & 0x78;
        default:
            return 0;
    }
}
...
PORT_READ_START(midway_readport){0x21, 0x21, matrix_r}, {0x22, 0x22, keypad_r},
    PORT_END

    PORT_WRITE_START(midway_writeport){0x00, 0x02, disp_w},
    {0x03, 0x03, lamp_w}, {0x04, 0x07, sol_w}, {0x10, 0x17, astrocade_sound1_w}, {0x20, 0x25, port_2x_w},
    PORT_END
...
e000-e01f  NVRAM
*/
    static MEMORY_READ_START(MIDWAY_readmem){0x0000, 0x17ff, MRA_ROM}, /* ROM */
    {0x1800, 0x1800, mem1800_r},                                       /* Possible code extension. More roms to come? */
    {0xc000, 0xc0ff, MRA_RAM},                                         /* RAM */
    {0xe000, 0xe01f, MRA_RAM},                                         /* NVRAM */
    MEMORY_END

    static MEMORY_WRITE_START(MIDWAY_writemem){0xc000, 0xc0ff, MWA_RAM}, /* RAM */
    {0xe000, 0xe01f, MIDWAY_CMOS_w, &MIDWAY_CMOS},                       /* NVRAM */
    MEMORY_END

    static struct astrocade_interface sndIntf = {1, 14138000. / 8., {75}};
...
CORE_PORTS
SIM_PORTS(1)
PORT_START /* 0 */ /* switch column 2 */
    COREPORT_BIT(0x0100, "Player #1 Start", KEYCODE_1) COREPORT_BIT(0x0400, "Player #2 Start", KEYCODE_2)
        COREPORT_BIT(0x0800, "Player #3 Start", KEYCODE_3)
            COREPORT_BIT(0x0200, "Player #4 Start", KEYCODE_4) /* switch column 3 */
    COREPORT_BIT(0x8000, "Ball Tilt", KEYCODE_INSERT) COREPORT_BIT(0x1000, "Slam Tilt", KEYCODE_HOME)
        COREPORT_BITDEF(0x4000, IPT_COIN1, IP_KEY_DEFAULT) /* switch column 4 */
    COREPORT_BITDEF(0x2000, IPT_COIN2, KEYCODE_6)          /* switch column 0 */
    COREPORT_BIT(0x0080, "Keypad 8", KEYCODE_8_PAD) COREPORT_BIT(0x0004, "Keypad 9", KEYCODE_9_PAD)
        COREPORT_BIT(0x0001, "Keypad Set", KEYCODE_ENTER) COREPORT_BIT(0x0002, "Keypad Dot", KEYCODE_STOP)
            COREPORT_BIT(0x0040, "Keypad Game", KEYCODE_0) COREPORT_BIT(0x0020, "Test 1 (Lamps)", KEYCODE_9)
                COREPORT_BIT(0x0010, "Test 2 (Switches)", KEYCODE_8)
                    COREPORT_BIT(0x0008, "Test 3 (Solenoids)", KEYCODE_7) PORT_START /* 1 */ /* switch column 9 */
    COREPORT_BIT(0x8000, "Test 4 (Encoder)", KEYCODE_V) COREPORT_BIT(0x0400, "Test 5 (Display)", KEYCODE_C)
        COREPORT_BIT(0x0200, "Test 6 (Position)", KEYCODE_X) COREPORT_BIT(0x0100, "Test 7 (unused)", KEYCODE_Z)
            COREPORT_BIT(0x4000, "Test 8 (Reset NVRAM)", KEYCODE_M) COREPORT_BIT(0x2000, "Test 9 (unused)", KEYCODE_N)
                COREPORT_BIT(0x1000, "Test 10 (unused)", KEYCODE_B)
                    COREPORT_BIT(0x0800, "Keypad End", KEYCODE_END) /* switch column 8 */
    COREPORT_BIT(0x0080, "Keypad 0", KEYCODE_0_PAD) COREPORT_BIT(0x0004, "Keypad 1", KEYCODE_1_PAD)
        COREPORT_BIT(0x0002, "Keypad 2", KEYCODE_2_PAD) COREPORT_BIT(0x0001, "Keypad 3", KEYCODE_3_PAD)
            COREPORT_BIT(0x0040, "Keypad 4", KEYCODE_4_PAD) COREPORT_BIT(0x0020, "Keypad 5", KEYCODE_5_PAD)
                COREPORT_BIT(0x0010, "Keypad 6", KEYCODE_6_PAD)
                    COREPORT_BIT(0x0008, "Keypad 7", KEYCODE_7_PAD) INPUT_PORTS_END

    core_tLCDLayout rot_disp[] = {{0, 0, 0, 6, CORE_SEG7},   {0, 16, 7, 6, CORE_SEG7}, {2, 0, 14, 6, CORE_SEG7},
                                  {2, 16, 21, 6, CORE_SEG7}, {4, 8, 35, 6, CORE_SEG7}, {0}};
