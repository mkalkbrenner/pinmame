...
#define INITGAME(name, disptype, balls, lamps, gs1, gs2)                                                               \
    LTD_INPUT_PORTS_START(name, balls)                                                                                 \
    LTD_INPUT_PORTS_END static core_tGameData name##GameData = {                                                       \
        0, disptype, {FLIP_SW(FLIP_L), 0, lamps, 0, 0, 0, gs1, gs2}};                                                  \
    static void init_##name(void) { core_gameData = &name##GameData; }

static core_tLCDLayout disp2p5[] = {{1, 0, 16, 4, CORE_SEG7},
                                    {1, 8, 15, 1, CORE_SEG7},
                                    {1, 16, 6, 4, CORE_SEG7},
                                    {1, 24, 14, 1, CORE_SEG7},
                                    {0, 12, 11, 1, CORE_SEG7},
...
/ Atlantis
/-------------------------------------------------------------------*/
static core_tLCDLayout dispAtla[] = {DISP_SEG_IMPORT(disp2p5), {5, 0, 4, 1, CORE_SEG7}, {5, 3, 5, 1, CORE_SEG7}, {0}};
...
/ Disco Dancing
/-------------------------------------------------------------------*/
static core_tLCDLayout dispDisco[] = {DISP_SEG_IMPORT(disp2p5), {5, 0, 5, 1, CORE_SEG7}, {0}};
...
/ Martian Queen
/-------------------------------------------------------------------*/
static core_tLCDLayout dispMarqn[] = {DISP_SEG_IMPORT(disp2p5), {5, 0, 13, 1, CORE_SEG7}, {0}};
INITGAME(marqueen, dispMarqn, 1, 5, 0x3f, 0x04)
LTD_2_ROMSTART(marqueen, "mqueen.bin",
               CRC(cb664001) SHA1(00152f89e58bc11567a8de32ccaaa47146dace0d) BAD_DUMP) // patched from bad dump
LTD_ROMEND
...
/ Force
/-------------------------------------------------------------------*/
static core_tLCDLayout dispForce[] = {{1, 0, 16, 4, CORE_SEG7},  {1, 8, 15, 1, CORE_SEG7},  {1, 16, 6, 4, CORE_SEG7},
                                      {1, 24, 5, 1, CORE_SEG7},  {0, 12, 11, 1, CORE_SEG7}, {2, 12, 10, 1, CORE_SEG7},
                                      {4, 0, 3, 1, CORE_SEG7S},  {4, 3, 4, 1, CORE_SEG7S},  {4, 6, 2, 1, CORE_SEG7S},
                                      {6, 0, 12, 2, CORE_SEG7S}, {6, 5, 14, 1, CORE_SEG7S}, {0}};
...
static core_tLCDLayout disp2p6[] = {
    {1, 0, 14, 6, CORE_SEG7}, {1, 18, 4, 6, CORE_SEG7}, {0, 14, 11, 1, CORE_SEG7}, {2, 14, 10, 1, CORE_SEG7}, {0}};

/*-------------------------------------------------------------------
/ Space Poker
/-------------------------------------------------------------------*/
INITGAME(spcpoker, disp2p6, 1, 9, 0x0f, 0x0c)
LTD_2_ROMSTART(spcpoker, "spoker.bin",
               CRC(98918b19) SHA1(b1bb3f408dae9fc77d396894c3c4ef08bce8c345) BAD_DUMP) // patched from bad dump
LTD_ROMEND
...
static core_tLCDLayout disp3p6[] = {{0, 0, 14, 6, CORE_SEG7}, {3, 0, 8, 6, CORE_SEG7}, {6, 0, 2, 6, CORE_SEG7},
                                    {9, 2, 1, 1, CORE_SEG7},  {9, 8, 0, 1, CORE_SEG7}, {0}};
...
#define INITGAME4(name, disptype, balls, lamps)                                                                        \
    LTD4_INPUT_PORTS_START(name, balls)                                                                                \
    LTD_INPUT_PORTS_END static core_tGameData name##GameData = {0, disptype, {0x03000102, 0, lamps}};                  \
    static void init_##name(void) { core_gameData = &name##GameData; }

static core_tLCDLayout disp4p6[] = {{0, 0, 25, 6, CORE_SEG7},
                                    {3, 0, 17, 6, CORE_SEG7},
                                    {6, 0, 9, 6, CORE_SEG7},
                                    {9, 0, 1, 6, CORE_SEG7},
                                    {12, 2, 15, 1, CORE_SEG7},
...
LTD_44_ROMSTART(cowboy, "cowboy_l.bin", CRC(87befe2a) SHA1(93fdf40b10e53d7d95e5dc72923b6be887411fc0), "cowboy_h.bin",
                CRC(105e5d7b) SHA1(75edeab8c8ba19f334479133802acbc25f405763))
LTD_ROMEND
...
/ Haunted Hotel
/-------------------------------------------------------------------*/
static core_tLCDLayout dispHH[] = {DISP_SEG_IMPORT(disp4p6),  {15, 0, 7, 2, CORE_SEG7},  {15, 4, 31, 1, CORE_SEG7},
                                   {15, 6, 16, 1, CORE_SEG7}, {15, 8, 23, 2, CORE_SEG7}, {0}};
INITGAME4(hhotel, dispHH, 3, 0)
LTD_44_ROMSTART(hhotel, "hh1.bin", CRC(a107a683) SHA1(5bb79d9a0a6b33f067cdd54942784c67ab557909), "hh2.bin",
                CRC(e0c2ebc1) SHA1(131240589162c7b3f44a2bb951945c7d64f89c8d))
LTD_ROMEND
...
/ Mr. & Mrs. Pec-Men
/-------------------------------------------------------------------*/
static core_tLCDLayout dispPec[] = {DISP_SEG_IMPORT(disp4p6),  {15, 0, 7, 2, CORE_SEG7},   {15, 5, 31, 1, CORE_SEG7},
                                    {15, 7, 16, 1, CORE_SEG7}, {15, 10, 23, 2, CORE_SEG7}, {0}};
INITGAME4(pecmen, dispPec, 1, 8)
LTD_44_ROMSTART(pecmen, "pecmen_l.bin", CRC(f86c724e) SHA1(635ec94a1c6e77800ef9774102cc639be86c4261), "pecmen_h.bin",
                CRC(013abca0) SHA1(269376af92368d214c3d09ec6d3eb653841666f3))
LTD_ROMEND
CORE_GAMEDEFNV(pecmen,"Mr. & Mrs. Pec-Men",198?,"LTD",gl_mLTD4,0)

static core_tLCDLayout disp4p7[] = {{0, 0, 24, 7, CORE_SEG7},
                                    {3, 0, 16, 7, CORE_SEG7},
                                    {6, 0, 8, 7, CORE_SEG7},
                                    {9, 0, 0, 7, CORE_SEG7},
                                    {12, 2, 15, 1, CORE_SEG7},
...
LTD_44_ROMSTART(alcapone, "alcapo_l.bin", CRC(c4270ba8) SHA1(f3d80af9900c94df2d43f2755341a346a0b64c87), "alcapo_h.bin",
                CRC(279f766d) SHA1(453c58e44c4ef8f1f9eb752b6163c61ebed70b27))
LTD_ROMEND
...
LTD_44_ROMSTART(columbia, "columb_l.bin", CRC(ac345dee) SHA1(14f03fa8059de5cd69cc83638aa6533fbcead37e), "columb_h.bin",
                CRC(acd2a85b) SHA1(30889ee4230ce05f6060f926b2137bbf5939db2d))
LTD_ROMEND
...
LTD_44_ROMSTART(tmacltd4, "tm4_l.bin", CRC(69691662) SHA1(3d86314967075e3f5b168c8d7bf6b26bbbb957bd), "tm4_h.bin",
                CRC(f5f97992) SHA1(ba31f71a600e7061b500e0750f50643503e52a80))
LTD_ROMEND
...
LTD_44_ROMSTART(tricksht, "tricks_l.bin", CRC(951413ff) SHA1(f4a28f7b41cb077377433dc7bfb6647e5d392481), "tricks_h.bin",
                CRC(2e4efb51) SHA1(3dd20addecf4b47bd68b05d557c378d1dbbbd892))
LTD_ROMEND
