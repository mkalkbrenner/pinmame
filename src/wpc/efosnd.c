...
*/
static z80ctc_interface ctc_intf = {2,
                                    {4000000, 4000000},
                                    {0, 0},
...
shi(UINT8 data) {
    int i;
    if (sndlocals.fifoSize < 16)
        sndlocals.fifoSize++;
    for (i = sndlocals.fifoSize - 1; i > 0; i--)
        sndlocals.fifo[i] = sndlocals.fifo[i - 1];
    sndlocals.fifo[0] = data;
}

static UINT8
sho(void) {
    if (sndlocals.fifoSize)
        sndlocals.fifoSize--;
    return sndlocals.fifo[sndlocals.fifoSize];
}

static void
clock_pulse(int dummy) {
...
static void
zsu_init(struct sndbrdData* brdData) {
...
const struct sndbrdIntf zsuIntf = {"ZSU",      zsu_init, NULL, NULL, zsu_data_w,
                                   zsu_data_w, NULL,     NULL, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
...
static WRITE_HANDLER(ay8910_1_a_w) {
    ...
    if (GET_BIT4)
        sndlocals.clockDiv = 4; // 250 kHz
    if (GET_BIT5)
        sndlocals.clockDiv = 2; // 500 kHz
    if (GET_BIT6)
        sndlocals.clockDiv = 1; // 1 MHz
}

struct AY8910interface zsu_8910Int = {2, 2000000, {30, 30}, {NULL}, {NULL}, {ay8910_0_a_w, ay8910_1_a_w}};
...
static MEMORY_READ_START(zsu_readmem){0x0000, 0x6fff, MRA_ROM}, {0x7000, 0x77ff, MRA_RAM},
    {0x8000, 0xffff, MRA_BANKNO(1)},
    MEMORY_END

    static MEMORY_WRITE_START(zsu_writemem){0x0000, 0x6fff, MWA_NOP},
    {0x7000, 0x77ff, MWA_RAM},
    MEMORY_END

    static PORT_READ_START(zsu_readport){0x00, 0x03, z80ctc_0_r},
    {0x04, 0x07, z80ctc_1_r}, {0x14, 0x14, snd_r},
    MEMORY_END

    static PORT_WRITE_START(zsu_writeport){0x00, 0x03, z80ctc_0_w},
    {0x04, 0x07, z80ctc_1_w}, {0x08, 0x08, fifo_w}, {0x0c, 0x0d, ay0_w}, {0x10, 0x11, ay1_w},
    MEMORY_END

    static Z80_DaisyChain zsu_DaisyChain[] = {{z80ctc_reset, z80ctc_interrupt, z80ctc_reti, 1},
