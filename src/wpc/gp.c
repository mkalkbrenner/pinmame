...
static void
GP_lampStrobe1(int lampadr, int lampdata) {
...
static void
GP_lampStrobe2(int lampadr, int lampdata) {
    UINT8 *matrix1, *matrix2;
    int bit, select;
...
static void
GP_UpdateSolenoids(int bank, int data) {
...
static SWITCH_UPDATE(GP) {
    ...
    if (inports) {
        ...
        coreGlobals.swMatrix[1] = (coreGlobals.swMatrix[1] & (~0xf7)) | ((inports[GP_COMINPORT] & 0xff) & 0xf7);
        coreGlobals.swMatrix[4] = (coreGlobals.swMatrix[4] & (~0x02)) | ((inports[GP_COMINPORT] >> 7) & 0x02);
    }
    /*-- Diagnostic buttons on CPU board --*/
    if (core_getSw(GP_SWTEST))
        generic_nvram[0xac] = val++;
...
static WRITE_HANDLER(ppi0_pa_w) { locals.PORTA_WRITE(offset, data); }
...
static WRITE_HANDLER(ppi0_pb_w) { logerror("PPI0 Port B Write: %02x\n", data); }
...
static WRITE_HANDLER(ppi0_pc_w) {
    ...
    if ((data >> 7) & 1) {
        ...
        coreGlobals.pulsedSolState |= 0x8000;
    } else {
        locals.solenoids &= ~0x8000;
        coreGlobals.pulsedSolState &= ~0x8000;
    }
    if ((data >> 5) & 1) {
        locals.solenoids |= 0x40000000;
        coreGlobals.pulsedSolState |= 0x40000000;
    } else {
        locals.solenoids &= ~0x40000000;
        coreGlobals.pulsedSolState &= ~0x40000000;
    }
    if ((data >> 6) & 1) {
        locals.solenoids |= 0x80000000;
        coreGlobals.pulsedSolState |= 0x80000000;
    } else {
        locals.solenoids &= ~0x80000000;
        coreGlobals.pulsedSolState &= ~0x80000000;
    }
...
static Z80_DaisyChain GP_DaisyChain[] = {{z80ctc_reset, z80ctc_interrupt, z80ctc_reti, 0}, {0, 0, 0, -1}};

/*-----------------------------------
/  Memory map for CPU board
/------------------------------------*/
static MEMORY_READ_START(GP_readmem){0x0000, 0x2fff, MRA_ROM},
    {0x8c00, 0x8cff, MRA_RAM}, /*256B CMOS RAM - Battery Backed*/
    {0x8d00, 0x8d7f, MRA_RAM}, /*128B NMOS RAM*/
    MEMORY_END

    static MEMORY_WRITE_START(GP_writemem){0x0000, 0x2fff, MWA_ROM},
    {0x8c00, 0x8cff, MWA_RAM, &generic_nvram, &generic_nvram_size}, /*256B CMOS RAM - Battery Backed*/
    {0x8d00, 0x8d7f, MWA_RAM},                                      /*128B NMOS RAM*/
    MEMORY_END

    static PORT_READ_START(GP_readport){0x04, 0x07, ppi8255_0_r},
    {0x08, 0x0b, z80ctc_0_r},
    PORT_END

    static PORT_WRITE_START(GP_writeport){0x04, 0x07, ppi8255_0_w},
    {0x08, 0x0b, z80ctc_0_w},
    PORT_END

    MACHINE_DRIVER_START(GP1) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD_TAG("mcpu", Z80, 2457600)
        MDRV_CPU_CONFIG(GP_DaisyChain) MDRV_CPU_MEMORY(GP_readmem, GP_writemem)
            MDRV_CPU_PORTS(GP_readport, GP_writeport) MDRV_CPU_VBLANK_INT(GP_vblank, 1)
                MDRV_TIMER_ADD(GP_zeroCross, GP_ZCFREQ) MDRV_CORE_INIT_RESET_STOP(GP1, NULL, GP)
                    MDRV_NVRAM_HANDLER(generic_0fill) MDRV_DIPS(32) MDRV_SWITCH_UPDATE(GP) MDRV_DIAGNOSTIC_LEDH(1)
                        MDRV_SOUND_CMD(GP_soundCmd) MDRV_SOUND_CMDHEADING("GP") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GP1S1) MDRV_IMPORT_FROM(GP1) MDRV_IMPORT_FROM(gpSSU1) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GP2) MDRV_IMPORT_FROM(GP1) MDRV_CORE_INIT_RESET_STOP(GP2, NULL, GP) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GP2S1) MDRV_IMPORT_FROM(GP2) MDRV_IMPORT_FROM(gpSSU1) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GP2S2) MDRV_IMPORT_FROM(GP2) MDRV_IMPORT_FROM(gpSSU2) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GP2S4) MDRV_IMPORT_FROM(GP2) MDRV_IMPORT_FROM(gpSSU4) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GP2SM) MDRV_IMPORT_FROM(GP2) MDRV_IMPORT_FROM(gpMSU1)
        MDRV_DIAGNOSTIC_LEDH(2) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(GP2SM3) MDRV_IMPORT_FROM(GP2) MDRV_IMPORT_FROM(gpMSU3)
        MDRV_DIAGNOSTIC_LEDH(2) MACHINE_DRIVER_END
