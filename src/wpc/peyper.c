...
static INTERRUPT_GEN(PEYPER_irq) { cpu_set_irq_line(PEYPER_CPU, 0, HOLD_LINE); }
...
static int
PEYPER_m2sw(int col, int row) {
    return (col - 1) * 10 + row;
}

static READ_HANDLER(dip_r) { return ~core_revbyte(core_getDip(offset / 4)); }

static READ_HANDLER(sw0_r) { return ~coreGlobals.swMatrix[0]; }
...
static WRITE_HANDLER(lamp7_w) { coreGlobals.tmpLampMatrix[7] = data; }
...
struct AY8910interface PEYPER_ay8910Int = {
    ...
    2500000,  /* 2.5 MHz */
    {30, 30}, /* Volume */
    {0, 0},   {0, 0}, {ay8910_0_porta_w, ay8910_1_porta_w}, {ay8910_0_portb_w, ay8910_1_portb_w},
};
...
static READ_HANDLER(i8279_r) {
    ...
    if ((locals.i8279cmd & 0xe0) == 0x40)
        lastData = coreGlobals.swMatrix[1 + (locals.i8279cmd & 0x07)]; // read switches (only 4 columns actually used)
    ...
    if (offset) { // command
        ...
        if ((locals.i8279cmd & 0xe0) == 0x80) { // write display ram
            if (!core_gameData->hw.gameSpecific1
                && (coreGlobals.tmpLampMatrix[8] & 0x11) == 0x11) { // load replay values
...
static MEMORY_READ_START(PEYPER_readmem){0x0000, 0x5fff, MRA_ROM}, {0x6000, 0x67ff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(PEYPER_writemem){0x0000, 0x5fff, MWA_NOP},
    {0x6000, 0x67ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static PORT_READ_START(PEYPER_readport){0x00, 0x01, i8279_r},
    {0x20, 0x24, dip_r}, // only 20, 24 used
    {0x28, 0x28, sw0_r},
    PORT_END

    static PORT_WRITE_START(PEYPER_writeport){0x00, 0x01, i8279_w},
    {0x04, 0x04, ay8910_0_ctrl_w}, {0x06, 0x06, ay8910_0_data_w}, {0x08, 0x08, ay8910_1_ctrl_w},
    {0x0a, 0x0a, ay8910_1_data_w}, {0x0c, 0x0c, sol_w}, {0x10, 0x18, lamp_w}, // only 10, 14, 18 used
    {0x2c, 0x2c, lamp7_w},
    PORT_END
