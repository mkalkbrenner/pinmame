...
#define CC_IRQ4FREQ                                                                                                    \
    TIME_IN_CYCLES(                                                                                                    \
        4000,                                                                                                          \
        0) //Seems to work well for both KP & FF //was 8000 before, but experiments showed that 4000 works better (no lamp flicker and quicker lamp response on KP)
...
static READ16_HANDLER(io_r) {
    ...
    switch (offset) {
        ...
        case 0x20000b:
            ...
            data =
                (coreGlobals.swMatrix[swcol + 4] << 8 | coreGlobals.swMatrix[swcol]) ^ 0xffff; //Switches are inverted
            break;
        ...
        case 0x400000:
            if (!core_gameData->hw.lampCol) {
                ...
                switch (locals.swCol) {
                    case 0x80:
                        data |= coreGlobals.swMatrix[1] << 8;
                        break;
                    case 0x40:
                        data |= coreGlobals.swMatrix[2] << 8;
                        break;
                    case 0x20:
                        data |= coreGlobals.swMatrix[3] << 8;
                        break;
                    case 0x10:
                        data |= coreGlobals.swMatrix[4] << 8;
                        break;
                    case 0x08:
                        data |= coreGlobals.swMatrix[5] << 8;
                        break;
                    case 0x04:
                        data |= coreGlobals.swMatrix[6] << 8;
                        break;
                    case 0x02:
                        data |= coreGlobals.swMatrix[7] << 8;
                        break;
                    case 0x01:
                        data |= coreGlobals.swMatrix[8] << 8;
                        break;
                }
            } else
    ...
    switch (offset) {
        ...
        case 0x0000000c:
            if (!core_gameData->hw.lampCol)
                ...
                for (i = 0; i < 8; i++) {
                    if (data & (0x0100 << i))
                        coreGlobals.tmpLampMatrix[i] |= ~data & 0xff;
                    for (j = 0; j < 8; j++) {
        ...
        case 0x0000000d:
            if (!locals.blanking) {
                for (i = 0; i < 8; i++) {
                    if (data & (0x0100 << i))
                        coreGlobals.tmpLampMatrix[core_gameData->hw.lampCol + i] |= ~data & 0xff;
                    for (j = 0; j < 8; j++) {
                        if ((data & (0x0100 << i)) && (~data & (1 << j))) {
                            locals.lamps[(core_gameData->hw.lampCol + i) * 8 + j] =
                                brt(locals.lamps[(core_gameData->hw.lampCol + i) * 8 + j]);
...
static MACHINE_INIT(cc) {
    ...
    timer_pulse(TIME_IN_CYCLES(2811, 0), 0,
                cc_u16irq1); //Only value that passes IRQ1 test (DO NOT CHANGE UNTIL CURRENT HACK IS REPLACED)
...
static NVRAM_HANDLER(cc) { core_nvram(file, read_or_write, CMOS, 0x10000, 0x00); }
...
----------------------
 ...
 1 1 (40c00000) - /CS		(U16)
*/
static MEMORY_READ16_START(cc_readmem){0x00000000, 0x0007ffff, MRA16_RAM}, /* DRAM */
    ...
    static MEMORY_WRITE16_START(cc_writemem){0x00000000, 0x0007ffff, MWA16_RAM, &ramptr}, /* DRAM */
    ...
    static PORT_READ16_START(cc_readport){M68306_PORTA_START, M68306_PORTA_END, cc_porta_r},
    {M68306_PORTB_START, M68306_PORTB_END, cc_portb_r},
    PORT_END static PORT_WRITE16_START(cc_writeport){M68306_PORTA_START, M68306_PORTA_END, cc_porta_w},
    {M68306_PORTB_START, M68306_PORTB_END, cc_portb_w},
    PORT_END

    //Default hardware - no sound support
    MACHINE_DRIVER_START(cc) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(cc, NULL, NULL)
        MDRV_CPU_ADD(M68306, CPU_CLOCK) MDRV_CPU_MEMORY(cc_readmem, cc_writemem)
            MDRV_CPU_PORTS(cc_readport, cc_writeport) MDRV_CPU_VBLANK_INT(cc_vblank, 1) MDRV_NVRAM_HANDLER(cc)
                MDRV_SWITCH_UPDATE(cc) MDRV_SWITCH_CONV(cc_sw2m, cc_m2sw) MDRV_DIAGNOSTIC_LEDH(2)
                    MDRV_TIMER_ADD(cc_zeroCross, CC_ZCFREQ) MACHINE_DRIVER_END

    //Sound hardware - 1 x TMS320AV120
    MACHINE_DRIVER_START(cc1) MDRV_IMPORT_FROM(cc) MDRV_IMPORT_FROM(capcom1s) MACHINE_DRIVER_END
    //Sound hardware - 2 x TMS320AV120
    MACHINE_DRIVER_START(cc2) MDRV_IMPORT_FROM(cc) MDRV_IMPORT_FROM(capcom2s) MACHINE_DRIVER_END
...
PINMAME_VIDEO_UPDATE(cc_lamp16x8) {
    ...
    for (jj = 0; jj < 16; jj++) {
        for (ii = 0; ii < 8; ii += 4) {
            *seg++ = (coreGlobals.dotCol[1 + ii][jj] - 63) | ((coreGlobals.dotCol[2 + ii][jj] - 63) << 4)
                     | ((coreGlobals.dotCol[3 + ii][jj] - 63) << 8) | ((coreGlobals.dotCol[4 + ii][jj] - 63) << 12);
        }
    }
    video_update_core_dmd(bitmap, cliprect, layout);
    return 0;
...
static READ16_HANDLER(snd_r) { return qsound_status_r(0); }
...
static WRITE16_HANDLER(vol_w) {
    if (data == 0x82 && locals.vset)
        locals.vset--;
    if (data == 0xaa && locals.vset < 100)
        locals.vset++;
...
static MEMORY_READ16_START(romstar_readmem){0x00000000, 0x0007ffff, MRA16_RAM}, {0x10000000, 0x100fffff, MRA16_ROM},
    {0x20000000, 0x200fffff, MRA16_ROM}, {0x30000000, 0x3000ffff, MRA16_RAM}, {0x50000008, 0x50000009, snd_r},
    {0x60000000, 0x60000003, sw_r},
    MEMORY_END

    static MEMORY_WRITE16_START(romstar_writemem){0x00000000, 0x0007ffff, MWA16_RAM, &ramptr},
    {0x10000000, 0x100fffff, MWA16_ROM, &rom_base}, {0x20000000, 0x200fffff, MWA16_ROM, &rom_base2},
    {0x30000000, 0x3000ffff, MWA16_RAM, &CMOS}, {0x40000000, 0x4000ffff, disp_w}, {0x50000008, 0x5000000b, snd_w},
    {0x5000000c, 0x5000000d, vol_w}, {0x60000004, 0x60000009, sol_w}, {0x6000000a, 0x6000000b, lamp_w},
...
static WRITE16_HANDLER(romstar_port_w) { coreGlobals.diagnosticLed = data & 8 ? 0 : 1; }

static PORT_READ16_START(romstar_readport){0, 1, romstar_port_r},
    PORT_END

    static PORT_WRITE16_START(romstar_writeport){0, 1, romstar_port_w},
    PORT_END

    static struct QSound_interface romstar_qsoundInt = {
        QSOUND_CLOCK, CAPCOMS_ROMREGION, {MIXER(100, MIXER_PAN_LEFT), MIXER(100, MIXER_PAN_RIGHT)}};
