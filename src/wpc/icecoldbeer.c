...
static READ_HANDLER(stick_r) { return ~coreGlobals.swMatrix[9]; }
...
static WRITE_HANDLER(snd_data_w) { locals.sndData = data; }
...
static WRITE_HANDLER(motor_w) { coreGlobals.solenoids = (coreGlobals.solenoids & 0x1ff00) | (data ^ 0xff); }
...
static MEMORY_WRITE_START(ice_writemem){0x0000, 0x07ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    {0x4000, 0x4007, ay_w}, {0x4010, 0x4013, pia_w(0)}, {0x4020, 0x4023, pia_w(1)},
    {0x4040, 0x4043, pia_w(2)}, // unused
    {0x4080, 0x4081, i8279_w}, {0x4100, 0x4100, motor_w}, {0xa000, 0xffff, MWA_NOP},
    MEMORY_END

    static MEMORY_READ_START(ice_readmem){0x0000, 0x07ff, MRA_RAM},
    {0x4000, 0x4007, ay_r}, {0x4010, 0x4013, pia_r(0)}, {0x4020, 0x4023, pia_r(1)},
    {0x4040, 0x4043, pia_r(2)}, // unused
    {0x4080, 0x4081, i8279_r}, {0xa000, 0xffff, MRA_ROM},
    MEMORY_END
...
static READ_HANDLER(ay8910_0_porta_r) { return core_getDip(1); }
...
static MACHINE_INIT(ICE) {
    int i;
    for (i = 0; i < 3; i++)
        pia_config(i, PIA_STANDARD_ORDERING, &ice_pia[i]);
    pia_reset();
...
static MACHINE_RESET(ICE) { memset(&locals, 0, sizeof(locals)); }

static MACHINE_STOP(ICE) { pia_unconfig(); }
...
static INTERRUPT_GEN(ice_vblank) {
    core_updateSw(TRUE);
    if (coreGlobals.swMatrix[1] || coreGlobals.swMatrix[2] || coreGlobals.swMatrix[3])
        sirq(1);
}
...
#define INITGAME(name, disptype)                                                                                       \
    ...
    SIM_PORTS(1)                                                                                                       \
    PORT_START /* 0 */                                                                                                 \
        COREPORT_BIT(0x0040, "Left Up Limit", KEYCODE_Z) COREPORT_BIT(0x0080, "Left Joystick Up", KEYCODE_X)           \
            COREPORT_BIT(0x0008, "Left Joystick Down", KEYCODE_C) COREPORT_BIT(                                        \
                0x0004, "Left Down Limit", KEYCODE_V) COREPORT_BIT(0x0010, "Right Up Limit", KEYCODE_B)                \
                COREPORT_BIT(0x0020, "Right Joystick Up", KEYCODE_N) COREPORT_BIT(                                     \
                    0x0002, "Right Joystick Down", KEYCODE_M) COREPORT_BIT(0x0001, "Right Down Limit", KEYCODE_COMMA)  \
                    COREPORT_BIT(0x0100, "Test", KEYCODE_0) COREPORT_BITDEF(                                           \
                        0x1000, IPT_COIN1, IP_KEY_DEFAULT) COREPORT_BITDEF(0x2000, IPT_START1,                         \
                                                                           IP_KEY_DEFAULT) PORT_START /* 1 */          \
                        COREPORT_DIPNAME(0x0001, 0x0000, "S1") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(           \
                            0x0001, "1") COREPORT_DIPNAME(0x0002, 0x0000, "S2") COREPORT_DIPSET(0x0000, "0")           \
                            COREPORT_DIPSET(0x0002, "1") COREPORT_DIPNAME(0x0004, 0x0000, "S3") COREPORT_DIPSET(       \
                                0x0000, "0") COREPORT_DIPSET(0x0004, "1") COREPORT_DIPNAME(0x0008, 0x0000, "S4")       \
                                COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0008, "1") COREPORT_DIPNAME(            \
                                    0x0010, 0x0000, "S5") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0010, "1")    \
                                    COREPORT_DIPNAME(0x0020, 0x0000, "S6") COREPORT_DIPSET(                            \
                                        0x0000, "0") COREPORT_DIPSET(0x0020, "1") COREPORT_DIPNAME(0x0040, 0x0000,     \
                                                                                                   "S7")               \
                                        COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0040, "1") COREPORT_DIPNAME(    \
                                            0x0080, 0x0000,                                                            \
                                            "S8") COREPORT_DIPSET(0x0000,                                              \
                                                                  "0") COREPORT_DIPSET(0x0080, "1")                    \
                                            COREPORT_DIPNAME(0x0100, 0x0000, "S9") COREPORT_DIPSET(0x0000, "0")        \
                                                    ...
                                                    COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(                      \
                                                        ...
                                                        COREPORT_DIPSET(0x0400, "1") COREPORT_DIPNAME(                 \
                                                            ...
                                                            "S12") COREPORT_DIPSET(0x0000,                             \
                                                                                   "0") COREPORT_DIPSET(0x0800, "1")   \
                                                            COREPORT_DIPNAME(0x1000, 0x0000, "S13") COREPORT_DIPSET(   \
                                                                0x0000,                                                \
                                                                "0") COREPORT_DIPSET(0x1000, "1")                      \
                                                                COREPORT_DIPNAME(0x2000, 0x0000,                       \
                                                                    ...
                                                                    COREPORT_DIPSET(0x2000, "1") COREPORT_DIPNAME(     \
                                                                        ...
                                                                        COREPORT_DIPSET(0x4000, "1") COREPORT_DIPNAME( \
                                                                            0x8000,                                    \
                                                                            0x0000,                                    \
                                                                            "S16") COREPORT_DIPSET(0x0000, "0")        \
                                                                            COREPORT_DIPSET(0x8000, "1")               \
                                                                                INPUT_PORTS_END static core_tGameData  \
                                                                                    name##GameData = {                 \
                                                                                        0,                             \
                                                                                        icb_disp,                      \
                                                                                        {FLIP_SW(FLIP_L), 0, -5}};     \
...
core_tLCDLayout icb_disp[] = {{0, 0, 7, 1, CORE_SEG7}, {3, 0, 4, 3, CORE_SEG7}, {6, 0, 0, 4, CORE_SEG7}, {0}};
