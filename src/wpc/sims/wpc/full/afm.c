...
/------------------------*/
static struct afm_tLocals { int bankPos; /* bank motor position */ } afm_locals;
...
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "M", KEYCODE_J) COREPORT_BIT(0x0002, "A", KEYCODE_K)
        COREPORT_BIT(0x0004, "R", KEYCODE_OPENBRACE) COREPORT_BIT(0x0008, "T", KEYCODE_CLOSEBRACE)
            COREPORT_BIT(0x0010, "I", KEYCODE_QUOTE) COREPORT_BIT(0x0020, "A", KEYCODE_COMMA)
                COREPORT_BIT(0x0040, "N", KEYCODE_COLON) COREPORT_BIT(0x0080, "Jet 1", KEYCODE_V)
                    COREPORT_BIT(0x0100, "Jet 2", KEYCODE_B) COREPORT_BIT(0x0200, "Jet 3", KEYCODE_N)
                        COREPORT_BIT(0x0400, "Force 1", KEYCODE_F) COREPORT_BIT(0x0800, "Force 2", KEYCODE_G)
                            COREPORT_BIT(0x1000, "Force 3", KEYCODE_H) COREPORT_BITIMP(0x2000, "Drop Target", KEYCODE_D)
                                COREPORT_BITIMP(0x4000, "Hole", KEYCODE_Z)
                                    COREPORT_BITIMP(0x8000, "Stroke of Luck", KEYCODE_U) PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BITIMP(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BITIMP(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BITIMP(0x0010, "L/R Loop", KEYCODE_L) COREPORT_BITIMP(0x0020, "L/R Bonus X", KEYCODE_X)
                COREPORT_BIT(0x0040, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0080, "L/R Inlane", KEYCODE_I)
                    COREPORT_BIT(0x0100, "L/R Saucer", KEYCODE_S) COREPORT_BITIMP(0x0200, "Center Ramp", KEYCODE_C)
                        COREPORT_BITIMP(0x0400, "Drain", KEYCODE_Q) WPC_INPUT_PORTS_END
...
static sim_tState afm_stateDef[] = {
    ...
    {"Left Popper", 1, swLPop, sLPop, stLIn, 10},
    /* stLPop */ /* up to left ramp */
    {"Right Popper", 1, swRPop, sRPop, stFree, 5},
    /* stRPop */                                                            /* to playfield */
...
static int
afm_getSol(int solNo) {
    if (solNo == sLGate)
        return (wpc_data[WPC_FLIPPERCOIL95] & 0x10) > 0;
    if (solNo == sRGate)
        return (wpc_data[WPC_FLIPPERCOIL95] & 0x20) > 0;
    if (solNo == sDiverter)
        return (wpc_data[WPC_FLIPPERCOIL95] & 0xc0) > 0;
    return 0;
}

static void
afm_handleMech(int mech) {
    /*---------------
  /  handle bank
  /----------------*/
    if (mech & 0x01) {
        if (core_getSol(sBankMotor))
            afm_locals.bankPos = (afm_locals.bankPos + 1) % AFM_BANKTIME;
        core_setSw(swMBankDn, afm_locals.bankPos < AFM_BANKSLACK);
        core_setSw(swMBankUp,
                   (afm_locals.bankPos >= AFM_BANKTIME / 2) && (afm_locals.bankPos < AFM_BANKTIME / 2 + AFM_BANKSLACK));
    }
    /*-------------
  /  Drop target
  /--------------*/
...
/----------------------------*/
static sim_tInportData afm_inportData[] = {{0, 0x0001, swmARTIAN},
                                           {0, 0x0002, swMaRTIAN},
                                           {0, 0x0004, swMArTIAN},
                                           {0, 0x0008, swMARtIAN},
                                           {0, 0x0010, swMARTiAN},
...
DCS_SOUNDROM3m("afm_1_00.s2", CRC(610ff107) SHA1(9590f809a05cb2bda4979fa16f165e2e994719a0), "afm_s3.l1",
               CRC(1cbce9b1) SHA1(7f258bfe1904a879a2cb007419483f4fee91e072), "afm_s4.l1",
               CRC(5ff7fbb7) SHA1(ebaf825d3b90b6acee1920e6703801a4bcddfc5b)) WPC_ROMEND

    WPC_ROMSTART(afm, 11u, "marsu1_1.rom", 0x080000, CRC(bc1c0a0a) SHA1(859b40aaae46623f9b3519b5e422977d1724b715))
        DCS_SOUNDROM3m("afm_s2.l1", CRC(6e39d96e) SHA1(b34e31bb1734c86614f153f7201163aaa9943cec), "afm_s3.l1",
                       CRC(1cbce9b1) SHA1(7f258bfe1904a879a2cb007419483f4fee91e072), "afm_s4.l1",
                       CRC(5ff7fbb7) SHA1(ebaf825d3b90b6acee1920e6703801a4bcddfc5b)) WPC_ROMEND
...
/  Game drivers
/---------------*/
    CORE_GAMEDEF(afm, 113, "Attack From Mars (1.13 / S1.1)", 1995, "Bally", wpc_m95S,
                 0) CORE_CLONEDEF(afm, 113b, 113, "Attack From Mars (1.13b / S1.1)", 1995, "Bally", wpc_m95S, 0)
        CORE_CLONEDEF(afm, 11, 113, "Attack From Mars (1.1 / S1.0)", 1995, "Bally", wpc_m95S,
                      0) CORE_CLONEDEF(afm, 11u, 113, "Attack From Mars (1.1 Ultrapin / S1.1)", 2006, "Global VR",
                                       wpc_m95S, 0)
            CORE_CLONEDEF(afm, 11pfx, 113, "Attack From Mars (1.1 Pinball FX / S1.1)", 2018, "Zen Studios", wpc_m95S,
                          0) CORE_CLONEDEF(afm, 10, 113, "Attack From Mars (1.0 / S1.0)", 1995, "Bally", wpc_m95S, 0)
...
static core_tGameData afmGameData = {
    GEN_WPC95,
    wpc_dispDMD,
    {FLIP_SW(FLIP_L | FLIP_U) | FLIP_SOL(FLIP_L), 0, 2, 3, 0, 0, 1, 0, // 2 extra lamp columns for the LEDs
     afm_getSol, afm_handleMech, afm_getMech, afm_drawMech, NULL, NULL},
    &afmSimData,
    {
...
static WRITE_HANDLER(parallel_0_out) { coreGlobals.lampMatrix[8] = coreGlobals.tmpLampMatrix[8] = data; }

static WRITE_HANDLER(parallel_1_out) { coreGlobals.lampMatrix[9] = coreGlobals.tmpLampMatrix[9] = data; }

static WRITE_HANDLER(qspin_0_out) { HC4094_data_w(1, data); }

static HC4094interface hc4094afm = {2, // 2 chips
...
/*
  ...
  */
void
afm_wpc_proc_solenoid_handler(int solNum, int enabled, int smoothed) {
