...
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BITIMP(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BIT(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BIT(0x0010, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0020, "L/R Inlane", KEYCODE_I)
                COREPORT_BIT(0x0040, "Left Jet", KEYCODE_W) COREPORT_BIT(0x0080, "Right Jet", KEYCODE_E)
                    COREPORT_BIT(0x0100, "Bottom Jet", KEYCODE_S) COREPORT_BIT(0x0200, "Hit Trunk", KEYCODE_T)
                                    ...
                                    COREPORT_BIT(0x2000, "Hokus Pokus Targets", KEYCODE_G)
                                        COREPORT_BIT(0x4000, "Trap Door Targets", KEYCODE_H)
                                            COREPORT_BIT(0x8000, "Poof Target", KEYCODE_J)
                                                ...
                                                PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Bonus X L", KEYCODE_N) COREPORT_BIT(0x0002, "Bonus X R", KEYCODE_M)
        COREPORT_BIT(0x0004, "Rear Trunk Entrance", KEYCODE_R) COREPORT_BIT(0x0008, "Buy-In", KEYCODE_2)
...
static int
tom_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
...
static void
tom_initSim(sim_tBallStatus* balls, int* inports, int noOfBalls) {
...
/*--------------------
  Drawing information
  --------------------*/
static core_tLampDisplay tom_lampPos = {{0, 0},   /* top left */
                                        {43, 26}, /* size */
                                        {/* 11 */ {1, {{29, 7, RED}}},
                                         /* 12 */ {1, {{28, 9, RED}}},
                                         /* 13 */ {1, {{27, 11, RED}}},
...
static wpc_tSamSolMap tom_SamSolMap[] = {
    ...
    {sCLoopPost, 0, SAM_SOLENOID_ON},
    {sCLoopPost, 0, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},

    /*Channel #1*/
    {sLeftSling, 1, SAM_LSLING},
    {sRightSling, 1, SAM_RSLING},
    {sSubwayPopper, 1, SAM_POPPER},
    {sBottomJet, 1, SAM_JET1},

    /*Channel #2*/
    {sTrapDoorUp, 2, SAM_FLAPOPEN},
    {sTrapDoorHold, 2, SAM_FLAPCLOSE, WPCSAM_F_ONOFF},
    {sBoxClockwise, 2, SAM_MOTOR_1, WPCSAM_F_CONT},
    {sBoxCounterClockwise, 2, SAM_MOTOR_1, WPCSAM_F_CONT},
    {sTDiverterPost, 2, SAM_SOLENOID_ON},
    {sTDiverterPost, 2, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},
...
tom_drawMech(BMTYPE** line) {
    ...
    core_textOutf(30, 40, BLACK, "L.Post: %-2s T.Post: %-2s", core_getSol(sCLoopPost) ? "Up" : "Dn",
                  core_getSol(sTDiverterPost) ? "Up" : "Dn");
    core_textOutf(30, 50, BLACK, "L.Gate: %-2s R.Gate: %-2s", core_getSol(sLUpDownGate) ? "Op" : "Cl",
                  core_getSol(sRUpDownGate) ? "Op" : "Cl");
...
#define TOM_SOUND                                                                                                      \
    DCS_SOUNDROM6x("tm_u2_s.l2", CRC(b128fbba) SHA1(59101f9f4f43c240630dfbdc7fb432a9939f122d), "tm_u3_s.l2",           \
                   CRC(128c7d3c) SHA1(1bd5b56d3f9c8485498746ae6c4d65a1e053161a), "tm_u4_s.l2",                         \
                   CRC(3d9b2354) SHA1(a39917c0cceda33288594652c47fd0385a85b8b1), "tm_u5_s.l2",                         \
                   CRC(44247b60) SHA1(519b9d6eab4fe05676382c5f99ea87d4f7a12c5e), "tm_u6_s.l2",                         \
                   CRC(f366bbe5) SHA1(aca23649a54521748e90aa9a182b9bbdde126409), "tm_u7_s.l2",                         \
                   CRC(f98e9e38) SHA1(bf8c204cfbbf5f9d59b7ad03d1784d37c638712c))

/*-----------------
/  ROM definitions  (Take them from Games.c)
/------------------*/
WPC_ROMSTART(tom, 14h, "1_40h.u6", 0x80000, CRC(4181db9b) SHA1(027ada8518207d5a841ec3cc8c7842c7b3841f70))
TOM_SOUND WPC_ROMEND WPC_ROMSTART(tom, 14hb, "1_40hb.u6", 0x80000,
                                  CRC(93fc2b7b) SHA1(8b180f7e92ab4f9daabb648ad3e64c6005869083)) TOM_SOUND WPC_ROMEND
    WPC_ROMSTART(tom, 13, "tom1_3x.rom", 0x80000,
                 CRC(aff4d14c) SHA1(9896f3034bb7a59c9e241d16bf231fefc0ae1fd0)) TOM_SOUND WPC_ROMEND
    WPC_ROMSTART(tom, 13f, "tom1_3f.rom", 0x80000,
                 CRC(8a87756b) SHA1(c1f600ed1b88880b5cfd0ae0cc3f1ed1cffbf8ea)) TOM_SOUND WPC_ROMEND
    WPC_ROMSTART(tom, 13c, "Theatre Of Magic U6 game ROM rev 13x patch f0f8.rom", 0x80000,
                 CRC(ae9f0009) SHA1(7bed19310e8fa8398ca3dfc6bda37510aabc0325)) TOM_SOUND WPC_ROMEND //current patch
    ...
    WPC_ROMSTART(tom, 12a, "tom1_2a.rom", 0x80000,
                 CRC(e560ebef) SHA1(8f975c71f6f70503f237d87655be67182c40fb9e)) TOM_SOUND WPC_ROMEND
    WPC_ROMSTART(tom, 12, "tom1_2x.rom", 0x80000,
                 CRC(bd8dd884) SHA1(2cb74ae5082d8ceaf89b8ef4df86f78cb5ba6463)) TOM_SOUND WPC_ROMEND
    WPC_ROMSTART(tom, 121, "tom1_21.rom", 0x80000,
                 CRC(d578ba58) SHA1(cbc195935a0ffad282941e605fd94c1b095d17de)) TOM_SOUND WPC_ROMEND
    WPC_ROMSTART(tom, 10f, "tom1.0f", 0x80000,
                 CRC(be7626ad) SHA1(2f7918b9d2d0618671d7a8676cf69ee76e86bcb9)) TOM_SOUND WPC_ROMEND
    WPC_ROMSTART(tom, 101f, "tom1.01f", 0x80000,
                 CRC(b46ce0c2) SHA1(679c3d7a0110e17090078f8fd921bd1523ab8f51)) TOM_SOUND WPC_ROMEND
    WPC_ROMSTART(tom, 06, "u6-06a.rom", 0x80000,
                 CRC(dc1d6681) SHA1(7e60e9fd6e953e3c2899ae2fb2900982f078a4ba)) TOM_SOUND WPC_ROMEND
    WPC_ROMSTART(tom, 061, "u6-061.rom", 0x80000,
                 CRC(b4e8045d) SHA1(f53fc7b8616d3a320814eae728bf2ac4f2e50819)) TOM_SOUND WPC_ROMEND
...
/---------------*/
    ...
    CORE_CLONEDEF(tom, 13c, 13, "Theatre of Magic (1.3XC Competition MOD)", 2019, "Bally", wpc_mSecurityS,
                  0) //based on 1.3X, NOT the "Tiger saw" rev. 1.4H, but it includes Tiger saw support!
    ...
    CORE_CLONEDEF(tom, 14h, 13, "Theatre of Magic (1.4H)", 2005, "Bally", wpc_mSecurityS,
                  0) CORE_CLONEDEF(tom, 14hb, 13, "Theatre of Magic (1.4H Coin Play)", 2005, "Bally", wpc_mSecurityS, 0)
            ...
            CORE_CLONEDEF(tom, 12, 13, "Theatre of Magic (1.2X)", 1995, "Bally", wpc_mSecurityS, 0)
                CORE_CLONEDEF(tom, 121, 13, "Theatre of Magic (1.21X LED Ghost Fix)", 1995, "Bally", wpc_mSecurityS, 0)
                    CORE_CLONEDEF(tom, 10f, 13, "Theatre of Magic (1.0 French)", 1995, "Bally", wpc_mSecurityS, 0)
                        CORE_CLONEDEF(tom, 101f, 13, "Theatre of Magic (1.01 French LED Ghost Fix)", 1995, "Bally",
                                      wpc_mSecurityS, 0) CORE_CLONEDEF(tom, 06, 13, "Theatre of Magic (0.6A Prototype)",
                                                                       1995, "Bally", wpc_mSecurityS, 0)
                            CORE_CLONEDEF(tom, 061, 13, "Theatre of Magic (0.61A LED Ghost Fix)", 1995, "Bally",
                                          wpc_mSecurityS, 0)
...
static core_tGameData tomGameData = {
    GEN_WPCSECURITY,
    wpc_dispDMD,
    {FLIP_SW(FLIP_L) | FLIP_SOL(FLIP_L), 0, 0, 0, 0, 0, 0, 0, NULL, tom_handleMech, tom_getMech, tom_drawMech,
