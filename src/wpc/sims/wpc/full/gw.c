...
--------------------------------------------------------------------------------------------------*/
static struct { int rampPos; /* Ramp Position */ } locals;
    ...
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BITIMP(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BITIMP(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BITIMP(0x0010, "L/R Loop", KEYCODE_L) COREPORT_BIT(0x0040, "L/R Slingshot", KEYCODE_MINUS)
                COREPORT_BIT(0x0080, "Jet Bumper 1", KEYCODE_W) COREPORT_BITIMP(0x0100, "Jet Bumper 2", KEYCODE_E)
                    COREPORT_BITIMP(0x0200, "Jet Bumper 3", KEYCODE_R) COREPORT_BIT(0x0400, "Gear Up", KEYCODE_LALT)
                        COREPORT_BITIMP(0x0800, "Tunnel Shot", KEYCODE_T) COREPORT_BIT(0x1000, "L/R Inlane", KEYCODE_I)
                            COREPORT_BIT(0x2000, "R. Bank DT 1", KEYCODE_V)
                                COREPORT_BITIMP(0x4000, "R. Bank DT 2", KEYCODE_B)
                                    COREPORT_BIT(0x8000, "R. Bank DT 3", KEYCODE_N)
                                        ...
                                        PORT_START /* 1 */
    COREPORT_BIT(0x0001, "L. Bank DT 1", KEYCODE_Z) COREPORT_BIT(0x0002, "L. Bank DT 2", KEYCODE_X)
        COREPORT_BIT(0x0004, "L. Bank DT 3", KEYCODE_C) COREPORT_BIT(0x0008, "Red DT 1", KEYCODE_J)
            COREPORT_BIT(0x0010, "Red DT 2", KEYCODE_K) COREPORT_BIT(0x0040, "Red DT 3", KEYCODE_L)
                COREPORT_BIT(0x0080, "Yellow DT 1", KEYCODE_F) COREPORT_BIT(0x0100, "Yellow DT 2", KEYCODE_G)
                    COREPORT_BIT(0x0200, "Yellow DT 3", KEYCODE_H) COREPORT_BITIMP(0x0400, "Green DT 1", KEYCODE_A)
                        COREPORT_BIT(0x0800, "Green DT 2", KEYCODE_S) COREPORT_BIT(0x1000, "Green DT 3", KEYCODE_D)
                            COREPORT_BIT(0x2000, "Drain", KEYCODE_Q)
                                COREPORT_BIT(0x4000, "Upper Left Loop", KEYCODE_U) WPC_INPUT_PORTS_END
...
static sim_tState gw_stateDef[] = {
    ...
    {"Ramp Diverter", 1, 0, 0, stLRampExit, 2, sDiverterLo, stSuperCh1,
     SIM_STNOTEXCL}, /*If Diverter firing, keep ball in Supercharger Loop, otherwise to Left Ramp Made!*/
    ...
    {"SuperCharger -", 1, swOptoLoopMade, 0, stLRampDiv, 0, sDiverterLo, stSuperCh1,
     1}, /*If Lo Diverter is Firing.. Keep Ball in Supercharger Loop*/
...
static int
gw_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
...
static sim_tInportData gw_inportData[] = {
    {0, 0x0005, stLRampEnt},  {0, 0x0006, stLLoopUp},   {0, 0x0009, stLOut},
    {0, 0x000a, stROut},      {0, 0x0011, stLLoopUp},   {0, 0x0012, stRLoopUp},
    {0, 0x0041, stLSling},    {0, 0x0042, stRSling},    {0, 0x0080, stJet1},
    {0, 0x0100, stJet3},      {0, 0x0200, stJet2},      {0, 0x0400, swGearHi, SIM_STANY},
    {0, 0x0800, stTunnel},    {0, 0x1001, stLIn},       {0, 0x1002, stRIn},
    {0, 0x2000, stRBankTop},  {0, 0x4000, stRBankMid},  {0, 0x8000, stRBankBot},
    {1, 0x0001, stLBankTop},  {1, 0x0002, stLBankMid},  {1, 0x0004, stLBankBot},
    {1, 0x0008, stTopGreen},  {1, 0x0010, stMidGreen},  {1, 0x0040, stBotGreen},
    {1, 0x0080, stTopYellow}, {1, 0x0100, stMidYellow}, {1, 0x0200, stBotYellow},
    {1, 0x0400, stTopRed},    {1, 0x0800, stMidRed},    {1, 0x1000, stBotRed},
    {1, 0x2000, stDrain},     {1, 0x4000, stULoop1},    {0}};

/*--------------------
  Drawing information
 ---------------------*/
static core_tLampDisplay gw_lampPos = {{0, 0},   /* top left */
                                       {39, 29}, /* size */
                                       {{1, {{34, 10, ORANGE}}},
                                        {1, {{35, 12, ORANGE}}},
                                        {1, {{35, 14, ORANGE}}},
...
static void
gw_drawMech(BMTYPE** line) {
    core_textOutf(30, 0, BLACK, "Gear Shifter: %-6s",
                  core_getSw(swGearLo) ? "Down  " : (core_getSw(swGearHi) ? "Up    " : "Middle"));
    core_textOutf(30, 10, BLACK, "Up/Down Ramp: %-6s", (locals.rampPos == UP) ? "Up" : "Down");
}

static void
gw_drawStatic(BMTYPE** line) {
...
WPCS_SOUNDROM8xx("u18snd", CRC(37bbe485) SHA1(e6b7ccef250db0c801e3dd8ebf93522b466ca1ec)) WPC_ROMEND
    ...
    WPC_ROMSTART(gw, pb, "u6-p-b.rom", 0x80000, CRC(c2c76ce7) SHA1(bc5874f8012feaa92230415067b56f56aae109bc))
        WPCS_SOUNDROM8xx("u18-sp1.rom", CRC(fc5a5ff6) SHA1(bbe810135e05f81d1399ee0cb490ee93d6f9bb03)) WPC_ROMEND
...
/---------------*/
    CORE_GAMEDEF(gw, l5, "Getaway: High Speed II, The (L-5)", 1992, "Williams", wpc_mFliptronS, 0)
        CORE_CLONEDEF(gw, d5, l5, "Getaway: High Speed II, The (D-5 LED Ghost Fix)", 1992, "Williams", wpc_mFliptronS,
                      0) CORE_CLONEDEF(gw, l5c, l5, "Getaway: High Speed II, The (L-5C Competition MOD)", 2017,
                                       "Williams", wpc_mFliptronS, 0) //rev. L-5 patch bc43
    CORE_CLONEDEF(gw, pb, l5, "Getaway: High Speed II, The (P-B Prototype)", 1992, "Williams", wpc_mFliptronS, 0)
        CORE_CLONEDEF(gw, pc, l5, "Getaway: High Speed II, The (P-C Prototype)", 1992, "Williams", wpc_mFliptronS, 0)
            CORE_CLONEDEF(gw, pd, l5, "Getaway: High Speed II, The (P-D LED Ghost Fix)", 1992, "Williams",
                          wpc_mFliptronS, 0) CORE_CLONEDEF(gw, p7, l5, "Getaway: High Speed II, The (P-7 Prototype)",
                                                           1992, "Williams", wpc_mFliptronS, 0)
                CORE_CLONEDEF(gw, p8, l5, "Getaway: High Speed II, The (P-8 LED Ghost Fix)", 1992, "Williams",
                              wpc_mFliptronS, 0)
                    CORE_CLONEDEF(gw, l1, l5, "Getaway: High Speed II, The (L-1)", 1992, "Williams", wpc_mFliptronS, 0)
                        CORE_CLONEDEF(gw, d1, l5, "Getaway: High Speed II, The (D-1 LED Ghost Fix)", 1992, "Williams",
                                      wpc_mFliptronS, 0) CORE_CLONEDEF(gw, l2, l5, "Getaway: High Speed II, The (L-2)",
                                                                       1992, "Williams", wpc_mFliptronS, 0)
                            CORE_CLONEDEF(gw, d2, l5, "Getaway: High Speed II, The (D-2 LED Ghost Fix)", 1992,
                                          "Williams", wpc_mFliptronS, 0)
                                CORE_CLONEDEF(gw, l3, l5, "Getaway: High Speed II, The (L-3)", 1992, "Williams",
                                              wpc_mFliptronS, 0)
                                    CORE_CLONEDEF(gw, d3, l5, "Getaway: High Speed II, The (D-3 LED Ghost Fix)", 1992,
                                                  "Williams", wpc_mFliptronS, 0)
...
static core_tGameData gwGameData = {
    ...
    {FLIP_SW(FLIP_L | FLIP_UR) | FLIP_SOL(FLIP_L | FLIP_UR), /* Which switches are the flippers */
     0, 0, 0, 0, 0, 0, 0, NULL, gw_handleMech, NULL, gw_drawMech, &gw_lampPos, NULL},
    &gwSimData,
    {
        {0}, /* no serial number */
