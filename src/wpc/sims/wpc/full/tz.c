...
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BITIMP(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BITIMP(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BITIMP(0x0010, "L/R Spiral", KEYCODE_S) COREPORT_BIT(0x0020, "L/R Mini PF", KEYCODE_M)
                COREPORT_BIT(0x0040, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0080, "L/R Inlane", KEYCODE_I)
                    COREPORT_BIT(0x0100, "Power Payoff", KEYCODE_Y) COREPORT_BIT(0x0200, "Jet 1", KEYCODE_V)
                        COREPORT_BIT(0x0400, "Jet 2", KEYCODE_B) COREPORT_BIT(0x0800, "Jet 3", KEYCODE_N)
                            COREPORT_BITIMP(0x1000, "Mini PF Top", KEYCODE_T)
                                COREPORT_BITIMP(0x2000, "Mini PF Exit", KEYCODE_E)
                                    COREPORT_BITIMP(0x4000, "Clock Lane", KEYCODE_U)
                                        COREPORT_BIT(0x8000, "Buy-in", KEYCODE_2) PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Greed 1", KEYCODE_A) COREPORT_BIT(0x0002, "Greed 2", KEYCODE_F)
        COREPORT_BIT(0x0004, "Greed 3", KEYCODE_G) COREPORT_BIT(0x0008, "Greed 4", KEYCODE_J)
            COREPORT_BIT(0x0010, "Greed 5", KEYCODE_K) COREPORT_BIT(0x0020, "Greed 6", KEYCODE_COLON)
                COREPORT_BIT(0x0040, "Greed 7", KEYCODE_QUOTE) COREPORT_BIT(0x0080, "Clock Target", KEYCODE_W)
                    COREPORT_BIT(0x0100, "Left Inner Inlane", KEYCODE_I)
                        COREPORT_BITIMP(0x0200, "Slot Machine", KEYCODE_S) COREPORT_BITIMP(0x0400, "Camera", KEYCODE_C)
                            COREPORT_BITIMP(0x0800, "Piano", KEYCODE_Z) COREPORT_BITIMP(0x1000, "Dead End", KEYCODE_D)
                                COREPORT_BITIMP(0x2000, "Lock", KEYCODE_L) COREPORT_BITIMP(0x4000, "Rocket", KEYCODE_R)
                                    COREPORT_BIT(0x8000, "Hitchhiker", KEYCODE_H) PORT_START /* 2 */
    COREPORT_BITIMP(0x8000, "Drain", KEYCODE_Q) COREPORT_DIPNAME(0x0007, 0x05, "Powerball") COREPORT_DIPSET(0x0000, "1")
        COREPORT_DIPSET(0x0001, "2") COREPORT_DIPSET(0x0002, "3") COREPORT_DIPSET(0x0003, "4")
            COREPORT_DIPSET(0x0004, "5") COREPORT_DIPSET(0x0005, "6") COREPORT_DIPSET(0x0006, "7")
                COREPORT_DIPNAME(0x0008, 0x00, "Third Magnet") COREPORT_DIPSET(0x0000, DEF_STR(No))
                    COREPORT_DIPSET(0x0008, DEF_STR(Yes)) COREPORT_DIPNAME(0x0010, 0x00, "Big Kick")
                        COREPORT_DIPSET(0x0000, DEF_STR(No)) COREPORT_DIPSET(0x0010, DEF_STR(Yes))
                            COREPORT_DIPNAME(0x0020, 0x00, "Clock Lane") COREPORT_DIPSET(0x0000, DEF_STR(No))
                                COREPORT_DIPSET(0x0020, DEF_STR(Yes)) WPC_INPUT_PORTS_END
...
static int
tz_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
        case stRTrough:
            if (core_getSol(sBallRel)) {
                core_setSw(swTProxi, FALSE);
                return setState(stShooterDiv, 6);
            } else {
                core_setSw(swTProxi, !(ball->type & TZ_POWERBALL));
                break;
            }
        case stRLMagDn:
            if ((ball->type & TZ_POWERBALL) || !core_getSol(sRLMag))
                return setFree(10);
            else
                break;
        case stRLMagUp:
            if ((ball->type & TZ_POWERBALL) || !core_getSol(sRLMag)) {
                if (inports[2] & 0x0008)
                    return setState(stRUMagUp, 5);
                else
                    return setState(stSpiral, 10);
            } else {
                forceState(stRLMagDn);
                break;
            } /* change direction */
        case stRUMagDn:
            if ((ball->type & TZ_POWERBALL) || !core_getSol(sRUMag))
                return setState(stRLMagDn, 10);
            else
                break;
        case stRUMagUp:
            if ((ball->type & TZ_POWERBALL) || !core_getSol(sRUMag))
                return setState(stSpiral, 5);
            else {
                forceState(stRLMagDn);
                break;
            } /* change direction */
        case stLMagDn:
            if ((ball->type & TZ_POWERBALL) || !core_getSol(sLMag))
                return setFree(10);
            else
                break;
        case stLMagUp:
        ...
        case stBallLane:
            if (ball->speed > 10)
                ...
                return setState(stShooter, 5);
        case stSkillR:
            if ((ball->speed > 13) && !core_getSw(swSkillO))
                return setState(stSkillO, 1);
            else {
                ball->speed = -1;
                return setState(stRocket, 20);
            }
        case stSkillO:
            if ((ball->speed > 17) && !core_getSw(swSkillY))
                return setState(stSkillY, 1);
            else {
                ball->speed = -1;
                return setState(stSkillR, 2);
            }
        case stSkillY:
            if (ball->speed > 20) {
                ball->speed = -1;
                return setState(stSlot, 5);
            } else {
                ball->speed = -1;
                return setState(stSkillO, 2);
            }
            break;
        case stBigKick:
            if (core_getSol(sAutoFire))
                return setState(stRLMagUp, 10);
            else if (!core_getSw(swAutoFire))
                return setState(stAutoFire, 2);
            break;
    }
    return 0;
}
...
static void
tz_initSim(sim_tBallStatus* balls, int* inports, int noOfBalls) {
...
/----------------------------*/
static sim_tInportData tz_inportData[] = {{0, 0x0005, stLRampEnt},
                                          {0, 0x0006, stRRampEnt},
                                          {0, 0x0011, stLMagUp},
                                          {0, 0x0012, stRLMagUp},
                                          {0, 0x0009, stLOut},
...
/------------------*/
#define TZ_SOUND                                                                                                       \
    WPCS_SOUNDROM882("tzu18_l2.rom", CRC(66575ec2) SHA1(deceb56324ee9785946f5771f8cfbaf1b1d2c8bc), "tzu15_l2.rom",     \
                     CRC(389d2442) SHA1(58a4bc7cc7a28b47c75d5c9bbf14abf34bd7a9e3), "tzu14_l2.rom",                     \
                     CRC(5a67bd56) SHA1(98669fbfdc5793bcf09fe72c231e2b4fa2524cc5))
...
static core_tGameData tzGameData = {
    GEN_WPCFLIPTRON,
    wpc_dispDMD,                                                                /* generation */
    {FLIP_SW(FLIP_L | FLIP_U) | FLIP_SOL(FLIP_L | FLIP_U), 1, 0, 9, 0, 0, 0, 0, // 1 switch column, 9 solenoids
     tz_getSol, tz_handleMech, tz_getMech, tz_drawMech, &tz_lampPos, NULL},
    &tzSimData,
    {
        "",
...
static mech_tInitData mechClock = {sClockRev,
                                   sClockFwd,
                                   MECH_TWODIRSOL | MECH_FAST,
                                   TZ_CLOCKTICKS * 48 * 8,
                                   720,
...
init_tz(void) {
    ...
    install_mem_read_handler(WPC_CPUNO, WPC_SWROWREAD + WPC_BASE, WPC_SWROWREAD + WPC_BASE, tz_swRowRead);
