...
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BITIMP(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BIT(0x0008, "L/R Inlane", KEYCODE_I)
            COREPORT_BITIMP(0x0010, "L/R Outlane", KEYCODE_O) COREPORT_BITIMP(0x0040, "L/R Loop", KEYCODE_L)
                COREPORT_BIT(0x0080, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0100, "Jet Bumper 1", KEYCODE_H)
                    COREPORT_BITIMP(0x0200, "Jet Bumper 2", KEYCODE_J)
                        COREPORT_BITIMP(0x0400, "Jet Bumper 3", KEYCODE_K) COREPORT_BITIMP(0x0800, "LIE 'L'", KEYCODE_Z)
                            COREPORT_BITIMP(0x1000, "LIE 'I'", KEYCODE_X) COREPORT_BIT(0x2000, "LIE 'E'", KEYCODE_C)
                                COREPORT_BIT(0x4000, "R. Bank DT 1", KEYCODE_N)
                                    COREPORT_BITIMP(0x8000, "R. Bank DT 2", KEYCODE_M)

                                        PORT_START /* 1 */
    COREPORT_BIT(0x0001, "L. Bank DT 1", KEYCODE_V) COREPORT_BIT(0x0002, "L. Bank DT 2", KEYCODE_B)
        COREPORT_BIT(0x0004, "Drain", KEYCODE_Q) COREPORT_BIT(0x0008, "Caster's Club", KEYCODE_T)
            COREPORT_BIT(0x0010, "Extra Ball", KEYCODE_E)
                COREPORT_BIT(0x0040, "Captive Ball", KEYCODE_W) WPC_INPUT_PORTS_END
...
static int
ft_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
...
static sim_tInportData ft_inportData[] = {{0, 0x0005, stLRampEnt},
                                          {0, 0x0006, stRRampEnt},
                                          {0, 0x0009, stLIn},
                                          {0, 0x000a, stRIn},
                                          {0, 0x0011, stLOut},
...
WPCS_SOUNDROM8xx("ft_u18.l1", CRC(48d2760a) SHA1(701b0bbb68f99332493ee1276e5a1cef5c85d499)) WPC_ROMEND
...
/---------------*/
    CORE_GAMEDEF(ft, l5, "Fish Tales (L-5)", 1992, "Williams", wpc_mFliptronS, 0)
        CORE_CLONEDEF(ft, l5p, l5, "Fish Tales (L-5 Text size patch)", 1992, "Williams", wpc_mFliptronS, 0)
            CORE_CLONEDEF(ft, d6, l5, "Fish Tales (D-6 Text size patch LED Ghost Fix)", 2015, "Inkochnito / Williams",
                          wpc_mFliptronS, 0) CORE_CLONEDEF(ft, d5, l5, "Fish Tales (D-5 LED Ghost Fix)", 1992,
                    ...
                    CORE_CLONEDEF(ft, l4, l5, "Fish Tales (L-4)", 1992, "Williams", wpc_mFliptronS, 0)
                        CORE_CLONEDEF(ft, p2, l5, "Fish Tales (P-2 Prototype)", 1992, "Williams", wpc_mFliptronS, 0)
                            CORE_CLONEDEF(ft, p4, l5, "Fish Tales (P-4 Prototype)", 1992, "Williams", wpc_mFliptronS, 0)
                                CORE_CLONEDEF(ft, p5, l5, "Fish Tales (P-5 LED Ghost Fix)", 1992, "Williams",
                                              wpc_mFliptronS, 0)
...
static core_tGameData ftGameData = {
    ...
    {FLIP_SW(FLIP_L | FLIP_UR) | FLIP_SOL(FLIP_L | FLIP_UR), /* Which switches are the flippers */
     0, 0, 3, 0, 0, 0, 0, ft_getSol, ft_handleMech, ft_getMech, ft_drawMech, NULL, NULL},
    &ftSimData,
    {
        {0}, /* no serial number */
...
static int
ft_getMech(int mechNo) {
    switch (mechNo) {
            //Return Reel Position
        case 0:
            return locals.reelPos;
