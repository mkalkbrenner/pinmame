...
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BIT(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BIT(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BIT(0x0010, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0020, "L1/L2/R Inlane", KEYCODE_I)
                COREPORT_BIT(0x0040, "L/R Loop", KEYCODE_L) COREPORT_BIT(0x0080, "Putt Out", KEYCODE_T)
                    COREPORT_BIT(0x0100, "Centre Ramp", KEYCODE_R) COREPORT_BIT(0x0200, "Golf Cart", KEYCODE_Y)
                        COREPORT_BIT(0x1000, "Hole In One", KEYCODE_STOP) COREPORT_BIT(0x4000, "Jet Advance", KEYCODE_A)
                            COREPORT_BIT(0x8000, "Drain", KEYCODE_Q)

                                PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Top Jet", KEYCODE_S) COREPORT_BIT(0x0002, "Middle Jet", KEYCODE_D)
        COREPORT_BIT(0x0004, "Bottom Jet", KEYCODE_F) COREPORT_BIT(0x0008, "(K)ick", KEYCODE_G)
            COREPORT_BIT(0x0010, "K(i)ck", KEYCODE_H) COREPORT_BIT(0x0020, "Ki(c)k", KEYCODE_J)
                COREPORT_BIT(0x0040, "Kic(k)", KEYCODE_K) COREPORT_BIT(0x0100, "Advance Trap", KEYCODE_Z)
                    COREPORT_BIT(0x0200, "Sand Trap", KEYCODE_X) COREPORT_BIT(0x0400, "Captive Ball", KEYCODE_C)
                        COREPORT_BIT(0x0800, "Advance Kick", KEYCODE_V) COREPORT_BIT(0x1000, "Top Skill", KEYCODE_B)
...
static sim_tState ngg_stateDef[] = {{"Not Installed", 0, 0, 0, stDrain, 0, 0, 0, SIM_STNOTEXCL},
                                    {"Moving"},
                                    ...
                                    {"Sand Trap", 1, swSandTrap, sLeftEject, stkiCk,
                                     50}, /* Sandtrap always hits Ki(c)k target (well it does on my machine!) */
...
static void
ngg_initSim(sim_tBallStatus* balls, int* inports, int noOfBalls) {
    core_setSw(swOuterWheel, 0);
    core_setSw(swInnerWheel, 0);
}

static int
ngg_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
        ...
        case stCRampTest:
            ...
            else if (locals.goferpos[0] == DOWN)
        ...
        case stRRampTest:
            ...
            else if (locals.goferpos[1] == DOWN)
...
static core_tLampDisplay ngg_lampPos = {{0, 0},   /* top left */
                                        {39, 29}, /* size */
                                        {
...
/**********************************
 Wheel Position
 *********************************/
static const char* WheelText[] = {" 0 Lite Xtra Ball", " 1 Cart Attack   ", " 2 Gofer Attack! ", " 3 Lite Q Jackpot",
...
DCS_SOUNDROM5xm("nggsndl1.s2", CRC(6263866d) SHA1(c72a2f176aa24e91ecafe1704affd16b86d671e2), "nggsndl1.s3",
                CRC(6871b20d) SHA1(0109c02282806016a6b22f7dfe3ac964931ba609), "nggsndl1.s4",
                CRC(86ed8f5a) SHA1(231f6313adff89ef4cec0d9f25b13e69ea96213d), "nggsndl1.s5",
                CRC(ea2062f0) SHA1(f8e45c1fcc6b8677a0745a5d83ca93b77fbde752), "nggsndl1.s6",
                CRC(b1b8b514) SHA1(e16651bcb2eae747987dc3c13a5dc20a33c0a1f8)) WPC_ROMEND
    ...
    WPC_ROMSTART(ngg, p06, "ngg0_6.rom", 0x80000, CRC(e0e0d331) SHA1(e1b91eccec6034bcd2029c15596aa0b129c9e53f))
        DCS_SOUNDROM5xm("ngg_s2.0_2", CRC(dde128d5) SHA1(214ee807d2323ecb407a3d116b038e15c60e5580), "nggsndl1.s3",
                        CRC(6871b20d) SHA1(0109c02282806016a6b22f7dfe3ac964931ba609), "nggsndl1.s4",
                        CRC(86ed8f5a) SHA1(231f6313adff89ef4cec0d9f25b13e69ea96213d), "nggsndl1.s5",
                        CRC(ea2062f0) SHA1(f8e45c1fcc6b8677a0745a5d83ca93b77fbde752), "nggsndl1.s6",
                        CRC(b1b8b514) SHA1(e16651bcb2eae747987dc3c13a5dc20a33c0a1f8)) WPC_ROMEND
...
static core_tGameData nggGameData = {
    GEN_WPC95,
    wpc_dispDMD,
    {FLIP_SW(FLIP_L | FLIP_U) | FLIP_SOL(FLIP_L | FLIP_UR), 0, 0, 8, 0, 0, 1, 0, ngg_getSol, ngg_handleMech,
     ngg_getMech, ngg_drawMech, &ngg_lampPos, NULL},
    &nggSimData,
    {
...
static mech_tInitData mechnggWheel = {
    -sWheelRev,
    -sWheelFwd,
    MECH_CIRCLE | MECH_LINEAR | MECH_TWODIRSOL | MECH_FAST | MECH_ACC(48) | MECH_RET(6),
    12 * NGG_WHEELRES,
    96 * NGG_WHEELRES,
...
static WRITE_HANDLER(ngg_wpc_w) {
    ...
    if (offset == WPC_FLIPPERCOIL95) {
        if (lastFlip & data)
            wpc_w(WPC_FLIPPERCOIL95, data);
        if (!lastFlip & !data)
            wpc_w(WPC_FLIPPERCOIL95, 0);
        lastFlip = data;
    } else
        wpc_w(offset, data);
...
static int
ngg_getSol(int solNo) {
    return wpc_data[WPC_EXTBOARD2] & (1 << (solNo - CORE_CUSTSOLNO(1)));
}

static int
ngg_getMech(int mechNo) {
    switch (mechNo) {
        case 0:
            return NGG_WHEEL_POS;
        case 1:
            return locals.goferpos[0] | (locals.goferpos[1] << 1);
        case 2:
            return locals.ramppos[0] | (locals.ramppos[1] << 1);
        case 3:
            return locals.slampos;
        case 4:
            return mech_getPos(0) * 360 / (64 * NGG_WHEELRES);
        case 5:
            return (mech_getSpeed(0) * 100) / NGG_SPEED;
    }
    return 0;
}
