    ...
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BIT(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BIT(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BIT(0x0010, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0020, "L/R Inlane", KEYCODE_I)
                COREPORT_BIT(0x0040, "TopL Rollover", KEYCODE_W) COREPORT_BIT(0x0100, "TopM Rollover", KEYCODE_E)
                    COREPORT_BIT(0x0200, "TopR Rollover", KEYCODE_R) COREPORT_BIT(0x0400, "TopL Popper", KEYCODE_T)
                        COREPORT_BIT(0x0800, "Left Jet", KEYCODE_Y) COREPORT_BIT(0x1000, "Right Jet", KEYCODE_U)
                            COREPORT_BIT(0x2000, "Bottom Jet", KEYCODE_I) COREPORT_BIT(0x4000, "BR Hole", KEYCODE_J)
                                COREPORT_BIT(0x8000, "LeftLoop", KEYCODE_L)

                                    PORT_START /* 1 */
    COREPORT_BIT(0x0001, "LBank B", KEYCODE_A) COREPORT_BIT(0x0002, "LBank M", KEYCODE_S)
        COREPORT_BIT(0x0004, "LBank T", KEYCODE_D) COREPORT_BIT(0x0008, "MBank L", KEYCODE_F)
            COREPORT_BIT(0x0010, "MBank M", KEYCODE_G) COREPORT_BIT(0x0020, "MBank R", KEYCODE_H)
                COREPORT_BIT(0x0040, "Mistery", KEYCODE_M) COREPORT_BIT(0x0080, "ExitMagnet", KEYCODE_ENTER)
...
static int
drac_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
...
*/
    ...
    /*Channel #2*/
    {sShooter, 2, SAM_SOLENOID},
    {sLeftSling, 2, SAM_LSLING},
    {sRightSling, 2, SAM_RSLING},
    {sCoffinPopper, 2, SAM_POPPER},

    /*Channel #3*/
    {sCastlePopper, 3, SAM_POPPER},
    {sRRampUp, 3, SAM_FLAPOPEN},
    {sRRampDown, 3, SAM_FLAPCLOSE},
    {sLDrop, 3, SAM_SOLENOID_ON},
    {sCastleRelease, 3, SAM_SOLENOID_ON},
    {sCastleRelease, 3, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},
    {-1}};
...
WPCS_SOUNDROM888("dracsnd.u18", CRC(372ffb90) SHA1(89979670869c565d3ab86abbce462e2f935a566b), "dracsnd.u15",
                 CRC(77b5abe2) SHA1(e5622ae9ae0c1a0be886a1e5dc25b5a42c00c2ae), "dracsnd.u14",
                 CRC(5137aaf5) SHA1(e6ee924e7e4718db0f7f315f2a6843e6f90afb41))
WPC_ROMEND
...
static sim_tSimData dracSimData = {
    ...
    {stTrough2, stTrough3, stTrough4, stTrough1, stDrain, stDrain,
...
static core_tGameData dracGameData = {
    GEN_WPCFLIPTRON,
    wpc_dispDMD,
    {FLIP_SW(FLIP_L | FLIP_U) | FLIP_SOL(FLIP_L), 0, 0, 0, 0, 0, 0, 0, NULL, drac_handleMech, NULL, drac_drawMech,
...
drac_handleMech(int mech) {
    ...
    if (mech & 0x08) {
       ...
       the fake diverter switch to 0 */
        if (core_getSw(swLRampDivMade) || core_getSw(swCastle3) || core_getSw(swCastle2) || core_getSw(swCastle3)
