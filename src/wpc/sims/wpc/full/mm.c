...
/*-----------------------
 ...
 ------------------------*/
static struct { int drawbridgePos; } locals;
        ...
        COREPORT_BIT(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BIT(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BIT(0x0010, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0020, "L/R Inlane", KEYCODE_I)
                COREPORT_BIT(0x0040, "Left Jet", KEYCODE_W) COREPORT_BIT(0x0080, "Right Jet", KEYCODE_E)
                    COREPORT_BIT(0x0100, "Bottom Jet", KEYCODE_R) COREPORT_BIT(0x0200, "L/R Loop", KEYCODE_L)
                        COREPORT_BIT(0x0400, "Eject", KEYCODE_M) COREPORT_BIT(0x0800, "L/R Troll", KEYCODE_T)
                            COREPORT_BIT(0x1000, "L/R Troll Targets", KEYCODE_D)
                                COREPORT_BIT(0x2000, "L/R Bonus X", KEYCODE_X) COREPORT_BIT(0x4000, "Lock", KEYCODE_L)
                                    COREPORT_BIT(0x8000, "Drain", KEYCODE_Q)

                                        PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Castle", KEYCODE_C) COREPORT_BIT(0x0100, "Catapult", KEYCODE_Z)
        COREPORT_BIT(0x1000, "Top Merlin Tgt", KEYCODE_V) COREPORT_BIT(0x2000, "Mid Merlin Tgt", KEYCODE_B)
...
static sim_tState mm_stateDef[] = {{"Not Installed", 0, 0, 0, stDrain, 0, 0, 0, SIM_STNOTEXCL},
                                   {"Moving"},
...
static int
mm_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
        ...
        case stCastle:
            ...
            else if (core_getSol(sCastleGateH))
...
static void
mm_initSim(sim_tBallStatus* balls, int* inports, int noOfBalls) {
...
mm_drawMech(BMTYPE** line) {
    ...
    core_textOutf(30, 20, BLACK, "L/R Troll  : %-4s/%-4s", core_getSol(sLTrollH) ? " Up " : "Down",
                  core_getSol(sRTrollH) ? " Up " : "Down");
}

/* Help */

static void
mm_drawStatic(BMTYPE** line) {
...
static wpc_tSamSolMap mm_samsolmap[] = {
    ...
    {sRightEject, 1, SAM_SOLENOID_ON},
    {sTowerDivP, 1, SAM_DIVERTER},
    {sTowerDivH, 1, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},

    /*Channel #2*/
    {sLaunch, 2, SAM_SOLENOID},
    {sLeftSling, 2, SAM_LSLING},
    {sRightSling, 2, SAM_RSLING},
    {sCatapult, 2, SAM_POPPER},

    /*Channel #3*/
    {sLeftPopper, 3, SAM_POPPER},
    {sCastle, 3, SAM_SOLENOID},
    {sCastleGateP, 3, SAM_FLAPOPEN},
    {sCastleGateH, 3, SAM_FLAPCLOSE, WPCSAM_F_ONOFF},

    /*Channel #4*/
    {sLTrollP, 4, SAM_SOLENOID},
    {sLTrollH, 4, SAM_FLAPCLOSE, WPCSAM_F_ONOFF},
    {sRTrollP, 4, SAM_SOLENOID},
    {sRTrollH, 4, SAM_FLAPCLOSE, WPCSAM_F_ONOFF},
    {sTowerLockPost, 4, SAM_SOLENOID_ON},
    {sTowerLockPost, 4, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},

    /*Channel #5*/
    {sLeftGate, 5, SAM_SOLENOID_ON},
    {sLeftGate, 5, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},
    {sRightGate, 5, SAM_SOLENOID_ON},
    {sRightGate, 5, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},
    {sDBMotor, 5, SAM_MOTOR_1, WPCSAM_F_CONT},
    {-1}};
...
/------------------*/
#define MM_SOUND                                                                                                       \
    DCS_SOUNDROM5xm("mm_s2.1_0", CRC(c55c3b71) SHA1(95febbf16645dd897bdd459ccad9501d2457d1f1), "mm_sav3.rom",          \
                    CRC(ed1be570) SHA1(ead4c4f89d63ee0b46d8a8bcd8650d506542d1ee), "mm_sav4.rom",                       \
                    CRC(9c89eacf) SHA1(594a2aa81e34658862a9b7f0a83cf514182f2a2d), "mm_sav5.rom",                       \
                    CRC(45089e30) SHA1(e83492109c59e8a2f1ba9e1f793788b97d150a9b), "mm_sav6.rom",                       \
                    CRC(439d55f2) SHA1(d80e7268223157d864674261d140322634fb3bc2))

WPC_ROMSTART(mm, 109, "mm_1_09.bin", 0x100000, CRC(9bac4d0c) SHA1(92cbe21802e1a77feff77b78f4dbbdbffb7b14bc))
MM_SOUND WPC_ROMEND WPC_ROMSTART(mm, 109b, "mm_109b.bin", 0x100000,
                                 CRC(4eaab86a) SHA1(694cbb1154e7374275becfbe4f743fb8d31df8fb)) MM_SOUND WPC_ROMEND
    WPC_ROMSTART(mm, 109c, "mm_1_09c.bin", 0x100000,
                 CRC(d9e5189f) SHA1(fc01855c139d408559605fe9932236250cd566a8)) MM_SOUND WPC_ROMEND
    WPC_ROMSTART(mm, 10, "mm_g11.1_0", 0x080000,
                 CRC(6bd735c6) SHA1(3922df00e785610837230d5d9c24b9e082aa6fb6)) MM_SOUND WPC_ROMEND
    WPC_ROMSTART(mm, 10u, "mmu_g11.1_0", 0x080000,
                 CRC(265e6192) SHA1(bd9606df5fb85b2048a07db0927e4a856c344276)) MM_SOUND WPC_ROMEND
    WPC_ROMSTART(mm, 10pfx, "mmpfx_g11.1_0", 0x080000,
                 CRC(4b3ab8d4) SHA1(f6841be6fde8ab1888c07bb2dbf90be8439cbd56)) MM_SOUND WPC_ROMEND

    WPC_ROMSTART(mm, 05, "g11-050.rom", 0x080000, CRC(d211ad16) SHA1(539fb0c4ca6fe19ac6140f5792c5b7cd51f737ce))
        DCS_SOUNDROM5xm("s2-020.rom", CRC(ee009ce4) SHA1(36843b2f1a07cf1e23bdff9b7347ceeca7e915bc), "mm_sav3.rom",
                        CRC(ed1be570) SHA1(ead4c4f89d63ee0b46d8a8bcd8650d506542d1ee), "mm_sav4.rom",
                        CRC(9c89eacf) SHA1(594a2aa81e34658862a9b7f0a83cf514182f2a2d), "mm_sav5.rom",
                        CRC(45089e30) SHA1(e83492109c59e8a2f1ba9e1f793788b97d150a9b), "mm_sav6.rom",
                        CRC(439d55f2) SHA1(d80e7268223157d864674261d140322634fb3bc2)) WPC_ROMEND

    /*--------------
/  Game drivers
/---------------*/
    CORE_GAMEDEF(mm, 10, "Medieval Madness (1.0)", 1997, "Williams", wpc_m95S, 0)
        CORE_CLONEDEF(mm, 10u, 10, "Medieval Madness (1.0 Ultrapin)", 2006, "Global VR", wpc_m95S,
                      0) // modded text (child/cow taken, website, phone, etc)
    CORE_CLONEDEF(mm, 10pfx, 10, "Medieval Madness (1.0 Pinball FX)", 2018, "Zen Studios", wpc_m95S,
                  0) // modded text (child/cow taken)
    ...
    CORE_CLONEDEF(mm, 109c, 10, "Medieval Madness (1.09C Profanity)", 1999, "Williams", wpc_m95S,
                  0) // Home version w/ profanity speech
...
static core_tGameData mmGameData = {
    GEN_WPC95,
    wpc_dispDMD,
    {FLIP_SW(FLIP_L | FLIP_U) | FLIP_SOL(FLIP_L), 0, 0, 0, 0, 0, 1, 0, NULL, mm_handleMech, mm_getMech, mm_drawMech,
