    ...
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BITIMP(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BITIMP(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BITIMP(0x0010, "L/R Loop", KEYCODE_L) COREPORT_BIT(0x0040, "L/R Slingshot", KEYCODE_MINUS)
                COREPORT_BIT(0x0080, "L/R Inlane", KEYCODE_I) COREPORT_BITIMP(0x0100, "Vault", KEYCODE_V)
                    COREPORT_BITIMP(0x0200, "Swamp", KEYCODE_S) COREPORT_BITIMP(0x0400, "Chair", KEYCODE_C)
                        COREPORT_BITIMP(0x0800, "Thing", KEYCODE_T) COREPORT_BIT(0x1000, "Bookcase", KEYCODE_B)
                            COREPORT_BIT(0x2000, "Left Inner Inlane", KEYCODE_I)
                                COREPORT_BITIMP(0x4000, "Thing Hole", KEYCODE_H)
                                    COREPORT_BIT(0x8000, "Train Wreck", KEYCODE_W) PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Grave", KEYCODE_E) COREPORT_BIT(0x0002, "gRave", KEYCODE_Y)
        COREPORT_BIT(0x0004, "grAve", KEYCODE_U) COREPORT_BIT(0x0008, "It", KEYCODE_Z)
            COREPORT_BIT(0x0010, "Upper Swamp", KEYCODE_D) COREPORT_BIT(0x0020, "Center Swamp", KEYCODE_F)
                COREPORT_BIT(0x0040, "Lower Swamp", KEYCODE_G) COREPORT_BIT(0x0100, "Jet 1", KEYCODE_N)
                    COREPORT_BIT(0x0200, "Jet 2", KEYCODE_M) COREPORT_BIT(0x0400, "Jet 3", KEYCODE_COMMA)
                        COREPORT_BIT(0x0800, "Jet 4", KEYCODE_STOP) COREPORT_BIT(0x1000, "Jet 5", KEYCODE_SLASH)
                            COREPORT_BITIMP(0x2000, "Drain", KEYCODE_Q) COREPORT_BIT(0x4000, "Buy-in", KEYCODE_2)
                                COREPORT_BIT(0x8000, "Lock Kickout", KEYCODE_K) WPC_INPUT_PORTS_END
...
static sim_tState taf_stateDef[] = {{"Not Installed", 0, 0, 0, stDrain, 0, 0, 0, SIM_STNOTEXCL},
                                    {"Moving"},
...
#else
                                     {"Ball Shooter", 1, swShooter, sShooterRel, stBallLane, 10, 0, 0,
...
static int
taf_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
        case stBallLane:
            if (ball->speed > 5)
                return setState(stBallLane2, 10);
            else {
                ball->speed = -1;
        ...
        case stBumpLane:
            ball->custom = 0;
            return setState(stTFlip, 1);
        case stThHole:
            if (core_getSw(swThingUp) && core_getSol(sThMag))
                return setState(stThing, 5);
            if (core_getSol(sThHole))
                return setFree(10);
            break;
        case stTFlip:
            if (core_getSol(sULFlip)) {
                if (ball->custom < 1)
                    return setFree(5);
                if (ball->custom < 2)
                    return setState(stUSwamp, 10);
                if (ball->custom < 3)
                    return setState(stULock, 15);
                if (ball->custom < 4)
                    return setState(stCSwamp, 10);
                if (ball->custom < 5)
                    return setState(stLSwamp, 10);
            }
            if (ball->custom >= 5)
                return setFree(10);
            ball->custom += 1;
        ...
        case stBallLane2:
            if (ball->speed > 20) {
                if (ball->speed < 25)
                    return setState(stThHole, 5);
                else
                    return setState(stRLoopDn, 10);
            } else {
                ball->speed = -1;
                return setState(stBallLane, 10);
            }
        case stThing:
            if (!core_getSol(sThMag)) {
                if (core_getSw(swThingUp))
                    return setState(stThHole, 1);
                if (core_getSw(swThingDn))
                    return setState(stThKick, 10);
...
/----------------------------*/
static sim_tInportData taf_inportData[] = {{0, 0x0005, stLRampEnt},
                                           {0, 0x0006, stRRampEnt},
                                           {0, 0x0009, stLOut},
                                           {0, 0x000a, stROut},
                                           {0, 0x0011, stLLoopUp},
...
static wpc_tSamSolMap taf_SamSolMap[] = {
    ...
    /*Channel #2*/
    {sLKick, 2, SAM_POPPER},
    {sRampDiv, 2, SAM_SOLENOID},

    /*Channel #3*/
    {sThHole, 3, SAM_SOLENOID},
    {sThKick, 3, SAM_SOLENOID},
    {-1}};

static void
taf_drawMech(BMTYPE** line) {
    core_textOutf(50, 0, BLACK, "Book: %-6s",
                  core_getSw(swBookOpen) ? "Open" : (core_getSw(swBookClose) ? "Closed" : ""));
...
static core_tGameData tafGameData = {
    ...
    {FLIP_BUT(FLIP_L) | FLIP_SW(FLIP_L | FLIP_U) | FLIP_SOL(FLIP_L | FLIP_U), 0, 0, 0, 0, 0, 0, 0, NULL, taf_handleMech,
     taf_getMech, taf_drawMech, &taf_lampPos, taf_SamSolMap},
    &tafSimData,
    {
        "",
...
static void
taf_handleMech(int mech) {
    /*-- update bookcase switches --*/
    if (mech & 0x01) {
        if (core_getSol(sBookMotor))
            locals.bookPos = (locals.bookPos + 1) % TAF_BOOKTICKS;
        core_setSw(swBookOpen, locals.bookPos < 15);
        core_setSw(swBookClose, (locals.bookPos >= (TAF_BOOKTICKS / 2)) && (locals.bookPos < (TAF_BOOKTICKS / 2 + 15)));
    }
    /*-- update thing switches --*/
    if (mech & 0x02) {
        if (core_getSol(sThMotor))
            locals.thingPos = (locals.thingPos + 1) % TAF_THINGTICKS;
        core_setSw(swThingDn, locals.thingPos < 15);
        core_setSw(swThingUp,
                   (locals.thingPos >= (TAF_THINGTICKS / 2)) && (locals.thingPos < (TAF_THINGTICKS / 2 + 15)));
    }
}

static int
taf_getMech(int mechNo) {
