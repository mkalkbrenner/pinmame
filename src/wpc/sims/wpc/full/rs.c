...
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BIT(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BIT(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BIT(0x0010, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0020, "L/R Inlane", KEYCODE_I)
                COREPORT_BIT(0x0040, "Rt Outer Inlane", KEYCODE_I) COREPORT_BIT(0x0100, "Red's Mouth", KEYCODE_R)
    //    COREPORT_BIT(0x0200,"Ted's Mouth",		KEYCODE_T)
    COREPORT_BIT(0x0400, "Blast Zone Tgts", KEYCODE_Z) COREPORT_BIT(0x0800, "Right Jet", KEYCODE_X)
        COREPORT_BIT(0x1000, "Left Jet", KEYCODE_C) COREPORT_BIT(0x2000, "Top Jet", KEYCODE_V)
            COREPORT_BIT(0x4000, "Red Standup L", KEYCODE_J) COREPORT_BIT(0x8000, "Red Standup U", KEYCODE_K)

                PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Upper Right Loop", KEYCODE_U) COREPORT_BIT(0x0002, "Start City", KEYCODE_S)
        COREPORT_BIT(0x0004, "White Standup", KEYCODE_D) COREPORT_BIT(0x0008, "Red Standup", KEYCODE_F)
            COREPORT_BIT(0x0010, "Yellow Standup", KEYCODE_G) COREPORT_BIT(0x0020, "Orange Standup", KEYCODE_N)
                COREPORT_BIT(0x0040, "Blast Zone Hole", KEYCODE_H) COREPORT_BIT(0x0100, "Lock Hole", KEYCODE_L)
                    COREPORT_BIT(0x0200, "Bulldozer", KEYCODE_B) COREPORT_BIT(0x0400, "Left Loop", KEYCODE_M)
                        COREPORT_BIT(0x0800, "Radio Riot Tgts", KEYCODE_W)
                            COREPORT_BIT(0x1000, "Buy-In Button", KEYCODE_E) COREPORT_BIT(0x2000, "Drain", KEYCODE_Q)
...
static int
rs_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
...
/*--------------------
  Drawing information
  --------------------*/
static core_tLampDisplay rs_lampPos = {{0, 0},   /* top left */
                                       {39, 29}, /* size */
                                       {{1, {{25, 12, YELLOW}}},
                                        {1, {{26, 13, YELLOW}}},
                                        {1, {{24, 13, WHITE}}},
                                        ...
                                        {1, {{39, 14, ORANGE}}},
                                        {1, {{30, 28, RED}}},
                                        {3, {{23, 24, WHITE}, {24, 25, WHITE}, {25, 26, WHITE}}},
                                        {1, {{17, 25, WHITE}}},
                                        {1, {{13, 14, LBLUE}}},
                                        {1, {{12, 16, YELLOW}}},
                                        {2, {{14, 18, WHITE}, {15, 17, WHITE}}},
                                        {1, {{11, 17, WHITE}}},
                                        {1, {{5, 21, RED}}},
                                        {1, {{9, 18, ORANGE}}},
                                        {1, {{7, 17, YELLOW}}},
...
rs_drawMech(BMTYPE** line) {
    ...
    core_textOutf(30, 10, BLACK, "%-8s/%-6s,", locals.tedmouthPos ? "Open" : "Closed",
                  locals.redmouthPos ? "Closed" : "Open");
...
static wpc_tSamSolMap rs_samsolmap[] = {
    ...
    {sTopJet, 1, SAM_JET3},
    {sLLDiverter, 1, SAM_DIVERTER},
    {sLLDiverter, 1, SAM_SOLENOID_ON, WPCSAM_F_ONOFF},

    /*Channel #2*/
    {sLeftSling, 2, SAM_LSLING},
    {sRightSling, 2, SAM_RSLING},
    {sULDiverter, 2, SAM_DIVERTER},
    {sULDiverter, 2, SAM_SOLENOID_ON, WPCSAM_F_ONOFF},
    {sURDiverter, 2, SAM_DIVERTER},
    {sURDiverter, 2, SAM_SOLENOID_ON, WPCSAM_F_ONOFF},

    /*Channel #3*/
    {sStartCity, 3, SAM_POPPER},
    {sLockKickout, 3, SAM_POPPER},
    {sRedEject, 3, SAM_SOLENOID_ON},

    /*Channel #4*/
    {sBulldozer, 4, SAM_MOTOR_1, WPCSAM_F_CONT},
    {-1}
...
DCS_SOUNDROM7x("rs_u2_s.l1", CRC(5a2db20c) SHA1(34ce236cc874b820db2d2268cc77815ed7ca061b), "rs_u3_s.l1",
               CRC(719be036) SHA1(fa975a6a93fcaefddcbd1c0b97c49bd9f9608ad4), "rs_u4_s.l1",
               CRC(d452d007) SHA1(b850bc8e17d8940f93c1e7b6a0ab786b092694b3), "rs_u5_s.l1",
               CRC(1faa04c9) SHA1(817bbd7fc0781d84af6c40cb477adf83cef07ab2), "rs_u6_s.l1",
               CRC(eee00add) SHA1(96d664ca73ac896e918d7011c1cda3e55e3731b7), "rs_u7_s.l1",
               CRC(3a222a54) SHA1(2a788e4ac573bf1d128e5bef9357e62c805014b9), "rs_u8_s.l1",
               CRC(c70f2210) SHA1(9be9f271d81d15a4eb123f1377b0c077eef97774)) WPC_ROMEND

    WPC_ROMSTART(rs, l6c, "Road Show U6 game ROM rev L-6 patch ccd1.rom", 0x80000,
                 CRC(d5b83269) SHA1(1b413100005efd63cb0344ff921e5d544abed6e4))
        DCS_SOUNDROM7x("rs_u2_s.l1", CRC(5a2db20c) SHA1(34ce236cc874b820db2d2268cc77815ed7ca061b), "rs_u3_s.l1",
                       CRC(719be036) SHA1(fa975a6a93fcaefddcbd1c0b97c49bd9f9608ad4), "rs_u4_s.l1",
                       CRC(d452d007) SHA1(b850bc8e17d8940f93c1e7b6a0ab786b092694b3), "rs_u5_s.l1",
                       CRC(1faa04c9) SHA1(817bbd7fc0781d84af6c40cb477adf83cef07ab2), "rs_u6_s.l1",
    ...
    WPC_ROMSTART(rs, pa2, "rs-u6.pa2", 0x80000, CRC(674fd680) SHA1(68a4ef5c40fea6b5eae69fbd0ea4339b5757d572))
        DCS_SOUNDROM7x("rs_u2_s.p3", CRC(3a987553) SHA1(bb174cc48533e19906b5ef9d099670e03a04cc89), "rs_u3_s.l1",
                       CRC(719be036) SHA1(fa975a6a93fcaefddcbd1c0b97c49bd9f9608ad4), "rs_u4_s.l1",
                       CRC(d452d007) SHA1(b850bc8e17d8940f93c1e7b6a0ab786b092694b3), "rs_u5_s.l1",
                       CRC(1faa04c9) SHA1(817bbd7fc0781d84af6c40cb477adf83cef07ab2), "rs_u6_s.l1",
                       CRC(eee00add) SHA1(96d664ca73ac896e918d7011c1cda3e55e3731b7), "rs_u7_s.l1",
...
/---------------*/
    CORE_GAMEDEF(rs, l6, "Red and Ted's Road Show (L-6)", 1994, "Williams", wpc_mSecurityS, 0)
        CORE_CLONEDEF(rs, l6c, l6, "Red and Ted's Road Show (L-6C Competition MOD)", 2019, "Williams", wpc_mSecurityS,
                      0) //rev. L-6 patch ccd1
    CORE_CLONEDEF(rs, la5, l6, "Red and Ted's Road Show (LA-5)", 1994, "Williams", wpc_mSecurityS, 0)
        CORE_CLONEDEF(rs, da5, l6, "Red and Ted's Road Show (DA-5 LED Ghost Fix)", 1994, "Williams", wpc_mSecurityS, 0)
            CORE_CLONEDEF(rs, lx5, l6, "Red and Ted's Road Show (LX-5)", 1994, "Williams", wpc_mSecurityS, 0)
                CORE_CLONEDEF(rs, dx5, l6, "Red and Ted's Road Show (DX-5 LED Ghost Fix)", 1994, "Williams",
                              wpc_mSecurityS, 0)
                    CORE_CLONEDEF(rs, la4, l6, "Red and Ted's Road Show (LA-4)", 1994, "Williams", wpc_mSecurityS, 0)
                        CORE_CLONEDEF(rs, da4, l6, "Red and Ted's Road Show (DA-4 LED Ghost Fix)", 1994, "Williams",
                                      wpc_mSecurityS, 0) CORE_CLONEDEF(rs, lx4, l6, "Red and Ted's Road Show (LX-4)",
                                                                       1994, "Williams", wpc_mSecurityS, 0)
                            CORE_CLONEDEF(rs, dx4, l6, "Red and Ted's Road Show (DX-4 LED Ghost Fix)", 1994, "Williams",
                                          wpc_mSecurityS, 0)
                                CORE_CLONEDEF(rs, lx3, l6, "Red and Ted's Road Show (LX-3)", 1994, "Williams",
                                              wpc_mSecurityS, 0)
                                    CORE_CLONEDEF(rs, dx3, l6, "Red and Ted's Road Show (DX-3 LED Ghost Fix)", 1994,
                                                  "Williams", wpc_mSecurityS, 0)
                                        CORE_CLONEDEF(rs, lx2, l6, "Red and Ted's Road Show (LX-2)", 1994, "Williams",
                                                      wpc_mSecurityS, 0)
                                            CORE_CLONEDEF(rs, dx2, l6, "Red and Ted's Road Show (DX-2 LED Ghost Fix)",
                                                          1994, "Williams", wpc_mSecurityS, 0)
                                                CORE_CLONEDEF(rs, pa2, l6, "Red and Ted's Road Show (PA-2 Prototype)",
                                                              1994, "Williams", wpc_mSecurityS, 0)
...
static core_tGameData rsGameData = {
    ...
    {FLIP_SW(FLIP_L | FLIP_U) | FLIP_SOL(FLIP_L | FLIP_U), /* actually 3 left flippers */
     0, 0, 8, 0, 0, 0, 0, rs_getSol, rs_handleMech, rs_getMech, rs_drawMech, &rs_lampPos, rs_samsolmap},
    &rsSimData,
    {
...
rs_handleMech(int mech) {
    ...
    if (mech & 0x20) {
        ...
        else if (core_getSol(sTedEyesRight))
    ...
    if (mech & 0x80) {
        ...
        else if (core_getSol(sRedEyesRight))
...
static int
rs_getMech(int mechNo) {
    switch (mechNo) {
        case 0:
            return locals.tedmouthPos;
        case 1:
            return locals.redmouthPos;
        case 2:
            return locals.bulldozerPos;
        case 3:
            return locals.frgatePos;
        case 4:
            return locals.tedeyesOC;
        case 5:
            return locals.tedeyesLR;
        case 6:
            return locals.redeyesOC;
        case 7:
            return locals.redeyesLR;
    }
    return 0;
}
