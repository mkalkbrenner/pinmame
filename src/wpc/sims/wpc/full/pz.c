...
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BIT(0x0004, "Ozone Ramp", KEYCODE_R) COREPORT_BIT(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BIT(0x0010, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0020, "L/R Inlane", KEYCODE_I)
                COREPORT_BIT(0x0040, "BOP - B", KEYCODE_W) COREPORT_BIT(0x0100, "BOP - O", KEYCODE_E)
                    COREPORT_BIT(0x0200, "BOP - P", KEYCODE_R) COREPORT_BIT(0x0400, "WOOC - Way", KEYCODE_T)
                        COREPORT_BIT(0x0800, "WOOC - Out", KEYCODE_Y) COREPORT_BIT(0x1000, "WOOC - Of", KEYCODE_U)
                            COREPORT_BIT(0x2000, "WOOC - Control", KEYCODE_I)
                                COREPORT_BIT(0x4000, "End Zone Target", KEYCODE_Z)
                                    COREPORT_BIT(0x8000, "WOOC Lane", KEYCODE_O)
                                        ...
                                        PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Left Jet Bumper", KEYCODE_A) COREPORT_BIT(0x0002, "Right Jet Bumper", KEYCODE_S)
        COREPORT_BIT(0x0004, "Bottom Jet Bumper", KEYCODE_D) COREPORT_BIT(0x0008, "Request Time - Request", KEYCODE_F)
            COREPORT_BIT(0x0010, "Request Time - Time", KEYCODE_H)
                COREPORT_BIT(0x0020, "Request Time - Eject", KEYCODE_G) COREPORT_BIT(0x0040, "EDM Target 1", KEYCODE_N)
                    COREPORT_BIT(0x0100, "EDM Target 2", KEYCODE_M) COREPORT_BIT(0x0200, "EDM Target 3", KEYCODE_COMMA)
                        COREPORT_BIT(0x0400, "EDM Target 4", KEYCODE_STOP)
                            COREPORT_BIT(0x0800, "EDM Target 5", KEYCODE_SLASH) COREPORT_BIT(0x1000, "Drain", KEYCODE_Q)
                                COREPORT_BIT(0x2000, "Comic Hole", KEYCODE_J)
                                    COREPORT_BIT(0x4000, "Dancing Dummy Lane", KEYCODE_K)
                                        COREPORT_BIT(0x8000, "Cosmic Cottage", KEYCODE_B)
                                            ...
                                            PORT_START /* 2 */
    COREPORT_BIT(0x1000, "Ha Target 1", KEYCODE_X) COREPORT_BIT(0x2000, "Ha Target 2", KEYCODE_C)
...
static int
pz_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
...
static wpc_tSamSolMap pz_samsolmap[] = {
    ...
    {sHeadOnOff, 1, SAM_MOTOR_1, WPCSAM_F_CONT},

    /*Channel #2*/
    {sBackPopper, 2, SAM_POPPER},
    {sLeftSling, 2, SAM_LSLING},
    {sRightSling, 2, SAM_RSLING},
    {sDummy, 2, SAM_SOLENOID_ON},
    {sDummy, 2, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},

    /*Channel #3*/
    {sDJEject, 3, SAM_SOLENOID_ON},
    {sComicMouth, 3, SAM_SOLENOID_ON},
    {sComicMouth, 3, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},
    {sRightPopper, 3, SAM_POPPER},

    /*Channel #4*/
    {sDJMouth, 4, SAM_SOLENOID_ON},
    {sDJMouth, 4, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},
    {-1}};
...
WPCS_SOUNDROM224("pz_u18.l1", CRC(b7fbba98) SHA1(6533a1474dd335419331d37d4a4447951171412b), "pz_u15.l1",
                 CRC(168bcc52) SHA1(0bae89278cd24950b2e247bba48eaa636f7b566c), "pz_u14.l1",
                 CRC(4d8897ce) SHA1(7a4ac9e849dae93078ddd60adbd34f3930e4cd46))
WPC_ROMEND
...
static core_tGameData pzGameData = {
    GEN_WPCFLIPTRON,
    wpc_dispDMD,
    {FLIP_SWNO(12, 11) | FLIP_SW(FLIP_L) | FLIP_SOL(FLIP_L), 0, 0, 0, 0, 0, 0, 0, NULL, pz_handleMech, pz_getMech,
     pz_drawMech, &pz_lampPos, pz_samsolmap},
    &pzSimData,
    {
        {0}, /* sNO */
...
pz_handleMech(int mech) {
    ...
    if (mech & 0x01) {
        ...
        if (trick >= 84) {
            core_setSw(swHeadOpto1, 0);
            core_setSw(swHeadOpto2, 1);
            core_setSw(swHeadOpto3, 0); /* 101 */
        } else if (trick >= 72) {
            core_setSw(swHeadOpto1, 0);
            core_setSw(swHeadOpto2, 1);
            ...
            core_setSw(swHeadOpto1, 0);
            core_setSw(swHeadOpto2, 0);
            core_setSw(swHeadOpto3, 1); /* 011 */
        } else if (trick >= 48) {
            core_setSw(swHeadOpto1, 0);
            core_setSw(swHeadOpto2, 0);
            core_setSw(swHeadOpto3, 0); /* 111 */
        } else if (trick >= 36) {
            core_setSw(swHeadOpto1, 1);
            core_setSw(swHeadOpto2, 0);
            core_setSw(swHeadOpto3, 0); /* 110 */
        } else if (trick >= 24) {
            core_setSw(swHeadOpto1, 1);
            core_setSw(swHeadOpto2, 0);
            core_setSw(swHeadOpto3, 1); /* 010 */
        } else if (trick >= 12) {
            core_setSw(swHeadOpto1, 1);
            core_setSw(swHeadOpto2, 1);
            core_setSw(swHeadOpto3, 1); /* 000 */
        } else {
            core_setSw(swHeadOpto1, 1);
            core_setSw(swHeadOpto2, 1);
