    ...
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BIT(0x0004, "L/R Ramp", KEYCODE_R) COREPORT_BIT(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BIT(0x0010, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0020, "L/R Inlane", KEYCODE_I)
                COREPORT_BIT(0x0040, "L/R Loop", KEYCODE_L) COREPORT_BIT(0x0080, "Upper Jet", KEYCODE_W)
                    COREPORT_BIT(0x0100, "Center Jet", KEYCODE_E) COREPORT_BIT(0x0200, "Lower Jet", KEYCODE_D)
                        COREPORT_BIT(0x0400, "Upper Slingshot", KEYCODE_MINUS)
                            COREPORT_BIT(0x0800, "Hit Crate", KEYCODE_C) COREPORT_BIT(0x1000, "Spider Hole", KEYCODE_S)
                                COREPORT_BIT(0x2000, "L. Skull Lane", KEYCODE_J)
                                    COREPORT_BIT(0x4000, "C. Skull Lane", KEYCODE_K)
                                        COREPORT_BIT(0x8000, "R. Skull Lane", KEYCODE_L)
                                            ...
                                            PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Secret Passage", KEYCODE_A) COREPORT_BIT(0x0002, "Ex. Ball Lane", KEYCODE_V)
        COREPORT_BIT(0x0008, "Left Leaper", KEYCODE_F) COREPORT_BIT(0x0004, "Center Leaper", KEYCODE_G)
            COREPORT_BIT(0x0010, "Right Leaper", KEYCODE_H) COREPORT_BIT(0x0020, "Lower Spell", KEYCODE_B)
                COREPORT_BIT(0x0040, "Middle Spell", KEYCODE_N) COREPORT_BIT(0x0080, "Upper Spell", KEYCODE_M)
                    COREPORT_BIT(0x0100, "Missed Right Ramp", KEYCODE_R) COREPORT_BIT(0x0200, "Telepathetic", KEYCODE_T)
...
static void
ss_initSim(sim_tBallStatus* balls, int* inports, int noOfBalls) {
    core_setSw(swWheelIndex, TRUE);
}

static int
ss_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
        case stShooter:
...
DCS_SOUNDROM3m("sssnd_11.s2", CRC(1b080a72) SHA1(be80e99e6bcc482379fe99519a4122f3b225be30), "sssnd_11.s3",
               CRC(c4f2e08a) SHA1(e20ff622a3f475db11f1f44d36a6669e160437a3), "sssnd_11.s4",
               CRC(258b0a27) SHA1(83763b98907cf38e6f7b9fe4f26ce93a54ba3568)) WPC_ROMEND
    ...
    WPC_ROMSTART(ss, 03, "ss_g11.0_3", 0x80000, CRC(5b9755d6) SHA1(207d9ea858c76c4991747b401dc83183c1ddf7e4))
        DCS_SOUNDROM3m("sssnd_s2.22", CRC(12b92d7a) SHA1(69151ffb5d2befe28e1ed2c8153c2227552b681c), "sssnd_s3.21",
                       CRC(c4f2e08a) SHA1(e20ff622a3f475db11f1f44d36a6669e160437a3), "sssnd_s4.21",
                       CRC(258b0a27) SHA1(83763b98907cf38e6f7b9fe4f26ce93a54ba3568)) WPC_ROMEND
    ...
    WPC_ROMSTART(ss, 01, "ss_g11.rom", 0x80000, CRC(affd278f) SHA1(e6f41da169fa15c25cfaac22057f3e491da18fc5))
        DCS_SOUNDROM3m("ss_s2.rom", CRC(ad079cbc) SHA1(77c7f676fc2f46e22b74b381638725269f7d23f4), "sssnd_s3.21",
                       CRC(c4f2e08a) SHA1(e20ff622a3f475db11f1f44d36a6669e160437a3), "sssnd_s4.21",
                       CRC(258b0a27) SHA1(83763b98907cf38e6f7b9fe4f26ce93a54ba3568)) WPC_ROMEND
            ...
            CORE_CLONEDEF(ss, 12, 15, "Scared Stiff (1.2)", 1996, "Bally", wpc_m95S, 0)
                CORE_CLONEDEF(ss, 03, 15, "Scared Stiff (0.3 Prototype)", 1996, "Bally", wpc_m95S, 0)
                    CORE_CLONEDEF(ss, 01, 15, "Scared Stiff (D.01R Prototype, Sound 0.25)", 1996, "Bally", wpc_m95S, 0)
                        CORE_CLONEDEF(ss, 01b, 15, "Scared Stiff (D.01R Prototype Coin Play, Sound 0.25)", 1996,
                                      "Bally", wpc_m95S, 0)
...
static WRITE_HANDLER(parallel_0_out) { coreGlobals.tmpLampMatrix[9] = core_revbyte(data); }

static WRITE_HANDLER(parallel_1_out) { coreGlobals.tmpLampMatrix[8] = core_revbyte(data); }

static WRITE_HANDLER(qspin_0_out) { HC4094_data_w(1, data); }

static HC4094interface hc4094ss = {2, // 2 chips
...
static core_tGameData ssGameData = {
    GEN_WPC95,
    wpc_dispDMD,
    {FLIP_SW(FLIP_L | FLIP_U) | FLIP_SOL(FLIP_L), 0, 2, 0, 0, 0, 1, 0, // 2 extra lamp columns for the LEDs of rev. 0.1
     NULL, ss_handleMech, ss_getMech, ss_drawMech, NULL, NULL},
    &ssSimData,
    {
...
static mech_tInitData ss_wheelMech = {39,  40,  MECH_LINEAR | MECH_CIRCLE | MECH_TWOSTEPSOL | MECH_FAST,
                                      200, 200, {{swWheelIndex, 25, 199}}};
...
static void
ss_handleMech(int mech) {
    //  if (mech & 0x01) mech_update(0);
}

static int
ss_getMech(int mechNo) {
    return mech_getPos(mechNo);
}

static const char* spiderWheelText[] = {
    "Collect Deadhead  ", "Jackpot Is Lit    ", "Double Trouble    ", "Collect Eyeball   ", "Beat The Crate    ",
    "Coffin Multiball  ", "Telepathetic Power", "Laboratory        ", "Telepathetic Power", "Boogie Man Boogie ",
    "Crate Multiball   ", "Collect Eyeball   ", "Leaper Mania      ", "Beast Hurry Up    ", "Collect Deadhead  ",
    "Collect Eyeball   ", "Collect Deadhead  "};

/*--------------------
  Drawing information
  --------------------*/
static void
ss_drawMech(BMTYPE** line) {
