...
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BIT(0x0004, "L/R Outlane", KEYCODE_O) COREPORT_BIT(0x0008, "L/R Slingshot", KEYCODE_MINUS)
            COREPORT_BIT(0x0010, "L/R Inlane", KEYCODE_I) COREPORT_BIT(0x0020, "Ramp Fail/OK", KEYCODE_R)
                COREPORT_BIT(0x0040, "RightLoop", KEYCODE_R) COREPORT_BIT(0x0100, "Left Jet", KEYCODE_T)
                    COREPORT_BIT(0x0200, "Right Jet", KEYCODE_Y) COREPORT_BIT(0x0400, "Bottom Jet", KEYCODE_U)
                        COREPORT_BIT(0x0800, "Small Wheel", KEYCODE_S) COREPORT_BIT(0x1000, "Left Loop", KEYCODE_L)
                            COREPORT_BIT(0x2000, "Center Ramp", KEYCODE_C) COREPORT_BIT(0x4000, "BonusX", KEYCODE_X)
                                COREPORT_BIT(0x8000, "Drain", KEYCODE_Q)

                                    PORT_START /* 1 */
    COREPORT_BIT(0x0001, "Right Top 5K", KEYCODE_B) COREPORT_BIT(0x0002, "Right Bot 5K", KEYCODE_N)
        COREPORT_BIT(0x0004, "Left 5K", KEYCODE_V) COREPORT_BIT(0x0008, "Spinner", KEYCODE_J)
...
static int
bop_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
...
/*--------------------
  Drawing information
  --------------------*/
static core_tLampDisplay bop_lampPos = {{0, 0},   /* top left */
                                        {41, 31}, /* size */
                                        {{1, {{32, 0, ORANGE}}},
                                         {1, {{32, 3, ORANGE}}},
                                         {1, {{32, 26, ORANGE}}},
...
static wpc_tSamSolMap bop_samsolmap[] = {
    ...
    {sHeadMotor, 1, SAM_MOTOR_1, WPCSAM_F_CONT},

    /*Channel #2*/
    {sWireBallHolder, 2, SAM_SOLENOID_ON},
    {sWireBallHolder, 2, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},
    {sLeftSling, 2, SAM_LSLING},
    {sRightSling, 2, SAM_RSLING},
    {sBumperSling, 2, SAM_LSLING},

    /*Channel #3*/
    {sSkillShot, 3, SAM_SOLENOID},
    {sUPKicker, 3, SAM_POPPER},
    {sControlledGate, 3, SAM_SOLENOID_ON},
    {sControlledGate, 3, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},
    {-1}};
...
/------------------*/
#define BOP_SOUND                                                                                                      \
    WPCS_SOUNDROM222("mach_u18.l1", CRC(f3f53896) SHA1(4be5a8a27c5ac4718713c05ff2ddf51658a1be27), "mach_u15.l1",       \
                     CRC(fb49513b) SHA1(01f5243ff258adce3a28b24859eba3f465444bdf), "mach_u14.l1",                      \
                     CRC(be2a736a) SHA1(ebf7b26a86d3ffcc35eaa1da8e4f432bd281fe15))

WPC_ROMSTART(bop, l7, "tmbopl_7.rom", 0x40000, CRC(773e1488) SHA1(36e8957b3903b99844a76bf15ba393b17db0db59))
BOP_SOUND WPC_ROMEND WPC_ROMSTART(bop, d7, "tmbopd_7.rom", 0x40000,
                                  CRC(43e69417) SHA1(3881a04e409b18319ad33874d742a611cc783feb)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, l8, "tmbopl_8.rom", 0x40000,
                 CRC(ffeb8991) SHA1(2d9af6274cd0f60c951456198492c0bc5ae433bf)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, d8, "tmbopd_8.rom", 0x40000,
                 CRC(166473a1) SHA1(5fd24c4a579652eafac5274bb55e7502f1519371)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, l6, "tmbopl_6.rom", 0x20000,
                 CRC(96b844d6) SHA1(981194c249a8fc2534e24ef672380d751a5dc5fd)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, d6, "tmbopd_6.rom", 0x20000,
                 CRC(6d5d3df8) SHA1(37fe8bc12321d97e4f368392c8c29c539d8aaedf)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, l5, "tmbopl_5.rom", 0x20000,
                 CRC(fd5c426d) SHA1(e006f8e39cf382249db0b969cf966fd8deaa344a)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, d5, "tmbopd_5.rom", 0x20000,
                 CRC(cfe43a8c) SHA1(6f5c1ddfc4f5ec40851612653924c77abc8309ee)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, l4, "tmbopl_4.rom", 0x20000,
                 CRC(eea14ecd) SHA1(afd670bdc3680f12360561a1a5e5854718c099f7)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, d4, "tmbopd_4.rom", 0x20000,
                 CRC(20295f41) SHA1(20f98b15bb15b6f534b2ea6fa13cbadc4501ff1f)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, l3, "bop_l3.u6", 0x20000,
                 CRC(cd4d219d) SHA1(4e73dca186867ebee07682deab058a45cee53be1)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, d3, "bop_d3.u6", 0x20000,
                 CRC(8e00fcba) SHA1(b8f9cb7f9715e45d43f47654ef553868c856235a)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, l2, "bop_l2.u6", 0x20000,
                 CRC(17ee1f56) SHA1(bee68ed5680455f23dc33e889acec83cba68b1dc)) BOP_SOUND WPC_ROMEND
    WPC_ROMSTART(bop, d2, "bop_d2.u6", 0x20000,
                 CRC(c362eb30) SHA1(93704d1634f170e10c7c6850e7f24be69aefd89e)) BOP_SOUND WPC_ROMEND

    /*--------------
/  Game drivers
/---------------*/
    CORE_GAMEDEF(bop, l7, "Machine: Bride of Pinbot, The (L-7)", 1992, "Williams", wpc_mAlpha2S, 0)
        CORE_CLONEDEF(bop, d7, l7, "Machine: Bride of Pinbot, The (D-7 LED Ghost Fix)", 1991, "Williams", wpc_mAlpha2S,
                      0) CORE_CLONEDEF(bop, l8, l7, "Machine: Bride of Pinbot, The (L-8 billionaire multiplayer patch)",
                                       1991, "Williams", wpc_mAlpha2S, 0)
            CORE_CLONEDEF(bop, d8, l7, "Machine: Bride of Pinbot, The (D-8 LED Ghost Fix)", 1991, "Williams",
                          wpc_mAlpha2S, 0) CORE_CLONEDEF(bop, l6, l7, "Machine: Bride of Pinbot, The (L-6)", 1991,
                                                         "Williams", wpc_mAlpha2S, 0)
                CORE_CLONEDEF(bop, d6, l7, "Machine: Bride of Pinbot, The (D-6 LED Ghost Fix)", 1991, "Williams",
                              wpc_mAlpha2S, 0) CORE_CLONEDEF(bop, l5, l7, "Machine: Bride of Pinbot, The (L-5)", 1991,
                                                             "Williams", wpc_mAlpha2S, 0)
                    CORE_CLONEDEF(bop, d5, l7, "Machine: Bride of Pinbot, The (D-5 LED Ghost Fix)", 1991, "Williams",
                                  wpc_mAlpha2S, 0) CORE_CLONEDEF(bop, l4, l7, "Machine: Bride of Pinbot, The (L-4)",
                                                                 1991, "Williams", wpc_mAlpha2S, 0)
                        CORE_CLONEDEF(bop, d4, l7, "Machine: Bride of Pinbot, The (D-4 LED Ghost Fix)", 1991,
                                      "Williams", wpc_mAlpha2S, 0)
                            CORE_CLONEDEF(bop, l3, l7, "Machine: Bride of Pinbot, The (L-3)", 1991, "Williams",
                                          wpc_mAlpha2S, 0)
                                CORE_CLONEDEF(bop, d3, l7, "Machine: Bride of Pinbot, The (D-3 LED Ghost Fix)", 1991,
                                              "Williams", wpc_mAlpha2S, 0)
                                    CORE_CLONEDEF(bop, l2, l7, "Machine: Bride of Pinbot, The (L-2)", 1991, "Williams",
                                                  wpc_mAlpha2S, 0)
                                        CORE_CLONEDEF(bop, d2, l7, "Machine: Bride of Pinbot, The (D-2 LED Ghost Fix)",
                                                      1991, "Williams", wpc_mAlpha2S, 0)
...
static core_tGameData bopGameData = {
    GEN_WPCALPHA_2,
    wpc_dispAlpha,
    {FLIP_SWNO(12, 11), 0, 2, // 2 extra lamp columns for the helmet lights
     0, 0, 0, 0, 0, NULL, bop_handleMech, bop_getMech, bop_drawMech, &bop_lampPos, bop_samsolmap},
    &bopSimData,
    {
        {0},
...
/-----------------*/
static void
bop_initSim(sim_tBallStatus* balls, int* inports, int noOfBalls) {
...
static HC4094interface hc4094bop = {2, // 2 chips
...
/*
  ...
  */
void
bop_wpc_proc_solenoid_handler(int solNum, int enabled, int smoothed) {
