...
static struct { int sol35, sol36; } locals;
    ...
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BIT(0x0004, "", KEYCODE_R) COREPORT_BIT(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BIT(0x0010, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0020, "L/R Inlane", KEYCODE_I)
                COREPORT_BIT(0x0040, "Upper Jet", KEYCODE_W) COREPORT_BIT(0x0080, "Lower Jet", KEYCODE_E)
...
static int
cv_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
...
static mech_tInitData cv_ringMech = {
    22, 43, MECH_LINEAR | MECH_REVERSE | MECH_ONEDIRSOL, 128, 128, {{42, 0, 4}, {43, 33, 38}, {44, 123, 127}}, 0};

static void
cv_handleMech(int mech) {
    //  if (mech & 0x01) mech_update(0);
    if (mech & 0x02) {
        if (core_getSol(35))
            locals.sol35 = 10;
        else if (locals.sol35 > 0)
            locals.sol35 -= 1;
        if (core_getSol(36))
            locals.sol36 = 10;
        else if (locals.sol36 > 0)
            locals.sol36 -= 1;
    }
}
...
static int
cv_getSol(int solNo) {
    if (solNo == CORE_CUSTSOLNO(1))
        return locals.sol35;
    if (solNo == CORE_CUSTSOLNO(2))
        return locals.sol36;
    return 0;
}

static int
cv_getMech(int mechNo) {
    return mech_getPos(mechNo);
}

static void
cv_drawMech(BMTYPE** line) {
...
DCS_SOUNDROM5xm("s2v1_0.rom", CRC(79dbb8ee) SHA1(f76c0db93b89beaf1e90c5f2199262e296fb1b78), "s3v0_4.rom",
                CRC(8c6c0c56) SHA1(792431cc5b06c3d5028168297614f5eb7e8af34f), "s4v0_4.rom",
                CRC(a9014b78) SHA1(abffe32ab729fb39ab2360d850c8b5476094fd92), "s5v0_4.rom",
                CRC(7e07a2fc) SHA1(f908363c968c15c0dc62e32695e5e2d0ca869391), "s6v0_4.rom",
                CRC(36ca43d3) SHA1(b599f88649c220143aa44cd5213e725e62afb0bc)) WPC_ROMEND
    ...
    WPC_ROMSTART(cv, d52, "cv_g11_d52.rom", 0x100000, CRC(2b6b2822) SHA1(177ddd826b7dee060d090cd79f972836a23d6df9))
        DCS_SOUNDROM5xm("s2v0_1.rom", NO_DUMP, "s3v0_1.rom", NO_DUMP, "s4v0_1.rom", NO_DUMP, "s5v0_1.rom", NO_DUMP,
                        "s6v0_1.rom", NO_DUMP) WPC_ROMEND
...
static core_tGameData cvGameData = {
    GEN_WPC95,
    wpc_dispDMD,
    {FLIP_SW(FLIP_L | FLIP_U) | FLIP_SOL(FLIP_L), 0, 0, 2, 0, 0, 1, 0, cv_getSol, cv_handleMech, cv_getMech,
     cv_drawMech, NULL, NULL},
    &cvSimData,
    {
