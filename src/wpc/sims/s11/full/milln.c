...
PORT_START /* 0 */
    COREPORT_BIT(0x0001, "Left Qualifier", KEYCODE_LCONTROL) COREPORT_BIT(0x0002, "Right Qualifier", KEYCODE_RCONTROL)
        COREPORT_BIT(0x0004, "L/R Slingshot", KEYCODE_MINUS) COREPORT_BIT(0x0008, "L/R Outlane", KEYCODE_O)
            COREPORT_BIT(0x0010, "10 Points", KEYCODE_MINUS) COREPORT_BIT(0x0020, "L/R Inlane", KEYCODE_I)
                COREPORT_BIT(0x0040, "Left Jet", KEYCODE_W) COREPORT_BIT(0x0080, "Right Jet", KEYCODE_E)
                    COREPORT_BIT(0x0100, "Bottom Jet", KEYCODE_R) COREPORT_BIT(0x0200, "L/R Spin", KEYCODE_S)
                        COREPORT_BIT(0x0400, "L/R Lock", KEYCODE_L) COREPORT_BIT(0x0800, "Bank", KEYCODE_D)
                            COREPORT_BIT(0x1000, "bAnk", KEYCODE_F) COREPORT_BIT(0x2000, "baNk", KEYCODE_G)
                                COREPORT_BIT(0x4000, "banK", KEYCODE_H) COREPORT_BIT(0x8000, "Drain", KEYCODE_Q)

                                    PORT_START
    COREPORT_BIT(0x0001, "Money", KEYCODE_T) COREPORT_BIT(0x0002, "mOney", KEYCODE_Y)
        COREPORT_BIT(0x0004, "moNey", KEYCODE_U) COREPORT_BIT(0x0008, "monEy", KEYCODE_I)
            COREPORT_BIT(0x0010, "moneY", KEYCODE_O) COREPORT_BIT(0x0020, "SilverGold", KEYCODE_S)
                COREPORT_BIT(0x0040, "Multiplier", KEYCODE_M) COREPORT_BIT(0x0080, "Cash Held", KEYCODE_C)
...
static int
milln_handleBallState(sim_tBallStatus* ball, int* inports) {
    switch (ball->state) {
...
/*--------------------
  ...
  --------------------*/
static void
milln_drawMech(BMTYPE** line) {
    core_textOutf(30, 10, BLACK, "Drop T.:  2X / 3X");
    core_textOutf(30, 20, BLACK, "         %-4s/%-4s", core_getSw(swBotDrop) ? "Down" : " Up",
                  core_getSw(swTopDrop) ? "Down" : " Up");
    core_textOutf(30, 30, BLACK, "Gates:  Left /Right");
    core_textOutf(30, 40, BLACK, "       %-6s/%-6s", core_getSol(sLeftGate) ? " Open" : "Closed",
                  core_getSol(sRightGate) ? "Open" : "Closed");
}

/* Help */
static void
milln_drawStatic(BMTYPE** line) {
...
static wpc_tSamSolMap milln_samsolmap[] = {
    ...
    {sLeftKicker, 1, SAM_LSLING},
    {sRightKicker, 1, SAM_RSLING},

    /*Channel #2*/
    {sTopKickbig, 2, SAM_SOLENOID},
    {sRightKickBig, 2, SAM_SOLENOID},
    {sMiddleKicker, 2, SAM_SOLENOID},

    /*Channel #3*/
    {sLeftGate, 3, SAM_SOLENOID_ON},
    {sLeftGate, 3, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},
    {sRightGate, 3, SAM_SOLENOID_ON},
    {sRightGate, 3, SAM_SOLENOID_OFF, WPCSAM_F_ONOFF},

    /*Channel #4*/
    {sLeftEject, 4, SAM_SOLENOID_ON},
    {sRightEject, 4, SAM_SOLENOID_ON},
    {-1}};

/*-----------------
/  ROM definitions
/------------------*/
S11_ROMSTART48(milln, l3, "mill_u26.l3", CRC(07bc9fff) SHA1(b16082fb51df3e4d2fb786cb8894b1c232521ef3), "mill_u27.l3",
               CRC(ba789c43) SHA1(c066a304882bea4cba1e215642416fcb22585aa4))
S11XS_SOUNDROM88("mill_u21.l1", CRC(4cd1ee90) SHA1(4e24b96138ced16eff9036303ca6347e3423dbfc), "mill_u22.l1",
                 CRC(73735cfc) SHA1(f74c873a20990263e0d6b35609fc51c08c9f8e31))
S11CS_SOUNDROM88("mill_u4.l1", CRC(cf766506) SHA1(a6e4df19a513102abbce2653d4f72245f54407b1), "mill_u19.l1",
                 CRC(e073245a) SHA1(cbaddde6bb19292ace574a8329e18c97c2ee9763))
S11_ROMEND
...
static sim_tSimData millnSimData = {
    ...
    {stRTrough, stLTrough, stDrain, stDrain, stDrain, stDrain,
...
/----------------------*/
static core_tGameData millnGameData = {GEN_S11A,
                                       s11_dispS11,
                                       {FLIP_SWNO(swLLaneChange, swRLaneChange), 0, 0, 0, 0, 0, 0, 0, NULL,
                                        milln_handleMech, NULL, milln_drawMech, NULL, milln_samsolmap},
                                       &millnSimData,
                                       {{0}},
...
/-----------------*/
static void
milln_initSim(sim_tBallStatus* balls, int* inports, int noOfBalls) {
    /* Set the Roulette's switches (Don't know if they're are optos, but
	   they act inverted */
    core_setSw(sw40K, 1);
    core_setSw(swLeftMB, 1);
    core_setSw(swLeftEB, 1);
    core_setSw(sw50K, 1);
    core_setSw(sw100K, 1);
    core_setSw(swTSpecial, 1);
    core_setSw(sw10K, 1);
    core_setSw(swRightMB, 1);
    core_setSw(swRightEB, 1);
    core_setSw(sw20K, 1);
    core_setSw(sw30K, 1);
    core_setSw(swBSpecial, 1);
}

static void
milln_handleMech(int mech) {
