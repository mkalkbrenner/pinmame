...
#define COREPORT_BIT(mask, name, key) PORT_BITX(mask, IP_ACTIVE_HIGH, IPT_BUTTON1, name, key, IP_JOY_NONE)
...
#define COREPORT_DIPNAME(mask, default, name) PORT_DIPNAME(mask, default, name)
#define COREPORT_DIPSET(mask, name)           PORT_DIPSETTING(mask, name)

/*-- only used in standard inport --*/
#define COREPORT_BITDEF(mask, type, key)      PORT_BITX(mask, IP_ACTIVE_HIGH, type, IP_NAME_DEFAULT, key, IP_JOY_DEFAULT)
...
#define CORE_PORTS                                                                                                     \
    PORT_START /* 0 */                                                                                                 \
        COREPORT_BIT(0x0001, "Column 1", KEYCODE_Q) COREPORT_BIT(0x0002, "Column 2", KEYCODE_W)                        \
            COREPORT_BIT(0x0004, "Column 3", KEYCODE_E) COREPORT_BIT(0x0008, "Column 4", KEYCODE_R)                    \
                COREPORT_BIT(0x0010, "Column 5", KEYCODE_T) COREPORT_BIT(0x0020, "Column 6", KEYCODE_Y)                \
                    COREPORT_BIT(0x0040, "Column 7", KEYCODE_U) COREPORT_BIT(0x0080, "Column 8", KEYCODE_I)            \
                        COREPORT_BIT(0x0100, "Row 1", KEYCODE_A) COREPORT_BIT(0x0200, "Row 2", KEYCODE_S)              \
                            COREPORT_BIT(0x0400, "Row 3", KEYCODE_D) COREPORT_BIT(0x0800, "Row 4", KEYCODE_F)          \
                                COREPORT_BIT(0x1000, "Row 5", KEYCODE_G) COREPORT_BIT(0x2000, "Row 6", KEYCODE_H)      \
                                    COREPORT_BIT(0x4000, "Row 7", KEYCODE_J) COREPORT_BIT(0x8000, "Row 8", KEYCODE_K)  \
                                        PORT_START /* 1 */                                                             \
...
#define DISP_SEG_16(row, type)                                                                                         \
    { 4 * (row), 0, 20 * (row), 16, type }
#define DISP_SEG_7(row, col, type)                                                                                     \
    { 4 * (row), 16 * (col), (row)*20 + (col)*8 + 1, 7, type }
#define DISP_SEG_CREDIT(no1, no2, type)                                                                                \
    {2, 2, no1, 1, type}, { 2, 4, no2, 1, type }
#define DISP_SEG_BALLS(no1, no2, type)                                                                                 \
    {2, 8, no1, 1, type}, { 2, 10, no2, 1, type }
#define DISP_SEG_IMPORT(x)                                                                                             \
    { 0, 0, 0, 1, CORE_IMPORT, NULL, x }
...
#define PINMAME_VIDEO_UPDATE(name)                                                                                     \
    int(name)(struct mame_bitmap * bitmap, const struct rectangle* cliprect, const struct core_dispLayout* layout)
typedef int (*ptPinMAMEvidUpdate)(struct mame_bitmap* bitmap, const struct rectangle* cliprect,
                                  const struct core_dispLayout* layout);
extern void video_update_core_dmd(struct mame_bitmap* bitmap, const struct rectangle* cliprect,
                                  const struct core_dispLayout* layout);
...
typedef struct {
    ...
    struct {                        /* WPC specific stuff */
        ...
        /* common switches */
        struct {
            int start, tilt, sTilt, coinDoor, shooter;
        } comSw;
    } wpc;
...
INLINE void
core_update_modulated_light(UINT32* light, int bit) {
    (*light) = (*light) << 1;
    if (bit)
        (*light) |= 0x01;
}
...
INLINE UINT8
core_revbyte(UINT8 x) {
    return (core_swapNyb[x & 0xf] << 4) | (core_swapNyb[x >> 4]);
}

INLINE UINT8
core_revnyb(UINT8 x) {
    return core_swapNyb[x];
}

INLINE UINT16
core_revword(UINT16 x) {
    UINT8 lo, hi;
