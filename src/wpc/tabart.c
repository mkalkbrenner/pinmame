...
static void
tabart_init(struct sndbrdData* brdData) {
    memset(&sndlocals, 0, sizeof sndlocals);
    memset(memory_region(REGION_CPU2) + 0x4000, 0xff,
           0x80); // sahalove needs a clean RAM for sound 05 to work correctly
...
const struct sndbrdIntf tabartIntf = {"TABART",        tabart_init,   NULL, NULL,
                                      tabart_manCmd_w, tabart_data_w, NULL, tabart_ctrl_w};
...
static void
tabart_irq(int state) {
    cpu_set_irq_line(1, 0, state ? ASSERT_LINE : CLEAR_LINE);
}

static struct YM2203interface tabart_ym2203Int = {1,
                                                  3579545,
...
static struct YM3526interface tabart_ym3526Int = {1, 3579545, {30}};

extern READ_HANDLER(YM3526_read_port_0_r);

MEMORY_READ_START(tabart1_readmem){0x0000, 0x3fff, MRA_ROM}, {0x4000, 0x407f, MRA_RAM},
    {0x5000, 0x5000, YM2203_status_port_0_r}, {0x5001, 0x5001, YM2203_read_port_0_r},
    {0x6000, 0x6000, YM3526_status_port_0_r}, {0x6001, 0x6001, YM3526_read_port_0_r},
    MEMORY_END

    MEMORY_WRITE_START(tabart1_writemem){0x0000, 0x3fff, MWA_NOP},
    {0x4000, 0x407f, MWA_RAM}, {0x5000, 0x5000, YM2203_control_port_0_w}, {0x5001, 0x5001, YM2203_write_port_0_w},
    {0x6000, 0x6000, YM3526_control_port_0_w}, {0x6001, 0x6001, YM3526_write_port_0_w},
    MEMORY_END

    MACHINE_DRIVER_START(TABART1) MDRV_CPU_ADD_TAG("scpu", Z80, 3579545) /* NTSC quartz */
    MDRV_CPU_FLAGS(CPU_AUDIO_CPU) MDRV_CPU_MEMORY(tabart1_readmem, tabart1_writemem)
        MDRV_SOUND_ADD(YM2203, tabart_ym2203Int) MDRV_SOUND_ADD(YM3526, tabart_ym3526Int) MACHINE_DRIVER_END
...
static WRITE_HANDLER(ay8912a_w) { logerror("AY W %02x\n", data); }

static struct AY8910interface tabart_ay8912Int = {1, 19660800. / 16., {33}, {ay8912a_r}, {NULL}, {ay8912a_w}};

static MEMORY_READ_START(tabart2_readmem){0x0000, 0x3fff, MRA_ROM}, {0x4000, 0x407f, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(tabart2_writemem){0x4000, 0x407f, MWA_RAM},
    {0x8000, 0x8000, MWA_NOP}, // watchdog: resets CPU if binary counters are not working
    MEMORY_END

    static PORT_READ_START(tabart2_readport){4, 4, AY8910_read_port_0_r},
    PORT_END

    static PORT_WRITE_START(tabart2_writeport){0, 0, AY8910_control_port_0_w},
