...
static WRITE_HANDLER(riot_portb_w) {
    ...
    vco -= ((data & 8) ? vco / 2.0 : 0) + ((data & 4) ? vco / 4.0 : 0) + ((data & 2) ? vco / 8.0 : 0)
           + ((data & 1) ? vco / 16.0 : 0);
...
static MACHINE_STOP(SPECTRA) { riot6532_unconfig(); }
...
static NVRAM_HANDLER(SPECTRA) { core_nvram(file, read_or_write, spectra_CMOS, 0x100, 0x00); }
...
static MEMORY_READ_START(cpu_readmem){0x0000, 0x00ff, MRA_RAM}, // 5101 RAM (battery backed)
    {0x0100, 0x017f, MRA_RAM},                                  // RIOT RAM
    {0x0180, 0x019f, riot6532_0_r},                             // RIOT registers
    {0x0400, 0x0fff, MRA_ROM}, {0xfff8, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(cpu_writemem){0x0000, 0x00ff, spectra_CMOS_w, &spectra_CMOS},
    {0x0100, 0x017f, MWA_RAM}, {0x0180, 0x019f, riot6532_0_w}, {0x0400, 0x0fff, MWA_ROM},
    MEMORY_END

    static struct SN76477interface spectra_sn76477Int = {
        1,
        {50},          /* mixing level */
                       /* pin description		*/
...
#define SPECTRA_COMPORTS(game, balls)                                                                                  \
    ...
    SIM_PORTS(balls)                                                                                                   \
    PORT_START /* 0 */                                                                                                 \
        COREPORT_BIT(0x0010, "Demand", KEYCODE_1) COREPORT_BIT(0x0008, "Coin/Reset", KEYCODE_5)                        \
            COREPORT_BITTOG(0x0040, "Set", KEYCODE_7) COREPORT_BITTOG(0x0020, "Test", KEYCODE_8)                       \
                COREPORT_BIT(0x0080, "Tilt", KEYCODE_INSERT) COREPORT_BIT(0x0001, "Slam Tilt", KEYCODE_HOME)           \
                    PORT_START /* 1 */                                                                                 \
                        COREPORT_DIPNAME(0x0001, 0x0000, "Reset NVRAM") COREPORT_DIPSET(0x0000, DEF_STR(Off))          \
                            COREPORT_DIPSET(0x0001, DEF_STR(On)) COREPORT_DIPNAME(0x00f0, 0x0020, "Tone pitch")        \
                                COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0010, "1") COREPORT_DIPSET(0x0020, "2") \
                                    COREPORT_DIPSET(0x0030, "3") COREPORT_DIPSET(0x0040, "4")                          \
                                        COREPORT_DIPSET(0x0050, "5") COREPORT_DIPSET(0x0060, "6")                      \
                                            COREPORT_DIPSET(0x0070, "7") COREPORT_DIPSET(0x0080, "8")                  \
                                                COREPORT_DIPSET(0x0090, "9") INPUT_PORTS_END

static core_tLCDLayout dispAlpha[] = {{9, 0, 24, 6, CORE_SEG8D}, {6, 0, 16, 6, CORE_SEG8D},  {3, 0, 8, 6, CORE_SEG8D},
                                      {0, 0, 0, 6, CORE_SEG8D},  {12, 0, 32, 6, CORE_SEG8D}, {0}};
