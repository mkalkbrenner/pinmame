...
#define SAM1_USE_REAL_FREQ                                                                                             \
    1 // Use real cpu frequency and sample rate //!! at least CSI and IJ4 need this, but FG timers are off/too slow (along with many other games)
...
#define SAM1_ZC_FREQ                                                                                                   \
    120 // was 111 for 60hz //112(?) for 50hz // This produces a 60Hz zero cross detection for Family Guy (and upwards) RoHs detection
#define SAM1_FIRQ_FREQ                                                                                                 \
    (4008                                                                                                              \
     * SAM_OVERSAMPLING) // measured on real machine, ~(SAM1_SOUNDFREQ/6) //!! with old SAM1_SOUNDFREQ (1600000/33/2) it was an experimental value of 4039 (found by destruk), originally 4040
...
#define SAM1_ZC_FREQ                                                                                                   \
    146 //!! has changed since ALU changes in ARM7core //143 for 60hz //144 for 50hz // This produces a 60Hz zero cross detection for Family Guy (and upwards) RoHs detection
    ...
    for (ii = 0; ii < 32; ii++) {
        ...
        for (jj = 0; jj < 128; jj += 4) {
            ...
            for (mm = 0; mm < 4; mm++) {
                ...
                *line++ = color_table[kk /*core_revnyb(((kk & 0x3) << 2) | ((kk & 0xc) >> 2))*/
                ]; // no revnyb needed, otherwise f.e. has some issues on scoring fade (f.e. BDK) or highscore fade (f.e. FG)
            }
        }
    }
    ...
    switch (offset) {
        ...
        case 0x02400026:
            ...
            if (core_gameData->hw.gameSpecific1 & SAM_WPT) {
                ...
                if (miniDMDCol < 17)
                    miniDMDCol++;           // limit the displayed column count to 16 for now
...
/**********************/
static void
sam1_sh_update(int num, INT16* buffer[2], int length) {
    int ii, jj;
    for (jj = 0; jj < 2; jj++) {
...
static int
sam1_sh_start(const struct MachineSound* msound) {
    ...
    return stream_init_multi(2, stream_name, volume, SAM1_SOUNDFREQ, 0, sam1_sh_update) < 0;
}
...
static void
sam1snd_init(struct sndbrdData* brdData) {}

const struct sndbrdIntf sam1Intf = {"SAM1", sam1snd_init, NULL, NULL, man3_w,
                                    scmd_w, NULL,         NULL, NULL, SNDBRD_NODATASYNC};
...
/*****************************/
static MEMORY_READ32_START(sam1_readmem){0x00000000, 0x000FFFFF, MRA32_RAM}, //Boot RAM
    ...
    static MEMORY_WRITE32_START(sam1_writemem){0x00000000, 0x000FFFFF, MWA32_RAM, &sam1_page0_ram}, //Boot RAM
    ...
    static PORT_READ32_START(sam1_readport){0x00, 0xFF, sam1_port_r},
    PORT_END static PORT_WRITE32_START(sam1_writeport){0x00, 0xFF, sam1_port_w},
    PORT_END
...
#define SAM1_COMPORTS                                                                                                  \
    PORT_START /* 0 */ /*Switch Col. 0*/                                                                               \
        COREPORT_BITDEF(0x0010, IPT_TILT, KEYCODE_INSERT) COREPORT_BIT(0x0020, "Slam Tilt", KEYCODE_HOME)              \
            COREPORT_BIT(0x0040, "Ticket Notch", KEYCODE_O) COREPORT_BIT(0x0080, "Reserved", KEYCODE_L)                \
        ...
        COREPORT_BITTOG(0x1000, "Coin Door", KEYCODE_END) PORT_START                                    /* 1 */        \
            COREPORT_DIPNAME(0x001f, 0x0000, "Country") COREPORT_DIPSET(0x0000, "USA") COREPORT_DIPSET(                \
                ...
                COREPORT_DIPSET(0x0003, "Canada 1") COREPORT_DIPSET(0x001a, "Canada 2") COREPORT_DIPSET(               \
                    0x0013, "Chuck-E-Cheese") COREPORT_DIPSET(0x0016, "Croatia") COREPORT_DIPSET(0x0009, "Denmark")    \
                    COREPORT_DIPSET(0x0005, "Finland") COREPORT_DIPSET(0x0006, "France") COREPORT_DIPSET(              \
                        0x0007, "Germany") COREPORT_DIPSET(0x000f, "Greece") COREPORT_DIPSET(0x0008, "Italy")          \
                        COREPORT_DIPSET(0x001b, "Lithuania") COREPORT_DIPSET(0x0015, "Japan") COREPORT_DIPSET(         \
                            0x0017, "Middle East") COREPORT_DIPSET(0x0004, "Netherlands")                              \
                            COREPORT_DIPSET(0x0010, "New Zealand") COREPORT_DIPSET(0x000a, "Norway")                   \
                                COREPORT_DIPSET(0x0011, "Portugal") COREPORT_DIPSET(0x0019, "Russia") COREPORT_DIPSET( \
                                    0x0014, "South Africa") COREPORT_DIPSET(0x0012, "Spain") COREPORT_DIPSET(0x000b,   \
                                                                                                             "Sweden") \
                                    COREPORT_DIPSET(0x000c, "Switzerland") COREPORT_DIPSET(0x0018, "Taiwan")           \
                                        COREPORT_DIPSET(0x000e, "U.K.") COREPORT_DIPSET(0x001c, "Unknown (00011100)")  \
...
//SAM1 Display Data
static struct core_dispLayout sam1_dmd128x32[] = {{0, 0, 32, 128, CORE_DMD | CORE_DMDNOAA, (genf*)sam1_dmd32_update},
                                                  {0}};
...
static struct core_dispLayout sam1_wptDisp[] = {{0, 0, 32, 128, CORE_DMD | CORE_DMDNOAA, (genf*)sam1_dmd32_update},
...
static PINMAME_VIDEO_UPDATE(sam1_minidmd2_update) {
    ...
    for (ii = 0; ii < 35; ii++) {
        *seg++ = (dotCol[1][ii] ? 16 : 0) | (dotCol[2][ii] ? 8 : 0) | (dotCol[3][ii] ? 4 : 0) | (dotCol[4][ii] ? 2 : 0)
                 | (dotCol[5][ii] ? 1 : 0);
    }
    if (!pmoptions.dmd_only)
        video_update_core_dmd(bitmap, cliprect, dotCol, layout);
...
static struct core_dispLayout sam1_wofDisp[] = {{0, 0, 32, 128, CORE_DMD | CORE_DMDNOAA, (genf*)sam1_dmd32_update},
...
#define SAM1_INIT(name, disp, lamps, flags)                                                                            \
    SAM1_INPUT_PORTS_START(name, 1)                                                                                    \
    INPUT_PORTS_END static core_tGameData name##_GameData = {                                                          \
        GEN_SAM, disp, {FLIP_SW(FLIP_L) | FLIP_SOL(FLIP_L), 0, lamps, 0, 0, 0, flags}};                                \
    static void init_##name(void) { core_gameData = &name##_GameData; }
...
CORE_CLONEDEFNV(scarn9nj, scarn200, "Simpson's Kooky Carnival, The (Redemption 0.90 New Jersey)", 2006, "Stern", sam1,
                0)
...
CORE_CLONEDEFNV(twd_111h, twd_124, "Walking Dead, The (1.11 Limited Edition)", 2014, "Stern", sam2,
