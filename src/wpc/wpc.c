...
#define DMD_FRAMES                                                                                                     \
    3 /* Some early machines like T2 could in some few animations (like T2 attract mode) profit from more shades, but very tricky to get right without flicker ! */
...
const struct core_dispLayout wpc_dispAlpha[] = {{0, 0, 0, 13, CORE_SEG16R},
                                                {0, 26, 13, 2, CORE_SEG16D},
                                                {0, 30, 15, 1, CORE_SEG16N},
                                                {4, 0, 20, 13, CORE_SEG16R},
                                                {4, 26, 33, 2, CORE_SEG16D},
                                                {4, 30, 35, 1, CORE_SEG16N},
                                                {0}};
const struct core_dispLayout wpc_dispDMD[] = {{0, 0, 32, 128, CORE_DMD, (genf*)wpcdmd_update, NULL}, {0}};
const struct core_dispLayout wpc_dispDMD64[] = {
    {0, 0, 0, 5, CORE_SEG7}, {11, 0, 64, 128, CORE_DMD, (genf*)wpcdmd_update64, NULL}, {0}};
...
/----------------------------*/
static MEMORY_READ_START(wpc_readmem){0x0000, 0x2fff, MRA_RAM}, {0x3000, 0x31ff, MRA_BANK4}, /* DMD */
    ...
    {0x3c00, 0x3faf, MRA_RAM}, {0x3fb0, 0x3fff, wpc_r}, {0x4000, 0x7fff, MRA_BANK1}, {0x8000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(wpc_writemem){0x0000, 0x2fff, wpc_ram_w, &wpc_ram},
    ...
    {0x3c00, 0x3faf, MWA_RAM}, {0x3fb0, 0x3fff, wpc_w, &wpc_data}, {0x8000, 0xffff, MWA_ROM},
    MEMORY_END
    ...
    // convert to 0-63 (+8)
    // i.e. 11=8,12=9,21=16
    static int wpc_sw2m(int no) {
    return (no / 10) * 8 + (no % 10 - 1);
}

int
wpc_m2sw(int col, int row) {
    return col * 10 + row + 1;
}
...
void
wpc_set_modsol_aux_board(int board) {
    wpc_modsol_aux_board = board;
}

void
wpc_set_fastflip_addr(int addr) {
...
void
default_wpc_proc_solenoid_handler(int solNum, int enabled, int smoothed) {
...
void
proc_immediate_solenoid_change(int offset, UINT8 new_data) {
...
/  Machine drivers
/------------------*/
static MACHINE_DRIVER_START(wpc) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(wpc, NULL, wpc)
    MDRV_CPU_ADD(M6809, 2000000) // XTAL8/4
    MDRV_CPU_MEMORY(wpc_readmem, wpc_writemem) MDRV_CPU_VBLANK_INT(wpc_vblank, WPC_VBLANKDIV)
        MDRV_CPU_PERIODIC_INT(wpc_irq, WPC_IRQFREQ) MDRV_NVRAM_HANDLER(wpc) MDRV_DIPS(8) MDRV_SWITCH_UPDATE(wpc)
            MDRV_DIAGNOSTIC_LEDH(1) MDRV_SWITCH_CONV(wpc_sw2m, wpc_m2sw) MDRV_LAMP_CONV(wpc_sw2m, wpc_m2sw)
                MDRV_TIMER_ADD(wpc_zc, 120) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(wpc_alpha) MDRV_IMPORT_FROM(wpc) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(wpc_alpha1S) MDRV_IMPORT_FROM(wpc) MDRV_IMPORT_FROM(wmssnd_s11cs) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(wpc_alpha2S) MDRV_IMPORT_FROM(wpc) MDRV_IMPORT_FROM(wmssnd_wpcs) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(wpc_dmd) MDRV_IMPORT_FROM(wpc) MDRV_VIDEO_START(wpc_dmd) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(wpc_dmdS) MDRV_IMPORT_FROM(wpc) MDRV_VIDEO_START(wpc_dmd)
        MDRV_IMPORT_FROM(wmssnd_wpcs) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(wpc_dcsS) MDRV_IMPORT_FROM(wpc) MDRV_VIDEO_START(wpc_dmd)
        MDRV_IMPORT_FROM(wmssnd_dcs1) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(wpc_95S) MDRV_IMPORT_FROM(wpc) MDRV_VIDEO_START(wpc_dmd)
        MDRV_IMPORT_FROM(wmssnd_dcs2) MACHINE_DRIVER_END
...
/--------------------------------------------------------------*/
    ...
    if (core_gameData->gen & GENWPC_HASDMD) {
        ...
        if ((wpclocals.vblankCount % (WPC_VBLANKDIV / 2)) == 0) {
            ...
            if (core_gameData->hw
                    .gameSpecific2) { // PH: DMD is toggled between half and full page size using DMD_FIRQLINE register bit
                if (wpc_data[DMD_FIRQLINE] & 0x20) { // half page (used by menu system)
                    dmdlocals.DMDFrames[0] = dmdlocals.DMDFrames[1] = memory_region(WPC_DMDREGION)
                                                                      + (wpc_data[DMD_VISIBLEPAGE] & 0x0f) * 0x200
                                                                      + (wpc_data[DMD_VISIBLEPAGE] % 2) * 0x200;
                } else { // full page
                    dmdlocals.DMDFrames[frameNo] = memory_region(WPC_DMDREGION) + wpc_data[DMD_VISIBLEPAGE] * 0x400;
                }
                frameNo = 1 - frameNo;
            } else {
                dmdlocals.DMDFrames[dmdlocals.nextDMDFrame] =
                    memory_region(WPC_DMDREGION) + (wpc_data[DMD_VISIBLEPAGE] & 0x0f) * 0x200;
            }
#ifdef PROC_SUPPORT
            if (coreGlobals.p_rocEn) {
  ...
  /-------------------------------------------------------------*/
    if ((wpclocals.vblankCount % (WPC_VBLANKDIV * WPC_SOLSMOOTH)) == 0) {
        ...
        coreGlobals.solenoids = (wpc_data[WPC_SOLENOID1] << 24) | (wpc_data[WPC_SOLENOID3] << 16)
                                | (wpc_data[WPC_SOLENOID4] << 8) | wpc_data[WPC_SOLENOID2];
    ...
    } else if (((wpclocals.vblankCount % WPC_VBLANKDIV) == 0) && (core_gameData->gen & GENWPC_HASFLIPTRON)) {
  ...
  /-------------------------------------------------------------*/
    ...
    if ((wpclocals.vblankCount % (WPC_VBLANKDIV * WPC_DISPLAYSMOOTH)) == 0) {
        if ((core_gameData->gen & GENWPC_HASDMD) == 0) {
            ...
            coreGlobals.segments[15].w &= ~0x8080;
            coreGlobals.segments[35].w &= ~0x8080;
...
READ_HANDLER(wpc_r) {
    ...
    switch (offset) {
        ...
        case WPC_SHIFTADRH:
            return wpc_data[WPC_SHIFTADRH] + ((wpc_data[WPC_SHIFTADRL] + (wpc_data[WPC_SHIFTBIT] >> 3)) >> 8);
            break;
        case WPC_SHIFTADRL:
...
WRITE_HANDLER(wpc_w) {
    ...
    if (coreGlobals.p_rocEn) {
        ...
        switch (offset) {
            case WPC_SOLENOID1:
                proc_immediate_solenoid_change(24, data);
                break;
            case WPC_SOLENOID2:
                proc_immediate_solenoid_change(0, data);
                break;
            case WPC_SOLENOID3:
                proc_immediate_solenoid_change(16, data);
                break;
            case WPC_SOLENOID4:
                proc_immediate_solenoid_change(8, data);
                break;
        }
    }
#endif
    ...
    switch (offset) {
        ...
        case WPC_FLIPPERCOIL95:                    /* WPC_EXTBOARD4 */
            if (core_gameData->hw.gameSpecific2) { // PH: LED digits
                if (data != 0xff) {
                    coreGlobals.segments[core_BitColToNum(0xff ^ data)].w =
                        wpclocals.alphaSeg[core_BitColToNum(0xff ^ data)].w = core_bcd2seg7[wpc_data[WPC_EXTBOARD1]];
                }
            ...
            for (ii = 0, tmp = data; ii < CORE_MAXGI; ii++, tmp >>= 1) {
                ...
                if (gi_bit) {
                    if (wpclocals.gi_active[ii]
                        == 2
                               + core_gameData->hw
                                     .gameSpecific1) { // if the bit was set last time as well, it's driven continuously.
        ...
        case DMD_PAGE3000: /* set the page that is visible at 0x3000 */
            cpu_setbank(4, memory_region(WPC_DMDREGION) + (data & 0x0f) * 0x200);
            break;
        case DMD_PAGE3200: /* set the page that is visible at 0x3200 */
            cpu_setbank(5, memory_region(WPC_DMDREGION) + (data & 0x0f) * 0x200);
            break;
        case DMD_PAGE3400: /* set the page that is visible at 0x3400 */
            cpu_setbank(6, memory_region(WPC_DMDREGION) + (data & 0x0f) * 0x200);
            break;
        case DMD_PAGE3600: /* set the page that is visible at 0x3600 */
            cpu_setbank(7, memory_region(WPC_DMDREGION) + (data & 0x0f) * 0x200);
            break;
...
/---------------------------*/
static WRITE_HANDLER(wpc_ram_w) {
    if ((wpc_data[WPC_PROTMEM] == WPC_PROTMEMCODE) || ((offset & wpclocals.memProtMask) != wpclocals.memProtMask))
        wpc_ram[offset] = data;
    else
        DBGLOG(("mem prot violation. PC=%04x a=%04x d=%02x\n", activecpu_get_pc(), offset, data));
}
...
wpc_pic_w(int data) {
    if (wpclocals.pic.codeW > 0) {
        if (wpclocals.pic.codeNo[3 - wpclocals.pic.codeW] != data)
            DBGLOG(("Wrong code %2x (expected %2x) sent to pic.", data, wpclocals.pic.codeNo[3 - wpclocals.pic.codeW]));
        ...
        if (wpclocals.modsol_sample < WPC_MODSOLSAMPLE - 1) {
            ...
            for (i = 4; i < 8; i++) {
                if (wpclocals.nonFlipBits & (1 << i)) {
                    core_update_modulated_light(&wpclocals.solenoidbits[i + 28],
                                                wpclocals.modsol_seen_flip_pulses & (1 << i));
                }
            }
            wpclocals.modsol_seen_flip_pulses = wpclocals.solFlipPulse;
            if (wpc_modsol_aux_board > 0) {
                for (i = 0; i < 8; i++) {
                    core_update_modulated_light(&wpclocals.solenoidbits[CORE_FIRSTCUSTSOL + i - 1],
                                                wpclocals.modsol_seen_aux_pulses & (1 << i));
                }
                wpclocals.modsol_seen_aux_pulses =
                    (wpc_modsol_aux_board == 1) ? wpc_data[WPC_EXTBOARD1] : wpc_data[WPC_EXTBOARD2];
            }
            if (wpclocals.modsol_count < WPC_MODSOLSMOOTH) {
                ...
                for (i = 0; i < ((core_gameData->gen & GEN_ALLWPC) ? 28 : 32); i++) {
                    coreGlobals.modulatedSolenoids[CORE_MODSOL_CUR][i] =
                        core_calc_modulated_light(wpclocals.solenoidbits[i], WPC_MODSOLSMOOTH,
                                                  &coreGlobals.modulatedSolenoids[CORE_MODSOL_PREV][i]);
                }
                for (i = 4; i < 8; i++) {
                    if (wpclocals.nonFlipBits & (1 << i)) {
                        coreGlobals.modulatedSolenoids[CORE_MODSOL_CUR][i + 28] =
                            core_calc_modulated_light(wpclocals.solenoidbits[i + 28], WPC_MODSOLSMOOTH,
                                                      &coreGlobals.modulatedSolenoids[CORE_MODSOL_PREV][i + 28]);
                    } else {
                        coreGlobals.modulatedSolenoids[CORE_MODSOL_CUR][i + 28] =
                            ((wpclocals.solFlip & (1 << i)) > 0) ? 1 : 0;
                    }
                }
                if (core_gameData->gen & (GEN_WPC95 | GEN_WPC95DCS)) {
                    for (i = 36; i < 40; i++) {
                        coreGlobals.modulatedSolenoids[CORE_MODSOL_CUR][i] =
                            core_calc_modulated_light(wpclocals.solenoidbits[i - 8], WPC_MODSOLSMOOTH,
                                                      &coreGlobals.modulatedSolenoids[CORE_MODSOL_PREV][i - 8]);
                    }
                } else {
                    for (i = 36; i < 40; i++) {
                ...
                for (i = 0; i < core_gameData->hw.custSol; i++) {
                    if (i < 8 && wpc_modsol_aux_board > 0)
                        coreGlobals.modulatedSolenoids[CORE_MODSOL_CUR][CORE_FIRSTCUSTSOL + i - 1] =
                            core_calc_modulated_light(
                                wpclocals.solenoidbits[CORE_FIRSTCUSTSOL + i - 1], WPC_MODSOLSMOOTH,
                                &coreGlobals.modulatedSolenoids[CORE_MODSOL_PREV][CORE_FIRSTCUSTSOL + i - 1]);
                    else
                        coreGlobals.modulatedSolenoids[CORE_MODSOL_CUR][CORE_FIRSTCUSTSOL + i - 1] =
                            core_getSol(CORE_FIRSTCUSTSOL + i) ? 1 : 0;
                }
            }
        }
    }
...
static MACHINE_INIT(wpc) {
    ...
    switch (core_gameData->gen) {
        ...
        case GEN_WPC95DCS:
            ...
            sndbrd_0_init(core_gameData->gen == GEN_WPC95DCS ? SNDBRD_DCS : SNDBRD_DCS95, 1,
                          memory_region(DCS_ROMREGION), NULL, NULL);
    }
    ...
    memcpy(memory_region(WPC_CPUREGION) + 0x8000, memory_region(WPC_ROMREGION) + romLength - 0x8000, 0x8000);
    ...
    if (core_gameData->gen & GENWPC_HASPIC)
        wpc_serialCnv(core_gameData->wpc.serialNo, wpclocals.pic.sData, wpclocals.pic.codeNo);
...
static NVRAM_HANDLER(wpc) {
    core_nvram(file, read_or_write, wpc_ram,
               (core_gameData->gen & (GEN_WPCDCS | GEN_WPCSECURITY | GEN_WPC95 | GEN_WPC95DCS)) ? 0x3000 : 0x2000,
               0xff);
}

static void
wpc_serialCnv(const char no[21], UINT8 pic[16], UINT8 code[3]) {
    int x;
...
PINMAME_VIDEO_UPDATE(wpcdmd_update) {
    ...
    for (kk = 0, ii = 1; ii < 33; ii++) {
        ...
        for (jj = 0; jj < 16; jj++) {
            ...
            const unsigned int intens1 = ((dmdlocals.DMDFrames[0][kk] & 0x55) + (dmdlocals.DMDFrames[1][kk] & 0x55)
                                          + (dmdlocals.DMDFrames[2][kk] & 0x55));
            const unsigned int intens2 = ((dmdlocals.DMDFrames[0][kk] & 0xaa) + (dmdlocals.DMDFrames[1][kk] & 0xaa)
