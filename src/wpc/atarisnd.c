...
static void
atari_init(struct sndbrdData* brdData) {
...
const struct sndbrdIntf atari1sIntf = {
    "ATARI1",      atari_init, NULL,          NULL, atari_data1_w,
    atari_data1_w, NULL,       atari_ctrl1_w, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
const struct sndbrdIntf atari2sIntf = {
    "ATARI2",     atari_init, NULL,         NULL, atari_data_w,
    atari_data_w, NULL,       atari_ctrl_w, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
...
playSound1(void) {
    if (atarilocals.sound & 0x01) { // wave on
        ...
        if (mixer_is_sample_playing(atarilocals.channel)) {
            mixer_set_sample_frequency(atarilocals.channel, GEN1_SND_CLK / (16 - atarilocals.frequency));
        } else {
            mixer_play_sample(atarilocals.channel, (signed char*)romWave, sizeof(romWave),
...
static int
atari_sh_start1(const struct MachineSound* msound) {
    if (Machine->gamedrv->flags & GAME_NO_SOUND) {
        return -1;
    }
...
static int
atari_sh_start2(const struct MachineSound* msound) {
    if (Machine->gamedrv->flags & GAME_NO_SOUND) {
        return -1;
    }
...
static void
atari_sh_reset1(void) {
    stopSound();
}

static void
atari_sh_reset2(void) {
    stopSound();
    stopNoise();
}
