...
static READ_HANDLER(port_r) {
    ...
    switch (locals.p2 >> 4) {
        case 0:
            retVal = ~core_revbyte(coreGlobals.swMatrix[1 + core_BitColToNum(0xff ^ locals.p1)]);
            break;
        // case 1: unused by code, possible expansion (J4-7)
        case 4:
            retVal = core_getDip(0);
            break;
        case 5:
            retVal = core_getDip(1);
            break;
        case 6:
            retVal = core_getDip(3);
#ifdef VERBOSE_OUTPUT
            printf("DIP3 used!!!");
#endif
            break; // unused, so place it at the end and don't map it
        case 7:
            retVal = core_getDip(2);
            break;
    }
    return retVal;
}
...
static MEMORY_READ_START(mirco_readmem){0x0000, 0x0bff, MRA_ROM}, {0x1000, 0x10ff, MRA_RAM},
    MEMORY_END

    static MEMORY_WRITE_START(mirco_writemem){0x0000, 0x0bff, MWA_ROM},
    {0x1000, 0x10ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    MEMORY_END

    static PORT_READ_START(mirco_readport){0x00, 0xff, port_r},
    {I8039_p1, I8039_p1, p1_r}, {I8039_p2, I8039_p2, p2_r},
    MEMORY_END

    static PORT_WRITE_START(mirco_writeport){0x00, 0xff, port_w},
    {I8039_p1, I8039_p1, p1_w}, {I8039_p2, I8039_p2, p2_w},
    MEMORY_END
...
static const core_tLCDLayout disp[] = {{0, 0, 0, 6, CORE_SEG7},
                                       {0, 28, 8, 6, CORE_SEG7},
                                       {5, 0, 16, 6, CORE_SEG7},
                                       {5, 28, 24, 6, CORE_SEG7},
                                       {3, 14, 32, 2, CORE_SEG7},
...
SIM_PORTS(1)
PORT_START /* 0 */
    COREPORT_BIT(0x0200, "Start", KEYCODE_1) COREPORT_BIT(0x0100, "Coin", KEYCODE_5)
        COREPORT_BIT(0x0001, "Self Test", KEYCODE_7) COREPORT_BIT(0x0400, "Tilt", KEYCODE_INSERT)
            COREPORT_BIT(0x0800, "Mercury Tilt", KEYCODE_HOME) PORT_START /* 1 */
    COREPORT_DIPNAME(0x0101, 0x0000, "Games / coins") COREPORT_DIPSET(0x0000, "1/1") COREPORT_DIPSET(0x0101, "3/2")
        COREPORT_DIPSET(0x0001, "2/1") COREPORT_DIPSET(0x0100, "3/1") COREPORT_DIPNAME(0x0202, 0x0202, "Max. replays")
            COREPORT_DIPSET(0x0000, "5") COREPORT_DIPSET(0x0002, "10") COREPORT_DIPSET(0x0200, "15") COREPORT_DIPSET(
                0x0202, "20") COREPORT_DIPNAME(0x0004, 0x0004, "S3") COREPORT_DIPSET(0x0000,
                                                                                     "0") COREPORT_DIPSET(0x0004, "1")
                COREPORT_DIPNAME(0x0008, 0x0008, "Match feature") COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(
                    0x0008, DEF_STR(On)) COREPORT_DIPNAME(0x0010, 0x0000, "Balls per game") COREPORT_DIPSET(0x0000, "3")
                    COREPORT_DIPSET(0x0010, "5") COREPORT_DIPNAME(0x0020, 0x0020, "S6") COREPORT_DIPSET(0x0000, "0")
                        COREPORT_DIPSET(0x0020, "1") COREPORT_DIPNAME(0x0040, 0x0040, "S7") COREPORT_DIPSET(0x0000, "0")
                            COREPORT_DIPSET(0x0040, "1") COREPORT_DIPNAME(0x0080, 0x0080, "S8")
                                COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0080, "1")
                                    COREPORT_DIPNAME(0x0400, 0x0400, "S11") COREPORT_DIPSET(0x0000, "0")
                                        COREPORT_DIPSET(0x0400, "1") COREPORT_DIPNAME(0x0800, 0x0800, "Special award")
                                            COREPORT_DIPSET(0x0000, "Extra Ball") COREPORT_DIPSET(0x0800, "Replay")
                                                COREPORT_DIPNAME(0x1000, 0x1000, "Startup tune")
                                                    COREPORT_DIPSET(0x0000, DEF_STR(Off)) COREPORT_DIPSET(0x1000,
                                                                                                          DEF_STR(On))
                                                                                ...
                                                                                COREPORT_DIPNAME(0x8000, 0x8000, "S16")
                                                                                    COREPORT_DIPSET(0x0000, "0")
                                                                                        COREPORT_DIPSET(0x8000, "1")
                                                                                            PORT_START /* 2 */
    COREPORT_DIPNAME(0x0001, 0x0000, "Free Play ") COREPORT_DIPSET(0x0000, DEF_STR(Off))
        COREPORT_DIPSET(0x0001, DEF_STR(On)) COREPORT_DIPNAME(0x0002, 0x0000, "S18") COREPORT_DIPSET(0x0000, "0")
            COREPORT_DIPSET(0x0002, "1") COREPORT_DIPNAME(0x0004, 0x0000, "S19") COREPORT_DIPSET(0x0000, "0")
                COREPORT_DIPSET(0x0004, "1") COREPORT_DIPNAME(0x0008, 0x0000, "S20") COREPORT_DIPSET(0x0000, "0")
                    COREPORT_DIPSET(0x0008, "1") COREPORT_DIPNAME(0x0010, 0x0000, "S21") COREPORT_DIPSET(0x0000, "0")
                        COREPORT_DIPSET(0x0010, "1") COREPORT_DIPNAME(0x0020, 0x0000, "S22")
...
static void
init_lckydraw(void) {
    core_gameData = &lckydrawGameData;
}
ROM_START(lckydraw)
...
static void
init_lckydraa(void) {
    core_gameData = &lckydraaGameData;
}
