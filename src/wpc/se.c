...
#else
#define VBLANK                                                                                                         \
    2 // (at least) LOTR requires some kind of oversampling of the lamp state, otherwise the flickering lamps during modes are always on!
    ...
    if ((selocals.vblankCount % SE_LAMPSMOOTH) == 0) {
        ...
        if ((selocals.vblankCount % (VBLANK * SE_LAMPSMOOTH)) == 0) {
            ...
            for (i = 0; i < 10; ++i) {
                int i2;
                for (i2 = 0; i2 < 8; ++i2)
                    coreGlobals.lampMatrix[i] |=
                        lampstate[i * 8 + i2] > 1
                            ? (1 << i2)
                            : 0; //!! > 1 related to the magic of VBLANK and SE_LAMPSMOOTH, so that LOTR lamps blink
            }
...
static MACHINE_INIT(se) {
    ...
    if (core_gameData->gen & GEN_WS_1) {
        ...
        if (selocals.traceRam)
            memset(selocals.traceRam, 0, 0x2000);
    }
#endif
}
...
static READ_HANDLER(ram_r) {
    if ((core_gameData->gen & GEN_WS_1) && (selocals.curBank & TRACERAM_SELECTED)) {
...
static WRITE_HANDLER(lampstrb_w) {
    core_setLamp(coreGlobals.tmpLampMatrix, selocals.lampColumn = (selocals.lampColumn & 0xff00) | data,
                 selocals.lampRow);
}
...
static WRITE_HANDLER(auxlamp_w) {
    core_setLamp(coreGlobals.tmpLampMatrix, selocals.lampColumn = (selocals.lampColumn & 0x00ff) | (data << 8),
                 selocals.lampRow);
}
...
// some point.
static READ_HANDLER(unknown_r) { return 0; }
...
static WRITE_HANDLER(dmdreset_w) { sndbrd_0_ctrl_w(0, data ? 0x02 : 0x00); }

/* Handler called by the Sound System to set the various data needed by the main cpu */
void
set_at91_data(int plin, int sst0, int led) {
    selocals.sst0 = sst0;
    selocals.plin = plin;
...
*/
static READ_HANDLER(dmdie_r) { return selocals.plin; }
        ...
        if (data & ~selocals.lastgiaux & 0x80) { /* clock in data to minidmd */
            ...
            if (!(selocals.auxdata & 0x80)) { /* enabled column? */
                ...
                if (tmp) {
                    ...
                    if (col == 5)
                        selocals.miniframe = (selocals.miniframe + 1) % 3;
                }
            }
        }
...
static MEMORY_READ_START(se_readmem){0x0000, 0x1fff, ram_r}, {0x2000, 0x2003, solenoid_r}, {0x2007, 0x2007, auxboard_r},
    {0x2008, 0x2008, lampstrb_r}, {0x2009, 0x2009, auxlamp_r}, {0x3000, 0x3000, dedswitch_r}, {0x3100, 0x3100, dip_r},
    {0x3400, 0x3400, switch_r}, {0x3406, 0x3407, gilamp_r}, // GI lamps on SPP?
    {0x3500, 0x3500, dmdie_r}, {0x3700, 0x3700, dmdstatus_r},
    ...
    static MEMORY_WRITE_START(se_writemem){0x0000, 0x1fff, ram_w},
    {0x2000, 0x2003, solenoid_w}, {0x2006, 0x2007, auxboard_w}, {0x2008, 0x2008, lampstrb_w},
    {0x2009, 0x2009, auxlamp_w}, {0x200a, 0x200a, lampdriv_w}, {0x200b, 0x200b, giaux_w},
    //{ 0x201a, 0x201a, x201a_w },    // Golden Cue unknown device
    {0x3200, 0x3200, mcpu_bank_w}, {0x3300, 0x3300, switch_w}, {0x3406, 0x3407, gilamp_w}, // GI lamps on SPP?
    {0x3600, 0x3600, dmdlatch_w}, {0x3601, 0x3601, dmdreset_w}, {0x3800, 0x3800, sndbrd_1_data_w},
    {0x3801, 0x3801, sndbrd_unk_data_w}, {0x4000, 0xffff, MWA_NOP},
    MEMORY_END

    static MACHINE_DRIVER_START(se) MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD(M6809, 2000000) // MC6809E
    MDRV_CORE_INIT_RESET_STOP(se, NULL, se) MDRV_CPU_MEMORY(se_readmem, se_writemem)
        MDRV_CPU_VBLANK_INT(se_vblank, VBLANK) MDRV_CPU_PERIODIC_INT(irq1_line_pulse, SE_FIRQFREQ)
            MDRV_NVRAM_HANDLER(se) MDRV_DIPS(8) MDRV_SWITCH_UPDATE(se) MDRV_DIAGNOSTIC_LEDH(1) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(se2aS) MDRV_IMPORT_FROM(se) MDRV_IMPORT_FROM(de2as) MDRV_IMPORT_FROM(se_dmd32)
        MDRV_SOUND_CMD(sndbrd_1_data_w) MDRV_SOUND_CMDHEADING("se") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(se2tS) MDRV_IMPORT_FROM(se) MDRV_IMPORT_FROM(de2ts) MDRV_IMPORT_FROM(se_dmd32)
        MDRV_SOUND_CMD(sndbrd_1_data_w) MDRV_SOUND_CMDHEADING("se") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(se2bS) MDRV_IMPORT_FROM(se) MDRV_IMPORT_FROM(de2bs) MDRV_IMPORT_FROM(se_dmd32)
        MDRV_SOUND_CMD(sndbrd_1_data_w) MDRV_SOUND_CMDHEADING("se") MACHINE_DRIVER_END

    MACHINE_DRIVER_START(se3aS)
    //from se driver - but I need a different init routine
    MDRV_IMPORT_FROM(PinMAME) MDRV_CPU_ADD(M6809, 2000000) MDRV_CORE_INIT_RESET_STOP(se3, NULL, se)
        MDRV_CPU_MEMORY(se_readmem, se_writemem) MDRV_CPU_VBLANK_INT(se_vblank, VBLANK)
            MDRV_CPU_PERIODIC_INT(irq1_line_pulse, SE_FIRQFREQ) MDRV_NVRAM_HANDLER(se) MDRV_DIPS(8)
                MDRV_SWITCH_UPDATE(se) MDRV_DIAGNOSTIC_LEDH(2)
    //
    MDRV_IMPORT_FROM(de3as) MDRV_IMPORT_FROM(se_dmd32) MDRV_SOUND_CMD(sndbrd_1_data_w)
        MDRV_SOUND_CMDHEADING("se") MACHINE_DRIVER_END
...
#ifdef PROC_SUPPORT
int
se_m2sw(int col, int row) {
    return col * 8 + (7 - row) + 1;
}
#endif
