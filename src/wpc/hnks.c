...
MEMORY_READ_START(hnks_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x0083, pia_r(SP_PIA0)}, {0x1000, 0x17ff, MRA_ROM},
    {0xf800, 0xffff, MRA_ROM}, /* reset vector */
    MEMORY_END

    MEMORY_WRITE_START(hnks_writemem){0x0000, 0x007f, MWA_RAM},
    {0x0080, 0x0083, pia_w(SP_PIA0)}, {0x1000, 0x17ff, MWA_ROM}, {0xf800, 0xffff, MWA_ROM}, /* reset vector */
    MEMORY_END
...
start_samples(void) {
    ...
    mixer_play_sample(hnks_locals.channel, samples[iBuffer], !hnks_locals.counterReset ? 0x20 : 1,
                      BASE_FREQUENCY / (hnks_locals.counterSpeed / ((1 - hnks_locals.div2) + 1) + 1), 1);
}
...
static void
hnks_init(struct sndbrdData* brdData) {
...
static int
hnks_sh_start(const struct MachineSound* msound) {
...
/ exported interface
/--------------------*/
const struct sndbrdIntf hankinIntf = {"HNK",       hnks_init, NULL,        NULL, hnks_manCmd_w,
                                      hnks_data_w, NULL,      hnks_ctrl_w, NULL};
