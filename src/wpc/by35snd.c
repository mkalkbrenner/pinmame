...
static const char* u318_sample_names[] = {
    "*s3250u3",  "w1801.wav", "w1802.wav", "w1803.wav", "w1804.wav", "w1805.wav",
    "w1806.wav", "w1807.wav", "w1808.wav", "w1809.wav", "w1810.wav", "w1811.wav",
    "w1812.wav", "w1813.wav", "w1814.wav", "w1815.wav", "w1815.wav", "w1817.wav",
    "w1818.wav", "w1819.wav", "w1820.wav", "w1821.wav", "w1822.wav", "w1823.wav",
    "w1824.wav", "w1825.wav", "w1826.wav", "w1827.wav", "w1828.wav", "w1829.wav",
    "w1830.wav", "w1831.wav", "w1831.wav", "w5101.wav", "w5102.wav", "w5103.wav",
    "w5104.wav", "w5105.wav", "w5106.wav", "w5107.wav", "w5108.wav", "w5109.wav",
    "w5110.wav", "w5111.wav", "w5112.wav", "w5113.wav", "w5114.wav", "w5115.wav",
    "w5115.wav", "w5117.wav", "w5118.wav", "w5119.wav", "w5120.wav", "w5121.wav",
    "w5122.wav", "w5123.wav", "w5124.wav", "w5125.wav", "w5126.wav", "w5127.wav",
    "w5128.wav", "w5129.wav", "w5130.wav", "w5131.wav", "w5131.wav", 0 /* end of array */
};

struct Samplesinterface u318_samples_interface = {3, 50, /* volume */
                                                  u318_sample_names, "Bally sounds"};
...
/ exported interface
/--------------------*/
const struct sndbrdIntf by32Intf = {"BY32",      by32_init, NULL,        NULL, by32_manCmd_w,
                                    by32_data_w, NULL,      by32_ctrl_w, NULL, SNDBRD_NODATASYNC | SNDBRD_NOCTRLSYNC};
...
static int
by32_sh_start(const struct MachineSound* msound) {
...
static WRITE_HANDLER(by32_manCmd_w) {
    by32_data_w(0, data);
    by32_ctrl_w(0, 0);
    by32_ctrl_w(0, ((data & 0x10) >> 3) | 0x01);
}

static void
by32_init(struct sndbrdData* brdData) {
    int w;
...
static MEMORY_READ_START(sp51_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x00ff, pia_r(SP_PIA0)},
    {0x1000, 0x1fff, MRA_ROM}, {0xf000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(sp_writemem){0x0000, 0x007f, MWA_RAM},
    {0x0080, 0x00ff, pia_w(SP_PIA0)}, {0x1000, 0x1fff, MWA_ROM}, {0x8000, 0xffff, MWA_ROM},
    MEMORY_END

    MACHINE_DRIVER_START(by51) MDRV_CPU_ADD_TAG("scpu", M6802, 3579545. / 4.) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(sp51_readmem, sp_writemem) MDRV_INTERLEAVE(500)
            MDRV_SOUND_ADD(AY8910, sp_ay8910Int) MACHINE_DRIVER_END

    static struct DACinterface sp_dacInt = {1, {20}};

static MEMORY_READ_START(sp51N_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x00ff, pia_r(SP_PIA0)},
    {0xf000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(sp51N_writemem){0x0000, 0x007f, MWA_RAM},
    {0x0080, 0x00ff, pia_w(SP_PIA0)}, {0x1000, 0x1000, DAC_0_data_w},
    MEMORY_END

    MACHINE_DRIVER_START(by51N) MDRV_IMPORT_FROM(by51) MDRV_CPU_MODIFY("scpu")
        MDRV_CPU_MEMORY(sp51N_readmem, sp51N_writemem) MDRV_SOUND_ADD(DAC, sp_dacInt) MACHINE_DRIVER_END

    static struct mc3417_interface sp_mc3417Int = {1, {100}};

static MEMORY_READ_START(sp56_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x00ff, pia_r(SP_PIA0)},
    {0x8000, 0xffff, MRA_ROM},
    MEMORY_END

    MACHINE_DRIVER_START(by56) MDRV_IMPORT_FROM(by51) MDRV_CPU_MODIFY("scpu") MDRV_CPU_MEMORY(sp56_readmem, sp_writemem)
        MDRV_SOUND_ADD(MC3417, sp_mc3417Int) MACHINE_DRIVER_END
...
static const struct pia6821_interface sp_pia = {
    /*i: A/B,CA/B1,CA/B2 */ sp_8910r,
    sp_pia0b_r,
    PIA_UNUSED_VAL(1),
    PIA_UNUSED_VAL(1),
    0,
    0,
    /*o: A/B,CA/B2       */ sp_pia0a_w,
    sp_pia0b_w,
    0,
    sp_pia0cb2_w,
    /*irq: A/B           */ sp_irq,
...
sp_init(struct sndbrdData* brdData) {
    ...
    //}
    for (i = 0; i < 0x80; i++)
        memory_region(BY51_CPUREGION)[i] = 0xff;
}
...
static READ_HANDLER(sp_8910r) {
    if (splocals.brdData.subType == 2)
        return ~splocals.lastcmd; // -51N
    if ((splocals.pia0b & 0x03) == 0x01)
        return AY8910Read(0);
    return 0;
}
...
static WRITE_HANDLER(sp_pia0a_w) {
    splocals.pia0a = data;
    if (splocals.pia0b & 0x02)
        AY8910Write(0, splocals.pia0b ^ 0x01, splocals.pia0a);
}

static READ_HANDLER(sp_pia0b_r) { return splocals.pia0b; }
...
static READ_HANDLER(sp_8910a_r) { return (0x1f & ~splocals.lastcmd) | 0x20; }
...
const struct sndbrdIntf by61Intf = {
    "BYSNT",    snt_init, NULL,       snt_diag, snt_manCmd_w,
    snt_data_w, NULL,     snt_ctrl_w, NULL,     0 //SNDBRD_NODATASYNC|SNDBRD_NOCTRLSYNC
};
...
static MEMORY_READ_START(snt_readmem){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x0083, pia_r(SNT_PIA0)},
    {0x0090, 0x0093, pia_r(SNT_PIA1)}, {0x1000, 0x1000, MRA_NOP}, {0xc000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(snt_writemem){0x0000, 0x007f, MWA_RAM},
    {0x0080, 0x0083, pia_w(SNT_PIA0)}, {0x0090, 0x0093, pia_w(SNT_PIA1)}, {0x1000, 0x1000, DAC_0_data_w},
    {0xc000, 0xffff, MWA_ROM},
    MEMORY_END

    MACHINE_DRIVER_START(by61) MDRV_CPU_ADD(M6802, 3579545. / 4.) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(snt_readmem, snt_writemem) MDRV_INTERLEAVE(500) MDRV_SOUND_ADD(TMS5220, snt_tms5220Int)
            MDRV_SOUND_ADD(DAC, snt_dacInt) MDRV_SOUND_ADD(AY8910, snt_ay8910Int) MACHINE_DRIVER_END

    static struct DACinterface snt_dacInt2 = {2, {20, 20}};

static MEMORY_READ_START(snt_readmem2){0x0000, 0x007f, MRA_RAM}, {0x0080, 0x0083, pia_r(SNT2_PIA0)},
    {0x0090, 0x0093, pia_r(SNT2_PIA1)}, {0x1000, 0x1000, MRA_NOP}, {0xc000, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(snt_writemem2){0x0000, 0x007f, MWA_RAM},
    {0x0080, 0x0083, pia_w(SNT2_PIA0)}, {0x0090, 0x0093, pia_w(SNT2_PIA1)}, {0x1000, 0x1000, DAC_1_data_w},
    {0xc000, 0xffff, MWA_ROM},
    MEMORY_END

    MACHINE_DRIVER_START(by61x2) MDRV_CPU_ADD(M6802, 3579545. / 4.) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(snt_readmem, snt_writemem)

            MDRV_CPU_ADD(M6802, 3579545. / 4.) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
                MDRV_CPU_MEMORY(snt_readmem2, snt_writemem2) MDRV_INTERLEAVE(500)

                    MDRV_SOUND_ADD(TMS5220, snt_tms5220Int) MDRV_SOUND_ADD(DAC, snt_dacInt2)
                        MDRV_SOUND_ADD(AY8910, snt_ay8910Int) MACHINE_DRIVER_END
...
static void
snt_init(struct sndbrdData* brdData) {
    int i;
    ...
    if (brdData->subType
        == 2) { // Mysterian, uses a 2nd board without any AY8910 or TMS speech chip, just the DAC is used
    ...
    for (i = 0; i < 0x80; i++)
        memory_region(BY61_CPUREGION)[i] = 0xff;
...
static READ_HANDLER(snt_pia0a_r) {
    if (sntlocals.brdData.subType)
        return snt_8910a_r(0); // -61B
    if ((sntlocals.pia0b & 0x03) == 0x01)
        return AY8910Read(0);
    return 0;
}

static READ_HANDLER(snt_pia0b_r) { return sntlocals.pia0b; }

static WRITE_HANDLER(snt_pia0a_w) {
    sntlocals.pia0a = data;
    if (sntlocals.pia0b & 0x02)
        AY8910Write(0, sntlocals.pia0b ^ 0x01, sntlocals.pia0a);
}
...
static READ_HANDLER(snt_pia1ca2_r) { return sntlocals.pia1ca2; }

static READ_HANDLER(snt_pia1cb1_r) { return sntlocals.pia1cb1; }

static READ_HANDLER(snt2_pia0a_r) { return snt_8910a_r(0); }

static WRITE_HANDLER(snt_data_w) { sntlocals.lastcmd = (sntlocals.lastcmd & 0x10) | (data & 0x0f); }
...
static void
snt_5220Irq(int state) {
    pia_set_input_cb1(SNT_PIA1, (sntlocals.pia1cb1 = !state));
}

static void
snt_5220Rdy(int state) {
    pia_set_input_ca2(SNT_PIA1, (sntlocals.pia1ca2 = state));
}
...
const struct sndbrdIntf by45Intf = {"BY45", cs_init, NULL, cs_diag, NULL, cs_cmd_w, NULL, cs_ctrl_w, NULL, 0};
static struct DACinterface cs_dacInt = {1, {20}};

static MEMORY_READ_START(cs_readmem){0x0000, 0x001f, m6803_internal_registers_r},
    {0x0080, 0x00ff, MRA_RAM}, /*Internal RAM*/
    {0x8000, 0xffff, MRA_ROM},
    MEMORY_END static MEMORY_WRITE_START(cs_writemem){0x0000, 0x001f, m6803_internal_registers_w},
    {0x0080, 0x00ff, MWA_RAM}, /*Internal RAM*/
    {0x8000, 0xffff, MWA_NOP}, MEMORY_END static PORT_READ_START(cs_readport){M6803_PORT2, M6803_PORT2, cs_port2_r},
    PORT_END static PORT_WRITE_START(cs_writeport){M6803_PORT1, M6803_PORT1, DAC_0_data_w},
    {M6803_PORT2, M6803_PORT2, cs_port2_w},
    PORT_END

    MACHINE_DRIVER_START(by45) MDRV_CPU_ADD(M6803, 3579545. / 4.) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(cs_readmem, cs_writemem) MDRV_CPU_PORTS(cs_readport, cs_writeport) MDRV_INTERLEAVE(500)
            MDRV_SOUND_ADD(DAC, cs_dacInt) MACHINE_DRIVER_END
...
static void
cs_init(struct sndbrdData* brdData) {
...
const struct sndbrdIntf byTCSIntf = {"BYTCS",   tcs_init,     NULL,       tcs_diag, NULL,
                                     tcs_cmd_w, tcs_status_r, tcs_ctrl_w, NULL,     SNDBRD_NOCBSYNC};
static struct DACinterface tcs_dacInt = {1, {20}};
static MEMORY_READ_START(tcs_readmem){0x0000, 0x1fff, MRA_RAM}, {0x6000, 0x6003, pia_r(TCS_PIA0)},
    {0x8000, 0xffff, MRA_ROM}, MEMORY_END static MEMORY_WRITE_START(tcs_writemem){0x0000, 0x1fff, MWA_RAM},
    {0x6000, 0x6003, pia_w(TCS_PIA0)},
    {0x8000, 0xffff, MWA_ROM}, MEMORY_END static MEMORY_READ_START(tcs2_readmem){0x0000, 0x07ff, MRA_RAM},
    {0x0800, 0x0803, pia_r(TCS_PIA0)},
    {0x0c00, 0xffff, MRA_ROM}, MEMORY_END static MEMORY_WRITE_START(tcs2_writemem){0x0000, 0x07ff, MWA_RAM},
    {0x0800, 0x0803, pia_w(TCS_PIA0)}, {0x0c00, 0xffff, MWA_ROM},
    MEMORY_END

    MACHINE_DRIVER_START(byTCS) MDRV_CPU_ADD_TAG("scpu", M6809, 2000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(tcs_readmem, tcs_writemem) MDRV_INTERLEAVE(500)
            MDRV_SOUND_ADD(DAC, tcs_dacInt) MACHINE_DRIVER_END

    MACHINE_DRIVER_START(byTCS2) MDRV_IMPORT_FROM(byTCS) MDRV_CPU_MODIFY("scpu")
        MDRV_CPU_MEMORY(tcs2_readmem, tcs2_writemem) MACHINE_DRIVER_END
...
static void
tcs_init(struct sndbrdData* brdData) {
...
static void
tcs_diag(int button) {
    cpu_set_nmi_line(tcslocals.brdData.cpuNo, button ? ASSERT_LINE : CLEAR_LINE);
}
...
const struct sndbrdIntf bySDIntf = {
    "BYSD",   sd_init,     NULL,      sd_diag, sd_man_w,
    sd_cmd_w, sd_status_r, sd_ctrl_w, NULL,    0 //SNDBRD_NODATASYNC|SNDBRD_NOCBSYNC
};
static struct DACinterface sd_dacInt = {1, {80}};
static MEMORY_READ16_START(sd_readmem){0x00000000, 0x0003ffff, MRA16_ROM}, /*ROM (4 X 64K)*/
    {0x00060000, 0x00060007, pia_msb_r(SD_PIA0)},                          /*PIA - CPU D8-15 connected to PIA D0-7*/
    {0x00070000, 0x00070fff, MRA16_RAM},                                   /*RAM*/
    MEMORY_END static MEMORY_WRITE16_START(sd_writemem){0x00000000, 0x0003ffff, MWA16_ROM}, /*ROM (4 X 64K)*/
    {0x00060000, 0x00060007, pia_msb_w(SD_PIA0)}, /*PIA - CPU D8-15 connected to PIA D0-7*/
    {0x00070000, 0x00070fff, MWA16_RAM},          /*RAM*/
    MEMORY_END

    MACHINE_DRIVER_START(bySD) MDRV_CPU_ADD(M68000, 8000000) MDRV_CPU_FLAGS(CPU_AUDIO_CPU)
        MDRV_CPU_MEMORY(sd_readmem, sd_writemem) MDRV_INTERLEAVE(500) MDRV_SOUND_ADD(DAC, sd_dacInt) MACHINE_DRIVER_END
...
static void
sd_init(struct sndbrdData* brdData) {
...
static WRITE_HANDLER(sd_pia0a_w) { sdlocals.dacdata = (sdlocals.dacdata & 0x00ff) | (data << 8); }
