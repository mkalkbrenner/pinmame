...
static WRITE_HANDLER(piaa_w) { locals.piaa = data; }
...
static WRITE_HANDLER(piab_w) {
    ...
    if (data & 0x80) {
        ...
        if (data & 0x08) {
            ...
            if (off == 5)
                coreGlobals.lampMatrix[8] = (coreGlobals.lampMatrix[8] & 0xf0) | (~locals.piaa & 0x0f);
            if (off == 7)
                coreGlobals.lampMatrix[8] = (coreGlobals.lampMatrix[8] & 0x0f) | ((~locals.piaa & 0x0f) << 4);
        }
    } else if (data & 0x20) {
        off = data & 0x0f;
...
static WRITE_HANDLER(piacb2_w) { coreGlobals.solenoids = (coreGlobals.solenoids & 0x7fff) | (data << 15); }
...
/------------------------------------*/
static MEMORY_READ_START(spirit_readmem){0x0000, 0x00ff, MRA_RAM}, {0x0600, 0x0fff, MRA_ROM},
    {0x2200, 0x2203, pia_r(0)}, {0x2400, 0x2400, sw_r}, {0xfff0, 0xffff, MRA_ROM},
    MEMORY_END

    static MEMORY_WRITE_START(spirit_writemem){0x0000, 0x00ff, MWA_RAM, &generic_nvram, &generic_nvram_size},
    {0x2200, 0x2203, pia_w(0)}, {0x2401, 0x2401, MWA_NOP},
    MEMORY_END

    static MACHINE_DRIVER_START(spirit) MDRV_IMPORT_FROM(PinMAME) MDRV_CORE_INIT_RESET_STOP(spirit, NULL, NULL)
        MDRV_CPU_ADD_TAG("mcpu", M6800, 500000) MDRV_CPU_MEMORY(spirit_readmem, spirit_writemem)
            MDRV_CPU_VBLANK_INT(spirit_vblank, 1) MDRV_CPU_PERIODIC_INT(spirit_irq, 500) MDRV_DIPS(16)
                MDRV_SWITCH_UPDATE(spirit) MDRV_NVRAM_HANDLER(generic_0fill) MACHINE_DRIVER_END

    static const core_tLCDLayout disp[] = {{2, 0, 0, 5, CORE_SEG7},
                                           {2, 10, 17, 1, CORE_SEG7},
                                           {2, 18, 10, 6, CORE_SEG7},
                                           {0, 13, 8, 2, CORE_SEG7},
                                           {4, 13, 6, 1, CORE_SEG7},
...
SIM_PORTS(1)
PORT_START /* 0 */
    COREPORT_BIT(0x0800, "Game start", KEYCODE_1) COREPORT_BIT(0x0080, "Coin 1", KEYCODE_3)
        COREPORT_BIT(0x0008, "Coin 2", KEYCODE_5) COREPORT_BIT(0x8000, "Tilt", KEYCODE_INSERT) PORT_START /* 1 */
    COREPORT_DIPNAME(0x0001, 0x0000, "S1") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0001, "1") COREPORT_DIPNAME(
        0x0002, 0x0002, "S2") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0002, "1")
        COREPORT_DIPNAME(0x0004, 0x0004, "S3") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0004, "1")
            COREPORT_DIPNAME(0x0008, 0x0000, "S4") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0008, "1")
                COREPORT_DIPNAME(0x0010, 0x0010, "S5") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0010, "1")
                    COREPORT_DIPNAME(0x0020, 0x0020, "S6") COREPORT_DIPSET(0x0000, "0") COREPORT_DIPSET(0x0020, "1")
