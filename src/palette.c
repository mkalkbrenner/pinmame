...
enum { PALETTIZED_16BIT = 0, DIRECT_15BIT = 1, DIRECT_32BIT = 2, DIRECT_RGB = DIRECT_15BIT | DIRECT_32BIT };
...
INLINE UINT32
rgb_to_direct32(rgb_t rgb) {
    return RGB_RED(rgb) * (direct_rgb_components[0] / 0xff) + RGB_GREEN(rgb) * (direct_rgb_components[1] / 0xff)
...
INLINE rgb_t
adjust_palette_entry(rgb_t entry, int pen_bright) {
...
palette_start(void) {
    ...
    if ((Machine->drv->video_attributes & VIDEO_RGB_DIRECT) && Machine->drv->color_table_len) {
...
static void
internal_set_shadow_preset(int mode, double factor, int dr, int dg, int db, int noclip, int style, int init) {
#define FP   16
#define FMAX (0x1f << FP)
    ...
    if (mode < 0 || mode >= MAX_SHADOW_PRESETS)
        return;

    if ((table_ptr32 = shadow_table_base[mode]) == NULL)
        return;
    ...
    } else // color shadows or highlights(style 0)
    {
        ...
        if (!init && oldclip == noclip && oldRGB[mode][0] == dr && oldRGB[mode][1] == dg && oldRGB[mode][2] == db)
            return;
...
void
palette_set_shadow_mode(int mode) {
    if (mode >= 0 && mode < MAX_SHADOW_PRESETS)
        palette_shadow_table = (UINT16*)shadow_table_base[mode];
}

void
palette_set_shadow_factor32(double factor) {
    internal_set_shadow_preset(0, factor, 0, 0, 0, 0, 1, 0);
}

void
palette_set_highlight_factor32(double factor) {
    internal_set_shadow_preset(1, factor, 0, 0, 0, 0, 2, 0);
}

void
palette_set_shadow_dRGB32(int mode, int dr, int dg, int db, int noclip) {
    internal_set_shadow_preset(mode, 0, dr, dg, db, noclip, 0, 0);
}

void
palette_set_highlight_method(int method) {
...
palette_alloc(void) {
    ...
    if (Machine->drv->color_table_len) {
        ...
        Machine->remapped_colortable =
            auto_malloc(Machine->drv->color_table_len * sizeof(Machine->remapped_colortable[0]));
    ...
    Machine->debug_remapped_colortable =
        auto_malloc(2 * DEBUGGER_TOTAL_COLORS * DEBUGGER_TOTAL_COLORS * sizeof(Machine->debug_remapped_colortable[0]));
...
#else
    {
        ...
        for (i = 0; i < MAX_SHADOW_PRESETS; i++)
            shadow_table_base[i] = NULL;

        if (!(colormode & DIRECT_RGB)) {
            if (Machine->drv->video_attributes & VIDEO_HAS_SHADOWS) {
                UINT16* table_ptr16;
                if (!(table_ptr16 = auto_malloc(65536 * sizeof(UINT16))))
                    return 1;
            ...
            if (Machine->drv->video_attributes & VIDEO_HAS_SHADOWS) {
                UINT32* table_ptr32;
                if (!(table_ptr32 = auto_malloc(65536 * sizeof(UINT32))))
                    return 1;
...
palette_init(void) {
    ...
    switch (colormode) {
        ...
        case DIRECT_15BIT: {
            ...
            /* map the UI pens */
            Machine->uifont->colortable[0] = Machine->uifont->colortable[3] =
                rgb_to_direct15(MAKE_RGB(0x00, 0x00, 0x00));
            Machine->uifont->colortable[1] = Machine->uifont->colortable[2] =
                rgb_to_direct15(MAKE_RGB(0xff, 0xff, 0xff));
            break;
        }
        ...
        case DIRECT_32BIT: {
            ...
            /* map the UI pens */
            Machine->uifont->colortable[0] = Machine->uifont->colortable[3] =
                rgb_to_direct32(MAKE_RGB(0x00, 0x00, 0x00));
            Machine->uifont->colortable[1] = Machine->uifont->colortable[2] =
                rgb_to_direct32(MAKE_RGB(0xff, 0xff, 0xff));
            break;
        }
    }
...
int
palette_get_total_colors_with_ui(void) {
...
void
palette_update_display(struct mame_display* display) {
...
static void
internal_modify_single_pen(pen_t pen, rgb_t color, int pen_bright) {
...
static void
internal_modify_pen(pen_t pen, rgb_t color, int pen_bright) //* new highlight operation
{
            ...
            if (shadow_factor > (1 << PEN_BRIGHTNESS_BITS) && highlight_method) // luminance > 1.0
            {
                ...
                if (highlight_method == 1) {
                    ...
                    if (r >= FMAX)
                        ...
                        r = (r >> PEN_BRIGHTNESS_BITS) << 16;
                    if (g >= FMAX)
                        g = 0x00ff00;
                    else
                        g = (g >> PEN_BRIGHTNESS_BITS) << 8;
                    if (b >= FMAX)
                        b = 0x0000ff;
                    else
                        b = (b >> PEN_BRIGHTNESS_BITS);
                } else {
...
static void
recompute_adjusted_palette(int brightness_or_gamma_changed) {
    int i;

    /* regenerate the color correction table if needed */
    if (brightness_or_gamma_changed)
        for (i = 0; i < sizeof(color_correct_table); i++) {
            int value = (int)(255.0 * (global_brightness * global_brightness_adjust)
                                  * pow((double)i * (1.0 / 255.0), 1.0 / global_gamma)
                              + 0.5);
...
void
palette_set_color(pen_t pen, UINT8 r, UINT8 g, UINT8 b) {
...
void
palette_set_colors(pen_t color_base, const UINT8* colors, int color_count) {
    while (color_count--) {
...
void
palette_get_color(pen_t pen, UINT8* r, UINT8* g, UINT8* b) {
...
void
palette_set_shadow_factor(double factor) {
...
void
palette_set_highlight_factor(double factor) {
...
void
palette_set_global_gamma(double _gamma) {
...
void
palette_set_global_brightness(double brightness) {
    /* if the gamma changed, recompute */
...
void
palette_set_global_brightness_adjust(double adjustment) {
    /* if the gamma changed, recompute */
...
double
palette_get_global_brightness(void) {
...
READ_HANDLER(paletteram_r) { return paletteram[offset]; }

READ_HANDLER(paletteram_2_r) { return paletteram_2[offset]; }

READ16_HANDLER(paletteram16_word_r) { return paletteram16[offset]; }

READ16_HANDLER(paletteram16_2_word_r) { return paletteram16_2[offset]; }

READ32_HANDLER(paletteram32_r) { return paletteram32[offset]; }
...
INLINE void
changecolor_xxxxBBBBGGGGRRRR(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_xxxxBBBBRRRRGGGG(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_xxxxRRRRBBBBGGGG(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_xxxxRRRRGGGGBBBB(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_RRRRGGGGBBBBxxxx(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_BBBBGGGGRRRRxxxx(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_xBBBBBGGGGGRRRRR(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_xRRRRRGGGGGBBBBB(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_xGGGGGRRRRRBBBBB(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_xGGGGGBBBBBRRRRR(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_RRRRRGGGGGBBBBBx(pen_t color, int data) {
    int r, g, b;
...
INLINE void
changecolor_IIIIRRRRGGGGBBBB(pen_t color, int data) {
    int i, r, g, b;

    static const int ztable[16] = {0x0, 0x3, 0x4, 0x5, 0x6, 0x7, 0x8, 0x9, 0xa, 0xb, 0xc, 0xd, 0xe, 0xf, 0x10, 0x11};
...
INLINE void
changecolor_RRRRGGGGBBBBIIII(pen_t color, int data) {
    int i, r, g, b;
...
INLINE void
changecolor_RRRRGGGGBBBBRGBx(pen_t color, int data) {
    int r, g, b;
