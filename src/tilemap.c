...
typedef void (*blitmask_t)(void* dest, const void* source, const UINT8* pMask, int mask, int value, int count,
                           UINT8* pri, UINT32 pcode);
...
static int
PenToPixel_Init(struct tilemap* tilemap) {
    /*
        ...
        if (pPenToPixel == NULL) {
            ...
            for (ty = 0; ty < tilemap->cached_tile_height; ty++) {
                for (tx = 0; tx < tilemap->cached_tile_width; tx++) {
                    ...
                    y = ty;
                    if (i & TILE_FLIPX)
                        x = tilemap->cached_tile_width - 1 - x;
                    if (i & TILE_FLIPY)
                        y = tilemap->cached_tile_height - 1 - y;
...
static void
PenToPixel_Term(struct tilemap* tilemap) {
    int i;
    for (i = 0; i < 4; i++) {
...
void
tilemap_set_transparent_pen(struct tilemap* tilemap, int pen) {
    tilemap->transparent_pen = pen;
}

void
tilemap_set_transmask(struct tilemap* tilemap, int which, UINT32 fgmask, UINT32 bgmask) {
...
void
tilemap_set_depth(struct tilemap* tilemap, int tile_depth, int tile_granularity) {
...
UINT32
tilemap_scan_rows(UINT32 col, UINT32 row, UINT32 num_cols, UINT32 num_rows) {
    /* logical (col,row) -> memory offset */
    return row * num_cols + col;
}

UINT32
tilemap_scan_rows_flip_x(UINT32 col, UINT32 row, UINT32 num_cols, UINT32 num_rows) {
    /* logical (col,row) -> memory offset */
    return row * num_cols + (num_cols - col - 1);
}

UINT32
tilemap_scan_rows_flip_y(UINT32 col, UINT32 row, UINT32 num_cols, UINT32 num_rows) {
    /* logical (col,row) -> memory offset */
    return (num_rows - row - 1) * num_cols + col;
}

UINT32
tilemap_scan_rows_flip_xy(UINT32 col, UINT32 row, UINT32 num_cols, UINT32 num_rows) {
    /* logical (col,row) -> memory offset */
    return (num_rows - row - 1) * num_cols + (num_cols - col - 1);
}

UINT32
tilemap_scan_cols(UINT32 col, UINT32 row, UINT32 num_cols, UINT32 num_rows) {
    /* logical (col,row) -> memory offset */
    return col * num_rows + row;
}

UINT32
tilemap_scan_cols_flip_x(UINT32 col, UINT32 row, UINT32 num_cols, UINT32 num_rows) {
    /* logical (col,row) -> memory offset */
    return (num_cols - col - 1) * num_rows + row;
}

UINT32
tilemap_scan_cols_flip_y(UINT32 col, UINT32 row, UINT32 num_cols, UINT32 num_rows) {
    /* logical (col,row) -> memory offset */
    return col * num_rows + (num_rows - row - 1);
}

UINT32
tilemap_scan_cols_flip_xy(UINT32 col, UINT32 row, UINT32 num_cols, UINT32 num_rows) {
    /* logical (col,row) -> memory offset */
...
static int
mappings_create(struct tilemap* tilemap) {
    ...
    if (tilemap->memory_offset_to_cached_indx) {
        ...
        if (tilemap->cached_indx_to_memory_offset)
            return 0; /* no error */
...
static void
mappings_dispose(struct tilemap* tilemap) {
    free(tilemap->cached_indx_to_memory_offset);
    free(tilemap->memory_offset_to_cached_indx);
}

static void
mappings_update(struct tilemap* tilemap) {
    int logical_flip;
    ...
    for (logical_flip = 0; logical_flip < 4; logical_flip++) {
        ...
        if (tilemap->attributes & TILEMAP_FLIPX)
            cached_flip ^= TILE_FLIPX;
        if (tilemap->attributes & TILEMAP_FLIPY)
...
static void
pio(void* dest, const void* source, int count, UINT8* pri, UINT32 pcode) {
    int i;
...
static void
pit(void* dest, const void* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri, UINT32 pcode) {
    int i;
...
#ifndef pdo16
static void
pdo16(UINT16* dest, const UINT16* source, int count, UINT8* pri, UINT32 pcode) {
    int i;
...
#ifndef pdo16pal
static void
pdo16pal(UINT16* dest, const UINT16* source, int count, UINT8* pri, UINT32 pcode) {
    int pal = pcode >> 16;
    int i;
    for (i = 0; i < count; i++) {
        dest[i] = source[i] + pal;
...
#ifndef pdo16np
static void
pdo16np(UINT16* dest, const UINT16* source, int count, UINT8* pri, UINT32 pcode) {
    memcpy(dest, source, count * sizeof(UINT16));
}
#endif

static void
pdo15(UINT16* dest, const UINT16* source, int count, UINT8* pri, UINT32 pcode) {
    int i;
...
#ifndef pdo32
static void
pdo32(UINT32* dest, const UINT16* source, int count, UINT8* pri, UINT32 pcode) {
    int i;
    pen_t* clut = &Machine->remapped_colortable[pcode >> 16];
    for (i = 0; i < count; i++) {
        dest[i] = clut[source[i]];
...
#ifndef npdo32
static void
npdo32(UINT32* dest, const UINT16* source, int count, UINT8* pri, UINT32 pcode) {
...
#ifndef pdt16
static void
pdt16(UINT16* dest, const UINT16* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri,
      UINT32 pcode) {
    int i;
...
#ifndef pdt16pal
static void
pdt16pal(UINT16* dest, const UINT16* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri,
         UINT32 pcode) {
    int pal = pcode >> 16;
    int i;
...
#ifndef pdt16np
static void
pdt16np(UINT16* dest, const UINT16* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri,
        UINT32 pcode) {
    int i;
...
static void
pdt15(UINT16* dest, const UINT16* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri,
      UINT32 pcode) {
    int i;
    pen_t* clut = &Machine->remapped_colortable[pcode >> 16];
    for (i = 0; i < count; i++) {
...
#ifndef pdt32
static void
pdt32(UINT32* dest, const UINT16* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri,
      UINT32 pcode) {
    int i;
    pen_t* clut = &Machine->remapped_colortable[pcode >> 16];
    for (i = 0; i < count; i++) {
...
#ifndef npdt32
static void
npdt32(UINT32* dest, const UINT16* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri,
       UINT32 pcode) {
    int oddcount = count & 3;
    int unrcount = count & ~3;
    int i;
    ...
    pMask += count, source += count;
    dest += count;
    for (i = -unrcount; i; i += 4) {
        if ((pMask[i] & mask) == value)
            dest[i] = clut[source[i]];
        if ((pMask[i + 1] & mask) == value)
            dest[i + 1] = clut[source[i + 1]];
        if ((pMask[i + 2] & mask) == value)
            dest[i + 2] = clut[source[i + 2]];
        if ((pMask[i + 3] & mask) == value)
            dest[i + 3] = clut[source[i + 3]];
    }
}
#endif
...
static void
pbo15(UINT16* dest, const UINT16* source, int count, UINT8* pri, UINT32 pcode) {
    int i;
    pen_t* clut = &Machine->remapped_colortable[pcode >> 16];
    for (i = 0; i < count; i++) {
...
#ifndef pbo32
static void
pbo32(UINT32* dest, const UINT16* source, int count, UINT8* pri, UINT32 pcode) {
    int i;
    pen_t* clut = &Machine->remapped_colortable[pcode >> 16];
    for (i = 0; i < count; i++) {
...
#ifndef npbo32
static void
npbo32(UINT32* dest, const UINT16* source, int count, UINT8* pri, UINT32 pcode) {
    int oddcount = count & 3;
    int unrcount = count & ~3;
    int i;
    pen_t* clut = &Machine->remapped_colortable[pcode >> 16];
...
static void
pbt15(UINT16* dest, const UINT16* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri,
      UINT32 pcode) {
    int i;
    pen_t* clut = &Machine->remapped_colortable[pcode >> 16];
    for (i = 0; i < count; i++) {
...
#ifndef pbt32
static void
pbt32(UINT32* dest, const UINT16* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri,
      UINT32 pcode) {
    int i;
    pen_t* clut = &Machine->remapped_colortable[pcode >> 16];
    for (i = 0; i < count; i++) {
...
#ifndef npbt32
static void
npbt32(UINT32* dest, const UINT16* source, const UINT8* pMask, int mask, int value, int count, UINT8* pri,
       UINT32 pcode) {
    int oddcount = count & 3;
    int unrcount = count & ~3;
    int i;
...
static void
install_draw_handlers(struct tilemap* tilemap) {
...
INLINE tilemap_draw_func
pick_draw_func(struct mame_bitmap* dest) {
...
struct tilemap*
tilemap_create(void (*tile_get_info)(int memory_offset),
               UINT32 (*get_memory_offset)(UINT32 col, UINT32 row, UINT32 num_cols, UINT32 num_rows), int type,
               int tile_width, int tile_height, int num_cols, int num_rows) {
    ...
    if (tilemap) {
        ...
        if (tilemap->logical_rowscroll && tilemap->cached_rowscroll && tilemap->logical_colscroll
            && tilemap->cached_colscroll && tilemap->pixmap && tilemap->transparency_data
            && tilemap->transparency_data_row && tilemap->transparency_bitmap && (mappings_create(tilemap) == 0)) {
...
void
tilemap_dispose(struct tilemap* tilemap) {
    struct tilemap* prev;
...
void
tilemap_set_enable(struct tilemap* tilemap, int enable) {
    tilemap->enable = enable ? 1 : 0;
}

void
tilemap_set_flip(struct tilemap* tilemap, int attributes) {
...
void
tilemap_set_scroll_cols(struct tilemap* tilemap, int n) {
...
void
tilemap_set_scroll_rows(struct tilemap* tilemap, int n) {
...
void
tilemap_mark_tile_dirty(struct tilemap* tilemap, int memory_offset) {
...
void
tilemap_mark_all_tiles_dirty(struct tilemap* tilemap) {
    if (tilemap == ALL_TILEMAPS) {
        tilemap = first_tilemap;
        while (tilemap) {
            tilemap_mark_all_tiles_dirty(tilemap);
...
static void
update_tile_info(struct tilemap* tilemap, UINT32 cached_indx, UINT32 col, UINT32 row) {
    UINT32 x0;
    UINT32 y0;
...
struct mame_bitmap*
tilemap_get_pixmap(struct tilemap* tilemap) {
...
struct mame_bitmap*
tilemap_get_transparency_bitmap(struct tilemap* tilemap) {
    return tilemap->transparency_bitmap;
}

UINT8*
tilemap_get_transparency_data(struct tilemap* tilemap) //*
{
...
void
tilemap_set_scrollx(struct tilemap* tilemap, int which, int value) {
...
void
tilemap_set_scrolly(struct tilemap* tilemap, int which, int value) {
...
void
tilemap_set_palette_offset(struct tilemap* tilemap, int offset) {
...
void
tilemap_draw(struct mame_bitmap* dest, const struct rectangle* cliprect, struct tilemap* tilemap, UINT32 flags,
             UINT32 priority) {
    ...
    if (tilemap->enable) {
        ...
        if (rows == 1 && cols == 1) { /* XY scrolling playfield */
            ...
            for (ypos = scrolly - tilemap->cached_height; ypos < blit.clip_bottom; ypos += tilemap->cached_height) {
                for (xpos = scrollx - tilemap->cached_width; xpos < blit.clip_right; xpos += tilemap->cached_width) {
            ...
            while (col < cols) {
                ...
                if (scrolly != TILE_LINE_DISABLED) {
                    ...
                    for (ypos = scrolly - tilemap->cached_height; ypos < blit.clip_bottom;
            ...
            while (row < rows) {
                ...
                if (scrollx != TILE_LINE_DISABLED) {
                    ...
                    if (blit.clip_bottom > bottom)
                        blit.clip_bottom = bottom;
                    for (xpos = scrollx - tilemap->cached_width; xpos < blit.clip_right;
...
/* notes:
   ...
   */
void
tilemap_draw_roz(struct mame_bitmap* dest, const struct rectangle* cliprect, struct tilemap* tilemap, UINT32 startx,
                 UINT32 starty, int incxx, int incxy, int incyx, int incyy, int wraparound, UINT32 flags,
                 UINT32 priority) {
    if ((incxx == 1 << 16) && !incxy & !incyx && (incyy == 1 << 16) && wraparound) {
        ...
        if (tilemap->enable) {
            ...
            switch (dest->depth) {
                ...
                case 32:
                    copyroz_core32BPP(dest, tilemap, startx, starty, incxx, incxy, incyx, incyy, wraparound, cliprect,
                                      mask, value, priority, tilemap->palette_offset);
                    break;

                case 15:
                case 16:
                    copyroz_core16BPP(dest, tilemap, startx, starty, incxx, incxy, incyx, incyy, wraparound, cliprect,
                                      mask, value, priority, tilemap->palette_offset);
                    break;
...
void
tilemap_nb_size(UINT32 number, UINT32* width, UINT32* height) {
...
void
tilemap_nb_draw(struct mame_bitmap* dest, UINT32 number, UINT32 scrollx, UINT32 scrolly) {
    tilemap_draw_func drawfunc = pick_draw_func(dest);
    int xpos, ypos;
    struct tilemap* tilemap = tilemap_nb_find(number);
...
DECLARE(copyroz_core,
        (struct mame_bitmap * bitmap, struct tilemap* tilemap, UINT32 startx, UINT32 starty, int incxx, int incxy,
         int incyx, int incyy, int wraparound, const struct rectangle* clip, int mask, int value, UINT32 priority,
            ...
            if (incxy == 0 && incyx == 0 && !wraparound) {
                ...
                if (wraparound) {
                    ...
                    while (sy <= ey) {
                        ...
                        pri = ((UINT8*)priority_bitmap->line[sy]) + sx;
                        while (x <= ex) {
                            if ((((UINT8*)transparency_bitmap->line[(cy >> 16) & ymask])[(cx >> 16) & xmask] & mask)
                                == value) {
                                *dest =
                                    ((UINT16*)srcbitmap->line[(cy >> 16) & ymask])[(cx >> 16) & xmask] + palette_offset;
                                *pri |= priority;
                            }
                            cx += incxx;
                            cy += incxy;
                ...
                if (transCur != transPrev) {
                    ...
                    if (transPrev != eWHOLLY_TRANSPARENT) {
                        ...
                        else /* transPrev == eMASKED */
                        {
                            ...
                            i = y;
                            for (;;) {
                                blit.draw_masked(dest0, source0, mask0, mask, value, count, pmap0,
                                                 tilemap_priority_code);
                                if (++i == y_next)
                                    break;
...
static UINT8
TRANSP(HandleTransparencyBitmask)(struct tilemap* tilemap, UINT32 x0, UINT32 y0, UINT32 flags) {
...
static UINT8
TRANSP(HandleTransparencyColor)(struct tilemap* tilemap, UINT32 x0, UINT32 y0, UINT32 flags) {
    UINT32 tile_width = tilemap->cached_tile_width;
    UINT32 tile_height = tilemap->cached_tile_height;
    struct mame_bitmap* pixmap = tilemap->pixmap;
    struct mame_bitmap* transparency_bitmap = tilemap->transparency_bitmap;
...
static UINT8
TRANSP(HandleTransparencyPen)(struct tilemap* tilemap, UINT32 x0, UINT32 y0, UINT32 flags) {
    UINT32 tile_width = tilemap->cached_tile_width;
    UINT32 tile_height = tilemap->cached_tile_height;
    struct mame_bitmap* pixmap = tilemap->pixmap;
    struct mame_bitmap* transparency_bitmap = tilemap->transparency_bitmap;
...
static UINT8
TRANSP(HandleTransparencyPenBit)(struct tilemap* tilemap, UINT32 x0, UINT32 y0, UINT32 flags) {
    UINT32 tile_width = tilemap->cached_tile_width;
    UINT32 tile_height = tilemap->cached_tile_height;
    struct mame_bitmap* pixmap = tilemap->pixmap;
    struct mame_bitmap* transparency_bitmap = tilemap->transparency_bitmap;
...
static UINT8
TRANSP(HandleTransparencyPens)(struct tilemap* tilemap, UINT32 x0, UINT32 y0, UINT32 flags) {
    UINT32 tile_width = tilemap->cached_tile_width;
    UINT32 tile_height = tilemap->cached_tile_height;
    struct mame_bitmap* pixmap = tilemap->pixmap;
    struct mame_bitmap* transparency_bitmap = tilemap->transparency_bitmap;
        ...
        for (ty = tile_height; ty != 0; ty--) {
            ...
            for (tx = tile_width / 2; tx != 0; tx--) {
                ...
                *(x + (UINT16*)pixmap->line[y]) = PAL_GET(pen);
                ...
                if (!((1 << pen) & fgmask))
                    code |= TILE_FLAG_FG_OPAQUE;
                if (!((1 << pen) & bgmask))
                    code |= TILE_FLAG_BG_OPAQUE;
                and_flags &= code;
                or_flags |= code;
                ((UINT8*)transparency_bitmap->line[y])[x] = code;
...
static UINT8
TRANSP(HandleTransparencyNone)(struct tilemap* tilemap, UINT32 x0, UINT32 y0, UINT32 flags) {
    UINT32 tile_width = tilemap->cached_tile_width;
    UINT32 tile_height = tilemap->cached_tile_height;
    struct mame_bitmap* pixmap = tilemap->pixmap;
    struct mame_bitmap* transparency_bitmap = tilemap->transparency_bitmap;
