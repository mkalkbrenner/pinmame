...
static unsigned char TMS9928A_palette[32 * 3] = {
    ...
    237,
    237,
    237 // raised white to 105%, as it looked too smudgy
    ...
    255,
    120,
    120, // note red value is 13% too low here!
    212,
    194,
    84,
    230,
...
static void (*ModeHandlers[])(int which, struct mame_bitmap*) = {
    ...
    _TMS9928A_mode3, _TMS9928A_modebogus, _TMS9928A_mode23, _TMS9928A_modebogus};

#define IMAGE_SIZE          (256 * 192) /* size of rendered image        */

#define TMS_SPRITES_ENABLED ((tms[which].Regs[1] & 0x50) == 0x40)
#define TMS_MODE                                                                                                       \
    ((tms[which].model == TMS99x8A ? (tms[which].Regs[0] & 2) : 0) | ((tms[which].Regs[1] & 0x10) >> 4)                \
     | ((tms[which].Regs[1] & 8) >> 1))
...
int
TMS9928A_start(int which, int model, unsigned int vram) {
...
TMS9928A_post_load(int which) {
    ...
    if (tms[which].INTCallback)
        tms[which].INTCallback(tms[which].INT);
}

void
TMS9928A_stop(int num_chips) {
    int which;
    /*For each chip*/
    for (which = 0; which < num_chips; which++) {
        ...
        tms[which].vMem = NULL;
        free(tms[which].dBackMem);
        tms[which].dBackMem = NULL;
        free(tms[which].DirtyColour);
        tms[which].DirtyColour = NULL;
        free(tms[which].DirtyName);
        tms[which].DirtyName = NULL;
        free(tms[which].DirtyPattern);
        tms[which].DirtyPattern = NULL;
        bitmap_free(tms[which].tmpbmp);
        tms[which].tmpbmp = NULL;
        bitmap_free(tms[which].tmpsbmp);
        tms[which].tmpsbmp = NULL;
    }
}
...
*/
static void
_TMS9928A_set_dirty(int which, char dirty) {
...
*/
int
TMS9928A_vram_r(int which, int offset) {
    UINT8 b;
...
void
TMS9928A_vram_w(int which, int offset, int data) {
    int i;
    ...
    if (tms[which].vMem[tms[which].Addr] != data) {
        ...
        tms[which].Change = 1;
        /* dirty optimization */
        if ((tms[which].Addr >= tms[which].nametbl) && (tms[which].Addr < (tms[which].nametbl + MAX_DIRTY_NAME))) {
...
int
TMS9928A_register_r(int which, int offset) {
    UINT8 b;
...
void
TMS9928A_register_w(int which, int offset, int data) {
    int reg;
...
static void
_TMS9928A_change_register(int which, int reg, UINT8 val) {
    static const UINT8 Mask[8] = {0x03, 0xfb, 0x0f, 0xff, 0x07, 0x7f, 0x07, 0xff};
    static const char* modes[] = {"Mode 0 (GRAPHIC 1)",
                                  "Mode 1 (TEXT 1)",
                                  "Mode 2 (GRAPHIC 2)",
        ...
        case 0:
            /*if (val & 2)*/ {
                ...
                if (val & 2) {
                    ...
                    tms[which].patternmask = (tms[which].Regs[4] & 3) * 256 | (tms[which].colourmask & 255);
                } else {
...
void
TMS9928A_int_callback(int which, void (*callback)(int)) {
    tms[which].INTCallback = callback;
}

void
TMS9928A_set_spriteslimit(int which, int limit) {
...
void
TMS9928A_refresh(int num_chips, struct mame_bitmap* bmp, int full_refresh) {
    int c, which;
    int update = 0;
...
*/
void
TMS9928A_refresh_test(int num_chips, struct mame_bitmap* bmp, int full_refresh) {
    int c, which;
    int update = 0;
    ...
    if (update) {
        ...
        /*For each chip*/
        for (which = 0; which < MAX_VDP; which++) {
            copybitmap(bmp, tms[which].tmpbmp, 0, 0, 256 * (which + 1), 0, &Machine->visible_area, TRANSPARENCY_NONE,
                       0);
        }
        ...
        for (which = 0; which < MAX_VDP; which++) {
            ...
            copybitmap(bmp, tms[which].tmpsbmp, 0, 0, 256 * (which + 1), 0, &Machine->visible_area, TRANSPARENCY_PEN,
                       0);
        }
    }
...
static void
_TMS9928A_mode1(int which, struct mame_bitmap* bmp) {
    ...
    for (y = 0; y < 24; y++) {
        for (x = 0; x < 40; x++) {
            ...
            if (!(tms[which].DirtyName[name++] || tms[which].DirtyPattern[charcode]) && !tms[which].anyDirtyColour)
                continue;
            patternptr = tms[which].vMem + tms[which].pattern + (charcode * 8);
            for (yy = 0; yy < 8; yy++) {
                pattern = *patternptr++;
                for (xx = 0; xx < 6; xx++) {
                    plot_pixel(bmp, 8 + x * 6 + xx, y * 8 + yy, (pattern & 0x80) ? fg : bg);
                    pattern *= 2;
                }
            }
        }
...
static void
_TMS9928A_mode12(int which, struct mame_bitmap* bmp) {
    int pattern, x, y, yy, xx, name, charcode;
    UINT8 fg, bg, *patternptr;
    struct rectangle rt;
...
static void
_TMS9928A_mode0(int which, struct mame_bitmap* bmp) {
    ...
    for (y = 0; y < 24; y++) {
        for (x = 0; x < 32; x++) {
            ...
            for (yy = 0; yy < 8; yy++) {
                pattern = *patternptr++;
                for (xx = 0; xx < 8; xx++) {
                    plot_pixel(bmp, x * 8 + xx, y * 8 + yy, (pattern & 0x80) ? fg : bg);
                    pattern *= 2;
                }
            }
        }
...
static void
_TMS9928A_mode2(int which, struct mame_bitmap* bmp) {
    ...
    for (y = 0; y < 24; y++) {
        for (x = 0; x < 32; x++) {
            ...
            if (!(tms[which].DirtyName[name++] || tms[which].DirtyPattern[pattern] || tms[which].DirtyColour[colour]))
                continue;
...
static void
_TMS9928A_mode3(int which, struct mame_bitmap* bmp) {
...
static void
_TMS9928A_mode23(int which, struct mame_bitmap* bmp) {
    int x, y, yy, yyy, name, charcode;
    UINT8 fg, bg, *patternptr;
...
static void
_TMS9928A_modebogus(int which, struct mame_bitmap* bmp) {
    UINT8 fg, bg;
    int x, y, n, xx;
    ...
    for (y = 0; y < 192; y++) {
        ...
        n = 8;
        while (n--)
            plot_pixel(bmp, xx++, y, bg);
        for (x = 0; x < 40; x++) {
            n = 4;
            while (n--)
                plot_pixel(bmp, xx++, y, fg);
            n = 2;
            while (n--)
                plot_pixel(bmp, xx++, y, bg);
        }
...
*/
static void
_TMS9928A_sprites(int which, struct mame_bitmap* bmp) {
    UINT8 *attributeptr, *patternptr, c;
    int p, x, y, size, i, j, large, yy, xx, limit[192], illegalsprite, illegalspriteline;
    UINT16 line, line2;
    ...
    for (p = 0; p < 32; p++) {
        ...
        x = *attributeptr++;
        patternptr =
            tms[which].vMem + tms[which].spritepattern + ((size == 16) ? *attributeptr & 0xfc : *attributeptr) * 8;
        attributeptr++;
        c = (*attributeptr & 0x0f);
        if (*attributeptr & 0x80)
            x -= 32;
        attributeptr++;
