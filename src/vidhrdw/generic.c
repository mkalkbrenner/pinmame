...
READ_HANDLER(videoram_r) { return videoram[offset]; }

READ_HANDLER(colorram_r) { return colorram[offset]; }
...
READ_HANDLER(spriteram_r) { return spriteram[offset]; }

WRITE_HANDLER(spriteram_w) { spriteram[offset] = data; }

READ16_HANDLER(spriteram16_r) { return spriteram16[offset]; }

WRITE16_HANDLER(spriteram16_w) { COMBINE_DATA(spriteram16 + offset); }

READ_HANDLER(spriteram_2_r) { return spriteram_2[offset]; }

WRITE_HANDLER(spriteram_2_w) { spriteram_2[offset] = data; }
...
WRITE_HANDLER(buffer_spriteram_w) { memcpy(buffered_spriteram, spriteram, spriteram_size); }

WRITE16_HANDLER(buffer_spriteram16_w) { memcpy(buffered_spriteram16, spriteram16, spriteram_size); }

WRITE32_HANDLER(buffer_spriteram32_w) { memcpy(buffered_spriteram32, spriteram32, spriteram_size); }

WRITE_HANDLER(buffer_spriteram_2_w) { memcpy(buffered_spriteram_2, spriteram_2, spriteram_2_size); }

WRITE16_HANDLER(buffer_spriteram16_2_w) { memcpy(buffered_spriteram16_2, spriteram16_2, spriteram_2_size); }

WRITE32_HANDLER(buffer_spriteram32_2_w) { memcpy(buffered_spriteram32_2, spriteram32_2, spriteram_2_size); }

void
buffer_spriteram(unsigned char* ptr, int length) {
    memcpy(buffered_spriteram, ptr, length);
}

void
buffer_spriteram_2(unsigned char* ptr, int length) {
...
void
set_vh_global_attribute(int* addr, int data) {
    if (!addr || *addr != data) {
...
int
get_vh_global_attribute_changed(void) {
