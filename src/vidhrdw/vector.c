...
void
vector_register_aux_renderer(int (*aux_renderer)(point* start, int num_points)) {
...
float
vector_get_gamma(void) {
    return gamma_correction;
}

void
vector_set_flicker(float _flicker) {
    flicker_correction = _flicker;
    flicker = (int)(flicker_correction * 2.55);
}

float
vector_get_flicker(void) {
    return flicker_correction;
}

void
vector_set_intensity(float _intensity) {
    intensity_correction = _intensity;
}

float
vector_get_intensity(void) {
...
static void
vector_draw_aa_pixel_15(int x, int y, rgb_t col, int dirty) {
    vector_pixel_t coords;
    UINT32 dst;
...
static void
vector_draw_aa_pixel_32(int x, int y, rgb_t col, int dirty) {
    vector_pixel_t coords;
    UINT32 dst;
...
void
vector_draw_to(int x2, int y2, rgb_t col, int intensity, int dirty, rgb_t (*color_callback)(void)) {
    unsigned char a1;
    ...
    if (antialias) {
        ...
        if (dx >= dy) {
            ...
            for (;;) {
                if (color_callback)
                    col = Tinten(intensity, (*color_callback)());
 ...
 */
void
vector_add_point(int x, int y, rgb_t color, int intensity) {
    point* newpoint;
...
void
vector_add_point_callback(int x, int y, rgb_t (*color_callback)(void), int intensity) {
    point* newpoint;
 ...
 */
void
vector_add_clip(int x1, int yy1, int x2, int y2) {
    point* newpoint;
 ...
 */
void
vector_set_clip(int x1, int yy1, int x2, int y2) {
    /* failsafe */
    if ((x1 >= x2) || (yy1 >= y2)) {
...
clever_mark_dirty(void) {
    ...
    for (i = min_index; i > 0; i--, old++, new ++) {
        ...
        if (old->status == VCLIP || new->status == VCLIP) {
            ...
            if (new->status == VCLIP)
                newclip = *new;
            clips_match = (newclip.x == oldclip.x) && (newclip.y == oldclip.y) && (newclip.arg1 == oldclip.arg1)
                          && (newclip.arg2 == oldclip.arg2);
            if (!clips_match)
                last_match = 0;
        ...
        else if (clips_match && (new->x == old->x) && (new->y == old->y) && (new->col == old->col)
                 && (new->intensity == old->intensity) && (!new->callback && !old->callback)) {
            if (last_match) {
                new->status = VCLEAN;
                continue;
            }
...
VIDEO_UPDATE(vector) {
    ...
    for (i = 0; i < new_index; i++) {
        if (curpoint->status == VCLIP) {
            ...
            vector_draw_to(curpoint->x, curpoint->y, curpoint->col, Tgamma[curpoint->intensity], curpoint->status,
                           curpoint->callback);
