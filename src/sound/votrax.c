...
static const char* PhonemeNames[65] = {
    "EH3", "EH2", "EH1", "PA0", "DT",  "A1",  "A2", "ZH", "AH2", "I3", "I2", "I1",  "M",    "N",   "B",   "V",   "CH",
    "SH",  "Z",   "AW1", "NG",  "AH1", "OO1", "OO", "L",  "K",   "J",  "H",  "G",   "F",    "D",   "S",   "A",   "AY",
    "Y1",  "UH3", "AH",  "P",   "O",   "I",   "U",  "Y",  "T",   "R",  "E",  "W",   "AE",   "AE1", "AW2", "UH2", "UH1",
    "UH",  "O2",  "O1",  "IU",  "U1",  "THV", "TH", "ER", "EH",  "E1", "AW", "PA1", "STOP", 0};
#endif
...
enum { T_COMMIT_PHONE, T_END_OF_PHONE };

static const double s_glottal_wave[9] = {0, -4 / 7.0, 7 / 7.0, 6 / 7.0, 5 / 7.0, 4 / 7.0, 3 / 7.0, 2 / 7.0, 1 / 7.0};

// ROM definition for the Votrax phone ROM
static const UINT8 sc01a_bin[512] = {
    };

void filters_commit(int force);

void
votraxsc01_set_clock(UINT32 newfreq) {
    // update if changed
    ...
    for (i = 0; i < 64; i++) {
        ...
        if (votraxsc01_locals.phone == ((val >> 56) & 0x3f)) {
            ...
            LOG(("commit fa=%x va=%x fc=%x f1=%x f2=%x f2q=%x f3=%x dur=%02x cld=%x vd=%d cl=%d pause=%d\n",
                 votraxsc01_locals.rom_fa, votraxsc01_locals.rom_va, votraxsc01_locals.rom_fc, votraxsc01_locals.rom_f1,
                 votraxsc01_locals.rom_f2, votraxsc01_locals.rom_f2q, votraxsc01_locals.rom_f3,
                 votraxsc01_locals.rom_duration, votraxsc01_locals.rom_cld, votraxsc01_locals.rom_vd,
                 votraxsc01_locals.rom_closure, votraxsc01_locals.rom_pause));
#endif
...
static void
interpolate(UINT8* const reg, const UINT8 target) {
...
#if VERBOSE
        LOG(("int fa=%x va=%x fc=%x f1=%x f2=%02x f2q=%02x f3=%x\n", votraxsc01_locals.cur_fa >> 4,
             votraxsc01_locals.cur_va >> 4, votraxsc01_locals.cur_fc >> 4, votraxsc01_locals.cur_f1 >> 4,
             votraxsc01_locals.cur_f2 >> 3, votraxsc01_locals.cur_f2q >> 4, votraxsc01_locals.cur_f3 >> 4));
#endif
    }
...
static void
build_standard_filter(double* const a, double* const b,
                      ...
                      const unsigned int c4)  // Cap over second op-amp
{
    ...
    const double k2 =
        ((UINT64)c4 * (UINT64)c2b) / (votraxsc01_locals.cclock * votraxsc01_locals.cclock * ((UINT64)c1b * (UINT64)c3));

    // Estimate the filter cutoff frequency
    //const double fpeak = sqrt(fabs(k0*k1 - k2))/((2.*M_PI)*k2);

    // figure radians from frequency
    double w0 = sqrt(fabs(votraxsc01_locals.cclock * votraxsc01_locals.cclock
                          * ((INT64)c3 * ((INT64)c1t * (INT64)c2t - (INT64)c1b * (INT64)c2b))
                          / ((UINT64)c2b * (UINT64)c4 * (UINT64)c2b))); //fpeak * (2.0 * M_PI);
...
static void
build_lowpass_filter(double* const a, double* const b,
...
static void
build_noise_shaper_filter(double* const a, double* const b,
...
static void
build_injection_filter(double* const a, double* const b,
                       const unsigned int c1b, // Switched cap, input, bottom
                       const unsigned int c2t, // Unswitched cap, over first amp-op, top
                       const unsigned int c2b, // Switched cap, over first amp-op, bottom
                       const unsigned int c3,  // Cap between the two op-amps
...
static unsigned int
bits_to_caps(unsigned int value, const unsigned int* const caps_values, const size_t N) {
    size_t i;
...
filters_commit(int force) {
    ...
    if (force || votraxsc01_locals.filt_f1 != votraxsc01_locals.cur_f1 >> 4) {
        ...
        build_standard_filter(votraxsc01_locals.f1_a, votraxsc01_locals.f1_b, 11247, 11797, 949, 52067,
                              2280 + bits_to_caps(votraxsc01_locals.filt_f1, f1_caps, 4), 166272);
    }

    if (force || votraxsc01_locals.filt_f2 != votraxsc01_locals.cur_f2 >> 3
        || votraxsc01_locals.filt_f2q != votraxsc01_locals.cur_f2q >> 4) {
        votraxsc01_locals.filt_f2 = votraxsc01_locals.cur_f2 >> 3;
        votraxsc01_locals.filt_f2q = votraxsc01_locals.cur_f2q >> 4;

        build_standard_filter(votraxsc01_locals.f2v_a, votraxsc01_locals.f2v_b, 24840, 29154,
                              829 + bits_to_caps(votraxsc01_locals.filt_f2q, f2v1_caps, 4), 38180,
                              2352 + bits_to_caps(votraxsc01_locals.filt_f2, f2v2_caps, 5), 34270);

        build_injection_filter(votraxsc01_locals.f2n_a, votraxsc01_locals.f2n_b, 29154,
                               829 + bits_to_caps(votraxsc01_locals.filt_f2q, f2n1_caps, 4), 38180,
                               2352 + bits_to_caps(votraxsc01_locals.filt_f2, f2n2_caps, 5), 34270);
    }

    if (force || votraxsc01_locals.filt_f3 != votraxsc01_locals.cur_f3 >> 4) {
        votraxsc01_locals.filt_f3 = votraxsc01_locals.cur_f3 >> 4;
        build_standard_filter(votraxsc01_locals.f3_a, votraxsc01_locals.f3_b, 0, 17594, 868, 18828,
                              8480 + bits_to_caps(votraxsc01_locals.filt_f3, f3_caps, 4), 50019);
    }

    if (force) {
        build_standard_filter(votraxsc01_locals.f4_a, votraxsc01_locals.f4_b, 0, 28810, 1165, 21457, 8558, 7289);

        build_lowpass_filter(votraxsc01_locals.fx_a, votraxsc01_locals.fx_b, 1122, 23131);

        build_noise_shaper_filter(votraxsc01_locals.fn_a, votraxsc01_locals.fn_b, 15500, 14854, 8450, 9523, 14083);
    }

#if VERBOSE
    if (votraxsc01_locals.filt_fa || votraxsc01_locals.filt_va || votraxsc01_locals.filt_fc || votraxsc01_locals.filt_f1
        || votraxsc01_locals.filt_f2 || votraxsc01_locals.filt_f2q || votraxsc01_locals.filt_f3)
        LOG(("filter fa=%x va=%x fc=%x f1=%x f2=%02x f2q=%x f3=%x\n", votraxsc01_locals.filt_fa,
             votraxsc01_locals.filt_va, votraxsc01_locals.filt_fc, votraxsc01_locals.filt_f1, votraxsc01_locals.filt_f2,
             votraxsc01_locals.filt_f2q, votraxsc01_locals.filt_f3));
#endif
}

// Shift a history of values by one and insert the new value at the front
static void
shift_hist(const double val, double* const hist_array, const size_t N) {
    size_t i;
    for (i = N - 1; i > 0; i--)
...
static double
apply_filter(const double* const x, const double* const y, const double* const a, const size_t Na,
             const double* const b, const size_t Nb) {
    size_t i;
    double total = 0;
    for (i = 0; i < Na; i++)
        total += x[i] * a[i];
...
analog_calc() {
    ...
    v = apply_filter(votraxsc01_locals.voice_1, votraxsc01_locals.voice_2, votraxsc01_locals.f1_a, 4,
                     votraxsc01_locals.f1_b, 4);
    shift_hist(v, votraxsc01_locals.voice_2, 4);

    // 4. Apply the f2 filter, voice half
    v = apply_filter(votraxsc01_locals.voice_2, votraxsc01_locals.voice_3, votraxsc01_locals.f2v_a, 4,
                     votraxsc01_locals.f2v_b, 4);
    ...
    n = apply_filter(votraxsc01_locals.noise_1, votraxsc01_locals.noise_2, votraxsc01_locals.fn_a, 3,
                     votraxsc01_locals.fn_b, 3);
    ...
    vn = apply_filter(votraxsc01_locals.vn_1, votraxsc01_locals.vn_2, votraxsc01_locals.f3_a, 4, votraxsc01_locals.f3_b,
                      4);
    ...
    vn = apply_filter(votraxsc01_locals.vn_3, votraxsc01_locals.vn_4, votraxsc01_locals.f4_a, 4, votraxsc01_locals.f4_b,
                      4);
    ...
    vn = apply_filter(votraxsc01_locals.vn_5, votraxsc01_locals.vn_6, votraxsc01_locals.fx_a, 1, votraxsc01_locals.fx_b,
                      2);
...
INLINE int
time_to_samples(int ms) {
    return sample_rate[votraxsc01_locals.actIntonation] * ms / 1000;
}

void
PrepareVoiceData(int nextPhoneme, int nextIntonation) {
    ...
    if (AdditionalSamples)
        memcpy(votraxsc01_locals.lpBuffer,
               PhonemeData[votraxsc01_locals.actPhoneme].lpStart[votraxsc01_locals.actIntonation],
               AdditionalSamples * sizeof(INT16));
    ...
    if (PhonemeData[votraxsc01_locals.actPhoneme].sameAs != PhonemeData[nextPhoneme].sameAs) {
        ...
        switch (PhonemeData[votraxsc01_locals.actPhoneme].iType) {
            ...
            case PT_FS:
                iFadeOutPos = 0;
                iFadeOutSamples = PhonemeData[votraxsc01_locals.actPhoneme].iLength[votraxsc01_locals.actIntonation]
                                  - PhonemeData[votraxsc01_locals.actPhoneme].iSecondStart;
                votraxsc01_locals.pActPos =
                    PhonemeData[votraxsc01_locals.actPhoneme].lpStart[votraxsc01_locals.actIntonation]
                    + PhonemeData[votraxsc01_locals.actPhoneme].iSecondStart;
        ...
        if (iFadeOutPos < iFadeOutSamples) {
            ...
            if (!votraxsc01_locals.iRemainingSamples) {
                votraxsc01_locals.iRemainingSamples =
                    PhonemeData[votraxsc01_locals.actPhoneme].iLength[votraxsc01_locals.actIntonation];
                votraxsc01_locals.pActPos =
                    PhonemeData[votraxsc01_locals.actPhoneme].lpStart[votraxsc01_locals.actIntonation];
            }
...
#ifdef OLD_VOTRAX
void
votraxsc01_set_base_frequency(double baseFrequency) {
    int i;
    if (baseFrequency >= 0) {
...
void
votraxsc01_set_volume(int volume) // currently just (ab)used to en/disable the output (e.g. volume = 0 or 100 only)
{
#ifdef OLD_VOTRAX
    int i;
    if (volume >= 0) {
...
static void
Votrax_Update(int num, INT16* buffer, int length) {
#ifndef REAL_DEVICE
    ...
    while (length) {
        ...
        if (votraxsc01_locals.iDelay) {
            ...
            samplesToCopy =
                (length <= votraxsc01_locals.iRemainingSamples) ? length : votraxsc01_locals.iRemainingSamples;
            ...
            samplesToCopy =
                (length <= votraxsc01_locals.iSamplesInBuffer) ? length : votraxsc01_locals.iSamplesInBuffer;
...
#if !defined(OLD_VOTRAX) && !defined(REAL_DEVICE)
static void
VOTRAXSC01_sh_start_timeout(int which) {
#ifdef REAL_DEVICE
    int help;
    ...
    switch (which) {
        case T_COMMIT_PHONE:
            ...
            timer_adjust(
                votraxsc01_locals.timer,
                (double)(16 * (votraxsc01_locals.rom_duration * 4u + 1) * 4 * 9 + 2)
                    / (double)votraxsc01_locals
                          .mainclock /*attotime::from_ticks(16 * (votraxsc01_locals.rom_duration * 4 + 1) * 4 * 9 + 2, votraxsc01_locals.mainclock)*/
                ,
                T_END_OF_PHONE, TIME_NEVER);
            break;
...
int
VOTRAXSC01_sh_start(const struct MachineSound* msound) {
#if defined(OLD_VOTRAX) && !defined(REAL_DEVICE)
    static char s[32];
    int i;
#endif
    ...
    votraxsc01_locals.sclock = votraxsc01_locals.mainclock / 18.0;
    votraxsc01_locals.cclock = votraxsc01_locals.mainclock / 36.0;
    votraxsc01_locals.stream = stream_init_float("Votrax - SC01", votraxsc01_locals.intf->mixing_level[0],
                                                 votraxsc01_locals.sclock, 0, Votrax_Update, 1);
    ...
    for (i = 0; i <= 3; i++) {
        ...
        votraxsc01_locals.channels[i] =
            stream_init(s, votraxsc01_locals.intf->mixing_level[0], sample_rate[i], i, Votrax_Update);
