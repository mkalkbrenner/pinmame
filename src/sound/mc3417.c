...
#define leak                                                                                                           \
    0.939413062813475786119710824622305084524680890549441822009 //=pow(1.0/M_E, 1.0/(INTEGRATOR_LEAK_TC * 16000.0));
...
#define decay                                                                                                          \
    0.984496437005408405986988829697020369707861003180350567476 //=pow(1.0/M_E, 1.0/(FILTER_DECAY_TC * 16000.0));
//#define	FILTER_CHARGE_TC		0.004
#define charge                                                                                                         \
    0.984496437005408405986988829697020369707861003180350567476 //=pow(1.0/M_E, 1.0/(FILTER_CHARGE_TC * 16000.0));
...
int
mc3417_sh_start(const struct MachineSound* msound) {
...
static void
mc3417_update(int num, INT16* buffer, int length) {
...
#define LOWPASS_ESTIMATE_CYCLES 666
        ...
        && chip->length_estimate_runs < LOWPASS_ESTIMATE_CYCLES) // and we are still tracking the clock?
    {
        ...
        chip->length_estimate_runs
        == LOWPASS_ESTIMATE_CYCLES) // enough tracking of the clock -> enable filter and estimate for which cut frequency
    {
        double freq_scale;

        chip->length_estimate /= LOWPASS_ESTIMATE_CYCLES;

        freq_scale = ((INT32)chip->length_estimate - SAMPLE_RATE / 48000)
                     / (double)(SAMPLE_RATE / 12000 - 1
                                - SAMPLE_RATE / 48000); // estimate to end up within 0..1 (with all tested machines)
        freq_scale = MAX(MIN(freq_scale, 1.), 0.);      // make sure to be in 0..1
        freq_scale =
            1.
            - sqrt(sqrt(freq_scale)); // penalty for low clock rates -> even more of the lower frequencies removed then

        if (freq_scale
            < 0.45) // assume that high clock rates/most modern machines (that would end up at ~12000Hz filtering, see below) do not need to be filtered at all (improves clarity at the price of some noise)
        {
            chip->filter_f = filter_lp_fir_alloc((2000 + 22000 * freq_scale) / SAMPLE_RATE,
                                                 FILTER_ORDER_MAX); // magic, Xenon uses about 22 kHz
...
void
mc3417_clock_w(int num, int state) {
    struct mc3417_data* chip = &mc3417[num];
    ...
    if (diffclock && !clock) {
        ...
        if (temp < -80000. / 32768.)
            temp = -80000. / 32768.;
        else if (temp > 80000. / 32768.)
            temp = 80000. / 32768.;
        /* Just wrap to prevent clipping */
        if (temp < -1.)
            chip->next_value = (-2. - temp);
        else if (temp > 1.)
            chip->next_value = (2. - temp);
        else
            chip->next_value = temp;
...
void
mc3417_set_gain(int num, double gain) {
    mc3417[num].gain = gain;
}

void
mc3417_digit_w(int num, int data) {
    mc3417[num].databit = data & 1;
}

void
mc3417_clock_clear_w(int num, int data) {
    mc3417_clock_w(num, 0);
}

void
mc3417_clock_set_w(int num, int data) {
    mc3417_clock_w(num, 1);
}

void
mc3417_digit_clock_clear_w(int num, int data) {
    mc3417[num].databit = data & 1;
    mc3417_clock_w(num, 0);
}
