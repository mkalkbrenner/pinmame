...
static const UINT8 reload_table[4] = {
    0, 2, 4,
    6}; //sample count reload for 5220c only; 5200 and 5220 always reload with 0; keep in mind this is loaded on IP=0 PC=12 subcycle=1 so it immediately will increment after one sample, effectively being 1,3,5,7 as in the comments above.
...
#define NEW_FRAME_UNVOICED_FLAG (tms->new_frame_pitch_idx == 0)    // ditto as above
    ...
    UINT8
        subcycle; /* contains the current subcycle for a given PC: 0 is A' (only used on SPKSLOW mode on 51xx), 1 is A, 2 is B */
    ...
    UINT8
        tms5220c_rate; /* only relevant for tms5220C's multi frame rate feature; is the actual 4 bit value written on a 0x2* or 0x0* command */
...
void
tms5220_set_irq_chip(void* chip, void (*func)(int)) {
    struct tms5220* tms = chip;
    tms->irq_func = func;
}

void
tms5220_set_irq(void (*func)(int)) {
...
void
tms5220_set_ready_chip(void* chip, void (*func)(int)) {
    struct tms5220* tms = chip;
    tms->readyq_func = func;
}

void
tms5220_set_ready(void (*func)(int)) {
...
void
tms5220_set_read_chip(void* chip, int (*func)(int)) {
    struct tms5220* tms = chip;
    tms->read_callback = func;
}

void
tms5220_set_read(int (*func)(int)) {
...
void
tms5220_set_load_address_chip(void* chip, void (*func)(int)) {
    struct tms5220* tms = chip;
    tms->load_address_callback = func;
}

void
tms5220_set_load_address(void (*func)(int)) {
...
void
tms5220_set_read_and_branch_chip(void* chip, void (*func)(void)) {
    struct tms5220* tms = chip;
    tms->read_and_branch_callback = func;
}

void
tms5220_set_read_and_branch(void (*func)(void)) {
...
static void
tms5220_set_variant_chip(void* chip, int variant) {
    struct tms5220* tms = chip;
    tms->variant = variant;
    switch (variant) {
...
void
tms5220_set_variant(int new_variant) {
...
void
tms5220_data_write_chip(void* chip, int data) {
    struct tms5220* tms = chip;
    ...
    if (tms->speak_external) // If we're in speak external mode
    {
        ...
        if (tms->fifo_count < FIFO_SIZE) {
            ...
            if ((tms->talk_status == 0)
                && (tms->buffer_low
                    == 0)) // we just unset buffer low with that last write, and talk status *was* zero...
            {
                int i;
#ifdef DEBUG_FIFO
                ...
                tms->PC = 0;
                tms->interp_period = reload_table
                    [tms->tms5220c_rate
                     & 0x3]; // is this correct? should this be always 7 instead, so that the new frame is loaded quickly?
                tms->new_frame_energy_idx = 0;
                tms->new_frame_pitch_idx = 0;
                for (i = 0; i < 4; i++)
...
static void
update_status_and_ints(struct tms5220* tms) {
...
int
tms5220_status_read_chip(void* chip) {
    struct tms5220* tms = chip;
    if (tms->RDB_flag) { /* if last command was read, return data register */
        tms->RDB_flag = FALSE;
...
int
tms5220_ready_read_chip(void* chip) {
    struct tms5220* tms = chip;
#ifdef DEBUG_PIN_READS
...
int
tms5220_cycles_to_ready_chip(void* chip) {
    struct tms5220* tms = chip;
    int answer;
    ...
    if (tms5220_ready_read_chip(tms))
        ...
        int samples_per_frame =
            tms->subc_reload
                ? 200
                : 304; // either (13 A cycles + 12 B cycles) * 8 interps for normal SPEAK/SPKEXT, or (13*2 A cycles + 12 B cycles) * 8 interps for SPKSLOW
...
int
tms5220_int_read_chip(void* chip) {
    struct tms5220* tms = chip;
#ifdef DEBUG_PIN_READS
...
void
tms5220_process_chip(void* chip, INT16* buffer, unsigned int size) {
    struct tms5220* tms = chip;
    int buf_count = 0;
    int i, bitout, zpar;
    INT32 this_sample;
     ...
     */
        ...
        if ((tms->interp_period == 0) && (tms->PC == 12) && (tms->subcycle == 1)) {
            ...
            if (tms->talk_status == 0) {
                ...
                fprintf(stderr, "tms5220_process: processing frame: talk status = 0 caused by stop frame or buffer "
                                "empty, halting speech.\n");
#endif
                if (tms->speaking_now == 1) // we're done, set all coeffs to idle state but keep going for a bit...
                {
                    ...
                    for (i = 7; i < tms->coeff->num_k; i++)
                        tms->new_frame_k_idx[i] = 0x7;
                    tms->speaking_now =
                        2; // wait 8 extra interp periods before shutting down so we can interpolate everything to zero state
       ...
       */
            ...
            fprintf(stderr,
                    "*** current Energy, Pitch and Ks =      %04d,   %04d, %04d, %04d, %04d, %04d, %04d, %04d, %04d, "
                    "%04d, %04d, %04d\n",
                    tms->current_energy, tms->current_pitch, tms->current_k[0], tms->current_k[1], tms->current_k[2],
                    tms->current_k[3], tms->current_k[4], tms->current_k[5], tms->current_k[6], tms->current_k[7],
                    tms->current_k[8], tms->current_k[9]);
            fprintf(stderr,
                    "*** target Energy(idx), Pitch, and Ks = %04d(%x),%04d, %04d, %04d, %04d, %04d, %04d, %04d, %04d, "
                    "%04d, %04d, %04d\n",
                    tms->target_energy, tms->new_frame_energy_idx, tms->target_pitch, tms->target_k[0],
                    tms->target_k[1], tms->target_k[2], tms->target_k[3], tms->target_k[4], tms->target_k[5],
                    tms->target_k[6], tms->target_k[7], tms->target_k[8], tms->target_k[9]);
#endif
            ...
            int inhibit_state =
                ((tms->inhibit == 1)
                 && (tms->interp_period
                     != 0)); // disable inhibit when reaching the last interp period, but don't overwrite the tms->inhibit value
#ifdef PERFECT_INTERPOLATION_HACK
            int samples_per_frame =
                tms->subc_reload
                    ? 175
                    : 266; // either (13 A cycles + 12 B cycles) * 7 interps for normal SPEAK/SPKEXT, or (13*2 A cycles + 12 B cycles) * 7 interps for SPKSLOW
            ...
            int current_sample = (tms->subcycle - tms->subc_reload) + (tms->PC * (3 - tms->subc_reload))
                                 + ((tms->subc_reload ? 25 : 38) * ((tms->interp_period - 1) & 7));
            ...
            if (tms->interp_period != 0) { // if we're still interpolating...
                tms->current_energy +=
                    (((tms->target_energy - tms->current_energy) * (1 - inhibit_state)) * current_sample)
                    / samples_per_frame;
                tms->current_pitch +=
                    (((tms->target_pitch - tms->current_pitch) * (1 - inhibit_state)) * current_sample)
                    / samples_per_frame;
                for (i = 0; i < tms->coeff->num_k; i++) {
                    tms->current_k[i] +=
                        (((tms->target_k[i] - tms->current_k[i]) * (1 - inhibit_state)) * current_sample)
                        / samples_per_frame;
                }
            ...
            if (tms->subcycle == 2) {
                switch (tms->PC) {
                    case 0: /* PC = 0, B cycle, write updated energy */
                        tms->current_energy +=
                            (((tms->target_energy - tms->current_energy) * (1 - inhibit_state)) INTERP_SHIFT);
                        break;
                    case 1: /* PC = 1, B cycle, write updated pitch */
                        tms->current_pitch +=
                            (((tms->target_pitch - tms->current_pitch) * (1 - inhibit_state)) INTERP_SHIFT);
                        break;
                    case 2:
                    case 3:
                    case 4:
                    ...
                    case 11:
                        ...
                        tms->current_k[tms->PC - 2] += (((tms->target_k[tms->PC - 2] - tms->current_k[tms->PC - 2])
                                                         * (1 - inhibit_state)) INTERP_SHIFT);
                        break;
        ...
        if (OLD_FRAME_UNVOICED_FLAG == 1) {
            ...
            if (tms->RNG & 1) {
                tms->excitation_data =
                    ~0x3F; /* according to the patent it is (either + or -) half of the maximum value in the chirp table, so either 01000000(0x40) or 11000000(0xC0)*/
            } else {
       ...
       */
        ...
        for (i = 0; i < 20; i++) {
            bitout = ((tms->RNG >> 12) & 1) ^ ((tms->RNG >> 3) & 1) ^ ((tms->RNG >> 2) & 1) ^ ((tms->RNG >> 0) & 1);
            tms->RNG <<= 1;
            tms->RNG |= bitout;
        }
        ...
        fprintf(stderr, "IP:%01d PC:%02d X:%04d E:%03d P:%03d Pc:%03d ", tms->interp_period, tms->PC,
                tms->excitation_data, tms->current_energy, tms->current_pitch, tms->pitch_count);
        ...
        if ((tms->subcycle == 2) && (tms->PC == 12)) {
            if ((tms->interp_period == 7) && (tms->inhibit == 1))
                tms->pitch_count = 0;
            tms->subcycle = tms->subc_reload;
            tms->PC = 0;
            tms->interp_period++;
...
empty:
    ...
    while (size > 0) {
        ...
        buffer[buf_count] =
            -1; /* should be just -1; actual chip outputs -1 every idle sample; (cf note in data sheet, p 10, table 4) */
        buf_count++;
        size--;
    }
}
...
void
tms5220_process(INT16* buffer, unsigned int size) {
...
clip_analog(INT16 cliptemp) {
   ...
   */
    ...
    if ((cliptemp > 2047) || (cliptemp < -2048))
        fprintf(stderr, "clipping cliptemp to range; was %d\n", cliptemp);
#endif
    if (cliptemp > 2047)
        cliptemp = 2047;
...
static INT32
matrix_multiply(INT32 a, INT32 b) {
    INT32 result;
    while (a > 511) {
        a -= 1024;
    }
    ...
    result =
        ((a * b)
         >> 9); /** TODO: this isn't technically right to the chip, which truncates the lowest result bit, but it causes glitches otherwise. **/
#ifdef VERBOSE
    if (result > 16383)
        fprintf(stderr, "matrix multiplier overflowed! a: %x, b: %x, result: %x", a, b, result);
    if (result < -16384)
        fprintf(stderr, "matrix multiplier underflowed! a: %x, b: %x, result: %x", a, b, result);
#endif
    return result;
}
...
static INT32
lattice_filter(struct tms5220* tms) {
    /* Lattice filter here */
...
static void
process_command(struct tms5220* tms, unsigned char cmd) {
    int i;
...
static int
extract_bits(struct tms5220* tms, int count) {
    int val = 0;
...
static void
parse_frame(struct tms5220* tms) {
    int indx, i, rep_flag;
...
static void
set_interrupt_state(struct tms5220* tms, int state) {
#ifdef DEBUG_PIN_READS
...
static void
update_ready_state(struct tms5220* tms) {
