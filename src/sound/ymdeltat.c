...
/*static*/ const INT32 ym_deltat_decode_tableB2[16] = {57, 57, 57, 57, 77, 102, 128, 153,
...
UINT8
YM_DELTAT_ADPCM_Read(YM_DELTAT* DELTAT) {
    UINT8 v = 0;
...
*/
            ...
            DELTAT->portstate =
                v & (0x80 | 0x40 | 0x20 | 0x10 | 0x01); /* start, rec, memory mode, repeat flag copy, reset(bit0) */
            ...
            if (DELTAT->portstate & 0x80) /* START,REC,MEMDATA,REPEAT,SPOFF,--,--,RESET */
            {
                ...
                if (DELTAT->start > DELTAT->end)
                    ...
                    logerror("DeltaT-Warning: Start: %06X, End: %06X, Limit %06X, MemMask %06X\n", DELTAT->start,
                             DELTAT->end, DELTAT->limit, DELTAT->memory_mask);
            }
...
void
YM_DELTAT_ADPCM_Init(YM_DELTAT* DELTAT, int emulation_mode, int portshift, INT32* output_ptr, int output_range) {
    ...
    if (DELTAT->emulation_mode == YM_DELTAT_EMULATION_MODE_YM2610) {
        ...
        DELTAT->portstate = 0x00;
        DELTAT->control2 =
            0x00; /* default setting depends on the emulation mode. MSX demo called "facdemo_4" doesn't setup control2 register at all and still works */
    }
...
void
YM_DELTAT_postload(YM_DELTAT* DELTAT, UINT8* regs) {
    int r;
...
void
YM_DELTAT_savestate(const char* statename, int num, YM_DELTAT* DELTAT) {
...
INLINE void
YM_DELTAT_synthesis_from_external_memory(YM_DELTAT* DELTAT) {
...
INLINE void
YM_DELTAT_synthesis_from_CPU_memory(YM_DELTAT* DELTAT) {
    DELTAT->now_step += DELTAT->step;
    if (DELTAT->now_step >= (1 << YM_DELTAT_SHIFT)) {
        UINT32 step = DELTAT->now_step >> YM_DELTAT_SHIFT;
        DELTAT->now_step &= (1 << YM_DELTAT_SHIFT) - 1;
...
INLINE void
YM_DELTAT_calc_mem_mask(YM_DELTAT* DELTAT) {
