...
/* mixer_play_sample() */
void
sample_start(int channel, int samplenum, int loop) {
    ...
    if (Machine->samples->sample[samplenum] == 0)
        return;

    if (Machine->samples->sample[samplenum]->resolution == 8) {
        logerror("play 8 bit sample %d, channel %d\n", samplenum, channel);
        mixer_play_sample(firstchannel + channel, Machine->samples->sample[samplenum]->data,
                          Machine->samples->sample[samplenum]->length, Machine->samples->sample[samplenum]->smpfreq,
                          loop);
    } else {
        logerror("play 16 bit sample %d, channel %d\n", samplenum, channel);
        mixer_play_sample_16(firstchannel + channel, (short*)Machine->samples->sample[samplenum]->data,
                             Machine->samples->sample[samplenum]->length, Machine->samples->sample[samplenum]->smpfreq,
                             loop);
    }
}
...
void
sample_set_freq(int channel, int freq) {
    if (Machine->sample_rate == 0)
        return;
    if (Machine->samples == 0)
        return;
...
void
sample_set_volume(int channel, int volume) {
    if (Machine->sample_rate == 0)
        return;
    if (Machine->samples == 0)
        return;
...
int
samples_sh_start(const struct MachineSound* msound) {
    int hi_sample = 0;
    int i, channelno;
    ...
    if (!game_samples[hi_samples])
        return 0;

    /* alloc memory for all samples read so far */
    Machine->samples = auto_malloc(hi_samples * sizeof(struct GameSamples)
                                   + ((hi_samples ? Machine->samples->total : 0) + game_samples[hi_samples]->total)
                                         * sizeof(struct GameSample));
    hi_samples++;
    ...
    for (i = 0; i < intf->channels; i++) {
        ...
        if (intf->prefix)
            sprintf(buf, "%s #%d", intf->prefix, i);
        else
            sprintf(buf, "Sample #%d", numchannels + i);
