...
static void
vco_envelope_cb(int param) {
    attack_decay(param);
}

static void
oneshot_envelope_cb(int param) {
    attack_decay(param);
}

#if VERBOSE
static const char* mixer_mode[8] = {"VCO",       "SLF",           "Noise",   "VCO/Noise",
 ...
 *****************************************************************************/
void
SN76477_mixer_w(int chip, UINT32 data) {
...
void
SN76477_mixer_a_w(int chip, UINT32 data) {
    struct SN76477* sn = sn76477[chip];
...
void
SN76477_mixer_b_w(int chip, UINT32 data) {
    struct SN76477* sn = sn76477[chip];
...
void
SN76477_mixer_c_w(int chip, UINT32 data) {
    struct SN76477* sn = sn76477[chip];
...
#if VERBOSE
static const char* envelope_mode[4] = {"VCO", "One-Shot", "Mixer only", "VCO with alternating Polarity"};
#endif
 ...
 *****************************************************************************/
void
SN76477_envelope_w(int chip, UINT32 data) {
    struct SN76477* sn = sn76477[chip];
...
void
SN76477_envelope_1_w(int chip, UINT32 data) {
    struct SN76477* sn = sn76477[chip];
...
void
SN76477_envelope_2_w(int chip, UINT32 data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_vco_w(int chip, UINT32 data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_enable_w(int chip, UINT32 data) {
    struct SN76477* sn = sn76477[chip];
        ...
        switch (sn->envelope) {
            case 0: /* VCO */
                if (sn->vco_res > 0 && sn->vco_cap > 0)
                    timer_adjust(sn->envelope_timer, TIME_IN_HZ(0.64 / (sn->vco_res * sn->vco_cap)), chip,
            ...
            default: /* VCO with alternating polariy */
                /* huh? */
                if (sn->vco_res > 0 && sn->vco_cap > 0)
                    timer_adjust(sn->envelope_timer, TIME_IN_HZ(0.64 / (sn->vco_res * sn->vco_cap) / 2), chip,
                                 TIME_IN_HZ(0.64 / (sn->vco_res * sn->vco_cap) / 2));
                else
                    oneshot_envelope_cb(chip);
                break;
        }
 ...
 *****************************************************************************/
void
SN76477_noise_clock_w(int chip, UINT32 data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_noise_res(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_filter_res(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_filter_cap(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_decay_res(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_attack_decay_cap(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_attack_res(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_amplitude_res(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
    INT32 i;
...
#if VERBOSE
        LOG(1, ("SN76477 #%d: volume range from -%d to +%d (clip at %d%%)\n", chip, sn->vol_lookup[VMAX - VMIN],
                sn->vol_lookup[VMAX - VMIN], clip * 100 / 32767));
#endif
    } else {
 ...
 *****************************************************************************/
void
SN76477_set_feedback_res(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
    INT32 i;
 ...
 *****************************************************************************/
void
SN76477_set_pitch_voltage(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_vco_res(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_vco_cap(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_vco_voltage(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
    ...
    LOG(1, ("SN76477 #%d: VCO ext. voltage %f (%f * %f = %f Hz)\n", chip, sn->vco_voltage, sn->vco_freq,
            10.0 * (5.0 - sn->vco_voltage) / 5.0, sn->vco_freq * 10.0 * (5.0 - sn->vco_voltage) / 5.0));
}
 ...
 *****************************************************************************/
void
SN76477_set_slf_res(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_slf_cap(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_oneshot_res(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
void
SN76477_set_oneshot_cap(int chip, double data) {
    struct SN76477* sn = sn76477[chip];
 ...
 *****************************************************************************/
static void
SN76477_update_0(int chip, INT16* buffer, int length) {
    struct SN76477* sn = sn76477[chip];
    while (length--) {
        UPDATE_VCO;
        UPDATE_VOLUME;
 ...
 *****************************************************************************/
static void
SN76477_update_1(int chip, INT16* buffer, int length) {
    struct SN76477* sn = sn76477[chip];
    while (length--) {
        UPDATE_SLF;
        UPDATE_VOLUME;
 ...
 *****************************************************************************/
static void
SN76477_update_2(int chip, INT16* buffer, int length) {
    struct SN76477* sn = sn76477[chip];
    while (length--) {
        UPDATE_NOISE;
        UPDATE_VOLUME;
 ...
 *****************************************************************************/
static void
SN76477_update_3(int chip, INT16* buffer, int length) {
    struct SN76477* sn = sn76477[chip];
    while (length--) {
        UPDATE_VCO;
        UPDATE_NOISE;
 ...
 *****************************************************************************/
static void
SN76477_update_4(int chip, INT16* buffer, int length) {
    struct SN76477* sn = sn76477[chip];
    while (length--) {
        UPDATE_SLF;
        UPDATE_NOISE;
 ...
 *****************************************************************************/
static void
SN76477_update_5(int chip, INT16* buffer, int length) {
    struct SN76477* sn = sn76477[chip];
    while (length--) {
        UPDATE_SLF;
        UPDATE_VCO;
        UPDATE_NOISE;
        UPDATE_VOLUME;
        *buffer++ = (sn->vco_out & sn->slf_out & sn->noise_out) ? sn->vol_lookup[sn->vol - VMIN]
 ...
 *****************************************************************************/
static void
SN76477_update_6(int chip, INT16* buffer, int length) {
    struct SN76477* sn = sn76477[chip];
    while (length--) {
        UPDATE_SLF;
        UPDATE_VCO;
 ...
 *****************************************************************************/
static void
SN76477_update_7(int chip, INT16* buffer, int length) {
    while (length--)
        *buffer++ = 0;
}

static void
SN76477_sound_update(int param, INT16* buffer, int length) {
    struct SN76477* sn = sn76477[param];
    if (sn->enable) {
...
int
SN76477_sh_start(const struct MachineSound* msound) {
    int i;
...
void
SN76477_sh_stop(void) {}

void
SN76477_sh_update(void) {
    int i;
    for (i = 0; i < intf->num; i++)
