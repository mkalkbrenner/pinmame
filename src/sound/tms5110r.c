...
const static unsigned short kbits[10] = {5, 5, 4, 4, 4, 4, 4, 3, 3, 3};
...
const static unsigned short energytable[0x10] = {0,  1,  2,  3,  4,  6,  8,   11,
                                                 16, 23, 33, 47, 63, 85, 114, COEFF_ENERGY_SENTINEL};

/* This is the pitch lookup table */ // == later_5110
const static unsigned short pitchtable[0x20] = {0,  15, 16, 17, 19, 21, 22, 25, 26, 29, 32,  36,  40,  42,  46,  50,
...
const static char interp_coeff[8] = {3, 3, 3, 2, 2, 2, 1, 0};
...
const static unsigned short later_0285_energytable[0x10] = {0, 1, 2, 3, 4, 6, 8, 11, 16, 23, 33, 47, 63, 85, 114, 0};

/* pitch */
const static unsigned short patent_0280_2801_pitchtable[0x20] = {0,   41,  43,  45,  47,  49,  51,  53,  55,  58, 60,
                                                                 63,  66,  70,  73,  76,  79,  83,  87,  90,  94, 99,
                                                                 103, 107, 112, 118, 123, 129, 134, 140, 147, 153};

const static unsigned short later_2802_pitchtable[0x20] = {0,  16, 18, 19, 21,  24,  26,  28,  31,  35, 37,
                                                           42, 44, 47, 50, 53,  56,  59,  63,  67,  71, 75,
                                                           79, 84, 89, 94, 100, 106, 112, 126, 141, 150};

const static unsigned short later_5110_pitchtable[0x20] = {0,  15, 16, 17, 19,  21,  22,  25,  26,  29, 32,
                                                           36, 40, 42, 46, 50,  55,  60,  64,  68,  72, 76,
                                                           80, 84, 86, 93, 101, 110, 120, 132, 144, 159};
...
const static short later_2801_2501E_ktable[10][0x20] = { // K1..K10
    ...
    {-249, -183, -110, -32, 48, 126, 198,
     261}, /* on patents 4,403,965 and 4,946,391 the 4th entry is 0x3ED (-19) which is a typo of the correct value of 0x3E0 (-32)*/
...
const static short later_2802_ktable[10][0x20] = { // K1..K10
    ...
    {-376, -357, -335, -312, -286, -258, -227, -195, -161, -124, -87, -49, -10, 29,  68,  106,
     143,  178,  212,  243,  272,  299,  324,  346,  366,  384,  400, 414, 427, 438, 448, 506},
    {-407, -381, -349, -311, -268, -218, -162, -102, -39, 25, 89, 149, 206, 257, 302, 341},
    {-289, -248, -202, -152, -98, -43, 14, 71, 125, 177, 225, 269, 307, 341, 371, 506},
    {-318, -283, -245, -202, -156, -107, -56, -3, 49, 101, 150, 196, 239, 278, 313, 344},
    {-193, -152, -109, -65, -20, 26, 71, 115, 158, 198, 235, 270, 301, 330, 355, 377},
    {-254, -218, -180, -140, -97, -53, -8, 36, 81, 124, 165, 204, 240, 274, 304, 332},
    {-205, -112, -10, 92, 187, 269, 336, 387},
...
const static char patent_0280_chirptable[52] = {
    0x00, 0x2a, 0xd4, 0x32, 0xb2, 0x12, 0x25, 0x14, 0x02, 0xe1, 0xc5, 0x02, 0x5f, 0x5a, 0x05, 0x0f, 0x26, 0xfc,
    0xa5, 0xa5, 0xd6, 0xdd, 0xdc, 0xfc, 0x25, 0x2b, 0x22, 0x21, 0x0f, 0xff, 0xf8, 0xee, 0xed, 0xef, 0xf7, 0xf6,
    0xfa, 0x00, 0x03, 0x02, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

// almost, but not exactly the same as the patent chirp above (25 bits differ)
const static char patent_0281_chirptable[52] = {
    0x00, 0x2b, 0xd4, 0x33, 0xb3, 0x12, 0x25, 0x14, 0x02, 0xe2, 0xc6, 0x03, 0x60, 0x5b, 0x05, 0x0f, 0x26, 0xfc,
    0xa6, 0xa5, 0xd6, 0xdd, 0xdd, 0xfd, 0x25, 0x2b, 0x23, 0x22, 0x0f, 0xff, 0xf8, 0xef, 0xed, 0xef, 0xf7, 0xf7,
    0xfa, 0x01, 0x04, 0x03, 0x01, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

const static char patent_0282_chirptable[52] = {
    0x00, 0xa5, 0xbd, 0xee, 0x34, 0x73, 0x7e, 0x3d, 0xe8, 0xea, 0x34, 0x24, 0xd1, 0x01, 0x13, 0xc3, 0x0c, 0xd2,
    0xe7, 0xdd, 0xd9, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00};

const static char later_chirptable[52] = {0x00, 0x03, 0x0f, 0x28, 0x4c, 0x6c, 0x71, 0x50, 0x25, 0x26, 0x4c, 0x44, 0x1a,
...
const static char interp_coeff[8] = {3, 3, 3, 2, 2, 1, 1, 0};
