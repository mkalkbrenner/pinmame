...
/* IRQ Handler */
static void
IRQHandler(int n, int irq) {
    if (intf->irqhandler[n])
        intf->irqhandler[n](irq);
}

static void
timer_callback_2151(int param) {
    int n = param & 0x7f;
    int c = param >> 7;
...
static void
YM2151UpdateRequest(int chip) {
    stream_update(stream[chip], 0);
}

#if (HAS_YM2151_NUKED)
static void
YM2151UpdateNuked(int num, INT16** buffers, int length) {
...
static void
update_timer_func(int timer_num) {
    int i;
...
static int
my_YM2151_sh_start(const struct MachineSound* msound, const int mode) {
    int i, j;
    ...
    if (mode == 1)
        FMMode = CHIP_YM2151_ALT;
    else if (mode == 2)
        FMMode = CHIP_YM2151_NUKED;
    else
    ...
    switch (FMMode) {
        ...
        case CHIP_YM2151_NUKED: {
            ...
            for (i = 0; i < intf->num; i++) {
                ...
                stream[i] = stream_init_multi_float(YM2151_NUMBUF, name, vol, rate, i, YM2151UpdateNuked, 1);
...
void
YM2151_set_mixing_levels(int chip, int l, int r) {
...
#if (HAS_YM2151)
int
YM2151_sh_start(const struct MachineSound* msound) {
...
WRITE_HANDLER(YM2151_register_port_0_w) { lastreg0 = data; }

WRITE_HANDLER(YM2151_register_port_1_w) { lastreg1 = data; }

WRITE_HANDLER(YM2151_register_port_2_w) { lastreg2 = data; }
...
#if (HAS_YM2151_ALT)
READ16_HANDLER(YM2151_status_port_0_lsb_r) { return YM2151_status_port_0_r(0); }

READ16_HANDLER(YM2151_status_port_1_lsb_r) { return YM2151_status_port_1_r(0); }

READ16_HANDLER(YM2151_status_port_2_lsb_r) { return YM2151_status_port_2_r(0); }
