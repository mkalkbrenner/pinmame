...
/************************************************************************/
int
dst_adder_step(struct node_description* node) {
    if (node->input[0]) {
...
double
dst_transform_pop(double* stack, int* pointer) {
    double value;
    //decrement THEN read
    if (*pointer > 0)
        (*pointer)--;
...
double
dst_transform_push(double* stack, int* pointer, double value) {
    //Strore THEN increment
    if (*pointer < MAX_TRANS_STACK)
        stack[(*pointer)++] = value;
    return value;
}

int
dst_transform_step(struct node_description* node) {
...
/************************************************************************/
int
dst_gain_step(struct node_description* node) {
    if (node->input[0]) {
...
/************************************************************************/
int
dst_divide_step(struct node_description* node) {
    if (node->input[0]) {
        if (node->input[2] == 0) {
...
/************************************************************************/
int
dst_switch_step(struct node_description* node) {
    if (node->input[0]) {
...
int
dst_ramp_step(struct node_description* node) {
    ...
    if (node->input[0]) {
        ...
        if (context->dir ? node->input[1] : !node->input[1])
            node->output += context->step;
        else
            node->output -= context->step;
        /* Clamp to min/max */
        if (context->dir ? (node->output < node->input[3]) : (node->output > node->input[3]))
            node->output = node->input[3];
        if (context->dir ? (node->output > node->input[4]) : (node->output < node->input[4]))
...
int
dst_ramp_reset(struct node_description* node) {
    struct dss_ramp_context* context;
    context = (struct dss_ramp_context*)node->context;
...
int
dst_ramp_init(struct node_description* node) {
...
/************************************************************************/
int
dst_oneshot_step(struct node_description* node) {
...
int
dst_oneshot_reset(struct node_description* node) {
...
int
dst_oneshot_init(struct node_description* node) {
...
/************************************************************************/
int
dst_clamp_step(struct node_description* node) {
    //struct dss_ramp_context *context;
    //context=(struct dss_ramp_context*)node->context;
    ...
    if (node->input[0]) {
        ...
        else if (node->input[1] > node->input[3])
            node->output = node->input[3];
        else
...
/************************************************************************/
int
dst_ladder_step(struct node_description* node) {
    ...
    if (select > ((1 << DISC_LADDER_MAXRES) - 1))
        select = (1 << DISC_LADDER_MAXRES) - 1;
    if (select < 0)
        select = 0;
    ...
    for (loop = 0; loop < DISC_LADDER_MAXRES; loop++) {
        if (select & 0x01)
            onres += ((struct discrete_ladder*)node->custom)->resistors[loop];
        select = select >> 1;
    }
...
int
dst_ladder_reset(struct node_description* node) {
    struct dst_ladder_context* context;
    int loop;
    context = (struct dst_ladder_context*)node->context;
    /* Sum the overall resistance for later use */
    ...
    context->exponent = -1.0 * ((struct discrete_ladder*)node->custom)->smoothing_cap
                        * ((struct discrete_ladder*)node->custom)->smoothing_res;

    return 0;
}

int
dst_ladder_init(struct node_description* node) {
    /* Allocate memory for the context array and the node execution order array */
...
/************************************************************************/
int
dst_samphold_step(struct node_description* node) {
    ...
    if (node->input[0]) {
        switch (context->clocktype) {
            case DISC_SAMPHOLD_REDGE:
                ...
                if (node->input[2] > context->lastinput)
                    node->output = node->input[1];
                break;
...
int
dst_samphold_reset(struct node_description* node) {
    struct dst_samphold_context* context;
    context = (struct dst_samphold_context*)node->context;
...
int
dst_samphold_init(struct node_description* node) {
    /* Allocate memory for the context array and the node execution order array */
...
/************************************************************************/
int
dst_logic_inv_step(struct node_description* node) {
    if (node->input[0]) {
...
/************************************************************************/
int
dst_logic_and_step(struct node_description* node) {
    if (node->input[0]) {
...
/************************************************************************/
int
dst_logic_nand_step(struct node_description* node) {
    if (node->input[0]) {
...
/************************************************************************/
int
dst_logic_or_step(struct node_description* node) {
    if (node->input[0]) {
        node->output = (node->input[1] || node->input[2] || node->input[3] || node->input[4]) ? 1.0 : 0.0;
    } else {
        node->output = 0.0;
...
/************************************************************************/
int
dst_logic_nor_step(struct node_description* node) {
    if (node->input[0]) {
        node->output = (node->input[1] || node->input[2] || node->input[3] || node->input[4]) ? 0.0 : 1.0;
    } else {
        node->output = 0.0;
...
/************************************************************************/
int
dst_logic_xor_step(struct node_description* node) {
    if (node->input[0]) {
...
/************************************************************************/
int
dst_logic_nxor_step(struct node_description* node) {
    if (node->input[0]) {
