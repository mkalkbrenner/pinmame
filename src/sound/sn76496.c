...
static void
SN76496Write(int chip, int data) {
...
static void
SN76496Update(int chip, INT16* buffer, int length) {
    int i;
    struct SN76496* R = &sn[chip];
    ...
    while (length > 0) {
        ...
        do {
            ...
            if (R->Count[3] <= 0) {
                ...
                {
                    if (((R->RNG & R->WhitenoiseTaps) != R->WhitenoiseTaps)
                        && ((R->RNG & R->WhitenoiseTaps) != 0)) /* crappy xor! */
                    {
                        R->RNG >>= 1;
        ...
        out = vol[0] * R->Volume[0] + vol[1] * R->Volume[1] + vol[2] * R->Volume[2] + vol[3] * R->Volume[3];
...
static void
SN76496_set_gain(int chip, int gain) {
    struct SN76496* R = &sn[chip];
    int i;
    double out;
...
static int
SN76496_init(const struct MachineSound* msound, int chip, double clock, int volume) {
    int i;
    struct SN76496* R = &sn[chip];
    char name[40];
    ...
    R->LastRegister = /*m_sega_style_psg?3:*/
        0;            // Sega VDP PSG defaults to selected period reg for 2nd channel //!! m_sega_style_psg true/false?
    for (i = 0; i < 8; i += 2) {
        R->Register[i] = 0;
        R->Register[i + 1] =
            0x0; //!! 0x0f?? // volume = 0x0 (max volume) on reset; this needs testing on chips other than SN76489A and Sega VDP PSG //!! m_sega_style_psg true/false?
    }
...
static int
generic_start(const struct MachineSound* msound, int feedbackmask, int noisetaps, int noiseinvert) {
    int chip;
    ...
    for (chip = 0; chip < intf->num; chip++) {
        ...
        vgm_header_set(R->vgm_idx, 0x07,
                       0); //!! m_sega_style_psg); // how ironic that it does just the opposite of its name
    }
    return 0;
}

int
SN76489_sh_start(const struct MachineSound* msound) {
    return generic_start(msound, 0x4000, 0x03, 1);
}

int
SN76489A_sh_start(const struct MachineSound* msound) {
    return generic_start(msound, 0x8000, 0x06, 0);
}

int
SN76494_sh_start(const struct MachineSound* msound) {
    return generic_start(msound, 0x8000, 0x06, 0);
}

int
SN76496_sh_start(const struct MachineSound* msound) {
    return generic_start(msound, 0x8000, 0x06, 0);
}

int
gamegear_sh_start(const struct MachineSound* msound) {
    return generic_start(msound, 0x8000, 0x09, 0);
}

int
smsiii_sh_start(const struct MachineSound* msound) {
    return generic_start(msound, 0x8000, 0x09, 0);
}
