...
static void
IRQHandler_3812(int n, int irq) {
    if (intf_3812->handler[n])
        (intf_3812->handler[n])(irq ? ASSERT_LINE : CLEAR_LINE);
}

static void
timer_callback_3812(int param) {
    int n = param >> 1;
    int c = param & 1;
    YM3812TimerOver(n, c);
}
...
static void
TimerHandler_3812(int c, double period) {
...
int
YM3812_sh_start(const struct MachineSound* msound) {
    int i;
    double rate; // = Machine->sample_rate;

    intf_3812 = msound->sound_interface;
    if (intf_3812->num > MAX_3812)
        return 1;
...
WRITE_HANDLER(YM3812_control_port_0_w) { YM3812Write(0, 0, data); }

WRITE_HANDLER(YM3812_write_port_0_w) { YM3812Write(0, 1, data); }

READ_HANDLER(YM3812_status_port_0_r) { return YM3812Read(0, 0); }

READ_HANDLER(YM3812_read_port_0_r) { return YM3812Read(0, 1); }

WRITE_HANDLER(YM3812_control_port_1_w) { YM3812Write(1, 0, data); }

WRITE_HANDLER(YM3812_write_port_1_w) { YM3812Write(1, 1, data); }

READ_HANDLER(YM3812_status_port_1_r) { return YM3812Read(1, 0); }

READ_HANDLER(YM3812_read_port_1_r) { return YM3812Read(1, 1); }
...
/* IRQ Handler */
static void
IRQHandler_3526(int n, int irq) {
    if (intf_3526->handler[n])
        (intf_3526->handler[n])(irq ? ASSERT_LINE : CLEAR_LINE);
}

/* Timer overflow callback from timer.c */
static void
timer_callback_3526(int param) {
    int n = param >> 1;
    int c = param & 1;
    YM3526TimerOver(n, c);
}
...
static void
TimerHandler_3526(int c, double period) {
    if (period == 0) { /* Reset FM Timer */
...
int
YM3526_sh_start(const struct MachineSound* msound) {
    int i;
    double rate; // = Machine->sample_rate;

    intf_3526 = msound->sound_interface;
    if (intf_3526->num > MAX_3526)
        return 1;
...
WRITE_HANDLER(YM3526_control_port_0_w) { YM3526Write(0, 0, data); }

WRITE_HANDLER(YM3526_write_port_0_w) { YM3526Write(0, 1, data); }

READ_HANDLER(YM3526_status_port_0_r) { return YM3526Read(0, 0); }

READ_HANDLER(YM3526_read_port_0_r) { return YM3526Read(0, 1); }

WRITE_HANDLER(YM3526_control_port_1_w) { YM3526Write(1, 0, data); }

WRITE_HANDLER(YM3526_write_port_1_w) { YM3526Write(1, 1, data); }

READ_HANDLER(YM3526_status_port_1_r) { return YM3526Read(1, 0); }

READ_HANDLER(YM3526_read_port_1_r) { return YM3526Read(1, 1); }
...
static void
IRQHandler_8950(int n, int irq) {
    if (intf_8950->handler[n])
        (intf_8950->handler[n])(irq ? ASSERT_LINE : CLEAR_LINE);
}

static void
timer_callback_8950(int param) {
    int n = param >> 1;
    int c = param & 1;
    Y8950TimerOver(n, c);
}
...
static void
TimerHandler_8950(int c, double period) {
    if (period == 0) { /* Reset FM Timer */
...
static void
Y8950PortHandler_w(int chip, unsigned char data) {
    if (intf_8950->portwrite[chip])
        intf_8950->portwrite[chip](chip, data);
}

static unsigned char
Y8950KeyboardHandler_r(int chip) {
...
static void
Y8950KeyboardHandler_w(int chip, unsigned char data) {
    if (intf_8950->keyboardwrite[chip])
        intf_8950->keyboardwrite[chip](chip, data);
}

int
Y8950_sh_start(const struct MachineSound* msound) {
    int i;
    double rate; // = Machine->sample_rate;

    intf_8950 = msound->sound_interface;
    if (intf_8950->num > MAX_8950)
        return 1;
    ...
    for (i = 0; i < intf_8950->num; i++) {
        ...
        /* ADPCM ROM data */
        Y8950SetDeltaTMemory(i, (void*)(memory_region(intf_8950->rom_region[i])),
...
WRITE_HANDLER(Y8950_control_port_0_w) { Y8950Write(0, 0, data); }

WRITE_HANDLER(Y8950_write_port_0_w) { Y8950Write(0, 1, data); }

READ_HANDLER(Y8950_status_port_0_r) { return Y8950Read(0, 0); }

READ_HANDLER(Y8950_read_port_0_r) { return Y8950Read(0, 1); }

WRITE_HANDLER(Y8950_control_port_1_w) { Y8950Write(1, 0, data); }

WRITE_HANDLER(Y8950_write_port_1_w) { Y8950Write(1, 1, data); }

READ_HANDLER(Y8950_status_port_1_r) { return Y8950Read(1, 0); }

READ_HANDLER(Y8950_read_port_1_r) { return Y8950Read(1, 1); }
