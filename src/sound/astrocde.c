...
static void
astrocade_update(int num, int newpos) {
    ...
    for (i = pos; i < newpos; i++) {
        ...
        data =
            (current_state_A[num] * vol_A[num] + current_state_B[num] * vol_B[num] + current_state_C[num] * vol_C[num]);
...
int
astrocade_sh_start(const struct MachineSound* msound) {
    int i;
...
void
astrocade_sound_w(int num, int offset, int data) {
...
WRITE_HANDLER(astrocade_sound1_w) { astrocade_sound_w(0, offset, data); }

WRITE_HANDLER(astrocade_sound2_w) { astrocade_sound_w(1, offset, data); }

void
astrocade_sh_update(void) {
    int num;
