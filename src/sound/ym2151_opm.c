...
/* Envelope generator */
static const bool eg_stephi[4][4] = {{0, 0, 0, 0}, {1, 0, 0, 0}, {1, 0, 1, 0}, {1, 1, 1, 0}};
...
static const freqtable_t pg_freqtable[64] = {
    {1299, 1, 19}, {1318, 1, 19}, {1337, 1, 19}, {1356, 1, 20}, {1376, 1, 20}, {1396, 1, 20}, {1416, 1, 21},
    {1437, 1, 20}, {1458, 1, 21}, {1479, 1, 21}, {1501, 1, 22}, {1523, 1, 22}, {0, 0, 16},    {0, 0, 16},
    {0, 0, 16},    {0, 0, 16},    {1545, 1, 22}, {1567, 1, 22}, {1590, 1, 23}, {1613, 1, 23}, {1637, 1, 23},
    {1660, 1, 24}, {1685, 1, 24}, {1709, 1, 24}, {1734, 1, 25}, {1759, 1, 25}, {1785, 1, 26}, {1811, 1, 26},
    {0, 0, 16},    {0, 0, 16},    {0, 0, 16},    {0, 0, 16},    {1837, 1, 26}, {1864, 1, 27}, {1891, 1, 27},
    {1918, 1, 28}, {1946, 1, 28}, {1975, 1, 28}, {2003, 1, 29}, {2032, 1, 30}, {2062, 1, 30}, {2092, 1, 30},
    {2122, 1, 31}, {2153, 1, 31}, {0, 0, 16},    {0, 0, 16},    {0, 0, 16},    {0, 0, 16},    {2185, 1, 31},
    {2216, 0, 31}, {2249, 0, 31}, {2281, 0, 31}, {2315, 0, 31}, {2348, 0, 31}, {2382, 0, 30}, {2417, 0, 30},
    {2452, 0, 30}, {2488, 0, 30}, {2524, 0, 30}, {2561, 0, 30}, {0, 0, 16},    {0, 0, 16},    {0, 0, 16},
    {0, 0, 16}};
...
static uint16_t lfo_counter2_table[] = {0x0000, 0x4000, 0x6000, 0x7000, 0x7800, 0x7c00, 0x7e00, 0x7f00,
                                        0x7f80, 0x7fc0, 0x7fe0, 0x7ff0, 0x7ff8, 0x7ffc, 0x7ffe, 0x7fff};

INLINE uint16_t
OPM_KCToFNum(const int32_t kcode) {
...
INLINE uint16_t
OPM_LFOApplyPMS(const uint8_t lfo, const uint8_t pms) {
    bool t;
    uint16_t out;
    uint8_t top = (lfo >> 4) & 7;
    if (pms != 7) {
...
INLINE int32_t
OPM_CalcKCode(const uint16_t kcf, uint16_t lfo, const bool lfo_sign, const uint8_t dt) {
    uint8_t t2;
    int32_t t3;
...
INLINE void
OPM_PhaseCalcFNumBlock(opm_t* const chip) {
...
INLINE void
OPM_PhaseCalcIncrement(opm_t* const chip) {
...
INLINE void
OPM_PhaseGenerate(opm_t* const chip) {
...
INLINE void
OPM_PhaseDebug(opm_t* const chip) {
    chip->pg_serial >>= 1;
    if (chip->cycles == 5) {
...
INLINE void
OPM_EnvelopePhase1(opm_t* const chip) {
...
INLINE void
OPM_EnvelopePhase2(opm_t* const chip) {
    const uint32_t slot = chip->cycles;
...
INLINE void
OPM_EnvelopePhase3(opm_t* const chip) {
...
INLINE void
OPM_EnvelopePhase4(opm_t* const chip) {
...
INLINE void
OPM_EnvelopePhase5(opm_t* const chip) {
...
INLINE void
OPM_EnvelopePhase6(opm_t* const chip) {
...
INLINE void
OPM_EnvelopeClock(opm_t* const chip) {
    chip->eg_clock <<= 1;
...
INLINE void
OPM_EnvelopeTimer(opm_t* const chip) {
...
INLINE void
OPM_OperatorPhase1(opm_t* const chip) {
    const uint32_t slot = chip->cycles;
...
INLINE void
OPM_OperatorPhase4(opm_t* const chip) {
    //uint32_t slot = (chip->cycles + 29) & 31;
    chip->op_logsin[1] = chip->op_logsin[0];
}

INLINE void
OPM_OperatorPhase5(opm_t* const chip) {
    //uint32_t slot = (chip->cycles + 28) & 31;
    chip->op_logsin[2] = chip->op_logsin[1];
}

INLINE void
OPM_OperatorPhase6(opm_t* const chip) {
    //uint32_t slot = (chip->cycles + 27) & 31;
...
INLINE void
OPM_OperatorPhase7(opm_t* const chip) {
...
INLINE void
OPM_OperatorPhase8(opm_t* const chip) {
...
INLINE void
OPM_OperatorPhase9(opm_t* const chip) {
...
INLINE void
OPM_OperatorPhase14(opm_t* const chip) {
...
INLINE void
OPM_OperatorPhase15(opm_t* const chip) {
...
INLINE void
OPM_OperatorPhase16(opm_t* const chip) {
...
INLINE void
OPM_OperatorCounter(opm_t* const chip) {
    if ((chip->cycles & 7) == 4) {
        chip->op_counter++;
    }
    if (chip->cycles == 12) {
...
INLINE void
OPM_NoiseTimer(opm_t* const chip) {
...
INLINE void
OPM_DoTimerA2(opm_t* const chip) {
    if (chip->cycles == 1) {
...
INLINE void
OPM_DoTimerB2(opm_t* const chip) {
...
INLINE void
OPM_DoTimerIRQ(opm_t* const chip) {
#ifdef PINMAME
    const bool old_timer_irq = chip->timer_irq;
#endif
    chip->timer_irq = chip->timer_a_status || chip->timer_b_status;
#ifdef PINMAME
    if ((chip->timer_irq != old_timer_irq) && (chip->irqhandler))
        (*chip->irqhandler)(chip->timer_irq);
#endif
}

INLINE void
OPM_DoLFOMult(opm_t* const chip) {
...
INLINE void
OPM_NoiseChannel(opm_t* const chip) {
...
INLINE void
OPM_DoRegWrite(opm_t* const chip) {
...
#ifdef PINMAME
    if (chip->porthandler && signal_porthandler)
        (*chip->porthandler)(0, chip->write_data >> 6);
#endif
}

INLINE void
OPM_DoIC(opm_t* const chip) {
    const uint32_t channel = chip->cycles & 7;
    const uint32_t slot = chip->cycles;
    if (chip->ic) {
...
void
OPM_Clock(opm_t* const chip, int16_t* const output, bool* const sh1, bool* const sh2, bool* const so) {
    OPM_Mixer2(chip);
    OPM_Mixer(chip);
...
void
OPM_Write(opm_t* const chip, const uint8_t port, const uint8_t data) {
#ifdef PINMAME
...
uint8_t
OPM_Read(const opm_t* const chip, const uint8_t port) {
    if (chip->mode_test[6]) {
        const uint16_t testdata =
            chip->op_out[5] | (((uint16_t)!chip->eg_serial_bit) << 14) | ((chip->pg_serial & 1) << 15);
        if (chip->mode_test[7]) {
            return testdata;
        } else {
            return testdata >> 8;
...
bool
OPM_ReadIRQ(const opm_t* const chip) {
    return chip->timer_irq;
}

bool
OPM_ReadCT1(const opm_t* const chip) {
    return chip->mode_test[3] ? chip->lfo_clock_test : chip->io_ct1;
}

bool
OPM_ReadCT2(const opm_t* const chip) {
    return chip->io_ct2;
}

void
OPM_SetIC(opm_t* const chip, bool ic) {
    if (chip->ic != ic) {
        chip->ic = ic;
        if (!ic) {
            chip->cycles = 0;
...
void
OPM_Reset(opm_t* const chip, double clock) {
#ifdef PINMAME
...
void
OPM_WriteBuffered(opm_t* const chip, uint8_t port, uint8_t data) {
    uint64_t time1;

    if (chip->writebuf[chip->writebuf_last].port & 0x02) {
        uint64_t skip;
        chip->writebuf[chip->writebuf_last].port &= 0x01;
        OPM_Write(chip, chip->writebuf[chip->writebuf_last].port, chip->writebuf[chip->writebuf_last].data);
...
void
OPM_FlushBuffer(opm_t* chip) {
    while (chip->writebuf[chip->writebuf_cur].port & 0x02) {
        uint64_t skip;
        chip->writebuf[chip->writebuf_cur].port &= 0x01;
        OPM_Write(chip, chip->writebuf[chip->writebuf_cur].port, chip->writebuf[chip->writebuf_cur].data);
...
INLINE void
OPM_GenerateOne(opm_t* const chip, float fbuf[2]) {
    uint32_t i;
    int16_t buf[2];
    for (i = 0; i < 32; i++) // over every 32 cycles a stereo sample is created
    {
        OPM_Clock(chip, (i == 15) ? buf : NULL, NULL, NULL,
                  NULL); // so grab that at some point (where in the middle both channels are in sync for sure)
...
// PinMAME specific
void
OPM_GenerateStream(opm_t* const chip, float** sndptr, uint32_t numsamples) {
    uint32_t i;
...
void
OPM_SetPortWriteHandler(opm_t* const chip, mem_write_handler handler) {
    chip->porthandler = handler;
}

void
OPM_SetIrqHandler(opm_t* const chip, void (*handler)(int irq)) {
