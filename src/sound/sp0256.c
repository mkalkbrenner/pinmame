...
static const INT16 qtbl[128] = {
    ...
    177, 185, 193, 201, 209, 217, 225, 233, 241, 249, 257, 265, 273, 281, 289, 297, 301, 305, 309, 313, 317, 321,
    325, 329, 333, 337, 341, 345, 349, 353, 357, 361, 365, 369, 373, 377, 381, 385, 389, 393, 397, 401, 405, 409,
    413, 417, 421, 425, 427, 429, 431, 433, 435, 437, 439, 441, 443, 445, 447, 449, 451, 453, 455, 457, 459, 461,
    463, 465, 467, 469, 471, 473, 475, 477, 479, 481, 482, 483, 484, 485, 486, 487, 488, 489, 490, 491, 492, 493,
...
/* ======================================================================== */
static int
lpc12_update(struct lpc12_t* f, int num_samp, INT16* out, UINT32* optr) {
    int i, j;
    INT16 samp;
    int do_int;
    int oidx = *optr;
...
#define CR(l, s, p, d, f, c5, ca)                                                                                      \
    ((((l)&15) << 0) | (((s)&15) << 4) | (((p)&15) << 8) | (((d)&1) << 12) | (((f)&1) << 13) | (((c5)&1) << 14)        \
     | (((ca)&1) << 15))
...
/* ======================================================================== */
void
sp0256_bitrevbuff(UINT8* buffer, unsigned int start, unsigned int length) {
    unsigned int i;
...
sp0256_getb(int len) {
    ...
    if (sp0256.fifo_sel) {
        ...
        LOG_FIFO(("sp0256: RD_FIFO %.3X %d.%d %d\n", data & ((1 << len) - 1), sp0256.fifo_tail, sp0256.fifo_bitp,
                  sp0256.fifo_head));
...
sp0256_micro(void) {
    ...
    while (sp0256.filt.rpt <= 0) {
        ...
        LOG(("$%.4X.%.1X: OPCODE %d%d%d%d.%d%d\n", (sp0256.pc >> 3) - 1, sp0256.pc & 7, !!(opcode & 1), !!(opcode & 2),
             !!(opcode & 4), !!(opcode & 8), !!(sp0256.mode & 4), !!(sp0256.mode & 2)));
        ...
        switch (opcode) {
            ...
            case 0xD: {
                ...
                /*  Figure out our branch target.                           */
                /* -------------------------------------------------------- */
                btrg = sp0256.page | (bitrev32(immed4) >> 17) | (bitrev32(sp0256_getb(8)) >> 21);
                ctrl_xfer = 1;
        ...
        if (ctrl_xfer) {
            ...
            if (sp0256.fifo_sel && sp0256.fifo_bitp) {
                ...
                if (sp0256.fifo_tail < sp0256.fifo_head)
                    sp0256.fifo_tail++;
                sp0256.fifo_bitp = 0;
            }
        ...
        for (i = idx0; i <= idx1; i++) {
            ...
            LOG(("$%.4X.%.1X: len=%2d shf=%2d prm=%2d d=%d f=%d ", sp0256.pc >> 3, sp0256.pc & 7, len, shf, prm,
                 !!delta, !!field));
            /* ------------------------------------------------------------ */
            ...
            LOG(("v=%.2X (%c%.2X)  ", value & 0xFF, value & 0x80 ? '-' : '+', 0xFF & (value & 0x80 ? -value : value)));
...
static void
sp0256_update(int num, INT16* output, int length) {
    int output_index = 0;
...
int
sp0256_sh_start(const struct MachineSound* msound) {
    ...
    if (sp0256.drq)
        sp0256.drq(ASSERT_LINE);
    if (sp0256.sby)
        sp0256.sby(sp0256.sby_line = ASSERT_LINE);
...
WRITE16_HANDLER(spb640_w) {
    ...
    if (offset == 1) {
        ...
        LOG(("spb640: WR_FIFO %.3X %d.%d %d\n", data & 0x3FF, sp0256.fifo_tail, sp0256.fifo_bitp, sp0256.fifo_head));
