...
#define SAMPLES_THIS_FRAME(channel) mixer_need_samples_this_frame((channel), stream_sample_rate[(channel)])
...
streams_sh_update(void) {
            ...
            if (stream_joined_channels[channel] > 1) {
                ...
                if (buflen > 0) {
                    ...
                    for (i = 0; i < stream_joined_channels[channel]; i++) {
                        ...
                        buf[i] = (UINT8*)(stream_buffer[channel + i])
                                 + stream_buffer_pos[channel + i]
                                       * (stream_is_float[channel + i] ? sizeof(float) : sizeof(INT16));
                    }
                ...
                if (buflen > 0) {
                    void* buf =
                        (UINT8*)(stream_buffer[channel])
                        + stream_buffer_pos[channel] * (stream_is_float[channel] ? sizeof(float) : sizeof(INT16));
    ...
    for (channel = 0; channel < MIXER_MAX_CHANNELS; channel += stream_joined_channels[channel]) {
        if (stream_buffer[channel]) {
            int i;
            for (i = 0; i < stream_joined_channels[channel]; i++)
                mixer_play_streamed_sample_16(channel + i, stream_buffer[channel + i], SAMPLES_THIS_FRAME(channel + i),
...
int
stream_init(const char* name, int default_mixing_level, double sample_rate, int param,
            void (*callback)(int param, INT16* buffer, int length)) {
    return stream_init_float(name, default_mixing_level, sample_rate, param, callback, 0);
}

int
stream_init_float(const char* name, int default_mixing_level, double sample_rate, int param,
                  void (*callback)(int param, INT16* buffer, int length), int is_float) {
...
void
stream_set_sample_rate(int channel, double sample_rate) {
...
double
stream_get_sample_rate(int channel) {
    return stream_sample_rate[channel];
}

void
stream_free(int channel) {
    if (stream_buffer[channel])
...
int
stream_init_multi(int channels, const char** names, const int* default_mixing_levels, double sample_rate, int param,
                  void (*callback)(int param, INT16** buffer, int length)) {
    return stream_init_multi_float(channels, names, default_mixing_levels, sample_rate, param, callback, 0);
}

int
stream_init_multi_float(int channels, const char** names, const int* default_mixing_levels, double sample_rate,
...
void
stream_update(int channel, int min_interval) {
    int newpos;
    int buflen;
    ...
    if (buflen * stream_sample_length[channel] > min_interval) {
        if (stream_joined_channels[channel] > 1) {
            ...
            for (i = 0; i < stream_joined_channels[channel]; i++)
                buf[i] =
                    (UINT8*)(stream_buffer[channel + i])
                    + stream_buffer_pos[channel + i] * (stream_is_float[channel + i] ? sizeof(float) : sizeof(INT16));
            ...
            void* buf = (UINT8*)(stream_buffer[channel])
                        + stream_buffer_pos[channel] * (stream_is_float[channel] ? sizeof(float) : sizeof(INT16));
