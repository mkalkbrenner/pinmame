...
static UINT16 coefs[128] = {
    ...
    177, 185, 193, 201, 203, 217, 225, 233, 241, 249, 257, 265, 273, 281, 289, 297, 301, 305, 309, 313, 317, 321,
...
static void
sp0250_timer_tick(int param) {
    stream_update(sp0250.stream, 0);
}

static void
sp0250_update(int num, INT16* output, int length) {
    int i;
    for (i = 0; i < length; i++) {
        if (sp0250.playing) {
            INT16 z0;
...
int
sp0250_sh_start(const struct MachineSound* msound) {
