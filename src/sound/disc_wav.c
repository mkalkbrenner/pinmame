...
/************************************************************************/
int
dss_sinewave_step(struct node_description* node) {
...
int
dss_sinewave_reset(struct node_description* node) {
...
int
dss_sinewave_init(struct node_description* node) {
...
/************************************************************************/
int
dss_squarewave_step(struct node_description* node) {
...
int
dss_squarewave_reset(struct node_description* node) {
...
int
dss_squarewave_init(struct node_description* node) {
...
/************************************************************************/
int
dss_squarewfix_step(struct node_description* node) {
...
int
dss_squarewfix_reset(struct node_description* node) {
    struct dss_squarewfix_context* context = (struct dss_squarewfix_context*)node->context;
    ...
    discrete_log("RESET in - F:%f D:%f P:%f == tOff:%f tOn:%f tLeft:%f", node->input[1], node->input[3], node->input[5],
                 context->tOff, context->tOn, context->tLeft);
    //	while (context->tLeft >= context->flip_flop ? context->tOn : context->tOff)
    //	{
    //		context->tLeft -= context->flip_flop ? context->tOn : context->tOff;
    //		context->flip_flop = context->flip_flop ? 0 : 1;
...
int
dss_squarewfix_init(struct node_description* node) {
...
/************************************************************************/
int
dss_squarewave2_step(struct node_description* node) {
    struct dss_squarewave_context* context = (struct dss_squarewave_context*)node->context;
    double newphase;
...
int
dss_squarewave2_reset(struct node_description* node) {
    struct dss_squarewave_context* context = (struct dss_squarewave_context*)node->context;
    double start;
...
int
dss_squarewave2_init(struct node_description* node) {
    discrete_log("dss_squarewave_init() - Creating node %d.", node->node - NODE_00);
...
/************************************************************************/
int
dss_trianglewave_step(struct node_description* node) {
...
int
dss_trianglewave_reset(struct node_description* node) {
...
int
dss_trianglewave_init(struct node_description* node) {
...
/************************************************************************/
int
dss_sawtoothwave_step(struct node_description* node) {
    struct dss_sawtoothwave_context* context = (struct dss_sawtoothwave_context*)node->context;

    if (node->input[0]) {
        node->output = (context->type == 0) ? context->phase * (node->input[2] / (2.0 * M_PI))
                                            : node->input[2] - (context->phase * (node->input[2] / (2.0 * M_PI)));
        node->output -= node->input[2] / 2.0;
        /* Add DC Bias component */
        node->output = node->output + node->input[3];
    } else {
...
int
dss_sawtoothwave_reset(struct node_description* node) {
...
int
dss_sawtoothwave_init(struct node_description* node) {
    discrete_log("dss_trianglewave_init() - Creating node %d.", node->node - NODE_00);
...
/************************************************************************/
int
dss_noise_step(struct node_description* node) {
...
int
dss_noise_reset(struct node_description* node) {
...
int
dss_noise_init(struct node_description* node) {
...
/************************************************************************/
int
dss_lfsr_function(int myfunc, int in0, int in1, int bitmask) {
    int retval;
...
int
dss_lfsr_step(struct node_description* node) {
    ...
    while (i < (int)shiftAmount) {
        ...
        context->lfsr_reg = dss_lfsr_function(lfsr_desc->feedback_function2, fbresult, (context->lfsr_reg),
                                              ((1 << (lfsr_desc->bitlength)) - 1));
        ...
        context->lfsr_reg = dss_lfsr_function(DISC_LFSR_REPLACE, (context->lfsr_reg),
                                              fbresult << (lfsr_desc->bitlength), ((2 << (lfsr_desc->bitlength)) - 1));

        /* Now select the output bit */
        node->output = ((context->lfsr_reg) >> (lfsr_desc->output_bit)) & 0x01;

        /* Final inversion if required */
        if (lfsr_desc->flags & DISC_LFSR_FLAG_OUT_INVERT)
            node->output = (node->output) ? 0.0 : 1.0;
...
int
dss_lfsr_reset(struct node_description* node) {
    struct dss_lfsr_context* context;
    struct discrete_lfsr_desc* lfsr_desc;
    ...
    context->lfsr_reg = dss_lfsr_function(
        DISC_LFSR_REPLACE, 0, (dss_lfsr_function(lfsr_desc->feedback_function0, 0, 0, 0x01)) << (lfsr_desc->bitlength),
        ((2 << (lfsr_desc->bitlength)) - 1));
...
int
dss_lfsr_init(struct node_description* node) {
    struct dss_lfsr_context* context;
...
/************************************************************************/
int
dss_adsrenv_step(struct node_description* node) {
...
int
dss_adsrenv_reset(struct node_description* node) {
...
int
dss_adsrenv_init(struct node_description* node) {
