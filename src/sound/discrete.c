...
int
dss_default_kill(struct node_description* node) {
    if (node->context)
        free(node->context);
    node->context = NULL;
    return 0;
...
struct discrete_module module_list[] = {
    ...
    {DSS_ADJUSTMENT, "DSS_ADJUSTMENT", dss_adjustment_init, dss_default_kill, dss_adjustment_reset,
     dss_adjustment_step},
    {DSS_SQUAREWAVE, "DSS_SQUAREWAVE", dss_squarewave_init, dss_default_kill, dss_squarewave_reset,
     dss_squarewave_step},
    {DSS_SQUAREWFIX, "DSS_SQUAREWFIX", dss_squarewfix_init, dss_default_kill, dss_squarewfix_reset,
     dss_squarewfix_step},
    {DSS_SQUAREWAVE2, "DSS_SQUAREWAVE2", dss_squarewave2_init, dss_default_kill, dss_squarewave2_reset,
     dss_squarewave2_step},
    ...
    {DSS_TRIANGLEWAVE, "DSS_TRIANGLEWAVE", dss_trianglewave_init, dss_default_kill, dss_trianglewave_reset,
     dss_trianglewave_step},
    {DSS_SAWTOOTHWAVE, "DSS_SAWTOOTHWAVE", dss_sawtoothwave_init, dss_default_kill, dss_sawtoothwave_reset,
     dss_sawtoothwave_step},
    ...
    {DSD_SQUAREW555, "DSD_SQUAREW555", dsd_squarew555_init, dss_default_kill, dsd_squarew555_reset,
     dsd_squarew555_step},
    {DSD_SQUAREW566, "DSD_SQUAREW566", dsd_squarew566_init, dss_default_kill, dsd_squarew566_reset,
     dsd_squarew566_step},
    {DSD_TRIANGLEW566, "DSD_TRIANGLEW566", dsd_trianglew566_init, dss_default_kill, dsd_trianglew566_reset,
     dsd_trianglew566_step},

    {DSO_OUTPUT, "DSO_OUTPUT", dso_output_init, NULL, NULL, dso_output_step},
    {DSS_NULL, "DSS_NULL", NULL, NULL, NULL, NULL}};

static struct node_description*
find_node(int node) {
    int loop;
    for (loop = 0; loop < node_count; loop++) {
        if (node_list[loop].node == node)
            return &node_list[loop];
    }
    return NULL;
}

static void
discrete_stream_update_stereo(int ch, INT16** buffer, int length) {
    ...
    for (loop = 0; loop < length; loop++) {
        for (loop2 = 0; loop2 < node_count; loop2++) {
            ...
            for (loop3 = 0; loop3 < node->active_inputs; loop3++) {
                if (node->input_node[loop3] && (node->input_node[loop3])->node != NODE_NC)
                    node->input[loop3] = (node->input_node[loop3])->output;
            }
            /* Now step the node */
            if (module_list[node->module].step)
                (*module_list[node->module].step)(node);
        }
...
static void
discrete_stream_update_mono(int ch, INT16* buffer, int length) {
    /* Now we must do length iterations of the node list, one output for each step */
    int loop, loop2, loop3;
    struct node_description* node;
    ...
    for (loop = 0; loop < length; loop++) {
        ...
        buffer[loop] = (((struct dso_output_context*)(output_node->context))->left
                        + ((struct dso_output_context*)(output_node->context))->right)
                       / 2;
    }
#ifdef DISCRETE_WAVELOG
...
discrete_sh_reset(void) {
    ...
    for (loop = 0; loop < node_count; loop++) {
        ...
        for (loop2 = 0; loop2 < node->active_inputs; loop2++) {
            if (node->input_node[loop2] && (node->input_node[loop2])->node != NODE_NC)
                node->input[loop2] = (node->input_node[loop2])->output;
        }
        ...
        if (module_list[node_list[loop].module].reset) {
            discrete_log("discrete_sh_reset() - Calling reset for %s node %d.",
                         module_list[node_list[loop].module].name, node_list[loop].node - NODE_00);
            (*module_list[node_list[loop].module].reset)(&node_list[loop]);
        } else if (module_list[node_list[loop].module].step) {
            discrete_log("discrete_sh_reset() - Node has no reset, calling step for %s node %d.",
                         module_list[node_list[loop].module].name, node_list[loop].node - NODE_00);
...
int
discrete_sh_start(const struct MachineSound* msound) {
    struct discrete_sound_block* intf;
    int loop, loop2, search, failed;

#ifdef DISCRETE_WAVELOG
    wav_file = wav_open("discrete.wav", (int)(Machine->sample_rate + 0.5),
                        ((Machine->drv->sound_attributes & SOUND_SUPPORTS_STEREO) == SOUND_SUPPORTS_STEREO) ? 2 : 1);
#endif
#ifdef DISCRETE_DEBUGLOG
    if (!disclogfile)
    ...
    while (1) {
        ...
        if (node_count > DISCRETE_MAX_NODES) {
            logerror("discrete_sh_start() - Upper limit of %d nodes exceeded, have you terminated the interface block.",
                     DISCRETE_MAX_NODES);
            return 1;
        }
    }
    ...
    for (loop = 0; loop < node_count; loop++) {
        ...
        for (loop2 = 0; loop2 < intf[loop].active_inputs; loop2++) {
            if (node_list[loop].input_node[loop2] == NULL && intf[loop].input_node[loop2] >= NODE_START
                && intf[loop].input_node[loop2] <= NODE_END) {
        ...
        while (1) {
            if (module_list[search].type == intf[loop].type) {
                ...
                if (module_list[search].init)
                    if (((*module_list[search].init)(&node_list[loop])) == 1)
                        failed = 1;
                break;
            } else if (module_list[search].type == DSS_NULL) {
                if (intf[loop].type == DSS_NULL)
                    break;
                else {
                    logerror(
                        "discrete_sh_start() - Invalid DSS/DST/DSO module type specified in interface, item %02d\n",
                        loop + 1);
                    failed = 1;
                    break;
                }
    ...
    if ((Machine->drv->sound_attributes & SOUND_SUPPORTS_STEREO) == SOUND_SUPPORTS_STEREO) {
        ...
        discrete_stream =
            stream_init_multi(2, stereo_names, vol, Machine->sample_rate, 0, discrete_stream_update_stereo);
...
discrete_sh_stop(void) {
    ...
    for (loop = 0; loop < node_count; loop++) {
        ...
        if (module_list[node_list[loop].module].kill)
            (*module_list[node_list[loop].module].kill)(&node_list[loop]);
    }
    if (node_list)
        free(node_list);
    if (running_order)
