...
****************************************************************************************************/
static void
reset_compression_flags(struct BSMT2000Chip* const chip) {
...
***************************************************/
static void
set_mode(struct BSMT2000Chip* const chip, int i) {
    /* force an update */
...
static void
bsmt2000_update(int num, INT16** buffer, int _length) {
    ...
    if (chip->adpcm && voice->reg[REG_BANK] < chip->total_banks && voice->reg[REG_RATE]) {
...
#ifdef PINMAME
        if (chip->stereo
            && !chip->right_volume_set) // Monopoly and RCT feature stereo hardware, but only ever set the left volume
            rvol = lvol;
#endif
        ...
        for (samp = 0; samp < length; samp++) {
            ...
            if (frac == 1 || frac == 4) {
                static const INT32 delta_tab[16] = {154, 154, 128, 102, 77, 58,  58,  58,
    ...
    for (voicenum = 0; voicenum < chip->voices; voicenum++) {
        ...
        if (voice->reg[REG_BANK] < chip->total_banks) {
            ...
            for (samp = 0; samp < length; samp++) {
                ...
                /* check for loop end */
                if (pos >= voice->reg[REG_LOOPEND]) {
                    pos += voice->reg[REG_LOOPSTART]
                           - voice->reg[REG_LOOPEND]; // looks whacky, but it seems to be correct like this
                    frac = 0;
                }
            }
...
#define bsmt_shift_data 16
    ...
    for (samp = 0; samp < length; samp++) {
        ...
        if (l
            > 32767) // this overflow check happens after each voices accumulation on the TMS (as it only has a 32bit ACC with saturation enabled), but this way we are more accurately mixing (not with respect to the actual HW though)
            l = 32767;
        else if (l < -32768)
            l = -32768;
        if (r > 32767)
...
INLINE void
init_voice(struct BSMT2000Voice* voice) {
...
INLINE void
init_all_voices(struct BSMT2000Chip* const chip) {
    int i;
    /* init the voices */
...
int
BSMT2000_sh_start(const struct MachineSound* msound) {
...
static void
bsmt2000_reg_write(struct BSMT2000Chip* const chip, offs_t offset, data16_t data, data16_t mem_mask) {
    struct BSMT2000Voice* voice;
    ...
    if (offset < 0x6d) /* regular/non-adpcm voice */
    {
        ...
        if (chip->use_de_rom_banking && regindex == REG_BANK) {
            ...
            UINT16 temp = (voice->reg[REG_BANK] & 0x07) | ((voice->reg[REG_BANK] & 0x18) << 1)
...
WRITE16_HANDLER(BSMT2000_data_0_w) { bsmt2000_reg_write(&bsmt2000[0], offset, data, mem_mask); }
