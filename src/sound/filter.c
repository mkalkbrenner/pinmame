...
void
filter_state_reset(filter* f, filter_state* s) {
    unsigned int i;
    s->prev_mac = 0;
    for (i = 0; i < f->order; ++i)
        s->xprev[i] = 0;
...
void
filter_state_free(filter_state* s) {
    free(s);
}
...
float
filter_compute(const filter* f, const filter_state* s) {
...
filter*
filter_lp_fir_alloc(double freq, const int order) {
    ...
    for (i = 1; i <= midorder; ++i) {
        ...
        const double w = 0.355768 - 0.487396 * cos((2. * M_PI) * n / (order - 1))
                         + 0.144232 * cos((4. * M_PI) * n / (order - 1))
                         - 0.012604 * cos((6. * M_PI) * n / (order - 1)); /* Nutall */
...
void
filter2_setup(const int type, const double fc, const double d, const double gain,
...
void
filter2_reset(filter2_context* const __restrict filter2) {
    filter2->x1 = 0;
    filter2->x2 = 0;
    filter2->y1 = 0;
    filter2->y2 = 0;
...
void
filter_setup(const double b0, const double b1, const double b2, const double a1, const double a2, // a0 = 1
...
void
filter_opamp_m_bandpass_setup(const double r1, const double r2, const double r3, const double c1, const double c2,
                              filter2_context* const __restrict filter2, const double sample_rate) {
...
//
static void
filter2_biquad_setup(const double f0, const double c1, const double c2, filter2_context* const __restrict context,
...
//
void
filter_rc_lp_setup(const double R1, const double R2, const double R3, const double C1,
...
//
void
filter_mf_lp_setup(const double R1, const double R2, const double R3, const double C1, const double C2,
                   filter2_context* const __restrict context, const double sample_rate) {
...
//
void
filter_active_lp_setup(const double R1, const double R2, const double R3, const double C1,
                       filter2_context* const __restrict context, const double sample_rate) {
    // Continuous-time transfer function for the analog filter:
    //
    //                     -1/(R1*R2*C1)
...
//
void
filter_sallen_key_lp_setup(const double R1, const double R2, const double C1, const double C2,
                           filter2_context* const __restrict context, const double sample_rate) {
    // Continuous-time transfer function for the analog filter:
    //
    //                       1/(R1*R2*C1*C2)
