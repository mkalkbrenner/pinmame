...
INLINE float
compress_loudness(const double sample) {
    // apply compression
...
//
static void
init_output_filter(struct hc55516_data* chip) {
...
//
static void
add_sample_out(struct hc55516_data* chip, const double sample, const double output_rate_ratio) {
    float fOut[128];
    SRC_DATA sd;
    long i;
...
//
static void
process_bit(struct hc55516_data* chip, const UINT8 bit, const double output_rate_ratio) {
...
//
static float
apply_filter(struct hc55516_data* chip, const float sample) {
    // run the sample through the two-stage filter
    const double filtered =
        filter2_step_with(&chip->output_filter.f2, filter2_step_with(&chip->output_filter.f1, sample));
    ...
    if (++chip->dynrange.nsamples > 100000 && dynrange_log_fp != NULL) {
        ...
        fprintf(dynrange_log_fp, "HC55516 #%d range [%lf..+%lf] -> max gain w/o clipping %d\n", (int)(chip - hc55516),
                chip->dynrange.vmin, chip->dynrange.vmax, (int)maxgain);
...
//
static void
hc55516_update(int num, INT16* buffer_ptr, int length) {
...
//
static void
collect_clock_stats(struct hc55516_data* chip) {
...
void
hc55516_set_gain(int num, double gain) {
...
void
hc55516_digit_w(int num, int data) {
    hc55516[num].databit = data & 1;
}

// Clear the clock (take it low)
void
hc55516_clock_clear_w(int num, int data) {
...
// last data write.
void
hc55516_clock_set_w(int num, int data) {
...
void
hc55516_digit_clock_clear_w(int num, int data) {
    hc55516[num].databit = data & 1;
    hc55516_clock_w(num, 0);
}
...
void
hc55516_set_sample_clock(int chipno, int frequency) {
...
//
int
hc55516_sh_start(const struct MachineSound* msound) {
    ...
    for (i = 0; i < intf->num; i++) {
        ...
        chip->channel = stream_init_float(
            name, intf->volume[i], Machine->sample_rate, i, hc55516_update,
            1); // pick output sample rate for max quality, also saves an additional filtering step in the mixer!
