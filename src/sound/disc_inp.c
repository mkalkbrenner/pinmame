...
/************************************************************************/
int
dss_input_step(struct node_description* node) {
    node->output = (node->input[0] * node->input[3]) + node->input[4];
    return 0;
}

int
dss_input_reset(struct node_description* node) {
...
int
dss_input_pulse_step(struct node_description* node) {
    /* Set a valid output */
    node->output = (node->input[0] * node->input[3]) + node->input[4];
...
int
dss_input_init(struct node_description* node) {
    int loop, addr, mask;
    /* We just allocate memory on the first call */
    if (dss_input_map == NULL) {
        if ((dss_input_map = malloc(DSS_INPUT_SPACE * sizeof(struct node_description*))) == NULL)
            return 1;
...
int
dss_input_kill(struct node_description* node) {
    free(dss_input_map);
    dss_input_map = NULL;
    return 0;
}
...
/************************************************************************/
int
dss_constant_step(struct node_description* node) {
...
/************************************************************************/
int
dss_adjustment_step(struct node_description* node) {
...
int
dss_adjustment_reset(struct node_description* node) {
    struct dss_adjustment_context* context = (struct dss_adjustment_context*)node->context;
...
int
dss_adjustment_init(struct node_description* node) {
...
int
discrete_sh_adjuster_count(struct discrete_sound_block* dsintf) {
    int node_counter = 0;
    int count;
    int sanity_abort = 0;

    if (dss_adjustment_map != NULL)
        free(dss_adjustment_map);

    if ((dss_adjustment_map = malloc(sizeof(int) * DISCRETE_MAX_ADJUSTERS)) == NULL)
        return -1;
    for (count = 0; count < DISCRETE_MAX_ADJUSTERS; count++)
        dss_adjustment_map[count] = 0;

    count = 0;
    while (1) {
        /* Check the node parameter is a valid node */
        if (dsintf[count].node < NODE_START || dsintf[count].node > NODE_END)
            return -1;
        if (sanity_abort++ > 255)
            return -1;
        /* Only count adjustment nodes */
        if (dsintf[count].type == DSS_ADJUSTMENT)
            dss_adjustment_map[node_counter++] = count;
        /* Abort on either END node */
        if (dsintf[count].type == DSS_NULL)
            break;
        count++;
    }
    return node_counter;
...
int
discrete_sh_adjuster_get(int arg, struct discrete_sh_adjuster* adjuster) {
    int node;
    if (adjuster == NULL)
        return -1;
    ...
    /* Sanity check */
    if (node < 0 || node > DISCRETE_MAX_NODES)
        return -1;
...
int
discrete_sh_adjuster_set(int arg, struct discrete_sh_adjuster* adjuster) {
    int node;
    if (adjuster == NULL)
        return -1;
