...
/************************************************************************/
int
dso_output_step(struct node_description* node) {
    ...
    /* Clamp outputs */
    if (node->input[0] < -32768)
        context->left = -32768;
    else if (node->input[0] > 32767)
        context->left = 32767;
    else
        context->left = (INT16)node->input[0];
    if (node->input[1] < -32768)
        context->right = -32768;
    else if (node->input[1] > 32767)
        context->right = 32767;
    else
        context->right = (INT16)node->input[1];
    return 0;
}

int
dso_output_init(struct node_description* node) {
