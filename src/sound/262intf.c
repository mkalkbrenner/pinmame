...
static void
IRQHandler_262(int n, int irq) {
    if (intf_262->handler[n])
        (intf_262->handler[n])(irq);
}

static void
timer_callback_262(int param) {
    int n = param >> 1;
    int c = param & 1;
    YMF262TimerOver(n, c);
}
...
static void
TimerHandler_262(int c, double period) {
...
int
YMF262_sh_start(const struct MachineSound* msound) {
    int chip;
    double rate; // = Machine->sample_rate;

    intf_262 = msound->sound_interface;
    if (intf_262->num > MAX_262)
        return 1;
...
/* chip #0 */
READ_HANDLER(YMF262_status_0_r) { return YMF262Read(0, 0); }

WRITE_HANDLER(YMF262_register_A_0_w) { YMF262Write(0, 0, data); }

WRITE_HANDLER(YMF262_data_A_0_w) { YMF262Write(0, 1, data); }

WRITE_HANDLER(YMF262_register_B_0_w) { YMF262Write(0, 2, data); }

WRITE_HANDLER(YMF262_data_B_0_w) { YMF262Write(0, 3, data); }

/* chip #1 */
READ_HANDLER(YMF262_status_1_r) { return YMF262Read(1, 0); }

WRITE_HANDLER(YMF262_register_A_1_w) { YMF262Write(1, 0, data); }

WRITE_HANDLER(YMF262_data_A_1_w) { YMF262Write(1, 1, data); }

WRITE_HANDLER(YMF262_register_B_1_w) { YMF262Write(1, 2, data); }

WRITE_HANDLER(YMF262_data_B_1_w) { YMF262Write(1, 3, data); }
