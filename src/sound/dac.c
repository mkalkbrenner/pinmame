...
static void
DAC_update(int num, INT16* const buffer, int length) {
    /* zero-length? bail */
    if (length == 0)
        return;
    else {
...
void
DAC_signed_data_w(int num, int data) {
...
void
DAC_data_16_w(int num, int data) {
...
void
DAC_DC_offset_correction_data_16_w(int num, int data) {
...
void
DAC_signed_data_16_w(int num, int data) {
...
int
DAC_sh_start(const struct MachineSound* msound) {
    int i;
...
#ifdef PINMAME
void
DAC_set_reverb_filter(int num, float delay, float force) {
    //stream_update(channel[num], 0); //!!?
    mixer_set_reverb_filter(channel[num], delay, force);
}

void
DAC_set_mixing_level(int num, int pctvol) {
