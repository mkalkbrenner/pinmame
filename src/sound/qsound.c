...
INLINE uint16_t
read_dsp_rom(uint16_t addr) {
    return ((uint16_t*)dl_1425)[addr & 0xfff];
}
...
int
qsound_sh_start(const struct MachineSound* msound) {
...
#define CHANNELS 2
        ...
        m_stream = stream_init_multi(CHANNELS, name, vol, intf->clock / QSOUND_CLOCKDIV, 0, qsound_update);
    }
...
WRITE_HANDLER(qsound_data_h_w) { m_data_latch = (m_data_latch & 0x00ff) | (data << 8); }

WRITE_HANDLER(qsound_data_l_w) { m_data_latch = (m_data_latch & 0xff00) | data; }
...
INLINE void
qsound_write_data(uint8_t address, uint16_t data) {
...
INLINE int16_t
read_sample(uint16_t bank, uint16_t address) {
...
INLINE int16_t
qsound_voice_update(qsound_voice* qv, int32_t* echo_out) {
    int32_t new_phase;
...
// emulators.
INLINE int16_t
qsound_adpcm_update(qsound_adpcm* qa, int16_t curr_sample, int nibble) {
    int8_t step;
    int32_t delta;
    if (!nibble) {
...
INLINE int16_t
qsound_echo_apply(qsound_echo* qe, int32_t input) {
    int32_t new_sample;
...
state_normal_update() {
    ...
    m_voice_output[16 + adpcm_voice] =
        qsound_adpcm_update(m_adpcm + adpcm_voice, m_voice_output[16 + adpcm_voice], m_state_counter / 3);
...
static int32_t
qsound_fir_apply(qsound_fir* qf, int16_t input) {
...
INLINE int32_t
qsound_delay_apply(qsound_delay* qd, const int32_t input) {
    int32_t output;
...
INLINE void
qsound_delay_update(qsound_delay* qd) {
...
void
qsound_update(int num, INT16** buffer, int length) {
    int i;
    for (i = 0; i < length; i++) {
        update_sample();
        buffer[0][i] = m_out[0];
