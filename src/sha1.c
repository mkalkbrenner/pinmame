...
unsigned int
READ_UINT32(const uint8_t* data) {
    return ((uint32_t)data[0] << 24) | ((uint32_t)data[1] << 16) | ((uint32_t)data[2] << 8) | ((uint32_t)data[3]);
}

void
WRITE_UINT32(unsigned char* data, uint32_t val) {
    data[0] = (val >> 24) & 0xFF;
    data[1] = (val >> 16) & 0xFF;
    data[2] = (val >> 8) & 0xFF;
    data[3] = (val >> 0) & 0xFF;
...
void
sha1_update(struct sha1_ctx* ctx, unsigned length, const uint8_t* buffer) {
...
sha1_final(struct sha1_ctx* ctx) {
    ...
    if (words > (SHA1_DATA_LENGTH - 2)) { /* No room for length in this block. Process it and
...
void
sha1_digest(const struct sha1_ctx* ctx, unsigned length, uint8_t* digest) {
    unsigned i;
    unsigned words;
    unsigned leftover;
