...
static int render_to_blit(struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels,
                          int update);
static int render_to_primary(struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels,
                             int update);
...
void
win_ddraw_fullscreen_margins(DWORD desc_width, DWORD desc_height, RECT* margins) {
    margins->left = 0;
    margins->top = 0;
...
INLINE void
erase_outer_rect(RECT* outer, RECT* inner, LPDIRECTDRAWSURFACE surface) {
...
int
win_ddraw_init(int width, int height, int depth, int attributes, const struct win_effect_data* effect) {
#ifndef DISABLE_DX7
    HRESULT result;
    DDCAPS hel_caps;
    ...
    result = IDirectDraw_SetCooperativeLevel(ddraw, win_video_window,
                                             win_window_mode ? DDSCL_NORMAL : DDSCL_FULLSCREEN | DDSCL_EXCLUSIVE);
    if (result != DD_OK) {
...
enum_callback(LPDDSURFACEDESC desc, LPVOID context) {
...
static HRESULT WINAPI
enum2_callback(LPDDSURFACEDESC2 desc, LPVOID context) {
...
set_resolution(void) {
    ...
    if (!win_window_mode && (win_switch_res || win_switch_bpp)) {
        ...
        best_score = 0.0;
        if (ddraw4)
            result = IDirectDraw4_EnumDisplayModes(
                ddraw4, (win_match_refresh || win_gfx_refresh) ? DDEDM_REFRESHRATES : 0, NULL, NULL, enum2_callback);
        else
...
create_surfaces(void) {
    ...
    if (verbose)
        fprintf(stderr, "Primary surface created: %dx%dx%d (R=%08x G=%08x B=%08x)\n", (int)primary_desc.dwWidth,
                (int)primary_desc.dwHeight, (int)primary_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount,
...
create_blit_surface(void) {
    ...
    if (verbose)
        fprintf(stderr, "Blit surface created: %dx%dx%d (R=%08x G=%08x B=%08x)\n", (int)blit_desc.dwWidth,
                (int)blit_desc.dwHeight, (int)blit_desc.ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount,
...
set_brightness(void) {
    ...
    result =
        IDirectDrawSurface_QueryInterface(primary_surface, &IID_IDirectDrawGammaControl, (void**)gamma_controladdr);
    if (result != DD_OK) {
        if (verbose)
...
static void
compute_color_masks(const DDSURFACEDESC* desc) {
    // 16bpp case
...
int
win_ddraw_draw(struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels, int update) {
    int result;
...
static int
blit_and_flip(LPDIRECTDRAWSURFACE target_surface, LPRECT src, LPRECT dst, int update) {
    HRESULT result;
...
static int
render_to_primary(struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels, int update) {
    int wait_for_lock = lock_must_succeed(bounds, vector_dirty_pixels);
