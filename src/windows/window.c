...
static void compute_multipliers_internal(const RECT* rect, int visible_width, int visible_height, int* xmult,
                                         int* ymult);
...
static void draw_video_contents(HDC dc, struct mame_bitmap* bitmap, const struct rectangle* bounds,
                                void* vector_dirty_pixels, int update);
...
INLINE int
wnd_extra_width(void) {
    RECT window = {100, 100, 200, 200};
    if (!win_window_mode)
        return 0;
#ifdef VPINMAME
    AdjustWindowRectEx(&window, GetWindowLongPtr(win_video_window, GWL_STYLE), win_has_menu(),
                       GetWindowLongPtr(win_video_window, GWL_EXSTYLE));
#else
...
INLINE void
set_aligned_window_pos(HWND wnd, HWND insert, int x, int y, int cx, int cy, UINT flags) {
#ifdef VPINMAME
...
#ifndef VPINMAME
INLINE void
erase_outer_rect(RECT* outer, RECT* inner, HDC dc) {
...
// in case this is VPM, register the class regardless of the MAME options
    ...
    win_video_window = CreateWindowEx(win_window_mode ? WINDOW_STYLE_EX : FULLSCREEN_STYLE_EX, TEXT("MAME"), title,
                                      win_window_mode ? WINDOW_STYLE : FULLSCREEN_STYLE, 20, 20, 100, 100, NULL, menu,
...
int
win_create_window(int width, int height, int depth, int attributes, double aspect) {
    int i, result = 0;
    ...
    set_aligned_window_pos(win_video_window, NULL, 20, 20, width + wnd_extra_width() + 2,
                           height + wnd_extra_height() + 2, SWP_NOZORDER);
    ...
    if (win_force_int_stretch != FORCE_INT_STRECT_FULL
        && (win_use_directx == USE_D3D || (win_use_directx == USE_DDRAW && win_dd_hw_stretch))) {
        aspect_ratio = aspect;
    }
    ...
    if (win_use_directx == USE_D3D) {
        if (win_blit_effect)
            fprintf(stderr, "Warning: non-hardware-accelerated blitting-effects engine enabled\n         use the "
                            "-d3deffect option to enable hardware acceleration\n");
    } else {
...
void
win_update_video_window(struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels) {
...
static void
draw_video_contents(HDC dc, struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels,
                    int update) {
#ifndef PINMAME
    ...
    if (win_window_mode && !update) {
        ...
        if (newleft != original.left)
            SetWindowPos(win_video_window, NULL, newleft, original.top, 0, 0,
...
video_window_proc(HWND wnd, UINT message, WPARAM wparam, LPARAM lparam) {
...
void
win_constrain_to_aspect_ratio(RECT* rect, int adjustment, int constraints, int coordinate_system) {
    ...
    if (constraints == CONSTRAIN_INTEGER_WIDTH) {
        ...
        while (maxwidth > 1
               && maxrect.bottom - maxrect.top
                      < (int)((double)maxwidth * win_visible_width / adjusted_ratio + 0.5) + extraheight)
            maxwidth--;
        ...
        while (maxheight > 1
               && maxrect.right - maxrect.left
                      < (int)((double)maxheight * win_visible_height * adjusted_ratio + 0.5) + extrawidth)
            maxheight--;
...
void
win_adjust_window_for_visible(int min_x, int max_x, int min_y, int max_y) {
    ...
    if (visible_area_set && win_window_mode && win_use_directx != USE_D3D
        && (win_use_directx != USE_DDRAW || !win_dd_hw_stretch)) {
        RECT r;
        int xmult, ymult;
        ...
        set_aligned_window_pos(win_video_window, NULL, r.left, r.top, r.right - r.left, r.bottom - r.top,
        ...
        if (win_window_mode) {
            ...
            // otherwise, just enforce the bounds
                ...
                set_aligned_window_pos(win_video_window, NULL, non_maximized_bounds.left, non_maximized_bounds.top,
                                       ...
                                       non_maximized_bounds.bottom - non_maximized_bounds.top, SWP_NOZORDER);
        }
...
win_toggle_maximize(void) {
    ...
    set_aligned_window_pos(win_video_window, NULL, current.left, current.top, current.right - current.left,
                           current.bottom - current.top, SWP_NOZORDER);
}
...
win_toggle_full_screen(void) {
    ...
    if (win_window_mode) {
        ...
        set_aligned_window_pos(win_video_window, 0, 0, 0, 0, 0,
        ...
        if (non_fullscreen_bounds.right != non_fullscreen_bounds.left)
            set_aligned_window_pos(win_video_window, HWND_TOP, non_fullscreen_bounds.left, non_fullscreen_bounds.top,
                                   non_fullscreen_bounds.right - non_fullscreen_bounds.left,
                                   non_fullscreen_bounds.bottom - non_fullscreen_bounds.top, SWP_NOZORDER);
        else {
            set_aligned_window_pos(win_video_window, HWND_TOP, 0, 0, win_visible_width + 2, win_visible_height + 2,
                                   SWP_NOZORDER);
            win_toggle_maximize();
        }
#endif
...
win_adjust_window(void) {
    ...
    if (original.left != window.left || original.top != window.top || original.right != window.right
        || original.bottom != window.bottom || original.left != get_aligned_window_pos(original.left))
        set_aligned_window_pos(win_video_window, win_window_mode ? HWND_TOP : HWND_TOPMOST, window.left, window.top,
...
void
win_process_events_periodic(void) {
...
UINT32*
win_prepare_palette(struct win_blit_params* params) {
...
static void
dib_draw_window(HDC dc, struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels,
                int update) {
    ...
    if ((video_dib_info->bmiHeader.biWidth == params.dstpitch / (depth / 8))
        ...
        SetDIBitsToDevice(dc, 0, 0, (client.right - client.left), (client.bottom - client.top), 0, 0, 0,
                          (client.bottom - client.top), converted_bitmap, video_dib_info, DIB_RGB_COLORS);
    else
#ifdef FAST_NN_BLIT
...
#endif
                      0, 0, win_visible_width * xmult, win_visible_height * ymult, converted_bitmap, video_dib_info,
...
#else
    {
        ...
        switch (bitmap->depth) {
            ...
            case 24:
                ...
                break;
            case 32:
                ResampleX8R8G8B8((UINT32*)upscale_bitmap, video_dib_info->bmiHeader.biWidth,
                                 (client.bottom - client.top), (UINT32*)converted_bitmap, params.dstpitch / (depth / 8),
                                 win_visible_height * ymult);
                break;
            default:
        ...
        SetDIBitsToDevice(dc, 0, 0, (client.right - client.left), (client.bottom - client.top), 0, 0, 0,
                          (client.bottom - client.top), upscale_bitmap, video_dib_info, DIB_RGB_COLORS);
    }
#endif
...
int
win_lookup_effect(const char* arg) {
    int effindex;
...
int
win_determine_effect(const struct win_blit_params* params) {
...
static void
compute_multipliers_internal(const RECT* rect, int visible_width, int visible_height, int* xmult, int* ymult) {
...
void
win_compute_multipliers(const RECT* rect, int* xmult, int* ymult) {
...
create_debug_window(void) {
    ...
    win_debug_window = CreateWindowEx(DEBUG_WINDOW_STYLE_EX, TEXT("MAMEDebug"), title, DEBUG_WINDOW_STYLE,
                                      ...
                                      work_bounds.bottom - (bounds.bottom - bounds.top), bounds.right - bounds.left,
                                      bounds.bottom - bounds.top, win_video_window, NULL, GetModuleHandle(NULL), NULL);
    if (!win_debug_window)
        return 1;
#endif
...
void
win_update_debug_window(struct mame_bitmap* bitmap, const rgb_t* palette) {
#ifdef MAME_DEBUG
    // get the client DC and draw to it
    if (win_debug_window) {
...
static void
draw_debug_contents(HDC dc, struct mame_bitmap* bitmap, const rgb_t* palette) {
#ifndef PINMAME
    ...
    // fill in bitmap-specific info
    debug_dib_info->bmiHeader.biWidth =
        (LONG)(((UINT8*)bitmap->line[1]) - ((UINT8*)bitmap->line[0])) / (bitmap->depth / 8);
    debug_dib_info->bmiHeader.biHeight = -bitmap->height;
    debug_dib_info->bmiHeader.biBitCount = bitmap->depth;

    // blit to the screen
    StretchDIBits(dc, 0, 0, bitmap->width, bitmap->height, 0, 0, bitmap->width, bitmap->height, bitmap->base,
                  debug_dib_info, DIB_RGB_COLORS, SRCCOPY);
}
...
static LRESULT CALLBACK
debug_window_proc(HWND wnd, UINT message, WPARAM wparam, LPARAM lparam) {
    // handle a few messages
    switch (message) {
        // paint: redraw the last bitmap
        case WM_PAINT: {
...
void
win_set_debugger_focus(int focus) {
