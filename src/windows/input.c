...
enum_keyboard_callback(LPCDIDEVICEINSTANCE instance, LPVOID ref) {
    HRESULT result;
    ...
    result = IDirectInputDevice_QueryInterface(keyboard_device[keyboard_count], &IID_IDirectInputDevice2,
                                               (void**)&keyboard_device2[keyboard_count]);
    if (result != DI_OK)
...
static BOOL CALLBACK
enum_mouse_callback(LPCDIDEVICEINSTANCE instance, LPVOID ref) {
    DIPROPDWORD value;
    HRESULT result;
    ...
    // try to get a version 2 device for it
    result = IDirectInputDevice_QueryInterface(mouse_device[mouse_count], &IID_IDirectInputDevice2,
                                               (void**)&mouse_device2[mouse_count]);
    if (result != DI_OK)
...
static BOOL CALLBACK
enum_joystick_callback(LPCDIDEVICEINSTANCE instance, LPVOID ref) {
    DIPROPDWORD value;
    HRESULT result;
    DWORD flags;
    ...
    // try to get a version 2 device for it
    result = IDirectInputDevice_QueryInterface(joystick_device[joystick_count], &IID_IDirectInputDevice2,
                                               (void**)&joystick_device2[joystick_count]);
    if (result != DI_OK)
...
win_poll_input(void) {
    ...
    for (i = 0; i < keyboard_count; i++) {
        ...
        // get the state
        result =
            IDirectInputDevice_GetDeviceState(keyboard_device[i], sizeof(keyboard_state[i]), &keyboard_state[i][0]);
        ...
        if ((result == DIERR_INPUTLOST || result == DIERR_NOTACQUIRED) && !input_paused) {
            ...
            if (result == DI_OK)
                result = IDirectInputDevice_GetDeviceState(keyboard_device[i], sizeof(keyboard_state[i]),
                                                           &keyboard_state[i][0]);
        }
    ...
    for (i = 0; i < joystick_count && use_joystick; i++) {
        ...
        if (result == DIERR_INPUTLOST || result == DIERR_NOTACQUIRED) {
            ...
            if (result == DI_OK)
                result = IDirectInputDevice_GetDeviceState(joystick_device[i], sizeof(joystick_state[i]),
                                                           &joystick_state[i]);
        }
    }
    ...
    if (mouse_active && !win_has_menu())
        for (i = 0; i < mouse_count && (use_mouse || use_lightgun); i++) {
            ...
            if ((result == DIERR_INPUTLOST || result == DIERR_NOTACQUIRED) && !input_paused) {
                ...
                if (result == DI_OK)
                    result =
                        IDirectInputDevice_GetDeviceState(mouse_device[i], sizeof(mouse_state[i]), &mouse_state[i]);
            }
        }
}
...
const struct KeyboardInfo*
osd_get_key_list(void) {
    return keylist;
}
...
int
osd_is_key_pressed(int keycode) {
...
static void
add_joylist_entry(const char* name, int code, int* joycount) {
...
init_joylist(void) {
    ...
    for (mouse = 0; mouse < mouse_count; mouse++)
        for (button = 0; button < 4; button++) {
            ...
            // attempt to get the object info
            instance.dwSize = STRUCTSIZE(DIDEVICEOBJECTINSTANCE);
            result = IDirectInputDevice_GetObjectInfo(mouse_device[mouse], &instance,
                                                      (DWORD)offsetof(DIMOUSESTATE, rgbButtons[button]), DIPH_BYOFFSET);
            if (result == DI_OK) {
    ...
    for (stick = 0; stick < joystick_count; stick++) {
        ...
        for (axis = 0; axis < MAX_AXES; axis++) {
            ...
            // attempt to get the object info
            instance.dwSize = STRUCTSIZE(DIDEVICEOBJECTINSTANCE);
            result = IDirectInputDevice_GetObjectInfo(joystick_device[stick], &instance,
                                                      offsetof(DIJOYSTATE, lX) + axis * sizeof(LONG), DIPH_BYOFFSET);
            if (result == DI_OK) {
                // add negative value
                ...
                result = IDirectInputDevice_GetProperty(joystick_device[stick], DIPROP_RANGE,
                                                        &joystick_range[stick][axis].diph);
            }
        }
        ...
        for (button = 0; button < MAX_BUTTONS; button++) {
            ...
            instance.dwSize = STRUCTSIZE(DIDEVICEOBJECTINSTANCE);
            result = IDirectInputDevice_GetObjectInfo(joystick_device[stick], &instance,
                                                      (DWORD)offsetof(DIJOYSTATE, rgbButtons[button]), DIPH_BYOFFSET);
            if (result == DI_OK) {
        ...
        for (pov = 0; pov < MAX_POV; pov++) {
            ...
            instance.dwSize = STRUCTSIZE(DIDEVICEOBJECTINSTANCE);
            result = IDirectInputDevice_GetObjectInfo(joystick_device[stick], &instance,
                                                      (DWORD)offsetof(DIJOYSTATE, rgdwPOV[pov]), DIPH_BYOFFSET);
            if (result == DI_OK) {
                // add up direction
...
const struct JoystickInfo*
osd_get_joy_list(void) {
    return joylist;
}
...
int
osd_is_joy_pressed(int joycode) {
...
void
osd_analogjoy_read(int player, int analog_axis[], InputCode analogjoy_input[]) {
...
int
osd_is_joystick_axis_code(int joycode) {
...
void
osd_lightgun_read(int player, int* deltax, int* deltay) {
    POINT point;
...
void
osd_trak_read(int player, int* deltax, int* deltay) {
    // if the mouse isn't yet active, make it so
    if (!mouse_active && use_mouse && !win_has_menu()) {
        mouse_active = 1;
        win_pause_input(0);
...
int
osd_joystick_needs_calibration(void) {
    return 0;
}
...
void
osd_joystick_start_calibration(void) {}
...
const char*
osd_joystick_calibrate_next(void) {
    return 0;
}
...
void
osd_joystick_end_calibration(void) {}
...
void
process_ctrlr_file(struct rc_struct* iptrc, const char* ctype, const char* filename) {
    mame_file* f;
...
void
process_ctrlr_game(struct rc_struct* iptrc, const char* ctype, const struct GameDriver* drv) {
...
void
process_ctrlr_system(struct rc_struct* iptrc, const char* ctype, const struct GameDriver* drv) {
    char buffer[128];
    ...
    tmp_gd = drv;
    while (tmp_gd != NULL) {
        if (strcmp(tmp_gd->name, buffer) == 0)
            break;
...
static int
ipdef_custom_rc_func(struct rc_option* option, const char* arg, int priority) {
    ...
    if (idef) {
        ...
        else if (pinput_keywords->type == IKT_IPT || pinput_keywords->type == IKT_IPT_EXT) {
...
void
osd_customize_inputport_defaults(struct ipd* defaults) {
...
osd_get_leds(void) {
    ...
    } else // WinNT/2K/XP, use DeviceIoControl
    {
        ...
        DeviceIoControl(hKbdDev, IOCTL_KEYBOARD_QUERY_INDICATORS, NULL, 0, &OutputBuffer, DataLength, &ReturnedLength,
                        NULL);
...
osd_set_leds(int state) {
    ...
    } else // WinNT/2K/XP, use DeviceIoControl
    {
        ...
        DeviceIoControl(hKbdDev, IOCTL_KEYBOARD_SET_INDICATORS, &InputBuffer, DataLength, NULL, 0, &ReturnedLength,
                        NULL);
    }
...
static void
add_procInputList_entry(const char* name, int code, int* proccount) {
...
const struct PROCInfo*
osd_get_proc_list(void) {
