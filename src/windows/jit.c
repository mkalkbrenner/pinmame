...
#define DbgVirtualProtect(addr, len, mode, pOldMode)                                                                   \
    {                                                                                                                  \
        BOOL VPres = VirtualProtect(addr, len, mode, pOldMode);                                                        \
        assert(VPres != 0);                                                                                            \
    }
...
struct jit_ctl*
_jit_create(int* cycle_counter, int rshift) {
...
static void
init_code_pages(struct jit_ctl* jit) {
    byte* res;
    int reslen;
...
static byte*
rtlookup(struct jit_ctl* jit, data32_t addr) {
...
static byte*
rtlookup_patch(struct jit_ctl* jit, data32_t addr, byte* caller) {
    // look up the address
...
static byte*
emit_lookup_code(struct jit_ctl* jit, int patch) {
...
static void
delete_code_pages(struct jit_ctl* jit) {
...
void
jit_create_map(struct jit_ctl* jit, data32_t minAddr, data32_t maxAddr) {
    int i, nBytes, nAddrs;
...
void
jit_set_mem_callbacks(struct jit_ctl* jit, data8_t (*read8)(int addr), data16_t (*read16)(int addr),
...
void
jit_untranslate(struct jit_ctl* jit, data32_t addr) {
    byte* p;
...
byte*
jit_reserve_native(struct jit_ctl* jit, int len, struct jit_page** pgp) {
    struct jit_page* pg;
    byte* res;
    DWORD prvPro;
...
void
jit_close_native(struct jit_ctl* jit, byte* addr, int len) {
    DWORD prvPro;
...
byte*
jit_store_native(struct jit_ctl* jit, const byte* code, int len) {
    // reserve space
    struct jit_page* pg;
...
void
jit_store_native_from_reserved(struct jit_ctl* jit, const byte* code, int len, struct jit_page* pg, byte* dst) {
    // copy the data, if any
    if (len != 0) {
        BOOL res;
...
static struct jit_page*
jit_add_page(struct jit_ctl* jit, int min_siz) {
#if JIT_DEBUG
    DWORD prvPro;
    BOOL res;
#endif
