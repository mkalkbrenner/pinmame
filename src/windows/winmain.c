...
main(int argc, char** argv) {
    ...
    if (argc <= 1 && startup_info.dwFlags && !(startup_info.dwFlags & STARTF_USESTDHANDLES)) {
        ...
        sprintf(message_text,
                APPLONGNAME " v%s - Multiple Arcade Machine Emulator\n"
...
exception_filter(struct _EXCEPTION_POINTERS* info) {
    static const struct {
        DWORD code;
        const char* string;
    } exception_table[] = {{EXCEPTION_ACCESS_VIOLATION, "ACCESS VIOLATION"},
    ...
    fprintf(stderr, "EAX=%08X EBX=%08X ECX=%08X EDX=%08X\n", (UINT32)info->ContextRecord->Eax,
            (UINT32)info->ContextRecord->Ebx, (UINT32)info->ContextRecord->Ecx, (UINT32)info->ContextRecord->Edx);
    fprintf(stderr, "ESI=%08X EDI=%08X EBP=%08X ESP=%08X\n", (UINT32)info->ContextRecord->Esi,
            (UINT32)info->ContextRecord->Edi, (UINT32)info->ContextRecord->Ebp, (UINT32)info->ContextRecord->Esp);
#else
#if defined(_M_ARM64)
#pragma message("Warning: No CPU state debug output implemented yet")
    ...
    fprintf(stderr, "RAX=%16X RBX=%16X RCX=%16X RDX=%16X\n", (UINT64)info->ContextRecord->Rax,
            (UINT64)info->ContextRecord->Rbx, (UINT64)info->ContextRecord->Rcx, (UINT64)info->ContextRecord->Rdx);
    fprintf(stderr, "RSI=%16X RDI=%16X RBP=%16X RSP=%16X\n", (UINT64)info->ContextRecord->Rsi,
            (UINT64)info->ContextRecord->Rdi, (UINT64)info->ContextRecord->Rbp, (UINT64)info->ContextRecord->Rsp);
#endif
#endif
...
static int
get_code_base_size(UINT32* base, UINT32* size) {
    FILE* map = fopen(mapfile_name, "r");
    char line[1024];
...
compare_start(const void* item1, const void* item2) {
    return ((const struct map_entry*)item1)->start - ((const struct map_entry*)item2)->start;
}

#if ENABLE_PROFILER
static int
compare_hits(const void* item1, const void* item2) {
...
static DWORD WINAPI
profiler_thread_entry(LPVOID lpParameter) {
