 ...
 */
void
jit_emit_queue_instr(data32_t addr) {
    // create a queue item
 ...
 */
void
jit_emit_process_queue(struct jit_ctl* jit, int (*func)(struct jit_ctl* jit, data32_t addr)) {
...
static struct instr*
alloc_instr(int len, const byte* b) {
    struct instr* iop;
    int alolen;
    const byte* op;
    int rem;
...
static struct instr*
add_instr(int len, const byte* b) {
    // allocate a new instruction
...
static struct instr*
ins_instr_before(struct instr* ibefore, int len, const byte* b) {
    // allocate a new instruction
    struct instr* i = alloc_instr(len, b);
...
static struct instr*
ins_instr_after(struct instr* iafter, int len, const byte* b) {
    // allocate a new instruction
    struct instr* i = alloc_instr(len, b);
 ...
 */
void
jit_emit_begin_instr(struct jit_ctl* jit, data32_t addr) {
    struct instr* i;
...
void
jit_emit_cancel_instr(struct jit_ctl* jit) {
    struct instr *i, *nxt;
...
struct jit_label*
jit_new_label_at(struct instr* i) {
    struct jit_label* l = jit_new_label();
    l->i = i;
    return l;
}
...
struct jit_label*
jit_new_label_after(struct instr* i) {
    // create the label
    struct jit_label* l = jit_new_label();
...
struct jit_label*
jit_new_addr_label(data32_t addr) {
    struct jit_label* l = jit_new_label();
    l->emuaddr = addr;
    return l;
}
...
struct jit_label*
jit_new_native_label(byte* addr) {
    struct jit_label* l = jit_new_label();
    l->nataddr = addr;
    return l;
}
...
void
jit_resolve_label(struct jit_label* l) {
...
static byte*
label_to_native(struct jit_ctl* jit, struct jit_label* l) {
    byte* p;
...
void
jit_emit_commit(struct jit_ctl* jit) {
    int mod;
    int len, reslen;
    data32_t emuaddr;
    byte* nataddr;
                ...
                if (i->len == 2 && (lbldelta < -128 || lbldelta > 127)) {
                    ...
                    if (i->effop >= opJO && i->effop <= opJG) {
                        ...
                        if (use_proxy) {
                            ...
                            // it in place.
                            i->b[1] =
                                i->op[0] + 0x10; // add 0x10 to go from 7x to 0F 8x primary opcode, move to second byte
    ...
    for (i = stktop->ihead; i != 0; i = i->nxt) {
        ...
        if (i->lbl != 0) {
            ...
            if (i->len == 2
                && (i->effop == opJMP_8 || (i->effop >= opJO && i->effop <= opJG)
                    || (i->effop >= opLOOPNZ && i->effop <= opJECXZ))) {
                // 8-bit offset
                ASSERT(lbldelta >= -128 && lbldelta <= 127);
                i->b[1] = (byte)lbldelta;
            } else if (i->len == 5 && (i->effop == opCALL || i->effop == opJMP_32)) {
                // 32-bit offset
                *(UINT32*)(&i->b[1]) = lbldelta;
            } else if (i->len == 6 && (i->effop >= opJO32 && i->effop <= opJG32)) {
                // 32-bit offset
...
void
jit_emit_return_to_emu(data32_t addr) {
    byte mov[5] = {0xB8, 0, 0, 0, 0}; // MOV EAX,Imm32
    byte ret[1] = {0xC3};             // RETN, no operands
...
oplen(const struct mnedef* o) {
    ...
    if (o->op1 == rm8 || o->op1 == rm16 || o->op1 == rm32 || o->op2 == rm8 || o->op2 == rm16 || o->op2 == rm32)
        ++len;
...
static int
match_operand(optype t, struct opdesc* op) {
    switch (t) {
        case none:
            return op->typ == opNone;
        case r8:
            return op->typ == opReg8;
        case r16:
            return op->typ == opReg16;
        case r32:
            return op->typ == opReg32;
        case rseg:
            return op->typ == opSegReg;
        case rm8:
            return op->typ == opMem || op->typ == opMem8 || op->typ == opReg8;
        case rm16:
            return op->typ == opMem || op->typ == opMem16 || op->typ == opReg16;
        case rm32:
            return op->typ == opMem || op->typ == opMem32 || op->typ == opReg32;
        case imm8:
            return op->typ == opImm && op->val.imm <= 0xFF;
        case imm8S:
            return op->typ == opImm && op->val.imm <= 0x7F;
        case imm16:
            return op->typ == opImm && op->val.imm <= 0xFFFF;
        case imm32:
            return op->typ == opImm;
        case ofs8:
            return (op->typ == opOfs && op->val.ofs >= -128 && op->val.ofs <= 127) || op->typ == opLbl;
        case ofs16:
            return (op->typ == opOfs && op->val.ofs >= -32768 && op->val.ofs <= 32767) || op->typ == opLbl;
        case ofs32:
            return op->typ == opOfs || op->typ == opLbl;
        case rAL:
            return op->typ == opReg8 && op->val.reg == rrr(AL);
        case rCL:
            return op->typ == opReg8 && op->val.reg == rrr(CL);
        case rEAX:
            return op->typ == opReg32 && op->val.reg == rrr(EAX);
        case rEBX:
            return op->typ == opReg32 && op->val.reg == rrr(EBX);
        case rECX:
            return op->typ == opReg32 && op->val.reg == rrr(ECX);
        case rEDX:
            return op->typ == opReg32 && op->val.reg == rrr(EDX);
        case rESP:
            return op->typ == opReg32 && op->val.reg == rrr(ESP);
        case rEBP:
            return op->typ == opReg32 && op->val.reg == rrr(EBP);
        case rESI:
            return op->typ == opReg32 && op->val.reg == rrr(ESI);
        case rEDI:
            return op->typ == opReg32 && op->val.reg == rrr(EDI);
        case rDS:
            return op->typ == opSegReg && op->val.reg == rrr(DS);
        case rES:
            return op->typ == opSegReg && op->val.reg == rrr(ES);
        case rSS:
            return op->typ == opSegReg && op->val.reg == rrr(SS);
        case rCS:
            return op->typ == opSegReg && op->val.reg == rrr(CS);
        case rFS:
            return op->typ == opSegReg && op->val.reg == rrr(FS);
        case rGS:
            return op->typ == opSegReg && op->val.reg == rrr(GS);
        case imm_1:
            return op->typ == opImm && op->val.imm == 1;
        case imm_3:
            return op->typ == opImm && op->val.imm == 3;
        default:
            return 0;
    }
}
...
static byte*
encode_modrm(byte* p, const struct mnedef* o, const struct opdesc* rm, const struct opdesc* r) {
...
static byte*
encode_immediate(byte* p, optype t, struct opdesc* op) {
    switch (t) {
        case imm8:
        case imm8S:
...
static opdesctyp
inferMemType(opdesctyp m, struct opdesc* o) {
...
static void
setInferredMemType(struct opdesc* o, opdesctyp t) {
...
jit_emit(intelMneId mne, ...) {
    ...
    for (mbest = 0, i = mneIdx[mne], m = mnetab + i; i < sizeof(mnetab) / sizeof(mnetab[0]) && m->mne == mne;
         ++i, ++m) {
        // check for an instruction match and operand matches
        if (m->mne == mne && match_operand(m->op1, &op[0]) && match_operand(m->op2, &op[1])
            && match_operand(m->op3, &op[2])) {
            // if this is the only instruction so far, or the least costly, or the
            // shortest, keep it
            if (mbest == 0 || m->cost < mbest->cost || (m->cost == mbest->cost && oplen(m) < oplen(mbest)))
                mbest = m;
        }
    }
