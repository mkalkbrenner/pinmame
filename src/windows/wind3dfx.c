...
struct rc_option win_d3d_opts[] = {
    ...
    {"d3deffectrotate", NULL, rc_bool, &d3d_rc_rotate, "1", 0, 0, win_d3d_decode_rotate,
    ...
    {"d3dcustom", NULL, rc_string, &d3d_rc_custom, NULL, 0, 0, win_d3d_decode_custom,
     "customised blitting effects preset"},
    {"d3dexpert", NULL, rc_string, &d3d_rc_expert, NULL, 0, 0, win_d3d_decode_expert,
...
static int
win_d3d_decode_scan(struct rc_option* option, const char* arg, int priority) {
...
static int
win_d3d_decode_feedback(struct rc_option* option, const char* arg, int priority) {
    option->priority = priority;
...
static int
win_d3d_decode_prescale(struct rc_option* option, const char* arg, int priority) {
    option->priority = priority;
...
static int
win_d3d_decode_rotate(struct rc_option* option, const char* arg, int priority) {
    option->priority = priority;
...
static int
win_d3d_decode_custom(struct rc_option* option, const char* arg, int priority) {
    option->priority = priority;

    if (priority_use_effect_preset <= priority) {
        static char pattern_name[256];

        sscanf(arg, "%i,%i,%i,%i,%i,%i,%i,%i,%i,%255s", &custom_preset.use_rgbeffect, &custom_preset.pattern_mode,
               ...
               &custom_preset.use_feedback, &custom_preset.use_prescale, &custom_preset.use_scanlines, pattern_name);
...
static int
win_d3d_decode_effect(struct rc_option* option, const char* arg, int priority) {
    int i;
...
static int
win_d3d_decode_expert(struct rc_option* option, const char* arg, int priority) {
    // ensure .ini settings will still be processed
    option->priority = 0;
...
int
win_d3d_effects_init(int attributes) {
    ...
    // scanlines
    if (use_scanlines != -1)
        active_preset.use_scanlines = (use_scanlines << 16) | (use_scanlines << 8) | (use_scanlines << 0);
    ...
    if (win_d3d_use_feedback) {
        win_d3d_preprocess_tfactor = (active_preset.use_feedback << 24) | ((255 - active_preset.use_feedback) << 16)
...
int
win_d3d_effects_init_surfaces(void) {
...
effects_scanline_init(void) {
    ...
    for (i = 0; i < 2; i++) {
        // lock the surface
        result = IDirectDrawSurface7_Lock(win_d3d_scanline_surface[i], NULL, &surface_desc,
...
effects_rgb_init(void) {
    ...
    for (y = 0; y < pattern_ysize; y++) {
        ...
        for (x = 0; x < pattern_xsize; x++) {
            ...
            if (pattern[(x << 2) + 0] != pattern[(x << 2) + 1] || pattern[(x << 2) + 0] != pattern[(x << 2) + 2]
                ...
                if (pattern[(x << 2) + 1] >= pattern[(x << 2) + 0] && pattern[(x << 2) + 1] >= pattern[(x << 2) + 2]) {
                    max = pattern[(x << 2) + 1];
                    desaturate |= 2;
                }
                if (pattern[(x << 2) + 2] >= pattern[(x << 2) + 0] && pattern[(x << 2) + 2] >= pattern[(x << 2) + 1]) {
                    max = pattern[(x << 2) + 2];
                    desaturate |= 4;
                }
            ...
            for (z = 0; z < 3; z++) {
                if ((active_preset.pattern_mode & 1) == 0 || pattern[(x << 2) + 3]) {
                    ...
                    pattern[(x << 2) + z] = (n > 255) ? 255
                                                      : active_preset.pattern_black_level
...
static int
replicate_pattern(int pattern_xsize, int pattern_ysize, UINT8* pattern_data) {
    DDSURFACEDESC2 surface_desc = {sizeof(DDSURFACEDESC2)};
    HRESULT result;
    unsigned int x, y;
    int colour;
    ...
    // lock the surface
    result = IDirectDrawSurface7_Lock(win_d3d_background_surface, NULL, &surface_desc,
                                      DDLOCK_SURFACEMEMORYPTR | DDLOCK_WAIT, NULL);
    if (result != DD_OK)
        return 1;
...
static void
win_ddrawsurf_plot_pixel(const LPDDSURFACEDESC2 surface_desc, int x, int y, int colour) {
    ...
    // red
    for (shift = 0, mask = surface_desc->DUMMYUNIONNAMEN(4).ddpfPixelFormat.DUMMYUNIONNAMEN(2).dwRBitMask;
         !((0x80000000 >> shift) & mask); shift++) {}
    dest_colour |= ((colour << 8) >> shift) & mask;

    // green
    for (shift = 0, mask = surface_desc->DUMMYUNIONNAMEN(4).ddpfPixelFormat.DUMMYUNIONNAMEN(3).dwGBitMask;
         !((0x80000000 >> shift) & mask); shift++) {}
    dest_colour |= ((colour << 16) >> shift) & mask;

    // blue
    for (shift = 0, mask = surface_desc->DUMMYUNIONNAMEN(4).ddpfPixelFormat.DUMMYUNIONNAMEN(4).dwBBitMask;
         !((0x80000000 >> shift) & mask); shift++) {}
    ...
    switch (surface_desc->DUMMYUNIONNAMEN(4).ddpfPixelFormat.DUMMYUNIONNAMEN(1).dwRGBBitCount >> 3) {
        // 16-bit
        case 2: {
            ((UINT16*)((char*)surface_desc->lpSurface + y * surface_desc->DUMMYUNIONNAMEN(1).lPitch))[x] =
                (UINT16)dest_colour;
            break;
        }
        // 32-bit
        case 4: {
            ((UINT32*)((char*)surface_desc->lpSurface + y * surface_desc->DUMMYUNIONNAMEN(1).lPitch))[x] =
                (UINT32)dest_colour;
            break;
        }
    }
