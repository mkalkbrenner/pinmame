...
// DirectDraw7
typedef HRESULT(WINAPI* type_directdraw_create_ex)(GUID FAR* lpGuid, LPVOID* lplpDD, REFIID iid,
...
static int render_to_blit(struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels,
                          int update);
...
INLINE void
erase_outer_rect(RECT* outer, RECT* inner, LPDIRECTDRAWSURFACE7 surface) {
...
static int
win_d3d_test_hardware_caps(void) {
    // scanlines
...
static void
adjust_prescale(int width, int height) {
    ...
    if (win_d3d_use_prescale)
        fprintf(stderr, "prescale set to %ix%i (image size %ix%i, window size %ix%i)\n", current_prescalex,
                current_prescaley, max_width, max_height, width, height);
    else
        fprintf(stderr, "prescale disabled\n");
#endif
}
...
int
win_d3d_init(int width, int height, int depth, int attributes, double aspect, const struct win_effect_data* effect) {
#ifndef DISABLE_DX7
    ...
    result = IDirectDraw7_SetCooperativeLevel(
        ddraw7, win_video_window,
        (win_window_mode ? DDSCL_NORMAL : DDSCL_ALLOWREBOOT | DDSCL_FULLSCREEN | DDSCL_EXCLUSIVE) | DDSCL_FPUPRESERVE);
    if (result != DD_OK) {
...
enum_callback(LPDDSURFACEDESC2 desc, LPVOID context) {
...
static double
compute_mode_score(int width, int height, int depth, int refresh) {
    ...
    size_score =
        1.0 / (1.0 + (abs(width - target_width) / win_screen_aspect + abs(height - target_height)) / 16 / win_gfx_zoom);
    ...
    fprintf(stderr, "%4ix%4i: size %1.6f aspect %1.4f depth %1.1f refresh %1.4f total %1.6f\n", width, height,
            size_score, aspect_score, depth_score, refresh_score, final_score);
#endif
...
set_resolution(void) {
    ...
    if (!win_window_mode && (win_switch_res || win_switch_bpp)) {
        ...
        best_score = 0.0;
        result = IDirectDraw7_EnumDisplayModes(ddraw7, (win_match_refresh || win_gfx_refresh) ? DDEDM_REFRESHRATES : 0,
                                               NULL, NULL, enum_callback);
        if (result != DD_OK) {
...
static int
print_surface_size(LPDIRECTDRAWSURFACE7 surface, char* string) {
    DDSURFACEDESC2 desc = {sizeof(DDSURFACEDESC2)};

    if (IDirectDrawSurface7_GetSurfaceDesc(surface, &desc) != DD_OK)
        return 1;

    sprintf(string, "%dx%dx%d", (int)desc.dwWidth, (int)desc.dwHeight,
...
static int
print_surface_colour(LPDIRECTDRAWSURFACE7 surface, char* string) {
    DDSURFACEDESC2 desc = {sizeof(DDSURFACEDESC2)};
...
create_effects_surfaces(void) {
    ...
    if (win_d3d_use_scanlines) {
        ...
        for (i = 0; i < 2; i++) {
            ...
            if (desc.dwWidth < d3d_device_desc.dwMinTextureWidth
                || desc.dwHeight < d3d_device_desc.dwMinTextureHeight) {
...
enum_textures_callback(LPDDPIXELFORMAT pixelformat, LPVOID preferred_pixelformat) {
    if (pixelformat->dwFlags == DDPF_RGB && pixelformat->DUMMYUNIONNAMEN(1).dwRGBBitCount == 16) {
        ...
        if (pixelformat->DUMMYUNIONNAMEN(2).dwRBitMask == 0x7C00 && pixelformat->DUMMYUNIONNAMEN(3).dwGBitMask == 0x03E0
    ...
    if (pixelformat->DUMMYUNIONNAMEN(2).dwRBitMask == 0xF800 && pixelformat->DUMMYUNIONNAMEN(3).dwGBitMask == 0x07E0
        && pixelformat->DUMMYUNIONNAMEN(4).dwBBitMask == 0x001F) {
        memcpy(preferred_pixelformat, pixelformat, sizeof(DDPIXELFORMAT));
    }
...
create_blit_surface(void) {
    ...
    if (texture_width > d3d_device_desc.dwMaxTextureWidth || texture_height > d3d_device_desc.dwMaxTextureHeight) {
        if (verbose)
            fprintf(stderr, "Error: required texture size too large (max: %ix%i, need %ix%i)\n",
                    (int)d3d_device_desc.dwMaxTextureWidth, (int)d3d_device_desc.dwMaxTextureHeight, texture_width,
                    texture_height);
        goto error_handling;
    }
    ...
    if (win_d3d_use_prescale || win_d3d_use_feedback) {
        ...
        if (preprocess_desc.dwWidth > d3d_device_desc.dwMaxTextureWidth
            || preprocess_desc.dwHeight > d3d_device_desc.dwMaxTextureHeight) {
            if (verbose)
...
set_brightness(void) {
    ...
    result =
        IDirectDrawSurface7_QueryInterface(primary_surface, &IID_IDirectDrawGammaControl, (void**)gamma_controladdr);
    if (result != DD_OK) {
        if (verbose)
...
erase_surfaces(void) {
    ...
    if (win_d3d_background_surface)
        result = IDirectDrawSurface7_Blt(win_d3d_background_surface, NULL, NULL, NULL, DDBLT_COLORFILL | DDBLT_WAIT,
                                         &blitfx);
...
static int
restore_surfaces(void) {
    HRESULT result;

#define RESTORE_SURFACE(s)                                                                                             \
    do {                                                                                                               \
        if (s) {                                                                                                       \
            result = IDirectDrawSurface7_Restore(s);                                                                   \
            if (result != DD_OK) {                                                                                     \
                return 1;                                                                                              \
            }                                                                                                          \
        }                                                                                                              \
...
static void
release_surfaces(void) {

#define RELEASE_SURFACE(s)                                                                                             \
    do {                                                                                                               \
        if (s) {                                                                                                       \
...
static void
compute_color_masks(const DDSURFACEDESC2* desc) {
    // 16bpp case
...
int
win_d3d_draw(struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels, int update) {
    int result;
...
static int
lock_must_succeed(const struct rectangle* bounds, void* vector_dirty_pixels) {
...
static int
render_to_blit(struct mame_bitmap* bitmap, const struct rectangle* bounds, void* vector_dirty_pixels, int update) {
...
tryagain:
    if (win_d3d_tex_manage) {
        ...
        result = IDirectDrawSurface7_Lock(blit_surface, &src, &blit_desc,
                                          DDLOCK_SURFACEMEMORYPTR | DDLOCK_WRITEONLY | DDLOCK_DISCARDCONTENTS
                                              | (wait_for_lock ? DDLOCK_WAIT : 0),
                                          NULL);
    } else {
        // attempt to lock the blit surface
        result = IDirectDrawSurface7_Lock(blit_surface, NULL, &blit_desc,
                                          DDLOCK_SURFACEMEMORYPTR | (wait_for_lock ? DDLOCK_WAIT : 0), NULL);
    }
    ...
    if (bounds && !update && !vector_dirty_pixels) {
        ...
        if (blit_swapxy) {
            params.dstxoffs += (bounds_disoriented.min_x + (blit_flipy ? win_visible_rect.top : -win_visible_rect.top))
                               * effect_min_xscale;
            params.dstyoffs +=
                (bounds_disoriented.min_y + (blit_flipx ? win_visible_rect.left : -win_visible_rect.left))
                * effect_min_yscale;
        } else {
            params.dstxoffs +=
                (bounds_disoriented.min_x + (blit_flipx ? win_visible_rect.left : -win_visible_rect.left))
                * effect_min_xscale;
            params.dstyoffs += (bounds_disoriented.min_y + (blit_flipy ? win_visible_rect.top : -win_visible_rect.top))
                               * effect_min_yscale;
        }
    ...
    if (!win_d3d_tex_manage) {
        ...
        result = IDirectDrawSurface7_BltFast(texture_surface, params.dstxoffs - dstxoffs, params.dstyoffs, blit_surface,
                                             &blt_src, DDBLTFAST_WAIT);
        if (result == DDERR_SURFACELOST)
            goto surface_lost;
        if (result != DD_OK) {
...
static int
render_and_flip(LPRECT src, LPRECT dst, int update, int wait_for_lock) {
    ...
    if (src->left != prev_src.left || src->right != prev_src.right || src->top != prev_src.top
        || src->bottom != prev_src.bottom || dst->left != prev_dst.left || dst->right != prev_dst.right
        || dst->top != prev_dst.top || dst->bottom != prev_dst.bottom) {
        position_changed = 1;
...
tryagain:
    ...
    if (win_d3d_use_prescale || win_d3d_use_feedback) {
        ...
        result = IDirect3DDevice7_DrawPrimitive(d3d_device7, D3DPT_TRIANGLESTRIP, D3DFVF_TLVERTEX,
                                                (void*)preprocess_vertex, 4, 0);
        if (result != DD_OK) {
            // error, print the error and fall back
            if (verbose)
    ...
    // now render the image using 3d hardware
    result =
        IDirect3DDevice7_DrawPrimitive(d3d_device7, D3DPT_TRIANGLESTRIP, D3DFVF_TLVERTEX2, (void*)screen_vertex, 4, 0);
    if (result != DD_OK) {
        // error, print the error and fall back
        if (verbose)
    ...
    // full screen mode: flip
        ...
        result = IDirectDrawSurface7_Flip(primary_surface, NULL,
                                          DDFLIP_WAIT | ((!win_triple_buffer || !throttle) ? DDFLIP_NOVSYNC : 0));
    ...
    if (win_d3d_use_rgbeffect) {
        ...
        if (win_d3d_use_auto_effect
            && (win_window_mode || !win_keep_aspect || win_force_int_stretch == FORCE_INT_STRECT_NONE
                || win_force_int_stretch == FORCE_INT_STRECT_HOR || zoom != win_d3d_current_zoom)) {
            RECT rgb_dst;
            RECT rgb_src = {0, 0, win_d3d_effects_swapxy ? zoom * win_visible_width : dst->right - dst->left,
            ...
            result = IDirectDrawSurface7_Blt(back_surface, win_window_mode ? &rgb_dst : dst, win_d3d_background_surface,
                                             &rgb_src, DDBLT_ASYNC, NULL);
        } else {
            RECT rect = {0, 0, dst->right - dst->left, dst->bottom - dst->top};

            result = IDirectDrawSurface7_BltFast(back_surface, win_window_mode ? 0 : dst->left,
                                                 win_window_mode ? 0 : dst->top, win_d3d_background_surface, &rect,
                                                 DDBLTFAST_WAIT);
        }
        if (result != DD_OK) {
            // error, print the error and fall back
...
static void
set_texture_coordinates(LPRECT src, int texture_width, int texture_height) {
    if (win_d3d_tex_manage) {
        if (blit_swapxy) {
        ...
        if (blit_swapxy) {
            screen_vertex[2].tu = screen_vertex[3].tu =
                !blit_flipy ? (float)(src->right - src->left) / texture_width : 0;
            screen_vertex[0].tu = screen_vertex[1].tu =
                !blit_flipy ? 0 : (float)(src->right - src->left) / texture_width;
            screen_vertex[1].tv = screen_vertex[3].tv =
                !blit_flipx ? (float)(src->bottom - src->top) / texture_height : 0;
            screen_vertex[0].tv = screen_vertex[2].tv =
                !blit_flipx ? 0 : (float)(src->bottom - src->top) / texture_height;
        } else {
            screen_vertex[0].tu = screen_vertex[2].tu =
                blit_flipx ? (float)(src->right - src->left) / texture_width : 0;
            screen_vertex[1].tu = screen_vertex[3].tu =
                blit_flipx ? 0 : (float)(src->right - src->left) / texture_width;
            screen_vertex[0].tv = screen_vertex[1].tv =
                blit_flipy ? (float)(src->bottom - src->top) / texture_height : 0;
            screen_vertex[2].tv = screen_vertex[3].tv =
                blit_flipy ? 0 : (float)(src->bottom - src->top) / texture_height;
        }
    }
}
...
static void
init_vertices_screen(LPRECT src, LPRECT dst) {
    int i;
    ...
    if (win_window_mode) {
        ...
        screen_vertex[0].sx = -0.5f;
        screen_vertex[0].sy = -0.5f;
        screen_vertex[1].sx = -0.5f + (float)(dst->right - dst->left);
        screen_vertex[1].sy = -0.5f;
        screen_vertex[2].sx = -0.5f;
        screen_vertex[2].sy = -0.5f + (float)(dst->bottom - dst->top);
        screen_vertex[3].sx = -0.5f + (float)(dst->right - dst->left);
        screen_vertex[3].sy = -0.5f + (float)(dst->bottom - dst->top);
    } else {
        screen_vertex[0].sx = -0.5f + (float)dst->left;
        screen_vertex[0].sy = -0.5f + (float)dst->top;
        screen_vertex[1].sx = -0.5f + (float)dst->right;
        screen_vertex[1].sy = -0.5f + (float)dst->top;
        screen_vertex[2].sx = -0.5f + (float)dst->left;
        screen_vertex[2].sy = -0.5f + (float)dst->bottom;
        screen_vertex[3].sx = -0.5f + (float)dst->right;
        screen_vertex[3].sy = -0.5f + (float)dst->bottom;
    }
...
static void
init_vertices_preprocess(LPRECT src) {
    ...
    preprocess_vertex[0].sx = -0.5f + rect.left;
    preprocess_vertex[0].sy = -0.5f + rect.top;
    preprocess_vertex[1].sx = -0.5f + rect.right;
    preprocess_vertex[1].sy = -0.5f + rect.top;
    preprocess_vertex[2].sx = -0.5f + rect.left;
    preprocess_vertex[2].sy = -0.5f + rect.bottom;
    preprocess_vertex[3].sx = -0.5f + rect.right;
    preprocess_vertex[3].sy = -0.5f + rect.bottom;
}
