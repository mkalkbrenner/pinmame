...
static void
update_sample_adjustment(int buffered) {
...
static void
copy_sample_data(INT16* data, int bytes_to_copy) // adopted from MAME 0.105
{
    ...
    result = IDirectSoundBuffer_Lock(stream_buffer, stream_buffer_in, bytes_to_copy, &buffer1, &length1, &buffer2,
                                     &length2, 0);
...
osd_update_audio_stream(INT16* buffer) {
...
#if (DISPLAY_UNDEROVERFLOW || LOG_SOUND)
    {
        ...
        if (total_frames > INGORE_UNDERFLOW_FRAMES
            && (buffer_overflows != prev_overflows || buffer_underflows != prev_underflows)) {
...
BOOL CALLBACK
EnumCallBack(LPGUID guid, LPCSTR desc, LPCSTR mod, LPVOID list) {
    AudioDevice* ad;
...
//============================================================
char*
osd_get_audio_device_description(int num) {
...
//============================================================
char*
osd_get_audio_device_module(int num) {
...
dsound_create_buffers(void) {
    ...
    if (verbose)
        fprintf(stderr, "Primary buffer: %d Hz, %d bits, %d channels\n", (int)primary_format.nSamplesPerSec,
                (int)primary_format.wBitsPerSample, (int)primary_format.nChannels);
