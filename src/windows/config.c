...
struct rc_option pinmame_opts[] = {
    ...
    {"dmd_colorize", NULL, rc_bool, &pmoptions.dmd_colorize, "0", 0, 0, NULL,
     "Use distinct colors for DMD intensities"},
    ...
    {"resampling_quality", NULL, rc_int, &pmoptions.resampling_quality, "0", 0, 1, NULL,
     "Quality of the resampling implementation (0=Fast,1=Normal)"},
#if defined(VPINMAME_ALTSOUND) || defined(VPINMAME_PINSOUND)
    {"sound_mode", NULL, rc_int, &pmoptions.sound_mode, "0", 0, 3, NULL,
...
#endif /* PROC_SUPPORT */
    {"vgmwrite", NULL, rc_bool, &pmoptions.vgmwrite, "0", 0, 0, NULL,
...
static int
video_set_beam(struct rc_option* option, const char* arg, int priority) {
...
static int
video_set_flicker(struct rc_option* option, const char* arg, int priority) {
...
static int
video_set_intensity(struct rc_option* option, const char* arg, int priority) {
...
static int
video_set_debugres(struct rc_option* option, const char* arg, int priority) {
    if (!strcmp(arg, "auto")) {
...
static int
init_errorlog(struct rc_option* option, const char* arg, int priority) {
...
struct rc_option core_opts[] = {
    ...
    {"debug_resolution", "dr", rc_string, &debugres, "auto", 0, 0, video_set_debugres,
...
int
penalty_compare(const char* s, const char* l) {
    int gaps = 0;
    int match;
    int last = 1;
 ...
 */
int
parse_config(const char* filename, const struct GameDriver* gamedrv) {
    mame_file* f;
    char buffer[128];
    int retval = 0;
...
int
cli_frontend_init(int argc, char** argv) {
    ...
    if (game_index == -1) {
        ...
        if (strcmp(gamename, "random") == 0) {
            ...
            fprintf(stderr, "running %s (%s) [press return]", drivers[game_index]->name,
                    drivers[game_index]->description);
            getchar();
        }
    }
...
FILE*
config_get_logfile(void) {
    return errorlog ? logfile : NULL;
}
#else                                                             /* VPINMAME */
...
static char*
win_strip_extension(char* filename) {
    char* newname;
    char* c;
