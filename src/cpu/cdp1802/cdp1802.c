...
static UINT8 cdp1802_reg_layout[] = {
    CDP1802_R0, CDP1802_R1, CDP1802_R2, CDP1802_R3, CDP1802_R4, CDP1802_R5, CDP1802_R6, CDP1802_R7, 0xFF,

    CDP1802_R8, CDP1802_R9, CDP1802_Ra, CDP1802_Rb, CDP1802_Rc, CDP1802_Rd, CDP1802_Re, CDP1802_Rf, 0xFF,

    CDP1802_P,  CDP1802_X,  CDP1802_D,  CDP1802_B,  CDP1802_T,  0};
...
unsigned
cdp1802_get_reg(int regnum) {
    switch (regnum) {
        case REG_PC:
            return cdp1802.r[cdp1802.p];
        case REG_SP:
            return 0;
        case CDP1802_P:
            return cdp1802.p;
        ...
        case CDP1802_R9:
            return cdp1802.r[9];
        case CDP1802_Ra:
            return cdp1802.r[0xa];
        case CDP1802_Rb:
            return cdp1802.r[0xb];
        case CDP1802_Rc:
            return cdp1802.r[0xc];
        case CDP1802_Rd:
            return cdp1802.r[0xd];
        case CDP1802_Re:
            return cdp1802.r[0xe];
        case CDP1802_Rf:
            return cdp1802.r[0xf];
        case CDP1802_DF:
            return cdp1802.df;
        case CDP1802_IE:
            return cdp1802.ie;
        case CDP1802_Q:
            return cdp1802.q;
        case REG_PREVIOUSPC:
            return cdp1802.oldpc;
    }
    return 0;
}

void
cdp1802_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
            cdp1802.r[cdp1802.p] = val;
            change_pc16(cdp1802.r[cdp1802.p]);
            break;
        case REG_SP:
            break;
...
INLINE void
cdp1802_add(int left, int right) {
...
INLINE void
cdp1802_add_carry(int left, int right) {
...
INLINE void
cdp1802_sub(int left, int right) {
...
INLINE void
cdp1802_sub_carry(int left, int right) {
...
INLINE void
cdp1802_short_branch(int taken) {
    if (taken) {
...
static void
cdp1802_output_state_code(void) {
...
static void
cdp1802_set_interrupt_line(int state) {
    cdp1802.irq = state;
}

static void
cdp1802_set_dmain_line(int state) {
    cdp1802.dmain = state;
}

static void
cdp1802_set_dmaout_line(int state) {
    cdp1802.dmaout = state;
}

void
cdp1802_set_irq_line(int irqline, int state) {
    switch (irqline) {
        case CDP1802_INPUT_LINE_INT:
            cdp1802_set_interrupt_line(state);
            break;
        case CDP1802_INPUT_LINE_DMAIN:
            cdp1802_set_dmain_line(state);
            break;
        case CDP1802_INPUT_LINE_DMAOUT:
            cdp1802_set_dmaout_line(state);
            break;
    }
}

void
cdp1802_set_irq_callback(int (*callback)(int irqline)) {
    // no callbacks
}
 ...
 ****************************************************************************/
const char*
cdp1802_info(void* context, int regnum) {
    ...
    switch (regnum) {
        case CPU_INFO_REG + CDP1802_R0:
            sprintf(buffer[which], "R0:%.4x", r->r[0]);
            break;
        case CPU_INFO_REG + CDP1802_R1:
            sprintf(buffer[which], "R1:%.4x", r->r[1]);
            break;
        case CPU_INFO_REG + CDP1802_R2:
            sprintf(buffer[which], "R2:%.4x", r->r[2]);
            break;
        case CPU_INFO_REG + CDP1802_R3:
            sprintf(buffer[which], "R3:%.4x", r->r[3]);
            break;
        case CPU_INFO_REG + CDP1802_R4:
            sprintf(buffer[which], "R4:%.4x", r->r[4]);
            break;
        case CPU_INFO_REG + CDP1802_R5:
            sprintf(buffer[which], "R5:%.4x", r->r[5]);
            break;
        case CPU_INFO_REG + CDP1802_R6:
            sprintf(buffer[which], "R6:%.4x", r->r[6]);
            break;
        case CPU_INFO_REG + CDP1802_R7:
            sprintf(buffer[which], "R7:%.4x", r->r[7]);
            break;
        case CPU_INFO_REG + CDP1802_R8:
            sprintf(buffer[which], "R8:%.4x", r->r[8]);
            break;
        case CPU_INFO_REG + CDP1802_R9:
            sprintf(buffer[which], "R9:%.4x", r->r[9]);
            break;
        case CPU_INFO_REG + CDP1802_Ra:
            sprintf(buffer[which], "Ra:%.4x", r->r[0xa]);
            break;
        case CPU_INFO_REG + CDP1802_Rb:
            sprintf(buffer[which], "Rb:%.4x", r->r[0xb]);
            break;
        case CPU_INFO_REG + CDP1802_Rc:
            sprintf(buffer[which], "Rc:%.4x", r->r[0xc]);
            break;
        case CPU_INFO_REG + CDP1802_Rd:
            sprintf(buffer[which], "Rd:%.4x", r->r[0xd]);
            break;
        case CPU_INFO_REG + CDP1802_Re:
            sprintf(buffer[which], "Re:%.4x", r->r[0xe]);
            break;
        case CPU_INFO_REG + CDP1802_Rf:
            sprintf(buffer[which], "Rf:%.4x", r->r[0xf]);
            break;
        case CPU_INFO_REG + CDP1802_P:
            sprintf(buffer[which], "P:%x", r->p);
            break;
        case CPU_INFO_REG + CDP1802_X:
            sprintf(buffer[which], "X:%x", r->x);
            break;
        case CPU_INFO_REG + CDP1802_D:
            sprintf(buffer[which], "D:%.2x", r->d);
            break;
        case CPU_INFO_REG + CDP1802_B:
            sprintf(buffer[which], "B:%.2x", r->b);
            break;
        case CPU_INFO_REG + CDP1802_T:
            sprintf(buffer[which], "T:%.2x", r->t);
            break;
        case CPU_INFO_REG + CDP1802_DF:
            sprintf(buffer[which], "DF:%x", r->df);
            break;
        case CPU_INFO_REG + CDP1802_IE:
            sprintf(buffer[which], "IE:%x", r->ie);
            break;
        case CPU_INFO_REG + CDP1802_Q:
            sprintf(buffer[which], "Q:%x", r->q);
            break;
        case CPU_INFO_FLAGS:
            sprintf(buffer[which], "%s%s%s", r->df ? "DF" : "..", r->ie ? "IE" : "..", r->q ? "Q" : ".");
            break;
        case CPU_INFO_NAME:
            return "CDP1802";
        ...
        case CPU_INFO_CREDITS:
            return "Copyright (c) 2008 Peter Trauner, all rights reserved.";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)cdp1802_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)cdp1802_win_layout;
    }
    return buffer[which];
}
...
#ifndef MAME_DEBUG
unsigned
cdp1802_dasm(char* buffer, unsigned pc) {
