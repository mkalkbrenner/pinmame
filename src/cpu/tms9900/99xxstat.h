...
static const UINT16 right_shift_mask_table[17] = {0xFFFF, 0x7FFF, 0x3FFF, 0x1FFF, 0x0FFF, 0x07FF,
                                                  0x03FF, 0x01FF, 0x00FF, 0x007F, 0x003F, 0x001F,
                                                  0x000F, 0x0007, 0x0003, 0x0001, 0x0000};

static const UINT16 inverted_right_shift_mask_table[17] = {0x0000, 0x8000, 0xC000, 0xE000, 0xF000, 0xF800,
                                                           0xFC00, 0xFE00, 0xFF00, 0xFF80, 0xFFC0, 0xFFE0,
                                                           0xFFF0, 0xFFF8, 0xFFFC, 0xFFFE, 0xFFFF};

INLINE UINT16
logical_right_shift(UINT16 val, int c) {
    return ((val >> c) & right_shift_mask_table[c]);
}

INLINE INT16
arithmetic_right_shift(INT16 val, int c) {
    if (val < 0)
...
*/
INLINE void
setst_c_lae(UINT16 to, UINT16 val) {
    I.STATUS &= ~(ST_LGT | ST_AGT | ST_EQ);
...
#define wadd(addr, expr)                                                                                               \
    {                                                                                                                  \
        int lval = setst_add_laeco(readword(addr), (expr));                                                            \
        writeword((addr), lval);                                                                                       \
    }
#define wsub(addr, expr)                                                                                               \
    {                                                                                                                  \
        int lval = setst_sub_laeco(readword(addr), (expr));                                                            \
        writeword((addr), lval);                                                                                       \
    }
...
static INT32 asm setst_add_32_laeco(register INT32 a, register INT32 b, register INT16 st) {
    ...
    mr r6,
        a // save operand
            ...
            addco.r3,
        b,
        a // add, set CR0, and set CA and OV
            clrlwi st,
        st,
        21            // clear L, A, E, C, O flags (-> keep bits 21-31)
            mcrxr cr1 // move XER (-> CA and CO bits) to CR1

                beq
            - null_result // if EQ is set, jump to null_result.  No jump is more likely than a jump.
                bgt
            + positive_result // if GT is set, jump to positive_result.  A jump is more likely than no jump.
                ori st,
        st,
        ST_LGT     // if result < 0, set ST_LGT
            b next // now set carry & overflow as needed

                null_result : ori st,
                              st,
                              ST_EQ // if result == 0, set ST_EQ
                                  b next
        ...
        : ori st,
          st,
          ST_LGT
              | ST_AGT // if result > 0, set ST_LGT and ST_AGT
        ...
        andi.st,
          ...
          r6 andis.r6,
          r6,
          0x0800 beq + nodecimalcarry ori st,
          st,
          ST_DC nodecimalcarry :
#endif

        bf
        + cr1 * 4
        + 2,
          nocarry // if CA is not set, jump to nocarry.  A jump is more likely than no jump.
              ori st,
          st,
          ST_C // else set ST_C
                  nocarry : bflr
                            + cr1 * 4
                            + 1 // if OV is not set, return.  A jump is more likely than no jump.
                            ori st,
          st,
          ST_OV   // else set ST_OV
              blr // return
}
...
static INT32 asm setst_sub_32_laeco(register INT32 a, register INT32 b, register INT16 st) {
    ...
    mr r6,
        a // save operand
            ...
            subco.r3,
        a,
        b // sub, set CR0, and set CA and OV
            clrlwi st,
        st,
        21            // clear L, A, E, C, O flags (-> keep bits 21-31)
            mcrxr cr1 // move XER (-> CA and CO bits) to CR1
        ...
        - cr1 * 4
        + 2,
          nocarry // if CA is not set, jump to nocarry.  No jump is more likely than a jump.
              ori st,
          st,
          ST_C // else set ST_C
                  nocarry : bflr
...
// Set laeco for add
//
static INT16 asm setst_add_laeco(register INT16 a, register INT16 b) { // a -> r3, b -> r4
    lwz r6,
        I(RTOC) // load pointer to I
        ...
        4(r6)
            ...
            mflr r12
                ...
                bl setst_sub_32_laeco // perform substraction and set flags
                    ...
                    mtlr r12 srwi r3,
        r3, 16 sth r5, 4(r6)blr
}

//
// Set laecop for add (BYTE)
//
static INT8 asm setst_addbyte_laecop(register INT8 a, register INT8 b) { // a -> r3, b -> r4
    lwz r6,
        I(RTOC) // load pointer to I
        ...
        mflr r12 // save LR (we KNOW setst_add_32_laeco will not alter this register)
            ...
            bl setst_add_32_laeco // perform addition and set flags
                ...
                srwi r3,
        r3,
        24       // shift back result
        mtlr r12 // restore LR
            sth r5,
        4(r6) // save new ST
        stb r3,
        lastparity(RTOC) // copy result to lastparity
        blr              // and return
}
...
//
static INT8 asm setst_subbyte_laecop(register INT8 a, register INT8 b) { // a -> r3, b -> r4
    lwz r6,
        I(RTOC)
        ...
        4(r6)
            ...
            mflr r12
                ...
                bl setst_sub_32_laeco // perform substraction and set flags
                    ...
                    srwi r3,
        r3, 24 mtlr r12 sth r5, 4(r6)stb r3, lastparity(RTOC) blr
}
...
*/
INLINE INT8
setst_addbyte_laecop(int a, int b) {
    unsigned int res;
    INT8 res2;
...
*/
INLINE INT8
setst_subbyte_laecop(int a, int b) {
    unsigned int res;
    INT8 res2;
...
*/
INLINE UINT16
setst_sra_laec(INT16 a, UINT16 c) {
...
*/
INLINE UINT16
setst_srl_laec(UINT16 a, UINT16 c) {
    I.STATUS &= ~(ST_LGT | ST_AGT | ST_EQ | ST_C);
...
//
INLINE UINT16
setst_src_laec(UINT16 a, UINT16 c) {
    I.STATUS &= ~(ST_LGT | ST_AGT | ST_EQ | ST_C);
...
//
INLINE UINT16
setst_sla_laeco(UINT16 a, UINT16 c) {
    I.STATUS &= ~(ST_LGT | ST_AGT | ST_EQ | ST_C | ST_OV);
