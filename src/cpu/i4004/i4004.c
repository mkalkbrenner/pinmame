...
static const UINT8 kbp_table[] = {0x00, 0x01, 0x02, 0x0f, 0x03, 0x0f, 0x0f, 0x0f,
                                  0x04, 0x0f, 0x0f, 0x0f, 0x0f, 0x0f, 0x0f, 0x0f};

/* Layout of the registers in the debugger */
static UINT8 i4004_reg_layout[] = {I4004_PC, I4004_S1, I4004_S2, I4004_S3, I4004_RAM, I4004_A,  /* I4004_C, */ I4004_T,
                                   0xFF,     I4004_01, I4004_23, I4004_45, I4004_67,  I4004_89, I4004_AB,
                                   I4004_CD, I4004_EF, 0};
 ...
 ****************************************************************************/
void
i4004_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
            I.PC.w.l = val;
            change_pc16(I.PC.d);
 ...
 ****************************************************************************/
const char*
i4004_info(void* context, int regnum) {
    ...
    switch (regnum) {
        case CPU_INFO_REG + I4004_PC:
            sprintf(buffer[which], "PC:%03X", r->PC.w.l);
            break;
        case CPU_INFO_REG + I4004_S1:
            sprintf(buffer[which], "S1:%03X", r->S1.w.l);
            break;
        case CPU_INFO_REG + I4004_S2:
            sprintf(buffer[which], "S2:%03X", r->S2.w.l);
            break;
        case CPU_INFO_REG + I4004_S3:
            sprintf(buffer[which], "S3:%03X", r->S3.w.l);
            break;
        case CPU_INFO_REG + I4004_EF:
            sprintf(buffer[which], "EF:%02X", r->REF);
            break;
        case CPU_INFO_REG + I4004_CD:
            sprintf(buffer[which], "CD:%02X", r->RCD);
            break;
        case CPU_INFO_REG + I4004_AB:
            sprintf(buffer[which], "AB:%02X", r->RAB);
            break;
        case CPU_INFO_REG + I4004_89:
            sprintf(buffer[which], "89:%02X", r->R89);
            break;
        case CPU_INFO_REG + I4004_67:
            sprintf(buffer[which], "67:%02X", r->R67);
            break;
        case CPU_INFO_REG + I4004_45:
            sprintf(buffer[which], "45:%02X", r->R45);
            break;
        case CPU_INFO_REG + I4004_23:
            sprintf(buffer[which], "23:%02X", r->R23);
            break;
        case CPU_INFO_REG + I4004_01:
            sprintf(buffer[which], "01:%02X", r->R01);
            break;
        case CPU_INFO_REG + I4004_RAM:
            sprintf(buffer[which], "RA:%03X", r->ramaddr.w.l);
            break;
        case CPU_INFO_REG + I4004_A:
            sprintf(buffer[which], "ACC:%X", I.accu);
            break;
        case CPU_INFO_REG + I4004_C:
            sprintf(buffer[which], "CRY:%X", I.carry);
            break;
        case CPU_INFO_REG + I4004_T:
            sprintf(buffer[which], "TST:%X", I.test);
            break;
        case CPU_INFO_FLAGS:
            sprintf(buffer[which], "%c", (I.carry ? 'C' : '.'));
            break;
        case CPU_INFO_NAME:
            return "4004";
        ...
        case CPU_INFO_CREDITS:
            return "written 2002 by G. Volkenborn";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)i4004_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)i4004_win_layout;
    }
    return buffer[which];
}

void
i4004_set_irq_line(int irqline, int state) {
    /* no IRQ line on the 4004 */
}

void
i4004_set_irq_callback(int (*callback)(int)) {
    /* no IRQ line on the 4004 */
}

unsigned
i4004_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
