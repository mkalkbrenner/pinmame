...
static UINT8 cop420_reg_layout[] = {COP400_PC, COP400_A,  COP400_B,  COP400_G,   COP400_EN,  COP400_Q, 0xFF,
...
cop420_init(void) {
    ...
    timer_adjust(cop420_counter_timer, TIME_IN_HZ(Machine->drv->cpu[cpu].cpu_clock), 0,
                 TIME_IN_HZ(Machine->drv->cpu[cpu].cpu_clock));
    ...
    InstLen[0x60] = InstLen[0x61] = InstLen[0x62] = InstLen[0x63] = InstLen[0x68] = InstLen[0x69] = InstLen[0x6a] =
        InstLen[0x6b] = InstLen[0x33] = InstLen[0x23] = 2;
...
void
cop420_set_irq_line(int irqline, int state) {
    /* no IRQ line on the COP */
}

void
cop420_set_irq_callback(int (*callback)(int)) {
    /* no IRQ line on the COP */
}
 ...
 ****************************************************************************/
void
cop420_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
            PC = val;
            change_pc16(PC);
...
const char*
cop420_info(void* context, int regnum) {
    ...
    switch (regnum) {
        case CPU_INFO_REG + COP400_PC:
            sprintf(buffer[which], "PC:%03X", r->R_PC);
            break;
        case CPU_INFO_REG + COP400_A:
            sprintf(buffer[which], "A:%X", r->R_A);
            break;
        case CPU_INFO_REG + COP400_B:
            sprintf(buffer[which], "B:%02X", r->R_B);
            break;
        case CPU_INFO_REG + COP400_EN:
            sprintf(buffer[which], "EN:%X", r->R_EN);
            break;
        case CPU_INFO_REG + COP400_G:
            sprintf(buffer[which], "G:%X", r->R_G);
            break;
        case CPU_INFO_REG + COP400_Q:
            sprintf(buffer[which], "Q:%02X", r->R_Q);
            break;
        case CPU_INFO_REG + COP400_SA:
            sprintf(buffer[which], "SA:%03X", r->R_SA);
            break;
        case CPU_INFO_REG + COP400_SB:
            sprintf(buffer[which], "SB:%03X", r->R_SB);
            break;
        case CPU_INFO_REG + COP400_SC:
            sprintf(buffer[which], "SC:%03X", r->R_SC);
            break;
        case CPU_INFO_REG + COP400_SIO:
            sprintf(buffer[which], "SIO:%X", r->R_SIO);
            break;
        case CPU_INFO_REG + COP400_T:
            sprintf(buffer[which], "T:%03X", r->counter);
            break;

        case CPU_INFO_FLAGS:
            sprintf(buffer[which], "%c%c", r->R_C ? 'C' : '.', r->R_SKL ? 'S' : '.');
            break;

        case CPU_INFO_NAME:
            return "COP420";
        case CPU_INFO_FAMILY:
            return "National Semiconductor COP420";
        case CPU_INFO_VERSION:
            return "1.1";
        case CPU_INFO_FILE:
            return __FILE__;
        case CPU_INFO_CREDITS:
            return "Copyright (C) 2006 MAME Team";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)cop420_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)cop420_win_layout;
    }
    return buffer[which];
}
...
READ_HANDLER(cop420_internal_r) { return offset < 64 ? R.R_RAM[offset] : 0; }

unsigned
cop420_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
