...
#define USE_MAME_TIMERS                                                                                                \
    1 //Set to 1 for performance but possibly less accurate timing, to 0 for more accuracy but huge performance penalty
...
void
at91_set_transmit_serial(void (*fp)(int usartno, data8_t* data, int size)) {
    at91_transmit_serial = fp;
}

void
at91_set_serial_receive_ready(void (*fp)(int usartno)) {
...
#define RESET_ICOUNT                                                                                                   \
    ARM7_ICOUNT = cycles;                                                                                              \
    at91.tot_prev_cycles = 0;
#define CAPTURE_NUM_CYCLES                                                                                             \
    at91.prev_cycles = cycles - ARM7_ICOUNT - at91.tot_prev_cycles;                                                    \
...
//Timer macros
#define PS_TIMER_CLOCK_ENABLED(x)                                                                                      \
    (at91.power_status & (0x10 << (x))) //Power Enabled for Clock: TC0 = bit 4, TC1 = bit 5, TC2 = bit 6
...
typedef struct {
    ...
    data32_t
        aic_irqmask; //holds the irq enabled mask for each interrupt 0-31 (0 = disabled, 1 = enabled) - 1 bit per interrupt.
    ...
    data32_t
        pio_enabled_status;     //holds status of each pio pin, 1 bit per pin. (0 = peripheral control, 1 = PIO control)
    data32_t pio_output_status; //holds output status of each pio pin, 1 bit per pin. (0 = Input Pin, 1 = Output Pin)
    data32_t
        pio_output_data_status; //holds output data status of each pio pin, 1 bit per pin. (0 = Pin Value is programmed as 0, 1 = Pin is programmed 1) - Only if pin is under PIO control and set as an output pin.
    data32_t pio_data_status;   //holds data status of each pio pin, 1 bit per pin. (0 = pin is low, 1 = pin is hi)
    data32_t
        pio_irq_mask; //holds irq mask of each pio pin, 1 bit per pin. (0 = irq disabled for the pin, 1 = irq enabled for the pin)
    data32_t
        pio_irq_status; //holds irq status of each pio pin, 1 bit per pin. (0 = no change since last read, 1 = change in value of pin since last read)
    data32_t
        power_status; //holds enabled status of each peripheral clock, 1 bit per pin. (0 = irq disabled for the pin, 1 = irq enabled for the pin)
...
static AT91_REGS_USART at91usart[2] = {{0, US_TXRDY | US_TXEMPTY | US_ENDTX, 0, 0, 0, 0, 0, 0},
                                       {0, US_TXRDY | US_TXEMPTY | US_ENDTX, 0, 0, 0, 0, 0, 0}};
...
/* external interfaces */
void
at91_set_ram_pointers(data32_t* reset_ram_ptr, data32_t* page0_ram_ptr) {
    at91rs.page0_ram_ptr = page0_ram_ptr;
    at91rs.reset_ram_ptr = reset_ram_ptr;
}

void
at91_init_jit(int min_addr, int max_addr) {
...
static const char* aic_offset[] = {"SOURCE MODE 0(FIQ)",
                                   "SOURCE MODE 1(SWIRQ)",
                                   "SOURCE MODE 2(UART0)",
                                   "SOURCE MODE 3(UART1)",
                                   "SOURCE MODE 4(TC0)",
                                   ...
                                   "SOURCE MODE 31(RSVD)",
                                   "SOURCE VECTOR 0(FIQ)",
                                   "SOURCE VECTOR 1(SWIRQ)",
                                   "SOURCE VECTOR 2(UART0)",
                                   "SOURCE VECTOR 3(UART1)",
                                   "SOURCE VECTOR 4(TC0)",
                                   "SOURCE VECTOR 5(TC1)",
                                   "SOURCE VECTOR 6(TC2)",
                                   "SOURCE VECTOR 7(WDOG)",
                                   "SOURCE VECTOR 8(PIO)",
                                   "SOURCE VECTOR 9(RSVD)",
                                   "SOURCE VECTOR 10(RSVD)",
                                   "SOURCE VECTOR 11(RSVD)",
                                   "SOURCE VECTOR 12(RSVD)",
                                   "SOURCE VECTOR 13(RSVD)",
                                   "SOURCE VECTOR 14(RSVD)",
                                   "SOURCE VECTOR 15(RSVD)",
                                   "SOURCE VECTOR 16(IRQ0)",
                                   "SOURCE VECTOR 17(IRQ1)",
                                   "SOURCE VECTOR 18(IRQ2)",
                                   "SOURCE VECTOR 19(RSVD)",
                                   "SOURCE VECTOR 20(RSVD)",
                                   "SOURCE VECTOR 21(RSVD)",
                                   "SOURCE VECTOR 22(RSVD)",
                                   "SOURCE VECTOR 23(RSVD)",
                                   "SOURCE VECTOR 24(RSVD)",
                                   "SOURCE VECTOR 25(RSVD)",
                                   "SOURCE VECTOR 26(RSVD)",
                                   "SOURCE VECTOR 27(RSVD)",
                                   "SOURCE VECTOR 28(RSVD)",
                                   "SOURCE VECTOR 29(RSVD)",
                                   "SOURCE VECTOR 30(RSVD)",
                                   "SOURCE VECTOR 31(RSVD)",
                                   "IRQ VECTOR",
                                   "FIQ VECTOR",
                                   "INTERRUPT STATUS",
                                   "INTERRUPT PENDING",
                                   ...
                                   "RESERVED",
                                   "RESERVED",
                                   "INTERRUPT ENABLE CMD",
                                   "INTERRUPT DISABLE CMD",
                                   "INTERRUPT CLEAR CMD",
                                   "INTERRUPT SET CMD",
...
static const char* pio_offset[] = {"PIO ENABLE",       "PIO DISABLE",       "PIO STATUS",         "RESERVED",
...
static const char* ps_offset[] = {"CONTROL REGISTER", "PERIPHERAL CLOCK ENABLE", "PERIPHERAL CLOCK DISABLE",
                                  "PERIPHERAL CLOCK STATUS REGISTER"};

char*
GetPSOffset(int addr) {
    int offset = addr & 0xf;
    if (offset <= 0xc)
...
static const char* uart_offset[] = {
    "CONTROL REGISTER",    "MODE REGISTER",     "INTERRUPT ENABLE",          "INTERRUPT DISABLE",
    "INTERRUPT MASK",      "CHANNEL STATUS",    "RECEIVER HOLDING REGISTER", "TRANSMITTER HOLDING REGISTER",
    "BAUD RATE GENERATOR", "RECEIVER TIME-OUT", "TRANSMITTER TIME-GUARD",    "RESERVED",
    "RECEIVE POINTER",     "RECEIVE COUNTER",   "TRANSMIT POINTER",          "TRANSMIT COUNTER"};

char*
GetUARTOffset(int addr) {
...
void
at91_pending_serial(int usartno, int delaytime) {
...
static void
serial_timer_event(int timer_num) {
    ...
    for (usartno = 0; usartno < 2; usartno++) {
        ...
        while (at91usart[usartno].at91_rbuf_tail != at91usart[usartno].at91_rbuf_head && at91usart[usartno].US_RPR != 0
               && at91usart[usartno].US_RCR > 0) {
            cpu_writemem32ledw(at91usart[usartno].US_RPR++,
                               at91usart[usartno].at91_receivebuf[at91usart[usartno].at91_rbuf_tail]);
...
int
at91_receive_serial(int usartno, data8_t* buf, int size) {
    int remaining = size;
    ...
    while (remaining) {
        // Is buffer full?
        if ((at91usart[usartno].at91_rbuf_tail == 0
             && at91usart[usartno].at91_rbuf_head == AT91_RECEIVE_BUFFER_SIZE - 1)
...
void
at91_usart_read(int usartno, int addr, data32_t* pData) {
    switch ((addr & 0xff) / 4) {
    ...
    LOG(("%08x: AT91-USART%d READ: %s (%08x) = %08x\n", activecpu_get_pc(), usartno + 1, GetUARTOffset(addr), addr,
         *pData));
}

void
at91_usart_write(int usartno, int addr, data32_t outdata) {
...
INLINE void
internal_write(int addr, data32_t data) {
    ...
    switch (offset) {
            ...
            //EBI - External Bus Interface
                ...
                switch ((UINT32)addr) {
                    ...
                    case 0xffe0001c: {
                        ...
                        sprintf(tmp, "ba=%8x,dbw=%d, nws=%d,wse=%d,tdf=%d,csen=%d,pages=%s,bat=%s", ba,
                                (dbw == 1 ? 16 : 8), nws, wse, tdf, csen, tmp1, tmp2);
                        LOG(("%08x: AT91-EBI_CSR%d = %8x (%s)\n", activecpu_get_pc(), (addr - 0xffe00000) / 4, data,
                             tmp));
                        break;
                    }
            ...
            if (offset2 < 0xc0) {
                ...
                switch (toffset) {
                        ...
                        if (trigger) {
                            ...
                            if (TC_ENABLED(timer_num)) {
                                ...
                                if (TC_RC_TRIGGER(timer_num) && TC_RC_IRQ_ENABLED(timer_num)) {
                                    ...
                                    if (logit)
                                        LOG(("Timer TC%d starting up for RC Compare @ Freq: %.2f\n", timer_num, freq));

                                    //Start up the timer
                                    if (at91rs.timer[timer_num])
                                        timer_adjust(at91rs.timer[timer_num], TIME_IN_HZ(freq), timer_num,
                                                     TIME_IN_HZ(freq));
                                } else
                    ...
                    case 0x04: {
                        ...
                        //log the raw data
                        if (logit)
                            LOG(("%08x: AT91-TIMER WRITE (TC%d): %s (%08x) = %08x\n", activecpu_get_pc(), timer_num,
                                 GetTimerOffset(addr), addr, data));

                        //Wave Mode
                        if (TC_WAVEMODE(timer_num)) {
                            char tmp[1000];
                            int clk, clki, burst, cpcstop, cpcdis, eevtedg, eevt, enetrg, cpctrg, wave, acpa, acpc,
                                aeevt, aswtrg, bcpb, bcpc, beevt, bswtrg;
                            int divr;
                            clk = (data & 7);
                            clki = (data & 8) >> 3;
                            burst = (data & 0x30) >> 4;
                            ...
                            sprintf(
                                ...
                                "clk=%d "
                                "(mclk/"
                                "%d),clki=%d,burst=%d,cpcstop=%d,cpcdis=%d,eevtedg=%d,eevt=%d,enetrg=%d,cpctrg=%d,wave="
                                "%d,acpa=%d, acpc=%d, aeevt=%d, aswtrg=%d, bcpb=%d, bcpc=%d, beevt=%d, bswtrg=%d \n",
                                clk, divr, clki, burst, cpcstop, cpcdis, eevtedg, eevt, enetrg, cpctrg, wave, acpa,
                                acpc, aeevt, aswtrg, bcpb, bcpc, beevt, bswtrg);
                        ...
                        //Capture Mode
                            ...
                            sprintf(tmp,
                                    "clk=%d "
                                    "(mclk/"
                                    "%d),clki=%d,burst=%d,ldbstop=%d,ldbdis=%d,etrgedge=%d,abetrg=%d,cpctrg=%d,wave=%d,"
                                    "ldra=%d,ldrb=%d \n",
                                    clk, divr, clki, burst, ldbstop, ldbdis, etrgedge, abetrg, cpctrg, wave, ldra,
                                    ldrb);
                            if (logit)
                                LOG(("%s", tmp));
                        }
                ...
                if (logit)
                    LOG(("%08x: AT91-TIMER WRITE (TC%d): %s (%08x) = %08x\n", activecpu_get_pc(), timer_num,
                         GetTimerOffset(addr), addr, data));
            } else if (offset2 == 0xc0) {
                if (logit)
                    LOG(("%08x: AT91-TIMER WRITE (TC BLOCK CONTROL REGISTER): %08x = %08x\n", activecpu_get_pc(), addr,
                         data));
            } else if (offset2 == 0xc4) {
                at91.tc_block_mode = data;
                if (logit)
                    LOG(("%08x: AT91-TIMER WRITE (TC BLOCK MODE REGISTER): %08x = %08x\n", activecpu_get_pc(), addr,
                         data));
            } else if (logit)
                LOG(("%08x: AT91-TIMER WRITE: %08x = %08x\n", activecpu_get_pc(), addr, data));
            break;
        }
        ...
        case 0xf0: {
            ...
            switch (offset2) {
                ...
                case 0x04:
                    data &= (0x18F0000
                             ^ 0xffffffff); //Pins 16,17,18,19,23,24 are Dedicated PIO lines, and cannot be cleared.
...
internal_read(int addr) {
    ...
    switch (offset2) {
            ...
            if (offset3 < 0xc0) {
                ...
                switch (toffset) {
                    ...
                    case 0x20:
                        ...
                        at91.tc_clock[timer_num].tc_status &=
                            0x10000; //Reading the status, clears it (not the clock enabled bit though)
                ...
                if (logit)
                    LOG(("%08x: AT91-TIMER READ (TC%d): %s (%08x) = %08x\n", activecpu_get_pc(), timer_num,
                         GetTimerOffset(addr), addr, data));
            } else if (offset3 == 0xc0) {
                //Timer Block Control
                if (logit)
                    LOG(("%08x: AT91-TIMER READ (TC BLOCK CONTROL REGISTER): %08x = %08x\n", activecpu_get_pc(), addr,
                         data));
            } else if (offset3 == 0xc4) {
                //Timer Block Mode
                data = at91.tc_block_mode;
                if (logit)
                    LOG(("%08x: AT91-TIMER READ (TC BLOCK MODE REGISTER): %08x = %08x\n", activecpu_get_pc(), addr,
                         data));
            } else
                //??
        ...
        case 0xf0: {
            ...
            if (logit)
                LOG(("%08x: AT91-PIO READ: %s (%08x) = %08x\n", activecpu_get_pc(), GetPIOOffset(addr), addr, data));
            break;
        }
        ...
        case 0xf4: {
            ...
            if (logit)
                LOG(("%08x: AT91-POWER READ: %s (%08x) = %08x\n", activecpu_get_pc(), GetPSOffset(addr), addr, data));
            break;
        }
        ...
        case 0xff: {
            ...
            if (logit)
                LOG(("%08x: AT91-AIC READ: %s (%08x) = %08x\n", activecpu_get_pc(), GetAICOffset(addr), addr, data));
            break;
        }
...
INLINE void
at91_cpu_write32(int addr, data32_t data) {
...
INLINE void
at91_cpu_write16(int addr, data16_t data) {
    //Atmel AT91 CPU On Chip Periperhals Mapped here
    if (addr >= 0xFFC00000) {
...
INLINE void
at91_cpu_write8(int addr, data8_t data) {
    //Atmel AT91 CPU On Chip Periperhals Mapped here
    if (addr >= 0xFFC00000) {
        internal_write(addr, (data32_t)data);
        return;
...
void
at91_set_sp(unsigned val) {
    SET_REGISTER(13, val);
}

unsigned
at91_get_reg(int regnum) {
    switch (regnum) {
        case ARM732_R0:
            return ARMREG(0);
        case ARM732_R1:
        ...
        case ARM732_FR8:
            return ARMREG(eR8_FIQ);
        case ARM732_FR9:
            return ARMREG(eR9_FIQ);
        case ARM732_FR10:
            return ARMREG(eR10_FIQ);
        case ARM732_FR11:
            return ARMREG(eR11_FIQ);
        case ARM732_FR12:
            return ARMREG(eR12_FIQ);
        case ARM732_FR13:
            return ARMREG(eR13_FIQ);
        case ARM732_FR14:
            return ARMREG(eR14_FIQ);
        case ARM732_FSPSR:
            return ARMREG(eSPSR_FIQ);
        case ARM732_IR13:
            return ARMREG(eR13_IRQ);
        case ARM732_IR14:
            return ARMREG(eR14_IRQ);
        case ARM732_ISPSR:
            return ARMREG(eSPSR_IRQ);
        case ARM732_SR13:
            return ARMREG(eR13_SVC);
        case ARM732_SR14:
            return ARMREG(eR14_SVC);
        case ARM732_SSPSR:
            return ARMREG(eSPSR_SVC);
        case REG_PC:
            return ARMREG(15);
    }
...
void
at91_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case ARM732_R0:
            ARMREG(0) = val;
            break;
        ...
        case ARM732_UR14:
            ...
            break;
        case ARM732_USPSR:
            ARMREG(eSPSR_UND) = val;
            break;
    }
}

void
at91_set_nmi_line(int state) {}

void
at91_irq_assert_if_greater(void) {
...
void
at91_set_irq_line(int irqline, int state) {
    arm7_core_set_irq_line(irqline, state);
}

void
at91_set_irq_callback(int (*callback)(int irqline)) {}

static const data8_t at91_reg_layout[] = {0xFF,         ARM732_R0,    ARM732_IR13,
                                          0xFF,         ARM732_R1,    ARM732_IR14,
                                          0xFF,         ARM732_R2,    ARM732_ISPSR,
                                          0xFF,         ARM732_R3,    0xFF,
                                          ...
                                          ARM732_R12,   0xFF,         ARM732_R13,
                                          ARM732_AR13,  0xFF,         ARM732_R14,
                                          ARM732_AR14,  0xFF,         ARM732_R15,
                                          ARM732_ASPSR, 0xFF,         0xFF,
...
const char*
at91_info(void* context, int regnum) {
    ...
    switch (regnum) {
        case CPU_INFO_REG + ARM732_R0:
            sprintf(buffer[which], "R0  :%08x", pRegs->sArmRegister[0]);
            break;
        case CPU_INFO_REG + ARM732_R1:
            sprintf(buffer[which], "R1  :%08x", pRegs->sArmRegister[1]);
            break;
        case CPU_INFO_REG + ARM732_R2:
            sprintf(buffer[which], "R2  :%08x", pRegs->sArmRegister[2]);
            break;
        case CPU_INFO_REG + ARM732_R3:
            sprintf(buffer[which], "R3  :%08x", pRegs->sArmRegister[3]);
            break;
        case CPU_INFO_REG + ARM732_R4:
            sprintf(buffer[which], "R4  :%08x", pRegs->sArmRegister[4]);
            break;
        case CPU_INFO_REG + ARM732_R5:
            sprintf(buffer[which], "R5  :%08x", pRegs->sArmRegister[5]);
            break;
        case CPU_INFO_REG + ARM732_R6:
            sprintf(buffer[which], "R6  :%08x", pRegs->sArmRegister[6]);
            break;
        case CPU_INFO_REG + ARM732_R7:
            sprintf(buffer[which], "R7  :%08x", pRegs->sArmRegister[7]);
            break;
        case CPU_INFO_REG + ARM732_R8:
            sprintf(buffer[which], "R8  :%08x", pRegs->sArmRegister[8]);
            break;
        case CPU_INFO_REG + ARM732_R9:
            sprintf(buffer[which], "R9  :%08x", pRegs->sArmRegister[9]);
            break;
        case CPU_INFO_REG + ARM732_R10:
            sprintf(buffer[which], "R10 :%08x", pRegs->sArmRegister[10]);
            break;
        case CPU_INFO_REG + ARM732_R11:
            sprintf(buffer[which], "R11 :%08x", pRegs->sArmRegister[11]);
            break;
        case CPU_INFO_REG + ARM732_R12:
            sprintf(buffer[which], "R12 :%08x", pRegs->sArmRegister[12]);
            break;
        case CPU_INFO_REG + ARM732_R13:
            sprintf(buffer[which], "R13 :%08x", pRegs->sArmRegister[13]);
            break;
        case CPU_INFO_REG + ARM732_R14:
            sprintf(buffer[which], "R14 :%08x", pRegs->sArmRegister[14]);
            break;
        case CPU_INFO_REG + ARM732_R15:
            sprintf(buffer[which], "R15 :%08x", pRegs->sArmRegister[15]);
            break;
        case CPU_INFO_REG + ARM732_CPSR:
            sprintf(buffer[which], "R16 :%08x", pRegs->sArmRegister[eCPSR]);
            break;
        case CPU_INFO_REG + ARM732_FR8:
            sprintf(buffer[which], "FR8 :%08x", pRegs->sArmRegister[eR8_FIQ]);
            break;
        case CPU_INFO_REG + ARM732_FR9:
            sprintf(buffer[which], "FR9 :%08x", pRegs->sArmRegister[eR9_FIQ]);
            break;
        case CPU_INFO_REG + ARM732_FR10:
            sprintf(buffer[which], "FR10:%08x", pRegs->sArmRegister[eR10_FIQ]);
            break;
        case CPU_INFO_REG + ARM732_FR11:
            sprintf(buffer[which], "FR11:%08x", pRegs->sArmRegister[eR11_FIQ]);
            break;
        case CPU_INFO_REG + ARM732_FR12:
            sprintf(buffer[which], "FR12:%08x", pRegs->sArmRegister[eR12_FIQ]);
            break;
        case CPU_INFO_REG + ARM732_FR13:
            sprintf(buffer[which], "FR13:%08x", pRegs->sArmRegister[eR13_FIQ]);
            break;
        case CPU_INFO_REG + ARM732_FR14:
            sprintf(buffer[which], "FR14:%08x", pRegs->sArmRegister[eR14_FIQ]);
            break;
        case CPU_INFO_REG + ARM732_FSPSR:
            sprintf(buffer[which], "FR16:%08x", pRegs->sArmRegister[eSPSR_FIQ]);
            break;
        case CPU_INFO_REG + ARM732_IR13:
            sprintf(buffer[which], "IR13:%08x", pRegs->sArmRegister[eR13_IRQ]);
            break;
        case CPU_INFO_REG + ARM732_IR14:
            sprintf(buffer[which], "IR14:%08x", pRegs->sArmRegister[eR14_IRQ]);
            break;
        case CPU_INFO_REG + ARM732_ISPSR:
            sprintf(buffer[which], "IR16:%08x", pRegs->sArmRegister[eSPSR_IRQ]);
            break;
        case CPU_INFO_REG + ARM732_SR13:
            sprintf(buffer[which], "SR13:%08x", pRegs->sArmRegister[eR13_SVC]);
            break;
        case CPU_INFO_REG + ARM732_SR14:
            sprintf(buffer[which], "SR14:%08x", pRegs->sArmRegister[eR14_SVC]);
            break;
        case CPU_INFO_REG + ARM732_SSPSR:
            sprintf(buffer[which], "SR16:%08x", pRegs->sArmRegister[eSPSR_SVC]);
            break;
        case CPU_INFO_REG + ARM732_AR13:
            sprintf(buffer[which], "AR13:%08x", pRegs->sArmRegister[eR13_ABT]);
            break;
        case CPU_INFO_REG + ARM732_AR14:
            sprintf(buffer[which], "AR14:%08x", pRegs->sArmRegister[eR14_ABT]);
            break;
        case CPU_INFO_REG + ARM732_ASPSR:
            sprintf(buffer[which], "AR16:%08x", pRegs->sArmRegister[eSPSR_ABT]);
            break;
        case CPU_INFO_REG + ARM732_UR13:
            sprintf(buffer[which], "UR13:%08x", pRegs->sArmRegister[eR13_UND]);
            break;
        case CPU_INFO_REG + ARM732_UR14:
            sprintf(buffer[which], "UR14:%08x", pRegs->sArmRegister[eR14_UND]);
            break;
        case CPU_INFO_REG + ARM732_USPSR:
            sprintf(buffer[which], "UR16:%08x", pRegs->sArmRegister[eSPSR_UND]);
            break;

        case CPU_INFO_FLAGS:
            sprintf(
                buffer[which], "%c%c%c%c%c%c%c %s (%08x)", (pRegs->sArmRegister[eCPSR] & N_MASK) ? 'N' : '-',
                (pRegs->sArmRegister[eCPSR] & Z_MASK) ? 'Z' : '-', (pRegs->sArmRegister[eCPSR] & C_MASK) ? 'C' : '-',
                (pRegs->sArmRegister[eCPSR] & V_MASK) ? 'V' : '-', (pRegs->sArmRegister[eCPSR] & I_MASK) ? 'I' : '-',
                (pRegs->sArmRegister[eCPSR] & F_MASK) ? 'F' : '-', (pRegs->sArmRegister[eCPSR] & T_MASK) ? 'T' : '-',
                GetModeText(pRegs->sArmRegister[eCPSR]), pRegs->sArmRegister[eCPSR]);
            break;
        case CPU_INFO_NAME:
            return "AT91";
        case CPU_INFO_FAMILY:
            return "Atmel 91 - Acorn Risc Machine";
        case CPU_INFO_VERSION:
            return "1.0";
        case CPU_INFO_FILE:
            return __FILE__;
        case CPU_INFO_CREDITS:
            return "Copyright 2004 Steve Ellenoff, sellenoff@hotmail.com";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)at91_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)at91_win_layout;
    }
...
unsigned
at91_dasm(char* buffer, unsigned int pc) {
#ifdef MAME_DEBUG
...
static void
timer_trigger_event(int timer_num) {
...
#else
INLINE
BeforeOpCodeHook(void) {
    //Update timers if running
    if ((TC_RUNNING(0) || TC_RUNNING(1)
         || TC_RUNNING(2))) //for speed we use this, but really it should loop and check using MAX_TIMER var
    {
        int i;
        int cycles = 0;
