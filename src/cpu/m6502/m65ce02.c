...
struct cpu_interface m65ce02_interface =
    CPU0(M65CE02, m65ce02, 1, 0, 1.00, M65CE02_INT_NONE, M65CE02_INT_IRQ, M65CE02_INT_NMI, 8, 16, 0, 16, LE, 1, 3);

extern void
m65ce02_runtime_loader_init(void) {
    cpuintf[CPU_M65CE02] = m65ce02_interface;
}
#endif

/* Layout of the registers in the debugger */
static UINT8 m65ce02_reg_layout[] = {M65CE02_A,         M65CE02_X,         M65CE02_Y, M65CE02_Z,  M65CE02_S,
                                     M65CE02_PC,        M65CE02_P,         -1,        M65CE02_EA, M65CE02_ZP,
                                     M65CE02_NMI_STATE, M65CE02_IRQ_STATE, M65CE02_B, 0};
...
void
m65ce02_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
            PCW = val;
            change_pc16(PCD);
        ...
        case M65CE02_ZP:
            ...
            break;
        case M65CE02_NMI_STATE:
            m65ce02_set_irq_line(IRQ_LINE_NMI, val);
            break;
        case M65CE02_IRQ_STATE:
...
m65ce02_take_irq(void) {
    if (!(P & F_I)) {
        ...
        if (m65ce02.irq_callback)
            (*m65ce02.irq_callback)(0);
        change_pc16(PCD);
    }
...
void
m65ce02_set_irq_line(int irqline, int state) {
    if (irqline == IRQ_LINE_NMI) {
        if (m65ce02.nmi_state == state)
            return;
...
void
m65ce02_set_irq_callback(int (*callback)(int)) {
    m65ce02.irq_callback = callback;
}

void
m65ce02_state_save(void* file) {
 ...
 ****************************************************************************/
const char*
m65ce02_info(void* context, int regnum) {
    ...
    switch (regnum) {
        case CPU_INFO_REG + M65CE02_PC:
            sprintf(buffer[which], "PC:%04X", r->pc.w.l);
            break;
        case CPU_INFO_REG + M65CE02_S:
            sprintf(buffer[which], "S:%04X", r->sp.w.l);
            break;
        case CPU_INFO_REG + M65CE02_P:
            sprintf(buffer[which], "P:%02X", r->p);
            break;
        case CPU_INFO_REG + M65CE02_A:
            sprintf(buffer[which], "A:%02X", r->a);
            break;
        case CPU_INFO_REG + M65CE02_X:
            sprintf(buffer[which], "X:%02X", r->x);
            break;
        case CPU_INFO_REG + M65CE02_Y:
            sprintf(buffer[which], "Y:%02X", r->y);
            break;
        case CPU_INFO_REG + M65CE02_Z:
            sprintf(buffer[which], "Z:%02X", r->z);
            break;
        case CPU_INFO_REG + M65CE02_B:
            sprintf(buffer[which], "B:%02X", r->zp.b.h);
            break;
        case CPU_INFO_REG + M65CE02_EA:
            sprintf(buffer[which], "EA:%04X", r->ea.w.l);
            break;
        case CPU_INFO_REG + M65CE02_ZP:
            sprintf(buffer[which], "ZP:%04X", r->zp.w.l);
            break;
        case CPU_INFO_REG + M65CE02_NMI_STATE:
            sprintf(buffer[which], "NMI:%X", r->nmi_state);
            break;
        case CPU_INFO_REG + M65CE02_IRQ_STATE:
            sprintf(buffer[which], "IRQ:%X", r->irq_state);
            break;
        case CPU_INFO_FLAGS:
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c", r->p & 0x80 ? 'N' : '.', r->p & 0x40 ? 'V' : '.',
                    r->p & 0x20 ? 'E' : '.', r->p & 0x10 ? 'B' : '.', r->p & 0x08 ? 'D' : '.', r->p & 0x04 ? 'I' : '.',
                    r->p & 0x02 ? 'Z' : '.', r->p & 0x01 ? 'C' : '.');
            break;
        case CPU_INFO_NAME:
            return "M65CE02";
        ...
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m65ce02_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m65ce02_win_layout;
    }
    return buffer[which];
}

unsigned int
m65ce02_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
