...
static UINT8 i8085_reg_layout[] = {
    ...
    0xFF,     I8085_HALT,       I8085_IM,         I8085_IREQ,        I8085_ISRV,        I8085_VECTOR,
    0xFF,     I8085_TRAP_STATE, I8085_INTR_STATE, I8085_RST55_STATE, I8085_RST65_STATE, I8085_RST75_STATE,
    0};
    ...
    switch (opcode) {
        ...
        case 0x20:
            if (I.cputype) { //!! misses fixes from newer MAME
                ...
                I.AF.b.h = I.IM;
                I.AF.b.h |= RIM_IEN;
                RIM_IEN = 0; //AT: read and clear IEN status latch
                             /*				survival_prot ^= 0x01; */
            } else {
                /* NOP undocumented */
            }
        ...
        case 0x27: /* DAA	*/
            ...
            I.AF.b.l =
                (I.AF.b.l & 3) | (I.AF.b.h & 0x28) | (I.AF.b.h > 0x99) | ((I.AF.b.h ^ I.XX.b.h) & 0x10) | ZSP[I.XX.b.h];
            I.AF.b.h = I.XX.b.h;
            break;
        ...
        case 0x30: //!! misses new port from MAME
            if (I.cputype) {
                ...
                if (I.AF.b.h & 0x40) //SOE - only when bit 0x40 is set!
                {
                    I.IM &= ~IM_SOD;
                    if (I.AF.b.h & 0x80)
                        I.IM |= IM_SOD; //is it needed ?
                    if (I.sod_callback)
                        (*I.sod_callback)(I.AF.b.h >> 7); //SOD - data = bit 0x80
                }
                //AT
        ...
        case 0xfb: /* EI */ //!! not ported from new MAME
            ...
            if (I.cputype) {
                ...
                if (!(I.IM & IM_M75) && (I.IREQ & IM_M75)) {
                    I.ISRV = IM_M75;
                    I.IRQ2 = ADDR_RST75;
                } else if (!(I.IM & IM_M65) && (I.IREQ & IM_M65)) {
                    I.ISRV = IM_M65;
                    I.IRQ2 = ADDR_RST65;
 ...
 ****************************************************************************/
void
i8085_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
            I.PC.w.l = val;
            change_pc16(I.PC.d);
...
/****************************************************************************/
void
i8085_set_SOD_callback(void (*callback)(int state)) {
...
/****************************************************************************/
void
i8085_set_TRAP(int state) {
    LOG(("i8085: TRAP %d\n", state));
    if (state) {
        I.IREQ |= IM_TRAP;
        if (I.ISRV & IM_TRAP)
            return;       /* already servicing TRAP ? */
...
/****************************************************************************/
void
i8085_set_RST75(int state) {
    LOG(("i8085: RST7.5 %d\n", state));
    if (state) {

        I.IREQ |= IM_M75; /* request RST7.5 */
        if (I.IM & IM_M75)
            return;  /* if masked, ignore it for now */
...
void
i8085_set_irq_line(int irqline, int state) {
...
void
i8085_set_irq_callback(int (*callback)(int)) {
 ...
 ****************************************************************************/
const char*
i8085_info(void* context, int regnum) {
    ...
    switch (regnum) {
        case CPU_INFO_REG + I8085_AF:
            sprintf(buffer[which], "AF:%04X", r->AF.w.l);
            break;
        case CPU_INFO_REG + I8085_BC:
            sprintf(buffer[which], "BC:%04X", r->BC.w.l);
            break;
        case CPU_INFO_REG + I8085_DE:
            sprintf(buffer[which], "DE:%04X", r->DE.w.l);
            break;
        case CPU_INFO_REG + I8085_HL:
            sprintf(buffer[which], "HL:%04X", r->HL.w.l);
            break;
        case CPU_INFO_REG + I8085_SP:
            sprintf(buffer[which], "SP:%04X", r->SP.w.l);
            break;
        case CPU_INFO_REG + I8085_PC:
            sprintf(buffer[which], "PC:%04X", r->PC.w.l);
            break;
        case CPU_INFO_REG + I8085_IM:
            sprintf(buffer[which], "IM:%02X", r->IM);
            break;
        case CPU_INFO_REG + I8085_HALT:
            sprintf(buffer[which], "HALT:%d", r->HALT);
            break;
        case CPU_INFO_REG + I8085_IREQ:
            sprintf(buffer[which], "IREQ:%02X", I.IREQ);
            break;
        case CPU_INFO_REG + I8085_ISRV:
            sprintf(buffer[which], "ISRV:%02X", I.ISRV);
            break;
        case CPU_INFO_REG + I8085_VECTOR:
            sprintf(buffer[which], "VEC:%02X", I.INTR);
            break;
        case CPU_INFO_REG + I8085_TRAP_STATE:
            sprintf(buffer[which], "TRAP:%X", I.nmi_state);
            break;
        case CPU_INFO_REG + I8085_INTR_STATE:
            sprintf(buffer[which], "INTR:%X", I.irq_state[I8085_INTR_LINE]);
            break;
        case CPU_INFO_REG + I8085_RST55_STATE:
            sprintf(buffer[which], "RST55:%X", I.irq_state[I8085_RST55_LINE]);
            break;
        case CPU_INFO_REG + I8085_RST65_STATE:
            sprintf(buffer[which], "RST65:%X", I.irq_state[I8085_RST65_LINE]);
            break;
        case CPU_INFO_REG + I8085_RST75_STATE:
            sprintf(buffer[which], "RST75:%X", I.irq_state[I8085_RST75_LINE]);
            break;
        case CPU_INFO_FLAGS:
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c", (r->AF.b.l & 0x80) ? 'S' : '.', (r->AF.b.l & 0x40) ? 'Z' : '.',
                    (r->AF.b.l & 0x20) ? 'X' : '.', // X5
                    (r->AF.b.l & 0x10) ? 'H' : '.', (r->AF.b.l & 0x08) ? '?' : '.', (r->AF.b.l & 0x04) ? 'P' : '.',
                    (r->AF.b.l & 0x02) ? 'V' : '.', (r->AF.b.l & 0x01) ? 'C' : '.');
            break;
        case CPU_INFO_NAME:
            return "8085A";
        ...
        case CPU_INFO_REG_LAYOUT:
            return (const char*)i8085_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)i8085_win_layout;
    }
    return buffer[which];
}

unsigned
i8085_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
...
static UINT8 i8080_reg_layout[] = {
    ...
    I8080_HALT, I8080_IREQ, I8080_ISRV, I8080_VECTOR, I8080_TRAP_STATE, I8080_INTR_STATE, 0};
...
int
i8080_execute(int cycles) {
    return i8085_execute(cycles);
}

unsigned
i8080_get_context(void* dst) {
    return i8085_get_context(dst);
}

void
i8080_set_context(void* src) {
    i8085_set_context(src);
}

unsigned
i8080_get_reg(int regnum) {
    return i8085_get_reg(regnum);
}

void
i8080_set_reg(int regnum, unsigned val) {
    i8085_set_reg(regnum, val);
}

void
i8080_set_irq_line(int irqline, int state) {
    if (irqline == IRQ_LINE_NMI) {
...
void
i8080_set_irq_callback(int (*callback)(int irqline)) {
    i8085_set_irq_callback(callback);
}

const char*
i8080_info(void* context, int regnum) {
    switch (regnum) {
        case CPU_INFO_NAME:
            return "8080";
        case CPU_INFO_VERSION:
            return "1.2";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)i8080_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)i8080_win_layout;
    }
    return i8085_info(context, regnum);
}

unsigned
i8080_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
    return Dasm8085(buffer, pc);
#else
    sprintf(buffer, "$%02X", cpu_readop(pc));
