...
static UINT8 M68K_layout[] = {M68K_PC, M68K_ISP, -1, M68K_SR, M68K_USP, -1, M68K_D0, M68K_A0, -1, M68K_D1, M68K_A1, -1,
                              M68K_D2, M68K_A2,  -1, M68K_D3, M68K_A3,  -1, M68K_D4, M68K_A4, -1, M68K_D5, M68K_A5, -1,
                              M68K_D6, M68K_A6,  -1, M68K_D7, M68K_A7,  0};
...
static int IntelFlag[32] = {0x0000, 0x0001, 0x0800, 0x0801, 0x0040, 0x0041, 0x0840, 0x0841, 0x0080, 0x0081, 0x0880,
                            0x0881, 0x00C0, 0x00C1, 0x08C0, 0x08C1, 0x0100, 0x0101, 0x0900, 0x0901, 0x0140, 0x0141,
                            0x0940, 0x0941, 0x0180, 0x0181, 0x0980, 0x0981, 0x01C0, 0x01C1, 0x09C0, 0x09C1};
...
static void
a68k_prepare_substate(void) {
    stopped = ((M68000_regs.IRQ_level & 0x80) != 0);

    M68000_regs.sr = ((M68000_regs.ccr >> 4) & 0x1C) | (M68000_regs.ccr & 0x01) | ((M68000_regs.ccr >> 10) & 0x02)
...
void
a68k_state_register(const char* type) {
...
static const struct m68k_memory_interface interface_a24_d32 = {
    WORD_XOR_BE(0),    cpu_readmem24bedw, readword_a24_d32, readlong_a24_d32,  cpu_writemem24bedw,
    writeword_a24_d32, writelong_a24_d32, changepc_a24_d32, cpu_readmem24bedw, readword_a24_d32,
    readlong_a24_d32,  readword_a24_d32,  readlong_a24_d32};
...
static const struct m68k_memory_interface interface_a32_d32 = {
    WORD_XOR_BE(0),    cpu_readmem32bedw, readword_a32_d32, readlong_a32_d32,  cpu_writemem32bedw,
    writeword_a32_d32, writelong_a32_d32, changepc_a32_d32, cpu_readmem32bedw, readword_a32_d32,
    readlong_a32_d32,  readword_a32_d32,  readlong_a32_d32};
...
m68000_get_reg(int regnum) {
    switch (regnum) {
        ...
        case M68K_VBR:
            return M68000_regs.vbr;
        case M68K_SFC:
            return M68000_regs.sfc;
        case M68K_DFC:
            return M68000_regs.dfc;
        case M68K_D0:
        ...
        case REG_PREVIOUSPC:
            return M68000_regs.previous_pc;
...
void
m68000_set_reset_callback(int (*callback)(void)) {
...
//extern int m68k_disassemble(char* str_buff, int pc, int cputype);
    ...
    switch (regnum) {
        case CPU_INFO_REG + M68K_PC:
            sprintf(buffer[which], "PC:%06X", r->pc);
            break;
        case CPU_INFO_REG + M68K_ISP:
            sprintf(buffer[which], "ISP:%08X", r->isp);
            break;
        case CPU_INFO_REG + M68K_USP:
            sprintf(buffer[which], "USP:%08X", r->usp);
            break;
        case CPU_INFO_REG + M68K_SR:
            sprintf(buffer[which], "SR:%08X", r->sr);
            break;
        case CPU_INFO_REG + M68K_VBR:
            sprintf(buffer[which], "VBR:%08X", r->vbr);
            break;
        case CPU_INFO_REG + M68K_SFC:
            sprintf(buffer[which], "SFC:%08X", r->sfc);
            break;
        case CPU_INFO_REG + M68K_DFC:
            sprintf(buffer[which], "DFC:%08X", r->dfc);
            break;
        case CPU_INFO_REG + M68K_D0:
            sprintf(buffer[which], "D0:%08X", r->d[0]);
            break;
        case CPU_INFO_REG + M68K_D1:
            sprintf(buffer[which], "D1:%08X", r->d[1]);
            break;
        case CPU_INFO_REG + M68K_D2:
            sprintf(buffer[which], "D2:%08X", r->d[2]);
            break;
        case CPU_INFO_REG + M68K_D3:
            sprintf(buffer[which], "D3:%08X", r->d[3]);
            break;
        case CPU_INFO_REG + M68K_D4:
            sprintf(buffer[which], "D4:%08X", r->d[4]);
            break;
        case CPU_INFO_REG + M68K_D5:
            sprintf(buffer[which], "D5:%08X", r->d[5]);
            break;
        case CPU_INFO_REG + M68K_D6:
            sprintf(buffer[which], "D6:%08X", r->d[6]);
            break;
        case CPU_INFO_REG + M68K_D7:
            sprintf(buffer[which], "D7:%08X", r->d[7]);
            break;
        case CPU_INFO_REG + M68K_A0:
            sprintf(buffer[which], "A0:%08X", r->a[0]);
            break;
        case CPU_INFO_REG + M68K_A1:
            sprintf(buffer[which], "A1:%08X", r->a[1]);
            break;
        case CPU_INFO_REG + M68K_A2:
            sprintf(buffer[which], "A2:%08X", r->a[2]);
            break;
        case CPU_INFO_REG + M68K_A3:
            sprintf(buffer[which], "A3:%08X", r->a[3]);
            break;
        case CPU_INFO_REG + M68K_A4:
            sprintf(buffer[which], "A4:%08X", r->a[4]);
            break;
        case CPU_INFO_REG + M68K_A5:
            sprintf(buffer[which], "A5:%08X", r->a[5]);
            break;
        case CPU_INFO_REG + M68K_A6:
            sprintf(buffer[which], "A6:%08X", r->a[6]);
            break;
        case CPU_INFO_REG + M68K_A7:
            sprintf(buffer[which], "A7:%08X", r->a[7]);
            break;
        case CPU_INFO_FLAGS:
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", r->sr & 0x8000 ? 'T' : '.',
                    r->sr & 0x4000 ? '?' : '.', r->sr & 0x2000 ? 'S' : '.', r->sr & 0x1000 ? '?' : '.',
                    r->sr & 0x0800 ? '?' : '.', r->sr & 0x0400 ? 'I' : '.', r->sr & 0x0200 ? 'I' : '.',
                    r->sr & 0x0100 ? 'I' : '.', r->sr & 0x0080 ? '?' : '.', r->sr & 0x0040 ? '?' : '.',
                    r->sr & 0x0020 ? '?' : '.', r->sr & 0x0010 ? 'X' : '.', r->sr & 0x0008 ? 'N' : '.',
                    r->sr & 0x0004 ? 'Z' : '.', r->sr & 0x0002 ? 'V' : '.', r->sr & 0x0001 ? 'C' : '.');
            break;
        case CPU_INFO_NAME:
            return "68000";
        ...
        case CPU_INFO_CREDITS:
            return "Copyright 1998,99 Mike Coates, Darren Olafson. All rights reserved";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)M68K_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m68k_win_layout;
    }
    return buffer[which];
}
...
m68010_set_irq_line(int irqline, int state) {
    m68000_set_irq_line(irqline, state);
}

void
m68010_set_irq_callback(int (*callback)(int irqline)) {
    m68000_set_irq_callback(callback);
}
...
m68020_get_reg(int regnum) {
    switch (regnum) {
        ...
        case M68K_USP:
            return M68020_regs.usp;
        case M68K_SR:
            return M68020_regs.sr;
        case M68K_VBR:
            return M68020_regs.vbr;
        case M68K_SFC:
            return M68020_regs.sfc;
        case M68K_DFC:
            return M68020_regs.dfc;
        case M68K_D0:
            return M68020_regs.d[0];
        case M68K_D1:
            return M68020_regs.d[1];
        case M68K_D2:
            return M68020_regs.d[2];
        case M68K_D3:
            return M68020_regs.d[3];
        case M68K_D4:
            return M68020_regs.d[4];
        case M68K_D5:
            return M68020_regs.d[5];
        case M68K_D6:
            return M68020_regs.d[6];
        case M68K_D7:
            return M68020_regs.d[7];
        case M68K_A0:
            return M68020_regs.a[0];
        case M68K_A1:
            return M68020_regs.a[1];
        case M68K_A2:
            return M68020_regs.a[2];
        case M68K_A3:
            return M68020_regs.a[3];
        case M68K_A4:
            return M68020_regs.a[4];
        case M68K_A5:
            return M68020_regs.a[5];
        case M68K_A6:
            return M68020_regs.a[6];
        case M68K_A7:
            return M68020_regs.a[7];
        case REG_PREVIOUSPC:
            return M68020_regs.previous_pc;
            /* TODO: Verify that this is the right thing to do for the purpose? */
        default:
            if (regnum <= REG_SP_CONTENTS) {
...
void
m68020_assert_irq(int int_line) {
    /* Save icount */
    int StartCount = m68k_ICount;
...
void
m68020_set_reset_callback(int (*callback)(void)) {
...
m68ec020_execute(int cycles) {
    return m68020_execute(cycles);
}

unsigned
m68ec020_get_context(void* dst) {
    return m68020_get_context(dst);
}
