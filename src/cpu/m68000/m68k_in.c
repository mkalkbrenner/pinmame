...
static opcode_handler_struct m68k_opcode_handler_table[] = {
    ...
    XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX M68KMAKE_TABLE_FOOTER

    {0, 0, 0, {0, 0, 0}}};

/* Build the opcode handler jump table */
void
m68ki_build_opcode_table(void) {
...
M68KMAKE_OP(1111, 0, ., .) { m68ki_exception_1111(); }
...
M68KMAKE_OP(andi, 16, toc, .) { m68ki_set_ccr(m68ki_get_ccr() & OPER_I_16()); }
...
M68KMAKE_OP(btst, 8, r, .) { FLAG_Z = M68KMAKE_GET_OPER_AY_8 & (1 << (DX & 7)); }

M68KMAKE_OP(btst, 32, s, d) { FLAG_Z = DY & (1 << (OPER_I_8() & 0x1f)); }
...
M68KMAKE_OP(eori, 16, toc, .) { m68ki_set_ccr(m68ki_get_ccr() ^ OPER_I_16()); }
...
M68KMAKE_OP(illegal, 0, ., .) { m68ki_exception_illegal(); }
...
M68KMAKE_OP(lea, 32, ., .) { AX = M68KMAKE_GET_EA_AY_32; }
...
M68KMAKE_OP(movea, 16, ., d) { AX = MAKE_INT_16(DY); }

M68KMAKE_OP(movea, 16, ., a) { AX = MAKE_INT_16(AY); }

M68KMAKE_OP(movea, 16, ., .) { AX = MAKE_INT_16(M68KMAKE_GET_OPER_AY_16); }

M68KMAKE_OP(movea, 32, ., d) { AX = DY; }

M68KMAKE_OP(movea, 32, ., a) { AX = AY; }

M68KMAKE_OP(movea, 32, ., .) { AX = M68KMAKE_GET_OPER_AY_32; }
...
M68KMAKE_OP(move, 16, toc, d) { m68ki_set_ccr(DY); }

M68KMAKE_OP(move, 16, toc, .) { m68ki_set_ccr(M68KMAKE_GET_OPER_AY_16); }
...
M68KMAKE_OP(nop, 0, ., .) { m68ki_trace_t0(); /* auto-disable (see m68kcpu.h) */ }
...
M68KMAKE_OP(ori, 16, toc, .) { m68ki_set_ccr(m68ki_get_ccr() | OPER_I_16()); }
...
M68KMAKE_OP(st, 8, ., d) { DY |= 0xff; }

M68KMAKE_OP(st, 8, ., .) { m68ki_write_8(M68KMAKE_GET_EA_AY_8, 0xff); }

M68KMAKE_OP(sf, 8, ., d) { DY &= 0xffffff00; }

M68KMAKE_OP(sf, 8, ., .) { m68ki_write_8(M68KMAKE_GET_EA_AY_8, 0); }
...
M68KMAKE_OP(scc, 8, ., .) { m68ki_write_8(M68KMAKE_GET_EA_AY_8, M68KMAKE_CC ? 0xff : 0); }
...
M68KMAKE_OP(unlk, 32, ., a7) { REG_A[7] = m68ki_read_32(REG_A[7]); }
