...
void generate_opcode_handler(FILE* filep, body_struct* body, replace_struct* replace, opcode_struct* opinfo,
                             int ea_mode);
void generate_opcode_ea_variants(FILE* filep, body_struct* body, replace_struct* replace, opcode_struct* op);
void generate_opcode_cc_variants(FILE* filep, body_struct* body, replace_struct* replace, opcode_struct* op_in,
                                 int offset);
...
int
check_strsncpy(char* dst, char* src, int maxlength) {
    char* p = dst;
    while (*src && *src != ' ') {
        *p++ = *src++;
        if (p - dst > maxlength)
...
int
check_strcncpy(char* dst, char* src, char delim, int maxlength) {
    char* p = dst;
    while (*src && *src != delim) {
        *p++ = *src++;
        if (p - dst > maxlength)
...
int
check_atoi(char* str, int* result) {
    int accum = 0;
    char* p = str;
    while (*p >= '0' && *p <= '9') {
        accum *= 10;
...
int
fgetline(char* buff, int nchars, FILE* file) {
    int length;
...
int
get_oper_cycles(opcode_struct* op, int ea_mode, int cpu_type) {
    ...
    if (cpu_type < CPU_TYPE_020) {
        ...
        if (cpu_type == CPU_TYPE_000 && ea_mode == EA_MODE_I
            && ((strcmp(op->name, "add") == 0 && strcmp(op->spec_proc, "er") == 0) || strcmp(op->name, "adda") == 0
                || (strcmp(op->name, "and") == 0 && strcmp(op->spec_proc, "er") == 0)
                || (strcmp(op->name, "or") == 0 && strcmp(op->spec_proc, "er") == 0)
                || (strcmp(op->name, "sub") == 0 && strcmp(op->spec_proc, "er") == 0) || strcmp(op->name, "suba") == 0))
...
opcode_struct*
find_opcode(char* name, int size, char* spec_proc, char* spec_ea) {
    opcode_struct* op;

    for (op = g_opcode_input_table; op->name != NULL; op++) {
        if (strcmp(name, op->name) == 0 && (size == op->size) && strcmp(spec_proc, op->spec_proc) == 0
...
int
extract_opcode_info(char* src, char* name, int* size, char* spec_proc, char* spec_ea) {
...
void
write_body(FILE* filep, body_struct* body, replace_struct* replace) {
    int i;
    int j;
    char* ptr;
...
void
get_base_name(char* base_name, opcode_struct* op) {
...
void
write_prototype(FILE* filep, char* base_name) {
    fprintf(filep, "void %s(void);\n", base_name);
}

/* Write the name of an opcode handler function */
void
write_function_name(FILE* filep, char* base_name) {
    fprintf(filep, "void %s(void)\n", base_name);
}

void
add_opcode_output_table_entry(opcode_struct* op, char* name) {
    opcode_struct* ptr;
...
void
print_opcode_output_table(FILE* filep) {
    int i;
    qsort((void*)g_opcode_output_table, g_opcode_output_table_length, sizeof(g_opcode_output_table[0]),
...
void
write_table_entry(FILE* filep, opcode_struct* op) {
    int i;

    fprintf(filep, "\t{%-28s, 0x%04x, 0x%04x, {", op->name, op->op_mask, op->op_match);
...
void
set_opcode_struct(opcode_struct* src, opcode_struct* dst, int ea_mode) {
    int i;
...
void
generate_opcode_handler(FILE* filep, body_struct* body, replace_struct* replace, opcode_struct* opinfo, int ea_mode) {
...
void
generate_opcode_ea_variants(FILE* filep, body_struct* body, replace_struct* replace, opcode_struct* op) {
...
void
generate_opcode_cc_variants(FILE* filep, body_struct* body, replace_struct* replace, opcode_struct* op_in, int offset) {
    char repl[20];
    char replnot[20];
    int i;
    int old_length = replace->length;
