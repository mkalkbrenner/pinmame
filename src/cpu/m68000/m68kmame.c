...
static data16_t
readword_a24_d32(offs_t address) {
    data16_t result;
...
static void
writeword_a24_d32(offs_t address, data16_t data) {
    if (!(address & 1)) {
...
static data32_t
readlong_a24_d32(offs_t address) {
    data32_t result;
...
static void
writelong_a24_d32(offs_t address, data32_t data) {
    if (!(address & 3)) {
...
static const struct m68k_memory_interface interface_a24_d32 = {WORD_XOR_BE(0),    cpu_readmem24bedw,  readword_a24_d32,
                                                               readlong_a24_d32,  cpu_writemem24bedw, writeword_a24_d32,
                                                               writelong_a24_d32, changepc_a24_d32};
#endif
#endif
...
static data16_t
readword_a32_d32(offs_t address) {
    data16_t result;
...
static void
writeword_a32_d32(offs_t address, data16_t data) {
    if (!(address & 1)) {
...
static data32_t
readlong_a32_d32(offs_t address) {
    data32_t result;
...
static void
writelong_a32_d32(offs_t address, data32_t data) {
    if (!(address & 3)) {
...
static void
changepc_a32_d32(offs_t pc) {
    change_pc32bedw(pc);
}

#if HAS_M68020
/* interface for 24-bit address bus, 32-bit data bus (68020) */
static const struct m68k_memory_interface interface_a32_d32 = {WORD_XOR_BE(0),    cpu_readmem32bedw,  readword_a32_d32,
                                                               readlong_a32_d32,  cpu_writemem32bedw, writeword_a32_d32,
                                                               writelong_a32_d32, changepc_a32_d32};
#endif
...
static UINT8 m68000_reg_layout[] = {M68K_PC, M68K_ISP, 0xFF,    M68K_SR, M68K_USP, 0xFF,    M68K_D0, M68K_A0,
                                    0xFF,    M68K_D1,  M68K_A1, 0xFF,    M68K_D2,  M68K_A2, 0xFF,    M68K_D3,
                                    M68K_A3, 0xFF,     M68K_D4, M68K_A4, 0xFF,     M68K_D5, M68K_A5, 0xFF,
                                    M68K_D6, M68K_A6,  0xFF,    M68K_D7, M68K_A7,  0};
...
int
m68000_execute(int cycles) {
    return m68k_execute(cycles);
}

unsigned
m68000_get_context(void* dst) {
    return m68k_get_context(dst);
}

void
m68000_set_context(void* src) {
...
void
m68000_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
...
void
m68000_set_irq_line(int irqline, int state) {
...
void
m68000_set_irq_callback(int (*callback)(int irqline)) {
    m68k_set_int_ack_callback(callback);
}

const char*
m68000_info(void* context, int regnum) {
    ...
    switch (regnum) {
        case CPU_INFO_REG + M68K_PC:
            sprintf(buffer[which], "PC :%08X", m68k_get_reg(context, M68K_REG_PC));
            break;
        case CPU_INFO_REG + M68K_SR:
            sprintf(buffer[which], "SR :%04X", m68k_get_reg(context, M68K_REG_SR));
            break;
        case CPU_INFO_REG + M68K_SP:
            sprintf(buffer[which], "SP :%08X", m68k_get_reg(context, M68K_REG_SP));
            break;
        case CPU_INFO_REG + M68K_ISP:
            sprintf(buffer[which], "ISP:%08X", m68k_get_reg(context, M68K_REG_ISP));
            break;
        case CPU_INFO_REG + M68K_USP:
            sprintf(buffer[which], "USP:%08X", m68k_get_reg(context, M68K_REG_USP));
            break;
        case CPU_INFO_REG + M68K_D0:
            sprintf(buffer[which], "D0 :%08X", m68k_get_reg(context, M68K_REG_D0));
            break;
        case CPU_INFO_REG + M68K_D1:
            sprintf(buffer[which], "D1 :%08X", m68k_get_reg(context, M68K_REG_D1));
            break;
        case CPU_INFO_REG + M68K_D2:
            sprintf(buffer[which], "D2 :%08X", m68k_get_reg(context, M68K_REG_D2));
            break;
        case CPU_INFO_REG + M68K_D3:
            sprintf(buffer[which], "D3 :%08X", m68k_get_reg(context, M68K_REG_D3));
            break;
        case CPU_INFO_REG + M68K_D4:
            sprintf(buffer[which], "D4 :%08X", m68k_get_reg(context, M68K_REG_D4));
            break;
        case CPU_INFO_REG + M68K_D5:
            sprintf(buffer[which], "D5 :%08X", m68k_get_reg(context, M68K_REG_D5));
            break;
        case CPU_INFO_REG + M68K_D6:
            sprintf(buffer[which], "D6 :%08X", m68k_get_reg(context, M68K_REG_D6));
            break;
        case CPU_INFO_REG + M68K_D7:
            sprintf(buffer[which], "D7 :%08X", m68k_get_reg(context, M68K_REG_D7));
            break;
        case CPU_INFO_REG + M68K_A0:
            sprintf(buffer[which], "A0 :%08X", m68k_get_reg(context, M68K_REG_A0));
            break;
        case CPU_INFO_REG + M68K_A1:
            sprintf(buffer[which], "A1 :%08X", m68k_get_reg(context, M68K_REG_A1));
            break;
        case CPU_INFO_REG + M68K_A2:
            sprintf(buffer[which], "A2 :%08X", m68k_get_reg(context, M68K_REG_A2));
            break;
        case CPU_INFO_REG + M68K_A3:
            sprintf(buffer[which], "A3 :%08X", m68k_get_reg(context, M68K_REG_A3));
            break;
        case CPU_INFO_REG + M68K_A4:
            sprintf(buffer[which], "A4 :%08X", m68k_get_reg(context, M68K_REG_A4));
            break;
        case CPU_INFO_REG + M68K_A5:
            sprintf(buffer[which], "A5 :%08X", m68k_get_reg(context, M68K_REG_A5));
            break;
        case CPU_INFO_REG + M68K_A6:
            sprintf(buffer[which], "A6 :%08X", m68k_get_reg(context, M68K_REG_A6));
            break;
        case CPU_INFO_REG + M68K_A7:
            sprintf(buffer[which], "A7 :%08X", m68k_get_reg(context, M68K_REG_A7));
            break;
        case CPU_INFO_REG + M68K_PREF_ADDR:
            sprintf(buffer[which], "PAR:%08X", m68k_get_reg(context, M68K_REG_PREF_ADDR));
            break;
        case CPU_INFO_REG + M68K_PREF_DATA:
            sprintf(buffer[which], "PDA:%08X", m68k_get_reg(context, M68K_REG_PREF_DATA));
            break;
        case CPU_INFO_FLAGS:
            sr = m68k_get_reg(context, M68K_REG_SR);
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", (sr & 0x8000) ? 'T' : '.',
                    (sr & 0x4000) ? '?' : '.', (sr & 0x2000) ? 'S' : '.', (sr & 0x1000) ? '?' : '.',
                    (sr & 0x0800) ? '?' : '.', (sr & 0x0400) ? 'I' : '.', (sr & 0x0200) ? 'I' : '.',
                    (sr & 0x0100) ? 'I' : '.', (sr & 0x0080) ? '?' : '.', (sr & 0x0040) ? '?' : '.',
                    (sr & 0x0020) ? '?' : '.', (sr & 0x0010) ? 'X' : '.', (sr & 0x0008) ? 'N' : '.',
                    (sr & 0x0004) ? 'Z' : '.', (sr & 0x0002) ? 'V' : '.', (sr & 0x0001) ? 'C' : '.');
            break;
        case CPU_INFO_NAME:
            return "68000";
        ...
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m68000_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m68000_win_layout;
    }
    return buffer[which];
}

unsigned
m68000_dasm(char* buffer, unsigned pc) {
...
static UINT8 m68010_reg_layout[] = {
    M68K_PC, M68K_ISP, -1,      M68K_SR, M68K_USP, -1,      M68K_SFC, M68K_VBR, -1,      M68K_DFC, -1, M68K_D0,
    M68K_A0, -1,       M68K_D1, M68K_A1, -1,       M68K_D2, M68K_A2,  -1,       M68K_D3, M68K_A3,  -1, M68K_D4,
    M68K_A4, -1,       M68K_D5, M68K_A5, -1,       M68K_D6, M68K_A6,  -1,       M68K_D7, M68K_A7,  0};
...
int
m68010_execute(int cycles) {
    return m68k_execute(cycles);
}

unsigned
m68010_get_context(void* dst) {
    return m68k_get_context(dst);
}

void
m68010_set_context(void* src) {
    if (m68k_memory_intf.read8 != cpu_readmem24bew)
        m68k_memory_intf = interface_a24_d16;
    m68k_set_context(src);
}
...
unsigned
m68010_get_reg(int regnum) {
    switch (regnum) {
        case M68K_VBR:
            return m68k_get_reg(NULL, M68K_REG_VBR); /* 68010+ */
        case M68K_SFC:
            return m68k_get_reg(NULL, M68K_REG_SFC); /* 68010" */
        case M68K_DFC:
            return m68k_get_reg(NULL, M68K_REG_DFC); /* 68010+ */
        case REG_PC:
            return m68k_get_reg(NULL, M68K_REG_PC) & 0x00ffffff;
        case M68K_PC:
            return m68k_get_reg(NULL, M68K_REG_PC);
...
void
m68010_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case M68K_VBR:
...
void
m68010_set_irq_line(int irqline, int state) {
    if (irqline == IRQ_LINE_NMI)
        irqline = 7;
    switch (state) {
        case CLEAR_LINE:
...
void
m68010_set_irq_callback(int (*callback)(int irqline)) {
    m68k_set_int_ack_callback(callback);
}

const char*
m68010_info(void* context, int regnum) {
    static char buffer[32][47 + 1];
    static int which = 0;
    int sr;
    ...
    switch (regnum) {
        case CPU_INFO_REG + M68K_SFC:
            sprintf(buffer[which], "SFC:%X", m68k_get_reg(context, M68K_REG_SFC));
            break;
        case CPU_INFO_REG + M68K_DFC:
            sprintf(buffer[which], "DFC:%X", m68k_get_reg(context, M68K_REG_DFC));
            break;
        case CPU_INFO_REG + M68K_VBR:
            sprintf(buffer[which], "VBR:%08X", m68k_get_reg(context, M68K_REG_VBR));
            break;
        case CPU_INFO_REG + M68K_PC:
            sprintf(buffer[which], "PC :%08X", m68k_get_reg(context, M68K_REG_PC));
            break;
        ...
        case CPU_INFO_CREDITS:
            return "Copyright 1999-2000 Karl Stenerud. All rights reserved. (2.1 fixes HJB)";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m68010_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m68010_win_layout;
    }
    return buffer[which];
}

unsigned
m68010_dasm(char* buffer, unsigned pc) {
    M68K_SET_PC_CALLBACK(pc);
#ifdef MAME_DEBUG
...
static UINT8 m68ec020_reg_layout[] = {M68K_PC,  M68K_MSP, -1, M68K_SR, M68K_ISP, -1, M68K_SFC, M68K_USP, -1,
                                      M68K_DFC, M68K_VBR, -1, M68K_D0, M68K_A0,  -1, M68K_D1,  M68K_A1,  -1,
                                      M68K_D2,  M68K_A2,  -1, M68K_D3, M68K_A3,  -1, M68K_D4,  M68K_A4,  -1,
                                      M68K_D5,  M68K_A5,  -1, M68K_D6, M68K_A6,  -1, M68K_D7,  M68K_A7,  0};
...
int
m68ec020_execute(int cycles) {
    return m68k_execute(cycles);
}

unsigned
m68ec020_get_context(void* dst) {
    return m68k_get_context(dst);
}

void
m68ec020_set_context(void* src) {
...
unsigned
m68ec020_get_reg(int regnum) {
    switch (regnum) {
        case M68K_MSP:
            return m68k_get_reg(NULL, M68K_REG_MSP); /* 68020+ */
        case M68K_CACR:
            return m68k_get_reg(NULL, M68K_REG_CACR); /* 68020+ */
        case M68K_CAAR:
            return m68k_get_reg(NULL, M68K_REG_CAAR); /* 68020+ */
        case M68K_VBR:
            return m68k_get_reg(NULL, M68K_REG_VBR); /* 68010+ */
        case M68K_SFC:
            return m68k_get_reg(NULL, M68K_REG_SFC); /* 68010" */
...
void
m68ec020_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case M68K_MSP:
...
void
m68ec020_set_irq_line(int irqline, int state) {
    if (irqline == IRQ_LINE_NMI)
        irqline = 7;
    switch (state) {
        case CLEAR_LINE:
...
void
m68ec020_set_irq_callback(int (*callback)(int irqline)) {
    m68k_set_int_ack_callback(callback);
}

const char*
m68ec020_info(void* context, int regnum) {
    static char buffer[32][47 + 1];
    static int which = 0;
    int sr;
    ...
    switch (regnum) {
        case CPU_INFO_REG + M68K_MSP:
            sprintf(buffer[which], "MSP:%08X", m68k_get_reg(context, M68K_REG_MSP));
            break;
        case CPU_INFO_REG + M68K_CACR:
            sprintf(buffer[which], "CCR:%08X", m68k_get_reg(context, M68K_REG_CACR));
            break;
        case CPU_INFO_REG + M68K_CAAR:
            sprintf(buffer[which], "CAR:%08X", m68k_get_reg(context, M68K_REG_CAAR));
            break;
        case CPU_INFO_REG + M68K_SFC:
            sprintf(buffer[which], "SFC:%X", m68k_get_reg(context, M68K_REG_SFC));
            break;
        ...
        case CPU_INFO_FLAGS:
            sr = m68k_get_reg(context, M68K_REG_SR);
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", (sr & 0x8000) ? 'T' : '.',
                    (sr & 0x4000) ? 't' : '.', (sr & 0x2000) ? 'S' : '.', (sr & 0x1000) ? 'M' : '.',
                    (sr & 0x0800) ? '?' : '.', (sr & 0x0400) ? 'I' : '.', (sr & 0x0200) ? 'I' : '.',
                    (sr & 0x0100) ? 'I' : '.', (sr & 0x0080) ? '?' : '.', (sr & 0x0040) ? '?' : '.',
                    (sr & 0x0020) ? '?' : '.', (sr & 0x0010) ? 'X' : '.', (sr & 0x0008) ? 'N' : '.',
                    (sr & 0x0004) ? 'Z' : '.', (sr & 0x0002) ? 'V' : '.', (sr & 0x0001) ? 'C' : '.');
        ...
        case CPU_INFO_CREDITS:
            return "Copyright 1999-2000 Karl Stenerud. All rights reserved. (2.1 fixes HJB)";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m68ec020_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m68ec020_win_layout;
    }
    return buffer[which];
}

unsigned
m68ec020_dasm(char* buffer, unsigned pc) {
    M68K_SET_PC_CALLBACK(pc);
#ifdef MAME_DEBUG
...
static UINT8 m68020_reg_layout[] = {M68K_PC,  M68K_MSP, -1, M68K_SR, M68K_ISP, -1, M68K_SFC, M68K_USP, -1,
                                    M68K_DFC, M68K_VBR, -1, M68K_D0, M68K_A0,  -1, M68K_D1,  M68K_A1,  -1,
                                    M68K_D2,  M68K_A2,  -1, M68K_D3, M68K_A3,  -1, M68K_D4,  M68K_A4,  -1,
                                    M68K_D5,  M68K_A5,  -1, M68K_D6, M68K_A6,  -1, M68K_D7,  M68K_A7,  0};
...
int
m68020_execute(int cycles) {
    return m68k_execute(cycles);
}

unsigned
m68020_get_context(void* dst) {
    return m68k_get_context(dst);
}

void
m68020_set_context(void* src) {
...
void
m68020_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case M68K_MSP:
            m68k_set_reg(M68K_REG_MSP, val);
            break; /* 68020+ */
...
void
m68020_set_irq_line(int irqline, int state) {
    if (irqline == IRQ_LINE_NMI)
        irqline = 7;
    switch (state) {
        case CLEAR_LINE:
...
void
m68020_set_irq_callback(int (*callback)(int irqline)) {
    m68k_set_int_ack_callback(callback);
}

const char*
m68020_info(void* context, int regnum) {
    static char buffer[32][47 + 1];
    static int which = 0;
    int sr;
    ...
    switch (regnum) {
        ...
        case CPU_INFO_CREDITS:
            return "Copyright 1999-2000 Karl Stenerud. All rights reserved. (2.1 fixes HJB)";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m68020_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m68020_win_layout;
    }
    return buffer[which];
}

unsigned
m68020_dasm(char* buffer, unsigned pc) {
    M68K_SET_PC_CALLBACK(pc);
#ifdef MAME_DEBUG
...
enum {
    dirDUMR1A,
    dirDUMR2A, //0,1 (exception, these are r&w, but there's 2 so it works out)
    dirDUSRA,
    dirDUCSRA,    //2,3
    ...
    dirDUMR1B = 16,
    dirDUMR2B, //16,17 (exception, these are r&w, but there's 2 so it works out)
    dirDUSRB,
    dirDUCSRB,     //18,19
    ...
    dirDUOPCR, //26,27
    dirDUOPS,
    dirDUOPRS //28,29 (these are probably not needed)
};
...
//WRITE 8BIT
static void
m68306_write8(offs_t address, data8_t data) {
...
//WRITE 16BIT
static void
m68306_write16(offs_t address, data16_t data) {
    if (address >= 0xfffff000)
...
//WRITE 32BIT
static void
m68306_write32(offs_t address, data32_t data) {
    if (address >= 0xfffff000) {
...
//-------------------------------------------------------------------
static void
m68306_changepc(offs_t pc) {
    change_pc32bew(pc);
}

static const struct m68k_memory_interface interface_m68306 = {
    /*WORD_XOR_BE(0)*/ 0, m68306_read8,   m68306_read16,  m68306_read32,
    m68306_write8,        m68306_write16, m68306_write32, m68306_changepc};

//--------------------------------
//  DUART Internal register read
//--------------------------------
static data16_t
m68306_duart_reg_r(offs_t address, int word) {
...
static void
m68306_duart_reg_w(offs_t address, data16_t data, int word) {
    //odd word checks already handled!
    switch (address) {
        //F7E1 - MODE A REGISTERS (DUMR1A & DUMR2A)
        case 0xf7e1:
...
static void
m68306_load_transmitter(int which, int data) {
...
#ifndef PINMAME_NO_UNUSED // currently unused function (GCC 3.4)
static void
m68306_rx_cause_int_channel_a(int data) {
    m68306_rx_cause_int(0, data);
}
#endif
static void
m68306_rx_cause_int_channel_b(int data) {
    m68306_rx_cause_int(1, data);
}

//Generate an Receiver Interrupt
static void
m68306_rx_cause_int(int which, int data) {
    int push = 0;
...
static void
duart_timer_callback(int param) {
    //Invert output
...
static void
m68306_duart_set_duimr(int data) {
    //Update register
...
static void
m68306_duart_set_duisr(int data) {
    //Update register
...
static void
m68306_duart_set_dusr(int which, int data) {
    int txRDY;
    int rxRDY;
...
static void
duart_command_register_w(int which, int data) {
    int cmd;
    //Bit 7   = 0
...
//----------------------------
static void
m68306_intreg_w(offs_t address, data16_t data, int word) {
    if (word && (address & 1)) {
    ...
    if (address >= 0xffffffc0) { /* internal regs */
        ...
        if (!word) { // handle byte writes
            if (address & 1)
                data = (oldval & 0xff00) | (data & 0x00ff);
            else
                data = (oldval & 0x00ff) | (data << 8);
        }
        switch (reg) {
            case irCS0H:
            case irCS1H:
            ...
            case irPDATA: { /* Port Data */
                ...
                if (tmp & 0xff00)
                    cpu_writeport16lew(M68306_PORTA_START, (data & m68306intreg[irPDIR]) >> 8);
                if (tmp & 0x00ff)
                    cpu_writeport16lew(M68306_PORTB_START, data & m68306intreg[irPDIR]);
            ...
            case irPDIR: { /* Port direction */
                ...
                if (tmp & 0xff00)
                    cpu_writeport16lew(M68306_PORTA_START, (data & m68306intreg[irPDATA]) >> 8);
                if (tmp & 0x00ff)
                    cpu_writeport16lew(M68306_PORTB_START, data & m68306intreg[irPDATA]);
                break;
            }
            case irICR: /* interrupt control */
                LOG(("%8x:ICR=%04x\n", activecpu_get_pc(), data));
                m68306intreg[irICR] = data;
                m68306irq(0, 0); // update irq level
                break;
            ...
            case irSYSTEM: /* system */
                ...
                if (data & 0x4000) {
                    LOG(("Bus Timeout Error not implmented %x\n", data));
                }
                break;
        }                               /* switch */
...
//----------------------------
static data16_t
m68306_intreg_r(offs_t address, int word) {
    data16_t data = 0;
    if (word && (address & 1)) {
    ...
    if (address >= 0xffffffc0) { /* internal regs */
        ...
        switch (reg) {
            ...
            case irPDIR:
                ...
                data = m68306intreg[reg];
                break;
            case irPPIN: /* port pins (read_only) */
                // B4-B7 is also IRQ pins (ignored for now)
                if (word)
                    data = (((cpu_readport16lew(M68306_PORTA_START) << 8) | cpu_readport16lew(M68306_PORTB_START))
                            & ~m68306intreg[irPDIR])
                           | (m68306intreg[irPDATA] & m68306intreg[irPDIR]);
                else if (address & 1)
                    data = (cpu_readport16lew(M68306_PORTB_START) & ~m68306intreg[irPDIR])
                           | (m68306intreg[irPDATA] & (m68306intreg[irPDIR] | 0xff00));
                else
                    data = ((cpu_readport16lew(M68306_PORTA_START) << 8) & ~m68306intreg[irPDIR])
                           | (m68306intreg[irPDATA] & (m68306intreg[irPDIR] | 0x00ff));
                break;
            case irISR: /* interrupt status (read only)*/
                data = m68306intreg[irISR];
                break;
            case irICR: /* interrupt control */
                data = m68306intreg[irICR];
                break;
            case irBUSERR: /* refresh + buserror */
                LOG(("buserror_r\n"));
                break;
...
static void
m68306irq(int irqline, int state) {
    if (irqline) {
        if (state) {
...
//---------------------
void
m68306_set_irq_line(int irqline, int state) {
    switch (state) {
        case PULSE_LINE:
            m68306irq(irqline, 1); // no break;
        case CLEAR_LINE:
            m68306irq(irqline, 0);
            break;
        case ASSERT_LINE:
...
void
m68306_set_irq_callback(int (*callback)(int irqline)) {
...
void
m68306_set_reg(int regnum, unsigned val) {
    if ((short)regnum == (short)REG_PC)
        m68k_set_reg(M68K_REG_PC, val);
    else
...
const char*
m68306_info(void* context, int regnum) {
    static char buffer[32][47 + 1];
    static int which = 0;
    int sr;
    ...
    switch (regnum) {
        ...
        case CPU_INFO_FLAGS:
            sr = m68k_get_reg(context, M68K_REG_SR);
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", sr & 0x8000 ? 'T' : '.', sr & 0x4000 ? '?' : '.',
                    sr & 0x2000 ? 'S' : '.', sr & 0x1000 ? '?' : '.', sr & 0x0800 ? '?' : '.', sr & 0x0400 ? 'I' : '.',
                    sr & 0x0200 ? 'I' : '.', sr & 0x0100 ? 'I' : '.', sr & 0x0080 ? '?' : '.', sr & 0x0040 ? '?' : '.',
                    sr & 0x0020 ? '?' : '.', sr & 0x0010 ? 'X' : '.', sr & 0x0008 ? 'N' : '.', sr & 0x0004 ? 'Z' : '.',
                    sr & 0x0002 ? 'V' : '.', sr & 0x0001 ? 'C' : '.');
            break;
        case CPU_INFO_NAME:
            return "68306";
        case CPU_INFO_FAMILY:
        ...
        case CPU_INFO_FILE:
            return __FILE__;
        case CPU_INFO_CREDITS:
            return "Copyright 2002 Martin Adrian, Steve Ellenoff and PinMAME team";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m68000_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m68000_win_layout;
...
unsigned
m68306_dasm(char* buffer, unsigned pc) {
    M68K_SET_PC_CALLBACK(pc);
#ifdef MAME_DEBUG
