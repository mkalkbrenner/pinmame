...
static char*
WritePadding(char* pBuf, const char* pBuf0) {
    pBuf0 += 8;
    while (pBuf < pBuf0) {
        *pBuf++ = ' ';
    }
...
static char*
DasmCoProc_RT(char* pBuf, data32_t opcode, char* pConditionCode, char* pBuf0) {
    ...
    pBuf += sprintf(pBuf, "p%d, %d, R%d, c%d, c%d", (opcode >> 8) & 0xf, (opcode >> 21) & 7, (opcode >> 12) & 0xf,
                    (opcode >> 16) & 0xf, opcode & 0xf);
    if ((opcode >> 5) & 7)
        pBuf += sprintf(pBuf, ", %d", (opcode >> 5) & 7);
    return pBuf;
}

static char*
DasmCoProc_DT(char* pBuf, data32_t opcode, char* pConditionCode, char* pBuf0) {
    /* co processor data transfer */
    /* xxxx 111P UNWL nnnn dddd pppp oooooooo */
    //todo: test this on valid instructions

    pBuf +=
        sprintf(pBuf, "%s%s", (opcode & 0x00100000) ? "LDC" : "STC", pConditionCode); //Bit 20 = 1 for Load, 0 for Store
    //Long Operation
    if (opcode & 0x400000)
        pBuf += sprintf(pBuf, "L");
    pBuf = WritePadding(pBuf, pBuf0);
    ...
    //Base Register (Rn)
    pBuf += sprintf(
        pBuf, "[R%d%s", (opcode >> 16) & 0x0f,
        (opcode & 0x1000000) ? "" : "]"); //If Bit 24 = 1, Pre-increment, otherwise, Post increment so close brace

    //immediate value ( 8 bit value is << 2 according to manual )
    if (opcode & 0xff)
        pBuf += sprintf(pBuf, ",%s#$%x", (opcode & 0x800000) ? "" : "-", (opcode & 0xff) << 2);
...
static char*
DasmCoProc_DO(char* pBuf, data32_t opcode, char* pConditionCode, char* pBuf0) {
    ...
    pBuf = WritePadding(pBuf, pBuf0);
    //p#,CPOpc,cd,cn,cm
    pBuf += sprintf(pBuf, "p%d, %d, c%d, c%d, c%d", (opcode >> 8) & 0xf, (opcode >> 20) & 0xf, (opcode >> 12) & 0xf,
                    (opcode >> 16) & 0xf, opcode & 0xf);
    if ((opcode >> 5) & 7)
        pBuf += sprintf(pBuf, ", %d", (opcode >> 5) & 7);
    return pBuf;
...
static char*
WriteImmediateOperand(char* pBuf, data32_t opcode) {
    /* rrrrbbbbbbbb */
    data32_t imm;
    int r;
...
static char*
WriteDataProcessingOperand(char* pBuf, data32_t opcode, int printOp0, int printOp1, int printOp2) {
    /* ccccctttmmmm */
...
static char*
WriteRegisterOperand1(char* pBuf, data32_t opcode) {
    /* ccccctttmmmm */
    static const char* pRegOp[4] = {"LSL", "LSR", "ASR", "ROR"};

    pBuf += sprintf(pBuf, ", R%d", /* Operand 1 register, Operand 2 register, shift type */
                    (opcode >> 0) & 0xf);
...
static char*
WriteBranchAddress(char* pBuf, data32_t pc, data32_t opcode, data8_t h_bit) {
    opcode <<= 2;
...
void
arm7_disasm(char* pBuf, data32_t pc, data32_t opcode) {
    const char* pBuf0;

    const char* pConditionCodeTable[16] = {"EQ", "NE", "CS", "CC", "MI", "PL", "VS", "VC",
                                           "HI", "LS", "GE", "LT", "GT", "LE", "",   "NV"};
    const char* pOperation[16] = {"AND", "EOR", "SUB", "RSB", "ADD", "ADC", "SBC", "RSC",
    ...
    } else if ((opcode & 0x0e000000) == 0 && (opcode & 0x80) && (opcode & 0x10)) //bits 27-25 == 000, bit 7=1, bit 4=1
    {
        ...
        if (opcode & 0x60) { //bits = 6-5 != 00
            ...
            pBuf += sprintf(pBuf, "%s%s", (opcode & 0x00100000) ? "LDR" : "STR",
                            pConditionCode); //Bit 20 = 1 for Load, 0 for Store
            ...
            //Base Register
            pBuf += sprintf(pBuf, "[R%d%s", (opcode >> 16) & 0x0f,
                            (opcode & 0x1000000)
                                ? ""
                                : "]"); //If Bit 24 = 1, Pre-increment, otherwise, Post increment so close brace
            ...
            if (opcode & 0x400000) { //Bit 22 - 1 = immediate, 0 = register
                ...
                pBuf += sprintf(pBuf, ",%s#$%x", (opcode & 0x800000) ? "" : "-",
                                ((((opcode >> 8) & 0x0f) << 4) | (opcode & 0x0f)));
            } else {
                //register
            ...
            if (opcode & 0x01000000) { //bit 24 = 1
                ...
                pBuf += sprintf(pBuf, "%s%s", pConditionCode,
                                (opcode & 0x400000) ? "B" : ""); //Bit 22 = Byte/Word selection
                //Rd, Rm, [Rn]
                ...
                if (opcode & 0x800000) //Bit 23 = 1 for Multiply Long
                {
                    ...
                    pBuf += sprintf(pBuf, "R%d, R%d, R%d, R%d", (opcode >> 12) & 0xf, (opcode >> 16) & 0xf,
               ...
               == 0) //bits 27-26 == 00 - This check can only exist properly after Multiplication check above
    {
        ...
        if (((opcode & 0x00100000) == 0) && ((opcode & 0x01800000) == 0x01000000)) {
            ...
            pBuf += sprintf(pBuf, "%s%s", pOperation[op], pConditionCode);
    ...
    } else if ((opcode & 0x0c000000) == 0x04000000) //bits 27-26 == 01
    {
        ...
        pBuf = WritePadding(pBuf, pBuf0);
        pBuf += sprintf(pBuf, "R%d, [R%d", (opcode >> 12) & 0xf, (opcode >> 16) & 0xf);
    ...
    {
        /* Software Interrupt */
        pBuf += sprintf(pBuf, "SWI%s $%x", pConditionCode, opcode & 0x00ffffff);
    } else {
