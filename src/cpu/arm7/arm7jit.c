...
#define cpsr_to_flags()                                                                                                \
    emit(MOV, EAX, RCPSR), emit(SHR, EAX, Imm, 22), emit(SHL, AH, Imm, 5), emit(SHL, EAX, Imm, 1),                     \
        emit(ADD, AL, Imm, 0x80), emit(SAHF)
...
#define flags_to_cpsr()                                                                                                \
    emit(LAHF), emit(SETO, AL), emit(SHL, AL, Imm, 7), emit(SHR, EAX, Imm, 1), emit(SHR, AH, Imm, 5),                  \
        emit(SHL, EAX, Imm, 22), emit(AND, DwordPtr, RCPSR, Imm, 0x0FFFFFFF), emit(OR, RCPSR, EAX)

// Copy just the carry flag to intel register.

#define cpsr_to_carry() emit(MOV, EAX, RCPSR), emit(SHR, EAX, Imm, 21), emit(SAHF)
...
#define carry_to_cpsr()                                                                                                \
    emit(SETC, CL), emit(SHL, ECX, Imm, 29), emit(AND, DwordPtr, RCPSR, Imm, ~C_MASK), emit(OR, RCPSR, ECX)
...
// subroutine calls.
#define count_cycles(n)                                                                                                \
    if ((n) == 1)                                                                                                      \
        emit(DEC, RCYCLECNT);                                                                                          \
    else if ((n) > 0)                                                                                                  \
        emit(SUB, RCYCLECNT, Imm, n);                                                                                  \
    else

// signed long mulitply
static int
MUL64(struct jit_ctl* jit, data32_t addr, data32_t insn, int* cycles) {
    int rm, rs;
    int rdhi, rdlo;
...
// 32-bit multiply
static int
MUL32(struct jit_ctl* jit, data32_t addr, data32_t insn, int* cycles) {
    int rd, rm, rs, rn;
...
static void
gen_test_abort(struct jit_ctl* jit, data32_t addr, int sp_inc) {
    ...
    if (sp_inc != 0)
        emit(ADD, ESP, Imm, sp_inc); // clear temps off the stack
...
static void
gen_test_irq(struct jit_ctl* jit, data32_t addr) {
...
static void
gen_mem_read(struct jit_ctl* jit, int siz, int rn, data32_t immAddr, int ofs) {
...
static void
gen_mem_write(struct jit_ctl* jit, int siz, data32_t addr, int rd, int rn, data32_t immAddr, int ofs) {
...
static void
gen_mem(struct jit_ctl* jit, int rd, int ld, int siz, int sx, int addr, int* is_br, int nativeReg, data32_t immAddr,
        int sp_inc) {
    if (ld) {
...
static int
LDRHB_STRHB(struct jit_ctl* jit, data32_t addr, data32_t insn, int* is_br, int* cycles) {
    int rn, rd, rm;
    ...
    immOfs = (((insn >> 8) & 0x0f) << 4)
             | (insn & 0x0f); // immediate value in high nibble (bits 8-11) and lo nibble (bit 0-3)
...
static int
genShift(data32_t insn, data32_t addr, int carry_out) {
...
static void
emit_jump(struct jit_ctl* jit, data32_t dest_addr, int* cycles) {
...
static int
ALU(struct jit_ctl* jit, data32_t addr, data32_t insn, int* is_br, int* cycles) {
    data32_t opcode;
    int result_reg;
    int is_logical_op;
    int is_sub_op;
...
static int
LDR_STR(struct jit_ctl* jit, data32_t addr, data32_t insn, int* is_br, int* cycles) {
...
static int
SWAP(struct jit_ctl* jit, data32_t addr, data32_t insn, int* cycles) {
    int rn, rm, rd;
    int siz;
...
// branch and exchange
static int
BX(struct jit_ctl* jit, data32_t addr, data32_t insn) {
...
static int
PSRX(struct jit_ctl* jit, data32_t addr, data32_t insn, int* cycles) {
...
static int
LDM_STM(struct jit_ctl* jit, data32_t addr, data32_t insn, int* is_br, int* cycles) {
...
// branch, branch link
static int
B(struct jit_ctl* jit, data32_t addr, data32_t insn, int* is_br, int* cycles) {
...
// fails.
static int
xlat_recursive(struct jit_ctl* jit, data32_t addr) {
    void* addrp;
...
static int
xlat(struct jit_ctl* jit, data32_t pc) {
    data32_t insn;
    int cnt;
    int ok;
    ...
    for (addr = pc, cnt = 0, ok = 1;; addr += 4) {
        ...
        switch ((insn & 0xF000000) >> 24) {
            ...
            case 1:
                ...
                } else if (((insn & 0x0100000) == 0)
                           && ((insn & 0x01800000) == 0x01000000)) // S=0, and bits 24-23 == 10 -> PSR transfer
                {
...
static int
arm7_jit_xlat(struct jit_ctl* jit, data32_t pc) {
    byte* p;
