 ...
 ***************************************************************************/
INLINE void
arm7_cpu_write32(int addr, data32_t data) {
    ...
    if (insn & INSN_S)                                                                                                 \
        SET_CPSR(                                                                                                      \
            ((GET_CPSR & ~(N_MASK | Z_MASK | V_MASK | C_MASK))                                                         \
             ...
             | ((((IsNeg(rn) & IsNeg(op2)) | (IsNeg(rn) & IsPos(rd)) | (IsNeg(op2) & IsPos(rd))) & SIGN_BIT)           \
                        == SIGN_BIT                                                                                    \
                    ? C_MASK                                                                                           \
                    : 0)                                                                                               \
             | HandleALUNZFlags(rd)));                                                                                 \
...
#define HandleALUSubFlags(rd, rn, op2)                                                                                 \
    if (insn & INSN_S)                                                                                                 \
        SET_CPSR(((GET_CPSR & ~(N_MASK | Z_MASK | V_MASK | C_MASK))                                                    \
                  ...
                  | ((((IsNeg(rn) & IsPos(op2)) | (IsNeg(rn) & IsPos(rd)) | (IsPos(op2) & IsPos(rd))) & SIGN_BIT)      \
                             == SIGN_BIT                                                                               \
                         ? C_MASK                                                                                      \
                         : 0)                                                                                          \
                  | HandleALUNZFlags(rd)));                                                                            \
...
#define HandleALUNZFlags(rd)     (((rd)&SIGN_BIT) | (((rd) == 0) ? Z_MASK : 0))

//Long ALU Functions use bit 63
#define HandleLongALUNZFlags(rd) ((((rd) & ((UINT64)1 << 63)) >> 32) | (((rd) == 0) ? Z_MASK : 0))

#define HandleALULogicalFlags(rd, sc)                                                                                  \
    if (insn & INSN_S)                                                                                                 \
        SET_CPSR(((GET_CPSR & ~(N_MASK | Z_MASK | C_MASK)) | HandleALUNZFlags(rd) | (((sc) != 0) ? C_MASK : 0)));      \
    R15 += 4;

//convert cpsr mode num into to text
static const char modetext[ARM7_NUM_MODES][5] = {"USER", "FIRQ", "IRQ",  "SVC", "ILL1", "ILL2", "ILL3", "ABT",
                                                 "ILL4", "ILL5", "ILL6", "UND", "ILL7", "ILL8", "ILL9", "SYS"};

static const char*
GetModeText(int cpsr) {
...
#define SetModeRegister(mode, rIndex, value)                                                                           \
    ((mode) == GET_MODE ? SetActiveRegister(rIndex, value) : SetSavedRegister(mode, rIndex, value))

INLINE void
SwitchMode(int cpsr_mode_val) {
...
INLINE data32_t
decodeShift(data32_t insn, data32_t* pCarry) {
...
static int
loadInc(data32_t pat, data32_t rbv, data32_t s) {
    int i, result;
    UINT32 data;
...
static int
loadIncMode(data32_t pat, data32_t rbv, data32_t s, int mode) {
    int i, result;
    UINT32 data;
...
static int
loadDec(data32_t pat, data32_t rbv, data32_t s) {
    int i, result;
    UINT32 data;
...
static int
loadDecMode(data32_t pat, data32_t rbv, data32_t s, int mode) {
    int i, result;
    UINT32 data;
...
static int
storeInc(data32_t pat, data32_t rbv) {
    int i, result;
...
static int
storeIncMode(data32_t pat, data32_t rbv, int mode) {
    int i, result;
...
static int
storeDec(data32_t pat, data32_t rbv) {
    int i, result = 0, cnt;
...
static int
storeDecMode(data32_t pat, data32_t rbv, int mode) {
    int i, result;
...
//CPU INIT
static void
arm7_core_init(const char* cpuname) {
    ...
    jit_set_mem_callbacks(ARM7.jit, PTR_READ8, PTR_READ16, PTR_READ32, PTR_WRITE8, PTR_WRITE16, PTR_WRITE32);
}
...
//CPU - SET IRQ LINE
static void
arm7_core_set_irq_line(int irqline, int state) {
    switch (irqline) {
...
HandleCoProcDO(data32_t insn) {
    ...
    if (arm7_coproc_do_callback)
        arm7_coproc_do_callback(
            insn, 0,
            0); //simply pass entire opcode to callback - since data format is actually dependent on co-proc implementation
#if ARM7_DEBUG_CORE
    else
...
HandleCoProcRT(data32_t insn) {
    ...
    if (insn & 0x00100000) //Bit 20 = Load or Store
    {
        if (arm7_coproc_rt_r_callback) {
            data32_t res =
                arm7_coproc_rt_r_callback(insn, 0); //RT Read handler must parse opcode & return appropriate result
...
static void
HandleBranch(data32_t insn, data8_t h_bit) {
        ...
        if (insn & INSN_SDT_U) {
            ...
            if (rd == rn) {
                ...
                if ((insn & INSN_SDT_W) != 0)
                    LOG(("%08x:  RegisterWritebackIncrement %d %d %d\n", R15, (insn & INSN_SDT_P) != 0,
                         (insn & INSN_SDT_W) != 0, (insn & INSN_SDT_U) != 0));
#endif
...
static void
HandleHalfWordDT(data32_t insn) {
    data32_t rn, rnv, off, rd, rnv_old = 0;
...
static void
HandleMSR(int spsr, data32_t val, data32_t fields) {
...
#if ARM7_DEBUG_CORE
    if (((insn & INSN_MUL_RM) == 0xf) || (((insn & INSN_MUL_RS) >> INSN_MUL_RS_SHIFT) == 0xf)
...
#if ARM7_DEBUG_CORE
    if (((insn & 0xf) == 15) || (((insn >> 8) & 0xf) == 15) || (((insn >> 16) & 0xf) == 15)
        || (((insn >> 12) & 0xf) == 15))
