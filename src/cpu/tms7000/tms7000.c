...
static UINT8 tms7000_reg_layout[] = {TMS7000_PC, TMS7000_SP, TMS7000_ST, 0};
...
void
WRF16(UINT32 mAddr, PAIR p) {
    WM((mAddr - 1) & 0xffff, p.b.h);
    WM(mAddr, p.b.l);
}

#define IMMBYTE(b)                                                                                                     \
    b = ((unsigned)cpu_readop_arg(pPC));                                                                               \
    pPC++
#define SIMMBYTE(b)                                                                                                    \
    b = ((signed)cpu_readop_arg(pPC));                                                                                 \
    pPC++
#define IMMWORD(w)                                                                                                     \
    w.b.h = (unsigned)cpu_readop_arg(pPC++);                                                                           \
    w.b.l = (unsigned)cpu_readop_arg(pPC++)
...
typedef struct {
    ...
    UINT8 timer1_decrementator, timer1_prescaler, timer1_capturelatch;
} tms7000_Regs;
 ...
 ****************************************************************************/
void
tms7000_set_context(void* src) {
    if (src)
        tms7000 = *(tms7000_Regs*)src;
}

unsigned
tms7000_get_reg(int regnum) {
    switch (regnum) {
        case REG_PC:
        case TMS7000_PC:
            return pPC;
...
void
tms7000_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
        case TMS7000_PC:
            pPC = val;
 ...
 ****************************************************************************/
const char*
tms7000_info(void* context, int regnum) {
    ...
    switch (regnum) {
        ...
        case CPU_INFO_FAMILY:
            return "Texas Instruments TMS7000";
        case CPU_INFO_VERSION:
            return "1.0";
        case CPU_INFO_FILE:
            return __FILE__;
        case CPU_INFO_CREDITS:
            return "Copyright (C) tim lindner 2001";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)tms7000_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)tms7000_win_layout;
        case CPU_INFO_FLAGS:
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c", (r->sr & 0x80) ? 'C' : 'c', (r->sr & 0x40) ? 'N' : 'n',
                    (r->sr & 0x20) ? 'Z' : 'z', (r->sr & 0x10) ? 'I' : 'i', (r->sr & 0x08) ? '?' : '.',
                    (r->sr & 0x04) ? '?' : '.', (r->sr & 0x02) ? '?' : '.', (r->sr & 0x01) ? '?' : '.');
            break;
        case CPU_INFO_REG + TMS7000_PC:
            sprintf(buffer[which], "PC:%04X", r->pc.w.l);
            break;
        case CPU_INFO_REG + TMS7000_SP:
            sprintf(buffer[which], "SP:%02X", r->sp);
            break;
        case CPU_INFO_REG + TMS7000_ST:
            sprintf(buffer[which], "ST:%02X", r->sr);
            break;
    }
    return buffer[which];
}
...
unsigned
tms7000_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
...
void
tms7000_set_irq_line(int irqline, int state) {
...
void
tms7000_set_irq_callback(int (*callback)(int irqline)) {
    tms7000.irq_callback = callback;
}

void
tms7000_check_IRQ_lines(void) {
    UINT16 newPC;
    int tms7000_state;
...
void
tms7000_int2_callback(int param) {
    tms7000_set_irq_line(TMS7000_IRQ2_LINE, ASSERT_LINE);
    tms7000_starttimer1();
}
...
UINT8
tms7000_calculate_timer1_decrementator(void) {
    UINT8 result;
	...
	*/
    ...
    double prescalertimer = TIME_IN_CYCLES(16, cpuNum) * (tms7000.pf[0x03] & 0x1f);
...
INLINE
WRITE_HANDLER(tms7000_writemem) {
    //Internal RAM (0-0xff)
    if (offset < 0x100)
