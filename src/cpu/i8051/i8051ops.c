...
pop(void) {
    ...
    IRAM_W(
        addr,
        IRAM_IR(
            R_SP)); //Store to contents of data addr, data pointed to by Stack - IRAM_IR needed to access upper 128 bytes of stack
...
push(void) {
    ...
    if (tmpSP == R_SP)          //Ensure it was able to write to new stack location
        IRAM_IW(
            tmpSP,
            IRAM_R(
                addr)); //Store to stack contents of data address - IRAM_IW needed to store to upper 128 bytes of stack, however, can't use IRAM_IR because that won't store the sfrs and it's not an indirect access anyway
}
