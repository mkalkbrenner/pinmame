...
static UINT8 i8051_reg_layout[] = {I8051_PC, I8051_SP, I8051_PSW, I8051_ACC, I8051_B,  I8051_DPH, I8051_DPL,
                                   I8051_IE, 0xFF,     I8051_R0,  I8051_R1,  I8051_R2, I8051_R3,  I8051_R4,
                                   I8051_R5, I8051_R6, I8051_R7,  I8051_RB,  0};
...
static UINT8 i8051_cycles[] = {
    12, 24, 24, 12, 12, 12, 12, 12, 12, 12,       12, 12, 12, 12, 12, 12, 24, 24, 24, 12, 12, 12, 12, 12, 12, 12,
    12, 12, 12, 12, 12, 12, 24, 24, 24, 12,       12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 24, 24, 24, 12,
    12, 12, 12, 12, 12, 12, 12, 12, 12, 12,       12, 12, 24, 24, 12, 24, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12,
    12, 12, 24, 24, 12, 24, 12, 12, 12, 12,       12, 12, 12, 12, 12, 12, 12, 12, 24, 24, 12, 24, 12, 12, 12, 12,
    12, 12, 12, 12, 12, 12, 12, 12, 24, 24,       24, 24, 12, 24, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 24, 24,
    24, 24, 48, 24, 24, 24, 24, 24, 24, 24,       24, 24, 24, 24, 24, 24, 24, 24, 12, 12, 12, 12, 12, 12, 12, 12,
    12, 12, 12, 12, 24, 24, 12, 24, 48, 12 /*?*/, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 12, 12, 24, 24,
    24, 24, 24, 24, 24, 24, 24, 24, 24, 24,       24, 24, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12,
    24, 24, 12, 12, 12, 24, 12, 12, 24, 24,       24, 24, 24, 24, 24, 24, 24, 24, 24, 24, 12, 12, 12, 12, 12, 12,
    12, 12, 12, 12, 12, 12, 24, 24, 12, 12,       12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12, 12};
...
/*NOTE: These are not used by the opcode functions, they are here only for MAME requirements*/
void
i8051_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
        case I8051_PC:
            PC = val & 0xffff;
        ...
        case I8051_SP:
            SFR_W(SP, val);
            break; //Use SFR to handle error checking
        case I8051_ACC:
            SFR_W(ACC, val);
            break;
        case I8051_PSW:
...
void
i8051_set_irq_line(int irqline, int state) {
    switch (irqline) {
...
void
i8051_set_irq_callback(int (*callback)(int irqline)) {
    i8051.irq_callback = callback;
}

void
i8051_set_serial_tx_callback(void (*callback)(int data)) {
...
void
i8051_set_serial_rx_callback(int (*callback)(void)) {
    //Hold in static variable since this function can get called before reset has run, which wipes i8051 memory clean
    hold_serial_rx_callback = callback;
}

void
i8051_set_eram_iaddr_callback(READ32_HANDLER((*callback))) {
    //Hold in static variable since this function can get called before reset has run, which wipes i8051 memory clean
    hold_eram_iaddr_callback = callback;
}

void
i8051_state_save(void* file) {}

void
i8051_state_load(void* file) {}
...
const char*
i8051_info(void* context, int regnum) {
    ...
    switch (regnum) {
        case CPU_INFO_REG + I8051_PC:
            sprintf(buffer[which], "PC:%04X", r->pc);
            break;
        case CPU_INFO_REG + I8051_SP:
            sprintf(buffer[which], "SP:%02X", r->sp);
            break;
        case CPU_INFO_REG + I8051_PSW:
            sprintf(buffer[which], "PSW:%02X", r->psw);
            break;
        case CPU_INFO_REG + I8051_ACC:
            sprintf(buffer[which], "A:%02X", r->acc);
            break;
        case CPU_INFO_REG + I8051_B:
            sprintf(buffer[which], "B:%02X", r->b);
            break;
        case CPU_INFO_REG + I8051_DPH:
            sprintf(buffer[which], "DPH:%02X", r->dph);
            break;
        case CPU_INFO_REG + I8051_DPL:
            sprintf(buffer[which], "DPL:%02X", r->dpl);
            break;
        case CPU_INFO_REG + I8051_IE:
            sprintf(buffer[which], "IE:%02X", r->ie);
            break;
        case CPU_INFO_REG + I8051_R0:
            sprintf(buffer[which], "R0:%02X", r->IntRam[0 + (8 * ((r->psw & 0x18) >> 3))]);
            break;
        case CPU_INFO_REG + I8051_R1:
            sprintf(buffer[which], "R1:%02X", r->IntRam[1 + (8 * ((r->psw & 0x18) >> 3))]);
            break;
        case CPU_INFO_REG + I8051_R2:
            sprintf(buffer[which], "R2:%02X", r->IntRam[2 + (8 * ((r->psw & 0x18) >> 3))]);
            break;
        case CPU_INFO_REG + I8051_R3:
            sprintf(buffer[which], "R3:%02X", r->IntRam[3 + (8 * ((r->psw & 0x18) >> 3))]);
            break;
        case CPU_INFO_REG + I8051_R4:
            sprintf(buffer[which], "R4:%02X", r->IntRam[4 + (8 * ((r->psw & 0x18) >> 3))]);
            break;
        case CPU_INFO_REG + I8051_R5:
            sprintf(buffer[which], "R5:%02X", r->IntRam[5 + (8 * ((r->psw & 0x18) >> 3))]);
            break;
        case CPU_INFO_REG + I8051_R6:
            sprintf(buffer[which], "R6:%02X", r->IntRam[6 + (8 * ((r->psw & 0x18) >> 3))]);
            break;
        case CPU_INFO_REG + I8051_R7:
            sprintf(buffer[which], "R7:%02X", r->IntRam[7 + (8 * ((r->psw & 0x18) >> 3))]);
            break;
        case CPU_INFO_REG + I8051_RB:
            sprintf(buffer[which], "RB:%02X", ((r->psw & 0x18) >> 3));
            break;

        case CPU_INFO_FLAGS:
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c", (r->psw & 0x80) ? 'C' : '.', (r->psw & 0x40) ? 'A' : '.',
                    (r->psw & 0x20) ? 'F' : '.', (r->psw & 0x10) ? '0' : '.', (r->psw & 0x08) ? '1' : '.',
                    (r->psw & 0x04) ? 'V' : '.', (r->psw & 0x02) ? '?' : '.', (r->psw & 0x01) ? 'P' : '.');
            break;
        case CPU_INFO_NAME:
            return "I8051";
        ...
        case CPU_INFO_CREDITS:
            return "Copyright (c) 2003 Steve Ellenoff, all rights reserved.";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)i8051_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)i8051_win_layout;
    }
    return buffer[which];
}

unsigned
i8051_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
...
static READ_HANDLER(internal_ram_iread) { return i8051.iram_iread(offset); }

/* Writes the contents of the Internal RAM memory (BUT CALLED FROM AN INDIRECT ADDRESSING MODE)   */
/* Different chip types handle differently, for speed, simply call the chip's handler */
static WRITE_HANDLER(internal_ram_iwrite) { i8051.iram_iwrite(offset, data); }
...
push_pc() {
    ...
    if (tmpSP == R_SP)               //Ensure it was able to write to new stack location
        IRAM_IW(
            tmpSP,
            ((PC & 0xff00)
             >> 8)); //Store hi byte of PC to next address in Internal Ram (Use IRAM_IW to store stack above 128 bytes)
}
...
INLINE void
do_add_flags(UINT8 a, UINT8 data, UINT8 c) {
...
INLINE void
do_sub_flags(UINT8 a, UINT8 data, UINT8 c) {
    UINT16 result = a - (data + c);
    INT16 result1 = (INT8)a - (INT8)(data + c);
    int cy, ac, ov;
    cy = (result & 0x100) >> 8;
...
int
i8752_execute(int cycles) {
    return i8051_execute(cycles);
}

unsigned
i8752_get_context(void* dst) {
    return i8051_get_context(dst);
}

void
i8752_set_context(void* src) {
    i8051_set_context(src);
}

unsigned
i8752_get_reg(int regnum) {
    return i8051_get_reg(regnum);
}

void
i8752_set_reg(int regnum, unsigned val) {
    i8051_set_reg(regnum, val);
}

void
i8752_set_irq_line(int irqline, int state) {
    i8051_set_irq_line(irqline, state);
}

void
i8752_set_irq_callback(int (*callback)(int irqline)) {
    i8051_set_irq_callback(callback);
}

void
i8752_set_serial_tx_callback(void (*callback)(int data)) {
    i8051_set_serial_tx_callback(callback);
}

void
i8752_set_serial_rx_callback(int (*callback)(void)) {
    i8051_set_serial_rx_callback(callback);
}

void
i8752_state_save(void* file) {
    i8051_state_save(file);
}

void
i8752_state_load(void* file) {
    i8051_state_load(file);
}

const char*
i8752_info(void* context, int regnum) {
    switch (regnum) {
        case CPU_INFO_NAME:
            return "I8752";
    }
...
unsigned
i8752_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
    return Dasm8051(buffer, pc);
#else
    sprintf(buffer, "$%02X", cpu_readop(pc));
