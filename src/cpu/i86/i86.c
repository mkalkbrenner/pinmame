...
static UINT8 i86_reg_layout[] = {I86_AX, I86_BX, I86_DS, I86_ES, I86_SS, I86_FLAGS, I86_CS, I86_VECTOR, 0xFF,
                                 I86_CX, I86_DX, I86_SI, I86_DI, I86_SP, I86_BP,    I86_IP, 0};
...
void
i86_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
        ...
        case I86_PENDING:
            break;
        case I86_NMI_STATE:
            i86_set_irq_line(IRQ_LINE_NMI, val);
            break;
        case I86_IRQ_STATE:
...
void
i86_set_irq_line(int irqline, int state) {
...
void
i86_set_irq_callback(int (*callback)(int)) {
    I.irq_callback = callback;
}

int
i86_execute(int num_cycles) {
    ...
    while (i86_ICount > 0) {
        ...
        logerror("[%04x:%04x]=%02x\tF:%04x\tAX=%04x\tBX=%04x\tCX=%04x\tDX=%04x %d%d%d%d%d%d%d%d%d\n", I.sregs[CS],
                 I.pc - I.base[CS], ReadByte(I.pc), I.flags, I.regs.w[AX], I.regs.w[BX], I.regs.w[CX], I.regs.w[DX],
                 I.AuxVal ? 1 : 0, I.OverVal ? 1 : 0, I.SignVal ? 1 : 0, I.ZeroVal ? 1 : 0, I.CarryVal ? 1 : 0,
                 I.ParityVal ? 1 : 0, I.TF, I.IF, I.DirVal < 0 ? 1 : 0);
#endif
        CALL_MAME_DEBUG;
 ...
 ****************************************************************************/
const char*
i86_info(void* context, int regnum) {
    ...
    switch (regnum) {
        case CPU_INFO_REG + I86_IP:
            sprintf(buffer[which], "IP: %04X", r->pc - r->base[CS]);
            break;
        case CPU_INFO_REG + I86_SP:
            sprintf(buffer[which], "SP: %04X", r->regs.w[SP]);
            break;
        case CPU_INFO_REG + I86_FLAGS:
            sprintf(buffer[which], "F:%04X", r->flags);
            break;
        case CPU_INFO_REG + I86_AX:
            sprintf(buffer[which], "AX:%04X", r->regs.w[AX]);
            break;
        case CPU_INFO_REG + I86_CX:
            sprintf(buffer[which], "CX:%04X", r->regs.w[CX]);
            break;
        case CPU_INFO_REG + I86_DX:
            sprintf(buffer[which], "DX:%04X", r->regs.w[DX]);
            break;
        case CPU_INFO_REG + I86_BX:
            sprintf(buffer[which], "BX:%04X", r->regs.w[BX]);
            break;
        case CPU_INFO_REG + I86_BP:
            sprintf(buffer[which], "BP:%04X", r->regs.w[BP]);
            break;
        case CPU_INFO_REG + I86_SI:
            sprintf(buffer[which], "SI: %04X", r->regs.w[SI]);
            break;
        case CPU_INFO_REG + I86_DI:
            sprintf(buffer[which], "DI: %04X", r->regs.w[DI]);
            break;
        case CPU_INFO_REG + I86_ES:
            sprintf(buffer[which], "ES:%04X", r->sregs[ES]);
            break;
        case CPU_INFO_REG + I86_CS:
            sprintf(buffer[which], "CS:%04X", r->sregs[CS]);
            break;
        case CPU_INFO_REG + I86_SS:
            sprintf(buffer[which], "SS:%04X", r->sregs[SS]);
            break;
        case CPU_INFO_REG + I86_DS:
            sprintf(buffer[which], "DS:%04X", r->sregs[DS]);
            break;
        case CPU_INFO_REG + I86_VECTOR:
            sprintf(buffer[which], "V:%02X", r->int_vector);
            break;
        case CPU_INFO_REG + I86_PENDING:
            sprintf(buffer[which], "P:%X", r->irq_state);
            break;
        case CPU_INFO_REG + I86_NMI_STATE:
            sprintf(buffer[which], "NMI:%X", r->nmi_state);
            break;
        case CPU_INFO_REG + I86_IRQ_STATE:
            sprintf(buffer[which], "IRQ:%X", r->irq_state);
            break;
        case CPU_INFO_FLAGS:
            r->flags = CompressFlags();
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", (r->flags & 0x8000) ? '?' : '.',
                    (r->flags & 0x4000) ? '?' : '.', (r->flags & 0x2000) ? '?' : '.', (r->flags & 0x1000) ? '?' : '.',
                    (r->flags & 0x0800) ? 'O' : '.', (r->flags & 0x0400) ? 'D' : '.', (r->flags & 0x0200) ? 'I' : '.',
                    (r->flags & 0x0100) ? 'T' : '.', (r->flags & 0x0080) ? 'S' : '.', (r->flags & 0x0040) ? 'Z' : '.',
                    (r->flags & 0x0020) ? '?' : '.', (r->flags & 0x0010) ? 'A' : '.', (r->flags & 0x0008) ? '?' : '.',
                    (r->flags & 0x0004) ? 'P' : '.', (r->flags & 0x0002) ? 'N' : '.', (r->flags & 0x0001) ? 'C' : '.');
            break;
        case CPU_INFO_NAME:
            return "I8086";
        ...
        case CPU_INFO_REG_LAYOUT:
            return (const char*)i86_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)i86_win_layout;
    }
    return buffer[which];
}

unsigned
i86_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
 ...
 ****************************************************************************/
const char*
i88_info(void* context, int regnum) {
    switch (regnum) {
        case CPU_INFO_NAME:
            return "I8088";
    }
...
i186_execute(int num_cycles) {
    ...
    while (i86_ICount > 0) {
#ifdef VERBOSE_DEBUG
        printf("[%04x:%04x]=%02x\tAX=%04x\tBX=%04x\tCX=%04x\tDX=%04x\n", I.sregs[CS], I.pc, ReadByte(I.pc),
               I.regs.w[AX], I.regs.w[BX], I.regs.w[CX], I.regs.w[DX]);
#endif
        CALL_MAME_DEBUG;
 ...
 ****************************************************************************/
const char*
i188_info(void* context, int regnum) {
    switch (regnum) {
        case CPU_INFO_NAME:
            return "I80188";
    }
 ...
 ****************************************************************************/
const char*
i186_info(void* context, int regnum) {
    switch (regnum) {
        case CPU_INFO_NAME:
            return "I80186";
    }
...
unsigned
i186_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
...
static void
v30_interrupt(unsigned int_num, BOOLEAN md_flag) {
...
static void
v30_reset(void* param) {
    i86_reset(param);
    SetMD(1);
}

static int
v30_execute(int num_cycles) {
    /* copy over the cycle counts if they're not correct */
    if (cycles.id != 30)
        cycles = v30_cycles;
 ...
 ****************************************************************************/
const char*
v30_info(void* context, int regnum) {
    ...
    switch (regnum) {
        ...
        case CPU_INFO_FLAGS:
            I.flags = CompressFlags();
            sprintf(buffer, "%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c%c", (I.flags & 0x8000) ? 'M' : '.',
                    (I.flags & 0x4000) ? '?' : '.', (I.flags & 0x2000) ? '?' : '.', (I.flags & 0x1000) ? '?' : '.',
                    (I.flags & 0x0800) ? 'O' : '.', (I.flags & 0x0400) ? 'D' : '.', (I.flags & 0x0200) ? 'I' : '.',
                    (I.flags & 0x0100) ? 'T' : '.', (I.flags & 0x0080) ? 'S' : '.', (I.flags & 0x0040) ? 'Z' : '.',
                    (I.flags & 0x0020) ? '?' : '.', (I.flags & 0x0010) ? 'A' : '.', (I.flags & 0x0008) ? '?' : '.',
                    (I.flags & 0x0004) ? 'P' : '.', (I.flags & 0x0002) ? 'N' : '.', (I.flags & 0x0001) ? 'C' : '.');
            break;
    }
    return i86_info(context, regnum);
}
 ...
 ****************************************************************************/
const char*
v20_info(void* context, int regnum) {
    switch (regnum) {
        case CPU_INFO_NAME:
            return "V20";
    }
 ...
 ****************************************************************************/
const char*
v33_info(void* context, int regnum) {
    switch (regnum) {
        case CPU_INFO_NAME:
            return "V33";
    }
...
unsigned
v30_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
