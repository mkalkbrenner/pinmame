...
#define CHECK_IRQ_LINES()                                                                                              \
    {                                                                                                                  \
        if (!(CC & 0x10)) {                                                                                            \
            if (m6800.irq_state[M6800_IRQ_LINE] != CLEAR_LINE) { /* standard IRQ */                                    \
...
#define EAT_CYCLES                                                                                                     \
    {                                                                                                                  \
        ...
        if (cycles_to_eat > m6800_ICount)                                                                              \
            cycles_to_eat = m6800_ICount;                                                                              \
        if (cycles_to_eat > 0) {                                                                                       \
...
/* IRQ enter */
static void
ENTER_INTERRUPT(const char* message, UINT16 irq_vector) {
 ...
 ****************************************************************************/
static void
state_register(const char* type) {
 ...
 ****************************************************************************/
void
m6800_set_reg(int regnum, unsigned val) {
    switch (regnum) {
        case REG_PC:
            PC = val;
            CHANGE_PC();
        ...
        case M6800_X:
            ...
            break;
        case M6800_NMI_STATE:
            m6800_set_irq_line(IRQ_LINE_NMI, val);
            break;
        case M6800_IRQ_STATE:
...
void
m6800_set_irq_line(int irqline, int state) {
...
void
m6800_set_irq_callback(int (*callback)(int irqline)) {
 ...
 ****************************************************************************/
const char*
m6800_info(void* context, int regnum) {
    /* Layout of the registers in the debugger */
    static UINT8 m6800_reg_layout[] = {M6800_PC, M6800_S,         M6800_CC,        M6800_A,         M6800_B, M6800_X,
                                       -1,       M6800_WAI_STATE, M6800_NMI_STATE, M6800_IRQ_STATE, 0};
    ...
    switch (regnum) {
        case CPU_INFO_REG + M6800_A:
            sprintf(buffer[which], "A:%02X", r->d.b.h);
            break;
        case CPU_INFO_REG + M6800_B:
            sprintf(buffer[which], "B:%02X", r->d.b.l);
            break;
        case CPU_INFO_REG + M6800_PC:
            sprintf(buffer[which], "PC:%04X", r->pc.w.l);
            break;
        case CPU_INFO_REG + M6800_S:
            sprintf(buffer[which], "S:%04X", r->s.w.l);
            break;
        case CPU_INFO_REG + M6800_X:
            sprintf(buffer[which], "X:%04X", r->x.w.l);
            break;
        case CPU_INFO_REG + M6800_CC:
            sprintf(buffer[which], "CC:%02X", r->cc);
            break;
        case CPU_INFO_REG + M6800_NMI_STATE:
            sprintf(buffer[which], "NMI:%X", r->nmi_state);
            break;
        case CPU_INFO_REG + M6800_IRQ_STATE:
            sprintf(buffer[which], "IRQ:%X", r->irq_state[M6800_IRQ_LINE]);
            break;
            //		case CPU_INFO_REG+M6800_TIN_STATE: sprintf(buffer[which], "TIN:%X", r->irq_state[M6800_TIN_LINE]); break;
        case CPU_INFO_FLAGS:
            sprintf(buffer[which], "%c%c%c%c%c%c%c%c", (r->cc & 0x80) ? '?' : '.', (r->cc & 0x40) ? '?' : '.',
                    (r->cc & 0x20) ? 'H' : '.', (r->cc & 0x10) ? 'I' : '.', (r->cc & 0x08) ? 'N' : '.',
                    (r->cc & 0x04) ? 'Z' : '.', (r->cc & 0x02) ? 'V' : '.', (r->cc & 0x01) ? 'C' : '.');
            break;
        case CPU_INFO_NAME:
            return "M6800";
        ...
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m6800_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
        case 6800:
            return (const char*)m6800_win_layout;
    }
    return buffer[which];
}

unsigned
m6800_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
...
int
m6801_execute(int cycles) {
    return m6803_execute(cycles);
}

unsigned
m6801_get_context(void* dst) {
    return m6800_get_context(dst);
}

void
m6801_set_context(void* src) {
    m6800_set_context(src);
}

unsigned
m6801_get_reg(int regnum) {
    return m6800_get_reg(regnum);
}

void
m6801_set_reg(int regnum, unsigned val) {
    m6800_set_reg(regnum, val);
}

void
m6801_set_irq_line(int irqline, int state) {
    m6800_set_irq_line(irqline, state);
}

void
m6801_set_irq_callback(int (*callback)(int irqline)) {
    m6800_set_irq_callback(callback);
}

const char*
m6801_info(void* context, int regnum) {
    /* Layout of the registers in the debugger */
    static UINT8 m6801_reg_layout[] = {M6801_PC, M6801_S,         M6801_CC,        M6801_A,         M6801_B, M6801_X,
                                       -1,       M6801_WAI_STATE, M6801_NMI_STATE, M6801_IRQ_STATE, 0};
    ...
    switch (regnum) {
        case CPU_INFO_NAME:
            return "M6801";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m6801_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m6801_win_layout;
    }
    return m6800_info(context, regnum);
}

unsigned
m6801_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
...
int
m6802_execute(int cycles) {
    return m6800_execute(cycles);
}

unsigned
m6802_get_context(void* dst) {
    return m6800_get_context(dst);
}

void
m6802_set_context(void* src) {
    m6800_set_context(src);
}

unsigned
m6802_get_reg(int regnum) {
    return m6800_get_reg(regnum);
}

void
m6802_set_reg(int regnum, unsigned val) {
    m6800_set_reg(regnum, val);
}

void
m6802_set_irq_line(int irqline, int state) {
    m6800_set_irq_line(irqline, state);
}

void
m6802_set_irq_callback(int (*callback)(int irqline)) {
    m6800_set_irq_callback(callback);
}

const char*
m6802_info(void* context, int regnum) {
    /* Layout of the registers in the debugger */
    static UINT8 m6802_reg_layout[] = {M6802_PC, M6802_S,         M6802_CC,        M6802_A,         M6802_B, M6802_X,
                                       -1,       M6802_WAI_STATE, M6802_NMI_STATE, M6802_IRQ_STATE, 0};
    ...
    switch (regnum) {
        case CPU_INFO_NAME:
            return "M6802";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m6802_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m6802_win_layout;
    }
    return m6800_info(context, regnum);
}

unsigned
m6802_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
        ...
        if (m6803.wai_state & M6800_WAI) {
            ...
            switch (ireg) {
                ...
                case 0xcc:
                    ...
                    break;
                case 0xcd:
                    std_im(); /* 6803 only -- orthogonality */
                    break;
                case 0xce:
                    ldx_im();
                    break;
...
#if (HAS_M6803)
unsigned
m6803_get_context(void* dst) {
    return m6800_get_context(dst);
}

void
m6803_set_context(void* src) {
    m6800_set_context(src);
}

unsigned
m6803_get_reg(int regnum) {
    return m6800_get_reg(regnum);
}

void
m6803_set_reg(int regnum, unsigned val) {
    m6800_set_reg(regnum, val);
}

void
m6803_set_irq_line(int irqline, int state) {
    m6800_set_irq_line(irqline, state);
}

void
m6803_set_irq_callback(int (*callback)(int irqline)) {
    m6800_set_irq_callback(callback);
}

const char*
m6803_info(void* context, int regnum) {
    /* Layout of the registers in the debugger */
    static UINT8 m6803_reg_layout[] = {M6803_PC, M6803_S,         M6803_CC,        M6803_A,         M6803_B, M6803_X,
                                       -1,       M6803_WAI_STATE, M6803_NMI_STATE, M6803_IRQ_STATE, 0};
    ...
    switch (regnum) {
        case CPU_INFO_NAME:
            return "M6803";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m6803_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m6803_win_layout;
    }
    return m6800_info(context, regnum);
}

unsigned
m6803_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
...
int
m6808_execute(int cycles) {
    return m6800_execute(cycles);
}

unsigned
m6808_get_context(void* dst) {
    return m6800_get_context(dst);
}

void
m6808_set_context(void* src) {
    m6800_set_context(src);
}

unsigned
m6808_get_reg(int regnum) {
    return m6800_get_reg(regnum);
}

void
m6808_set_reg(int regnum, unsigned val) {
    m6800_set_reg(regnum, val);
}

void
m6808_set_irq_line(int irqline, int state) {
    m6800_set_irq_line(irqline, state);
}

void
m6808_set_irq_callback(int (*callback)(int irqline)) {
    m6800_set_irq_callback(callback);
}

const char*
m6808_info(void* context, int regnum) {
    /* Layout of the registers in the debugger */
    static UINT8 m6808_reg_layout[] = {M6808_PC, M6808_S,         M6808_CC,        M6808_A,         M6808_B, M6808_X,
                                       -1,       M6808_WAI_STATE, M6808_NMI_STATE, M6808_IRQ_STATE, 0};
    ...
    switch (regnum) {
        case CPU_INFO_NAME:
            return "M6808";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)m6808_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)m6808_win_layout;
    }
    return m6800_info(context, regnum);
}

unsigned
m6808_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
...
unsigned
hd63701_get_context(void* dst) {
    return m6800_get_context(dst);
}

void
hd63701_set_context(void* src) {
    m6800_set_context(src);
}

unsigned
hd63701_get_reg(int regnum) {
    return m6800_get_reg(regnum);
}

void
hd63701_set_reg(int regnum, unsigned val) {
    m6800_set_reg(regnum, val);
}

void
hd63701_set_irq_line(int irqline, int state) {
    m6800_set_irq_line(irqline, state);
}

void
hd63701_set_irq_callback(int (*callback)(int irqline)) {
    m6800_set_irq_callback(callback);
}

const char*
hd63701_info(void* context, int regnum) {
    /* Layout of the registers in the debugger */
    static UINT8 hd63701_reg_layout[] = {HD63701_PC,        HD63701_S,         HD63701_CC, HD63701_A,
                                         HD63701_B,         HD63701_X,         -1,         HD63701_WAI_STATE,
                                         HD63701_NMI_STATE, HD63701_IRQ_STATE, 0};
    ...
    switch (regnum) {
        case CPU_INFO_NAME:
            return "HD63701";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)hd63701_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)hd63701_win_layout;
    }
    return m6800_info(context, regnum);
}
...
*/
void
hd63701_trap_pc(void) {
    TAKE_TRAP;
}

READ_HANDLER(hd63701_internal_registers_r) { return m6803_internal_registers_r(offset); }

WRITE_HANDLER(hd63701_internal_registers_w) { m6803_internal_registers_w(offset, data); }

unsigned
hd63701_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
...
unsigned
nsc8105_get_context(void* dst) {
    return m6800_get_context(dst);
}

void
nsc8105_set_context(void* src) {
    m6800_set_context(src);
}

unsigned
nsc8105_get_reg(int regnum) {
    return m6800_get_reg(regnum);
}

void
nsc8105_set_reg(int regnum, unsigned val) {
    m6800_set_reg(regnum, val);
}

void
nsc8105_set_irq_line(int irqline, int state) {
    m6800_set_irq_line(irqline, state);
}

void
nsc8105_set_irq_callback(int (*callback)(int irqline)) {
    m6800_set_irq_callback(callback);
}

const char*
nsc8105_info(void* context, int regnum) {
    /* Layout of the registers in the debugger */
    static UINT8 nsc8105_reg_layout[] = {NSC8105_PC,        NSC8105_S,         NSC8105_CC, NSC8105_A,
                                         NSC8105_B,         NSC8105_X,         -1,         NSC8105_WAI_STATE,
                                         NSC8105_NMI_STATE, NSC8105_IRQ_STATE, 0};
    ...
    switch (regnum) {
        case CPU_INFO_NAME:
            return "NSC8105";
        case CPU_INFO_REG_LAYOUT:
            return (const char*)nsc8105_reg_layout;
        case CPU_INFO_WIN_LAYOUT:
            return (const char*)nsc8105_win_layout;
    }
    return m6800_info(context, regnum);
}

unsigned
nsc8105_dasm(char* buffer, unsigned pc) {
#ifdef MAME_DEBUG
    ...
    switch (offset) {
        ...
        case 0x02:
            if (m6800.port1_ddr == 0xff)
                return m6800.port1_data;
            return (cpu_readport16(M6803_PORT1) & (m6800.port1_ddr ^ 0xff)) | (m6800.port1_data & m6800.port1_ddr);
        case 0x03:
            if ((m6800.port2_ddr & 0x1f) == 0x1f)
                return m6800.port2_data;
            return (cpu_readport16(M6803_PORT2) & (m6800.port2_ddr ^ 0xff)) | (m6800.port2_data & m6800.port2_ddr);
        case 0x04:
        ...
        case 0x06:
            if (m6800.port3_ddr == 0xff)
                return m6800.port3_data;
            return (cpu_readport16(M6803_PORT3) & (m6800.port3_ddr ^ 0xff)) | (m6800.port3_data & m6800.port3_ddr);
        case 0x07:
            if (m6800.port4_ddr == 0xff)
                return m6800.port4_data;
            return (cpu_readport16(M6803_PORT4) & (m6800.port4_ddr ^ 0xff)) | (m6800.port4_data & m6800.port4_ddr);
        case 0x08:
...
WRITE_HANDLER(m6803_internal_registers_w) {
    ...
    switch (offset) {
        ...
        case 0x03:
#ifdef PINMAME
            m6800.port2_data = (m6800.port2_data & m6800.port2_ddr & 0x02) | (data & m6800.port2_ddr & 0x1d)
                               | (cpu_readport16(M6803_PORT2) & ~m6800.port2_ddr & 0x1d);
