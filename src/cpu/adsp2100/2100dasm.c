...
static const char* condition[] = {"IF EQ ",     "IF NE ", "IF GT ",     "IF LE ",  "IF LT ",  "IF GE ", "IF AV ",
                                  "IF NOT AV ", "IF AC ", "IF NOT AC ", "IF NEG ", "IF POS ", "IF MV ", "IF NOT MV ",
                                  "IF NOT CE ", ""};

static const char* do_condition[] = {"NE",     "EQ", "LE",  "GT",  "GE",     "LT", "NOT AV", "AV",
...
static const char* shift_by_op[] = {"SR = LSHIFT %s BY %d (HI)",
...
int
alumac(char* buffer, int dest, int op) {
...
unsigned
dasm2100(char* buffer, unsigned pc) {
    ...
    switch ((op >> 16) & 0xff) {
        ...
        case 0x04:
            ...
            if ((op & 0x00ffe0) == 0x000000) {
                ...
                if (op & 0x000008)
                    buffer += sprintf(buffer, "%s", "POP LOOP ");
                if (op & 0x000004)
                    buffer += sprintf(buffer, "%s", "POP CNTR ");
                if ((op & 0x000003) == 0x000002)
                    buffer += sprintf(buffer, "%s", "PUSH STAT ");
                else if ((op & 0x000003) == 0x000003)
                    buffer += sprintf(buffer, "%s", "POP STAT ");
            } else
                buffer += sprintf(buffer, "??? (%06X)", op);
            break;
        case 0x05:
        ...
        case 0x0d:
            ...
            if ((op & 0x00f000) == 0x000000)
                buffer += sprintf(buffer, "%s = %s", reg_grp[(op >> 10) & 3][(op >> 4) & 15],
                                  reg_grp[(op >> 8) & 3][op & 15]);
            else
                buffer += sprintf(buffer, "??? (%06X)", op);
            break;
        case 0x0e:
        ...
        case 0x11:
            ...
            buffer += sprintf(buffer, shift_op[(op >> 11) & 15], shift_xop[(op >> 8) & 7]);
            if (op & 0x008000)
                buffer += sprintf(buffer, ", PM(I%d,M%d) = %s", 4 + ((op >> 2) & 3), 4 + (op & 3),
                                  reg_grp[0][(op >> 4) & 15]);
            else
                buffer += sprintf(buffer, ", %s = PM(I%d,M%d)", reg_grp[0][(op >> 4) & 15], 4 + ((op >> 2) & 3),
                                  4 + (op & 3));
            break;
        case 0x12:
        case 0x13:
            ...
            temp = (op >> 14) & 4;
            if (op & 0x008000)
                buffer += sprintf(buffer, ", DM(I%d,M%d) = %s", temp + ((op >> 2) & 3), temp + (op & 3),
                                  reg_grp[0][(op >> 4) & 15]);
            else
                buffer += sprintf(buffer, ", %s = DM(I%d,M%d)", reg_grp[0][(op >> 4) & 15], temp + ((op >> 2) & 3),
                                  temp + (op & 3));
            break;
        case 0x14:
        case 0x15:
        ...
        case 0x5f:
            ...
            if (op & 0x080000)
                buffer +=
                    sprintf(buffer, "PM(I%d,M%d) = %s", 4 + ((op >> 2) & 3), 4 + (op & 3), reg_grp[0][(op >> 4) & 15]);
            else
                buffer +=
                    sprintf(buffer, "%s = PM(I%d,M%d)", reg_grp[0][(op >> 4) & 15], 4 + ((op >> 2) & 3), 4 + (op & 3));
            break;
        case 0x60:
        case 0x61:
        case 0x62:
        ...
        case 0xbf:
            ...
            temp = (op >> 18) & 4;
            buffer +=
                sprintf(buffer, "DM(I%d,M%d) = $%04X", temp + ((op >> 2) & 3), temp + (op & 3), (op >> 4) & 0xffff);
            break;
        case 0xc0:
        case 0xc1:
        case 0xc2:
        ...
        case 0xff:
            ...
            buffer += sprintf(buffer, "%s = DM(I%d,M%d), %s = PM(I%d,M%d)", dual_xreg[(op >> 18) & 3], (op >> 2) & 3,
                              op & 3, dual_yreg[(op >> 20) & 3], 4 + ((op >> 6) & 3), 4 + ((op >> 4) & 3));
            break;
    }
