...
static struct bank_data bankdata[MAX_BANKS + 1];
    /* data gathered for each bank */ // +1 because various checks allow to set/check MAX_BANKS
...
static void set_static_handler(int idx, read8_handler r8handler, read16_handler r16handler, read32_handler r32handler,
...
void
memory_set_opcode_base(int cpunum, void* base) {
...
void
memory_set_encrypted_opcode_range(int cpunum, offs_t min_address, offs_t max_address) {
...
void
memory_set_context(int activecpu) {
...
void
memory_set_unmap_value(data32_t value) {
    unmap_value = value;
}
...
void
memory_set_bankhandler_w(int bank, offs_t offset, mem_write_handler handler) {
    /* determine the new offset */
...
opbase_handler
memory_set_opbase_handler(int cpunum, opbase_handler function) {
...
data8_t*
install_mem_read_handler(int cpunum, offs_t start, offs_t end, mem_read_handler handler) {
    /* sanity check */
...
data16_t*
install_mem_read16_handler(int cpunum, offs_t start, offs_t end, mem_read16_handler handler) {
    /* sanity check */
...
data32_t*
install_mem_read32_handler(int cpunum, offs_t start, offs_t end, mem_read32_handler handler) {
    /* sanity check */
...
data8_t*
install_mem_write_handler(int cpunum, offs_t start, offs_t end, mem_write_handler handler) {
    /* sanity check */
    if (cpudata[cpunum].mem.dbits != 8) {
...
data16_t*
install_mem_write16_handler(int cpunum, offs_t start, offs_t end, mem_write16_handler handler) {
    /* sanity check */
    if (cpudata[cpunum].mem.dbits != 16) {
...
data32_t*
install_mem_write32_handler(int cpunum, offs_t start, offs_t end, mem_write32_handler handler) {
    /* sanity check */
    if (cpudata[cpunum].mem.dbits != 32) {
...
void
install_port_read_handler(int cpunum, offs_t start, offs_t end, port_read_handler handler) {
    /* sanity check */
...
void
install_port_read16_handler(int cpunum, offs_t start, offs_t end, port_read16_handler handler) {
    /* sanity check */
...
void
install_port_read32_handler(int cpunum, offs_t start, offs_t end, port_read32_handler handler) {
    /* sanity check */
...
void
install_port_write_handler(int cpunum, offs_t start, offs_t end, port_write_handler handler) {
    /* sanity check */
    if (cpudata[cpunum].port.dbits != 8) {
...
void
install_port_write16_handler(int cpunum, offs_t start, offs_t end, port_write16_handler handler) {
    /* sanity check */
    if (cpudata[cpunum].port.dbits != 16) {
...
void
install_port_write32_handler(int cpunum, offs_t start, offs_t end, port_write32_handler handler) {
    /* sanity check */
    if (cpudata[cpunum].port.dbits != 32) {
...
void*
memory_find_base(int cpunum, offs_t offset) {
    int ext_entry;
...
void*
memory_get_read_ptr(int cpunum, offs_t offset) {
    struct memport_data* memport = &cpudata[cpunum].mem;
    struct handler_data* handlist = (memport->dbits == 32)   ? rmemhandler32
                                    : (memport->dbits == 16) ? rmemhandler16
                                                             : rmemhandler8;
...
void*
memory_get_write_ptr(int cpunum, offs_t offset) {
    struct memport_data* memport = &cpudata[cpunum].mem;
    struct handler_data* handlist = (memport->dbits == 32)   ? wmemhandler32
                                    : (memport->dbits == 16) ? wmemhandler16
                                                             : wmemhandler8;
    UINT8 minbits = memport->abits - memport->ebits;
    UINT8 entry;
...
UINT8
get_handler_index(struct handler_data* table, genf* handler, offs_t start) {
    int i;
...
UINT8
alloc_new_subtable(const struct memport_data* memport, struct table_data* tabledata, UINT8 previous_value) {
...
void
populate_table(struct memport_data* memport, int iswrite, offs_t start, offs_t stop, UINT8 handler) {
...
genf*
assign_dynamic_bank(int cpunum, offs_t start) {
    size_t bank;
...
void
install_mem_handler(struct memport_data* memport, int iswrite, offs_t start, offs_t end, genf* handler) {
    struct table_data* tabledata = iswrite ? &memport->write : &memport->read;
    UINT8 idx;
...
void
install_port_handler(struct memport_data* memport, int iswrite, offs_t start, offs_t end, genf* handler) {
    struct table_data* tabledata = iswrite ? &memport->write : &memport->read;
...
static void
set_static_handler(int idx, read8_handler r8handler, read16_handler r16handler, read32_handler r32handler,
                   write8_handler w8handler, write16_handler w16handler, write32_handler w32handler) {
...
init_cpudata(void) {
    ...
    for (cpunum = 0; cpunum < cpu_gettotalcpu(); cpunum++) {
        ...
        if (!init_memport(cpunum, &cpudata[cpunum].mem, mem_address_bits_of_cpu(cputype), cpunum_databus_width(cpunum),
                          1))
            return 0;

        /* initialize the readport and writeport tables */
        if (!init_memport(cpunum, &cpudata[cpunum].port, port_address_bits_of_cpu(cputype),
                          cpunum_databus_width(cpunum), 0))
            return 0;
...
static int
init_memport(int cpunum, struct memport_data* data, int abits, int dbits, int ismemory) {
...
static int
needs_ram(int cpunum, genf* handler) {
        ...
        while (1) {
            ...
            for (mwa = Machine->drv->cpu[cpunum].memory_write; !IS_MEMPORT_END(mwa); mwa++)
                if (!IS_MEMPORT_MARKER(mwa))
                    if (mwa->end >= size && mwa->start < lowest
                        && (mwa->base || needs_ram(cpunum, (genf*)mwa->handler)))
                        lowest = mwa->start;
            ...
            while (end != lastend) {
                ...
                for (mwa = Machine->drv->cpu[cpunum].memory_write; !IS_MEMPORT_END(mwa); mwa++)
                    if (!IS_MEMPORT_MARKER(mwa))
                        if (mwa->start <= end + 1 && mwa->end > end
                            && (mwa->base || needs_ram(cpunum, (genf*)mwa->handler)))
                            end = mwa->end;
            }
        ...
        if (mwa_start) {
            ...
            for (mwa--; mwa >= mwa_start; mwa--)
                if (!IS_MEMPORT_MARKER(mwa)) {
                    ...
                    if (mwa->base)
                        *mwa->base = memory_find_base(cpunum, mwa->start);
                    if (mwa->size)
                        *mwa->size = mwa->end - mwa->start + 1;
                }
        }
    }
    return 1;
...
static void
rg_add_entry(UINT32 start, UINT32 end, int mode) {
    rg_map_entry** cur;
    cur = &rg_map;
...
static void
register_zone(int cpunum, UINT32 start, UINT32 end) {
    char name[256];
...
#define SETOPBASE(name, abits, minbits, table)                                                                         \
    void name(offs_t pc) {                                                                                             \
        ...
        OP_MEM_MAX = (entry >= STATIC_RAM && entry <= STATIC_RAMROM) ? cpudata[cpu_getactivecpu()].ramlength - 1       \
                                                                     : table[entry].top;                               \
    }
...
#define GENERATE_PORT_HANDLERS_8BIT(abits)     GENERATE_HANDLERS_8BIT(port, abits)

#define GENERATE_PORT_HANDLERS_16BIT_BE(abits) GENERATE_HANDLERS_16BIT_BE(port, abits)

#define GENERATE_PORT_HANDLERS_16BIT_LE(abits) GENERATE_HANDLERS_16BIT_LE(port, abits)

#define GENERATE_PORT_HANDLERS_32BIT_BE(abits) GENERATE_HANDLERS_32BIT_BE(port, abits)

#define GENERATE_PORT_HANDLERS_32BIT_LE(abits) GENERATE_HANDLERS_32BIT_LE(port, abits)
...
static const struct memory_address_table readmem_to_bits[] = {
    {16, cpu_readmem16},     {20, cpu_readmem20},     {21, cpu_readmem21},     {24, cpu_readmem24},

    {16, cpu_readmem16bew},  {18, cpu_readmem18bew},  {24, cpu_readmem24bew},  {32, cpu_readmem32bew},

    {16, cpu_readmem16lew},  {17, cpu_readmem17lew},  {24, cpu_readmem24lew},  {29, cpu_readmem29lew},
    {32, cpu_readmem32lew},

    {24, cpu_readmem24bedw}, {29, cpu_readmem29bedw}, {32, cpu_readmem32bedw},

    {24, cpu_readmem24ledw}, {26, cpu_readmem26ledw}, {29, cpu_readmem29ledw}, {32, cpu_readmem32ledw},
...
int
mem_address_bits_of_cpu(int cputype) {
...
int
port_address_bits_of_cpu(int cputype) {
#if (HAS_V60)
...
static WRITE_HANDLER(mwh8_bad) {
    ...
    if (activecpu_address_bits() <= SPARSE_THRESH)
        cpu_bankbase[STATIC_RAM][offset] = data;
}
...
static WRITE16_HANDLER(mwh16_bad) {
    ...
    if (activecpu_address_bits() <= SPARSE_THRESH)
        COMBINE_DATA(&((data16_t*)cpu_bankbase[STATIC_RAM])[offset]);
}

static WRITE32_HANDLER(mwh32_bad) {
    logerror("cpu #%d (PC=%08X): unmapped memory dword write to %08X = %08X & %08X\n", cpu_getactivecpu(),
             activecpu_get_pc(), offset * 4, data, mem_mask ^ 0xffffffff);
    if (activecpu_address_bits() <= SPARSE_THRESH)
        COMBINE_DATA(&((data32_t*)cpu_bankbase[STATIC_RAM])[offset]);
}
...
static WRITE16_HANDLER(mwh16_rom) {
    logerror("cpu #%d (PC=%08X): word write to %08X = %04X & %04X\n", cpu_getactivecpu(), activecpu_get_pc(),
             offset * 2, data, mem_mask ^ 0xffff);
}

static WRITE32_HANDLER(mwh32_rom) {
    logerror("cpu #%d (PC=%08X): dword write to %08X = %08X & %08X\n", cpu_getactivecpu(), activecpu_get_pc(),
             offset * 4, data, mem_mask ^ 0xffffffff);
}
...
static WRITE_HANDLER(mwh8_ramrom) {
    cpu_bankbase[STATIC_RAM][offset] = cpu_bankbase[STATIC_RAM][offset + (OP_ROM - OP_RAM)] = data;
}

static WRITE16_HANDLER(mwh16_ramrom) {
    COMBINE_DATA(&cpu_bankbase[STATIC_RAM][offset * 2]);
    COMBINE_DATA(&cpu_bankbase[0][offset * 2 + (OP_ROM - OP_RAM)]);
}

static WRITE32_HANDLER(mwh32_ramrom) {
    COMBINE_DATA(&cpu_bankbase[STATIC_RAM][offset * 4]);
    COMBINE_DATA(&cpu_bankbase[0][offset * 4 + (OP_ROM - OP_RAM)]);
}
...
static void
dump_map(FILE* file, const struct memport_data* memport, const struct table_data* table) {
    static const char* strings[] = {"invalid", "bank 1",   "bank 2",   "bank 3",  "bank 4",  "bank 5",  "bank 6",
                                    "bank 7",  "bank 8",   "bank 9",   "bank 10", "bank 11", "bank 12", "bank 13",
                                    "bank 14", "bank 15",  "bank 16",  "bank 17", "bank 18", "bank 19", "bank 20",
                                    "bank 21", "bank 22",  "bank 23",  "bank 24", "RAM",     "ROM",     "RAMROM",
    ...
    for (i = 0; i < l1count; i++) {
        ...
        if (entry != STATIC_UNMAP) {
            fprintf(file, "%05X  %08X-%08X    = %02X: ", i, i << (l2bits + minbits),
            ...
            if (entry < STATIC_COUNT)
                ...
                fprintf(file, "handler(%08X) [offset=%08X]\n", (UINT32)table->handlers[entry].handler,
                        table->handlers[entry].offset);
            else {
                ...
                for (j = 0; j < l2count; j++) {
                    ...
                    if (entry2 != STATIC_UNMAP) {
                        fprintf(file, "   %05X  %08X-%08X = %02X: ", j, (i << (l2bits + minbits)) | (j << minbits),
                        ...
                        if (entry2 < STATIC_COUNT)
                            ...
                            fprintf(file, "handler(%08X) [offset=%08X]\n", (UINT32)table->handlers[entry2].handler,
                                    table->handlers[entry2].offset);
                        else
...
static int
track_entry(void* entry, void* handler) {
    int i;
                                    ...
                                    if (size != 1) {
                                        if (!track_entry((void*)mra, (void*)mra->handler))
                                            printf("%s: %s cpu %d readmem inval size  { %08X, %08X }\n",
                                                   drivers[i]->source_file, drivers[i]->name, cpunum, mra->start,
                                                   mra->end);
                                    }
                                }
                                size--;
                                    ...
                                    if (size != 1) {
                                        if (!track_entry((void*)mwa, (void*)mwa->handler))
                                            printf("%s: %s cpu %d writemem inval size  { %08X, %08X }\n",
                                                   drivers[i]->source_file, drivers[i]->name, cpunum, mwa->start,
                                                   mwa->end);
                                    }
                                }
                                size--;
                                ...
                                if (size) {
                                    ...
                                    if (size != 1) {
                                        if (!track_entry((void*)iora, (void*)iora->handler))
                                            printf("%s: %s cpu %d readmem inval size  { %08X, %08X }\n",
                                                   drivers[i]->source_file, drivers[i]->name, cpunum, iora->start,
                                                   iora->end);
                                    }
                                }
                                size--;
                                ...
                                if (size) {
                                    ...
                                    if (size != 1) {
                                        if (!track_entry((void*)iowa, (void*)iowa->handler))
                                            printf("%s: %s cpu %d writemem inval size  { %08X, %08X }\n",
                                                   drivers[i]->source_file, drivers[i]->name, cpunum, iowa->start,
                                                   iowa->end);
                                    }
                                }
                                size--;
