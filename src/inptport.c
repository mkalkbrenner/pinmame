...
const char ipdn_defaultstrings[][MAX_DEFSTR_LEN] = {
    "Off", "On", "No", "Yes", "Lives", "Bonus Life", "Difficulty", "Demo Sounds", "Coinage", "Coin A", "Coin B",
    "9 Coins/1 Credit", "8 Coins/1 Credit", "7 Coins/1 Credit", "6 Coins/1 Credit", "5 Coins/1 Credit",
    "4 Coins/1 Credit", "3 Coins/1 Credit", "8 Coins/3 Credits", "4 Coins/2 Credits", "2 Coins/1 Credit",
    "5 Coins/3 Credits", "3 Coins/2 Credits", "4 Coins/3 Credits", "4 Coins/4 Credits", "3 Coins/3 Credits",
    "2 Coins/2 Credits", "1 Coin/1 Credit", "4 Coins/5 Credits", "3 Coins/4 Credits", "2 Coins/3 Credits",
    "4 Coins/7 Credits", "2 Coins/4 Credits", "1 Coin/2 Credits", "2 Coins/5 Credits", "2 Coins/6 Credits",
    "1 Coin/3 Credits", "2 Coins/7 Credits", "2 Coins/8 Credits", "1 Coin/4 Credits", "1 Coin/5 Credits",
    "1 Coin/6 Credits", "1 Coin/7 Credits", "1 Coin/8 Credits", "1 Coin/9 Credits", "Free Play", "Cabinet", "Upright",
    "Cocktail", "Flip Screen", "Service Mode",
    /*"Pause",
	"Test",
	"Tilt",
	"Version",
...
struct ipd inputport_defaults[] = {
    ...
    {IPT_BUTTON1 | IPF_PLAYER1, "P1 Button 1",
     SEQ_DEF_5(KEYCODE_LCONTROL, CODE_OR, JOYCODE_1_BUTTON1, CODE_OR, JOYCODE_MOUSE_1_BUTTON1)},
    {IPT_BUTTON2 | IPF_PLAYER1, "P1 Button 2",
     SEQ_DEF_5(KEYCODE_LALT, CODE_OR, JOYCODE_1_BUTTON2, CODE_OR, JOYCODE_MOUSE_1_BUTTON3)},
    {IPT_BUTTON3 | IPF_PLAYER1, "P1 Button 3",
     SEQ_DEF_5(KEYCODE_SPACE, CODE_OR, JOYCODE_1_BUTTON3, CODE_OR, JOYCODE_MOUSE_1_BUTTON2)},
...
struct ik input_keywords[] = {{"KEYCODE_A", IKT_STD, KEYCODE_A},
        ...
        while (in->type != IPT_END && in->type != IPT_PORT) {
            if ((in->type & ~IPF_MASK) != IPT_DIPSWITCH_SETTING && /* skip dipswitch definitions */
                ...
                ((in->type & ~IPF_MASK) >= IPT_COIN1 &&        /* skip if coin input and it's locked out */
                 (in->type & ~IPF_MASK) <= IPT_COIN4 && coinlockedout[(in->type & ~IPF_MASK) - IPT_COIN1])) {
                player = IP_GET_PLAYER(in);

                if (((in->type & ~IPF_MASK) > IPT_ANALOG_START) && ((in->type & ~IPF_MASK) < IPT_ANALOG_END)) {
                    analog_player_port[port] = player;
                }
                if (((in->type & ~IPF_MASK) == IPT_BUTTON1) || ((in->type & ~IPF_MASK) == IPT_BUTTON2)
                    || ((in->type & ~IPF_MASK) == IPT_BUTTON3) || ((in->type & ~IPF_MASK) == IPT_BUTTON4)
                    || ((in->type & ~IPF_MASK) == IPT_JOYSTICK_UP) || ((in->type & ~IPF_MASK) == IPT_JOYSTICK_DOWN)
                    || ((in->type & ~IPF_MASK) == IPT_JOYSTICK_LEFT) || ((in->type & ~IPF_MASK) == IPT_JOYSTICK_RIGHT)
                    ...
                    || ((in->type & ~IPF_MASK) == IPT_JOYSTICKLEFT_RIGHT) || ((in->type & ~IPF_MASK) == IPT_PADDLE)
                    || ((in->type & ~IPF_MASK) == IPT_DIAL) || ((in->type & ~IPF_MASK) == IPT_TRACKBALL_X)
                    || ((in->type & ~IPF_MASK) == IPT_TRACKBALL_Y) || ((in->type & ~IPF_MASK) == IPT_LIGHTGUN_X)
                    || ((in->type & ~IPF_MASK) == IPT_LIGHTGUN_Y) || ((in->type & ~IPF_MASK) == IPT_AD_STICK_X)
                    || ((in->type & ~IPF_MASK) == IPT_AD_STICK_Y) || ((in->type & ~IPF_MASK) == IPT_AD_STICK_Z)) {
...
const char*
input_port_name(const struct InputPort* in) {
    int i;
    unsigned type;
    ...
    while (inputport_defaults[i].type != IPT_END && inputport_defaults[i].type != type)
        i++;
...
InputSeq*
input_port_seq(const struct InputPort* in) {
    int i, type;

    static InputSeq ip_none = SEQ_DEF_1(CODE_NONE);

    while (seq_get_1((InputSeq*)&in->seq) == CODE_PREVIOUS)
        in--;
...
update_analog_port(int port) {
    ...
    if (!options.cheat && (in->type & IPF_CHEAT))
        return;
    ...
    if (type != IPT_PEDAL && type != IPT_PEDAL2) {
        ...
        if (!delta && seq_get_1(incseq) == KEYCODE_Y)
            delta += keydelta;
        delta = -delta;
    }
    ...
    if (is_stick) {
        ...
        if ((new != 0) || (new - prev != 0)) {
            ...
            if (in->mask > 0xff) {
                if (new > 0) {
                    current =
                        (int)((pow(new / 32768.0, 100.0 / sensitivity) * (max - in->default_value) + in->default_value)
                              * 100 / sensitivity);
                } else {
                    current =
                        (int)((pow(-new / 32768.0, 100.0 / sensitivity) * (min - in->default_value) + in->default_value)
                              * 100 / sensitivity);
                }
            } else {
                if (new > 0) {
                    current =
                        (int)((pow(new / 128.0, 100.0 / sensitivity) * (max - in->default_value) + in->default_value)
                              * 100 / sensitivity);
                } else {
                    current =
                        (int)((pow(-new / 128.0, 100.0 / sensitivity) * (min - in->default_value) + in->default_value)
                              * 100 / sensitivity);
                }
            }
        }
    ...
    while (in->type != IPT_END && port < MAX_INPUT_PORTS) {
        ...
        while (in->type != IPT_END && in->type != IPT_PORT) {
            if ((in->type & ~IPF_MASK) != IPT_DIPSWITCH_SETTING && /* skip dipswitch definitions */
                ...
                (in->type & ~IPF_MASK) != IPT_EXTENSION)        /* skip analog extension fields */
            {
                input_port_value[port] = (input_port_value[port] & ~in->mask) | (in->default_value & in->mask);
#ifdef MAME_NET
                if (net_active())
        ...
        for (in = start; in->type != IPT_END && in->type != IPT_PORT; in++, ib++) {
#ifdef MAME_NET
            player = IP_GET_PLAYER(in);
#endif                                                             /* MAME_NET */
            if ((in->type & ~IPF_MASK) != IPT_DIPSWITCH_SETTING && /* skip dipswitch definitions */
                ...
                (in->type & ~IPF_MASK) != IPT_EXTENSION) /* skip analog extension fields */
            {
                if ((in->type & ~IPF_MASK) == IPT_VBLANK) {
                    ...
                    if (Machine->drv->vblank_duration == 0)
                        logerror("Warning: you are using IPT_VBLANK with vblank_duration = 0. You need to increase "
                                 "vblank_duration for IPT_VBLANK to work.\n");
                }
                         ...
                         && ((in->type & ~IPF_MASK) < IPT_ANALOG_END)) /* LBO 120897 */
                {
                    ...
                    if (seq_pressed(seq)) {
                        ...
                        if ((in->type & ~IPF_MASK) >= IPT_COIN5 && (in->type & ~IPF_MASK) <= IPT_COIN8
...
#ifdef MAME_NET
void
set_default_player_controls(int player) {
...
static unsigned
input_port_count(const struct InputPortTiny* src) {
    unsigned total;
...
struct InputPort*
input_port_allocate(const struct InputPortTiny* src) {
...
void
input_port_free(struct InputPort* dst) {
    free(dst);
}

void
seq_set_string(InputSeq* a, const char* buf) {
    char* lbuf;
    char* arg;
    int j;
    struct ik* pik;
