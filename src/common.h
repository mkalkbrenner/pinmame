...
#define ROM_INHERITEDFLAGS                                                                                             \
    (ROM_GROUPMASK | ROM_SKIPMASK | ROM_REVERSEMASK | ROM_BITWIDTHMASK | ROM_BITSHIFTMASK | ROM_BIOSFLAGSMASK)
...
#define ROM_GETBITWIDTH(r)                                                                                             \
    (((ROM_GETFLAGS(r) & ROM_BITWIDTHMASK) >> 21) + 8 * ((ROM_GETFLAGS(r) & ROM_BITWIDTHMASK) == 0))
...
#define ROM_COPY(rgn, srcoffset, offset, length)                                                                       \
    ROMX_LOAD(ROMENTRY_COPY, offset, length, (const char*)srcoffset, (rgn) << 24)

/* ----- nibble loading macros ----- */
#define ROM_LOAD_NIB_HIGH(name, offset, length, hash)                                                                  \
    ROMX_LOAD(name, offset, length, hash, ROM_NIBBLE | ROM_SHIFT_NIBBLE_HI)
#define ROM_LOAD_NIB_LOW(name, offset, length, hash)                                                                   \
    ROMX_LOAD(name, offset, length, hash, ROM_NIBBLE | ROM_SHIFT_NIBBLE_LO)
...
#define ROM_LOAD16_WORD_SWAP(name, offset, length, hash)                                                               \
    ROMX_LOAD(name, offset, length, hash, ROM_GROUPWORD | ROM_REVERSE)
...
#define ROM_LOAD32_WORD_SWAP(name, offset, length, hash)                                                               \
    ROMX_LOAD(name, offset, length, hash, ROM_GROUPWORD | ROM_REVERSE | ROM_SKIP(2))
...
#define BITSWAP8(val, B7, B6, B5, B4, B3, B2, B1, B0)                                                                  \
    ((BIT(val, B7) << 7) | (BIT(val, B6) << 6) | (BIT(val, B5) << 5) | (BIT(val, B4) << 4) | (BIT(val, B3) << 3)       \
...
#define BITSWAP24(val, B23, B22, B21, B20, B19, B18, B17, B16, B15, B14, B13, B12, B11, B10, B9, B8, B7, B6, B5, B4,   \
                  B3, B2, B1, B0)                                                                                      \
