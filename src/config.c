...
static int
readint(mame_file* f, UINT32* num) {
    unsigned i;
...
static void
writeint(mame_file* f, UINT32 num) {
    unsigned i;
...
static int
readword(mame_file* f, UINT16* num) {
    unsigned i;
    int res;
...
static void
writeword(mame_file* f, UINT16 num) {
    unsigned i;
...
static int
seq_read_ver_8(mame_file* f, InputSeq* seq) {
    int j, len;
    UINT32 i;
    UINT16 w;
...
static int
input_port_read_ver_8(mame_file* f, struct InputPort* in) {
    UINT32 i;
    UINT16 w;
    if (readint(f, &i) != 0)
        return -1;
...
static config_file*
config_init(const char* name, int save) {
    static const struct cfg_format formats[] = {/* mame 0.74 with 8 coin counters */
...
static unsigned int
count_input_ports(const struct InputPort* in) {
...
int
config_read_ports(config_file* cfg, struct InputPort* input_ports_default, struct InputPort* input_ports) {
    //unsigned int total;
    ...
    while (in->type != IPT_END) {
        ...
        if (in->mask != saved.mask || in->default_value != saved.default_value || in->type != saved.type
...
int
config_read_default_ports(config_file* cfg, struct ipd* input_ports_default) {
    UINT32 type;
    InputSeq def_seq;
    InputSeq seq;
    int (*read_seq)(mame_file*, InputSeq*);
...
int
config_read_coin_and_ticket_counters(config_file* cfg, unsigned int* coins, unsigned int* lastcoin,
...
int
config_read_mixer_config(config_file* cfg, struct mixer_config* mixercfg) {
    if (cfg->is_write)
        return CONFIG_ERROR_BADMODE;
...
static void
seq_write(mame_file* f, const InputSeq* seq) {
    int j, len;
...
static void
input_port_write(mame_file* f, const struct InputPort* in) {
    writeint(f, in->type);
    writeword(f, in->mask);
...
int
config_write_ports(config_file* cfg, const struct InputPort* input_ports_default, const struct InputPort* input_ports) {
    unsigned int total;
...
int
config_write_default_ports(config_file* cfg, const struct ipd* input_ports_default_backup,
                           const struct ipd* input_ports_default) {
    int i = 0;
...
int
config_write_coin_and_ticket_counters(config_file* cfg, const unsigned int* coins, const unsigned int* lastcoin,
...
int
config_write_mixer_config(config_file* cfg, const struct mixer_config* mixercfg) {
    if (!cfg->is_write)
        return CONFIG_ERROR_BADMODE;
    if (cfg->position != POSITION_AFTER_COINS)
        return CONFIG_ERROR_BADPOSITION;
