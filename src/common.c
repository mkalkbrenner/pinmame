...
void
showdisclaimer(void) /* MAURY_BEGIN: dichiarazione */
{
#ifdef PINMAME
...
static struct GameSample*
read_wav_sample(mame_file* f) {
...
struct GameSamples*
readsamples(const char** samplenames, const char* basename)
...
/* if first samplename is *dir, looks for samples into "basename" first, then "dir" */
{
    ...
    i = 0;
    while (samplenames[i + skipfirst] != 0)
        i++;
...
int
new_memory_region(int num, size_t length, UINT32 flags) {
    if (num < MAX_MEMORY_REGIONS) {
...
void
nvram_handler_generic_0fill(mame_file* file, int read_or_write) {
#ifdef PINMAME
...
void
nvram_handler_generic_1fill(mame_file* file, int read_or_write) {
#ifdef PINMAME
...
struct mame_bitmap*
bitmap_alloc_core(int width, int height, int depth, int use_auto) {
...
struct mame_bitmap*
bitmap_alloc(int width, int height) {
...
struct mame_bitmap*
bitmap_alloc_depth(int width, int height, int depth) {
...
void
bitmap_free(struct mame_bitmap* bitmap) {
    /* skip if NULL */
    if (!bitmap)
        return;
...
struct mame_bitmap*
auto_bitmap_alloc(int width, int height) {
...
struct mame_bitmap*
auto_bitmap_alloc_depth(int width, int height, int depth) {
...
void
save_screen_snapshot_as(mame_file* fp, struct mame_bitmap* bitmap) {
...
void
save_screen_snapshot(struct mame_bitmap* bitmap) {
    char name[20];
    mame_file* fp;
...
rom_first_region(const struct GameDriver* drv) {
    return drv->rom;
}
...
const struct RomModule*
rom_next_region(const struct RomModule* romp) {
    romp++;
...
const struct RomModule*
rom_first_file(const struct RomModule* romp) {
    romp++;
...
const struct RomModule*
rom_next_file(const struct RomModule* romp) {
    romp++;
    while (!ROMENTRY_ISFILE(romp) && !ROMENTRY_ISREGIONEND(romp))
        romp++;
    return ROMENTRY_ISREGIONEND(romp) ? NULL : romp;
...
const struct RomModule*
rom_first_chunk(const struct RomModule* romp) {
...
const struct RomModule*
rom_next_chunk(const struct RomModule* romp) {
    romp++;
...
int
determine_bios_rom(const struct SystemBios* bios) {
...
static int
count_roms(const struct RomModule* romp) {
...
static void
fill_random(UINT8* base, UINT32 length) {
    while (length--)
        *base++ = rand();
}
...
static void
handle_missing_file(struct rom_load_data* romdata, const struct RomModule* romp) {
...
static void
dump_wrong_and_correct_checksums(struct rom_load_data* romdata, const char* hash, const char* acthash) {
    unsigned i;
    char chksum[256];
...
static void
verify_length_and_hash(struct rom_load_data* romdata, const char* name, UINT32 explength, const char* hash) {
    UINT32 actlength;
    const char* acthash;
...
static int
display_rom_load_results(struct rom_load_data* romdata) {
...
static void
region_post_process(struct rom_load_data* romdata, const struct RomModule* regiondata) {
...
static int
open_rom_file(struct rom_load_data* romdata, const struct RomModule* romp) {
...
static int
rom_fread(struct rom_load_data* romdata, UINT8* buffer, int length) {
...
static int
read_rom_data(struct rom_load_data* romdata, const struct RomModule* romp) {
    ...
    debugload("Loading ROM data: offs=%X len=%X mask=%02X group=%d skip=%d reverse=%d\n", ROM_GETOFFSET(romp), numbytes,
              datamask, groupsize, skip, reversed);
...
static int
fill_rom_data(struct rom_load_data* romdata, const struct RomModule* romp) {
...
static int
copy_rom_data(struct rom_load_data* romdata, const struct RomModule* romp) {
    UINT8* base = romdata->regionbase + ROM_GETOFFSET(romp);
...
static int
process_rom_entries(struct rom_load_data* romdata, const struct RomModule* romp) {
    UINT32 lastflags = 0;
...
static int
process_disk_entries(struct rom_load_data* romdata, const struct RomModule* romp) {
    /* loop until we hit the end of this region */
    while (!ROMENTRY_ISREGIONEND(romp)) {
        /* handle files */
        if (ROMENTRY_ISFILE(romp)) {
            ...
            if (hash_data_used_functions(ROM_GETHASHDATA(romp)) != HASH_MD5) {
                sprintf(&romdata->errorbuf[strlen(romdata->errorbuf)],
                        "%-12s IGNORED ADDITIONAL CHECKSUM INFORMATIONS (ONLY MD5 SUPPORTED)\n", filename);
            }
...
int
rom_load(const struct RomModule* romp) {
...
void
printromlist(const struct RomModule* romp, const char* basename) {
