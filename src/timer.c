...
INLINE void
timer_list_insert(mame_timer* timer) {
...
INLINE void
timer_list_remove(mame_timer* timer) {
/* sanity checks for the debug build */
#ifdef MAME_DEBUG
    {
        mame_timer* t;
...
double
timer_time_until_next_timer(void) {
...
void
timer_adjust_global_time(double delta) {
    mame_timer* timer;
...
void
timer_adjust(mame_timer* which, double duration, int param, double period) {
    double time = get_relative_time();
...
void
timer_pulse(double period, int param, void (*callback)(int)) {
...
void
timer_set(double duration, int param, void (*callback)(int)) {
    mame_timer* timer = timer_alloc(callback);
...
void
timer_reset(mame_timer* which, double duration) {
    /* adjust the timer */
...
int
timer_enable(mame_timer* which, int enable) {
    int old;
...
double
timer_timeelapsed(mame_timer* which) {
    double time = get_relative_time();
...
double
timer_timeleft(mame_timer* which) {
    double time = get_relative_time();
...
double
timer_starttime(mame_timer* which) {
...
double
timer_firetime(mame_timer* which) {
    return global_offset + which->expire;
}

#ifdef PINMAME
double
timer_expire(mame_timer* which) {
    return which->expire;
}

int
timer_param(mame_timer* which) {
    return which->callback_param;
}

int
timer_enabled(mame_timer* which) {
