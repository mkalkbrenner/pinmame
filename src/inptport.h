...
/* start of table */
#define INPUT_PORTS_START(name)       static const struct InputPortTiny input_ports_##name[] = {
...
#define PORT_START                               {0, 0, IPT_PORT, 0},

/* input bit definition */
#define PORT_BIT_NAME(mask, default, type, name) {mask, default, type, name},
#define PORT_BIT(mask, default, type)            PORT_BIT_NAME(mask, default, type, IP_NAME_DEFAULT)
...
#define PORT_CODE(key, joy) {key, joy, IPT_EXTENSION, 0},
...
#define PORT_ANALOG(mask, default, type, sensitivity, delta, min, max)                                                 \
    PORT_BIT(mask, default, type){min, max, IPT_EXTENSION | IPF_SENSITIVITY(sensitivity) | IPF_DELTA(delta),           \
                                  IP_NAME_DEFAULT},

#define PORT_ANALOGX(mask, default, type, sensitivity, delta, min, max, keydec, keyinc, joydec, joyinc)                \
    PORT_BIT(mask, default, type){min, max, IPT_EXTENSION | IPF_SENSITIVITY(sensitivity) | IPF_DELTA(delta),           \
                                  IP_NAME_DEFAULT},                                                                    \
        PORT_CODE(keydec, joydec) PORT_CODE(keyinc, joyinc)

/* dip switch definition */
#define PORT_DIPNAME(mask, default, name) PORT_BIT_NAME(mask, default, IPT_DIPSWITCH_NAME, name)

#define PORT_DIPSETTING(default, name)    PORT_BIT_NAME(0, default, IPT_DIPSWITCH_SETTING, name)
