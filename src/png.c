...
static UINT32
convert_from_network_order(UINT8* v) {
    UINT32 i;
...
int
png_unfilter(struct png_info* p) {
    int i, j, bpp;
...
int
png_verify_signature(mame_file* fp) {
    INT8 signature[8];
...
int
png_inflate_image(struct png_info* p) {
...
int
png_read_file(mame_file* fp, struct png_info* p) {
...
int
png_read_info(mame_file* fp, struct png_info* p) {
    UINT32 chunk_type = 0;
    UINT8* chunk_data;
    UINT8 str_chunk_type[5], v[4];
    int res = 0;
        ...
        switch (chunk_type) {
            ...
            case PNG_CN_tEXt: {
                ...
                if (strcmp((const char*)chunk_data, "Screen") == 0) {
                    int c = sscanf(text, "%i%i%i%i", &p->screen.min_x, &p->screen.max_x, &p->screen.min_y,
                                   &p->screen.max_y);
                    if (c == 4) {
                        res = 1;
...
int
png_expand_buffer_8bit(struct png_info* p) {
    if (p->bit_depth < 8) {
        int i;
...
void
png_delete_unused_colors(struct png_info* p) {
...
static void
convert_to_network_order(UINT32 i, UINT8* v) {
    v[0] = i >> 24;
    v[1] = (i >> 16) & 0xff;
    v[2] = (i >> 8) & 0xff;
    v[3] = i & 0xff;
...
int
png_add_text(const char* keyword, const char* text) {
    struct png_text* pt;
...
static int
write_chunk(mame_file* fp, UINT32 chunk_type, UINT8* chunk_data, UINT32 chunk_length) {
    UINT32 crc;
    UINT8 v[4];
    int written;
...
int
png_write_datastream(mame_file* fp, struct png_info* p) {
    UINT8 ihdr[13];
    struct png_text* pt;
...
int
png_deflate_image(struct png_info* p) {
...
static int
png_pack_buffer(struct png_info* p) {
    if (p->bit_depth < 8) {
        int i;
        UINT8 *outp, *inp;
...
static int
png_create_datastream(void* fp, struct mame_bitmap* bitmap) {
    int i, j;
    struct png_info p;
...
int
png_write_bitmap(mame_file* fp, struct mame_bitmap* bitmap) {
    char text[1024];
...
int
mng_capture_start(mame_file* fp, struct mame_bitmap* bitmap) {
    UINT8 mhdr[28];
    /*	UINT8 term; */
...
int
mng_capture_frame(mame_file* fp, struct mame_bitmap* bitmap) {
    if (mng_status) {
        if (png_create_datastream(fp, bitmap) == 0)
            return 0;
        return 1;
