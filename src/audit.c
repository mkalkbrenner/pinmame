...
void*
audit_hard_disk_open(const char* filename, const char* mode) {
...
void
audit_hard_disk_close(void* file) {
...
UINT32
audit_hard_disk_read(void* file, UINT64 offset, UINT32 count, void* buffer) {
...
UINT32
audit_hard_disk_write(void* file, UINT64 offset, UINT32 count, const void* buffer) {
    return 0;
}

static struct hard_disk_interface audit_hard_disk_interface = {audit_hard_disk_open, audit_hard_disk_close,
                                                               audit_hard_disk_read, audit_hard_disk_write};

/* returns 1 if rom is defined in this set */
int
RomInSet(const struct GameDriver* gamedrv, const char* hash) {
...
int
AuditRomSet(int game, tAuditRecord** audit) {
...
static void
VerifyDumpHashData(const char* hash, const char* head, verify_printf_proc verify_printf) {
    char buf[512];
...
int
VerifyRomSet(int game, verify_printf_proc verify_printf) {
    tAuditRecord* aud;
    int count;
    ...
    if (archive_status
        & (AUD_ROM_NOT_FOUND | AUD_BAD_CHECKSUM | AUD_MEM_ERROR | AUD_LENGTH_MISMATCH | AUD_DISK_NOT_FOUND
           | AUD_DISK_BAD_MD5))
        return INCORRECT;
...
int
AuditSampleSet(int game, tMissingSample** audit) {
...
int
VerifySampleSet(int game, verify_printf_proc verify_printf) {
    tMissingSample* aud;
    int count;
