 ...
 */
void
container_alloc(char*** map, unsigned* mac, unsigned* max) {
    *map = 0;
    *mac = 0;
    *max = 0;
}
 ...
 */
void
container_free(char*** map, unsigned* mac, unsigned* max) {
    unsigned i;
 ...
 */
int
container_add(char*** map, unsigned* mac, unsigned* max, const char* s, unsigned len) {
    if (*mac >= *mac) {
        *max = 256 + *mac * 2;
 ...
 */
int
container_compare(const void* a, const void* b) {
...
enum token_t { token_open, token_close, token_data };
...
void
print_item(FILE* os, const char* s, unsigned len) {
    unsigned i;
...
void
print_string(FILE* os, const char* s, unsigned len) {
    unsigned i;
    fprintf(os, "\"");
    for (i = 0; i < len; ++i) {
        switch (s[i]) {
...
void
process_error(struct state_t* state, const char* tag, const char* msg) {
    fprintf(stderr, "%d:%s:%s\n", XML_GetCurrentLineNumber(state->parser), tag, msg);
    state->error = 1;
}

void
process_game(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_open) {
        unsigned i;
        if (!attributes) {
...
void
process_string2(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
...
void
process_item2(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        fprintf(state->os, L1P "%s ", state->level[2].tag);
        if (len == 0) {
...
void
process_gamename(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
...
void
process_reference(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
...
void
process_set2(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_open) {
...
void
process_string3(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
...
void
process_item3(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        fprintf(state->os, L2P "%s ", state->level[3].tag);
        if (len == 0) {
...
void
process_num3(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        unsigned i;
        for (i = 0; i < len; ++i) {
            if (!isdigit(s[i])) {
...
void
process_hex3(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        unsigned i;
        for (i = 0; i < len; ++i) {
            if (!isxdigit(s[i])) {
...
void
process_float3(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        unsigned i;
        for (i = 0; i < len; ++i) {
            if (!isdigit(s[i]) && s[i] != '.') {
...
void
process_samplename(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        fprintf(state->os, L1P "%s ", state->level[2].tag);
        if (len == 0) {
            process_error(state, state->level[2].tag, "empty token");
...
void
process_romoffset(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
...
void
process_romstatus(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
...
void
process_romdispose(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
...
void
process_romsoundonly(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        if (strncmp("yes", s, len) == 0) {
...
void
process_chipsoundonly(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        if (strncmp("yes", s, len) == 0) {
...
void
process_inputtilt(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        if (strncmp("yes", s, len) == 0) {
...
void
process_inputservice(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        if (strncmp("yes", s, len) == 0) {
...
void
process_videorefresh(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
...
void
process_videowidth(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        state->level[3].tag = "x";
...
void
process_videoheight(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_data) {
        state->level[3].tag = "y";
        process_num3(state, t, s, len, attributes);
    }
...
void
process_dipswitchdipvalue(struct state_t* state, enum token_t t, const char* s, unsigned len, const char** attributes) {
    if (t == token_open) {
        unsigned i;
        if (!attributes) {
...
void
process_dipswitchdipvaluename(struct state_t* state, enum token_t t, const char* s, unsigned len,
...
struct conversion_t {
    ...
    process_t* process;
} CONV[] = {{1, {"mame", "game", 0, 0, 0}, process_game},
 ...
 */
struct conversion_t*
identify(unsigned depth, const struct level_t* level) {
    unsigned i, j;
 ...
 */
void
end_handler(void* data, const XML_Char* name) {
    struct state_t* state = (struct state_t*)data;

    if (state->depth < DEPTH_MAX) {
        if (state->error == 0) {
            if (state->level[state->depth].process) {
                state->level[state->depth].process(state, token_data, state->level[state->depth].data,
 ...
 */
void
data_handler(void* data, const XML_Char* s, int len) {
    struct state_t* state = (struct state_t*)data;
 ...
 */
void
start_handler(void* data, const XML_Char* name, const XML_Char** attributes) {
    struct state_t* state = (struct state_t*)data;
