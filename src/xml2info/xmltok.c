...
#define VTABLE1                                                                                                        \
    {PREFIX(prologTok), PREFIX(contentTok), PREFIX(cdataSectionTok) IGNORE_SECTION_TOK_VTABLE},                        \
        {PREFIX(attributeValueTok), PREFIX(entityValueTok)}, PREFIX(sameName), PREFIX(nameMatchesAscii),               \
        PREFIX(nameLength), PREFIX(skipS), PREFIX(getAtts), PREFIX(charRefNumber), PREFIX(predefinedEntityName),       \
...
static void PTRCALL
utf8_toUtf8(const ENCODING* enc, const char** fromP, const char* fromLim, char** toP, const char* toLim) {
    char* to;
    const char* from;
...
static void PTRCALL
utf8_toUtf16(const ENCODING* enc, const char** fromP, const char* fromLim, unsigned short** toP,
...
static void PTRCALL
latin1_toUtf8(const ENCODING* enc, const char** fromP, const char* fromLim, char** toP, const char* toLim) {
    for (;;) {
        unsigned char c;
        if (*fromP == fromLim)
            break;
...
static void PTRCALL
latin1_toUtf16(const ENCODING* enc, const char** fromP, const char* fromLim, unsigned short** toP,
               const unsigned short* toLim) {
...
static void PTRCALL
ascii_toUtf8(const ENCODING* enc, const char** fromP, const char* fromLim, char** toP, const char* toLim) {
    while (*fromP != fromLim && *toP != toLim)
        *(*toP)++ = *(*fromP)++;
}
...
#define DEFINE_UTF16_TO_UTF8(E)                                                                                        \
    static void PTRCALL E##toUtf8(const ENCODING* enc, const char** fromP, const char* fromLim, char** toP,            \
                                  const char* toLim) {                                                                 \
        const char* from;                                                                                              \
...
#define DEFINE_UTF16_TO_UTF16(E)                                                                                       \
    static void PTRCALL E##toUtf16(const ENCODING* enc, const char** fromP, const char* fromLim, unsigned short** toP, \
                                   const unsigned short* toLim) {                                                      \
...
static void PTRCALL
initUpdatePosition(const ENCODING* enc, const char* ptr, const char* end, POSITION* pos) {
...
*/
static int
parsePseudoAttribute(const ENCODING* enc, const char* ptr, const char* end, const char** namePtr,
...
static int
doParseXmlDecl(const ENCODING* (*encodingFinder)(const ENCODING*, const char*, const char*), int isGeneralTextEntity,
...
static void PTRCALL
unknown_toUtf8(const ENCODING* enc, const char** fromP, const char* fromLim, char** toP, const char* toLim) {
    const struct unknown_encoding* uenc = AS_UNKNOWN_ENCODING(enc);
...
static void PTRCALL
unknown_toUtf16(const ENCODING* enc, const char** fromP, const char* fromLim, unsigned short** toP,
                const unsigned short* toLim) {
    const struct unknown_encoding* uenc = AS_UNKNOWN_ENCODING(enc);
    while (*fromP != fromLim && *toP != toLim) {
...
static int
initScan(const ENCODING** encodingTable, const INIT_ENCODING* enc, int state, const char* ptr, const char* end,
    ...
    if (ptr + 1 == end) {
        ...
        switch (((unsigned char)ptr[0] << 8) | (unsigned char)ptr[1]) {
            ...
            case 0x3C00:
                if ((INIT_ENC_INDEX(enc) == UTF_16BE_ENC || INIT_ENC_INDEX(enc) == UTF_16_ENC)
