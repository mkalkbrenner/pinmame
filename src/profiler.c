...
void
profiler_show(struct mame_bitmap* bitmap) {
    int i, j;
    ...
    static const char* names[PROFILER_TOTAL] = {
        "CPU 1  ", "CPU 2  ", "CPU 3  ", "CPU 4  ", "CPU 5  ", "CPU 6  ", "CPU 7  ", "CPU 8  ", "Mem rd ", "Mem wr ",
        "Video  ", "drawgfx", "copybmp", "tmdraw ", "tmdrroz", "tmupdat", "Artwork", "Blit   ", "Sound  ", "Mixer  ",
        "Callbck", "Hiscore", "Input  ", "Extra  ", "User1  ", "User2  ", "User3  ", "User4  ", "Profilr", "Idle   ",
    };
    ...
    if (total == 0 || normalize == 0)
        return; /* we have been just reset */
    ...
    for (i = 0; i < PROFILER_TOTAL; i++) {
        ...
        if (computed || showdelay[i]) {
            if (computed)
                showdelay[i] = (int)Machine->drv->frames_per_second;
            showdelay[i]--;

            if (i < PROFILER_PROFILER)
                sprintf(buf, "%s%3d%%%3d%%", names[i], (int)((computed * 100 + total / 2) / total),
                        (int)((computed * 100 + normalize / 2) / normalize));
            else
                sprintf(buf, "%s%3d%%", names[i], (int)((computed * 100 + total / 2) / total));
