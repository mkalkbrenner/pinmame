...
#define MEMORY_READ_START(name)                                                                                        \
    MEMPORT_ARRAY_START(Memory_ReadAddress, name, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_MEM | MEMPORT_WIDTH_8)
#define MEMORY_WRITE_START(name)                                                                                       \
    MEMPORT_ARRAY_START(Memory_WriteAddress, name, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_MEM | MEMPORT_WIDTH_8)
#define MEMORY_READ16_START(name)                                                                                      \
    MEMPORT_ARRAY_START(Memory_ReadAddress16, name, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_MEM | MEMPORT_WIDTH_16)
#define MEMORY_WRITE16_START(name)                                                                                     \
    MEMPORT_ARRAY_START(Memory_WriteAddress16, name, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_MEM | MEMPORT_WIDTH_16)
#define MEMORY_READ32_START(name)                                                                                      \
    MEMPORT_ARRAY_START(Memory_ReadAddress32, name, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_MEM | MEMPORT_WIDTH_32)
#define MEMORY_WRITE32_START(name)                                                                                     \
    MEMPORT_ARRAY_START(Memory_WriteAddress32, name, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_MEM | MEMPORT_WIDTH_32)
...
#define PORT_READ_START(name)                                                                                          \
    MEMPORT_ARRAY_START(IO_ReadPort, name, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_IO | MEMPORT_WIDTH_8)
#define PORT_WRITE_START(name)                                                                                         \
    MEMPORT_ARRAY_START(IO_WritePort, name, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_IO | MEMPORT_WIDTH_8)
#define PORT_READ16_START(name)                                                                                        \
    MEMPORT_ARRAY_START(IO_ReadPort16, name, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_IO | MEMPORT_WIDTH_16)
#define PORT_WRITE16_START(name)                                                                                       \
    MEMPORT_ARRAY_START(IO_WritePort16, name, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_IO | MEMPORT_WIDTH_16)
#define PORT_READ32_START(name)                                                                                        \
    MEMPORT_ARRAY_START(IO_ReadPort32, name, MEMPORT_DIRECTION_READ | MEMPORT_TYPE_IO | MEMPORT_WIDTH_32)
#define PORT_WRITE32_START(name)                                                                                       \
    MEMPORT_ARRAY_START(IO_WritePort32, name, MEMPORT_DIRECTION_WRITE | MEMPORT_TYPE_IO | MEMPORT_WIDTH_32)
...
#define LEVEL1_BITS(x)                                                                                                 \
    (((x) < (2 * LEVEL1_BITS_PREF - LEVEL1_BITS_BIAS)) ? LEVEL1_BITS_PREF : ((x) + LEVEL1_BITS_BIAS) / 2)
...
#define LEVEL2_INDEX(e, a, b, m)                                                                                       \
    ((1 << LEVEL1_BITS((b) - (m))) + (((e)&SUBTABLE_MASK) << LEVEL2_BITS((b) - (m)))                                   \
     + (((a) >> (m)) & LEVEL2_MASK((b) - (m))))
...
#define DECLARE_PORT_HANDLERS_8BIT(abits)     DECLARE_HANDLERS_8BIT(port, abits)

#define DECLARE_PORT_HANDLERS_16BIT_BE(abits) DECLARE_HANDLERS_16BIT_BE(port, abits)

#define DECLARE_PORT_HANDLERS_16BIT_LE(abits) DECLARE_HANDLERS_16BIT_LE(port, abits)

#define DECLARE_PORT_HANDLERS_32BIT_BE(abits) DECLARE_HANDLERS_32BIT_BE(port, abits)

#define DECLARE_PORT_HANDLERS_32BIT_LE(abits) DECLARE_HANDLERS_32BIT_LE(port, abits)
...
INLINE data8_t
cpu_readop(offs_t A) {
    if (address_is_unsafe(A)) {
        activecpu_set_op_base(A);
    }
    return cpu_readop_unsafe(A);
}

INLINE data16_t
cpu_readop16(offs_t A) {
    if (address_is_unsafe(A)) {
        activecpu_set_op_base(A);
    }
    return cpu_readop16_unsafe(A);
}

INLINE data32_t
cpu_readop32(offs_t A) {
    if (address_is_unsafe(A)) {
        activecpu_set_op_base(A);
    }
    return cpu_readop32_unsafe(A);
}

INLINE data8_t
cpu_readop_arg(offs_t A) {
    if (address_is_unsafe(A)) {
        activecpu_set_op_base(A);
    }
    return cpu_readop_arg_unsafe(A);
}

INLINE data16_t
cpu_readop_arg16(offs_t A) {
    if (address_is_unsafe(A)) {
        activecpu_set_op_base(A);
    }
    return cpu_readop_arg16_unsafe(A);
}

INLINE data32_t
cpu_readop_arg32(offs_t A) {
    if (address_is_unsafe(A)) {
        activecpu_set_op_base(A);
    }
    return cpu_readop_arg32_unsafe(A);
}
