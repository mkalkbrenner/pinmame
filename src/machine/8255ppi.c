...
void
ppi8255_init(ppi8255_interface* intfce) {
    int i;
...
int
ppi8255_r(int which, int offset) {
            ...
            if (chip->in_mask[0]) {
                if (chip->portAread)
                    ...
                    LOG(("8255 chip %d: Port A is being read (mask %02x) but has no handler.  PC: %08X\n", which,
                         chip->in_mask[0], activecpu_get_pc()));
                }
            }
            ...
            if (chip->in_mask[1]) {
                if (chip->portBread)
                    ...
                    LOG(("8255 chip %d: Port B is being read (mask %02x) but has no handler.  PC: %08X\n", which,
                         chip->in_mask[1], activecpu_get_pc()));
                }
            }
            ...
            if (chip->in_mask[2]) {
                if (chip->portCread)
                    ...
                    LOG(("8255 chip %d: Port C is being read (mask %02x) but has no handler.  PC: %08X\n", which,
                         chip->in_mask[2], activecpu_get_pc()));
                }
            }
...
void
ppi8255_w(int which, int offset, int data) {
    ppi8255* chip = &chips[which];
...
#ifdef MESS
data8_t
ppi8255_peek(int which, offs_t offset) {
    ppi8255* chip;
...
void
ppi8255_set_portAread(int which, mem_read_handler portAread) {
    chips[which].portAread = portAread;
}

void
ppi8255_set_portBread(int which, mem_read_handler portBread) {
    chips[which].portBread = portBread;
}

void
ppi8255_set_portCread(int which, mem_read_handler portCread) {
    chips[which].portCread = portCread;
}

void
ppi8255_set_portAwrite(int which, mem_write_handler portAwrite) {
    chips[which].portAwrite = portAwrite;
}

void
ppi8255_set_portBwrite(int which, mem_write_handler portBwrite) {
    chips[which].portBwrite = portBwrite;
}

void
ppi8255_set_portCwrite(int which, mem_write_handler portCwrite) {
    chips[which].portCwrite = portCwrite;
}

static void
set_mode(int which, int data, int call_handlers) {
    ppi8255* chip = &chips[which];
