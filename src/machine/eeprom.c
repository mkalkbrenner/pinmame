...
*/
static int
EEPROM_command_match(const char* buf, const char* cmd, size_t len) {
    if (cmd == 0)
        return 0;
    if (len == 0)
        return 0;
...
void
nvram_handler_93C46(mame_file* file, int read_or_write) {
    if (read_or_write)
        EEPROM_save(file);
    else {
...
void
EEPROM_init(struct EEPROM_interface* _interface) {
    intf = _interface;
...
EEPROM_write(int bit) {
    ...
    if ((serial_count > intf->address_bits)
        && EEPROM_command_match((char*)serial_buffer, intf->cmd_read,
                                strlen((char*)serial_buffer) - intf->address_bits)) {
        int i, address;
        ...
        logerror("EEPROM read %04x from address %02x\n", eeprom_data_bits, address);
               ...
               && EEPROM_command_match((char*)serial_buffer, intf->cmd_erase,
                                       strlen((char*)serial_buffer) - intf->address_bits)) {
        int i, address;
...
void
EEPROM_set_clock_line(int state) {
#if VERBOSE
        ...
        if (reset_line == CLEAR_LINE) {
            if (sending) {
                if (eeprom_clock_count == intf->data_bits && intf->enable_multi_read) {
                    ...
                    if (intf->data_bits == 16)
                        eeprom_data_bits =
                            (eeprom_data[2 * eeprom_read_address + 0] << 8) + eeprom_data[2 * eeprom_read_address + 1];
                    else
