...
#ifdef PINMAME
void
pic8259_0_config(int cpuno, int irqno) {
    memset(&pic8259[0], 0, sizeof(pic8259[0]));
    pic8259[0].enable = 0xff;
    pic8259[0].cpuno = cpuno;
    pic8259[0].irqno = irqno;
}

void
pic8259_1_config(int cpuno, int irqno) {
    memset(&pic8259[1], 0, sizeof(pic8259[1]));
    pic8259[1].enable = 0xff;
    pic8259[1].cpuno = cpuno;
    pic8259[1].irqno = irqno;
}
#endif /* PINMAME */
static void
pic8259_issue_irq(PIC8259* self, int irq) {
    UINT8 mask = 1 << irq;
...
static int
pic8259_irq_pending(PIC8259* self, int irq) {
    UINT8 mask = 1 << irq;
    return (self->pending & mask) ? 1 : 0;
}

static void
pic8259_w(PIC8259* self, offs_t offset, data8_t data) {
    switch (offset) {
        case 0:                /* PIC acknowledge IRQ */
            if (data & 0x10) { /* write ICW1 ? */
                ...
                self->icw4 = data & 1;
                DBG_LOG(1, "PIC_ack_w",
                        ("$%02x: ICW1, icw4 %d, cascade %d, vec size %d, ltim %d\n", data, self->icw4, self->cascade,
                         self->vector_size, self->level_trig_mode));
            } else if (data & 0x08) {
        ...
        case 1: /* PIC ICW2,3,4 or OCW1 */
            if (self->icw2) {
                ...
                self->x86 = data & 1;
                DBG_LOG(1, "PIC_enable_w",
                        ("$%02x: ICW4 x86 mode %d, auto EOI %d, mode %d, nested %d\n", data, self->x86, self->auto_eoi,
                         self->mode, self->nested));
                self->icw4 = 0;
            } else {
    ...
    if (self->pending & 0x01)
        pic8259_issue_irq(self, 0);
    if (self->pending & 0x02)
        pic8259_issue_irq(self, 1);
    if (self->pending & 0x04)
        pic8259_issue_irq(self, 2);
    if (self->pending & 0x08)
        pic8259_issue_irq(self, 3);
    if (self->pending & 0x10)
        pic8259_issue_irq(self, 4);
    if (self->pending & 0x20)
        pic8259_issue_irq(self, 5);
    if (self->pending & 0x40)
        pic8259_issue_irq(self, 6);
    if (self->pending & 0x80)
        pic8259_issue_irq(self, 7);
}

static int
pic8259_r(PIC8259* self, offs_t offset) {
    int data = 0xff;
    switch (offset) {
        case 0: /* PIC acknowledge IRQ */
            if (self->special) {
...
void
pic8259_0_issue_irq(int irq) {
    pic8259_issue_irq(pic8259, irq);
}

void
pic8259_1_issue_irq(int irq) {
    pic8259_issue_irq(pic8259 + 1, irq);
}

int
pic8259_0_irq_pending(int irq) {
    return pic8259_irq_pending(pic8259, irq);
}

int
pic8259_1_irq_pending(int irq) {
