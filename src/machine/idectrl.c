...
INLINE void
signal_interrupt(struct ide_state* ide) {
...
INLINE void
clear_interrupt(struct ide_state* ide) {
...
int
ide_controller_init_custom(int which, struct ide_interface* intf, void* diskhandle) {
...
UINT8*
ide_get_features(int which) {
    struct ide_state* ide = &idestate[which];
    return ide->features;
}

void
ide_set_master_password(int which, UINT8* password) {
    struct ide_state* ide = &idestate[which];
...
void
ide_set_user_password(int which, UINT8* password) {
    struct ide_state* ide = &idestate[which];
...
INLINE int
convert_to_offset_and_size32(offs_t* offset, data32_t mem_mask) {
    int size = 4;
...
INLINE int
convert_to_offset_and_size16(offs_t* offset, data32_t mem_mask) {
    int size = 2;
...
INLINE UINT32
lba_address(struct ide_state* ide) {
    /* LBA direct? */
...
INLINE void
next_sector(struct ide_state* ide) {
    /* LBA direct? */
    if (ide->cur_head_reg & 0x40) {
        ide->cur_sector++;
        if (ide->cur_sector == 0) {
...
static void
swap_strncpy(UINT8* dst, const char* src, int field_size_in_words) {
    int i;
...
static void
ide_build_features(struct ide_state* ide) {
...
static void
security_error(struct ide_state* ide) {
    /* set error state */
    ide->status |= IDE_STATUS_ERROR;
    ide->status &= ~IDE_STATUS_DRIVE_READY;
...
static void
continue_read(struct ide_state* ide) {
    /* reset the totals */
    ide->buffer_offset = 0;
...
static void
write_buffer_to_dma(struct ide_state* ide) {
...
static void
read_first_sector(struct ide_state* ide) {
    /* mark ourselves busy */
    ide->status |= IDE_STATUS_BUSY;
...
static void
read_next_sector(struct ide_state* ide) {
    /* mark ourselves busy */
    ide->status |= IDE_STATUS_BUSY;
...
static void
continue_write(struct ide_state* ide) {
    /* reset the totals */
    ide->buffer_offset = 0;
...
static void
read_buffer_from_dma(struct ide_state* ide) {
    int bytesleft = IDE_DISK_SECTOR_SIZE;
    UINT8* data = ide->buffer;
...
void
handle_command(struct ide_state* ide, UINT8 command) {
    ...
    switch (command) {
        ...
        case IDE_COMMAND_SET_FEATURES:
            LOGPRINT(("IDE Set features (%02X %02X %02X %02X %02X)\n", ide->precomp_offset, ide->sector_count & 0xff,
                      ide->cur_sector, ide->cur_cylinder & 0xff, ide->cur_cylinder >> 8));
...
static UINT32
ide_controller_read(struct ide_state* ide, offs_t offset, int size) {
    UINT32 result = 0;
...
static void
ide_controller_write(struct ide_state* ide, offs_t offset, int size, UINT32 data) {
    /* logit */
...
static UINT32
ide_bus_master_read(struct ide_state* ide, offs_t offset, int size) {
...
static void
ide_bus_master_write(struct ide_state* ide, offs_t offset, int size, UINT32 data) {
