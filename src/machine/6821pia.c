...
static void (*pia_postload_funcs[MAX_PIA])(void) = {pia_postload_0, pia_postload_1, pia_postload_2, pia_postload_3,
                                                    pia_postload_4, pia_postload_5, pia_postload_6, pia_postload_7};
...
void
pia_config(int which, int addressing, const struct pia6821_interface* intf) {
    if (which >= MAX_PIA)
        return;
    ...
    if ((intf->in_a_func) && ((FPTR)(intf->in_a_func) <= 0x100)) {
        pia[which].in_a = ((FPTR)(intf->in_a_func) - 1);
        pia[which].in_set |= PIA_IN_SET_A;
    }
    if ((intf->in_b_func) && ((FPTR)(intf->in_b_func) <= 0x100)) {
        pia[which].in_b = ((FPTR)(intf->in_b_func) - 1);
        pia[which].in_set |= PIA_IN_SET_B;
    }
    if ((intf->in_ca1_func) && ((FPTR)(intf->in_ca1_func) <= 0x100)) {
        pia[which].in_ca1 = ((FPTR)(intf->in_ca1_func) - 1);
        pia[which].in_set |= PIA_IN_SET_CA1;
    }
    if ((intf->in_ca2_func) && ((FPTR)(intf->in_ca2_func) <= 0x100)) {
        pia[which].in_ca2 = ((FPTR)(intf->in_ca2_func) - 1);
        pia[which].in_set |= PIA_IN_SET_CA2;
    }
    if ((intf->in_cb1_func) && ((FPTR)(intf->in_cb1_func) <= 0x100)) {
        pia[which].in_cb1 = ((FPTR)(intf->in_cb1_func) - 1);
        pia[which].in_set |= PIA_IN_SET_CB1;
    }
    if ((intf->in_cb2_func) && ((FPTR)(intf->in_cb2_func) <= 0x100)) {
        pia[which].in_cb2 = ((FPTR)(intf->in_cb2_func) - 1);
        pia[which].in_set |= PIA_IN_SET_CB2;
    }
}
...
pia_reset(void) {
    ...
    for (i = 0; i < MAX_PIA; i++)
        pia_config(i, pia[i].addr, pia[i].intf);
}

/******************* wire-OR for all interrupt handlers *******************/

static void
update_shared_irq_handler(void (*irq_func)(int state)) {
    int i;
...
static void
update_6821_interrupts(struct pia6821* p) {
    int new_state;

    /* start with IRQ A */
    new_state = 0;
    if ((p->irq_a1 && IRQ1_ENABLED(p->ctl_a)) || (p->irq_a2 && IRQ2_ENABLED(p->ctl_a)))
        new_state = 1;
    if (new_state != p->irq_a_state) {
        p->irq_a_state = new_state;
        if (p->intf->irq_a_func)
            update_shared_irq_handler(p->intf->irq_a_func);
    }

    /* then do IRQ B */
    new_state = 0;
    if ((p->irq_b1 && IRQ1_ENABLED(p->ctl_b)) || (p->irq_b2 && IRQ2_ENABLED(p->ctl_b)))
        new_state = 1;
    if (new_state != p->irq_b_state) {
        p->irq_b_state = new_state;
        if (p->intf->irq_b_func)
...
int
pia_read(int which, int offset) {
    struct pia6821* p = pia + which;
    int val = 0;

    /* adjust offset for 16-bit and ordering */
    offset &= 3;
    if (p->addr & PIA_ALTERNATE_ORDERING)
        ...
        case PIA_DDRA:
            ...
            if (OUTPUT_SELECTED(p->ctl_a)) {
                ...
                if (C2_OUTPUT(p->ctl_a) && C2_STROBE_MODE(p->ctl_a)) {
                    ...
                    if (p->out_ca2)
                        if (p->intf->out_ca2_func)
                            p->intf->out_ca2_func(0, 0);
                    p->out_ca2 = 0;

                    /* if the CA2 strobe is cleared by the E, reset it right away */
                    if (STROBE_E_RESET(p->ctl_a)) {
                        if (p->intf->out_ca2_func)
                            p->intf->out_ca2_func(0, 1);
                        p->out_ca2 = 1;
                    }
                }
...
void
pia_write(int which, int offset, int data) {
    struct pia6821* p = pia + which;
    ...
    switch (offset) {
        ...
        case PIA_DDRA:
            ...
            if (OUTPUT_SELECTED(p->ctl_a)) {
                ...
                if (p->intf->out_a_func && p->ddr_a)
                    p->intf->out_a_func(0, p->out_a & p->ddr_a); /* NS990130 */
            }
        ...
        case PIA_DDRB:
            ...
            if (OUTPUT_SELECTED(p->ctl_b)) {
                ...
                /* send it to the output function */
                if (p->intf->out_b_func && p->ddr_b)
                    p->intf->out_b_func(0, p->out_b & p->ddr_b); /* NS990130 */
                ...
                if (C2_OUTPUT(p->ctl_b) && C2_STROBE_MODE(p->ctl_b)) {
                    ...
                    if (p->out_cb2)
                        if (p->intf->out_cb2_func)
                            p->intf->out_cb2_func(0, 0);
                    p->out_cb2 = 0;

                    /* if the CB2 strobe is cleared by the E, reset it right away */
                    if (STROBE_E_RESET(p->ctl_b)) {
                        if (p->intf->out_cb2_func)
                            p->intf->out_cb2_func(0, 1);
                        p->out_cb2 = 1;
                    }
                }
            }
...
void
pia_set_input_a(int which, int data) {
    struct pia6821* p = pia + which;
...
void
pia_set_input_ca1(int which, int data) {
    struct pia6821* p = pia + which;
...
void
pia_set_input_ca2(int which, int data) {
    struct pia6821* p = pia + which;
...
void
pia_set_input_b(int which, int data) {
    struct pia6821* p = pia + which;
...
void
pia_set_input_cb1(int which, int data) {
    struct pia6821* p = pia + which;
...
void
pia_set_input_cb2(int which, int data) {
    struct pia6821* p = pia + which;
...
WRITE16_HANDLER(pia_0_lsb_w) {
    if (ACCESSING_LSB)
        pia_write(0, offset, data & 0xff);
}

WRITE16_HANDLER(pia_1_lsb_w) {
    if (ACCESSING_LSB)
        pia_write(1, offset, data & 0xff);
}

WRITE16_HANDLER(pia_2_lsb_w) {
    if (ACCESSING_LSB)
        pia_write(2, offset, data & 0xff);
}

WRITE16_HANDLER(pia_3_lsb_w) {
    if (ACCESSING_LSB)
        pia_write(3, offset, data & 0xff);
}

WRITE16_HANDLER(pia_4_lsb_w) {
    if (ACCESSING_LSB)
        pia_write(4, offset, data & 0xff);
}

WRITE16_HANDLER(pia_5_lsb_w) {
    if (ACCESSING_LSB)
        pia_write(5, offset, data & 0xff);
}

WRITE16_HANDLER(pia_6_lsb_w) {
    if (ACCESSING_LSB)
        pia_write(6, offset, data & 0xff);
}

WRITE16_HANDLER(pia_7_lsb_w) {
    if (ACCESSING_LSB)
        pia_write(7, offset, data & 0xff);
}
...
WRITE16_HANDLER(pia_1_msb_w) {
    if (ACCESSING_MSB)
        pia_write(1, offset, data >> 8);
}

WRITE16_HANDLER(pia_2_msb_w) {
    if (ACCESSING_MSB)
        pia_write(2, offset, data >> 8);
}

WRITE16_HANDLER(pia_3_msb_w) {
    if (ACCESSING_MSB)
        pia_write(3, offset, data >> 8);
}

WRITE16_HANDLER(pia_4_msb_w) {
    if (ACCESSING_MSB)
        pia_write(4, offset, data >> 8);
}

WRITE16_HANDLER(pia_5_msb_w) {
    if (ACCESSING_MSB)
        pia_write(5, offset, data >> 8);
}

WRITE16_HANDLER(pia_6_msb_w) {
    if (ACCESSING_MSB)
        pia_write(6, offset, data >> 8);
}

WRITE16_HANDLER(pia_7_msb_w) {
    if (ACCESSING_MSB)
        pia_write(7, offset, data >> 8);
}
