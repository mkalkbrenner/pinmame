...
void
riot6532_set_clock(int which, double clock) {
    riot[which].sec_to_cycles = clock;
    riot[which].cycles_to_sec = 1.0 / riot[which].sec_to_cycles;
}

void
riot6532_config(int which, const struct riot6532_interface* intf) {
    if (which >= MAX_RIOT_6532)
        return;
...
static void
update_6532_interrupts(struct riot6532* p) {
    int new_state;
...
static void
check_pa7_interrupt(struct riot6532* p, int old_porta, int new_porta) {
...
int
riot6532_read(int which, int offset) {
    struct riot6532* p = riot + which;
    int val = 0;
...
void
riot6532_write(int which, int offset, int data) {
    struct riot6532* p = riot + which;
...
void
riot6532_set_input_a(int which, int data) {
    struct riot6532* p = riot + which;
    int old_port_a = p->in_a;
...
void
riot6532_set_input_b(int which, int data) {
    struct riot6532* p = riot + which;
...
WRITE16_HANDLER(riot6532_0_lsb_w) {
    if (ACCESSING_LSB)
        riot6532_write(0, offset, data & 0xff);
}

WRITE16_HANDLER(riot6532_1_lsb_w) {
    if (ACCESSING_LSB)
        riot6532_write(1, offset, data & 0xff);
}

WRITE16_HANDLER(riot6532_2_lsb_w) {
    if (ACCESSING_LSB)
        riot6532_write(2, offset, data & 0xff);
}

WRITE16_HANDLER(riot6532_3_lsb_w) {
    if (ACCESSING_LSB)
        riot6532_write(3, offset, data & 0xff);
}

WRITE16_HANDLER(riot6532_4_lsb_w) {
    if (ACCESSING_LSB)
        riot6532_write(4, offset, data & 0xff);
}

WRITE16_HANDLER(riot6532_5_lsb_w) {
    if (ACCESSING_LSB)
        riot6532_write(5, offset, data & 0xff);
}

WRITE16_HANDLER(riot6532_6_lsb_w) {
    if (ACCESSING_LSB)
        riot6532_write(6, offset, data & 0xff);
}

WRITE16_HANDLER(riot6532_7_lsb_w) {
    if (ACCESSING_LSB)
        riot6532_write(7, offset, data & 0xff);
}
...
WRITE16_HANDLER(riot6532_0_msb_w) {
    if (ACCESSING_MSB)
        riot6532_write(0, offset, data >> 8);
}

WRITE16_HANDLER(riot6532_1_msb_w) {
    if (ACCESSING_MSB)
        riot6532_write(1, offset, data >> 8);
}

WRITE16_HANDLER(riot6532_2_msb_w) {
    if (ACCESSING_MSB)
        riot6532_write(2, offset, data >> 8);
}

WRITE16_HANDLER(riot6532_3_msb_w) {
    if (ACCESSING_MSB)
        riot6532_write(3, offset, data >> 8);
}

WRITE16_HANDLER(riot6532_4_msb_w) {
    if (ACCESSING_MSB)
        riot6532_write(4, offset, data >> 8);
}

WRITE16_HANDLER(riot6532_5_msb_w) {
    if (ACCESSING_MSB)
        riot6532_write(5, offset, data >> 8);
}

WRITE16_HANDLER(riot6532_6_msb_w) {
    if (ACCESSING_MSB)
        riot6532_write(6, offset, data >> 8);
}

WRITE16_HANDLER(riot6532_7_msb_w) {
    if (ACCESSING_MSB)
        riot6532_write(7, offset, data >> 8);
}
