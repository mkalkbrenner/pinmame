...
enum { I8155_PORT_A = 0, I8155_PORT_B, I8155_PORT_C, I8155_PORT_COUNT };
...
INLINE UINT8
get_timer_mode(i8155_t* i8155) {
    return (i8155->count_length >> 8) & I8155_TIMER_MODE_MASK;
}

INLINE void
timer_output(i8155_t* i8155) {
...
INLINE void
pulse_timer_output(i8155_t* i8155) {
    i8155->to = 0;
    timer_output(i8155);
    i8155->to = 1;
    timer_output(i8155);
...
INLINE int
get_port_mode(i8155_t* i8155, int port) {
    int mode = -1;
        ...
        case I8155_PORT_C:
            switch (i8155->command & I8155_COMMAND_PC_MASK) {
                case I8155_COMMAND_PC_ALT_1:
                    mode = I8155_PORT_MODE_INPUT;
                    break;
                case I8155_COMMAND_PC_ALT_2:
                    mode = I8155_PORT_MODE_OUTPUT;
                    break;
                case I8155_COMMAND_PC_ALT_3:
                    mode = I8155_PORT_MODE_STROBED_PORT_A;
                    break;
                case I8155_COMMAND_PC_ALT_4:
                    mode = I8155_PORT_MODE_STROBED;
                    break;
            }
            break;
    }
...
INLINE UINT8
read_port(i8155_t* i8155, int port) {
    UINT8 data = 0;
...
INLINE void
write_port(i8155_t* i8155, int port, UINT8 data) {
    switch (get_port_mode(i8155, port)) {
        case I8155_PORT_MODE_OUTPUT:
            i8155->output[port] = data;
            if (i8155->out_port_func[port])
                (i8155->out_port_func[port])(0, i8155->output[port]);
            break;
    }
}
...
counter_tick(int n) {
    ...
    if (i8155->counter == 0) {
        if (LOG)
            logerror("8155 #%x Timer Count Reached\n", n);
...
void
i8155_w(int which, int offset, UINT8 data) {
    i8155_t* i8155 = &chips[which];
    ...
    switch (offset & 0x07) {
        case I8155_REGISTER_COMMAND:
            ...
            if (LOG)
                logerror("8155 #%x Port A Mode: %s\n", which, (data & I8155_COMMAND_PA) ? "output" : "input");
            if (LOG)
                logerror("8155 #%x Port B Mode: %s\n", which, (data & I8155_COMMAND_PB) ? "output" : "input");

            if (LOG)
                logerror("8155 #%x Port A Interrupt: %s\n", which, (data & I8155_COMMAND_IEA) ? "enabled" : "disabled");
            if (LOG)
                logerror("8155 #%x Port B Interrupt: %s\n", which, (data & I8155_COMMAND_IEB) ? "enabled" : "disabled");

            switch (data & I8155_COMMAND_PC_MASK) {
                case I8155_COMMAND_PC_ALT_1:
                    if (LOG)
                        logerror("8155 #%x Port C Mode: Alt 1\n", which);
                    break;

                case I8155_COMMAND_PC_ALT_2:
                    if (LOG)
                        logerror("8155 #%x Port C Mode: Alt 2\n", which);
                    break;

                case I8155_COMMAND_PC_ALT_3:
                    if (LOG)
                        logerror("8155 #%x Port C Mode: Alt 3\n", which);
                    break;

                case I8155_COMMAND_PC_ALT_4:
                    if (LOG)
                        logerror("8155 #%x Port C Mode: Alt 4\n", which);
                    break;
            }
            ...
            switch (data & I8155_COMMAND_TM_MASK) {
                ...
                case I8155_COMMAND_TM_STOP:
                    ...
                    if (LOG)
                        logerror("8155 #%x Timer Command: Stop\n", which);
                    if (i8155->timer)
                        timer_enable(i8155->timer, 0);
                    break;
                ...
                case I8155_COMMAND_TM_STOP_AFTER_TC:
                    ...
                    if (LOG)
                        logerror("8155 #%x Timer Command: Stop after TC\n", which);
                    break;

                case I8155_COMMAND_TM_START:
                    if (LOG)
                        logerror("%04x: 8155 #%x Timer Command: Start\n", activecpu_get_pc(), which);
...
i8155_reset(int which) {
    ...
    i8155_w(which, I8155_REGISTER_COMMAND,
            i8155->command & ~(I8155_COMMAND_PA | I8155_COMMAND_PB | I8155_COMMAND_PC_MASK));
...
void
i8155_init(i8155_interface* intfce) {
    int i;
