...
void
z80ctc_init(z80ctc_interface* intf) {
    int i;
...
double
z80ctc_getperiod(int which, int ch) {
...
static void
z80ctc_interrupt_check(z80ctc* ctc) {
    int state = 0;
    int ch;
...
void
z80ctc_w(int which, int offset, int data) {
    z80ctc* ctc = ctcs + which;
    int mode, ch;
    ...
    if ((mode & CONSTANT) == CONSTANT_LOAD) {
        ...
        if ((mode & MODE) == MODE_TIMER) {
            ...
            if ((mode & TRIGGER) == TRIGGER_AUTO) {
                ...
                if (!(ctc->notimer & (1 << ch)))
                    timer_adjust(ctc->timer[ch], clock * (double)ctc->tconst[ch], (which << 2) + ch,
                                 clock * (double)ctc->tconst[ch]);
                else
...
static void
z80ctc_timercallback(int param) {
    int which = param >> 2;
    int ch = param & 3;
    z80ctc* ctc = ctcs + which;
...
void
z80ctc_trg_w(int which, int trg, int offset, int data) {
    z80ctc* ctc = ctcs + which;
    int ch = trg & 3;
    int mode;
...
static void
z80pio_set_rdy(z80pio* pio, int ch, int state) {
    /* set state */
    pio->rdy[ch] = state;
...
void
z80pio_init(z80pio_interface* intf) {
    int i;
...
static void
z80pio_interrupt_check(z80pio* pio) {
    int state;

    if (pio->int_state[1] & Z80_INT_IEO)
        state = Z80_INT_IEO;
    else
...
static void
z80pio_check_irq(z80pio* pio, int ch) {
    int irq = 0;
    int old_state;
...
void
z80pio_d_w(int which, int ch, int data) {
    z80pio* pio = pios + which;
    if (ch)
        ch = 1;
...
void
z80pio_c_w(int which, int ch, int data) {
    z80pio* pio = pios + which;
    if (ch)
        ch = 1;
...
/* z80pio port write */
void
z80pio_p_w(int which, int ch, int data) {
    z80pio* pio = pios + which;
...
READ_HANDLER(z80pio_0_r) { return (offset & 1) ? z80pio_c_r(0, (offset / 2) & 1) : z80pio_d_r(0, (offset / 2) & 1); }
...
static void
z80pio_update_strobe(int which, int ch, int state) {
    z80pio* pio = pios + which;
...
void
z80pio_astb_w(int which, int state) {
    z80pio_update_strobe(which, 0, state);
}

/* set bstb input */
void
z80pio_bstb_w(int which, int state) {
