...
int
RegisterUnregisterVPinMAME(HWND hWnd, int fRegister) {
    // so now let's see if we can load vpinmame.dll
    HMODULE hModule = LoadLibrary(VPMDLLNAME);
    if (!hModule) {
        DisplayError(hWnd, GetLastError(), "The '" VPMDLLNAME "' file can't be loaded.",
                     "Be sure to have this file in the current directory!");
        return 0;
    }
    ...
    if (fRegister)
        ...
        wsprintf(szMsg,
                 TEXT("You have selected to uninstall version %s of Visual PinMAME! \nAre you ready to proceed?"),
                 szVersion);
    ...
    if (MessageBox(hWnd, szMsg, g_szCaption, MB_ICONQUESTION | MB_YESNO) == IDNO) {
        if (fRegister)
            ...
            MessageBox(hWnd, "UnInstallation aborted!\nVisual PinMAME is still installed on your system", g_szCaption,
    ...
    if (fRegister) {
        ...
        if (!DllRegisterServer) {
            DisplayError(hWnd, GetLastError(),
                         "Can't find the registering function (DllRegisterServer) in the " VPMDLLNAME ".",
                         "Please check if you have a valid vpinmame.dll!");
            FreeLibrary(hModule);
            return 0;
        }
        ...
        if (FAILED(hr)) {
            DisplayError(hWnd, hr, "Unable to register the class object!",
                         "Please check if you have a valid " VPMDLLNAME "!");
            FreeLibrary(hModule);
            return 0;
        }
        FreeLibrary(hModule);
        ...
        if (!DllUnregisterServer) {
            DisplayError(hWnd, GetLastError(),
                         "Can't find the unregistering function (DllUnegisterServer) in the " VPMDLLNAME ".",
                         "Please check if you have a valid vpinmame.dll!");
            FreeLibrary(hModule);
            return 0;
        }
        ...
        if (FAILED(hr)) {
            DisplayError(hWnd, hr, "Unable to unregister the class object!",
                         "Please check if you have a valid " VPMDLLNAME "!");
            FreeLibrary(hModule);
            return 0;
        }
    ...
    if (!fRegister) {
        if (MessageBox(hWnd, "Do you want also delete all registry entrys Visual PinMAME has made?", g_szCaption,
...
int
DisplayDialogs(HWND hWnd, int nDialog) {
    ...
    EXCEPINFO ExcepInfo;
    hr = pDispatch->Invoke(dispid, IID_NULL, LOCALE_SYSTEM_DEFAULT, DISPATCH_METHOD, &DispParams, NULL, &ExcepInfo,
                           NULL);

    if (FAILED(hr)) {
        if (hr == DISP_E_EXCEPTION)
            // actually we call the ONLY UNICODE function in Windows 9x
            MessageBoxW(0, ExcepInfo.bstrDescription, L"Error from the VPinMAME.Controller interface",
                        MB_ICONERROR | MB_OK);
        else
            DisplayError(hWnd, hr, "Can't get the dispatch interface");
...
void
DisplayInstalledVersion(HWND hWnd) {
    ...
    if (strlen(szInstalledVersion) > 0)
        wsprintf(szVersionText, TEXT("* Visual PinMAME Version %s is currently installed on your computer *"),
                 szInstalledVersion);
    else
...
MainDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG:
            ...
            MoveWindow(hWnd, ScreenPos.x, ScreenPos.y, WindowRect.right - WindowRect.left,
            ...
            switch (LOWORD(wParam)) {
                ...
                case IDC_REGISTER:
                    if (RegisterUnregisterVPinMAME(hWnd, true))
                        MessageBox(hWnd, "Visual PinMAME installed successfully!", g_szCaption,
                ...
                case IDC_UNREGISTER:
                    if (RegisterUnregisterVPinMAME(hWnd, false))
                        MessageBox(hWnd,
                                   "Visual PinMAME uninstalled successfully! \nYou may now safely delete your Visual "
                                   "PinMAME Directory!",
                                   g_szCaption, MB_ICONINFORMATION | MB_OK);
                    DisplayInstalledVersion(hWnd);
                    EnabledButtons(hWnd);
                    break;
