...
class CControllerEvents : public _IControllerEvents {
  public:
    // IUnknown
    STDMETHODIMP_(ULONG) AddRef() { return ++m_uRef; }
    ...
    STDMETHODIMP_(long) GetTypeInfo(UINT iInfo, LCID lcid, ITypeInfo** ppTInfo) { return E_NOTIMPL; }
    ...
    STDMETHODIMP_(long)
    Invoke(DISPID dispIdMember, REFIID riid, LCID lcid, WORD wFlags, DISPPARAMS* pDispParams, VARIANT* pVarResult,
  ...
  public:
    CControllerEvents(HWND hWnd, IController* pController) : m_uRef(0) {
        m_hWnd = hWnd;
...
const char*
GetInstalledVersion(char* szVersion, int iSize) {
    if (!szVersion)
        return NULL;
...
/***********************************************************/
BOOL
PopulateListGreaterV1_10(HWND hWnd, IController* pController) {
    ...
    while (SUCCEEDED(pEnumGames->Next(1, &vGame, &uFetched)) && uFetched) {
        ...
        WideCharToMultiByte(CP_ACP, 0, (LPOLESTR)sHelp, -1, pGameInfo->szGameDescription,
                            sizeof pGameInfo->szGameDescription, NULL, NULL);
        lstrcpy(szListEntry, pGameInfo->szGameDescription);
        lstrcat(szListEntry, "\t");

        pGame->get_Name(&sHelp);
        WideCharToMultiByte(CP_ACP, 0, (LPOLESTR)sHelp, -1, pGameInfo->szGameName, sizeof pGameInfo->szGameName, NULL,
                            NULL);
...
/***********************************************************/
BOOL
PopulateList(HWND hWnd, IController* pController) {
...
/***************************************************************************************/
void
RunGame(HWND hWnd, IController* pController) {
...
/***************************************************************************************/
void
CheckRoms(HWND hWnd, IController* pController) {
    LRESULT iIndex = SendDlgItemMessage(hWnd, IDC_GAMESLIST, LB_GETCURSEL, 0, 0);
...
/***************************************************************************************/
void
GameOptions(HWND hWnd, IController* pController) {
    LRESULT iIndex = SendDlgItemMessage(hWnd, IDC_GAMESLIST, LB_GETCURSEL, 0, 0);
...
void
EnableButtons(HWND hWnd, IController* pController) {
...
INT_PTR PASCAL
RunDlgProc(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    switch (uMsg) {
        case WM_INITDIALOG:
            ...
            if (FAILED(hr)) {
                DisplayError(hWnd, hr,
                             "Can't create the Controller class! \nPlease check that you have installed Visual PinMAME "
                             "properly!");
                EndDialog(hWnd, IDCANCEL);
                return FALSE;
            }
            ...
            hr = pController->QueryInterface(IID_IConnectionPointContainer,
                                             (void**)&pControllerConnectionPointContainer);

            if (SUCCEEDED(hr))
                hr = pControllerConnectionPointContainer->FindConnectionPoint(__uuidof(_IControllerEvents),
                                                                              &pControllerConnectionPoint);
