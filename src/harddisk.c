...
#define SET_ERROR_AND_CLEANUP(err)                                                                                     \
    do {                                                                                                               \
        last_error = (err);                                                                                            \
        goto cleanup;                                                                                                  \
    } while (0)
...
INLINE UINT64
offset_from_mapentry(mapentry_t* entry) {
...
INLINE UINT32
length_from_mapentry(mapentry_t* entry) {
    return *entry >> 44;
}
...
/* workaround for some bug in codewarrior pro 6.3 compiler */
INLINE void
encode_mapentry(mapentry_t* entry, UINT64 offset, UINT32 length) {
...
INLINE void
byteswap_mapentry(mapentry_t* entry) {
#ifdef LSB_FIRST
...
INLINE UINT32
get_bigendian_uint32(UINT8* base) {
    return (base[0] << 24) | (base[1] << 16) | (base[2] << 8) | base[3];
}

INLINE void
put_bigendian_uint32(UINT8* base, UINT32 value) {
    base[0] = value >> 24;
    base[1] = value >> 16;
    base[2] = value >> 8;
    base[3] = value;
...
void
hard_disk_set_interface(struct hard_disk_interface* new_interface) {
    if (new_interface)
...
void
hard_disk_save_interface(struct hard_disk_interface* interface_save) {
...
int
hard_disk_create(const char* filename, const struct hard_disk_header* _header) {
...
void*
hard_disk_open(const char* filename, int writeable, void* parent) {
    ...
    if (parent
        && (memcmp(info.parent->header.md5, info.header.parentmd5, 16) || !(info.header.flags & HDFLAGS_HAS_PARENT)))
...
UINT32
hard_disk_read(void* disk, UINT32 lbasector, UINT32 numsectors, void* buffer) {
    struct hard_disk_info* info = disk;
    UINT32 block, offset;
    int err;
...
UINT32
hard_disk_write(void* disk, UINT32 lbasector, UINT32 numsectors, const void* buffer) {
    struct hard_disk_info* info = disk;
...
const struct hard_disk_header*
hard_disk_get_header(void* disk) {
...
int
hard_disk_set_header(const char* filename, const struct hard_disk_header* header) {
    void* file = NULL;
    int err;
...
int
hard_disk_binary_search(struct hard_disk_block* blocks, UINT8 hash[16], int low, int high) {
    high--;
    while (low <= high) {
        int mid = (low + high) >> 1;
...
int
hard_disk_compare_hash(const UINT8 one[16], const UINT8 two[16]) {
    int i;
    for (i = 0; i < 16; i++) {
        if (one[i] != two[i])
            return two[i] - one[i];
...
int
hard_disk_compress(const char* rawfile, UINT32 offset, const char* newfile, const struct hard_disk_header* header,
                   const char* difffile, void (*progress)(const char*, ...)) {
    ...
    while (block < readbackheader->totalblocks) {
        ...
        if (curtime - lastupdate > CLOCKS_PER_SEC / 2) {
            ...
            if (progress && sourcepos)
                (*progress)("Compressing sector %d/%d... (ratio=%d%%)  \r", block * destfile->header.blocksize,
                            totalsectors, 100 - destfile->eof * 100 / sourcepos);
            lastupdate = curtime;
        }
        ...
        if ((header->flags & HDFLAGS_IS_WRITEABLE) == 0) {
            ...
            if (duplicate_block < 0) {
                ...
                /* but this works */
                for (i = last_crc - 1; i >= duplicate_block; i--)
                    unique_blocks[i + 1] = unique_blocks[i];
            ...
            /* else check to see if it is a good block */
                ...
                do {
                    ...
                    memset(temp_cache, 0, blocksizebytes);
                    (*_interface.read)(sourcefile, unique_blocks[duplicate_block].ptr * blocksizebytes, blocksizebytes,
                                       temp_cache);
                    if (memcmp(temp_cache, destfile->cache, blocksizebytes) == 0)
                        current_is_good = 1;
                    else
                        duplicate_block++;
                } while (current_is_good == 0
                         && memcmp(unique_blocks[duplicate_block].hash, unique_blocks[duplicate_block + 1].hash, 16)
                                == 0
                         && duplicate_block < last_crc);
            ...
            if (block != unique_blocks[duplicate_block].ptr) {
                ...
                bytes = (*_interface.write)(destfile->file, destfile->header.length + block * sizeof(destfile->map[0]),
                                            sizeof(entry), &entry);
        ...
        if (comparefile) {
            if (read_block_into_cache(comparefile, block) == HDERR_NONE
                && memcmp(comparefile->cache, destfile->cache, blocksizebytes) == 0)
                write_this_block = 0;
        }
...
int
hard_disk_verify(const char* hdfile, void (*progress)(const char*, ...), UINT8 headermd5[16], UINT8 actualmd5[16]) {
...
static int
read_block_into_cache(struct hard_disk_info* info, UINT32 block) {
...
static int
write_block_from_cache(struct hard_disk_info* info, UINT32 block) {
    UINT64 fileoffset = offset_from_mapentry(&info->map[block]);
    UINT32 length = length_from_mapentry(&info->map[block]);
...
static int
read_header(void* file, struct hard_disk_header* header) {
...
static int
write_header(void* file, const struct hard_disk_header* header) {
    UINT8 rawheader[HARD_DISK_MAX_HEADER_SIZE];
    UINT32 length;
    UINT32 count;
...
static int
read_sector_map(struct hard_disk_info* info) {
    mapentry_t cookie;
    int i, err;
    UINT32 count;
    ...
    count =
        (*_interface.read)(info->file, info->header.length, sizeof(info->map[0]) * info->header.totalblocks, info->map);
    ...
    /* verify the cookie */
    count = (*_interface.read)(info->file, info->header.length + sizeof(info->map[0]) * info->header.totalblocks,
...
static voidpf
fast_alloc(voidpf opaque, uInt items, uInt size) {
...
static void
fast_free(voidpf opaque, voidpf address) {
    struct zlib_codec_data* data = opaque;
...
static int
init_codec(struct hard_disk_info* info) {
    int err = HDERR_NONE;
...
static void
free_codec(struct hard_disk_info* info) {
    /* now decompress based on the compression method */
    switch (info->header.compression) {
        case HDCOMPRESSION_NONE:
            /* nothing to do */
