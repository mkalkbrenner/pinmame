...
// Fill in a dot.
void
procDrawDot(int x, int y, int color) {
    uint8_t temp_dot = 0;
    uint8_t position = 0;
...
void
procFillDMDSubFrame(int frameIndex, UINT8* dotData, int length) {
    memcpy(procdmd[frameIndex], dotData, length);
}

void
procReverseSubFrameBytes(int frameIndex) {
    int i;
    uint8_t byte, new_byte;
    for (i = 0; i < 0x200; i++) {
        byte = procdmd[frameIndex][i];
        new_byte = ((byte & 0x1) << 7) | ((byte & 0x2) << 5) | ((byte & 0x4) << 3) | ((byte & 0x8) << 1)
...
void
procDisplayText(char* string_1, char* string_2) {
    // Start at ASCII table offset 32: ' '
    const UINT16 asciiSegments[] = {
        0x0000, // ' '
        0x0000, // '!'
        0x0000, // '"'
        0x0000, // '#'
    ...
    for (i = 0; i < 16; i++) {
        ...
        if (machineType != kPRMachineWPCAlphanumeric) {
            for (j = 0; j < 16; j++) {
                if ((segs_a[i] >> j) & 0x1)
                    procDrawSegment(i * 8, 3, j);
                if ((segs_b[i] >> j) & 0x1)
                    procDrawSegment(i * 8, 19, j);
            }
        }
    }
...
void
procUpdateAlphaDisplay(UINT16* top, UINT16* bottom) {
