...
void
set_swState(int value, int type) {
    int row, col;
...
void
ConfigureWPCFlipperSwitchRule(int swNum, PRCoilList coilList[], int coilCount) {
    int i;
...
void
ConfigureWPCFlipperSwitchRule(int swNum, int mainCoilNum, int holdCoilNum, int pulseTime) {
    PRCoilList coils[2];
...
void
ConfigureSternFlipperSwitchRule(int swNum, int mainCoilNum, int pulseTime, int patterOnTime, int patterOffTime) {
...
void
ConfigureBumperRule(int swNum, int coilNum, int pulseTime) {
    const int numDriverRules = 1;
    PRDriverState fldrivers[numDriverRules];
    PRSwitchRule sw;
...
void
ConfigureKickerRule(int lampNum, int swNum, int coilNum, int pulseTime) {
    const int numDriverRules = 1;
    PRDriverState fldrivers[numDriverRules];
    PRSwitchRule sw;
    PRDriverGetState(proc, coilNum, &fldrivers[0]);
...
void
ClearSwitchRule(int swNum, int coilNum) {
    PRSwitchRule sw;
...
procKickbackCheck(int num) {
    ...
    if (cyclesSinceTransition[num] == 0) {
        ...
        if (mame_debug)
            fprintf(stderr, "Transition count is zero, need to check P-ROC\n");
        if (mame_debug)
            fprintf(stderr, "P-ROC reports that lamp state is %s\n", lampState.state ? "on" : "off");
    ...
    if (cyclesSinceTransition[num] == kickbackOffDelay[num]) {
        if (mame_debug)
            fprintf(stderr, "Disable kickback\n");
    ...
    // If we've reached the cycle count required to turn the related kickback on
        ...
        if (mame_debug)
            fprintf(stderr, "Enable kickback with pulse time %d\n", pulseTime);
...
void
AddIgnoreCoil(int num) {
    if (mame_debug)
        fprintf(stderr, "Ignoring drives to coil %i\n", num);
...
void
procConfigureDriverDefaults(void) {
    if (yamlDoc.size() > 0) {
        // WPC Flippers
        std::string numStr;
        const YAML::Node& coils = yamlDoc[kCoilsSection];
...
void
procConfigureFlipperSwitchRules_default(int enabled) {
    if (yamlDoc.size() > 0 && yamlDoc[kFlippersSection]) {
        ...
        for (YAML::const_iterator flippersIt = flippers.begin(); flippersIt != flippers.end(); ++flippersIt) {
            ...
            if (machineType == kPRMachineWPC || machineType == kPRMachineWPC95) {
                ...
                ConfigureSternFlipperSwitchRule(swNum, coilMain, kFlipperPulseTime, kFlipperPatterOnTime,
                                                kFlipperPatterOffTime);
                AddIgnoreCoil(coilMain);
            }
        }
    }
...
void
procFullTroughDisablesFlippers(void) {
    ...
    if (troughCount == -1 && yamlDoc.size() > 0 && yamlDoc[kSwitchesSection]) {
        ...
        if (procGetYamlPinmameSettingInt("fullTroughDisablesFlippers", 0)) {
            ...
            for (YAML::const_iterator switchesIt = switches.begin(); switchesIt != switches.end(); ++switchesIt) {
                ...
                if (strncmp("trough", switchName.c_str(), 6) == 0 && isdigit(switchName.c_str()[6])) {
...
void
procConfigureSwitchRules(int enabled) {
    std::string numStr;
    ...
    if (yamlDoc.size() > 0) {
        ...
        if (const YAML::Node& kickbacks = yamlDoc[kKickbacksSection]) {
            for (YAML::const_iterator kickbacksIt = kickbacks.begin(); kickbacksIt != kickbacks.end(); ++kickbacksIt) {
                int swNum = 0, coilNum = 0, lampNum = 0, delayOnTime = kKickbackDelayOnTime,
                    delayOffTime = kKickbackDelayOffTime;
...
procSetSwitchStates(void) {
    ...
    if (PRSwitchGetStates(proc, procSwitchStates, kPRSwitchPhysicalLast + 1) == kPRFailure) {
        ...
        if (mame_debug)
            fprintf(stderr, "Initial switch states:");
...
void
procConfigureDefaultSwitchRules(void) {
    int ii;
    ...
    for (ii = 0; ii <= kPRSwitchPhysicalLast; ii++) {
        PRSwitchRule swRule;
        if ((machineType == kPRMachineWPC || machineType == kPRMachineWPC95) && !((ii < 32) || ((ii % 16) < 8))) {
            swRule.notifyHost = 0;
        } else {
            swRule.notifyHost = 1;
        }
...
void
procDriveLamp(int num, int state) {
    PRDriverState lampState;
    memset(&lampState, 0, sizeof(lampState));
    PRDriverGetState(proc, num, &lampState);
    ...
    if (isArduino) {
        ...
        if (lamp_RGB_Equiv[num]) {
            ...
            if (state == 0)
                ...
                sched = 255; // Schedule is all on (255) or all off (0))
            cmd[2] = (char)sched;
            cmd[3] = (char)sched;
            cmd[4] = (char)sched;
            cmd[5] = (char)sched;
...
void
CoilDriver::SetNum(int number) {
    num = number;
}

void
CoilDriver::SetPulseTime(int ms) {
    pulseTime = ms;
    useDefaultPulseTime = 0;
}

void
CoilDriver::SetPatterTimes(int msOn, int msOff) {
    patterOnTime = msOn;
    patterOffTime = msOff;
...
void
CoilDriver::SetPatterDetectionEnable(int enable) {
    patterDetectionEnable = 0;
}

int
CoilDriver::GetPulseTime() {
    return pulseTime;
}

int
CoilDriver::GetPatterDetectionEnable() {
    return patterDetectionEnable;
}

void
CoilDriver::ResetPatter(void) {
    numPatterOn = 0;
    numPatterOff = 0;
    avgOnTime = 0;
    avgOffTime = 0;
...
void
CoilDriver::CheckEndPatter(void) {
    if (patterActive) {
...
void
CoilDriver::Patter(int msOn, int msOff) {
    long int msTime = clock() / CLOCKS_PER_MS;
...
void
CoilDriver::RequestDrive(int state) {
...
void
procDriveCoilDirect(int num, int state) {
    coilDrivers[num].RequestDrive(state);
}

void
procDriveCoil(int num, int state) {
    if (mame_debug) {
...
void
procCheckActiveCoilsUnused(void) {
...
void
procInitializeCoilDrivers(void) {
    int i;
    count = 0;
    for (i = 0; i < 256; i++) {
...
osd_is_proc_pressed(int code) {
    ...
    switch (code) {
        ...
        case kESQSequence:
            retcode = ((osd_is_proc_pressed(kFlipperLwL) && osd_is_proc_pressed(kFlipperLwR)
