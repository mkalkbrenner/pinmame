...
void
proc_shiftRegInit(int procClockCoil, int procDataCoil) {
...
int
proc_shiftRegIsEmpty(void) {
    return shr.bit == 0;
}

int
proc_shiftRegDequeue(void) {
    int retval = -1;
    if (shr.bit > 0) {
        retval = shr.queue & 1;
        shr.queue >>= 1;
...
void
proc_shiftRegCheckQueue(int dummy) {
    int data;
